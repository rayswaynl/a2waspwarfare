...
4. **`python -m pytest Tools/Lint -q`**: **476 passed, 8 failed** (final, after the stale-test-assertion fix below). Compared directly against a clean `origin/master` baseline worktree (no rollup changes): baseline is **433 passed, 8 failed**. The remaining 8 failures are exactly the baseline set — unchanged/pre-existing, unrelated WIP tests for features not in this rollup at all (`test_aicap_midhigh_trim`, `test_cargo_airdrop_stage_a/b`, `test_handlespecial_cases`, `test_overrun_razer_reachability`, `test_territorial_hud_contract`, `test_veh_delete_probe` x2).

   Two tests briefly failed after Phase B's merges (#1493, #1507) because they asserted the OLD buggy behavior those PRs correctly fixed. Per a follow-up instruction, both were updated with minimal edits (assertion values/comments only, no test-logic rewrites), commit "staging: update 2 stale test assertions to the fixed behavior (#1493 backslash path, #1507 pilot fallback)":
   - `test_realplayersnear_vetoes.py` now asserts the backslash compile path (`Common\Functions\Common_RealPlayersNear.sqf`) PR #1493 shipped — forward slashes break `preprocessFileLineNumbers` inside a PBO, the backslash IS the fix.
   - `test_naval_cap_pilot_fallback.py` now asserts the `"GUE_Soldier_Pilot"` fallback classname PR #1507 shipped, replacing the old generic `"GUE_Soldier"` expectation.
...
6. **Commit count**: `git log --oneline origin/master..HEAD` = **106 commits** ahead of `origin/master` (includes all individual PR-branch commits pulled in by each merge, not just this rollup's own merge/follow-up commits; was 104 before the test-assertion fix, 90 before Phase G).
