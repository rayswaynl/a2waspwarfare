...
The `Claude Notes` table in the report is part of the gate, not optional commentary. Mark each runtime check `PASS`, `FAIL`, or `UNCERTAIN`; every `PASS` row must include concrete row-specific evidence and an explicit keep/tune/revert/investigate/patch/retest recommendation, and every `FAIL` or `UNCERTAIN` row should include coordinates, screenshot filenames, RPT excerpts, or repeatable repro steps. Hosted boot and dedicated boot must be proven in separate rows with matching RPT/init evidence. Spatial `PASS` rows need coordinates or screenshot filenames; balance/init/feature `PASS` rows need RPT excerpts or runtime values. When `-EvidenceRoot` is used, key visual rows must include screenshot filenames: population placement, base sightlines/statics, wall origin/gaps, town-defense blocking, and priority defense arcs. The population/SP-SV placement row must compare the map audit Population Flow/value tiers against runtime screenshots or coordinates for the city, district/market belt, airfield, farms, outskirts, camps, and any town center that feels misplaced. Put referenced screenshots and `zargabad-map-audit.md` in the same evidence folder as the report, or pass that folder with `-EvidenceRoot` so Codex can verify the files exist, the images have PNG/JPEG signatures, and the map-audit packet contains the Population Flow, Rim Test Points, WDDM Fortification Review, and Claude Screenshot Targets sections. Codex should listen to that evidence: if Claude proves the mission is wrong, Codex updates mission code or validators before asking Claude to repeat the same pass.

The report's `Objective Coverage` table is the final mission-goal checklist. Every row from `Guides/Zargabad-Completion-Gates.md` must be present, `PASS`, backed by current static/runtime evidence, and include a keep/tune/revert/investigate/patch/retest Codex action recommendation before Claude can recommend stop or Codex can say stop.

Before Codex makes a stop/go call on a filled runtime report, run the report validator against Claude's edited markdown. It fails if expected gate or failure-scan rows are missing, required gates are still `MISSING`, the failure scan contains `FOUND`, key evidence placeholders remain, any expected Claude Notes row is missing, any Claude Notes row is not `PASS`, any `PASS` row has empty/generic/non-specific evidence, any `PASS` row omits a keep/tune/revert/investigate/patch/retest recommendation, or any key visual row omits a screenshot filename when `-EvidenceRoot` is supplied:
...
| Population/SP-SV placement feel | Use map audit Population Flow/value tiers and screenshots/coords to confirm city/district/market/airfield are the main population/economy belt and farms/outskirts remain lower-value routes; flag any town center/camp/SP-SV value that feels misplaced. |
...
- Population/SP-SV placement notes with screenshot filenames or coordinates for city, airfield, district/market belt, and low-density farm/outskirt routes.
