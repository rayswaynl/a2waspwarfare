...
The current layout keeps the highest SV objectives in the city, markets, north/south districts and airfield while using farms/outskirts as lower-value flank objectives. Static validation now locks the named placement matrix below: each objective must keep its intended coordinate, start SV, max SV and capture range, so future edits cannot silently drift the population/value flow.
