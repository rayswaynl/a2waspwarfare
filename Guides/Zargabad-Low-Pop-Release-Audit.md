...
`Tools/New-ZargabadRuntimeReport.ps1` wraps the runtime validator and emits a compact markdown gate snapshot, failure scan, key RPT excerpts, and Claude note prompts. Its Claude Notes table now separates hosted boot context from dedicated boot context and has dedicated base-static-position and priority-defense-mix rows that point testers to actual spawned base static coordinates/facing plus the city, airfield, North/South District, Northwest Base and Rahim Villa defense rows from the map audit packet. Claude should paste this report back to Codex after each hosted/dedicated/JIP/HC pass so runtime findings are frequent, evidence-backed, and easy to act on.
