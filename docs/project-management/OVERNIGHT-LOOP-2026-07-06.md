...
- 05:0x — Iter 8: SOAK HARVEST #2 (t=125, ~2h). Build STILL HEALTHY: fps=42, DELEG remotePct=91%, errors 39/38 (~16/hr steady background, no flood), grpNull=0, build=cmdcon44uaicom, box empty. DECAP 248 lines, ALL IDLE (inRange=0 sensed=0). ⚠️ OBSERVATION for report: AI-vs-AI war is BALANCED+SLOW — 42/46 towns still neutral after 2h, both sides 1-3 towns near base. Neither side nears dominance → live sense/roll/commit chain will NOT fire this session (needs late-game team-within-3km-of-HQ). Soak VALIDATES shadow-contract + #724 regression health; live sensing-chain validation needs longer/asymmetric soak OR targeted scenario (recommend for report). Note: cc44u is a TEST build on the LIVE box, flag-0 byte-identical to prod behavior + extra shadow telemetry = safe for morning players; owner can keep (strict telemetry improvement) or roll back to prior official build.
