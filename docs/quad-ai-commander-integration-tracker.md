...
| #19 | `codex/ai-commander-context` | Phase 2 context/belief scaffold stacked on `codex/ai-commander-logs` | Open draft; advisory-only; inherited hot-path content refreshed onto #18 blobs; graph still diverged and must refresh after #18 graph refresh; synthetic/runtime evidence pending |
...
| 2 Context/beliefs | `codex/ai-commander-context` | Phase 1 logs | Draft PR #19 open; advisory-only scaffold with manual smoke helper; content refreshed onto #18 hot-path blobs; graph refresh and synthetic/runtime evidence pending | no |
...
| `docs/quad-ai-commander-runtime-report-template.md` | Paste-ready PR/runtime report template for recording RPT excerpts, manual observations, PASS/FAIL/UNCERTAIN rows, and downstream decisions |
...
4. Graph-refresh/rebase PR #19 onto the refreshed #18 head.
...
current stack state: content refreshed onto #14 hardening; graph refresh/rebase still needed before runtime smoke; compare may show 14 ahead / 5 behind
```
...
current stack state: content refreshed onto refreshed #18 hot-path blobs; graph refresh/rebase still needed after #18 graph refresh; compare may show 20 ahead / 6 behind
```

Current Phase 2 implementation surface:

- `AI_Commander.sqf` compiles Phase 2 context helpers and runs guarded context update every 30 seconds while active.
- `AI_Commander_ContextUpdate.sqf` drains `CONTACT`, `INTEL`, and `LOSS` records into side-logic context.
- `AI_Commander_BeliefMerge.sqf` merges nearby/category-compatible beliefs and caps confidence.
- `AI_Commander_BeliefDecay.sqf` decays and expires stale beliefs.
- `AI_Commander_ContextDebug.sqf` emits slow summary-only context output.
- `AI_Commander_ContextSyntheticSmoke.sqf` appends synthetic records and immediately forces one context update/debug pass.
- Inherited `AI_Commander_AssignTowns.sqf`, `AI_Commander_AssignTypes.sqf`, `AI_Commander_Execute.sqf`, and `AI_Commander_Produce.sqf` now match #18 content; they still do not read `wfbe_aicom_context`.
...
| Phase 2 belief merge/decay excerpts | Phase 2 | draft implementation open in PR #19 with manual smoke helper; content refreshed but graph refresh and synthetic/runtime evidence pending |
