...
As of 2026-07-02 after the PR #125 Build83/cmdcon35, AICOM latch-reset, release-doc package-identity refresh, attack-wave authority follow-ups, ICBM RequestSpecial authority hardening, supply mission PV guards, SCUD authority binding, GUER mortar authority guard, player support/UAV authority binding, `fa2e4019d9` package refresh, and follow-up head check:
...
- PR #125, `[codex] Prepare WASP release command center`, branch `codex/release-command-center-20260630`: broad release command-center candidate with tooling, package provenance, AICOM work, and runtime collection gates. GitHub currently reports the branch at `fa2e4019d9e7d81ccfffcb16a67f17d1746c6bc8`; the recorded package checkpoint matches that head, marker `fa2e4019d9`, `_MISSIONS.7z` SHA256 `E887E7920AAE620A7DFCB20FFD17FFAB5F16DF8B1F40D58E73173DB9CD236B77`, `1,885` entries, `7,166,244` bytes, handoff status `ready_for_runtime_collection`, and public wiki commit `6d0dfa8`. Exact Chernarus and Takistan runtime RPT proof is still pending, and GitHub reports PR #125 open, draft, clean, and mergeable against `master`.
...
| `rayswaynl/a2waspwarfare.wiki` | Public remote master is at wiki commit `6d0dfa8`, which records the current PR #125 package tuple for `fa2e4019d9`, marker-sweep artifact output, PR #126 fallback-template guard, the runtime-evidence manifest generator/checker with role/terrain stamps and optional package artifact hashing, and clarifies that PR #126 HC-audit markers are required only for packages that include that instrumentation. The wiki still treats runtime/deploy evidence as pending. Wiki commit hashes are not release identities; use the package/git/archive tuple and RPT markers for release proof. | Wiki freshness and release-doc synchronization. |
...
9. Existing release PRs already agree on the main gate: source/static/package checks are not enough.
   ...
   - PR #125 now has package/handoff proof for `fa2e4019d9` / `_MISSIONS.7z` SHA256 `E887E7920AAE620A7DFCB20FFD17FFAB5F16DF8B1F40D58E73173DB9CD236B77`, but that is still package/tooling proof, not final runtime proof. PR #126 and PR #131 artifacts remain companion/tooling proof unless explicitly promoted into the active package lane.
...
11. Public stats and server-side stats are health context, not release proof.
   ...
   - This is encouraging live health evidence only. It does not bind to PR #125 package marker `fa2e4019d9`, archive SHA `E887E7920AAE620A7DFCB20FFD17FFAB5F16DF8B1F40D58E73173DB9CD236B77`, the ten required role RPTs, or the runtime packet manifest.
...
17. PR #125 source and package proof are refreshed to the same head.
   - The current package checkpoint commit is `fa2e4019d9e7d81ccfffcb16a67f17d1746c6bc8`.
   - The PR/wiki/package tuple records `_MISSIONS.7z` built from marker `fa2e4019d9`, SHA256 `E887E7920AAE620A7DFCB20FFD17FFAB5F16DF8B1F40D58E73173DB9CD236B77`, `1,885` entries, `7,166,244` bytes, and handoff status `ready_for_runtime_collection`.
   - Validation recorded on PR #125 includes LoadoutManager RELEASE rebuild, package gates, release handoff generation, wiki parse/diff checks, static smoke, final-check package-worktree hygiene, Chernarus/Takistan OA lint pass, high-only BugHunt clean, the HC AICOM delegation PVF follow-up in both maintained `Client_HandlePVF.sqf` files, FPS report PV payload guards, marker-feed consumer guards, SCUD authority binding, GUER mortar authority guard, player support/UAV requester/team/destination/upgrade/cooldown/funds binding, and marker-sweep defaults for `B74.2 REQ-MARK` plus `aiStatus=`.
   - Action: collect runtime evidence only against the `fa2e4019d9` / `E887E792...` tuple unless the release owner explicitly promotes a newer package.

18. PR #126 now has a tuple guard for this active package checkpoint.
   - `Tools/Ops/Test-WaspReleaseTupleDocs.ps1` checks the PR #126 release ledger and monitor examples for current PR #125 branch head `fa2e4019`, expected package git marker `fa2e4019d9`, archive SHA `E887E7920AAE620A7DFCB20FFD17FFAB5F16DF8B1F40D58E73173DB9CD236B77`, package size `7,166,244`, wiki commit `6d0dfa8`, runtime-pending wording, and absence of stale `190475B4...`, `1C58A4A3...`, `3DB01AC...`, `703F71E6...`, `40883616...`, `50AD...`, `F057...`, `D323...`, `35B864...`, `D5CA434E...`, and `ECC6F9D5...` active-surface archive hashes.
