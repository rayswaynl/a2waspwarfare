...
- PR #125, `[codex] Prepare WASP release command center`, branch `codex/release-command-center-20260630`: broad release command-center candidate with tooling, package provenance, AICOM work, and runtime collection gates. Current source/package head observed: `d77765f82bde9a3a07ff9c321bc9804007dffa8e` / marker `d77765f82b`. The latest mission deltas reconcile Build83/cmdcon35, keep the uncompiled AICOM HC top-up draft worker/wiring excluded, preserve AICOM/PVF authority hardening, and replace stale commander handoff `wfbe_exec_sig` resets with the active `wfbe_exec_lastmode` / `wfbe_exec_lastgoto` / `wfbe_exec_at` latch reset in both maintained terrains; the latest branch head applies the focused placement-preview static gate fix. PR #125 package/handoff artifacts now record `_MISSIONS.7z` SHA256 `AB37F7112B67AFF1C84986894BDFCA8C1215FE8A98475A79A82BFEF51882F5AF`, `1,881` entries, `7,158,858` bytes, handoff status `ready_for_runtime_collection`; public wiki refresh is still pending from the previous `1d713bcf2b` tuple. This proves package identity only; exact Chernarus and Takistan runtime RPT proof is still pending, and GitHub reports PR #125 open, draft, clean, and mergeable against `master`.
...
| `rayswaynl/a2waspwarfare.wiki` | Cloned locally and synced through wiki commit `4957bfb`, which still records the prior PR #125 package tuple for `1d713bcf2b`. The current `d77765f82b` package tuple is proven locally and recorded in PR #125/PR #126, but public wiki refresh remains pending. The wiki still treats runtime/deploy evidence as pending. Wiki commit hashes are not release identities; use the package/git/archive tuple and RPT markers for release proof. | Wiki freshness and release-doc synchronization. |
...
9. Existing release PRs already agree on the main gate: source/static/package checks are not enough.
   ...
   - PR #125 now has package/handoff proof for `d77765f82b` / `_MISSIONS.7z` SHA256 `AB37F7112B67AFF1C84986894BDFCA8C1215FE8A98475A79A82BFEF51882F5AF`, but that is still package/tooling proof, not final runtime proof. PR #126 and PR #131 artifacts remain companion/tooling proof unless explicitly promoted into the active package lane.
...
12. Latest RPT candidates exist and were marker-swept without copying private logs.
   ...
   - Action: these live logs confirm useful pre-instrumentation AICOM/HC health, but cannot satisfy the exact-build HC-drop audit gate for the current PR #125 package `AB37F7112B67AFF1C84986894BDFCA8C1215FE8A98475A79A82BFEF51882F5AF` or the PR #126 companion instrumentation lane. Added `Tools/Monitor/Get-WaspRptMarkerSweep.ps1` so future livehost/archive checks can repeat this marker sweep with path hashes and line hashes instead of copied logs or raw lines. Added `Tools/Monitor/Test-WaspRptMarkerSweep.SelfTest.ps1`; synthetic validation passes for required-marker success, missing-marker `-NoFail`, missing-marker hard fail, comma-separated required markers, default no-raw-line samples, and exact Chernarus/Takistan release marker generation via `-ExpectedCandidate`, `-ExpectedGit`, and `-RequireReleaseMarkers`. Only copy/analyze broader private RPT contents after the owner confirms target build/window and privacy scope.

13. Release marker fallback must never look like a real release identity.
   - The active PR #125 package contains generated `version.sqf` files with exact `d77765f82b` markers, but the tracked fallback/template marker strings still used the older `build83-cmdcon35` identity if generation was skipped.
...
- Use `Tools/Monitor/Get-WaspRptMarkerSweep.ps1` with `-ExpectedCandidate release-command-center-20260630 -ExpectedGit d77765f82b -RequireReleaseMarkers` for redaction-safe first-pass livehost/archive RPT marker counts before deciding whether any private RPT copies are needed.
