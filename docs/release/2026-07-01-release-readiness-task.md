...
- PR #125, `[codex] Prepare WASP release command center`, branch `codex/release-command-center-20260630`: broad release command-center candidate with tooling, package provenance, AICOM work, and runtime collection gates. GitHub currently reports the branch at `e2b26027a63b8606dd14fdc248e6cdd3def5f6bd`; the recorded package checkpoint matches that head, marker `e2b26027a6`, `_MISSIONS.7z` SHA256 `3E98F3B65C3D1FA4EB04F02D752EB6484DCDA94411BEAAB3F18E22C77E36678A`, `1,885` entries, `7,167,199` bytes, handoff status `ready_for_runtime_collection`, and public wiki commit `f593b34`. Exact Chernarus and Takistan runtime RPT proof is still pending, and GitHub reports PR #125 open, draft, clean, and mergeable against `master`.
...
| `rayswaynl/a2waspwarfare.wiki` | Public remote master is at wiki commit `f593b34`, which records the current PR #125 package tuple for `e2b26027a6`, marker-sweep artifact output, PR #126 fallback-template guard, the runtime-evidence manifest generator/checker with role/terrain stamps and optional package artifact hashing, and clarifies that PR #126 HC-audit markers are required only for packages that include that instrumentation. The wiki still treats runtime/deploy evidence as pending. Wiki commit hashes are not release identities; use the package/git/archive tuple and RPT markers for release proof. | Wiki freshness and release-doc synchronization. |
...
11. Public stats and server-side stats are health context, not release proof.
   ...
   - This is encouraging live health evidence only. It does not bind to PR #125 package marker `e2b26027a6`, archive SHA `3E98F3B65C3D1FA4EB04F02D752EB6484DCDA94411BEAAB3F18E22C77E36678A`, the ten required role RPTs, or the runtime packet manifest.
...
17. PR #125 source and package proof are refreshed to the same head.
   - The current package checkpoint commit is `e2b26027a63b8606dd14fdc248e6cdd3def5f6bd`.
   - The PR/wiki/package tuple records `_MISSIONS.7z` built from marker `e2b26027a6`, SHA256 `3E98F3B65C3D1FA4EB04F02D752EB6484DCDA94411BEAAB3F18E22C77E36678A`, `1,885` entries, `7,167,199` bytes, and handoff status `ready_for_runtime_collection`.
   ...
   - Action: collect runtime evidence only against the `e2b26027a6` / `3E98F3B6...` tuple unless the release owner explicitly promotes a newer package.

18. PR #126 now has a tuple guard for this active package checkpoint.
   - `Tools/Ops/Test-WaspReleaseTupleDocs.ps1` checks the PR #126 release ledger and monitor examples for current PR #125 branch head `e2b26027`, expected package git marker `e2b26027a6`, archive SHA `3E98F3B65C3D1FA4EB04F02D752EB6484DCDA94411BEAAB3F18E22C77E36678A`, package size `7,167,199`, wiki commit `f593b34`, runtime-pending wording, and absence of stale `190475B4...`, `1C58A4A3...`, `3DB01AC...`, `703F71E6...`, `40883616...`, `50AD...`, `F057...`, `D323...`, `76A5EE56...`, `E887E792...`, `8D06CDA3...`, `35B864...`, `D5CA434E...`, `ECC6F9D5...`, `F4F19086...`, `0AC507E1...`, `22F5AEE4...`, `E7B12EDA...`, `FB52623D...`, `35280646...`, and `F5232AE7...` active-surface archive hashes.
...
20. PR #125 branch head and package proof realigned after the SCUD, GUER mortar, player support/UAV, GUER result receiver, repair, GUER support, commander-support and AI focus follow-ups.
   - GitHub now reports PR #125 at `e2b26027a63b8606dd14fdc248e6cdd3def5f6bd`.
   - PR #125 now records package proof for ICBM `RequestSpecial` authority/payment hardening, supply mission PV guards for start, completion and active-town query payloads, SCUD requester/team/action-grant/pad-proximity binding, GUER mortar requester/resistance-driver/vehicle/range binding plus nested client result unwrapping, player support/UAV requester/team/destination/upgrade/cooldown/funds binding, supply-truck `RespawnST` commander/team validation, side-player `aicom-focus` AI-run validation, camp repair server-side price/debit/actor validation, GUER VBIED object/class guards, and GUER FOB requester/truck/build-range/class guards.
   ...
   - Impact: runtime proof should target the refreshed `e2b26027a6` / `3E98F3B6...` package tuple; older `062ab44ff5`, `78ba49d540`, `cf4e93af1a`, `4893faaa5d`, `9e628781a8`, and stale package hashes including `22F5AEE4...`, `FB52623D...`, `35280646...`, and `F5232AE7...` are now stale unless the release owner explicitly chooses them.
