...
As of 2026-07-01 after the PR #125 Build83/cmdcon35 and AICOM latch-reset package refresh:
...
- PR #125, `[codex] Prepare WASP release command center`, branch `codex/release-command-center-20260630`: broad release command-center candidate with tooling, package provenance, AICOM work, and runtime collection gates. Current source/package head observed: `b4628c35afeef15a1703021e6171706a949e5afa` / marker `b4628c35af`. The latest deltas reconcile Build83/cmdcon35, keep the uncompiled AICOM HC top-up draft worker/wiring excluded, preserve AICOM/PVF authority hardening, and replace stale commander handoff `wfbe_exec_sig` resets with the active `wfbe_exec_lastmode` / `wfbe_exec_lastgoto` / `wfbe_exec_at` latch reset in both maintained terrains. PR #125 and the public wiki now record `_MISSIONS.7z` SHA256 `FA7E07C6D8A02C0574E619C54BB61FC0D6244D3559F2C15428B3BEADD9169A54`, `1,881` entries, `7,158,447` bytes, handoff status `ready_for_runtime_collection`. This proves package identity only; exact Chernarus and Takistan runtime RPT proof is still pending, and GitHub reports PR #125 open, draft, clean, and mergeable against `master`.
...
| `rayswaynl/a2waspwarfare.wiki` | Cloned locally and synced through wiki commit `f6cdfce`, which records the PR #125 package tuple for `b4628c35af`: `_MISSIONS.7z` SHA256 `FA7E07C6D8A02C0574E619C54BB61FC0D6244D3559F2C15428B3BEADD9169A54`, `1,881` entries, `7,158,447` bytes, handoff `ready_for_runtime_collection`. The wiki still treats runtime/deploy evidence as pending. Wiki commit hashes are not release identities; use the package/git/archive tuple and RPT markers for release proof. | Wiki freshness and release-doc synchronization. |
