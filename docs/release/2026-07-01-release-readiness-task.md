...
As of 2026-07-02 after the PR #125 Build83/cmdcon35, AICOM latch-reset, release-doc package-identity refresh, attack-wave authority follow-ups, ICBM RequestSpecial authority hardening, supply mission PV guards, `f894419db9` package refresh, and follow-up head check:
...
- PR #125, `[codex] Prepare WASP release command center`, branch `codex/release-command-center-20260630`: broad release command-center candidate with tooling, package provenance, AICOM work, and runtime collection gates. GitHub currently reports the branch at `f894419db929d0900160944760f1c6646a6fa3d9`; the recorded package checkpoint matches that head, marker `f894419db9`, `_MISSIONS.7z` SHA256 `0EFED527088FE4EBD0F0C94702DE9A1818972EE9741597F8B400E9B78268A74C`, `1,885` entries, `7,164,905` bytes, handoff status `ready_for_runtime_collection`, and public wiki commit `8099a35`. Exact Chernarus and Takistan runtime RPT proof is still pending, and GitHub reports PR #125 open, draft, clean, and mergeable against `master`.
...
| `rayswaynl/a2waspwarfare.wiki` | Public remote master is at wiki commit `8099a35`, which records the current PR #125 package tuple for `f894419db9`, marker-sweep artifact output, PR #126 fallback-template guard, the runtime-evidence manifest generator/checker with role/terrain stamps and optional package artifact hashing, and clarifies that PR #126 HC-audit markers are required only for packages that include that instrumentation. The wiki still treats runtime/deploy evidence as pending. Wiki commit hashes are not release identities; use the package/git/archive tuple and RPT markers for release proof. | Wiki freshness and release-doc synchronization. |
...
11. Public stats and server-side stats are health context, not release proof.
   ...
   - This is encouraging live health evidence only. It does not bind to PR #125 package marker `f894419db9`, archive SHA `0EFED527088FE4EBD0F0C94702DE9A1818972EE9741597F8B400E9B78268A74C`, the ten required role RPTs, or the runtime packet manifest.
...
17. PR #125 source and package proof are refreshed to the same head.
   - The current package checkpoint commit is `f894419db929d0900160944760f1c6646a6fa3d9`.
   - The PR/wiki/package tuple records `_MISSIONS.7z` built from marker `f894419db9`, SHA256 `0EFED527088FE4EBD0F0C94702DE9A1818972EE9741597F8B400E9B78268A74C`, `1,885` entries, `7,164,905` bytes, and handoff status `ready_for_runtime_collection`.
   ...
   - Action: collect runtime evidence only against the `f894419db9` / `0EFED527...` tuple unless the release owner explicitly promotes a newer package.

18. PR #126 now has a tuple guard for this active package checkpoint.
   - `Tools/Ops/Test-WaspReleaseTupleDocs.ps1` checks the PR #126 release ledger and monitor examples for current PR #125 branch head `f894419d`, expected package git marker `f894419db9`, archive SHA `0EFED527088FE4EBD0F0C94702DE9A1818972EE9741597F8B400E9B78268A74C`, package size `7,164,905`, wiki commit `8099a35`, runtime-pending wording, and absence of stale `190475B4...`, `1C58A4A3...`, `3DB01AC...`, `703F71E6...`, `40883616...`, `50AD...`, `F057...`, `D323...`, and `35B864...` active-surface archive hashes.
...
19. Marker sweeps needed a matrix-level proof wrapper.
   ...
   - Action: PR #126 now adds `Tools/Monitor/Test-WaspRuntimeEvidenceManifest.ps1`, `New-WaspRuntimeEvidenceManifestTemplate.ps1`, and their self-tests. The template generator writes the full default manifest skeleton so operators do not hand-copy ten rows, and can also write a local marker-sweep command template with one command per terrain/role slot. The manifest checker requires one marker-sweep JSON per `chernarus,takistan` x `server,hc1,hc2,start-client,late-jip` slot by default, verifies each sweep matches candidate `release-command-center-20260630`, git marker `f894419db9`, archive SHA `0EFED527088FE4EBD0F0C94702DE9A1818972EE9741597F8B400E9B78268A74C`, expected role, expected terrain, has no missing required markers, and contains the terrain release marker for its slot. This prevents a server sweep from accidentally satisfying an HC/JIP manifest slot, or a broad/wrong-terrain sweep from satisfying the other terrain. When `-ArchivePath` is supplied, the checker also hashes the local package artifact and compares it to `-ExpectedArchiveSha256`; when the manifest includes a top-level release tuple, the checker validates that too. Use narrower required terrain/role arguments only if the release owner explicitly reduces the runtime evidence matrix.

20. PR #125 branch head and package proof realigned after the supply mission guard follow-up.
   - GitHub now reports PR #125 at `f894419db929d0900160944760f1c6646a6fa3d9`.
   - PR #125 now records package proof for ICBM `RequestSpecial` authority/payment hardening plus supply mission PV guards for start, completion, and active-town query payloads.
   - The current package keeps Attack Wave PV/activation authority hardening, shared factory queue empty-head guards, FPS report PV guards, marker-feed shape guards, ICBM authority validation, and server-stamped supply mission cargo/side plus friendly Command Center completion validation.
   - Impact: runtime proof should target the refreshed `f894419db9` / `0EFED527...` package tuple; older `062ab44ff5` and `78ba49d540` packets are now stale unless the release owner explicitly chooses them.
...
- Continue requester-bound `RequestSpecial` authority review for SCUD and any non-ICBM tactical support cases not already covered by the current `f894419db9` supply/ICBM guard set.
