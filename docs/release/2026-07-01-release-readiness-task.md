...
As of 2026-07-01 after the PR #125 Build83/cmdcon35, AICOM latch-reset, and release-doc package-identity refresh:
...
- PR #125, `[codex] Prepare WASP release command center`, branch `codex/release-command-center-20260630`: broad release command-center candidate with tooling, package provenance, AICOM work, and runtime collection gates. Current package identity observed from the PR/wiki handoff: `c1c914f2d3f09acd7a2397bbc36da6005714035c` / marker `c1c914f2d3`. The latest mission deltas add AICOM capture-stall and assault arrival-gate guardrails, marker-helper window/literal-marker hardening, and refreshed package provenance while preserving Build83/cmdcon35 reconciliation, HCTopUp draft exclusion, AICOM/PVF authority hardening, lifecycle/refund hardening, and the active commander handoff latch reset in both maintained terrains. PR #125 package/handoff artifacts and public wiki commit `d49d73e` now record `_MISSIONS.7z` SHA256 `17872D1700260645AA03FAE1505923AA650705CE0C8E6D9117824DE43EE3714D`, `1,885` entries, `7,161,398` bytes, handoff status `ready_for_runtime_collection`. This proves package identity only; exact Chernarus and Takistan runtime RPT proof is still pending, and GitHub reports PR #125 open, draft, clean, and mergeable against `master`.
...
| `rayswaynl/a2waspwarfare.wiki` | Cloned locally and synced through wiki commit `d49d73e`, which records the PR #125 package tuple for `c1c914f2d3` and clarifies that PR #126 HC-audit markers are required only for packages that include that instrumentation. The wiki still treats runtime/deploy evidence as pending. Wiki commit hashes are not release identities; use the package/git/archive tuple and RPT markers for release proof. | Wiki freshness and release-doc synchronization. |
...
11. Public stats and server-side stats are health context, not release proof.
   ...
   - This is encouraging live health evidence only. It does not bind to PR #125 package marker `c1c914f2d3`, archive SHA `17872D1700260645AA03FAE1505923AA650705CE0C8E6D9117824DE43EE3714D`, the ten required role RPTs, or the runtime packet manifest.
...
12. Latest RPT candidates exist and were marker-swept without copying private logs.
   ...
   - Action: these live logs confirm useful pre-instrumentation AICOM/HC health, but cannot satisfy the exact package runtime gate for the current PR #125 package `17872D1700260645AA03FAE1505923AA650705CE0C8E6D9117824DE43EE3714D` or the PR #126 companion instrumentation lane. Added `Tools/Monitor/Get-WaspRptMarkerSweep.ps1` so future livehost/archive checks can repeat this marker sweep with path hashes and line hashes instead of copied logs or raw lines. Added `Tools/Monitor/Test-WaspRptMarkerSweep.SelfTest.ps1`; synthetic validation passes for required-marker success, missing-marker `-NoFail`, missing-marker hard fail, comma-separated required markers, default no-raw-line samples, and exact Chernarus/Takistan release marker generation via `-ExpectedCandidate`, `-ExpectedGit`, and `-RequireReleaseMarkers`. Require `HCDROP_AICOM_AUDIT` and `HCRECON_AICOM_AUDIT` only when the package under test includes PR #126's HC-audit instrumentation. Only copy/analyze broader private RPT contents after the owner confirms target build/window and privacy scope.

13. Release marker fallback must never look like a real release identity.
   - The active PR #125 package contains generated `version.sqf` files with exact `c1c914f2d3` markers, but the tracked fallback/template marker strings still used the older `build83-cmdcon35` identity if generation was skipped.
...
- Use `Tools/Monitor/Get-WaspRptMarkerSweep.ps1` with `-ExpectedCandidate release-command-center-20260630 -ExpectedGit c1c914f2d3 -RequireReleaseMarkers` for redaction-safe first-pass livehost/archive RPT marker counts before deciding whether any private RPT copies are needed.
