...
| `rayswaynl/a2waspwarfare.wiki` | Cloned locally and remote master is synced through wiki commit `51faaca`, which records the current PR #125 package tuple for `7f81115edf`, marker-sweep artifact output, PR #126 fallback-template guard, and clarifies that PR #126 HC-audit markers are required only for packages that include that instrumentation. The wiki still treats runtime/deploy evidence as pending. Wiki commit hashes are not release identities; use the package/git/archive tuple and RPT markers for release proof. | Wiki freshness and release-doc synchronization. |
