...
- PR #125, `[codex] Prepare WASP release command center`, branch `codex/release-command-center-20260630`: broad release command-center candidate with tooling, package provenance, AICOM work, and runtime collection gates. GitHub currently reports the branch at `cf4e93af1a2665bdab13448a03d2fbf3e3b33edf`; the recorded package checkpoint matches that head, marker `cf4e93af1a`, `_MISSIONS.7z` SHA256 `F4F19086A4D51881C61B16CA024FE1C586478161672C7EFE268F5A42C4852B7A`, `1,885` entries, `7,166,267` bytes, handoff status `ready_for_runtime_collection`, and public wiki commit `a82afcd`. Exact Chernarus and Takistan runtime RPT proof is still pending, and GitHub reports PR #125 open, draft, clean, and mergeable against `master`.
...
| `rayswaynl/a2waspwarfare.wiki` | Public remote master is at wiki commit `a82afcd`, which records the current PR #125 package tuple for `cf4e93af1a`, marker-sweep artifact output, PR #126 fallback-template guard, the runtime-evidence manifest generator/checker with role/terrain stamps and optional package artifact hashing, and clarifies that PR #126 HC-audit markers are required only for packages that include that instrumentation. The wiki still treats runtime/deploy evidence as pending. Wiki commit hashes are not release identities; use the package/git/archive tuple and RPT markers for release proof. | Wiki freshness and release-doc synchronization. |
...
11. Public stats and server-side stats are health context, not release proof.
   ...
   - This is encouraging live health evidence only. It does not bind to PR #125 package marker `cf4e93af1a`, archive SHA `F4F19086A4D51881C61B16CA024FE1C586478161672C7EFE268F5A42C4852B7A`, the ten required role RPTs, or the runtime packet manifest.
...
17. PR #125 source and package proof are refreshed to the same head.
   - The current package checkpoint commit is `cf4e93af1a2665bdab13448a03d2fbf3e3b33edf`.
   - The PR/wiki/package tuple records `_MISSIONS.7z` built from marker `cf4e93af1a`, SHA256 `F4F19086A4D51881C61B16CA024FE1C586478161672C7EFE268F5A42C4852B7A`, `1,885` entries, `7,166,267` bytes, and handoff status `ready_for_runtime_collection`.
   - Validation recorded on PR #125 includes LoadoutManager RELEASE rebuild, package gates, release handoff generation, wiki parse/diff checks, static smoke, final-check package-worktree hygiene, Chernarus/Takistan OA lint pass, high-only BugHunt clean, the HC AICOM delegation PVF follow-up in both maintained `Client_HandlePVF.sqf` files, FPS report PV payload guards, marker-feed consumer guards, SCUD authority binding, GUER mortar authority guard plus nested result receiver unwrap, player support/UAV requester/team/destination/upgrade/cooldown/funds binding, and marker-sweep defaults for `B74.2 REQ-MARK` plus `aiStatus=`.
   - Action: collect runtime evidence only against the `cf4e93af1a` / `F4F19086...` tuple unless the release owner explicitly promotes a newer package.

18. PR #126 now has a tuple guard for this active package checkpoint.
   - `Tools/Ops/Test-WaspReleaseTupleDocs.ps1` checks the PR #126 release ledger and monitor examples for current PR #125 branch head `cf4e93af`, expected package git marker `cf4e93af1a`, archive SHA `F4F19086A4D51881C61B16CA024FE1C586478161672C7EFE268F5A42C4852B7A`, package size `7,166,267`, wiki commit `a82afcd`, runtime-pending wording, and absence of stale `190475B4...`, `1C58A4A3...`, `3DB01AC...`, `703F71E6...`, `40883616...`, `50AD...`, `F057...`, `D323...`, `76A5EE56...`, `E887E792...`, `8D06CDA3...`, `35B864...`, `D5CA434E...`, and `ECC6F9D5...` active-surface archive hashes.
...
- Continue requester-bound `RequestSpecial` authority review for any remaining tactical support cases not already covered by the current `cf4e93af1a` ICBM, supply, SCUD, GUER mortar, and player support/UAV requester-binding guard set; keep runtime proof pending for server-owned player support/UAV payment, cooldown, funds and active-state behavior.
