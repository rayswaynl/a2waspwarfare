...
As of 2026-07-02 after the PR #125 Build83/cmdcon35, AICOM latch-reset, release-doc package-identity refresh, attack-wave authority follow-ups, ICBM RequestSpecial authority hardening, supply mission PV guards, SCUD authority binding, GUER mortar authority guard, paradrop support requester binding, `fa3d0a159d` package refresh, and follow-up head check:
...
- PR #125, `[codex] Prepare WASP release command center`, branch `codex/release-command-center-20260630`: broad release command-center candidate with tooling, package provenance, AICOM work, and runtime collection gates. GitHub currently reports the branch at `fa3d0a159d1746e860c1cbf12062b2f45835ab57`; the recorded package checkpoint matches that head, marker `fa3d0a159d`, `_MISSIONS.7z` SHA256 `76A5EE569DDA8E1486A16A7C20DF44E3332170F7590A2AE3008C6058E32E25DA`, `1,885` entries, `7,165,509` bytes, handoff status `ready_for_runtime_collection`, and public wiki commit `299fad9`. Exact Chernarus and Takistan runtime RPT proof is still pending, and GitHub reports PR #125 open, draft, clean, and mergeable against `master`.
...
| `rayswaynl/a2waspwarfare.wiki` | Public remote master is at wiki commit `299fad9`, which records the current PR #125 package tuple for `fa3d0a159d`, marker-sweep artifact output, PR #126 fallback-template guard, the runtime-evidence manifest generator/checker with role/terrain stamps and optional package artifact hashing, and clarifies that PR #126 HC-audit markers are required only for packages that include that instrumentation. The wiki still treats runtime/deploy evidence as pending. Wiki commit hashes are not release identities; use the package/git/archive tuple and RPT markers for release proof. | Wiki freshness and release-doc synchronization. |
...
9. Existing release PRs already agree on the main gate: source/static/package checks are not enough.
   ...
   - PR #125 now has package/handoff proof for `fa3d0a159d` / `_MISSIONS.7z` SHA256 `76A5EE569DDA8E1486A16A7C20DF44E3332170F7590A2AE3008C6058E32E25DA`, but that is still package/tooling proof, not final runtime proof. PR #126 and PR #131 artifacts remain companion/tooling proof unless explicitly promoted into the active package lane.
...
11. Public stats and server-side stats are health context, not release proof.
   ...
   - This is encouraging live health evidence only. It does not bind to PR #125 package marker `fa3d0a159d`, archive SHA `76A5EE569DDA8E1486A16A7C20DF44E3332170F7590A2AE3008C6058E32E25DA`, the ten required role RPTs, or the runtime packet manifest.
...
17. PR #125 source and package proof are refreshed to the same head.
   - The current package checkpoint commit is `fa3d0a159d1746e860c1cbf12062b2f45835ab57`.
   - The PR/wiki/package tuple records `_MISSIONS.7z` built from marker `fa3d0a159d`, SHA256 `76A5EE569DDA8E1486A16A7C20DF44E3332170F7590A2AE3008C6058E32E25DA`, `1,885` entries, `7,165,509` bytes, and handoff status `ready_for_runtime_collection`.
   - Validation recorded on PR #125 includes LoadoutManager RELEASE rebuild, package gates, release handoff generation, wiki parse/diff checks, static smoke, final-check package-worktree hygiene, Chernarus/Takistan OA lint pass, high-only BugHunt clean, the HC AICOM delegation PVF follow-up in both maintained `Client_HandlePVF.sqf` files, FPS report PV payload guards, marker-feed consumer guards, SCUD authority binding, GUER mortar authority guard, paradrop support requester/team/destination/upgrade binding, and marker-sweep defaults for `B74.2 REQ-MARK` plus `aiStatus=`.
   - Action: collect runtime evidence only against the `fa3d0a159d` / `76A5EE56...` tuple unless the release owner explicitly promotes a newer package.

18. PR #126 now has a tuple guard for this active package checkpoint.
   - `Tools/Ops/Test-WaspReleaseTupleDocs.ps1` checks the PR #126 release ledger and monitor examples for current PR #125 branch head `fa3d0a159`, expected package git marker `fa3d0a159d`, archive SHA `76A5EE569DDA8E1486A16A7C20DF44E3332170F7590A2AE3008C6058E32E25DA`, package size `7,165,509`, wiki commit `299fad9`, runtime-pending wording, and absence of stale `190475B4...`, `1C58A4A3...`, `3DB01AC...`, `703F71E6...`, `40883616...`, `50AD...`, `F057...`, `D323...`, `35B864...`, `D5CA434E...`, and `ECC6F9D5...` active-surface archive hashes.
...
- Continue requester-bound `RequestSpecial` authority review for any remaining tactical support cases not already covered by the current `fa3d0a159d` ICBM, supply, SCUD, GUER mortar, and paradrop requester-binding guard set; keep paradrop payment/cooldown as follow-up debt until server-side proof or code exists.
