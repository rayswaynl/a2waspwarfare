...
- PR #125, `[codex] Prepare WASP release command center`, branch `codex/release-command-center-20260630`: broad release command-center candidate with tooling, package provenance, AICOM work, and runtime collection gates. GitHub currently reports the branch at `e3b6e379032d65241f0f4da1aa72a333dcd0df67` after an HC AICOM delegation PVF follow-up, while the current package checkpoint remains marker `a94818f2eb`; `_MISSIONS.7z` SHA256 `703F71E6C9F7E2FEB5323ADE32B5A4AFF7A2B277FC8B5BBE2687F5ED91601BED`, `1,885` entries, `7,161,776` bytes, handoff status `ready_for_runtime_collection`, and public wiki commit `e4404a4`. Exact Chernarus and Takistan runtime RPT proof is still pending, and GitHub reports PR #125 open, draft, clean, and mergeable against `master`.
...
| `rayswaynl/a2waspwarfare.wiki` | Cloned locally and remote master is synced through wiki commit `e4404a4`, which records the current PR #125 package tuple for `a94818f2eb`, marker-sweep artifact output, PR #126 fallback-template guard, and clarifies that PR #126 HC-audit markers are required only for packages that include that instrumentation. The wiki still treats runtime/deploy evidence as pending. Wiki commit hashes are not release identities; use the package/git/archive tuple and RPT markers for release proof. | Wiki freshness and release-doc synchronization. |
...
11. Public stats and server-side stats are health context, not release proof.
   ...
   - This is encouraging live health evidence only. It does not bind to PR #125 package marker `a94818f2eb`, archive SHA `703F71E6C9F7E2FEB5323ADE32B5A4AFF7A2B277FC8B5BBE2687F5ED91601BED`, the ten required role RPTs, or the runtime packet manifest.
...
17. PR #125 source and package proof were refreshed to the same head, then the branch gained follow-ups.
   - The current package checkpoint commit is `a94818f2ebf77a8c825df2ee9b3ae65e49a980d9`.
   - The PR/wiki/package tuple records `_MISSIONS.7z` built from marker `a94818f2eb`, SHA256 `703F71E6C9F7E2FEB5323ADE32B5A4AFF7A2B277FC8B5BBE2687F5ED91601BED`, `1,885` entries, `7,161,776` bytes, and handoff status `ready_for_runtime_collection`.
   - Validation recorded on PR #125 includes LoadoutManager RELEASE rebuild, package gates, release handoff generation, wiki parse/diff checks, static smoke, final-check package-worktree hygiene, Chernarus/Takistan OA lint pass, and high-only BugHunt clean.
   - Action: collect runtime evidence only against the `a94818f2eb` / `703F71E6...` tuple unless the release owner explicitly promotes a newer package.

18. PR #125 branch head advanced after the current package checkpoint.
   - GitHub now reports PR #125 branch `codex/release-command-center-20260630` at `e3b6e379032d65241f0f4da1aa72a333dcd0df67`.
   - The delta from `a94818f2` to `e3b6e379` touches `JOURNAL.md`, `Tools/PrTestHarness/Smoke/Test-WaspStaticSmoke.ps1`, and both maintained `Client/Functions/Client_HandlePVF.sqf` files.
   - Impact: the `a94818f2eb` package remains the last recorded package checkpoint, but it is no longer exact for the literal branch head because maintained mission files changed. Runtime collection should still target the `a94818f2eb` / `703F71E6...` package only if the release owner intentionally chooses that checkpoint; otherwise PR #125 must cut and document a new package from `e3b6e379` or a later head first.
...
- Use `Tools/Monitor/Get-WaspRptMarkerSweep.ps1` with `-ExpectedCandidate release-command-center-20260630 -ExpectedGit a94818f2eb -ExpectedArchiveSha256 703F71E6C9F7E2FEB5323ADE32B5A4AFF7A2B277FC8B5BBE2687F5ED91601BED -RequireReleaseMarkers -OutFile <redaction-safe-marker-sweep.json>` for redaction-safe first-pass livehost/archive RPT marker counts before deciding whether any private RPT copies are needed.
