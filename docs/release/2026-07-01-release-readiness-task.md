...
As of 2026-07-02 after the PR #125 Build83/cmdcon35, AICOM latch-reset, release-doc package-identity refresh, attack-wave authority follow-ups, ICBM RequestSpecial authority hardening, supply mission PV guards, SCUD authority binding, GUER mortar authority guard plus result receiver unwrap, player support/UAV authority binding, support-adjacent repair/GUER guard hardening, commander-support requester/logging/ownership/default guards, AI focus payload count/type and side-player AI-run validation, `177ef1385f` package refresh, and follow-up head check:
...
- PR #125, `[codex] Prepare WASP release command center`, branch `codex/release-command-center-20260630`: broad release command-center candidate with tooling, package provenance, AICOM work, and runtime collection gates. GitHub currently reports the branch at `177ef1385f22aefb466368ef9c381d64a57801c0`; the recorded package checkpoint matches that head, marker `177ef1385f`, `_MISSIONS.7z` SHA256 `F60F3C7D2C568BB1713A9C550D180B3DFA47C1400C7B1725680B8EFC3BC6B00F`, `1,885` entries, `7,167,223` bytes, handoff status `ready_for_runtime_collection`, and public wiki commit `c27aa57`. Exact Chernarus and Takistan runtime RPT proof is still pending, and GitHub reports PR #125 open, draft, clean, and mergeable against `master`.
...
| `rayswaynl/a2waspwarfare.wiki` | Public remote master is at wiki commit `c27aa57`, which records the current PR #125 package tuple for `177ef1385f`, marker-sweep artifact output, PR #126 fallback-template guard, the runtime-evidence manifest generator/checker with role/terrain stamps and optional package artifact hashing, and clarifies that PR #126 HC-audit markers are required only for packages that include that instrumentation. The wiki still treats runtime/deploy evidence as pending. Wiki commit hashes are not release identities; use the package/git/archive tuple and RPT markers for release proof. | Wiki freshness and release-doc synchronization. |
...
9. Existing release PRs already agree on the main gate: source/static/package checks are not enough.
   ...
   - PR #125 now has package/handoff proof for `177ef1385f` / `_MISSIONS.7z` SHA256 `F60F3C7D2C568BB1713A9C550D180B3DFA47C1400C7B1725680B8EFC3BC6B00F`, but that is still package/tooling proof, not final runtime proof. PR #126 and PR #131 artifacts remain companion/tooling proof unless explicitly promoted into the active package lane.
...
11. Public stats and server-side stats are health context, not release proof.
   ...
   - This is encouraging live health evidence only. It does not bind to PR #125 package marker `177ef1385f`, archive SHA `F60F3C7D2C568BB1713A9C550D180B3DFA47C1400C7B1725680B8EFC3BC6B00F`, the ten required role RPTs, or the runtime packet manifest.
...
17. PR #125 source and package proof are refreshed to the same head.
   - The current package checkpoint commit is `177ef1385f22aefb466368ef9c381d64a57801c0`.
   - The PR/wiki/package tuple records `_MISSIONS.7z` built from marker `177ef1385f`, SHA256 `F60F3C7D2C568BB1713A9C550D180B3DFA47C1400C7B1725680B8EFC3BC6B00F`, `1,885` entries, `7,167,223` bytes, and handoff status `ready_for_runtime_collection`.
   - Validation recorded on PR #125 includes LoadoutManager RELEASE rebuild, package gates, release handoff generation, wiki parse/diff checks, static smoke, final-check package-worktree hygiene, Chernarus/Takistan OA lint pass, high-only BugHunt clean, the HC AICOM delegation PVF follow-up in both maintained `Client_HandlePVF.sqf` files, FPS report PV payload guards, marker-feed consumer guards, SCUD authority binding, GUER mortar authority guard plus nested result receiver unwrap, player support/UAV requester/team/destination/upgrade/cooldown/funds binding, server-authoritative supply-truck respawn, server-side camp repair debit/validation, GUER VBIED/FOB payload guards, both-terrain HC delegated-AI smoke coverage, and marker-sweep defaults for `B74.2 REQ-MARK` plus `aiStatus=`.
   - Action: collect runtime evidence only against the `177ef1385f` / `F60F3C7D...` tuple unless the release owner explicitly promotes a newer package.

18. PR #126 now has a tuple guard for this active package checkpoint.
   - `Tools/Ops/Test-WaspReleaseTupleDocs.ps1` checks the PR #126 release ledger and monitor examples for current PR #125 branch head `177ef138`, expected package git marker `177ef1385f`, archive SHA `F60F3C7D2C568BB1713A9C550D180B3DFA47C1400C7B1725680B8EFC3BC6B00F`, package size `7,167,223`, wiki commit `c27aa57`, runtime-pending wording, and absence of stale `190475B4...`, `1C58A4A3...`, `3DB01AC...`, `703F71E6...`, `40883616...`, `50AD...`, `F057...`, `D323...`, `76A5EE56...`, `E887E792...`, `8D06CDA3...`, `35B864...`, `D5CA434E...`, `ECC6F9D5...`, `F4F19086...`, `0AC507E1...`, `22F5AEE4...`, `E7B12EDA...`, `FB52623D...`, `35280646...`, `3E98F3B6...`, `11AEED5C...`, and `F5232AE7...` active-surface archive hashes.
...
19. Marker sweeps needed a matrix-level proof wrapper.
   ...
   - Action: PR #126 now adds `Tools/Monitor/Test-WaspRuntimeEvidenceManifest.ps1`, `New-WaspRuntimeEvidenceManifestTemplate.ps1`, and their self-tests. The template generator writes the full default manifest skeleton so operators do not hand-copy ten rows, and can also write a local marker-sweep command template with one command per terrain/role slot. The manifest checker requires one marker-sweep JSON per `chernarus,takistan` x `server,hc1,hc2,start-client,late-jip` slot by default, verifies each sweep matches candidate `release-command-center-20260630`, git marker `177ef1385f`, archive SHA `F60F3C7D2C568BB1713A9C550D180B3DFA47C1400C7B1725680B8EFC3BC6B00F`, expected role, expected terrain, has no missing required markers, and contains the terrain release marker for its slot. This prevents a server sweep from accidentally satisfying an HC/JIP manifest slot, or a broad/wrong-terrain sweep from satisfying the other terrain. When `-ArchivePath` is supplied, the checker also hashes the local package artifact and compares it to `-ExpectedArchiveSha256`; when the manifest includes a top-level release tuple, the checker validates that too. Use narrower required terrain/role arguments only if the release owner explicitly reduces the runtime evidence matrix.

20. PR #125 branch head and package proof realigned after the SCUD, GUER mortar, player support/UAV, GUER result receiver, repair, GUER support, commander-support and AI focus follow-ups.
   - GitHub now reports PR #125 at `177ef1385f22aefb466368ef9c381d64a57801c0`.
   - PR #125 now records package proof for ICBM `RequestSpecial` authority/payment hardening, supply mission PV guards for start, completion and active-town query payloads, SCUD requester/team/action-grant/pad-proximity binding, GUER mortar requester/resistance-driver/vehicle/range binding plus nested client result unwrapping, player support/UAV requester/team/destination/upgrade/cooldown/funds binding, supply-truck `RespawnST` commander/team validation, side-player `aicom-focus` payload count/type and AI-run validation, camp repair server-side price/debit/actor validation, GUER VBIED object/class guards, and GUER FOB requester/truck/build-range/class guards.
   - The current package keeps Attack Wave PV/activation authority hardening, shared factory queue empty-head guards, FPS report PV guards, marker-feed shape guards, ICBM authority validation, server-stamped supply mission cargo/side plus friendly Command Center completion validation, SCUD server revalidation, GUER mortar server revalidation and result receiver unwrapping, and paradrop server requester/destination revalidation. Player support payment/cooldown is now server-owned for paratroops, vehicle drops and ammo drops; UAV requests validate active-state and funds server-side. Side-player AI focus requests are server-gated to the AI-commanding state, while human-commander-only AICOM actions remain commander-bound. Remaining authority debt still includes construction/defense PVFs and `RequestOnUnitKilled` kill/score/tech/funds attribution.
   - Impact: runtime proof should target the refreshed `177ef1385f` / `F60F3C7D...` package tuple; older `062ab44ff5`, `78ba49d540`, `cf4e93af1a`, `4893faaa5d`, `9e628781a8`, and stale package hashes including `22F5AEE4...`, `FB52623D...`, `35280646...`, `3E98F3B6...`, `11AEED5C...`, and `F5232AE7...` are now stale unless the release owner explicitly chooses them.
...
- Use `Tools/Monitor/Get-WaspRptMarkerSweep.ps1` with `-ExpectedCandidate release-command-center-20260630 -ExpectedGit 177ef1385f -ExpectedArchiveSha256 F60F3C7D2C568BB1713A9C550D180B3DFA47C1400C7B1725680B8EFC3BC6B00F -ExpectedRole <server|hc1|hc2|start-client|late-jip> -RequireReleaseMarkers -OutFile <redaction-safe-marker-sweep.json>` for redaction-safe first-pass livehost/archive RPT marker counts before deciding whether any private RPT copies are needed.
- Use `Tools/Monitor/New-WaspRuntimeEvidenceManifestTemplate.ps1` to build a redaction-safe `a2waspwarfare-runtime-evidence-manifest-v1` manifest that maps each terrain/role slot to its marker-sweep JSON, then run `Tools/Monitor/Test-WaspRuntimeEvidenceManifest.ps1` against the active package tuple, with `-ArchivePath` when `_MISSIONS.7z` is locally available, before declaring runtime proof complete.
- Continue requester-bound authority review for any remaining tactical support cases not already covered by the current `177ef1385f` ICBM, supply, SCUD, GUER mortar, player support/UAV, supply-truck respawn, AI focus, commander-support tuple/logging/ownership/default, camp repair, GUER VBIED, and GUER FOB guard set; keep runtime proof pending for server-owned payment, cooldown, funds, active-state, repair, focus, and support behavior, and keep construction/defense PVFs plus `RequestOnUnitKilled` in the open authority backlog.
