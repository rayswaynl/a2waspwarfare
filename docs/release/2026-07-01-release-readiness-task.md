...
- PR #125, `[codex] Prepare WASP release command center`, branch `codex/release-command-center-20260630`: broad release command-center candidate with tooling, package provenance, AICOM work, and runtime collection gates. Current package identity observed from the PR/wiki handoff: `5face13a0a0bcb7fbf9bc56e0e64bc4af89ef8f2` / marker `5face13a0a`. The latest mission deltas add AICOM capture-stall guardrails, marker-helper window/literal-marker hardening, and refreshed package provenance while preserving Build83/cmdcon35 reconciliation, HCTopUp draft exclusion, AICOM/PVF authority hardening, lifecycle/refund hardening, and the active commander handoff latch reset in both maintained terrains. PR #125 package/handoff artifacts and public wiki commit `6fc1689` now record `_MISSIONS.7z` SHA256 `06A7E8C3EF1DFE9BC67306D43C5CF66AD79D26401CA87E815434B92B13A17608`, `1,885` entries, `7,161,540` bytes, handoff status `ready_for_runtime_collection`. This proves package identity only; exact Chernarus and Takistan runtime RPT proof is still pending, and GitHub reports PR #125 open, draft, clean, and mergeable against `master`.
...
| `rayswaynl/a2waspwarfare.wiki` | Cloned locally and synced through wiki commit `6fc1689`, which records the PR #125 package tuple for `5face13a0a` and clarifies that PR #126 HC-audit markers are required only for packages that include that instrumentation. The wiki still treats runtime/deploy evidence as pending. Wiki commit hashes are not release identities; use the package/git/archive tuple and RPT markers for release proof. | Wiki freshness and release-doc synchronization. |
...
11. Public stats and server-side stats are health context, not release proof.
   ...
   - This is encouraging live health evidence only. It does not bind to PR #125 package marker `5face13a0a`, archive SHA `06A7E8C3EF1DFE9BC67306D43C5CF66AD79D26401CA87E815434B92B13A17608`, the ten required role RPTs, or the runtime packet manifest.
...
- Use `Tools/Monitor/Get-WaspRptMarkerSweep.ps1` with `-ExpectedCandidate release-command-center-20260630 -ExpectedGit 5face13a0a -RequireReleaseMarkers` for redaction-safe first-pass livehost/archive RPT marker counts before deciding whether any private RPT copies are needed.
