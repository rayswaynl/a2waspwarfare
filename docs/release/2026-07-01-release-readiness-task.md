...
- PR #125, `[codex] Prepare WASP release command center`, branch `codex/release-command-center-20260630`: broad release command-center candidate with tooling, package provenance, AICOM work, and runtime collection gates. Current source/package head observed: `b3f4d3664f10b3a40f1ffe0aee22653bbef8c754` / marker `b3f4d3664f`. The latest mission deltas add AICOM lifecycle/refund hardening on top of the cmdcon35 placement gate while preserving Build83/cmdcon35 reconciliation, HCTopUp draft exclusion, AICOM/PVF authority hardening, and the active commander handoff latch reset in both maintained terrains. PR #125 package/handoff artifacts and public wiki commit `adb002c` now record `_MISSIONS.7z` SHA256 `4CE5BF8843C880F2D16D0027BEC807620377DBEFD1061D85EE07F415C7ED7803`, `1,881` entries, `7,159,557` bytes, handoff status `ready_for_runtime_collection`. This proves package identity only; exact Chernarus and Takistan runtime RPT proof is still pending, and GitHub reports PR #125 open, draft, clean, and mergeable against `master`.
...
11. Public stats and server-side stats are health context, not release proof.
   ...
   - This is encouraging live health evidence only. It does not bind to PR #125 package marker `b3f4d3664f`, archive SHA `4CE5BF8843C880F2D16D0027BEC807620377DBEFD1061D85EE07F415C7ED7803`, the ten required role RPTs, or the runtime packet manifest.
...
- Use `Tools/Monitor/Get-WaspRptMarkerSweep.ps1` with `-ExpectedCandidate release-command-center-20260630 -ExpectedGit b3f4d3664f -RequireReleaseMarkers` for redaction-safe first-pass livehost/archive RPT marker counts before deciding whether any private RPT copies are needed.
