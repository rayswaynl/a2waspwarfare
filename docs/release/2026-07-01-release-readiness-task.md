...
As of 2026-07-02 after the PR #125 Build83/cmdcon35, AICOM latch-reset, release-doc package-identity refresh, attack-wave authority follow-ups, ICBM RequestSpecial authority hardening, `d8f15b554f` package refresh, and follow-up head check:
...
- PR #125, `[codex] Prepare WASP release command center`, branch `codex/release-command-center-20260630`: broad release command-center candidate with tooling, package provenance, AICOM work, and runtime collection gates. GitHub currently reports the branch at `d8f15b554faf7e1a5f08fa037d5db58eac616918`; the recorded package checkpoint matches that head, marker `d8f15b554f`, `_MISSIONS.7z` SHA256 `35B864A10626B8AA92F4C7A7729E1CF889310A8DA8E3BAEE58C09BA2BFC5053E`, `1,885` entries, `7,163,413` bytes, handoff status `ready_for_runtime_collection`, and public wiki commit `99d0ddf`. Exact Chernarus and Takistan runtime RPT proof is still pending, and GitHub reports PR #125 open, draft, clean, and mergeable against `master`.
...
| `rayswaynl/a2waspwarfare.wiki` | Public remote master is at wiki commit `99d0ddf`, which records the current PR #125 package tuple for `d8f15b554f`, marker-sweep artifact output, PR #126 fallback-template guard, the runtime-evidence manifest generator/checker with role/terrain stamps and optional package artifact hashing, and clarifies that PR #126 HC-audit markers are required only for packages that include that instrumentation. The wiki still treats runtime/deploy evidence as pending. Wiki commit hashes are not release identities; use the package/git/archive tuple and RPT markers for release proof. | Wiki freshness and release-doc synchronization. |
...
11. Public stats and server-side stats are health context, not release proof.
   ...
   - This is encouraging live health evidence only. It does not bind to PR #125 package marker `d8f15b554f`, archive SHA `35B864A10626B8AA92F4C7A7729E1CF889310A8DA8E3BAEE58C09BA2BFC5053E`, the ten required role RPTs, or the runtime packet manifest.
...
17. PR #125 source and package proof are refreshed to the same head.
   - The current package checkpoint commit is `d8f15b554faf7e1a5f08fa037d5db58eac616918`.
   - The PR/wiki/package tuple records `_MISSIONS.7z` built from marker `d8f15b554f`, SHA256 `35B864A10626B8AA92F4C7A7729E1CF889310A8DA8E3BAEE58C09BA2BFC5053E`, `1,885` entries, `7,163,413` bytes, and handoff status `ready_for_runtime_collection`.
   - Validation recorded on PR #125 includes LoadoutManager RELEASE rebuild, package gates, release handoff generation, wiki parse/diff checks, static smoke, final-check package-worktree hygiene, Chernarus/Takistan OA lint pass, high-only BugHunt clean, the HC AICOM delegation PVF follow-up in both maintained `Client_HandlePVF.sqf` files, FPS report PV payload guards, marker-feed consumer guards, and marker-sweep defaults for `B74.2 REQ-MARK` plus `aiStatus=`.
   - Action: collect runtime evidence only against the `d8f15b554f` / `35B864...` tuple unless the release owner explicitly promotes a newer package.

18. PR #126 now has a tuple guard for this active package checkpoint.
   - `Tools/Ops/Test-WaspReleaseTupleDocs.ps1` checks the PR #126 release ledger and monitor examples for current PR #125 branch head `d8f15b55`, expected package git marker `d8f15b554f`, archive SHA `35B864A10626B8AA92F4C7A7729E1CF889310A8DA8E3BAEE58C09BA2BFC5053E`, package size `7,163,413`, wiki commit `99d0ddf`, runtime-pending wording, and absence of stale `190475B4...`, `1C58A4A3...`, `3DB01AC...`, `703F71E6...`, `40883616...`, `50AD...`, `F057...`, and `D323...` active-surface archive hashes.
...
- Continue requester-bound `RequestSpecial` authority review for SCUD and non-ICBM tactical support cases; PR #125 currently records these as remaining authority debt after the ICBM pass.
