...
As of 2026-07-02 after the PR #125 Build83/cmdcon35, AICOM latch-reset, release-doc package-identity refresh, attack-wave authority follow-ups, `0ee16d18e0` package refresh, and follow-up head check:
...
- PR #125, `[codex] Prepare WASP release command center`, branch `codex/release-command-center-20260630`: broad release command-center candidate with tooling, package provenance, AICOM work, and runtime collection gates. GitHub currently reports the branch at `0ee16d18e08de590bbdbbc109566390956423547`; the recorded package checkpoint matches that head, marker `0ee16d18e0`, `_MISSIONS.7z` SHA256 `D323434629AB90F90CDD4C4874F164422F38B94075101861F8B1E726C76FE81E`, `1,885` entries, `7,163,301` bytes, handoff status `ready_for_runtime_collection`, and public wiki commit `b273830`. Exact Chernarus and Takistan runtime RPT proof is still pending, and GitHub reports PR #125 open, draft, clean, and mergeable against `master`.
...
| `rayswaynl/a2waspwarfare.wiki` | Public remote master is at wiki commit `b273830`, which records the current PR #125 package tuple for `0ee16d18e0`, marker-sweep artifact output, PR #126 fallback-template guard, the runtime-evidence manifest generator/checker with role/terrain stamps and optional package artifact hashing, and clarifies that PR #126 HC-audit markers are required only for packages that include that instrumentation. The wiki still treats runtime/deploy evidence as pending. Wiki commit hashes are not release identities; use the package/git/archive tuple and RPT markers for release proof. | Wiki freshness and release-doc synchronization. |
...
11. Public stats and server-side stats are health context, not release proof.
   ...
   - This is encouraging live health evidence only. It does not bind to PR #125 package marker `0ee16d18e0`, archive SHA `D323434629AB90F90CDD4C4874F164422F38B94075101861F8B1E726C76FE81E`, the ten required role RPTs, or the runtime packet manifest.
...
17. PR #125 source and package proof are refreshed to the same head.
   - The current package checkpoint commit is `0ee16d18e08de590bbdbbc109566390956423547`.
   - The PR/wiki/package tuple records `_MISSIONS.7z` built from marker `0ee16d18e0`, SHA256 `D323434629AB90F90CDD4C4874F164422F38B94075101861F8B1E726C76FE81E`, `1,885` entries, `7,163,301` bytes, and handoff status `ready_for_runtime_collection`.
   ...
   - Action: collect runtime evidence only against the `0ee16d18e0` / `D3234346...` tuple unless the release owner explicitly promotes a newer package.

18. PR #126 now has a tuple guard for this active package checkpoint.
   - `Tools/Ops/Test-WaspReleaseTupleDocs.ps1` checks the PR #126 release ledger and monitor examples for current PR #125 branch head `0ee16d18e`, expected package git marker `0ee16d18e0`, archive SHA `D323434629AB90F90CDD4C4874F164422F38B94075101861F8B1E726C76FE81E`, package size `7,163,301`, wiki commit `b273830`, runtime-pending wording, and absence of stale `190475B4...`, `1C58A4A3...`, `3DB01AC...`, `703F71E6...`, `40883616...`, `50AD...`, and `F057...` active-surface archive hashes.
...
19. Marker sweeps needed a matrix-level proof wrapper.
   ...
   - Action: PR #126 now adds `Tools/Monitor/Test-WaspRuntimeEvidenceManifest.ps1`, `New-WaspRuntimeEvidenceManifestTemplate.ps1`, and their self-tests. The template generator writes the full default manifest skeleton so operators do not hand-copy ten rows, and can also write a local marker-sweep command template with one command per terrain/role slot. The manifest checker requires one marker-sweep JSON per `chernarus,takistan` x `server,hc1,hc2,start-client,late-jip` slot by default, verifies each sweep matches candidate `release-command-center-20260630`, git marker `0ee16d18e0`, archive SHA `D323434629AB90F90CDD4C4874F164422F38B94075101861F8B1E726C76FE81E`, expected role, expected terrain, has no missing required markers, and contains the terrain release marker for its slot. This prevents a server sweep from accidentally satisfying an HC/JIP manifest slot, or a broad/wrong-terrain sweep from satisfying the other terrain. When `-ArchivePath` is supplied, the checker also hashes the local package artifact and compares it to `-ExpectedArchiveSha256`; when the manifest includes a top-level release tuple, the checker validates that too. Use narrower required terrain/role arguments only if the release owner explicitly reduces the runtime evidence matrix.

20. PR #125 branch head and package proof realigned after the attack-wave follow-up.
   - GitHub now reports PR #125 at `0ee16d18e08de590bbdbbc109566390956423547`.
   - The delta from `062ab44ff5` to `0ee16d18e` is `b984337bf fix: guard attack wave pv payloads` and `0ee16d18e fix: bind attack wave activation authority`, and PR #125 now records package proof for that head.
   - The changed files include both maintained terrains' `Common/Functions/Common_AttackWaveActivate.sqf`, `Server/Functions/Server_AttackWave.sqf`, and `Server/PVFunctions/AttackWave.sqf`, plus `JOURNAL.md` and static-smoke tooling.
