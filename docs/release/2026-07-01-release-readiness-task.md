...
- PR #125, `[codex] Prepare WASP release command center`, branch `codex/release-command-center-20260630`: broad release command-center candidate with tooling, package provenance, AICOM work, and runtime collection gates. Current source/package head observed: `1d713bcf2b2daa6eea722bca5221f4e430bbe746` / marker `1d713bcf2b`. The latest mission deltas reconcile Build83/cmdcon35, keep the uncompiled AICOM HC top-up draft worker/wiring excluded, preserve AICOM/PVF authority hardening, and replace stale commander handoff `wfbe_exec_sig` resets with the active `wfbe_exec_lastmode` / `wfbe_exec_lastgoto` / `wfbe_exec_at` latch reset in both maintained terrains; the latest branch head is a documentation-only package-identity refresh. PR #125 and the public wiki now record `_MISSIONS.7z` SHA256 `2513C37DFDA376EA15D631AD7490B2133BCCFE78726B684960F26EF6BF82F8B6`, `1,881` entries, `7,158,563` bytes, handoff status `ready_for_runtime_collection`. This proves package identity only; exact Chernarus and Takistan runtime RPT proof is still pending, and GitHub reports PR #125 open, draft, clean, and mergeable against `master`.
...
| `rayswaynl/a2waspwarfare.wiki` | Cloned locally and synced through wiki commit `4957bfb`, which records the PR #125 package tuple for `1d713bcf2b`: `_MISSIONS.7z` SHA256 `2513C37DFDA376EA15D631AD7490B2133BCCFE78726B684960F26EF6BF82F8B6`, `1,881` entries, `7,158,563` bytes, handoff `ready_for_runtime_collection`. The wiki still treats runtime/deploy evidence as pending. Wiki commit hashes are not release identities; use the package/git/archive tuple and RPT markers for release proof. | Wiki freshness and release-doc synchronization. |
...
- Use `Tools/Monitor/Get-WaspRptMarkerSweep.ps1` with `-ExpectedCandidate release-command-center-20260630 -ExpectedGit 1d713bcf2b -RequireReleaseMarkers` for redaction-safe first-pass livehost/archive RPT marker counts before deciding whether any private RPT copies are needed.
