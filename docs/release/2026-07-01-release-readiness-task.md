...
| `rayswaynl/a2waspwarfare.wiki` | Public remote master is at wiki commit `00cdc18`, which records the current PR #125 package tuple for `e3b6e37903`, marker-sweep artifact output, PR #126 fallback-template guard, and clarifies that PR #126 HC-audit markers are required only for packages that include that instrumentation. The wiki still treats runtime/deploy evidence as pending. Wiki commit hashes are not release identities; use the package/git/archive tuple and RPT markers for release proof. | Wiki freshness and release-doc synchronization. |
