...
- PR #125, `[codex] Prepare WASP release command center`, branch `codex/release-command-center-20260630`: broad release command-center candidate with tooling, package provenance, AICOM work, and runtime collection gates. GitHub currently reports the branch at `78ba49d54065f20d9f13ec7242514a00ee4e4467`; the last recorded package checkpoint remains `e3b6e379032d65241f0f4da1aa72a333dcd0df67`, marker `e3b6e37903`, `_MISSIONS.7z` SHA256 `3DB01AC1656329ECCAE9896CE9442680D5D904C563DD44590FFBD20954CF7B87`, `1,885` entries, `7,162,113` bytes, handoff status `ready_for_runtime_collection`, and public wiki commit `1883a1b`. The post-package `78ba49d5` / `c8ab1ff6` commits touch maintained Chernarus/Takistan mission files and monitor/static-smoke tooling, so exact branch-head runtime collection requires a new package unless the owner explicitly selects the `e3b6e37903` checkpoint. Exact Chernarus and Takistan runtime RPT proof is still pending, and GitHub reports PR #125 open, draft, clean, and mergeable against `master`.
...
| `rayswaynl/a2waspwarfare.wiki` | Public remote master is at wiki commit `1883a1b`, which records the current PR #125 package tuple for `e3b6e37903`, the observed `78ba49d5` branch-head drift, marker-sweep artifact output, PR #126 fallback-template guard, the runtime-evidence manifest/preflight note, and clarifies that PR #126 HC-audit markers are required only for packages that include that instrumentation. The wiki still treats runtime/deploy evidence as pending. Wiki commit hashes are not release identities; use the package/git/archive tuple and RPT markers for release proof. | Wiki freshness and release-doc synchronization. |
...
9. Existing release PRs already agree on the main gate: source/static/package checks are not enough.
   ...
   - PR #125 now has package/handoff proof for `e3b6e37903` / `_MISSIONS.7z` SHA256 `3DB01AC1656329ECCAE9896CE9442680D5D904C563DD44590FFBD20954CF7B87`, but that is still package/tooling proof, not final runtime proof. PR #126 and PR #131 artifacts remain companion/tooling proof unless explicitly promoted into the active package lane.
...
11. Public stats and server-side stats are health context, not release proof.
   ...
   - This is encouraging live health evidence only. It does not bind to PR #125 package marker `e3b6e37903`, archive SHA `3DB01AC1656329ECCAE9896CE9442680D5D904C563DD44590FFBD20954CF7B87`, the ten required role RPTs, or the runtime packet manifest.
...
17. PR #125 source and package proof are refreshed to the same head.
   - The current package checkpoint commit is `e3b6e379032d65241f0f4da1aa72a333dcd0df67`.
   - The PR/wiki/package tuple records `_MISSIONS.7z` built from marker `e3b6e37903`, SHA256 `3DB01AC1656329ECCAE9896CE9442680D5D904C563DD44590FFBD20954CF7B87`, `1,885` entries, `7,162,113` bytes, and handoff status `ready_for_runtime_collection`.
   - Validation recorded on PR #125 includes LoadoutManager RELEASE rebuild, package gates, release handoff generation, wiki parse/diff checks, static smoke, final-check package-worktree hygiene, Chernarus/Takistan OA lint pass, high-only BugHunt clean, and the HC AICOM delegation PVF follow-up in both maintained `Client_HandlePVF.sqf` files.
   - Action: collect runtime evidence only against the `e3b6e37903` / `3DB01AC...` tuple unless the release owner explicitly promotes a newer package.

18. PR #126 now has a tuple guard for this active package checkpoint.
   - `Tools/Ops/Test-WaspReleaseTupleDocs.ps1` checks the PR #126 release ledger and monitor examples for current PR #125 branch head `78ba49d5`, expected package git marker `e3b6e37903`, archive SHA `3DB01AC1656329ECCAE9896CE9442680D5D904C563DD44590FFBD20954CF7B87`, package size `7,162,113`, wiki commit `1883a1b`, runtime-pending wording, and absence of stale `703F71E6...`, `40883616...`, `50AD...`, and `F057...` active-surface archive hashes.
...
20. PR #125 branch head advanced after the latest recorded package checkpoint.
   - GitHub now reports PR #125 at `78ba49d54065f20d9f13ec7242514a00ee4e4467`.
   - The delta from `e3b6e37903` to `78ba49d5` includes `c8ab1ff6 fix: guard fps report pv payloads` and `78ba49d5 fix: guard marker feed consumers`.
   - The changed files include both maintained terrains' `Client/FSM/updateaicommarkers.sqf`, `Client/FSM/updatepatrolmarkers.sqf`, `Server/FSM/server_side_patrols.sqf`, `Server/Init/Init_Server.sqf`, plus monitor/static-smoke tooling.
   - Impact: runtime proof against the literal PR #125 branch head should wait for a refreshed `_MISSIONS.7z` package and handoff tuple. Runtime proof against the existing `e3b6e37903` / `3DB01AC...` package is still valid only if the release owner intentionally chooses that checkpoint.
