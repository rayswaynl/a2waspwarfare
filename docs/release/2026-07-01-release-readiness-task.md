...
- PR #125, `[codex] Prepare WASP release command center`, branch `codex/release-command-center-20260630`: broad release command-center candidate with tooling, package provenance, AICOM work, and runtime collection gates. GitHub currently reports the branch at `7f81115edf6226791d2156b330b7b38652d7a989` after source/harness commits `9dd208d7b` and `7f81115ed`, while the last recorded package identity remains `c441d6f38df103e32694791f0bca5c5a70428c12` / marker `c441d6f38d`. The `c441d6f38d` package/handoff artifacts and public wiki commit `7ceb95b` record `_MISSIONS.7z` SHA256 `F057952E3DEDF4AB7D75FD1B3BEECFF4183506A7B83798241120B2F8D14B5F43`, `1,885` entries, `7,161,823` bytes, handoff status `ready_for_runtime_collection`. Because mission SQF changed after `c441d6f38d`, that package tuple is now a historical checkpoint until PR #125 regenerates package/handoff proof from `7f81115ed` or intentionally resets/promotes a different head. Exact Chernarus and Takistan runtime RPT proof is still pending, and GitHub reports PR #125 open, draft, clean, and mergeable against `master`.
...
| `rayswaynl/a2waspwarfare.wiki` | Cloned locally and synced through wiki commit `7ceb95b`, which records the PR #125 package tuple for `c441d6f38d`, marker-sweep artifact output, PR #126 fallback-template guard, and clarifies that PR #126 HC-audit markers are required only for packages that include that instrumentation. The wiki still treats runtime/deploy evidence as pending. After PR #125 advanced to `7f81115ed`, the c441 package tuple is a checkpoint until the package/handoff proof is refreshed or the release owner intentionally chooses that older head. Wiki commit hashes are not release identities; use the package/git/archive tuple and RPT markers for release proof. | Wiki freshness and release-doc synchronization. |
...
9. Existing release PRs already agree on the main gate: source/static/package checks are not enough.
   ...
   - PR #125 has package/handoff proof for the older `c441d6f38d` checkpoint / `_MISSIONS.7z` SHA256 `F057952E3DEDF4AB7D75FD1B3BEECFF4183506A7B83798241120B2F8D14B5F43`, but the branch has since advanced to `7f81115ed` with mission source changes. That makes c441 package/tooling proof historical until refreshed or intentionally selected; it is not final runtime proof. PR #126 and PR #131 artifacts remain companion/tooling proof unless explicitly promoted into the active package lane.
...
11. Public stats and server-side stats are health context, not release proof.
   ...
   - This is encouraging live health evidence only. It does not bind to current PR #125 head `7f81115ed`, a refreshed package marker/archive SHA for that head, the ten required role RPTs, or the runtime packet manifest. It also does not complete the older `c441d6f38d` package checkpoint.
...
17. PR #125 source advanced after the last package proof.
   - GitHub reports PR #125 branch `codex/release-command-center-20260630` at `7f81115edf6226791d2156b330b7b38652d7a989`.
   - The PR/wiki/package tuple still records `_MISSIONS.7z` built from `c441d6f38df103e32694791f0bca5c5a70428c12`.
   - The two newer commits change mission SQF and static smoke coverage (`9dd208d7b` and `7f81115ed`), so the c441 archive cannot be used as exact-build proof for the current branch head.
   - Action: before runtime collection, either regenerate `version.sqf`, `_MISSIONS.7z`, package manifest, handoff packet and wiki/PR tuple from `7f81115ed`, or intentionally choose the older `c441d6f38d` package as the runtime target and make the branch/package mismatch explicit in all operator notes.
...
- For the older c441 checkpoint only, use `Tools/Monitor/Get-WaspRptMarkerSweep.ps1` with `-ExpectedCandidate release-command-center-20260630 -ExpectedGit c441d6f38d -ExpectedArchiveSha256 F057952E3DEDF4AB7D75FD1B3BEECFF4183506A7B83798241120B2F8D14B5F43 -RequireReleaseMarkers -OutFile <redaction-safe-marker-sweep.json>` for redaction-safe first-pass livehost/archive RPT marker counts before deciding whether any private RPT copies are needed. If PR #125 head `7f81115ed` remains the runtime target, refresh the package tuple first and use the new expected git/archive SHA.
