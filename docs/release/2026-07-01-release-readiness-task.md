...
As of 2026-07-02 after the PR #125 Build83/cmdcon35, AICOM latch-reset, release-doc package-identity refresh, 78ba package refresh, and follow-up head check:
...
- PR #125, `[codex] Prepare WASP release command center`, branch `codex/release-command-center-20260630`: broad release command-center candidate with tooling, package provenance, AICOM work, and runtime collection gates. GitHub currently reports the branch at `062ab44ff5696af548e4fdcde85d4f9913a414bc`; the last recorded package checkpoint remains `78ba49d54065f20d9f13ec7242514a00ee4e4467`, marker `78ba49d540`, `_MISSIONS.7z` SHA256 `1C58A4A3FD2CBE90C7BF0F37062C1035D0DAD82C1875A801E639CC870EAC2C4B`, `1,885` entries, `7,162,733` bytes, handoff status `ready_for_runtime_collection`, and public wiki commit `e869bf0`. The post-package `062ab44f` commit touches maintained Chernarus/Takistan `Server_BuyUnit.sqf` and static-smoke tooling, so exact branch-head runtime collection requires a new package unless the owner explicitly selects the `78ba49d540` checkpoint. Exact Chernarus and Takistan runtime RPT proof is still pending, and GitHub reports PR #125 open, draft, clean, and mergeable against `master`.
...
| `rayswaynl/a2waspwarfare.wiki` | Public remote master is at wiki commit `e869bf0`, which records the current PR #125 package tuple for `78ba49d540`, the observed `062ab44f` branch-head drift, marker-sweep artifact output, PR #126 fallback-template guard, the runtime-evidence manifest generator/checker and preflight note, and clarifies that PR #126 HC-audit markers are required only for packages that include that instrumentation. The wiki still treats runtime/deploy evidence as pending. Wiki commit hashes are not release identities; use the package/git/archive tuple and RPT markers for release proof. | Wiki freshness and release-doc synchronization. |
...
11. Public stats and server-side stats are health context, not release proof.
   ...
   - This is encouraging live health evidence only. It does not bind to PR #125 package marker `78ba49d540`, archive SHA `1C58A4A3FD2CBE90C7BF0F37062C1035D0DAD82C1875A801E639CC870EAC2C4B`, the ten required role RPTs, or the runtime packet manifest.
...
17. PR #125 source and package proof are refreshed to the same head.
   - The current package checkpoint commit is `78ba49d54065f20d9f13ec7242514a00ee4e4467`.
   - The PR/wiki/package tuple records `_MISSIONS.7z` built from marker `78ba49d540`, SHA256 `1C58A4A3FD2CBE90C7BF0F37062C1035D0DAD82C1875A801E639CC870EAC2C4B`, `1,885` entries, `7,162,733` bytes, and handoff status `ready_for_runtime_collection`.
   ...
   - Action: collect runtime evidence only against the `78ba49d540` / `1C58A4...` tuple unless the release owner explicitly promotes a newer package.

18. PR #126 now has a tuple guard for this active package checkpoint.
   - `Tools/Ops/Test-WaspReleaseTupleDocs.ps1` checks the PR #126 release ledger and monitor examples for current PR #125 branch head `062ab44f`, expected package git marker `78ba49d540`, archive SHA `1C58A4A3FD2CBE90C7BF0F37062C1035D0DAD82C1875A801E639CC870EAC2C4B`, package size `7,162,733`, wiki commit `e869bf0`, runtime-pending wording, and absence of stale `3DB01AC...`, `703F71E6...`, `40883616...`, `50AD...`, and `F057...` active-surface archive hashes.
...
19. Marker sweeps needed a matrix-level proof wrapper.
   ...
   - Action: PR #126 now adds `Tools/Monitor/Test-WaspRuntimeEvidenceManifest.ps1`, `New-WaspRuntimeEvidenceManifestTemplate.ps1`, and their self-tests. The template generator writes the full default manifest skeleton so operators do not hand-copy ten rows. The manifest checker requires one marker-sweep JSON per `chernarus,takistan` x `server,hc1,hc2,start-client,late-jip` slot by default, verifies each sweep matches candidate `release-command-center-20260630`, git marker `78ba49d540`, archive SHA `1C58A4A3FD2CBE90C7BF0F37062C1035D0DAD82C1875A801E639CC870EAC2C4B`, has no missing required markers, and contains the terrain release marker for its slot. When the manifest includes a top-level release tuple, the checker validates that too. Use narrower required terrain/role arguments only if the release owner explicitly reduces the runtime evidence matrix.

20. PR #125 branch head advanced after the latest recorded package checkpoint.
   - GitHub now reports PR #125 at `062ab44ff5696af548e4fdcde85d4f9913a414bc`.
   - The delta from `78ba49d540` to `062ab44f` is `062ab44f fix: guard factory queue empty head`.
   - The changed files include both maintained terrains' `Server/Functions/Server_BuyUnit.sqf`, plus `JOURNAL.md` and static-smoke tooling.
   - Impact: runtime proof against the literal PR #125 branch head should wait for a refreshed `_MISSIONS.7z` package and handoff tuple. Runtime proof against the existing `78ba49d540` / `1C58A4...` package is still valid only if the release owner intentionally chooses that checkpoint.
   - Action: keep `Tools/Ops/Test-WaspReleaseTupleDocs.ps1` guarding both the current PR #125 branch head and the last package checkpoint until PR #125 publishes a new package tuple.
...
- Use `Tools/Monitor/Get-WaspRptMarkerSweep.ps1` with `-ExpectedCandidate release-command-center-20260630 -ExpectedGit 78ba49d540 -ExpectedArchiveSha256 1C58A4A3FD2CBE90C7BF0F37062C1035D0DAD82C1875A801E639CC870EAC2C4B -RequireReleaseMarkers -OutFile <redaction-safe-marker-sweep.json>` for redaction-safe first-pass livehost/archive RPT marker counts before deciding whether any private RPT copies are needed.
- Use `Tools/Monitor/New-WaspRuntimeEvidenceManifestTemplate.ps1` to build a redaction-safe `a2waspwarfare-runtime-evidence-manifest-v1` manifest that maps each terrain/role slot to its marker-sweep JSON, then run `Tools/Monitor/Test-WaspRuntimeEvidenceManifest.ps1` against the active package tuple before declaring runtime proof complete.
