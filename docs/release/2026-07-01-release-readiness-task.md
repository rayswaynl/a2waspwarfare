...
- PR #125, `[codex] Prepare WASP release command center`, branch `codex/release-command-center-20260630`: broad release command-center candidate with tooling, package provenance, AICOM work, and runtime collection gates. Current source/package head observed: `153a513fb6511a4154f2ed825ba9ff2578c070e7` / marker `153a513fb6`. The latest deltas remove the uncompiled AICOM HC top-up draft worker/wiring from both maintained terrains, remove stale supervisor private declarations, and tighten the static smoke guard for forbidden draft tokens. PR #125 and the public wiki now record `_MISSIONS.7z` SHA256 `CEAFADFE1661D814B521DD7A29B24C351D2CF1451C74749DFCF446B0EBAD25B4`, `1,877` entries, `7,133,822` bytes, handoff status `ready_for_runtime_collection`. This proves package identity only; exact Chernarus and Takistan runtime RPT proof is still pending.
...
| `rayswaynl/a2waspwarfare.wiki` | Cloned locally and synced through wiki commit `2ec2f5f`, which records the PR #125 package tuple for `153a513fb6` after refresh commit `96d10f1`: `_MISSIONS.7z` SHA256 `CEAFADFE1661D814B521DD7A29B24C351D2CF1451C74749DFCF446B0EBAD25B4`, `1,877` entries, `7,133,822` bytes, handoff `ready_for_runtime_collection`. The wiki still treats runtime/deploy evidence as pending. Wiki commit hashes are not release identities; use the package/git/archive tuple and RPT markers for release proof. | Wiki freshness and release-doc synchronization. |
...
9. Existing release PRs already agree on the main gate: source/static/package checks are not enough.
   ...
   - PR #125 now has package/handoff proof for `153a513fb6` / `_MISSIONS.7z` SHA256 `CEAFADFE1661D814B521DD7A29B24C351D2CF1451C74749DFCF446B0EBAD25B4`, but that is still package/tooling proof, not final runtime proof. PR #126 and PR #131 artifacts remain companion/tooling proof unless explicitly promoted into the active package lane.
