...
- PR #125, `[codex] Prepare WASP release command center`, branch `codex/release-command-center-20260630`: broad release command-center candidate with tooling, package provenance, AICOM work, and runtime collection gates. Current package identity observed from the PR/wiki handoff: `c441d6f38df103e32694791f0bca5c5a70428c12` / marker `c441d6f38d`. The latest mission deltas add AICOM capture-stall and assault arrival-gate guardrails, marker-helper window/literal-marker hardening, and refreshed package provenance while preserving Build83/cmdcon35 reconciliation, HCTopUp draft exclusion, AICOM/PVF authority hardening, lifecycle/refund hardening, and the active commander handoff latch reset in both maintained terrains. PR #125 package/handoff artifacts and public wiki commit `8746d9c` now record `_MISSIONS.7z` SHA256 `F057952E3DEDF4AB7D75FD1B3BEECFF4183506A7B83798241120B2F8D14B5F43`, `1,885` entries, `7,161,823` bytes, handoff status `ready_for_runtime_collection`. This proves package identity only; exact Chernarus and Takistan runtime RPT proof is still pending, and GitHub reports PR #125 open, draft, clean, and mergeable against `master`.
...
| `rayswaynl/a2waspwarfare.wiki` | Cloned locally and synced through wiki commit `8746d9c`, which records the PR #125 package tuple for `c441d6f38d`, marker-sweep artifact output, and clarifies that PR #126 HC-audit markers are required only for packages that include that instrumentation. The wiki still treats runtime/deploy evidence as pending. Wiki commit hashes are not release identities; use the package/git/archive tuple and RPT markers for release proof. | Wiki freshness and release-doc synchronization. |
...
9. Existing release PRs already agree on the main gate: source/static/package checks are not enough.
   ...
   - PR #125 now has package/handoff proof for `c441d6f38d` / `_MISSIONS.7z` SHA256 `F057952E3DEDF4AB7D75FD1B3BEECFF4183506A7B83798241120B2F8D14B5F43`, but that is still package/tooling proof, not final runtime proof. PR #126 and PR #131 artifacts remain companion/tooling proof unless explicitly promoted into the active package lane.
...
11. Public stats and server-side stats are health context, not release proof.
   ...
   - This is encouraging live health evidence only. It does not bind to PR #125 package marker `c441d6f38d`, archive SHA `F057952E3DEDF4AB7D75FD1B3BEECFF4183506A7B83798241120B2F8D14B5F43`, the ten required role RPTs, or the runtime packet manifest.
