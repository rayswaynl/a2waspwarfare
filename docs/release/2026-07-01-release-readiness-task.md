...
- PR #125, `[codex] Prepare WASP release command center`, branch `codex/release-command-center-20260630`: broad release command-center candidate with tooling, package provenance, AICOM work, and runtime collection gates. GitHub currently reports the branch at `9e628781a8a0e6ed9089d55f5226c2b03031bece`; the recorded package checkpoint matches that head, marker `9e628781a8`, `_MISSIONS.7z` SHA256 `FB52623DAC5CF93EE8E4E9E2B1542E1A531C5EF0FE063AADED33892439027245`, `1,885` entries, `7,166,946` bytes, handoff status `ready_for_runtime_collection`, and public wiki commit `0cc616e`. Exact Chernarus and Takistan runtime RPT proof is still pending, and GitHub reports PR #125 open, draft, clean, and mergeable against `master`.
...
| `rayswaynl/a2waspwarfare.wiki` | Public remote master is at wiki commit `0cc616e`, which records the current PR #125 package tuple for `9e628781a8`, marker-sweep artifact output, PR #126 fallback-template guard, the runtime-evidence manifest generator/checker with role/terrain stamps and optional package artifact hashing, and clarifies that PR #126 HC-audit markers are required only for packages that include that instrumentation. The wiki still treats runtime/deploy evidence as pending. Wiki commit hashes are not release identities; use the package/git/archive tuple and RPT markers for release proof. | Wiki freshness and release-doc synchronization. |
...
11. Public stats and server-side stats are health context, not release proof.
   ...
   - This is encouraging live health evidence only. It does not bind to PR #125 package marker `9e628781a8`, archive SHA `FB52623DAC5CF93EE8E4E9E2B1542E1A531C5EF0FE063AADED33892439027245`, the ten required role RPTs, or the runtime packet manifest.
...
17. PR #125 source and package proof are refreshed to the same head.
   - The current package checkpoint commit is `9e628781a8a0e6ed9089d55f5226c2b03031bece`.
   - The PR/wiki/package tuple records `_MISSIONS.7z` built from marker `9e628781a8`, SHA256 `FB52623DAC5CF93EE8E4E9E2B1542E1A531C5EF0FE063AADED33892439027245`, `1,885` entries, `7,166,946` bytes, and handoff status `ready_for_runtime_collection`.
   ...
   - Action: collect runtime evidence only against the `9e628781a8` / `FB52623D...` tuple unless the release owner explicitly promotes a newer package.

18. PR #126 now has a tuple guard for this active package checkpoint.
   - `Tools/Ops/Test-WaspReleaseTupleDocs.ps1` checks the PR #126 release ledger and monitor examples for current PR #125 branch head `9e628781`, expected package git marker `9e628781a8`, archive SHA `FB52623DAC5CF93EE8E4E9E2B1542E1A531C5EF0FE063AADED33892439027245`, package size `7,166,946`, wiki commit `0cc616e`, runtime-pending wording, and absence of stale `190475B4...`, `1C58A4A3...`, `3DB01AC...`, `703F71E6...`, `40883616...`, `50AD...`, `F057...`, `D323...`, `76A5EE56...`, `E887E792...`, `8D06CDA3...`, `35B864...`, `D5CA434E...`, `ECC6F9D5...`, `F4F19086...`, `0AC507E1...`, `22F5AEE4...`, and `E7B12EDA...` active-surface archive hashes.
...
- Continue requester-bound authority review for any remaining tactical support cases not already covered by the current `9e628781a8` ICBM, supply, SCUD, GUER mortar, player support/UAV, supply-truck respawn, commander-support tuple/logging/ownership/default, camp repair, GUER VBIED, and GUER FOB guard set; keep runtime proof pending for server-owned payment, cooldown, funds, active-state, repair, and support behavior, and keep construction/defense PVFs plus `RequestOnUnitKilled` in the open authority backlog.
