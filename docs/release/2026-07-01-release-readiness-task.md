...
- PR #125, `[codex] Prepare WASP release command center`, branch `codex/release-command-center-20260630`: broad release command-center candidate with tooling, package provenance, AICOM work, and runtime collection gates. GitHub currently reports the branch and package checkpoint at `78ba49d54065f20d9f13ec7242514a00ee4e4467`, marker `78ba49d540`, `_MISSIONS.7z` SHA256 `1C58A4A3FD2CBE90C7BF0F37062C1035D0DAD82C1875A801E639CC870EAC2C4B`, `1,885` entries, `7,162,733` bytes, handoff status `ready_for_runtime_collection`, and public wiki commit `3f85c6b`. Exact Chernarus and Takistan runtime RPT proof is still pending, and GitHub reports PR #125 open, draft, clean, and mergeable against `master`.
...
| `rayswaynl/a2waspwarfare.wiki` | Public remote master is at wiki commit `3f85c6b`, which records the current PR #125 package tuple for `78ba49d540`, marker-sweep artifact output, PR #126 fallback-template guard, the runtime-evidence manifest generator/checker and preflight note, and clarifies that PR #126 HC-audit markers are required only for packages that include that instrumentation. The wiki still treats runtime/deploy evidence as pending. Wiki commit hashes are not release identities; use the package/git/archive tuple and RPT markers for release proof. | Wiki freshness and release-doc synchronization. |
...
18. PR #126 now has a tuple guard for this active package checkpoint.
   - `Tools/Ops/Test-WaspReleaseTupleDocs.ps1` checks the PR #126 release ledger and monitor examples for current PR #125 branch head `78ba49d5`, expected package git marker `78ba49d540`, archive SHA `1C58A4A3FD2CBE90C7BF0F37062C1035D0DAD82C1875A801E639CC870EAC2C4B`, package size `7,162,733`, wiki commit `3f85c6b`, runtime-pending wording, and absence of stale `3DB01AC...`, `703F71E6...`, `40883616...`, `50AD...`, and `F057...` active-surface archive hashes.
...
20. PR #125 branch-head drift was resolved by a refreshed package checkpoint.
   - GitHub now reports PR #125 at `78ba49d54065f20d9f13ec7242514a00ee4e4467`, and PR #125's release identity now records the same head.
   - The refreshed package includes `c8ab1ff6 fix: guard fps report pv payloads` and `78ba49d5 fix: guard marker feed consumers`.
   - The package tuple is marker `78ba49d540`, SHA256 `1C58A4A3FD2CBE90C7BF0F37062C1035D0DAD82C1875A801E639CC870EAC2C4B`, `1,885` entries, `7,162,733` bytes, handoff `ready_for_runtime_collection`.
   - Impact: final runtime evidence can now target the literal PR #125 branch head again, but only after collecting the exact Chernarus/Takistan RPT matrix for this refreshed tuple.
   - Action: keep `Tools/Ops/Test-WaspReleaseTupleDocs.ps1` pointed at `78ba49d540` / `1C58A4...` until PR #125 promotes a newer package tuple.
