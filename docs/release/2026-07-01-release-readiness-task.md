...
- PR #125, `[codex] Prepare WASP release command center`, branch `codex/release-command-center-20260630`: broad release command-center candidate with tooling, package provenance, AICOM work, and runtime collection gates. GitHub currently reports the branch and package checkpoint at `e3b6e379032d65241f0f4da1aa72a333dcd0df67` after an HC AICOM delegation PVF follow-up; marker `e3b6e37903`; `_MISSIONS.7z` SHA256 `3DB01AC1656329ECCAE9896CE9442680D5D904C563DD44590FFBD20954CF7B87`, `1,885` entries, `7,162,113` bytes, handoff status `ready_for_runtime_collection`, and public wiki commit `069020f`. Exact Chernarus and Takistan runtime RPT proof is still pending, and GitHub reports PR #125 open, draft, clean, and mergeable against `master`.
...
| `rayswaynl/a2waspwarfare.wiki` | Public remote master is at wiki commit `069020f`, which records the current PR #125 package tuple for `e3b6e37903`, marker-sweep artifact output, PR #126 fallback-template guard, the runtime-evidence manifest/preflight note, and clarifies that PR #126 HC-audit markers are required only for packages that include that instrumentation. The wiki still treats runtime/deploy evidence as pending. Wiki commit hashes are not release identities; use the package/git/archive tuple and RPT markers for release proof. | Wiki freshness and release-doc synchronization. |
