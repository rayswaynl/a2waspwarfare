...
- PR #125, `[codex] Prepare WASP release command center`, branch `codex/release-command-center-20260630`: broad release command-center candidate with tooling, package provenance, AICOM work, and runtime collection gates. Current package identity observed from the PR/wiki handoff: `f7a5e507398463985ab2021c8591b1b74beabc66` / marker `f7a5e50739`. The latest mission deltas add AICOM capture-stall and assault arrival-gate guardrails, marker-helper window/literal-marker hardening, and refreshed package provenance while preserving Build83/cmdcon35 reconciliation, HCTopUp draft exclusion, AICOM/PVF authority hardening, lifecycle/refund hardening, and the active commander handoff latch reset in both maintained terrains. PR #125 package/handoff artifacts and public wiki commit `8746d9c` now record `_MISSIONS.7z` SHA256 `5F542D7F2113BEE21B8E1F3D5CF2651EC6F0B9ED5A6865ADF67FDC2D7357269C`, `1,885` entries, `7,161,625` bytes, handoff status `ready_for_runtime_collection`. This proves package identity only; exact Chernarus and Takistan runtime RPT proof is still pending, and GitHub reports PR #125 open, draft, clean, and mergeable against `master`.
...
| `rayswaynl/a2waspwarfare.wiki` | Cloned locally and synced through wiki commit `8746d9c`, which records the PR #125 package tuple for `f7a5e50739`, marker-sweep artifact output, and clarifies that PR #126 HC-audit markers are required only for packages that include that instrumentation. The wiki still treats runtime/deploy evidence as pending. Wiki commit hashes are not release identities; use the package/git/archive tuple and RPT markers for release proof. | Wiki freshness and release-doc synchronization. |
...
11. Public stats and server-side stats are health context, not release proof.
   ...
   - This is encouraging live health evidence only. It does not bind to PR #125 package marker `f7a5e50739`, archive SHA `5F542D7F2113BEE21B8E1F3D5CF2651EC6F0B9ED5A6865ADF67FDC2D7357269C`, the ten required role RPTs, or the runtime packet manifest.
