...
| --- | --- | --- | --- | --- | --- | --- |
| PVF dispatch | Any `WFBE_CO_FNC_SendToServer`, `SendToClient` or `SendToClients` command | `Server_HandlePVF.sqf:14`, `Client_HandlePVF.sqf:22`, registry in `Init_PublicVariables.sqf:23-51` | Dispatchers run `Spawn (Call Compile _script)` on command strings from the PV payload; init already compiles `SRVFNC*` and `CLTFNC*`. | Validated handler lookup in `missionNamespace`. | Use `missionNamespace getVariable _script`, then OA-safe `isNil` and `typeName == "CODE"` guards; reject non-`CODE` and unregistered names, keep `Spawn` until scheduled handler needs are audited. | Valid `RequestJoin` or `RequestVehicleLock`, valid `LocalizeMessage` or `HandleSpecial`, bogus handler rejected and logged once. |
