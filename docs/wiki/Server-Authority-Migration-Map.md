...
This table is the post-dispatch work queue. Current stable has stopped compiling arbitrary sender-provided handler strings, but the dispatcher still needs explicit registered-handler membership before it is fully closed. These registered commands still need their own payload, requester and authority checks because a valid command name can still carry forged state.

Registration source: current stable `Common/Init/Init_PublicVariables.sqf:9-23` registers server-bound commands including `RequestEnqueue` / `RequestDequeue`, while `:56,61` compile each handler and add the server/client `WFBE_PVF_<Command>` PVEHs.
...
Patch boundary: the current stable PVF namespace lookup is a prerequisite, not the finish line. An explicit registered-handler allowlist/logging path should still reject forged handler names, and it still will not validate score values, vehicle objects, team arrays, side scalars, upgrade ids or delegation UID/FPS payloads inside legitimate handlers.
...
| --- | --- | --- | --- | --- | --- | --- |
| PVF dispatch | Any `WFBE_CO_FNC_SendToServer`, `SendToClient` or `SendToClients` command | Current stable `Server_HandlePVF.sqf:14-15`, `Client_HandlePVF.sqf:32-33`, registry in `Init_PublicVariables.sqf:9-23,28-52,56,61` | Current stable resolves `_script` with `missionNamespace getVariable` and spawns only a resolved `CODE`, so dispatch-time text compile is gone there. The dispatcher still does not prove `_script` belongs to the registered `SRVFNC*` / `CLTFNC*` set before spawning. | Validated registered-handler lookup in `missionNamespace`. | Add explicit server/client allowed-handler membership and reject non-registered names before `Spawn _code`; keep `Spawn` until scheduled handler needs are audited. For Miksuu/perf/historical-release branches still using `Call Compile`, port the current stable lookup first. | Valid `RequestJoin` or `RequestVehicleLock`, valid `LocalizeMessage` or `HandleSpecial`, forged unregistered handler name rejected and logged once. |
| PVF sender authentication (DR-55) | Any client-to-server `WFBE_PVF_*` request or direct authority PV | `Init_PublicVariables.sqf:51-53` forwards only `(_this select 1)` into `WFBE_SE_FNC_HandlePVF`; `Server_HandlePVF.sqf:9-14` forwards only handler parameters. | The effect handler sees payload data but not an authenticated publisher identity, so legitimate handler names can still carry forged side/team/object/funds fields. | Server requester/context authority. | Carry authenticated requester context to server handlers (owner/UID/player object, cross-checked against the PVEH origin or a server-owned session table), then re-derive side, commander/team role, funds and object ownership inside each handler. Keep this separate from the handler-name allowlist patch. | From a non-owning/enemy client, forged `RequestVehicleLock`, `RequestChangeScore`, `RequestTeamUpdate`, `RequestSpecial`, attack-wave detail and supply-completion payloads reject; valid owner/commander/support flows still work. |
