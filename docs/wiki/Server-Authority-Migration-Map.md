...
2026-06-22 DR-55 branch refresh: current docs/source `HEAD@4d4610f1` is unchanged from `ade4d356` for checked PVF init/dispatcher paths and still registers value-only PVEHs at `Common/Init/Init_PublicVariables.sqf:46,51`, dispatching through `Server_HandlePVF.sqf:14` / `Client_HandlePVF.sqf:22` `Call Compile`; Miksuu `b8389e748243` and `origin/perf/quick-wins@0076040f` match that old dispatcher shape in checked maintained roots. Current stable `origin/master@0139a346` and current B69 `origin/claude/b69@8d465fce` replace dispatch-time compile with `missionNamespace getVariable` + `CODE` guards at server `:14-15` and client `:32-33`, but their `Init_PublicVariables.sqf:56,61` registrations still pass only `(_this select 1)` to the PVF dispatcher. The B69 `0a1ccb4d..8d465fce` delta touches `Client/PVFunctions/HandleSpecial.sqf` in both maintained roots and adds default-off `aicom-team-merge` at `:57` / `WFBE_C_AICOM_HC_MERGE_ENABLE` gate at `:59`; it does not add authenticated requester context to the server PVF path. BIKI's [`addPublicVariableEventHandler`](https://community.bistudio.com/wiki/addPublicVariableEventHandler) page documents the callback value as event data; Wasp currently discards even the variable-name part before the server handler layer, so DR-55 remains a separate requester-context migration from the handler-name allowlist patch.
