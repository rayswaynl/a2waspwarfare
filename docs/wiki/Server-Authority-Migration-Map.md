...
This table is the post-dispatch work queue. On current `origin/master@0139a346`, the generic PVF dispatcher has stopped compiling arbitrary sender-provided handler strings and now uses `missionNamespace getVariable` plus `CODE` guards; it still needs explicit registered-handler allowlists/logging. These registered commands also need their own payload, requester and authority checks because a valid command name can still carry forged state.

Registration source on current stable: `Common/Init/Init_PublicVariables.sqf:9-27` registers 19 server-bound commands, while `:60-61` compiles each `SRVFNC*` handler and adds `WFBE_PVF_<Command>` server PVEHs.
...
| Attack waves | Docs/source `Client/FSM/updateclient.sqf:240`; current stable/B69 `Client/FSM/updateclient.sqf:260`; `Common/Functions/Common_AttackWaveActivate.sqf:3-8` | `Server_AttackWave.sqf:1-38`, `Server/PVFunctions/AttackWave.sqf:1-62` | Docs/source `HEAD@1c1ea559`, current stable `origin/master@0139a346` and current B69 `origin/claude/b69@8d465fce` all keep `ATTACK_WAVE_INIT = [_supply, _side]`; server uses payload `_supply` to compute discount/duration and `ATTACK_WAVE_DETAILS` also trusts `_side`, `_priceModifier` and `_attackLength`. B69 checked attack-wave request/detail/client-price deltas from `0a1ccb4d` and `b8530477` are empty; adjacent B74 `origin/claude/b74-aicom-spend@b23f557fc912` changes only Chernarus constants among checked attack-wave paths and keeps the same request/detail trust. | Server support-mode authority. | Treat direct PVs as request/detail surfaces; server re-derives side supply, validates requester side/permission, deducts intended cost and clamps modifier/duration before starting wave, then protects the detail handler from forged payloads. | Legit wave starts; forged exaggerated supply/detail payload cannot change modifier, duration, side or cost; wrong-side requests reject; late-join attack-wave state remains documented. |
