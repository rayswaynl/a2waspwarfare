...
2026-06-22 DR-55 branch refresh: current docs branch `HEAD@ade4d356` Chernarus still registers value-only PVEHs at `Common/Init/Init_PublicVariables.sqf:46,51` and dispatches through `Server_HandlePVF.sqf:14` / `Client_HandlePVF.sqf:22` `Call Compile`; Miksuu `b8389e74` and `origin/perf/quick-wins@0076040f` match that old dispatcher shape in checked maintained roots. Current stable `origin/master@0139a346` and B69 `origin/claude/b69@0a1ccb4d` replace dispatch-time compile with `missionNamespace getVariable` + `CODE` guards at server `:14-15` and client `:32-33`, but their `Init_PublicVariables.sqf:56,61` registrations still pass only `(_this select 1)` to the PVF dispatcher. BIKI's [`addPublicVariableEventHandler`](https://community.bistudio.com/wiki/addPublicVariableEventHandler) page documents the callback value as event data; Wasp currently discards even the variable-name part before the server handler layer, so DR-55 remains a separate requester-context migration from the handler-name allowlist patch.
