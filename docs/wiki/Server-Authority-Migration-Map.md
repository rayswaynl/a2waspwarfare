...
| Attack waves | Docs/source `Client/FSM/updateclient.sqf:240`; current stable/B74.1/B69/B74 `Client/FSM/updateclient.sqf:260`; `Common/Functions/Common_AttackWaveActivate.sqf:3-8` | `Server_AttackWave.sqf:1-38`, `Server/PVFunctions/AttackWave.sqf:1-62` | Docs/source `HEAD@a764d51107de`, current stable/B74.1 `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`, current B69 `8d465fce` and adjacent B74 `b23f557f` all keep `ATTACK_WAVE_INIT = [_supply, _side]`; server uses payload `_supply` to compute discount/duration and `ATTACK_WAVE_DETAILS` also trusts `_side`, `_priceModifier` and `_attackLength` (`Server_AttackWave.sqf:5-6,15,23,27,36,38`; `AttackWave.sqf:19,23-25,40`). Scoped `0139a3468609..f8a76de34`, B69..B74 and B74..B74.1 attack-wave path deltas only touch constants, not request/detail trust. | Server support-mode authority. | Treat direct PVs as request/detail surfaces; server re-derives side supply, validates requester side/permission, deducts intended cost and clamps modifier/duration before starting wave, then protects the detail handler from forged payloads. | Legit wave starts; forged exaggerated supply/detail payload cannot change modifier, duration, side or cost; wrong-side requests reject; late-join attack-wave state remains documented. |
