...
| Harden dispatch before payloads | Current stable has the PVF namespace lookup/`CODE` guard; finish explicit registered-handler membership before deeper per-handler rewrites. |
...
| Commander vote/reassignment semantics | [Commander vote/reassignment](Commander-Vote-And-Reassignment-Playbook) |
...
This table is the post-dispatch work queue. On current `origin/master@f8a76de34`, the same commit as `origin/claude/b74.1-aicom@f8a76de34`, the generic PVF dispatcher has stopped compiling arbitrary sender-provided handler strings and now uses `missionNamespace getVariable` plus `CODE` guards; it still needs explicit registered-handler allowlists/logging. These registered commands also need their own payload, requester and authority checks because a valid command name can still carry forged state.

Registration source on current stable/B74.1: `Common/Init/Init_PublicVariables.sqf:9-27` registers 19 server-bound commands, while `:60-61` compiles each `SRVFNC*` handler and adds value-only `WFBE_PVF_<Command>` server PVEHs.
...
Patch boundary: the PVF dispatcher namespace lookup is now source-present on current stable, but the registered-handler allowlist/logging part remains open. Even after that final dispatcher guard, it will not validate score values, vehicle objects, team arrays, side scalars, upgrade ids or delegation UID/FPS payloads inside legitimate handlers.
...
2026-06-23 DR-55 current-B74.1 refresh: current docs/source `HEAD@925d56e01a1e` is unchanged from `a0721301d4f5` for checked PVF init/dispatcher paths and still registers value-only PVEHs at `Common/Init/Init_PublicVariables.sqf:46,51`, dispatching through `Server_HandlePVF.sqf:14` / `Client_HandlePVF.sqf:22` `Call Compile`; Miksuu `b8389e748243` and `origin/perf/quick-wins@0076040f` match that old dispatcher shape in checked maintained roots. Current stable `origin/master@f8a76de34` equals `origin/claude/b74.1-aicom@f8a76de34`; it still registers value-only PVEHs at `Init_PublicVariables.sqf:56,61` and uses `missionNamespace getVariable` + `CODE` guards at server `Server_HandlePVF.sqf:14-15` and client `Client_HandlePVF.sqf:32-33`. Current B69 `origin/claude/b69@8d465fce` and adjacent B74 `origin/claude/b74-aicom-spend@b23f557f` match that generic PVF boundary; the checked B69..B74 and B74..B74.1 generic PVF dispatcher/init deltas are empty. No allowlist/sender/requester symbol was found in checked current-stable/B69/B74 dispatcher/init files. BIKI's [`addPublicVariableEventHandler`](https://community.bistudio.com/wiki/addPublicVariableEventHandler) page documents the callback value as event data; Wasp currently discards even the variable-name part before the server handler layer, so DR-55 remains a separate requester-context migration from the handler-name allowlist patch.
...
| `RequestCommanderVote` | Starts side vote when side logic `wfbe_votetime <= 0`, using payload side/name (`RequestCommanderVote.sqf:14-22`). | Payload-side workflow trigger. | Validate requester belongs to the side, derive name from requester, and rate-limit or ignore duplicate vote starts. Keep this trigger hardening separate from DR-47 vote-resolution semantics in `Server_VoteForCommander.sqf:24-29,43`; use [Commander vote/reassignment](Commander-Vote-And-Reassignment-Playbook) for the vote matrix. |
| `RequestNewCommander` | Sets side logic `wfbe_commander`, spawns commander assignment and broadcasts `new-commander-assigned` (`RequestNewCommander.sqf:12-14`). | High-impact commander mutation. | Accept only from the server vote/assignment path or validate requester authority, side, candidate team membership and vote state. Keep call-shape/notification/UI identity smoke tied to [Commander vote/reassignment](Commander-Vote-And-Reassignment-Playbook). |
...
| `RequestMHQRepair` | Forwards side-only payload to `MHQRepair` (`RequestMHQRepair.sqf:1`). | HQ repair/recovery authority gap. | Include requester and repair/recovery context; server validates dead HQ, side, vehicle/range/class, repair count, funds and one-time WASP recovery flags, then takes a server-side repair mutex before creating a replacement HQ. |
| `RequestSpecial` | Spawns `HandleSpecial`, which multiplexes group, support, UAV, ICBM, HC, town delegation, camp repair, AICOM/patrol bookkeeping and other tags (`RequestSpecial.sqf:1`; current B74.1 `Server_HandleSpecial.sqf:5-520`). | Broad mixed-risk router; ICBM is P0. | Split by tag. Harden `"ICBM"` first; then validate support fees/upgrades/cooldowns, HC identity, town-delegation vehicle ownership, camp repair side/object state, AICOM/patrol bookkeeping payloads and HQ-kill payload shape. |
...
| --- | --- | --- |
| Bookkeeping: `update-teamleader`, `group-query`, `upgrade-sync`, `update-clientfps`, `track-playerobject` | Mutates teamleader, group join, upgrade sync flags, HC/client FPS records or player-object lists from payloads (current B74.1 `Server_HandleSpecial.sqf:6-41,77-93,469-480`). Depth scout 2026-06-04 confirmed `group-query` trusts payload `_group`, `_player` and `_side`; when the target leader is AI and the group lacks `wfbe_uid`, it calls `Common_ChangeUnitGroup.sqf:3-11` and moves the payload player into that group (`Server_HandleSpecial.sqf:13-31`). | Shape-check objects/UIDs/groups and ensure the requester can speak for the team/player record; for `group-query`, derive the requester/player relationship and side server-side before any group move. |
| Support effects: `Paratroops`, `ParaVehi`, `ParaAmmo`, `uav`, `RespawnST`, `repair-camp` | The client tactical/UAV paths own fee, upgrade and cooldown checks locally (`GUI_Menu_Tactical.sqf:262-276,371-373,513-527`; `Client/Module/UAV/uav.sqf:27-52`), while the server spawns support scripts, kills supply trucks or recreates camp bunkers from payload side/position/team/object state (current B74.1 `Server_HandleSpecial.sqf:43-71,483-504`). 2026-06-04 support scout refinements: paratroopers create units into payload `_playerTeam`; `RespawnST` can damage the payload side's `wfbe_ai_supplytrucks`; `repair-camp` trusts payload camp and side; old UAV cleanup tracks a client-created object. | Recompute requester, side, role/commander status, funds, cooldown, upgrade, target object/team and destination bounds server-side before effect. Keep this non-ICBM lane separate from the P0 nuke patch. |
| Delegation/HC: `update-town-delegation`, `connected-hc` | Registers HC groups and appends delegated vehicles to town active-vehicle state (current B74.1 `Server_HandleSpecial.sqf:96-124,427-463`). | Validate HC identity/owner and ensure delegated vehicle lists belong to the expected town/HC handoff. |
| HQ/endgame-sensitive: `process-killed-hq` | Forwards payload killed event into `WFBE_SE_FNC_OnHQKilled` (current B74.1 `Server_HandleSpecial.sqf:144-145`). | Validate killed object is the side HQ/MHQ being tracked and reject duplicate killed-event processing. |
| P0 special weapon: `ICBM` | Trusts side/base/target/team payload, waits for target death and spawns `NukeDammage` (current B74.1 `Server_HandleSpecial.sqf:127-135`). | Use [ICBM authority](ICBM-Authority-Playbook) before any other `RequestSpecial` hardening claim. |

## Migration Table

| Flow | Client entrypoint | Current server/current handler | Current trust | Target authority owner | Patch shape | First validation |
| --- | --- | --- | --- | --- | --- | --- |
| PVF dispatch | Any `WFBE_CO_FNC_SendToServer`, `SendToClient` or `SendToClients` command | Current stable/B74.1/B69/B74 `Server_HandlePVF.sqf:14-15`, `Client_HandlePVF.sqf:32-33`, registry in `Init_PublicVariables.sqf:55-61`; docs/source, Miksuu/perf/historical refs still need the old `Call Compile` replacement. | Current stable/B74.1/B69/B74 dispatchers use `missionNamespace getVariable _script` and spawn only resolved `CODE`, but they do not prove the name is one of the registered `SRVFNC*` / `CLTFNC*` handlers and do not log rejected names. | Registered handler lookup. | Add explicit allowlists exported from `Init_PublicVariables.sqf`, reject non-registered or non-`CODE` names with a compact warning log, and keep `Spawn` until scheduled handler needs are audited. | Valid `RequestJoin` or `RequestVehicleLock`, valid `LocalizeMessage` or `HandleSpecial`, bogus handler rejected and logged once. |
| PVF sender authentication (DR-55) | Any client-to-server `WFBE_PVF_*` request or direct authority PV | Current stable `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`, current B69 `8d465fce` and adjacent B74 `b23f557f` forward only `(_this select 1)` into `WFBE_SE_FNC_HandlePVF` at `Init_PublicVariables.sqf:56,61`; `Server_HandlePVF.sqf:11-15` resolves/spawns the chosen handler with only handler parameters. Docs/source `HEAD@925d56e01a1e` is unchanged from `a0721301d4f5` for checked PVF paths and still has value-only registration plus old `Call Compile` dispatchers; Miksuu `b8389e748243` and perf `0076040f` match that old shape. The checked B69..B74 and B74..B74.1 generic PVF dispatcher/init deltas are empty, and no allowlist/sender/requester symbol was found in checked current-stable/B69/B74 dispatcher/init files. | The effect handler sees payload data but not an authenticated publisher identity, so legitimate handler names can still carry forged side/team/object/funds fields. | Server requester/context authority. | Carry authenticated requester context to server handlers (owner/UID/player object, cross-checked against the PVEH origin or a server-owned session table), then re-derive side, commander/team role, funds and object ownership inside each handler. Keep this separate from the handler-name allowlist patch. | From a non-owning/enemy client, forged `RequestVehicleLock`, `RequestChangeScore`, `RequestTeamUpdate`, `RequestSpecial`, attack-wave detail and supply-completion payloads reject; valid owner/commander/support flows still work. |
| ICBM / nuke | `Client/Module/Nuke/nukeincoming.sqf:23` | `Server/PVFunctions/RequestSpecial.sqf:1`, `"ICBM"` in current B74.1 `Server_HandleSpecial.sqf:127-135` | Client supplies side, base/target object, cruise object and team; server later spawns `NukeDammage`. | Server special-weapons authority. | Use [ICBM authority](ICBM-Authority-Playbook): validate commander/team, side, module/upgrade state, funds/cost and impact anchor server-side; server owns debit and launch acceptance. | Valid commander launch still works; forged non-commander, wrong-side, dead/invalid target and invalid base do not spawn `NukeDammage`. |
...
| Side supply | Common `ChangeSideSupply` callers | Current Chernarus `Common_ChangeSideSupply.sqf:24-26`, `Server_ChangeSideSupply.sqf:1-71`; maintained Vanilla/old-shape `Common_ChangeSideSupply.sqf:28-30`, `Server_ChangeSideSupply.sqf:1-47` | Common side broadcasts signed temp amount. Current stable/B74.1/B74.2 Chernarus floors server negatives to `0` and adds `wfbe_supply_temp_resistance`, but maintained Vanilla remains old-shape and all checked temp handlers still trust payload side/amount instead of deriving side from the channel/requester. | Server side-supply ledger. | Preserve/port negative clamp to zero/valid floor, validate side/channel/amount/requester consistency, restrict supply mutation callers, and prefer server-owned supply mutations for spend/reward flows. | Positive reward still applies; overspend cannot increase supply; west/east/resistance side mismatch rejects; resistance owner decision documented. |
...
| Attack waves | Docs/source `Client/FSM/updateclient.sqf:240`; current stable/B74.1/B69/B74/B74.2 `Client/FSM/updateclient.sqf:260`; `Common/Functions/Common_AttackWaveActivate.sqf:3-8` | `Server_AttackWave.sqf:1-38`, `Server/PVFunctions/AttackWave.sqf:1-62` | Docs/source `HEAD@426de4570993`, current stable/B74.1 `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`, current B74.2 `origin/claude/b74.2-aicom@21b62b04`, current B69 `8d465fce` and adjacent B74 `b23f557f` all keep `ATTACK_WAVE_INIT = [_supply, _side]`; server uses payload `_supply` to compute discount/duration and `ATTACK_WAVE_DETAILS` also trusts `_side`, `_priceModifier` and `_attackLength` (`Server_AttackWave.sqf:5-6,15,23,27,36,38`; `AttackWave.sqf:19,23-25,40`). B74.2 source-Chernarus line drift is buy-menu/constants only (`GUI_Menu_BuyUnits.sqf:105,250-251,422`; `Init_CommonConstants.sqf:604`); maintained Vanilla stays stable-shaped. Scoped `origin/master..origin/claude/b74.2-aicom` touches only source-Chernarus `GUI_Menu_BuyUnits.sqf` and `Init_CommonConstants.sqf` among checked attack-wave paths, and scoped `d472da6a..21b62b04` is empty. | Server support-mode authority. | Treat direct PVs as request/detail surfaces; server re-derives side supply, validates requester side/permission, deducts intended cost and clamps modifier/duration before starting wave, then protects the detail handler from forged payloads. | Legit wave starts; forged exaggerated supply/detail payload cannot change modifier, duration, side or cost; wrong-side requests reject; late-join attack-wave state remains documented. |
...
| Normal MHQ repair | `Client/Action/Action_RepairMHQ.sqf:5-35` | `Server/PVFunctions/RequestMHQRepair.sqf:1`, `Server_MHQRepair.sqf:7-79` | Client checks dead HQ/range/repair count/funds, debits locally and sends only side; server creates repaired MHQ from side state and only sets `wfbe_hq_repairing` after entering the worker. | Server HQ repair authority. | Include requester and repair-vehicle context; server validates dead HQ, vehicle range/class, side, repair count and funds, takes a server-side in-flight repair mutex, then debits and creates the MHQ. | Valid repair works; wrong-side, alive-HQ, far-vehicle, exhausted-count and insufficient-funds requests reject; two rapid repair requests produce one accepted repair. |
| WASP HQ recovery | `WASP/actions/Action_RepairMHQDepot.sqf:8-28` | `RequestMHQRepair` server path plus client-side `cashrepaired` and town supply reset | Client checks/debits cash, sends side-only repair request, sets public flags, moves HQ wreck and resets town `supplyvalue`; its duplicate guard is also client-local. | Server HQ recovery authority. | Include requester context; server validates dead HQ, commander/side, one-time flag, funds, recovery/drop position and town-SV side effects before repair, sharing the same server-side repair mutex as normal repair. | Valid recovery works once; wrong-side, insufficient-funds and second repair reject; town supply reset is server-owned; concurrent normal/cash repair attempts cannot create two HQs. |
