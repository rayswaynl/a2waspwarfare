...
| Attack waves | `Common/Functions/Common_AttackWaveActivate.sqf:3-8` | `Server_AttackWave.sqf:1-38`, `Server/PVFunctions/AttackWave.sqf:1-55` | Client/common side sends `ATTACK_WAVE_INIT = [_supply, _side]`; server uses `_supply` to compute discount and duration. | Server support-mode authority. | Treat direct PV as a request; server re-derives side supply and validates requester side/permission before starting wave. | Legit wave starts; forged exaggerated supply cannot change modifier, duration or cost; late-join attack-wave state remains documented. |
...
3. Treat `RequestSpecial` `"ICBM"`, construction/defense, upgrades, side supply, player buys, structure sale, supply rewards, gear/EASA/service and WASP HQ recovery as a single trust-boundary class.
