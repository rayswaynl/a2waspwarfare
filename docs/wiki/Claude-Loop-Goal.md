...
1. **Sync + read.** The live GitHub wiki is the source of truth (clone `C:\Users\Steff\_wasp_wiki_tmp`; in-repo mirror branch `docs/developer-wiki-claude` in `C:\Users\Steff\a2waspwarfare-docs`). Pull-rebase, then read `Coordination-Board.md`, `Agent-Worklog.md`, `agent-context.json`, `agent-status.json`, `agent-hardening-backlog.jsonl`, `Codebase-Coverage-Ledger.md`, `Deep-Review-Findings.md`, and any atlas/page Codex changed recently.
...
5. **Publish** only after `powershell -ExecutionPolicy Bypass -File .\Tools\ValidateWiki.ps1` passes. Update the focused page, `Codebase-Coverage-Ledger.md` if the matrix changes, `Deep-Review-Findings.md` when a finding changes, `Agent-Worklog.md`, `Coordination-Board.md`, `agent-context.json` and the compact machine files when their facts/routes change. Commit a small `claude:`-prefixed wiki commit, pull-rebase while preserving append-only additions, push wiki master and sync the `docs/wiki` mirror.
