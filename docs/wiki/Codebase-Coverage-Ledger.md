...
| Integrations (Extension / Discord / **AntiStack DB** / BattlEye) | 🟡 | ✅ | ✅ | 🟡 | 🟡 | n/a | [External integrations](External-Integrations); AntiStack DB (DR-7..DR-10) + GLOBALGAMESTATS extension (DR-29) + **BattlEye (DR-30 — shipped filter is a 22-byte `kickAFK` stub; no `scripts.txt`; option (b) "rely on BattlEye" for the economy/forgery class is NOT shipped)** all done; only **Discord data path ⬜** |
