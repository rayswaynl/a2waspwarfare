...
| Economy / town / supply | ✅ | ✅ | ✅ | 🟡 | 🟡 | ✅ | [Economy](Economy-Towns-And-Supply), [Gameplay atlas](Gameplay-Systems-Atlas); DR-20, DR-22, DR-23, DR-27, DR-28, **DR-41, DR-44**. **Economy authority FULLY characterized — every spend path client-authoritative AND the supply ledger itself is client-writable (DR-44 forged `wfbe_supply_temp_<side>`): build/buy/sell/supply/upgrade/ICBM/gear-rearm/attack-wave/supply-balance. The forgery class has TWO surfaces: the PVF dispatcher (DR-1) AND direct `publicVariableServer` channels (DR-41 `ATTACK_WAVE_INIT`, DR-44 `wfbe_supply_temp_*`) — the server-authority redesign must cover both** |
...
| AI / headless / perf | ✅ | 🟡 | 🟡 | ✅ | ✅ | n/a | [AI/headless](AI-Headless-And-Performance); DR-21 (HC disconnect: server load migration, no re-delegation), **DR-42 (static-defence HC delegation update-back commented out at Client_DelegateAIStaticDefence.sqf:28 → server never tracks HC-created static-defence units; town-AI delegation does report back)**, **DR-45 (town-AI inactivity despawn `server_town_ai.sqf:213-216` deletes `wfbe_active_vehicles` by `!(isPlayer leader group)` without checking passengers/cargo → deletes a vehicle a player rides as cargo)** |
| UI / HUD / menus | ✅ | ✅ | 🟡 | 🟡 | ✅ | ✅ | [UI atlas](Client-UI-Systems-Atlas); DR-16 (client-side sale), DR-17/DR-25a (dup IDDs 23000/10200), DR-24 (dead RscMenu_Upgrade), DR-25b (malformed soundPush) — Curie candidates all confirmed. **JIP/HC reviewed with caveats: `initJIPCompatible.sqf:70-76,224-238` excludes headless clients from UI init; late-join UI has marker/vote/HUD recovery, but several `Init_Client.sqf` waits are unbounded and event-style marker/support handlers still need feature smoke** |
...
| WASP overlay | ✅ | 🟡 | 🟡 | ✅ | ✅ | ✅ | [WASP overlay](WASP-Overlay); **DR-40 branch refresh 2026-06-22: current stable `origin/master@0139a346` and B69 carry `sleep 0.1` in the display-54 loop (`global_marking_monitor.sqf:64`) in both maintained roots, while docs/source `HEAD@46840f04`, current Miksuu `b8389e748243` and perf `0076040f` still keep the unslept old shape. JIP/HC clean — per-client init from Init_Client; old `initJIPCompatible.sqf:243-244` WASP path dead/commented.** Auth/PV 🟡 = WASP action authority (owner, economy-class follow-up) |
| Tooling / LoadoutManager | ✅ | n/a | n/a | n/a | n/a | ✅ | [Tools](Tools-And-Build-Workflow), DR-4, **DR-43a rechecked 2026-06-14: current docs checkout has no present or tracked Chernarus/Vanilla `version.sqf`; generated files remain git-ignored LoadoutManager outputs, so clean checkouts and generated/terrain target roots still need version-file generation/verification before pack/test claims** |
...
| Modules (Artillery / ICBM / IRS / CM / UAV / NEURO / Skill / …) | ✅ | 🟡 | 🟡 | 🟡 | 🟡 | ✅ | [Modules atlas](Modules-Atlas) (source-verified: IRS, CM, Reaktiv, Engines, AutoFlip, ZetaCargo, Valhalla, Skill, NEURO, CIPHER) + [Deep-review findings](Deep-Review-Findings) DR-27 (**ICBM nuke fully client-authoritative — forged `RequestSpecial` PV = server-applied map-wide kill, CRITICAL**); UAV-007 branch disabled. Auth/PV 🟡 = only Nuke/ICBM has a trust-surface defect; rest config-gated QoL/AI |
...
Previous: [Feature status register](Feature-Status-Register) | Next: [Deep-review findings](Deep-Review-Findings)

Main map: [Home](Home) | Current work: [Progress dashboard](Progress-Dashboard) | Agent file: [`agent-context.json`](agent-context.json)
