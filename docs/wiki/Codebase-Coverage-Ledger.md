...
| WASP overlay | ✅ | 🟡 | 🟡 | ✅ | ✅ | ✅ | [WASP overlay](WASP-Overlay); **DR-40 (Perf clean except `global_marking_monitor.sqf:62` display-wait loop already has `sleep 0.1` (line 64) — matches its `:80` sibling; Perf fully clean; JIP/HC clean — per-client init from Init_Client; old `initJIPCompatible.sqf:243-244` WASP path dead/commented)**. Auth/PV 🟡 = WASP action authority (owner, economy-class follow-up) |
