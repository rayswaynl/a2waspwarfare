...
Refreshed 2026-06-13 against docs checkout `8d611092`, stable `origin/master` `cf2a6d6a`, Miksuu `b8389e74`, `origin/perf/quick-wins` `0076040f`, release `origin/release/2026-06-feature-bundle` `a96fdda2` and `origin/feat/buymenu-easa-qol` `a66d4691`.

| Scope | Factory purchase status |
| --- | --- |
| Docs checkout `8d611092` | Both maintained roots still have the old buy-menu selected-detail price drift, no level-0 `UNIT_COST_MODIFIER` reset, mixed driver-default keys and no destroyed-factory refund on the dead/null abort. |
| Stable `origin/master` `cf2a6d6a` and release `a96fdda2` | Both maintained roots now pass `_currentCost` into `BuildUnit`, refund the real dead/null factory abort, fix selected-detail price display and mirror both driver-default profile keys. The level-0 `UNIT_COST_MODIFIER` reset is still open. |
| Miksuu `b8389e74` and `perf/quick-wins` `0076040f` | Both maintained roots keep the older docs-checkout buy-menu/refund shape for the rows below. No upstream/perf rescue found. |
| `origin/feat/buymenu-easa-qol` `a66d4691` | Chernarus-only UI QoL branch fixes selected-detail price and adds affordability/queue UI work, but Vanilla is untouched and the refund/key/reset issues remain old-shape. |
...
Branch recheck found the buy-menu work is now split by ref. Docs checkout `8d611092`, Miksuu `b8389e74` and `perf/quick-wins` `0076040f` still keep the old selected-detail price drift, driver-default key split and no level-0 `UNIT_COST_MODIFIER` reset. Stable `origin/master` `cf2a6d6a` and release `a96fdda2` fix selected-detail display in both maintained roots and mirror both driver-default keys, while `origin/feat/buymenu-easa-qol` `a66d4691` fixes selected-detail display only in Chernarus.

The still-open cross-branch issue is the unit-cost reset trap. `Client_UIFillListBuyUnits.sqf:7-16` still sets `UNIT_COST_MODIFIER` only when the upgrade level is `1` or `2`, while `Init_CommonConstants.sqf` initializes the global once (`HEAD`/Miksuu/perf/branch `:203`, stable `:220`, release `:216`). If a long-lived client has already seen a discounted state, a later no-upgrade list fill can inherit the stale discount. Patch by assigning the default before the `if (_unitCostUpgradeLevel > 0)` branch or by using a local price helper instead of a mutable global.

### Buy-Menu Price And Driver-Key Branch Matrix

Refreshed 2026-06-13 for docs checkout `8d611092`, stable `origin/master` `cf2a6d6a`, upstream `miksuu/master` `b8389e74`, `origin/perf/quick-wins` `0076040f`, release `origin/release/2026-06-feature-bundle` `a96fdda2` and branch `origin/feat/buymenu-easa-qol` `a66d4691`.
...
| Stable `origin/master` `cf2a6d6a` and release `a96fdda2` | Both maintained roots use the modifier-bearing selected-detail formula (`origin/master` `GUI_Menu_BuyUnits.sqf:286`, release `:284`) and the list filler uses `_price` from the same formula (`Client_UIFillListBuyUnits.sqf:61`). | Still no level-0 reset: `Client_UIFillListBuyUnits.sqf:11,14` are the only modifier writes, with one-time init in `Init_CommonConstants.sqf:220` on stable and `:216` on release. | Init/toggle/max-out/reset now mirror lowercase and uppercase keys (`origin/master` `GUI_Menu_BuyUnits.sqf:42-43,183-185,309-310,354-366,399-413`; release line drift `:40-41,181-183,307-308,352-364,397-411`), while live reads use lowercase. | Selected-detail and compatibility-key drift are fixed in both maintained roots; level-0 modifier reset remains open. |
...
| `origin/feat/buymenu-easa-qol` `a66d4691` | Chernarus selected detail is fixed at `GUI_Menu_BuyUnits.sqf:280`; maintained Vanilla still uses the old `:261` formula. | No reset in either root. | Mixed profile key remains in both roots. | Useful Chernarus UI branch evidence, but not a full maintained-root price/key cleanup. |

Patch order: first reset or localize the unit-cost modifier so list row, selected-detail display, affordability check and debit agree for levels `0/1/2`; then decide whether stable/release's dual-key mirroring is the intended compatibility policy or whether one normalized key should replace it. Smoke low/exact/high funds, unit-cost levels `0/1/2`, infantry, crewless vehicles, full-crew vehicles, checkbox toggle, max-out and reset.
...
Destroyed-factory refund behavior is now branch-scoped. The docs checkout, Miksuu, `perf/quick-wins` and `origin/feat/buymenu-easa-qol` still debit after spawning `BuildUnit` without passing `_currentCost`, so the dead/null factory abort only removes queue state. Stable `origin/master` `cf2a6d6a` and release `a96fdda2` now pass `_currentCost` into `BuildUnit` and refund the real dead/null factory abort in both maintained roots. If porting or rewriting this lane, preserve the refund shape and smoke factory death, buyer disconnect, empty/crewless vehicles and normal completion together.
...
Refreshed 2026-06-13 for docs checkout `8d611092`, stable `origin/master` `cf2a6d6a`, upstream `miksuu/master` `b8389e74`, `origin/perf/quick-wins` `0076040f`, release `origin/release/2026-06-feature-bundle` `a96fdda2` and `origin/feat/buymenu-easa-qol` `a66d4691`.

| Scope | Menu/debit shape | Dead/null factory abort | Development meaning |
| --- | --- | --- | --- |
| Docs checkout `8d611092`, Miksuu `b8389e74` and `perf/quick-wins` `0076040f` | Both maintained roots check the local queue cap, increment `WFBE_C_QUEUE_%1`, spawn `BuildUnit`, then debit with `-(_currentCost) Call ChangePlayerFunds` at `GUI_Menu_BuyUnits.sqf:154-156`. `_currentCost` is not passed to `BuildUnit`. | `Client_BuildUnit.sqf:211-214` exits when the building is dead/null and decrements `unitQueu` plus `WFBE_C_QUEUE_%1`, but does not refund funds. | The player pays even when the build worker aborts after a destroyed/null factory on these refs. Treat as patch-ready if targeting those roots. |
| Stable `origin/master` `cf2a6d6a` and release `a96fdda2` | Both maintained roots pass `_currentCost` into `BuildUnit` (`origin/master` `GUI_Menu_BuyUnits.sqf:162`, release `:160`), spawn at the next line, then debit with `ChangePlayerFunds` (`origin/master` `:164`, release `:162`). | `Client_BuildUnit.sqf:7` reads the optional cost payload; the dead/null exit at `:212-216` decrements queue/counter and refunds `_currentCost` when positive. The later crewless/Depot branch explicitly says no refund because the vehicle already spawned (`:376-377`). | Destroyed-factory refund closure is source-present in stable/release for Chernarus and maintained Vanilla, but purchase authority is still client-local and Arma smoke remains required. |
| `origin/feat/buymenu-easa-qol` `a66d4691` | Chernarus spawns/debits at `GUI_Menu_BuyUnits.sqf:163-164`; maintained Vanilla stays at `:155-156`. Neither root passes `_currentCost` to `BuildUnit`. | Both roots keep the old dead/null exit at `Client_BuildUnit.sqf:211-214` with queue cleanup only. | UI QoL branch evidence does not include the destroyed-factory refund fix. |
...
| Destroyed-factory refund is branch-scoped | Docs checkout `8d611092`, Miksuu `b8389e74`, `perf/quick-wins` `0076040f` and `origin/feat/buymenu-easa-qol` still debit after spawning `BuildUnit` without passing `_currentCost`; stable `origin/master` `cf2a6d6a` and release `a96fdda2` pass `_currentCost` and refund the dead/null abort at `Client_BuildUnit.sqf:212-216` in both maintained roots. | Use the branch matrix above. Preserve or port the stable/release refund shape when merging, but do not confuse it with server-side purchase authority; smoke factory death, buyer disconnect, empty/crewless vehicles and normal completion. |
...
| Buy-menu price/key alignment | Docs checkout, Miksuu and perf keep selected-detail price drift, no level-0 `UNIT_COST_MODIFIER` reset and the old driver-default profile key split. Stable/release fix selected-detail display and mirror both driver-default keys in both maintained roots; `origin/feat/buymenu-easa-qol` fixes selected-detail display in Chernarus only. No checked ref resets `UNIT_COST_MODIFIER` to `1` for level 0. | Use the branch matrix above. Patch/reset the modifier path first, then decide whether stable/release dual-key mirroring is enough or one normalized key should replace it; smoke list/detail/charge, funds guard, unit-cost levels `0/1/2`, crew toggles, max-out and reset. |
