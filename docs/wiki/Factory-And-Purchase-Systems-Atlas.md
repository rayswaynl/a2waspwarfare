...
| Current buy-menu price, nil-data guard, squad cap and driver-default branch truth | [B750 buy-list and squad-cap current-master route](#b750-buy-list-and-squad-cap-current-master-route), [Buy-menu price and driver-key branch matrix](#buy-menu-price-and-driver-key-branch-matrix) | [Feature status](Feature-Status-Register#partial--deferred--needs-review), [Client UI systems atlas](Client-UI-Systems-Atlas) |
...
| B750 buy-menu current-master route | `origin/master@c4c9a6479` / PR #87 guards missing per-class unit data before upgrade-gate reads and restores infantry squad-cap growth in both maintained roots; smoke is still required before release-ready wording. |
...
PR #87 merged `origin/feat/b750-factory-aicom@605ad7326707b9422303e60ca6c286d6e0700864` into current stable `origin/master@c4c9a64790c63fe264bd1a25f84f0b3ca7b49362` on 2026-06-24T15:10:56Z. The purchase-side subset of the scoped B750 delta `6a34799b4..origin/master` is four maintained-root files / +24 / -16 and clean under `git diff --check`: source Chernarus and maintained Vanilla `Client_UIFillListBuyUnits.sqf` plus `GUI_Menu_BuyUnits.sqf`.

| Scope | Buy-menu result | Release gate |
| --- | --- | --- |
| Current master `origin/master@c4c9a6479` / PR #87 B750 | Both maintained roots default `_upgradePass = false`, then only read `QUERYUNITUPGRADE` when `_c` exists at `Client_UIFillListBuyUnits.sqf:88-92`, so roster classnames without registered unit metadata hide instead of stranding `_upgradePass` undefined. Both roots also replace the previous quarter-tier cap with `GUI_Menu_BuyUnits.sqf:137-138`: read Barracks level, then use `(10 + (_realSize * 2)) min _mbu`; commander bonus still adds `+10` at `:139-145`. | Smoke missing or delayed unit-data rows, faction/upgrade filtering, Barracks levels 0-3, commander bonus, depot infantry camp gate, exact/full group limits and normal queue/debit behavior in source Chernarus plus maintained Vanilla. This is current-master source evidence, not package/release proof. |

Buy-menu price/key B74.2 current-head follow-up 2026-06-24: checked docs/source Chernarus and maintained Vanilla `GUI_Menu_BuyUnits.sqf`, `Client_UIFillListBuyUnits.sqf` and `Init_CommonConstants.sqf` paths remain old-shape for price/key behavior; targeted diffs from `b8180bb319ed`, `4120137f`, `8d611092` and `ebd86fad0` to docs/source `HEAD@f9601c92e0a4` returned no checked price/key source changes, and the last source-touch commit for these paths is `1154f5d43`. Current stable/B74.1 is `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`; current B74.2 is `origin/claude/b74.2-aicom@21b62b04`; current B69 is `origin/claude/b69@8d465fce`; adjacent B74 is `origin/claude/b74-aicom-spend@b23f557f`; direct Miksuu upstream is `master@b8389e748243`; perf is `origin/perf/quick-wins@0076040f`. `0139a3468609..origin/master` and B69..B74 touch only constants among checked price/key paths. Current B74.2 keeps the previous B74.2 price/key shape: checked `d472da6a..21b62b04` is empty for buy-menu controller/list-filler/constants paths, while `origin/master..origin/claude/b74.2-aicom` changes only source Chernarus buy-menu pop-tier AI-cap setup plus source Chernarus constants among checked price/key files; maintained Vanilla is unchanged from current stable for checked price/key paths. Current origin exposes one live buy-menu head, `origin/claude/trello-buymenu-gear-display@db1291beba`, but it is current-stable-based gear/ammo preview work, not a price/key or reset rescue. `a96fdda2` and `a66d4691` remain historical commit evidence.
...
| --- | --- |
| Current master `origin/master@c4c9a6479` / PR #87 B750 | Purchase-side B750 is source-present in both maintained roots: nil unit-data rows are guarded before upgrade reads, and the player infantry cap starts at 10 plus 2 per Barracks level, clamped by `_mbu`, with commander bonus preserved. This does not close the older level-0 `UNIT_COST_MODIFIER` reset debt or player-purchase server-authority gap. |
...
Current master B750 adds one important defensive rule: if a classname in a factory roster has no registered unit metadata array, the row defaults to hidden instead of evaluating `QUERYUNITUPGRADE` on nil. The source proof is `Client_UIFillListBuyUnits.sqf:88-92` in both source Chernarus and maintained Vanilla at `origin/master@c4c9a6479`.
...
| Group size | Uses live units plus local `unitQueu`, scaled by Barracks upgrade and with a commander bonus. Current master B750 uses `GUI_Menu_BuyUnits.sqf:137-138` for `10 + 2 * Barracks level` clamped to `_mbu`, then keeps the commander `+10` branch at `:139-145`. |
...
| B750 buy-list guard and infantry cap are source-present, smoke pending | Current master `origin/master@c4c9a6479` guards nil `_c` before upgrade reads at `Client_UIFillListBuyUnits.sqf:88-92` and restores infantry cap growth at `GUI_Menu_BuyUnits.sqf:137-138` in both maintained roots. | Do not reopen the older four-at-Barracks-0 cap regression on current-master-shaped targets. Smoke missing unit-data rosters and Barracks level caps before release-ready wording; keep this separate from queue-token, refund and server-authority work. |
