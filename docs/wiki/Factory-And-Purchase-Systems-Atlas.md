...
Rechecked 2026-06-14 against docs head `3b911594`. Targeted `git diff --name-only 8d611092..HEAD` over the source scope above returned no Chernarus or maintained Vanilla factory/purchase source changes, so the `8d611092` line anchors remain valid. Branch refs for this matrix still resolve to stable `origin/master` `cf2a6d6a`, Miksuu `b8389e74`, `origin/perf/quick-wins` `0076040f`, release `origin/release/2026-06-feature-bundle` `a96fdda2` and `origin/feat/buymenu-easa-qol` `a66d4691`.
...
| Destroyed-factory refund is branch-scoped | The exact docs/stable/release/Miksuu/perf/QoL split is preserved in the [destroyed-factory refund branch matrix](#destroyed-factory-refund-branch-matrix). Stable/release carry a source-present dead/null refund; old-shape branches do not. | Preserve or port the stable/release refund shape when merging, but do not confuse it with server-side purchase authority; smoke factory death, buyer disconnect, empty/crewless vehicles and normal completion. |
...
| Buy-menu price/key alignment | The exact docs/stable/release/Miksuu/perf/QoL split is preserved in the [buy-menu price and driver-key branch matrix](#buy-menu-price-and-driver-key-branch-matrix). No checked ref resets `UNIT_COST_MODIFIER` to `1` for level 0. | Patch/reset the modifier path first, then decide whether stable/release dual-key mirroring is enough or one normalized key should replace it; smoke list/detail/charge, funds guard, unit-cost levels `0/1/2`, crew toggles, max-out and reset. |
