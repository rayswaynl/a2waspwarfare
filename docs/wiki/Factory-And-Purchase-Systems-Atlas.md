...
| --- | --- | --- |
| Empty-vehicle buys leak the client queue counter. | `Client_BuildUnit.sqf:365` exits before the later `WFBE_C_QUEUE` decrement at `:469`. After repeated empty-vehicle purchases, the local buy menu can think the factory category is permanently capped. | Decrement `WFBE_C_QUEUE_*` / `unitQueu` on every exit path, including factory-dead and empty-vehicle exits. |
| Factory FIFO tokens are noisy and not unique. | `_unique` is generated from several `random` calls, and the building `queu` array is broadcast with `setVariable [..., true]` on enqueue/progress/completion. | Use a truly unique token per purchase and reduce public broadcasts to state changes that clients actually need. |
...
| Queue state is fragile | Building `queu` and client `WFBE_C_QUEUE_*` counters are manually incremented/decremented; DR-33 confirms the empty-vehicle exit skips the decrement and random FIFO tokens can collide. | Always test cancellation, factory destruction, menu close, empty-vehicle buys and repeated purchases when touching queues. |
