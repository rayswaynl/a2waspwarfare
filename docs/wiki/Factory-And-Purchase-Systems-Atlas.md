...
| Buy-menu selected-detail price can drift from list/charge price | List rows and purchase/check/charge math include `UNIT_COST_MODIFIER` (`Client_UIFillListBuyUnits.sqf:60`; `GUI_Menu_BuyUnits.sqf:90,155-156`), but the selected-detail refresh recomputes `_currentCost` without it at `GUI_Menu_BuyUnits.sqf:261` and displays that value at `:465`. | Keep one exact price helper before changing unit-cost discounts or attack-wave modifiers; smoke list row, selected-detail price, funds guard and charged funds together. |
