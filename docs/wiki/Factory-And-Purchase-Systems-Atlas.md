...
Current-source recheck corrected an older price-drift note: both the list row and the current/charge path use the modifier-bearing formula in this branch (`Client_UIFillListBuyUnits.sqf:60`; `GUI_Menu_BuyUnits.sqf:90,155-156`). The remaining risk is shared mutable state rather than a list/detail formula split: if `UNIT_COST_MODIFIER` is stale, both display and charge can inherit the same wrong discount. A future fix should compute a local modifier per fill/purchase, or reset `UNIT_COST_MODIFIER = 1` at the top of every list fill and route list/current-cost/charge through one helper.
...
| Old buy-menu price-drift note is branch-stale | Current source has list and current/charge math both using `UNIT_COST_MODIFIER` (`Client_UIFillListBuyUnits.sqf:60`; `GUI_Menu_BuyUnits.sqf:90,155-156`). The actual risk is the mutable global reset trap below. | Keep one exact price helper before changing unit-cost discounts or attack-wave modifiers, and verify source before carrying old branch notes forward. |
