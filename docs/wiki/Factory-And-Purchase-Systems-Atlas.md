...
| --- | --- |
| Docs/source `HEAD@4120137f` (source-unchanged from `8d611092` / `ebd86fad0` for checked buy-menu controller/list-filler paths) | Both maintained roots still have the old buy-menu selected-detail price drift, no level-0 `UNIT_COST_MODIFIER` reset, mixed driver-default keys and no destroyed-factory refund on the dead/null abort. |
| Current stable `origin/master@0139a346`, current B69 `origin/claude/b69@8d465fce` and adjacent B74 `origin/claude/b74-aicom-spend@b23f557f` | Both maintained roots pass `_currentCost` into `BuildUnit`, refund the real dead/null factory abort, fix selected-detail price display, mirror driver-default profile keys and still leave the level-0 `UNIT_COST_MODIFIER` reset open. B69..B74 is constants-only for checked buy-menu price/key paths. |
