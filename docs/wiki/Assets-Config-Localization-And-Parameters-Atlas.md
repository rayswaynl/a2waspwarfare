...
| Tactical menu icon references resolved | `Client/FSM/updateavailableactions.fsm:64-82` references 19 `Client\Images\icon_wf_*.paa` paths; all 19 files exist in `Client\Images\`. `Rsc/Dialogs.hpp` contains no references to these paths. | No missing assets. Icon references and local files are consistent. |
