...
| Release refs | `git ls-remote --heads origin release/*` returned no current release head on 2026-06-22, and historical `a96fdda2` was not available as a local object in this checkout. | Do not call release status current from the older row without restoring or rechecking a release ref. |
