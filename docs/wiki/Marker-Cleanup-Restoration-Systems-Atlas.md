...
The empty supply-truck matrix below was refreshed on 2026-06-22 after current stable advanced past the older `cf2a6d6a` evidence. Docs head `b4e10b5f`, current stable `origin/master@0139a346`, direct current Miksuu `master@b8389e748243`, `origin/perf/quick-wins@0076040f`, B69 `origin/claude/b69@0094647d` and B69 finalpieces `origin/claude/b69-finalpieces@80d3267c` all still drain `WF_Logic` `emptyVehicles` through `WFBE_SE_FNC_HandleEmptyVehicle` and set supply-truck `_delay = 86400` in both maintained roots. Current origin exposed no `release/*`, `feat/*cleanup*`, `feat/*supply*`, `feature/*cleanup*` or `feature/*supply*` heads during this pass; historical `a96fdda2` is available in the source checkout and matches the 24-hour policy, but remains historical-only evidence rather than current release proof.
...
| --- | --- | --- |
| Docs checkout Chernarus `b4e10b5f` | `Server/FSM/emptyvehiclescollector.sqf:9-19,30`; `Server/Functions/Server_HandleEmptyVehicle.sqf:22-23,27,30,33` | Queue drains through `emptyQueu`, spawns `WFBE_SE_FNC_HandleEmptyVehicle`, then supply trucks set `_delay = 86400` and are only deleted after the handler sees enough empty time. |
| Docs checkout maintained Vanilla Takistan `b4e10b5f` | Same `emptyvehiclescollector.sqf:9-19,30`; same `Server_HandleEmptyVehicle.sqf:22-23,27,30,33` | Same 24-hour supply-truck behavior in the maintained generated/copy target. |
| Current stable `origin/master@0139a346` | Chernarus and maintained Vanilla keep `emptyvehiclescollector.sqf:9-19,30`; `Server_HandleEmptyVehicle.sqf:30-31,35,38,41` line-drifts after the body/wreck cleanup split. | No current-stable rescue; the hard-coded 24-hour supply-truck delay remains in both maintained roots. |
...
| B69 family `origin/claude/b69@0094647d` and `origin/claude/b69-finalpieces@80d3267c` | Chernarus and maintained Vanilla match current-stable line drift: `Server_HandleEmptyVehicle.sqf:30-31,35,38,41`, with unchanged collector anchors. | B69 AI commander work does not rescue empty supply-truck cleanup; treat it as same 24-hour behavior until a cleanup-specific branch proves otherwise. |
| Release refs | `git ls-remote --heads origin release/*` returned no current release head on 2026-06-22. Historical `a96fdda2` is available as local source evidence and both maintained roots keep `emptyvehiclescollector.sqf:9,12,15-17,19,26`, `Server_HandleEmptyVehicle.sqf:23,30,33` and `Client_BuildUnit.sqf:255-256`. | Historical `a96fdda2` also keeps the 24-hour policy, but do not call release status current without restoring or rechecking a live release ref. |
