...
The empty supply-truck matrix below was refreshed on 2026-06-23 after adjacent B74 `origin/claude/b74-aicom-spend@b23f557f` was checked against current B69 `origin/claude/b69@8d465fce`. Docs/source `HEAD@7e7aebec` is unchanged from `b4e10b5f` for checked collector/handler paths; current stable `origin/master@0139a346`, B69, B74, direct current Miksuu `master@b8389e748243`, `origin/perf/quick-wins@0076040f` and historical `a96fdda2` all still drain `WF_Logic` `emptyVehicles` through `WFBE_SE_FNC_HandleEmptyVehicle` and set supply-truck `_delay = 86400` in both maintained roots. Checked path deltas `origin/claude/b69..origin/claude/b74-aicom-spend`, `0094647d..origin/claude/b74-aicom-spend` and `80d3267c..origin/claude/b74-aicom-spend` are empty. Current origin exposed no `release/*`, `feat/*cleanup*`, `feat/*supply*`, `feature/*cleanup*` or `feature/*supply*` heads during this pass; historical `a96fdda2` is local source evidence, not current release proof.
...
| --- | --- | --- |
| Docs/source Chernarus `HEAD@7e7aebec` | Checked collector/handler paths are unchanged from `b4e10b5f`: `Server/FSM/emptyvehiclescollector.sqf:9,12,15-17,19,30`; `Server/Functions/Server_HandleEmptyVehicle.sqf:21-23,29-30,33` | Queue drains through `emptyQueu`, spawns `WFBE_SE_FNC_HandleEmptyVehicle`, then supply trucks set `_delay = 86400` and are only deleted after the handler sees enough empty time. |
| Docs/source maintained Vanilla Takistan `HEAD@7e7aebec` | Same unchanged-from-`b4e10b5f` anchors: `emptyvehiclescollector.sqf:9,12,15-17,19,30`; `Server_HandleEmptyVehicle.sqf:21-23,29-30,33` | Same 24-hour supply-truck behavior in the maintained generated/copy target. |
...
| Current B69 / adjacent B74 `origin/claude/b69@8d465fce` / `origin/claude/b74-aicom-spend@b23f557f` | Chernarus and maintained Vanilla match current-stable line drift: `emptyvehiclescollector.sqf:9,12,15-17,19,30`; `Server_HandleEmptyVehicle.sqf:29-31,37-38,41`. Checked deltas `origin/claude/b69..origin/claude/b74-aicom-spend`, `0094647d..origin/claude/b74-aicom-spend` and `80d3267c..origin/claude/b74-aicom-spend` are empty for maintained collector/handler paths. | B69/B74 AI commander work does not rescue empty supply-truck cleanup; treat `0094647d` and `80d3267c` as provenance only, not current-head proof. |
...
| Release refs | `git ls-remote --heads origin release/* feat/*cleanup* feat/*supply* feature/*cleanup* feature/*supply*` returned no current release or cleanup/supply rescue head on 2026-06-23. Historical `a96fdda2` is available as local source evidence and both maintained roots keep `emptyvehiclescollector.sqf:9,12,15-17,19,30` and `Server_HandleEmptyVehicle.sqf:21-23,29-30,33`. | Historical `a96fdda2` also keeps the 24-hour policy, but do not call release status current without restoring or rechecking a live release ref. |
...
| --- | --- | --- |
| Empty supply trucks bypass normal timeout | [Empty Supply Truck Branch Matrix](#empty-supply-truck-branch-matrix) owns the refreshed docs/current-stable/B69/B74/Miksuu/perf proof: checked maintained roots still drain `emptyVehicles` through the collector and set supply-truck `_delay = 86400`; adjacent B74 `b23f557f` is unchanged from current B69 `8d465fce`, older `0094647d` and `80d3267c` for these paths; no live `release/*` or cleanup/supply feature head was found on 2026-06-23, and verified `a96fdda2` remains historical-only release evidence. | Decide intentional logistics persistence versus shorter/parameterized cleanup; if changing, patch source Chernarus plus maintained Vanilla and smoke ordinary empty vehicles, double-timeout medical/repair vehicles and supply mission/logistics flows. |
