...
The empty supply-truck matrix below was refreshed on 2026-06-22 after current B69 advanced past the older `0094647d` / `80d3267c` evidence. Docs/source `HEAD@a3bbcd48` is unchanged from `b4e10b5f` for checked collector/handler paths; current stable `origin/master@0139a346`, direct current Miksuu `master@b8389e748243`, `origin/perf/quick-wins@0076040f`, current B69 `origin/claude/b69@8d465fce` and historical `a96fdda2` all still drain `WF_Logic` `emptyVehicles` through `WFBE_SE_FNC_HandleEmptyVehicle` and set supply-truck `_delay = 86400` in both maintained roots. B69 checked path deltas `0094647d..8d465fce` and `80d3267c..8d465fce` are empty. Current origin exposed no `release/*`, `feat/*cleanup*`, `feat/*supply*`, `feature/*cleanup*` or `feature/*supply*` heads during this pass; historical `a96fdda2` is local source evidence, not current release proof.
...
- Empty supply trucks have a special long lifetime: `Server_HandleEmptyVehicle.sqf:21-23` on docs-shaped refs and `:29-31` on stable/B69-shaped refs set a 24-hour delay for `V3S_Supply_TK_GUE_EP1`, Warfare supply trucks and OA supply truck classes. Long-lived abandoned logistics vehicles are therefore design/config behavior, not necessarily collector failure.
...
| --- | --- | --- |
| Docs/source Chernarus `HEAD@a3bbcd48` | Checked collector/handler paths are unchanged from `b4e10b5f`: `Server/FSM/emptyvehiclescollector.sqf:9,12,15-17,19,30`; `Server/Functions/Server_HandleEmptyVehicle.sqf:21-23,29-30,33` | Queue drains through `emptyQueu`, spawns `WFBE_SE_FNC_HandleEmptyVehicle`, then supply trucks set `_delay = 86400` and are only deleted after the handler sees enough empty time. |
| Docs/source maintained Vanilla Takistan `HEAD@a3bbcd48` | Same unchanged-from-`b4e10b5f` anchors: `emptyvehiclescollector.sqf:9,12,15-17,19,30`; `Server_HandleEmptyVehicle.sqf:21-23,29-30,33` | Same 24-hour supply-truck behavior in the maintained generated/copy target. |
| Current stable `origin/master@0139a346` | Chernarus and maintained Vanilla keep `emptyvehiclescollector.sqf:9,12,15-17,19,30`; `Server_HandleEmptyVehicle.sqf:29-31,37-38,41` line-drifts after the nil/null guard and body/wreck cleanup split. | No current-stable rescue; the hard-coded 24-hour supply-truck delay remains in both maintained roots. |
| Direct current Miksuu `master@b8389e748243` | Chernarus and maintained Vanilla keep the docs-shaped collector and handler anchors: `emptyvehiclescollector.sqf:9-19,30`; `Server_HandleEmptyVehicle.sqf:22-23,27,30,33`. | No upstream rescue; this behavior is inherited, not a docs-branch-only change. |
...
| Current B69 `origin/claude/b69@8d465fce` | Chernarus and maintained Vanilla match current-stable line drift: `emptyvehiclescollector.sqf:9,12,15-17,19,30`; `Server_HandleEmptyVehicle.sqf:29-31,37-38,41`. Checked deltas `0094647d..8d465fce` and `80d3267c..8d465fce` are empty for maintained collector/handler paths. | B69 AI commander work still does not rescue empty supply-truck cleanup; treat `0094647d` and `80d3267c` as provenance only, not current-head proof. |
| Release refs | `git ls-remote --heads origin release/*` returned no current release head on 2026-06-22. Historical `a96fdda2` is available as local source evidence and both maintained roots keep `emptyvehiclescollector.sqf:9,12,15-17,19,30`, `Server_HandleEmptyVehicle.sqf:21-23,29-30,33` and `Client_BuildUnit.sqf:255-256`. | Historical `a96fdda2` also keeps the 24-hour policy, but do not call release status current without restoring or rechecking a live release ref. |
...
| --- | --- | --- |
| Empty supply trucks bypass normal timeout | [Empty Supply Truck Branch Matrix](#empty-supply-truck-branch-matrix) owns the refreshed docs/current-stable/Miksuu/perf/current-B69 proof: checked maintained roots still drain `emptyVehicles` through the collector and set supply-truck `_delay = 86400`; current B69 `8d465fce` is unchanged from `0094647d` / `80d3267c` for these paths; no live `release/*` or cleanup/supply feature head was found on 2026-06-22, and verified `a96fdda2` remains historical-only release evidence. | Decide intentional logistics persistence versus shorter/parameterized cleanup; if changing, patch source Chernarus plus maintained Vanilla and smoke ordinary empty vehicles, double-timeout medical/repair vehicles and supply mission/logistics flows. |
