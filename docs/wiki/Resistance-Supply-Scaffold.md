...
- Current stable/B74.1/B74.2 Chernarus and B69/B74 Chernarus register `wfbe_supply_temp_resistance` at `Server_ChangeSideSupply.sqf:25-45`, but it still trusts `_side` from the payload and has no channel/requester validation. Live `origin/claude/faction-tint-sidefix@32acd272f92e` diverges before current stable and removes this handler, so treat it as a regression warning if revived.
...
| Current B74.2 `origin/claude/b74.2-aicom@21b62b04`; previous B74.2 `d472da6a`; current B69 `origin/claude/b69@8d465fce`; adjacent B74 `origin/claude/b74-aicom-spend@b23f557f` | Current B74.2 has no checked side-supply/resistance delta over previous B74.2 or current stable; B69..B74 is also empty for checked side-supply paths. Chernarus and maintained Vanilla define `WFBE_L_GUE` and include resistance in the present-side loop. | Chernarus and Vanilla use the current-stable funds-only `0` default read branch. | Same current-stable split: Chernarus has `wfbe_supply_temp_resistance` plus floor-to-zero; maintained Vanilla remains west/east only and old-shape. |
| Live `origin/claude/faction-tint-sidefix@32acd272f92e` | Stale divergent branch: merge-base with current stable is `d35d7c8fb774`, and the branch does not contain current stable. | No checked read-path rescue over current stable for this lane. | Compared with current stable, removes the Chernarus `wfbe_supply_temp_resistance` block while keeping west/east handlers. If revived, rebase/restore the resistance handler before any three-way economy claim. |
