...
| B69 `origin/claude/b69@39eed5c0` | Chernarus and maintained Vanilla define `WFBE_L_GUE` and include resistance in the present-side loop. | Chernarus and Vanilla use the current-stable funds-only `0` default read branch. | Chernarus adds `wfbe_supply_temp_resistance` at `Server_ChangeSideSupply.sqf:25-45` and floors west/resistance/east negatives to `0` at `:12,36,60`; maintained Vanilla remains west/east only and old-shape at `:12,36`. |
