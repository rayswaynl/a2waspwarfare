...
| PR #63 repair anti-spam branch `origin/claude/trello-repair-antispam@9ba13a662263346c7fc6754912eec8274348ce7b` | Open draft master-target PR with stale base after the B75/GUER master move: base/merge-base `f8a76de34`, current master `4a163bb65`, local counts `95 1`, and GitHub API mergeability currently `null` / `unknown`. The clean base-relative diff is eight maintained-root files / +82 / -12. In both roots, `_martyServiceGetPrice` returns `0` for `REPAIR` while `wfbe_veh_repairing` is set (`GUI_Menu_Service.sqf:60` on branch), so repeated single/batch/full-service repair selection is skipped while `Client_SupportRepair.sqf:10-11,77-78,93` owns the local flag. Current master has no `wfbe_veh_repairing` or `STR_WF_INFO_Repair_Veh_InProgress` hits and still spawns `SupportRepair` from the service menu at `GUI_Menu_Service.sqf:183,221,501`. | No EASA menu/module changes. | Branch-only same-client repair anti-spam evidence. It does not close repair/heal affordability, action-time service context, stale funds, rearm/refuel guard gaps on older docs/source, or server-owned service economy. Route the base-repair half through [WASP base-repair system reference](WASP-Base-Repair-System-Reference#2026-06-24-pr-63-repair-anti-spam-branch-addendum) and smoke before stable wording. |
