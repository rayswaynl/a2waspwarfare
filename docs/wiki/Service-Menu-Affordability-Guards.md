...
| --- | --- | --- | --- |
| Current docs/source Chernarus `8b71e2a1` | Buttons still use cached prices before recalculation (`GUI_Menu_Service.sqf:130-190`); rearm/refuel debit without action-time funds/context guards (`:198`, `:219`), while repair/heal only check positive price (`:208`, `:229`). | Purchase still uses strict `_funds > price` and client-side `ChangePlayerFunds` (`GUI_Menu_EASA.sqf:40-53`, action at `:47-49`). | Patch-ready and source-unpatched. Fix Chernarus first if this lane becomes code work. |
| Maintained Vanilla Takistan `8b71e2a1` | Same source shape as Chernarus for service and EASA. | Same strict exact-funds rejection and client-side debit as Chernarus. | Must be propagated deliberately; do not call a Chernarus-only fix complete. |
| Stable `origin/master` `cf2a6d6a` | Newer controller/debit helper exists in both maintained roots (`GUI_Menu_Service.sqf:157,197`), and legacy action branches partially guard rearm/refuel with `_funds >= _price` (`:466`, `:489`). Repair/heal still only check positive price (`:478`, `:501`). | Exact-funds rejection remains in both maintained roots (`GUI_Menu_EASA.sqf:76-78`). | Stable narrows stale-button and rearm/refuel behavior, but it is not complete service/EASA authority or local context hardening. |
| Miksuu `b8389e74`, perf `0076040f` and EASA QoL `a66d4691` | Service action branches still debit from client state: rearm/refuel debit directly and repair/heal only check positive price (`GUI_Menu_Service.sqf:326-358` in checked roots). | Exact-funds rejection remains at old EASA lines for Miksuu/perf and QoL Vanilla (`GUI_Menu_EASA.sqf:47-49`); QoL Chernarus shifts the same strict check/debit to `:58-60`. | No stable-style service guard rescue exists on these refs; QoL only changes Chernarus EASA orientation. |
| Release `origin/release/2026-06-feature-bundle` `a96fdda2` | Matches the stable partial shape in both maintained roots: helper debits at `:157`, `:197`; rearm/refuel guard with `_funds >= _price` at `:466`, `:489`; repair/heal still only positive-price guarded at `:478`, `:501`. | Release Chernarus and Vanilla still use strict `_funds > price` for EASA (`GUI_Menu_EASA.sqf:76-78`). | Treat release as partial local QoL, not a complete service/EASA affordability or authority fix. |
