...
Refreshed 2026-06-23. Docs/source service/EASA paths were checked through `docs/developer-wiki-index@b798fd66` and remain source-unchanged from `e9dd7f37`, `8906ee89`, `9b3fc38e` and `8b71e2a1` for the paths below. Current stable/B74.1 is `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`; B74.2 is `origin/claude/b74.2-aicom@d472da6a`; current B69 is `origin/claude/b69@8d465fce`; adjacent B74 is `origin/claude/b74-aicom-spend@b23f557f`. Targeted path diffs `0139a3468609..origin/master`, `origin/master..origin/claude/b74.2-aicom` and `origin/claude/b69..origin/claude/b74-aicom-spend` are empty for checked service/EASA menu paths except the old-stable-to-current Chernarus/Vanilla `GUI_Menu_EASA.sqf` exact-funds/categorization drift. Miksuu upstream is `miksuu/master@b8389e748243`; `origin/perf/quick-wins@0076040f` is still old-shape. Current origin exposes no live `release/*` service/EASA rescue head, but it does expose two current-stable-based Trello branches that are branch-only UI/wiring evidence: `origin/claude/trello-service-ammo-count@9159f935874e` and `origin/claude/trello-easa-weapon-categories@0881911849da`.
...
| Current stable/B74.1 `origin/master@f8a76de34` / B74.2 `origin/claude/b74.2-aicom@d472da6a` | Newer controller/debit helper exists in both maintained roots (`GUI_Menu_Service.sqf:167,207`), button enable checks require positive prices and funds (`:421`, `:448-455`), and legacy action branches partially guard rearm/refuel with `_funds >= _price` (`:484`, `:507`). Repair/heal still only check positive price at the debit point (`:496-497`, `:519-520`). B74.2 has no checked service/EASA menu delta over current stable. | Chernarus now allows exact-funds purchase with `_funds >= price` (`GUI_Menu_EASA.sqf:118-120`), but maintained Vanilla still rejects exact funds with `_funds > price` at the same lines. Current stable also has the guarded category call site (`GUI_Menu_EASA.sqf:34-36`) and default-on `WFBE_C_EASA_CATEGORIES` constant (Chernarus `Init_CommonConstants.sqf:821`, Vanilla `:623`) but does not compile `WFBE_EASA_FNC_LoadoutCat`. | Current stable/B74.1/B74.2 narrow stale-button and rearm/refuel behavior and fix Chernarus exact-funds EASA only. Repair/heal, action-time context and server authority remain open; Vanilla exact-funds remains open. |
