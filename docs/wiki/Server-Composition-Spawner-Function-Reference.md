...
> Source-verified 2026-06-24 for auto-wall/spawner notes against docs/source `HEAD@de471ea5ba96` (unchanged from `eca0e4a7` for checked auto-wall paths), current stable/B74.1 `origin/master@f8a76de34`, B74.2 `origin/claude/b74.2-aicom@21b62b04`, B69 `8d465fce`, B74 `b23f557f`, Miksuu commit `b8389e748243`, `origin/perf/quick-wins@0076040f`, live `origin/feat/oa-auto-countermeasures@2015a632` and historical `a96fdda2`. Paths relative to Missions/[55-2hc]warfarev2_073v48co.chernarus/ unless noted. Arma 2 OA 1.64.
...
The Medium and Small site callers resolve the wall template via `missionNamespace getVariable format ["WFBE_NEURODEF_%1_WALLS", _rlType]`. The HQ caller passes the hardcoded string `"WFBE_NEURODEF_HEADQUARTERS_WALLS"` directly (no `format` call, no `_rlType` variable). Branch note from the 2026-06-24 B74.2 current-head auto-wall refresh: docs/source, current Miksuu and perf have no SmallSite/MediumSite structure exclusions beyond the global toggle, historical `a96fdda2` and live `origin/feat/oa-auto-countermeasures@2015a632` exclude only `AARadar`, and current stable/B74.1/B74.2/B69/B74 exclude `AARadar`/`CBRadar` for SmallSite plus `AARadar`/`Bank`/`Reserve`/`ArtilleryRadar` for MediumSite. `d472da6a..21b62b04` is empty for checked auto-wall paths, and B74.2 construction diffs add source-Chernarus stats attribution only, not a wall-gate change.

**Auto-wall gate:** `isAutoWallConstructingEnabled` is a mission-global value. Docs/source `HEAD@de471ea5ba96`, current Miksuu and perf initialize it `false` at `Common/Init/Init_Common.sqf:201`; current stable/B74.1, current B74.2, B69 and B74 initialize it `true` at `:214`; historical `a96fdda2` initializes it `true` at `:201`, while live `origin/feat/oa-auto-countermeasures@2015a632` initializes it `true` at `:202` with the same global request path. Players with the commander role can toggle it via `User14` keybind (`coin_interface.sqf:180,207-217`); the new value is sent to the server via the `RequestAutoWallConstructinChange` public variable (`Common/Init/Init_PublicVariables.sqf:21`; handler writes one global at `RequestAutoWallConstructinChange.sqf:3-7`).
