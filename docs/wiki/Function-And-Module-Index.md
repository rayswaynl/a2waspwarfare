...
- `Server_FNC_Delegation`: selects delegation targets for town/player AI. No `setGroupOwner` rebalancing path has been found; OA 1.64 has no live group-transfer equivalent, so rebalancing can only affect future spawns.
- `Server_AssignNewCommander`: commander assignment notification/AI-commander stop helper; current source/Vanilla unpack `_side = _this select 0` for the caller array and `RequestNewCommander.sqf` no longer sends the duplicate caller-side reassignment notification.
