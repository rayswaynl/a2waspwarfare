...
Canonical implementation pages remain [Client UI systems atlas](Client-UI-Systems-Atlas), [Client UI/HUD/menus](Client-UI-HUD-And-Menus), [Commander vote/reassignment](Commander-Vote-And-Reassignment-Playbook), [Gear/loadout/EASA atlas](Gear-Loadout-And-EASA-Atlas), [Service menu affordability guards](Service-Menu-Affordability-Guards), [UI IDD collision repair](UI-IDD-Collision-Repair), [Respawn/death lifecycle](Respawn-And-Death-Lifecycle-Atlas), [Skin selector/class swap](Skin-Selector-And-Class-Swap-Reference), [QoL trio player hints](QoL-Trio-Player-Hints-Reference) and [WASP overlay](WASP-Overlay). This page is the player-clickable workflow tour, not the detailed dialog atlas.
...
| Skin selector/class swap | Optional infantry skin dialog opens on join, through WF-menu action 21 or through `User11` only when `WFBE_C_SKIN_SELECTOR` is enabled; Apply creates a replacement player body, restores client actions/handlers and persists the chosen class for respawn reapply. | `Client/Module/Skill/Skill_Init.sqf:75-77`; `Client/GUI/GUI_Menu.sqf:211-215`; `Client/Init/Init_Keybind.sqf:1-14`; `WASP/actions/SkinSelector/SkinSelector_Apply.sqf:207-257`; `Client/Functions/Client_OnRespawnHandler.sqf:130-146` | Source-present but disabled by default; full route is [Skin selector/class swap](Skin-Selector-And-Class-Swap-Reference). |
...
| QoL hints | Receive local salvage payout toast, factory-upgrade unlock banner and advisor nudges for unspent funds or unreached Patrols research. | `Client/FSM/updatesalvage.sqf:50-55`; `Client/Functions/Client_FNC_Special.sqf:204-235`; `Client/Functions/Client_QOL_Advisor.sqf:55-90` | Live; full route is [QoL trio player hints](QoL-Trio-Player-Hints-Reference). |
...
| Command | Commander changes team properties, respawn, auto-AI and buy-type settings. Move/Patrol/Defense are two-step: the button arms `MenuAction`, then a map click writes replicated move mode/destination variables and spawns marker/radio feedback. Task send is branch-split: current stable/B69/B74 and `origin/feat/naval-hvt-objectives` revive Commander Objective Ping (`MenuAction 306` -> `SetTask` via `WFBE_CO_FNC_SendToClient` to selected/all player-led squad leaders), while docs/source, Miksuu and perf still leave the sends commented. | `Rsc/Dialogs.hpp:1789-1792`; `GUI_Menu_Command.sqf:252-306`, `:315-348`, `:477-501` | Mixed: property PVF is live, map-order executor proof is pending, Objective Ping is source-present/smoke-pending only on current stable-shaped branches. |
...
| Stale or partial surfaces | Old upgrade dialog, visible command task controls, help unload mismatch, duplicate EASA/Economy IDD and duplicate title-resource IDs. | [UI IDD collision repair](UI-IDD-Collision-Repair), [Abandoned feature revival](Abandoned-Feature-Revival-Review#old-upgrade-dialog), [Commander vote/reassignment](Commander-Vote-And-Reassignment-Playbook) |
...
Related: [Player vehicle/travel actions](Player-Vehicle-And-Travel-Actions-Reference) | [Feature status](Feature-Status-Register) | [Server authority map](Server-Authority-Migration-Map) | [Testing workflow](Testing-Debugging-And-Release-Workflow)
