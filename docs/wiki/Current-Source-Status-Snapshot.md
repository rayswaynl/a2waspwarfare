...
| --- | --- | --- | --- |
| docs mirror branch `origin/docs/developer-wiki-index` | source-check basis `154b7f38`; later docs-only commits may advance the branch | Documentation-only commits since the source-fix batch; source line shapes for the 2026-06-03 propagated-fix matrix remain the same as the earlier docs/source check. A follow-up fetch saw the branch at `d9416609` before this wording fix, again with docs-only changes. | Current wiki/docs mirror truth for documentation. Do not treat this branch name alone as gameplay release proof. |
