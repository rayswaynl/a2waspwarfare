...
| New player / commander | [New player quickstart](New-Player-Quickstart-Guide) | [Commander's Handbook](Commanders-Handbook), [Upgrade costs](Upgrade-Research-Cross-Faction-Reference), [Unit/vehicle roster](Faction-Unit-And-Vehicle-Roster-Catalog) |
...
| --- | --- |
| Startup, lifecycle, includes | [Architecture overview](Architecture-Overview) -> [Mission entrypoints](Mission-Entrypoints-And-Lifecycle) -> [Mission config/version graph](Mission-Config-Version-Include-Graph) -> [Lifecycle wait-chain](Lifecycle-Wait-Chain) -> [Experimental feature-flag constants](Experimental-Feature-Flag-Constants-Reference) -> [Per-unit client init pipeline](Per-Unit-Client-Init-Pipeline-Reference) -> [Server-init deadspawn & airfield probe](Server-Init-Deadspawn-And-Airfield-Probe) -> [Mission tunable constants catalog](Mission-Tunable-Constants-Catalog) -> [Stringtable localization catalog](Stringtable-Localization-Catalog) |
...
| Tools, build, release | [Tools/build workflow](Tools-And-Build-Workflow) -> [Operator monitor/CPU affinity tools](Operator-Monitor-And-CPU-Affinity-Tools-Reference) -> [Source fix propagation queue](Source-Fix-Propagation-Queue) -> [LoadoutManager build configs](LoadoutManager-Build-Configurations-And-Generated-Defines) -> [Arma2Warfare GPT assistant](Arma2Warfare-GPT-Assistant-Reference) -> [Testing workflow](Testing-Debugging-And-Release-Workflow) |
| Integrations and ops | [External integrations](External-Integrations) -> [Integration trust boundary audit](Integration-Trust-Boundary-Audit) -> [Server runtime and operations](Server-Runtime-And-Operations) -> [Server broadcast & telemetry loops](Server-Broadcast-And-Telemetry-Loop-Reference) -> [GLOBALGAMESTATS extension logging](GLOBALGAMESTATS-Extension-Logging-Subsystem) -> [Discord bot instrumented logging](Discord-Bot-Instrumented-Logging-Framework) -> [Day/night cycle & weather](Day-Night-Cycle-And-Weather-System) |
...
| Map content | [Chernarus](Chernarus-Map-Content-Reference) -> [Takistan](Takistan-Map-Content-Reference) -> [Ruleset model/object config](Map-Ruleset-Model-And-Object-Config) -> [Mission audio catalog](Mission-Audio-Catalog) -> [Eden/Everon & Taviana content](Eden-Everon-And-Taviana-Map-Content-Reference) |
...
| Player feature guides | [New player quickstart](New-Player-Quickstart-Guide) -> [Earning funds & score](Earning-Funds-And-Score-Player-Guide) -> [Vehicle service & logistics](Vehicle-Service-And-Logistics-Player-Guide) -> [Commander's Handbook](Commanders-Handbook) -> [Tactical support menu](Tactical-Support-Menu-Player-Guide) -> [Supply missions](Supply-Mission-Player-Guide) -> [Squad/group join](Player-Squad-Group-Join-Protocol) -> [In-game briefing & Diary](In-Game-Briefing-And-Diary-Field-Manual) |
