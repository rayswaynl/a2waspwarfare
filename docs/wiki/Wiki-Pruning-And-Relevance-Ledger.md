...
| P2 | Overlapping atlases | First gateway-pruning pass is in progress for [Core systems index](Core-Systems-Index) and [Gameplay systems atlas](Gameplay-Systems-Atlas); UI/runtime quick-ref pruning remains a later pass. | Prefer short gateway summaries with strong related-page links. |
