...
| P2 | Overlapping atlases | First gateway-pruning pass is complete for [Core systems index](Core-Systems-Index), [Gameplay systems atlas](Gameplay-Systems-Atlas), [Feature Status](Feature-Status-Register) and the UI/runtime quick-reference pages. Remaining pruning work should be narrow, page-owner based and source-checked. | Prefer short gateway summaries with strong related-page links. |
...
| Fresh pruning scouts on 2026-06-05 | UI/runtime gateway bloat has been source-checked and pruned. Navigation-chain inconsistencies had a first small fix; archive-page caveats remain an optional future pass. | [Client UI/HUD and menus](Client-UI-HUD-And-Menus), [Client UI systems atlas](Client-UI-Systems-Atlas), [Server ops runbook](Server-Ops-Runbook), [Server gameplay runtime atlas](Server-Gameplay-Runtime-Atlas), [Miksuu upstream wiki import](Miksuu-Upstream-Wiki-Import), [Progress dashboard](Progress-Dashboard) | Optional future batch: archive-page caveat standardization and any remaining page-owner-based navigation fixes. | Treat raw scout output as harvested; do not expand gateway pages with duplicate proof. |
