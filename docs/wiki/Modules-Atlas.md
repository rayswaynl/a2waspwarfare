...
Use [Player skill abilities reference](Player-Skill-Abilities-Reference) for the current master class/action catalog. On `cf2a6d6a`, `Skill_Init.sqf` compiles `WFBE_SK_FNC_Apply`, assigns `Engineer`, `Soldier`, `SpecOps`, `Spotter` or `Medic`, and applies the Soldier `ceil(1.5 * WFBE_C_PLAYERS_AI_MAX)` local cap boost (`Client/Module/Skill/Skill_Init.sqf:9-10`; `Client/Module/Skill/Skill_Init.sqf:39-49`). `Skill_Apply.sqf` wires the current action set: Engineer repair/salvage/camp repair, SpecOps supply load/unload plus fast repair, Spotter spot/lockpick/fast repair/camp repair, and Medic/Soldier fast repair plus camp repair (`Client/Module/Skill/Skill_Apply.sqf:12-39`; `Client/Module/Skill/Skill_Apply.sqf:48-83`; `Client/Module/Skill/Skill_Apply.sqf:84-121`; `Client/Module/Skill/Skill_Apply.sqf:125-157`). The old Officer/MASH wording is branch-sensitive: current master keeps a latent `Officer` case that says MASH deploy was removed, while the initializer does not assign the Officer type (`Client/Module/Skill/Skill_Init.sqf:39-45`; `Client/Module/Skill/Skill_Apply.sqf:42-45`).
