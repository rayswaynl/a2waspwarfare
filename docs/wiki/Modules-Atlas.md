...
| MASH markers | [Respawn and death lifecycle](Respawn-And-Death-Lifecycle-Atlas#mash-split-live-respawn-dead-marker-relay) | Old-shape docs/Miksuu/perf roots have a commented client receiver at `Init_Client.sqf:132` and server marker helper compile/comment split at `Init_Server.sqf:70,92`; current-stable/B74-shaped maintained roots remove the deploy/module path. |
| UAV terminal/module | [Support specials](Support-Specials-And-Tactical-Modules-Atlas), [Client UI systems](Client-UI-Systems-Atlas) | Current UI keeps the `_button == 007` branch disabled in the A2 interface (`uav_interface.sqf:226`, inside `comment 'DISABLED'`); the OA interface (`uav_interface_oa.sqf`) omits the branch entirely — its mousebuttondown handler only reads the button value and takes no action. |
...
| AFKkick | [Public variable channel index](Public-Variable-Channel-Index), [Feature status](Feature-Status-Register#networking--public-variable-hardening-lane-source-backed) | Client init compiles/runs `Client\Module\AFKkick` at `Init_Client.sqf:256-264`; server handler compile is `Init_Server.sqf:63` with an older commented duplicate at `:88`. |
...
Branch-only audio candidate: `origin/claude/trello-audio-cues@a0e4fb458` adds Trello #91 to both maintained roots at branch `Common/Module/IRS/IRS_OnIncomingMissile.sqf:51-61`. After the existing IR-smoke deploy and sound path, it sleeps `WFBE_IRS_FLARE_DELAY` and plays `ARTY_cooldown_over` for the local player only if the vehicle is alive, the player remains crew, `wfbe_irs_flares` is still positive and `wfbe_irs_disabled` is false/default. The `wfbe_irs_disabled` variable belongs to the separate IR-smoke toggle branch, so combined promotion should smoke both branches together.
...
Use [Player skill abilities reference](Player-Skill-Abilities-Reference) for the current-stable class/action catalog. On current stable/B74.1 `origin/master@f8a76de34` and current B74.2 `origin/claude/b74.2-aicom@21b62b04`, `Skill_Init.sqf` compiles `WFBE_SK_FNC_Apply`, assigns the non-Officer skill types, and applies the Soldier local cap boost. `Skill_Apply.sqf` wires the current action set for Engineer, SpecOps, Spotter, Medic and Soldier. The old Officer/MASH wording is branch-sensitive: current-stable/B74-shaped maintained roots keep a latent `Officer` case that says MASH deploy was removed at `Skill_Apply.sqf:43`, while old-shape docs/Miksuu/perf roots still carry local MASH deploy plus orphaned marker relay.
