...
Use [Player skill abilities reference](Player-Skill-Abilities-Reference) for the current-stable class/action catalog. On current stable `origin/master@0139a3468609`, `Skill_Init.sqf` compiles `WFBE_SK_FNC_Apply`, assigns the non-Officer skill types, and applies the Soldier local cap boost. `Skill_Apply.sqf` wires the current action set for Engineer, SpecOps, Spotter, Medic and Soldier. The old Officer/MASH wording is branch-sensitive: current stable keeps a latent `Officer` case that says MASH deploy was removed at `Skill_Apply.sqf:43`, while old-shape docs/Miksuu/perf roots still carry local MASH deploy plus orphaned marker relay.
