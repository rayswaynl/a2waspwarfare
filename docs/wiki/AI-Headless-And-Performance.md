...
- If the HC disconnects mid-mission, Arma 2 OA migrates delegated AI to the server, but this mission does not re-delegate them afterward; `Server/Functions/Server_OnPlayerDisconnected.sqf:26` only removes the HC group from the candidate pool. See [Deep-review findings](Deep-Review-Findings) DR-21.
