...
Historical branch caveat: upstream `HeadlessClientMultithreading` split HC slots by role, introduced typed HC registration, added wrong-name errors and filtered side-less HC client calls. Use [HC upstream history and lessons](HC-Upstream-History-And-Lessons) before flattening `WFBE_HEADLESSCLIENTS_ID`, changing PVF side routing or propagating HC scripts to generated missions.
