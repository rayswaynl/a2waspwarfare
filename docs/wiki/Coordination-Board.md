...
- 2026-06-04 Codex update: `Codebase-Coverage-Ledger` is linked from Home, `_Footer.md`, `llms.txt`, `agent-context.json` and now `_Sidebar.md`; the stale open mailbox status is closed.
...
| `documentation-finisher-loop` | Codex | Active / ongoing | Continue bounded source-backed wiki upkeep, keep the Progress Dashboard compact, and publish scoped validation batches. |
| `autonomous-claude-research` | Claude | Autonomous-ready | Coverage Ledger navigation is wired; Claude may self-select the next bounded source-backed review when active. |
| `feature-status-reconciliation` | Codex / future agent | Watchlist | Keep newly confirmed findings reflected in [Feature status](Feature-Status-Register), owner pages and machine-readable records; no untriaged docs blocker is open right now. |
| `implementation-hardening-from-backlog` | Future code owner | Owner decision / code lane | Pick implementation work from [`agent-hardening-backlog.jsonl`](agent-hardening-backlog.jsonl) only when Steff asks for gameplay patches or a code owner claims it. |
| `testing-debugging-release-workflow` | Codex / future tester | Published release gate | Use [Testing/debugging/release workflow](Testing-Debugging-And-Release-Workflow) to distinguish source review, propagation and actual Arma smoke evidence. |
...
| `pvf-hardening-review` | Claude | Published / integrated | Use [PVF dispatch playbook](PVF-Dispatch-Implementation-Playbook) before implementation; this is no longer a dashboard review blocker. |
| `victory-endgame-review` | Claude | Published / integrated | DR-11..DR-13 and DR-36 are routed through [Deep-review findings](Deep-Review-Findings), [Victory/endgame](Victory-And-Endgame-Atlas), Feature Status and the roadmap. |
| `factory-purchase-authority` | Claude | Published / integrated | DR-14..DR-15 are routed through [Factory/purchase](Factory-And-Purchase-Systems-Atlas), [Factory queue cleanup](Factory-Queue-Counter-Token-Cleanup), Feature Status and the backlog. |
