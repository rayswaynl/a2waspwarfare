...
Current B74.1/B74.2 refresh `town-ai-despawn-current-b741-b742-refresh-2026-06-23` rechecked the maintained roots after current stable advanced to `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34` and B74.2 became visible at `origin/claude/b74.2-aicom@d472da6a`. Current stable/B74.1/B74.2 now carry the tracked-vehicle crew guard in source Chernarus only; maintained Vanilla remains unsafe. Docs/source, Miksuu, perf, historical release and the live AI fleet branch remain old-shape unsafe in both maintained roots. A fresh branch scan also exposed `origin/claude/upstream-town-defense-diag-sync@cde1f1fc` as delegated-cleanup diagnostics evidence, not DR-45 closure.

| Branch / root | Evidence | Status |
| --- | --- | --- |
| Docs/source `HEAD@2a4020586` Chernarus and maintained Vanilla | `server_town_ai.sqf` initializes `wfbe_active_vehicles` at `:30`, appends server-created/delegated vehicles at `:161,:179`, deletes tracked inactive vehicles with only `!(isPlayer leader group _x)` at `:214`, clears at `:219`, and does not check player `crew`. Checked paths are source-unchanged from `3a9ba92e` (`git diff --name-status 3a9ba92e..HEAD` is empty for both maintained-root files). | Patch-ready DR-45 still present in both maintained roots. |
...
| B74.2 `origin/claude/b74.2-aicom@d472da6a` | Source Chernarus line-drifts the current-stable candidate through pop-tier town-AI work: initializes at `:43`, appends at `:254,:273`, skips player units at `:324`, and keeps the crew-count vehicle delete at `:337`. Maintained Vanilla has no checked delta from current stable and remains leader-only at `:319`. | Branch-only source Chernarus line drift over the same partial candidate; maintained Vanilla still needs the guard. |
...
| `Server/FSM/server_town_ai.sqf:205-219` on docs/source `HEAD@2a4020586`; current stable/B74.1 Chernarus line drift `:305-331`; current stable/B74.1 Vanilla `:305-325` | Inactivity cleanup deletes town team units/groups, then town vehicles, then clears `wfbe_active_vehicles`. |
| `Server/FSM/server_town_ai.sqf:214` on docs/source `HEAD@2a4020586`; current stable/B74.1 Vanilla line drift `:319` | Vehicle deletion on old-shape and maintained Vanilla paths checks `alive _x` and `!(isPlayer leader group _x)`, but does not inspect vehicle crew/cargo/turret occupants. Current stable/B74.1/B74.2 source Chernarus instead uses the crew-count guard at `:325` / `:337`. |
