...
Current B69-head refresh `town-ai-vehicle-despawn-current-b69-head-refresh-2026-06-22` rechecked the maintained roots after docs/source advanced to `HEAD@948db4ab` and current B69 stayed at `origin/claude/b69@8d465fce`. Checked `server_town_ai.sqf` paths are unchanged from the older B69 proof commit `0a1ccb4d`: B69 still exposes only a Chernarus candidate guard, current stable remains unsafe, and current origin exposes no `release/*` heads.

| Branch / root | Evidence | Status |
| --- | --- | --- |
| Docs/source `HEAD@948db4ab` Chernarus and maintained Vanilla | `server_town_ai.sqf` initializes `wfbe_active_vehicles` at `:30`, appends server-created/delegated vehicles at `:161,:179`, deletes tracked inactive vehicles with only `!(isPlayer leader group _x)` at `:214`, clears at `:219`, and does not check player `crew`. Checked path is source-unchanged from the earlier `f7aad9a8` proof. | Patch-ready DR-45 still present in both maintained roots. |
| Current stable `origin/master@0139a346` Chernarus and maintained Vanilla | Same tracked-vehicle cleanup shape with line drift: initialize `:41`, append `:238,:257`, delete tracked vehicles at `:309`, clear at `:315`; the adjacent town-team cleanup has HC-locality handling, but the vehicle delete still has no player `crew` check. Stable does not contain `Server_CleanupExpiredTownDefenseAssets.sqf`. | Current stable remains source-unpatched for DR-45; persistent-defense helper evidence is historical. |
...
| `origin/claude/b69@8d465fce` Chernarus and maintained Vanilla | `git diff --name-status 0a1ccb4d..origin/claude/b69` is empty for checked `server_town_ai.sqf` paths. Chernarus contains the same candidate guard: player units in town groups are preserved at `:312` and tracked vehicles delete only when `({isPlayer _x} count crew _x) == 0` at `:325`. Maintained Vanilla on the same current B69 head still deletes units locally without `!isPlayer` at `:309` and tracked vehicles with only `!(isPlayer leader group _x)` at `:319`. | Branch source-present candidate, but Chernarus-only for DR-45. Do not call this closed until maintained Vanilla propagation and Arma smoke are done. |
...
| `Server/FSM/server_town_ai.sqf:205-219` on docs/source `HEAD@948db4ab`; current stable line drift `:294-315` | Inactivity cleanup deletes town team units/groups, then town vehicles, then clears `wfbe_active_vehicles`. |
| `Server/FSM/server_town_ai.sqf:214` on docs/source `HEAD@948db4ab`; current stable line drift `:309` | Vehicle deletion checks `alive _x` and `!(isPlayer leader group _x)`, but does not inspect vehicle crew/cargo/turret occupants. |
...
Earlier `89ae9dad`-era docs recorded a town-defense diagnostics / captured-defender persistence batch with `Server_CleanupExpiredTownDefenseAssets.sqf`. A fresh 2026-06-22 ref scan found no `Server_CleanupExpiredTownDefenseAssets.sqf` path in docs/source `HEAD@948db4ab`, current stable `origin/master@0139a346`, current Miksuu `b8389e748243`, `origin/perf/quick-wins@0076040f`, historical release commit `a96fdda2`, `origin/feat/aicom-fleet-improvements@cc5090bed4ad` or current B69 `origin/claude/b69@8d465fce`.
