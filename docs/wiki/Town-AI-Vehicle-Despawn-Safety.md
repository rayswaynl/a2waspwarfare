...
Current B74 refresh `town-ai-despawn-current-b74-refresh-2026-06-23` rechecked the maintained roots after docs/source advanced to `HEAD@3a9ba92e` and adjacent B74 landed at `origin/claude/b74-aicom-spend@b23f557f`. Checked `server_town_ai.sqf` paths are unchanged across B69 and B74: B69/B74 still expose only a Chernarus candidate guard, maintained Vanilla remains unsafe, current stable remains unsafe, and the only matching live remote branch is the already-unsafe AI fleet branch.

| Branch / root | Evidence | Status |
| --- | --- | --- |
| Docs/source `HEAD@3a9ba92e` Chernarus and maintained Vanilla | `server_town_ai.sqf` initializes `wfbe_active_vehicles` at `:30`, appends server-created/delegated vehicles at `:161,:179`, deletes tracked inactive vehicles with only `!(isPlayer leader group _x)` at `:214`, clears at `:219`, and does not check player `crew`. Checked path is source-unchanged from the earlier `f7aad9a8` proof. | Patch-ready DR-45 still present in both maintained roots. |
...
| `origin/claude/b69@8d465fce` and `origin/claude/b74-aicom-spend@b23f557f` Chernarus and maintained Vanilla | `git diff --name-status origin/claude/b69..origin/claude/b74-aicom-spend`, `0a1ccb4d..origin/claude/b74-aicom-spend` and `b8530477..origin/claude/b74-aicom-spend` are empty for checked `server_town_ai.sqf` paths. Chernarus contains the same candidate guard: player units in town groups are preserved at `:312` and tracked vehicles delete only when `({isPlayer _x} count crew _x) == 0` at `:325`. Maintained Vanilla on both heads still deletes units locally without `!isPlayer` at `:309` and tracked vehicles with only `!(isPlayer leader group _x)` at `:319`. | Branch source-present candidate, but Chernarus-only for DR-45. Do not call this closed until maintained Vanilla propagation and Arma smoke are done. |
...
| `Server/FSM/server_town_ai.sqf:205-219` on docs/source `HEAD@3a9ba92e`; current stable line drift `:294-315` | Inactivity cleanup deletes town team units/groups, then town vehicles, then clears `wfbe_active_vehicles`. |
| `Server/FSM/server_town_ai.sqf:214` on docs/source `HEAD@3a9ba92e`; current stable line drift `:309` | Vehicle deletion checks `alive _x` and `!(isPlayer leader group _x)`, but does not inspect vehicle crew/cargo/turret occupants. |
...
Earlier `89ae9dad`-era docs recorded a town-defense diagnostics / captured-defender persistence batch with `Server_CleanupExpiredTownDefenseAssets.sqf`. A fresh 2026-06-23 ref scan found no `Server_CleanupExpiredTownDefenseAssets.sqf` path in docs/source `HEAD@3a9ba92e`, current stable `origin/master@0139a346`, current Miksuu `b8389e748243`, `origin/perf/quick-wins@0076040f`, historical release commit `a96fdda2`, `origin/feat/aicom-fleet-improvements@cc5090bed4ad`, current B69 `origin/claude/b69@8d465fce` or adjacent B74 `origin/claude/b74-aicom-spend@b23f557f`.
