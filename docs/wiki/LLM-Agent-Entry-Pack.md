...
| Do not claim Vanilla propagation unless LoadoutManager ran and generated diffs were inspected. | Branch-sensitive root discovery: docs/current source and current stable `origin/master@0139a346` accept either an `a2waspwarfare` ancestor or repo markers; direct current Miksuu `b8389e74` and `origin/perf/quick-wins@0076040f` still need the ancestor. Use `A2WASP_SKIP_ZIP=1` for propagation-only runs; runtime smoke is still a separate gate. |
