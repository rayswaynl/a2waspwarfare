...
| 2026-06-23 side-supply/B74 branch refresh | Docs/source `HEAD@a908284c` is source-unchanged from `4db90f1c` / `7047da5d9` / `f52ccee8` for the checked side-supply/reason paths. Docs checkout, current stable `origin/master@0139a346` and current Miksuu upstream `b8389e748243` still use `_currentSupply - _amount` for negative side-supply results in Chernarus and maintained Vanilla, still trust the direct temp-channel payload side, and still gate `_reason` behind `count _this > 3`. Current origin exposes no `release/*` or side-supply/economy rescue heads; historical release `a96fdda2` kept the same old shape. `origin/perf/quick-wins@0076040f` fixes only the Chernarus arithmetic floor to `0`. Current B69 `origin/claude/b69@8d465fce` and adjacent B74 `origin/claude/b74-aicom-spend@b23f557f` have no checked side-supply path delta; both fix only Chernarus server-side arithmetic and add a Chernarus resistance temp-channel handler, while maintained Vanilla stays old-shape and side/channel/requester validation plus reason parsing remain open. |
...
| Current B69 `origin/claude/b69@8d465fce` and adjacent B74 `origin/claude/b74-aicom-spend@b23f557f` | B69..B74 has no checked side-supply path delta. Chernarus and maintained Vanilla still keep the `count _this > 3` reason guard (`Common_ChangeSideSupply.sqf:8-14`); the Chernarus arithmetic/resistance work does not alter reason parsing. | Same AttackWave/supply-completion contrast; Chernarus publishes through line-drifted `wfbe_supply_temp_<side>` at `Common_ChangeSideSupply.sqf:24-26`, while Vanilla keeps the old `:28-30` publish lines. | Do not treat B69 or B74 as a reason-string fix. If porting the B69/B74 side-supply arithmetic, still patch reason parsing in both maintained roots. |
...
| Current B69 `origin/claude/b69@8d465fce` and adjacent B74 `origin/claude/b74-aicom-spend@b23f557f` | B69..B74 has no checked side-supply path delta. Chernarus only: `Common_ChangeSideSupply.sqf:20-22` removes dead local clamp arithmetic; server west/resistance/east handlers floor negatives to `0` at `Server_ChangeSideSupply.sqf:12,36,60`. Maintained Vanilla remains old-shape (`Common_ChangeSideSupply.sqf:25`; `Server_ChangeSideSupply.sqf:12,36`). | Chernarus adds `wfbe_supply_temp_resistance` at `Server_ChangeSideSupply.sqf:25-45`, but west/resistance/east handlers still read `_side` from payload and do not validate channel suffix/requester. Vanilla still has only west/east handlers. | Stronger Chernarus candidate than perf because resistance is wired, but still not current stable, not Vanilla-propagated, and not a DR-44 validation/authority closure. |
...
For income system `4`, server payout applies a `1.5` multiplier before commander/player split (`updateresources.sqf:42-43` on old-shape refs, `:56-57` on current stable, `:69-70` on B69/B74), while `Client_GetIncome.sqf:24-28` displays the split without that multiplier. UI work around RHUD/menu income should preserve or deliberately correct this mismatch with balance owner approval.
