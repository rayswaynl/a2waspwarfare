...
| Current B69 `origin/claude/b69@8d465fce` (checked side-supply paths unchanged from `39eed5c0`) | Chernarus and maintained Vanilla still keep the `count _this > 3` reason guard (`Common_ChangeSideSupply.sqf:8-14`); B69's Chernarus arithmetic/resistance work does not alter reason parsing. | Same AttackWave/supply-completion contrast; Chernarus publishes through line-drifted `wfbe_supply_temp_<side>` at `Common_ChangeSideSupply.sqf:24-26`, while Vanilla keeps the old `:28-30` publish lines. | Do not treat B69 as a reason-string fix. If porting B69 side-supply arithmetic, still patch reason parsing in both maintained roots. |
