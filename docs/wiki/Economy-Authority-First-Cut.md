...
| 2026-06-14 branch refresh | Docs checkout `f52ccee8`, stable `origin/master` `cf2a6d6a`, release `a96fdda2` and Miksuu upstream `b8389e74` all still use `_currentSupply - _amount` for negative side-supply results in Chernarus and maintained Vanilla, still trust the direct temp-channel payload side, and still gate `_reason` behind `count _this > 3`. `origin/perf/quick-wins` `0076040f` fixes only the Chernarus arithmetic floor to `0`; maintained Vanilla, side/channel validation and the reason-string fix remain open. |
...
| --- | --- | --- | --- |
| Docs checkout `f52ccee8` Chernarus | `Common_ChangeSideSupply.sqf:8-14` reads both `_includeStagnation` and `_reason` only when `count _this > 3`; `:28` publishes the selected reason through `wfbe_supply_temp_<side>`. | `Server/PVFunctions/AttackWave.sqf:40` passes three arguments and loses `"Heavy attack mode activated."`; `Server/Module/supplyMission/supplyMissionCompleted.sqf:26` passes four arguments and keeps its formatted reason. | Patch-ready low-risk diagnostics cleanup. Parse `_reason` at `count _this > 2` and `_includeStagnation` at `count _this > 3`, while retaining the default error text for malformed/no-reason payloads. |
| Docs checkout `f52ccee8` maintained Vanilla | Same helper guard and same AttackWave/supply-completion caller shape in the maintained root. | Same three-argument AttackWave reason drop; same four-argument supply mission reason preservation. | Propagate with the Chernarus helper edit; do not call the reason fix source-complete if Vanilla is still old. |
| Stable `origin/master` `cf2a6d6a` | Same `count _this > 3` reason guard in both maintained roots. | Same AttackWave three-argument drop; supply completion is the heli-aware four-argument path at `supplyMissionCompleted.sqf:40`. | Stable remains source-unpatched for audit provenance. |
| Release `origin/release/2026-06-feature-bundle` `a96fdda2` | Same `count _this > 3` reason guard in both maintained roots. | Same AttackWave three-argument drop; supply completion is the heli-aware four-argument path at `supplyMissionCompleted.sqf:40`. | Release does not rescue the reason-string lane. Keep line-drift notes when cherry-picking docs or patches. |
...
| --- | --- | --- | --- |
| Docs checkout `f52ccee8` Chernarus | Old bug remains: `Common_ChangeSideSupply.sqf:24-30` computes `_change`, then floors negatives with `_currentSupply - _amount`; `Server_ChangeSideSupply.sqf:11-13,35-37` does the same for west/east handlers. | Open: handlers at `Server_ChangeSideSupply.sqf:1-21,25-45` read `_side` from the payload and write `wfbe_supply_%1`; channel suffix does not constrain the mutated side. | Patch-ready and current-source-unpatched. Clamp to `0`, keep max cap, validate side/channel/amount shape, and still treat spend authorization as future server-ledger work. |
| Docs checkout `f52ccee8` maintained Vanilla | Same old floor in `Common_ChangeSideSupply.sqf:24-30` and `Server_ChangeSideSupply.sqf:11-13,35-37`. | Same payload-trusting west/east handlers. | Any fix must be propagated; do not cite a Chernarus-only branch as Vanilla-ready. |
| Stable `origin/master` `cf2a6d6a` | Same old floor in Chernarus and Vanilla (`Common_ChangeSideSupply.sqf:25`; `Server_ChangeSideSupply.sqf:12,36`). | Same direct `wfbe_supply_temp_west/east` handlers and payload-side trust. | Stable remains vulnerable to overspend-as-credit plus direct-channel authority. |
| Release `origin/release/2026-06-feature-bundle` `a96fdda2` | Same old floor in release Chernarus and release Vanilla. | Same payload-trusting direct temp channels. | Release branch does not rescue the side-supply lane. |
