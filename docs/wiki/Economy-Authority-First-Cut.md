...
| 2026-06-21 side-supply branch refresh | Docs branch `7047da5d9` is source-unchanged from `f52ccee8` for the checked side-supply/reason paths. Docs checkout, current stable `origin/master@0139a346` and Miksuu upstream `b8389e74` all still use `_currentSupply - _amount` for negative side-supply results in Chernarus and maintained Vanilla, still trust the direct temp-channel payload side, and still gate `_reason` behind `count _this > 3`. Current origin exposes no `release/*` heads; historical release `a96fdda2` kept the same old shape. `origin/perf/quick-wins@0076040f` fixes only the Chernarus arithmetic floor to `0`; maintained Vanilla, side/channel validation and the reason-string fix remain open. |
...
| --- | --- | --- | --- |
| Docs branch `7047da5d9` Chernarus, unchanged from `f52ccee8` for checked paths | `Common_ChangeSideSupply.sqf:8-14` reads both `_includeStagnation` and `_reason` only when `count _this > 3`; `:28` publishes the selected reason through `wfbe_supply_temp_<side>`. | `Server/PVFunctions/AttackWave.sqf:40` passes three arguments and loses `"Heavy attack mode activated."`; `Server/Module/supplyMission/supplyMissionCompleted.sqf:26` passes four arguments and keeps its formatted reason. | Patch-ready low-risk diagnostics cleanup. Parse `_reason` at `count _this > 2` and `_includeStagnation` at `count _this > 3`, while retaining the default error text for malformed/no-reason payloads. |
| Docs branch `7047da5d9` maintained Vanilla, unchanged from `f52ccee8` for checked paths | Same helper guard and same AttackWave/supply-completion caller shape in the maintained root. | Same three-argument AttackWave reason drop; same four-argument supply mission reason preservation. | Propagate with the Chernarus helper edit; do not call the reason fix source-complete if Vanilla is still old. |
| Current stable `origin/master@0139a346` | Same `count _this > 3` reason guard in both maintained roots. | Same AttackWave three-argument drop; supply completion is the heli-aware four-argument path at `supplyMissionCompleted.sqf:40`. | Stable remains source-unpatched for audit provenance. |
| Historical release commit `a96fdda2`; no current `release/*` head on 2026-06-21 | Same `count _this > 3` reason guard in both maintained roots. | Same AttackWave three-argument drop; supply completion is the heli-aware four-argument path at `supplyMissionCompleted.sqf:40`. | Historical release evidence does not rescue the reason-string lane. Recheck a restored release head before release wording. |
| Miksuu upstream `b8389e74` | Same `count _this > 3` reason guard in both maintained roots. | Same AttackWave/supply-completion contrast, with supply completion at `supplyMissionCompleted.sqf:26`. | No upstream rescue candidate in the checked upstream head. |
...
| --- | --- | --- | --- |
| Docs branch `7047da5d9` Chernarus, unchanged from `f52ccee8` for checked paths | Old bug remains: `Common_ChangeSideSupply.sqf:24-30` computes `_change`, then floors negatives with `_currentSupply - _amount`; `Server_ChangeSideSupply.sqf:11-13,35-37` does the same for west/east handlers. | Open: handlers at `Server_ChangeSideSupply.sqf:1-21,25-45` read `_side` from the payload and write `wfbe_supply_%1`; channel suffix does not constrain the mutated side. | Patch-ready and current-source-unpatched. Clamp to `0`, keep max cap, validate side/channel/amount shape, and still treat spend authorization as future server-ledger work. |
| Docs branch `7047da5d9` maintained Vanilla, unchanged from `f52ccee8` for checked paths | Same old floor in `Common_ChangeSideSupply.sqf:24-30` and `Server_ChangeSideSupply.sqf:11-13,35-37`. | Same payload-trusting west/east handlers. | Any fix must be propagated; do not cite a Chernarus-only branch as Vanilla-ready. |
| Current stable `origin/master@0139a346` | Same old floor in Chernarus and Vanilla (`Common_ChangeSideSupply.sqf:25`; `Server_ChangeSideSupply.sqf:12,36`). | Same direct `wfbe_supply_temp_west/east` handlers and payload-side trust. | Stable remains vulnerable to overspend-as-credit plus direct-channel authority. |
| Historical release commit `a96fdda2`; no current `release/*` head on 2026-06-21 | Same old floor in Chernarus and maintained Vanilla when last checked. | Same payload-trusting direct temp channels. | Historical release evidence does not rescue the side-supply lane. Recheck a restored release head before release wording. |
| Miksuu upstream `b8389e74` | Same old floor in Chernarus and Vanilla. | Same payload-trusting direct temp channels. | No upstream rescue candidate in current upstream head. |
...
Old-shape refs place the side-supply increase, team paychecks and AI-commander funds inside an `_supply < WFBE_C_ECONOMY_SUPPLY_MAX_TEAM_LIMIT` guard: docs branch `d7a30e15`, Miksuu `b8389e74`, `origin/perf/quick-wins@0076040f` and historical release commit `a96fdda2` keep `Server/FSM/updateresources.sqf:29-70` with the guard at `:31`, side supply at `:49`, team funds at `:63` and AI commander funds at `:67`. `_supply` is the computed town supply income for the side, so the guard is not simply "current side supply is full". Do not refactor the income loop as a pure supply-cap cleanup without checking player/commander money payouts.

Current stable `origin/master@0139a346` is now branch-split: maintained Chernarus and Vanilla still guard side-supply growth (`updateresources.sqf:63`) and team paychecks (`:77`) behind `_supply < _supply_max_limit` at `:45`, but AI commander cash has an over-cap income/stipend fallback at `:102-109`. The current branch/root route lives in [Economy, towns and supply](Economy-Towns-And-Supply#resource-income-branch-matrix); perf Chernarus changes the resource-loop wait cadence, not this payout/display behavior.

For income system `4`, server payout applies a `1.5` multiplier before commander/player split (`updateresources.sqf:42-43` on old-shape refs, `:56-57` on current stable), while `Client_GetIncome.sqf:24-28` displays the split without that multiplier. UI work around RHUD/menu income should preserve or deliberately correct this mismatch with balance owner approval.
