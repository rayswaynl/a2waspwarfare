...
Scope: Chernarus source mission first, Arma 2 Operation Arrowhead 1.64 only, then LoadoutManager propagation. It complements [Documentation implementation plan](Documentation-Implementation-Plan), [Economy, towns and supply](Economy-Towns-And-Supply), [Public variable channel index](Public-Variable-Channel-Index), [PVF dispatch playbook](PVF-Dispatch-Implementation-Playbook), [Networking authority surfaces](Networking-And-Public-Variables#authority-surfaces-to-audit-together) and [Pending owner decisions](Pending-Owner-Decisions).
...
- Existing wiki records: [Deep-review findings](Deep-Review-Findings), [Documentation implementation plan](Documentation-Implementation-Plan), [Pending owner decisions](Pending-Owner-Decisions), [Public variable channel index](Public-Variable-Channel-Index), [Economy, towns and supply](Economy-Towns-And-Supply), [Networking authority surfaces](Networking-And-Public-Variables#authority-surfaces-to-audit-together).
...
`RequestStructure.sqf:3-22` accepts side, class, position and direction from the payload, resolves the structure script and starts construction. `RequestDefense.sqf:2-10` accepts side, class, position, direction and manned flag and calls `ConstructDefense`. The exploit proof and validation rationale live in [Deep-review findings](Deep-Review-Findings) DR-6; this page keeps only the sequencing implications for the economy hardening pass.
...
Canonical finding: [Deep-review findings](Deep-Review-Findings) DR-6. This step is sequenced after side-supply and upgrade authority because it already has server entrypoints, but the final acceptance/debit rules touch more placement and base-area behavior.
...
- [Networking authority surfaces](Networking-And-Public-Variables#authority-surfaces-to-audit-together): `ATTACK_WAVE_INIT` is a direct publicVariable channel with its own server recomputation shape.
...
3. Record the validation in [Tools and build workflow](Tools-And-Build-Workflow) terms.
...
- If owner wants public-server hardening before economy redesign, start from [External integrations](External-Integrations#battleye-filter), [Public variable channel index](Public-Variable-Channel-Index) and DR-30, then design any `publicvariable.txt` / `scripts.txt` filters as a deployment-specific handoff.
