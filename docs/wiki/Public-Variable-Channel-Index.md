...
| `WFBE_CL_MASH_MARKER_CREATED` | (intended) client -> server | **DR-34 / 2026-06-05 branch recheck** - active server relay exists in source/Vanilla, `origin/master`, `miksuu/master` and `origin/release/2026-06-feature-bundle`, but maintained deploy paths do not send this channel. Modded `eden`/`lingor` still emit it, which is sender-only drift rather than proof maintained markers work. | `Server/Module/MASH/MASHMarker.sqf:1-13`, `Client/Module/Skill/Skill_Officer.sqf:25-27`, modded `Skill_Officer.sqf:38-40` |
| `WFBE_SE_MASH_MARKER_SENT` | (intended) server -> clients | **DR-34 / 2026-06-05 branch recheck** - server can rebroadcast, but maintained clients keep the receiver compile commented in source/Vanilla, stable, upstream and release. Revival needs server-held marker state, delete replay and JIP resend; otherwise retire/archive the relay. | `Server/Module/MASH/MASHMarker.sqf:9-11`, `Client/Module/MASH/receiverMASHmarker.sqf:1-29`, `Client/Init/Init_Client.sqf:132` |
...
| `ICBM_launched` | legacy/unclear server -> clients | **Dead-code scan 2026-06-05:** receiver-only legacy handler in source/Vanilla. `Client/FSM/updateclient.sqf:20` registers the PVEH and `OnEventHandler_ICBM_Launch.sqf` documents the intended payload, but no active `publicVariable "ICBM_launched"` sender was found. Current tactical nuke flow uses `GUI_Menu_Tactical.sqf:499` -> `Client/Module/Nuke/nukeincoming.sqf:23,27` -> `RequestSpecial ["ICBM", ...]` and `HandleSpecial "icbm-display"`. Retire or revive deliberately; do not delete ICBM itself. | `Client/FSM/updateclient.sqf:20`, `Client/Module/Nuke/OnEventHandler_ICBM_Launch.sqf:5-9`, `Client/Module/Nuke/nukeincoming.sqf:23,27`, `Client/PVFunctions/HandleSpecial.sqf:22` |
