...
- **PVF (registered):** travels as `WFBE_PVF_<Name>`; the dispatcher `Spawn`s `WFBE_SE_FNC_HandlePVF` / `WFBE_CL_FNC_HandlePVF`, which **`Call Compile` the sender-chosen command string** — the DR-1 RCE/forgery surface. Payload fields are trusted (no server re-derivation) unless a specific handler validates them, and DR-55 confirms the server path does not pass authenticated sender context to those handlers.
...
| `origin/release/2026-06-feature-bundle` `a96fdda2` | Same receiver registration at `updateclient.sqf:12` and receiver compile at `Client_onEventHandler_SEND_MESSAGE.sqf:27` in release Chernarus and release maintained Vanilla. | Same helper compile at `Common_SendMessage.sqf:26` and broadcast at `:38` in both release roots. | Current release head is still source-unpatched for DR-46; keep it in the P0 patch-ready queue. |

The `89ae9dad..cf2a6d6a` diff changes `updateclient.sqf` context in both maintained roots, but it does not touch `Client_onEventHandler_SEND_MESSAGE.sqf` or `Common_SendMessage.sqf`. DR-46 remains source-unpatched.
