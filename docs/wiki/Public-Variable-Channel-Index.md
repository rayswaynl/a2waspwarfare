...
| `WFBE_Server_PV_SupplyMissionCompletedMessage` | server -> clients | supply completion message and client-side player cash award; exact channel matters for BattlEye filters and should not be hidden behind `...CompletedMessage` shorthand | `Server/Module/supplyMission/supplyMissionCompleted.sqf:24,34`, `Client/Module/supplyMission/supplyMissionCompletedMessage.sqf:2` |
...
| `SERVER_FPS_GUI` / `WFBE_VAR_SERVER_FPS` | server → clients | server FPS GUI/HUD publication. Current source Chernarus player UI reads `SERVER_FPS_GUI`; `WFBE_VAR_SERVER_FPS` is a second dedicated publisher with no source Chernarus player-UI reader found, so future ops cleanup should either preserve it as a compatibility channel or retire it deliberately. | `Server/GUI/serverFpsGUI.sqf:6-7`, `Server/Module/serverFPS/monitorServerFPS.sqf:5-6`, `Client/Client_UpdateRHUD.sqf:113`, `Server/Init/Init_Server.sqf:578,595` |
