...
| P2 | Tooling gates are now machine-readable, but not enforced by CI. | `agent-release-readiness.json` `toolingAuditGates` records packaging exit-code, soft-copy/delete, analyzer, DiscordBot/Extension contract and BattlEye filter gates. | Turn those gate records into scripts or CI checks before using tool output as release proof. |
