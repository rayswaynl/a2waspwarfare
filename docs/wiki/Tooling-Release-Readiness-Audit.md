...
| DiscordBot presence timeout | Channel rename passes a cancel token, but `SetGameAsync` sits inside a timeout-looking block without receiving the token. | Medium reliability issue if Discord presence update hangs. | `GameStatusUpdater.cs:91-106`. |
