...
Checked on 2026-06-22 after `git fetch --all --prune`. No checked branch or maintained root fixes the direct-PV authority issue. Docs/source `HEAD@1c1ea55970dc` is unchanged from the older `f3e157f2` line-anchor proof for checked Chernarus/Vanilla attack-wave request/detail/client-price paths. Current origin exposes no live `release/*`, `feat/ai-commander` or attack-wave feature heads on 2026-06-22, so `a96fdda2`, `c20ce153` and `994150da` remain historical evidence. A late fetch exposed adjacent `origin/claude/b74-aicom-spend@b23f557fc912`; it changes only Chernarus constants among checked attack-wave paths and is not an authority rescue.

| Ref | Chernarus source | Maintained Vanilla | Result |
| --- | --- | --- | --- |
| Docs/source `HEAD@1c1ea55970dc` | `updateclient.sqf:240` gates the action client-side; `Common_AttackWaveActivate.sqf:6,8` sends `ATTACK_WAVE_INIT`; `Server_AttackWave.sqf:5-6,15,23,27,36,38` trusts payload `_supply` / `_side` and publishes details; `AttackWave.sqf:19,23-25,40` trusts detail payload and debits side supply; constants are at `Init_CommonConstants.sqf:166,197-199`. | Same maintained-root line shape. | Patch-ready, current-docs-source-unpatched. |
