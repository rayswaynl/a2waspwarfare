...
| Profile template import guard allows six-element rows before reading index 6. | `Client/Init/Init_ProfileGear.sqf:17` accepts `count _x >= 6`, then `:25` reads `_x select 6` for backpack data. | Require `count _x >= 7` before selecting backpack data, or branch old six-field templates through a compatibility default. Smoke old profile rows and current seven-field rows. |
