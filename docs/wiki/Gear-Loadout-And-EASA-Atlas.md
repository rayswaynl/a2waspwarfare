...
| Vehicle/backpack cargo application | Docs checkout, Miksuu and historical EASA QoL commit `a66d4691` still carry inclusive loops in both maintained roots. Current stable and historical release commit `a96fdda2` fix all five loops to `count(_items) - 1` in both maintained roots. Perf fixes Chernarus only; perf Vanilla remains old-shape. | [Vehicle cargo equip loop bounds](Vehicle-Cargo-Equip-Loop-Bounds) |
...
Historical EASA QoL commit `a66d4691` adds EASA current-loadout highlighting/preselect in the menu (`GUI_Menu_EASA.sqf:29-40`) but does not change the purchase action at `GUI_Menu_EASA.sqf:46-50` or `EASA_Equip.sqf`. Current origin exposes no `feat/buymenu-easa-qol` head on 2026-06-21, so treat that commit as historical UI-orientation evidence only; exact-funds and stale/unsupported context gates remain open. See [BuyMenu EASA QoL branch audit](BuyMenu-EASA-QoL-Branch-Audit).
...
| Vehicle/backpack cargo equip loops overrun by one. | `Common_EquipVehicle.sqf:27,33,39` and `Common_EquipBackpack.sqf:35,41` use inclusive `for '_i' from 0 to count(_items)` bounds in old-shape targets; [Vehicle cargo equip loop bounds](Vehicle-Cargo-Equip-Loop-Bounds) owns the fix route and [Current Branch Scope](#current-branch-scope) preserves the branch split. | Port or preserve the stable/release `count(_items)-1` fix for any target branch still carrying inclusive loops, then propagate Vanilla and smoke cargo application. |
...
[
  ...
  {"fact":"gear_easa_branch_scope_2026_06_21","source":"docs 43c3ba05 source-unchanged from 8b71e2a1 for checked gear/EASA paths; origin/master@0139a346; Miksuu b8389e74; perf/quick-wins@0076040f; historical release a96fdda2; historical EASA QoL a66d4691","summary":"Gear profile/template save filtering is branch-split: docs/source and old refs keep undefined _u_upgrade, while current stable fixes it in both maintained roots. The six-field profile import guard and template creation/display policy remain open across checked refs; cargo loops are fixed on current stable/historical release, perf Chernarus-only; EASA exact-funds/client debit remains open everywhere checked."},
