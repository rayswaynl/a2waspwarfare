...
Refreshed 2026-06-14 against docs checkout `8b71e2a1`, stable `origin/master` `cf2a6d6a`, Miksuu `b8389e74`, `origin/perf/quick-wins` `0076040f`, release `a96fdda2` and `origin/feat/buymenu-easa-qol` `a66d4691`.

| Area | Branch result | Canonical route |
| --- | --- | --- |
| Profile/template save, import and creation gates | Every checked Chernarus and maintained Vanilla root keeps undefined `_u_upgrade` in `Client_UI_Gear_SaveTemplateProfile.sqf:33,52,75`, the six-field `Init_ProfileGear.sqf:17` guard before reading `:25`, the AddTemplate Barracks-or-Gear gate at `Client_UI_Gear_AddTemplate.sqf:136` and Gear-only FillTemplates display at `Client_UI_Gear_FillTemplates.sqf:17`. | [Gear template profile filter](Gear-Template-Profile-Filter) |
| Vehicle/backpack cargo application | Docs checkout, Miksuu and the EASA QoL branch still carry inclusive loops in both maintained roots. Stable and release fix all five loops to `count(_items) - 1` in both maintained roots. Perf fixes Chernarus only; perf Vanilla remains old-shape. | [Vehicle cargo equip loop bounds](Vehicle-Cargo-Equip-Loop-Bounds) |
| Service rearm/refuel/repair/heal | Docs checkout keeps the old cached-price/action-debit shape. Stable and release use newer controller code and partially guard rearm/refuel in both maintained roots, but repair/heal and broader action-time context/funds checks remain open. Miksuu, perf and the EASA QoL branch do not carry the stable/release rearm/refuel guards. | [Service menu affordability guards](Service-Menu-Affordability-Guards) |
| EASA purchase | All checked roots still use strict `_funds > price` and client-side `EASA_Equip` / `ChangePlayerFunds` debit. Line drift: docs/Miksuu/perf use `GUI_Menu_EASA.sqf:47-49`; stable/release use `:76-78`; the QoL branch uses `:58-60` in Chernarus and old lines in Vanilla. | [Service menu affordability guards](Service-Menu-Affordability-Guards), [BuyMenu EASA QoL branch audit](BuyMenu-EASA-QoL-Branch-Audit) |
| EASA QoL branch | `origin/feat/buymenu-easa-qol` `a66d4691` adds current-loadout highlighting/preselect only in Chernarus; it does not fix exact-funds purchase, stale/unsupported context handling, profile templates, cargo loops or maintained Vanilla propagation. | [BuyMenu EASA QoL branch audit](BuyMenu-EASA-QoL-Branch-Audit) |
...
| Profile template upgrade filtering references undefined `_u_upgrade`. | `Client_UI_Gear_SaveTemplateProfile.sqf:33`, `:52`, `:75`; [Gear template profile filter](Gear-Template-Profile-Filter). Branch check 2026-06-14 found the same shape in docs checkout `8b71e2a1`, stable `cf2a6d6a`, Miksuu `b8389e74`, perf `0076040f`, release `a96fdda2` and the EASA QoL branch `a66d4691` across checked maintained roots. | Fix before trusting saved templates to enforce upgrade gates. |
| Profile template import guard allows six-element rows before reading index 6. | `Client/Init/Init_ProfileGear.sqf:17` accepts `count _x >= 6`, then `:25` reads `_x select 6` for backpack data. Branch check 2026-06-14 found the same shape in docs checkout `8b71e2a1`, stable `cf2a6d6a`, Miksuu `b8389e74`, perf `0076040f`, release `a96fdda2` and the EASA QoL branch `a66d4691` across checked maintained roots. | Require `count _x >= 7` before selecting backpack data, or branch old six-field templates through a compatibility default. Smoke old profile rows and current seven-field rows. |
...
| Vehicle/backpack cargo equip loops overrun by one. | `Common_EquipVehicle.sqf:27,33,39` and `Common_EquipBackpack.sqf:35,41` use inclusive `for '_i' from 0 to count(_items)` bounds in docs checkout `8b71e2a1`, Miksuu `b8389e74` and the EASA QoL branch `a66d4691`; stable `cf2a6d6a` and release `a96fdda2` fix both maintained roots; perf `0076040f` fixes Chernarus only. See [Vehicle cargo equip loop bounds](Vehicle-Cargo-Equip-Loop-Bounds). | Port or preserve the stable/release `count(_items)-1` fix for any target branch still carrying inclusive loops, then propagate Vanilla and smoke cargo application. |
...
| EASA/Economy duplicate dialog IDD. | `Rsc/Dialogs.hpp:3209-3212`, `:3287-3290`; Claude DR-17. Branch check 2026-06-14: docs checkout `2fef1e3d`, Miksuu `b8389e74` and perf `0076040f` still duplicate `23000` in both maintained roots; stable `cf2a6d6a` and release `a96fdda2` move EASA to `24000` while Economy stays `23000` in both maintained roots. No checked maintained root has a `findDisplay 23000` caller. | Preserve or port the stable/release dialog IDD split before adding scripts that use display-ID lookup; use [UI IDD collision repair](UI-IDD-Collision-Repair) for the patch shape and smoke checklist. |
...
[
  ...
  {"fact":"gear_easa_branch_scope_2026_06_14","source":"docs 8b71e2a1; origin/master cf2a6d6a; miksuu/master b8389e74; perf/quick-wins 0076040f; release a96fdda2; feat/buymenu-easa-qol a66d4691","summary":"Gear profile/template issues remain open everywhere checked; cargo loops are fixed on stable/release, perf Chernarus-only; EASA exact-funds/client debit remains open everywhere checked."},
