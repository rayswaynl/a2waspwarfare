...
| Vehicle/backpack cargo application | Docs branch `b2544207`, older Miksuu checkpoint `b8389e74` and historical EASA QoL commit `a66d4691` still carry inclusive loops in both maintained roots. Current stable `origin/master@0139a346`, current Miksuu `master@d9506078` and historical release commit `a96fdda2` fix all five loops to `count(_items) - 1` in both maintained roots. Perf fixes Chernarus only; perf Vanilla remains old-shape. | [Vehicle cargo equip loop bounds](Vehicle-Cargo-Equip-Loop-Bounds) |
...
| Vehicle/backpack cargo equip loops overrun by one. | `Common_EquipVehicle.sqf:27,33,39` and `Common_EquipBackpack.sqf:35,41` use inclusive `for '_i' from 0 to count(_items)` bounds in old-shape targets; [Vehicle cargo equip loop bounds](Vehicle-Cargo-Equip-Loop-Bounds) owns the fix route and [Current Branch Scope](#current-branch-scope) preserves the branch split. | Port or preserve the current stable/current Miksuu/release `count(_items)-1` fix for any target branch still carrying inclusive loops, then propagate Vanilla and smoke cargo application. |
...
[
  ...
  {"fact":"gear_easa_branch_scope_2026_06_22","source":"docs branch b2544207 cargo paths; origin/master@0139a346; current Miksuu master@d9506078; older Miksuu checkpoint b8389e74; perf/quick-wins@0076040f; historical release a96fdda2; historical EASA QoL a66d4691","summary":"Gear profile/template save filtering is branch-split: docs/source and old refs keep undefined _u_upgrade, while current stable fixes it in both maintained roots. The six-field profile import guard and template creation/display policy remain open across checked refs; cargo loops are fixed on current stable, current Miksuu and historical release, perf is Chernarus-only and docs/EASA old-shape targets still need the loop-bound port. EASA exact-funds/client debit remains open everywhere checked."},
