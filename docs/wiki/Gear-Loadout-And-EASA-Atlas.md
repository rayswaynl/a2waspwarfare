...
Refreshed 2026-06-21 against current docs checkout `43c3ba05`, current stable `origin/master@0139a346`, Miksuu `b8389e74`, `origin/perf/quick-wins@0076040f`, historical release commit `a96fdda2` and historical EASA QoL commit `a66d4691`. Targeted diffs from the earlier gear/EASA checkpoint `8b71e2a1` to current docs `HEAD` over checked Chernarus, maintained Vanilla and `Tools/LoadoutManager` gear/EASA paths returned no source changes, so the docs/source line evidence still uses the earlier anchors. Cargo loop scope was corrected again on 2026-06-22 after direct upstream verification showed current Miksuu is `master@b8389e748243`, not local branch commit `d9506078`. Service/EASA paths were rechecked through `origin/docs/developer-wiki-index@9b3fc38e` and remain source-unchanged from `8b71e2a1`; current origin exposes no `release/*`, `feat/*service*`, `feat/*easa*` or `feat/buymenu-easa-qol` heads on 2026-06-22.

| Area | Branch result | Canonical route |
| --- | --- | --- |
| Profile/template save, import and creation gates | Branch-split after current-stable task 44: docs/source, Miksuu, perf, historical release and historical EASA QoL keep undefined `_u_upgrade` in `Client_UI_Gear_SaveTemplateProfile.sqf:33,52,75`; current stable `origin/master@0139a346` fixes that save-filter comparison in both maintained roots at `:34,57,82`. All checked refs still keep the six-field `Init_ProfileGear.sqf:17` guard before reading `:25`, the AddTemplate Barracks-or-Gear gate at `Client_UI_Gear_AddTemplate.sqf:136` and Gear-only FillTemplates display at `Client_UI_Gear_FillTemplates.sqf:17`. | [Gear template profile filter](Gear-Template-Profile-Filter) |
