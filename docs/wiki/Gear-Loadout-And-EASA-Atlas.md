...
Refreshed 2026-06-14 against current docs checkout `3fdf1898`, stable `origin/master` `cf2a6d6a`, Miksuu `b8389e74`, `origin/perf/quick-wins` `0076040f`, release `a96fdda2` and `origin/feat/buymenu-easa-qol` `a66d4691`. Targeted diffs from the earlier gear/EASA checkpoint `8b71e2a1` to `HEAD` over checked Chernarus, maintained Vanilla and `Tools/LoadoutManager` gear/EASA paths returned no source changes, so the branch results below preserve the earlier line evidence.
...
| Profile template upgrade filtering references undefined `_u_upgrade`. | `Client_UI_Gear_SaveTemplateProfile.sqf:33`, `:52`, `:75`; [Gear template profile filter](Gear-Template-Profile-Filter) owns the patch shape, while [Current Branch Scope](#current-branch-scope) preserves the checked branch coverage. | Fix before trusting saved templates to enforce upgrade gates. |
