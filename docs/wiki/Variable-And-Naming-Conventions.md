...
| `wfbe_*` (lowercase) | **Per-object state variable** set via `setVariable` (often broadcast with the `true` global flag → last-value JIP-compatible while the varspace/object exists) | attached to logic/side/town/building objects | `Client/Functions/Client_BuildUnit.sqf:172` (`_building setVariable ["queu",_queu,true]`); `wfbe_funds`, `wfbe_teams`, `wfbe_structures`, `wfbe_commander` |
...
- **`wfbe_*` with `setVariable [..., true]` is the replication mechanism** — the `true` flag broadcasts a supported value and can make the last value JIP-compatible while the varspace/object exists. It does not make client-written economy state authoritative, and it does not replay a feature's event history or collection state by itself. This is why the economy ledger is *replicated client state* rather than a server source of truth (the economy-authority class, DR-6/14/16/22/23/27/28/41/44).
- **BI command pages can mix OA primitives with newer notes** — `setVariable` is OA-safe, but some page details are Arma 3-era object/display/control notes. Do not copy those newer examples into OA patches unless the page marks them as OA-compatible.
