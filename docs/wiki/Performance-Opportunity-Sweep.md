...
| P1 | B74.6 branch-only client RPT/FPS regression fix route | `origin/claude/b745@712a7a668` is one commit past the older documented B74.5 head `b996bcb3`; the current-head-only delta is 9 files / +61 / -4 and clean under `git diff --check b996bcb3..origin/claude/b745`. Source Chernarus guards RHUD/common helper nil paths at `Client_UpdateRHUD.sqf:387-389`, `Common_GetSideStructures.sqf:8-11`, `Common_GetFactories.sqf:8-9` and `Common_GetClosestEntity.sqf:13`. Chernarus plus maintained Vanilla guard death-camera state at `Client_OnKilled.sqf:132,137-138`; touched full modded Napf/Eden/Lingor forks carry the same death-camera guard at `:60,65-66`. | The commit message names per-frame undefined-variable/RPT spam as the regression cause, but that is branch-author evidence until paired with a target mission package/hash and runtime logs. The RHUD/common helper guards are not propagated to maintained Vanilla in this delta; Vanilla only receives the death-camera guard. | Treat as a B74.6 branch smoke lane, not a general performance cleanup. Boot source Chernarus early-join/JIP before side structures replicate, exercise RHUD/action availability/factory scans and death/respawn camera paths, then compare client RPT volume/FPS against the old B74.5 head. Record whether maintained Vanilla and full modded forks are intentionally in or out of scope before release wording. |
