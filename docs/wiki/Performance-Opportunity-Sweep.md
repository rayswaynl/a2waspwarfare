...
| P1 | Hosted server FPS loop smoke/consolidation | Repo docs branch `HEAD@4a1bc13edde6` is source-unchanged from `d0161083` / `a27086cd` and guards both maintained-root publishers at `serverFpsGUI.sqf:1` and `monitorServerFPS.sqf:1`; current stable/B74.1 `origin/master@f8a76de34`, current B74.2 `21b62b04`, B69, B74, `feat/client-fps@709258e7`, `perf/asr-ai-server-fps@07609ef4` and historical `a96fdda2` guard the single `serverFpsGUI.sqf` publisher at `:4` and have no maintained-root `monitorServerFPS.sqf`; Miksuu `b8389e748243` and perf `0076040f` still keep the old branch-only sleep shape. No `release/*` or `origin/dev/july-update-hosted-server-fps-loop-fix` head was found on 2026-06-24. | The old busy-loop is no longer a current stable/B74-shaped defect, but Miksuu/perf old-shape refs still need deliberate propagation if they remain targets. Docs-source and stable/B74-shaped refs differ on two-publisher versus single-publisher compatibility, the planned July branch must be created/restored before code work, and Arma smoke remains pending. | Smoke dedicated RHUD FPS updates and hosted/listen no-spin behavior on the target branch first; decide separately whether to keep `WFBE_VAR_SERVER_FPS` compatibility publishing or consolidate to `SERVER_FPS_GUI`. See [Hosted server FPS loop sleep](Hosted-Server-FPS-Loop-Sleep). |
| P1 | B74.6/B74.9/B75 current-master client RPT/FPS, settings/FPS picker, JIP retry and AICOM cap route | Current master `origin/master@c4c9a6479` contains PR #85 from `origin/claude/b745@dd1401811`, PR #86 from `origin/feat/b749-enrollment-bodyfix@2327e5c6a` and direct branch `origin/VBIED_factory_kill_rewards@756d6918`. The broad B745 merge from previous stable `f8a76de34` to `34f652ae4` is 226 files / +7611 / -9082; PR #85 adds the B748 settings-menu delta, 9 files / +265 / -7; PR #86 adds B749 enrollment/settings propagation, 14 files / +308 / -21; B75 adds the maintained-root VBIED reward delta, 2 files / +88 / -2; checked ranges are clean under `git diff --check`. Current master carries RHUD/common helper nil guards, 30 FPS picker and AICOM hard cap/veteran cap in source Chernarus plus maintained Vanilla: RHUD `_baseStructures` guard, `Common_GetSideStructures.sqf:7-10`, `Common_GetFactories.sqf:7-10`, `Common_GetClosestEntity.sqf:13`, `Rsc/Dialogs.hpp:3528`, `FPSPicker_Open.sqf:48`, `Init_CommonConstants.sqf:159` and `AI_Commander_Teams.sqf:113-114,184`. PR #86 also carries B749 JIP body enrollment in both roots (`RequestJoin.sqf:95-96`, `Server_OnPlayerConnected.sqf:32,78-80`) and maintained-Vanilla settings (`GUI_Menu.sqf:230`, `Rsc/Dialogs.hpp:1258,1266,3436`, `Settings_Open.sqf:19,75-79`). B75 changes server GUER reward handling, not the checked UI/performance files. | These paths are now current-master static/source evidence, but B74.6 per-frame undefined-variable/RPT-spam claims, B749 JIP claims, B748/B749 settings live-use claims and B75 reward claims still need target mission package/hash and runtime logs. The merge improves maintained Vanilla parity for several UI/performance/AICOM routes, but it does not make the package release-ready or full-modded-fork complete. | Treat as target-package smoke, not a general performance cleanup. Boot source Chernarus and maintained Vanilla early-join/JIP before side structures replicate; exercise RHUD/action availability/factory scans, death/respawn camera paths, FPS picker 30/45/50/60 profile persistence, B748/B749 settings toggles including no A2 `allMapMarkers` RPT error, B749 body-store enrollment retry, plus low-pop/banked-funds and veteran-slot AICOM founding. Smoke B75 VBIED reward attribution separately from client performance. Compare client/server RPT volume, client FPS and AICOM team count against older B74.5/B74.6/B74.7 heads. Record full-modded fork scope separately. |
...
| P1 | Supply mission command-center scan narrowing | Docs/source `15563691` `supplyMissionStarted.sqf:25,28,44`; current stable `origin/master@0139a346` `:7,55,61,83`; current Miksuu `b8389e74` and perf `0076040f` `:25,28,44` | Current stable already uses the typed `Base_WarfareBUAVterminal` scan with heli/truck radius in both maintained roots, while current Miksuu and perf still scan all object classes inside 80 m and post-filter in SQF. Historical release commits `a96fdda2` / `7ff18c49` also carry the heli-aware typed shape, but current origin exposes no live `release/*` head. Arma smoke remains pending. | Do not reopen the scan on current stable. For old-shape targets, port/adopt the typed scan deliberately; for current stable, smoke truck/heli delivery at command centers and no-completion near unrelated objects. See [Supply mission scan narrowing](Supply-Mission-Scan-Narrowing). |
...
| P2 | WASP marker dialog wait propagation/smoke | Docs/source `HEAD@c9df2a8b7264` is unchanged from `46840f048bd4` for checked WASP paths and still has the unslept display-54 wait at `WASP/global_marking_monitor.sqf:57,62,64,68-69,80`; current stable/B74.1 `origin/master@f8a76de34`, B74.2 `origin/claude/b74.2-aicom@21b62b04`, B69 `origin/claude/b69@8d465fce`, B74 `origin/claude/b74-aicom-spend@b23f557f`, `origin/feat/client-fps@709258e7` and live map branches already add `sleep 0.1` at `:64` in both maintained roots; Miksuu `b8389e748243`, perf `0076040f8a5e` and historical `a96fdda28087` remain old-shape. | The hot-loop patch is no longer a current-stable/B74.1/B74.2/B69/B74/client-fps/live-map code task, but docs/source, current Miksuu, perf and historical old-shape targets still need the one-line throttle or a deliberate `waitUntil` refactor if they remain targets. All targets still need marker-dialog smoke before verified-runtime wording. | For old-shape targets, add `sleep 0.1` before `findDisplay 54` or refactor to throttled `waitUntil`, then smoke map double-click marker naming, Enter prefixing, Escape cleanup and timeout/no-dialog input re-enable. See [WASP marker wait cleanup](WASP-Marker-Wait-Cleanup). |
...
| P3 | Cleaner/restorer wide scans | `Server/FSM/cleaners/crater_cleaner.sqf:14-49`, `Server/FSM/cleaners/droppeditems_cleaner.sqf:62`, `Server/FSM/cleaners/ruins_cleaner.sqf:9-28`, `Server/FSM/restorers/buildings_restorer.sqf:10-26`; cadence/cost guide in [Marker cleanup/restoration atlas](Marker-Cleanup-Restoration-Systems-Atlas#cadence-and-cost-interpretation) | These use very wide class-filtered `nearestObjects` scans, but run on long timers, sleep between per-item work, and already emit PerformanceAudit records. `droppeditems_cleaner.sqf` now runs a single ~20 km `weaponholder` scan at `:62` (the B40 header at `:5-14` documents the two redundant `Mine`/`MineE` scans being removed; `_perfMines` stays `0`), so it is largely audit-first rather than a scan-narrowing target. Garbage/empty-vehicle collectors are the opposite shape: 0.5-second registry drains where idempotent queue/flag behavior matters more than scan narrowing. | Do not patch first. Use RPT audit rows to prove actual cost, then consider smaller terrain-aware centers/radii, combined scan strategy or tracked-object lists. Fix mine pair removal and garbage flag mismatch as correctness items before speculative cadence changes. |
...
Repo docs branch `HEAD@4a1bc13edde6` is source-unchanged from `d0161083` / `a27086cd` for the checked FPS paths and already has the low-risk DR-19 shape in both maintained roots: both publishers exit immediately on `!isDedicated`, then keep the dedicated 8-second cadence. Current stable/B74.1 `origin/master@f8a76de34`, current B74.2 `21b62b04`, B69, B74, client-fps, ASR FPS and historical `a96fdda2` use a different fixed shape: one guarded `serverFpsGUI.sqf` publisher and no maintained-root `monitorServerFPS.sqf`; B74.2 only line-drifts source-Chernarus init/removal anchors for this lane. The remaining current work is smoke plus the separate decision of whether to consolidate two closely related FPS variables; do not tell stable/B74-shaped refs to port the old two-publisher guard blindly. No `release/*` or `origin/dev/july-update-hosted-server-fps-loop-fix` head was found on 2026-06-24, so create or restore that branch before code work.
...
Docs/source `supplyMissionStarted.sqf:25,28` now uses a truck-only typed `Base_WarfareBUAVterminal` scan in both maintained roots. Current stable `origin/master@0139a346` uses the heli-aware typed scan at `:55,61`, with 400 m heli / 80 m truck radius selection. Current Miksuu `b8389e74` and `origin/perf/quick-wins@0076040f` still scan all object classes within 80 meters and post-filter `Base_WarfareBUAVterminal` at `:25,28`.

Docs/source `15563691` carries the truck-only typed 80 m scan; current stable carries the heli-aware typed scan; historical release-line anchors `a96fdda2` and `7ff18c49` carry the PR #1-compatible truck/heli shape, but current origin exposes no live `release/*` head in the 2026-06-22 remote scan. Current Miksuu/perf still need the old-shape port if they remain targets. The code already wanted that class family, and the 8-meter nearby-player scan intentionally remains broad. Arma smoke remains pending.
...
- Current stable: typed truck/heli command-center scan is source-present in both maintained roots.
- Docs/source: typed truck-only command-center scan is source-present in both maintained roots.
- Current Miksuu/perf: still broad-enumerate the 80-meter command-center scan, then post-filter `Base_WarfareBUAVterminal`.
...
Docs/source `HEAD@c9df2a8b7264` is unchanged from `46840f048bd4` for checked WASP paths, and docs/source, current Miksuu `b8389e748243`, `origin/perf/quick-wins@0076040f8a5e` and historical `a96fdda28087` still need a throttled display-54 wait while preserving input re-enable after display-open or timeout (`global_marking_monitor.sqf:57,62,64,68-69` with display-12 already throttled at `:80`). Current stable/B74.1 `origin/master@f8a76de34`, B74.2 `origin/claude/b74.2-aicom@21b62b04`, B69 `origin/claude/b69@8d465fce`, B74 `origin/claude/b74-aicom-spend@b23f557f`, `origin/feat/client-fps@709258e7` and live map branches already add `sleep 0.1` at `global_marking_monitor.sqf:64` in both maintained roots; the launch lines are stable/B74.1/B69/B74/live-map `Init_Client.sqf:397`, B74.2 source Chernarus `:403` and maintained Vanilla `:397`, client-fps Chernarus `:308` and client-fps Vanilla `:287`. B74.2's checked `Init_Client.sqf` delta is unrelated vehicle-tint/join-ACK/intro-music drift, not a WASP monitor behavior change.

This remains a small local UI cleanup, not a server-wide performance fix. See [WASP marker wait cleanup](WASP-Marker-Wait-Cleanup) for the target-specific patch shape and smoke plan.
...
3. Branch-scoped by target: docs/source and current stable carry typed supply mission scan narrowing, current Miksuu/perf remain broad/post-filter, no live release head is exposed by origin, and Arma smoke remains.
...
6. WASP marker dialog wait cleanup is target-specific: docs/source, current Miksuu, perf and historical old-shape refs still need the throttle/refactor; current stable/B74.1/B74.2/B69/B74/client-fps/live-map refs need marker-dialog smoke before verified wording.
...
- Completed branch-local cleanup packages: [Client skill init idempotency](Client-Skill-Init-Idempotency) and [Hosted server FPS loop sleep](Hosted-Server-FPS-Loop-Sleep); [Supply mission scan narrowing](Supply-Mission-Scan-Narrowing) is source-present on docs/source and current stable, old-shape on current Miksuu/perf, and still smoke pending.
