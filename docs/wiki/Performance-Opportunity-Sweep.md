...
| P1 | Old-FPS opportunity pack | Returned scout thread `old-vs-current-fps-opportunity-scout`; report `C:\Users\Steff\Documents\Codex\2026-06-05\wasp-old-vs-current-fps-investigation\outputs\Old-BE-vs-Current-Wasp-FPS-Opportunity-Audit.md`; owner pages [Old WarfareBE performance comparison](Old-WarfareBE-Performance-Comparison), [Player AI caps](Player-AI-Caps-And-Role-Balance), [Testing workflow](Testing-Debugging-And-Release-Workflow#full-server-fps-opportunity-pack) | The evidence-backed order is now: player-owned AI cap pilot, AI-source scoreboard, HC proof pack, low-SV town group branch, static-defense lifecycle audit, matched view-distance runs, then duplicate FPS publisher / cleanup / PV refinements. Old BE is a conservative baseline, not proof that old FSM town loops are cheaper. | Treat as a test/config/docs bundle first. Public pilot can cap normal roles near `10`, but final design should be a server-published role-aware dynamic budget with buy menu and RHUD parity. Only patch source after the 10/15/20/25/30 test matrix chooses a lane. |
