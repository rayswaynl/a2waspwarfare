...
| P1 | Hosted server FPS loop sleep/consolidation | Server/GUI/serverFpsGUI.sqf:1-9, Server/Module/serverFPS/monitorServerFPS.sqf:1-7, Server/Init/Init_Server.sqf:577-595 | Source/Vanilla patched: both publishers now exit immediately when !isDedicated, preserving dedicated 8-second publishing without hosted/listen busy-spin. | Smoke dedicated RHUD FPS updates and hosted/listen no-spin behavior. See [Hosted server FPS loop sleep](Hosted-Server-FPS-Loop-Sleep). |
| P1 | Supply mission command-center scan narrowing | `Server/Module/supplyMission/supplyMissionStarted.sqf:37-45` | Source/Vanilla patched: the 80-meter command-center scan now filters `nearestObjects` to `["Base_WarfareBUAVterminal"]` while preserving the 3-second cadence and the separate 8-meter nearby-player scan. | Smoke truck/heli delivery at command centers and no-completion near unrelated objects. See [Supply mission scan narrowing](Supply-Mission-Scan-Narrowing). |
| P2 | Duplicate `Skill_Init` and non-idempotent Soldier AI cap | `Client/Init/Init_Client.sqf:547`, `Client/Init/Init_Client.sqf:571`, `Client/Module/Skill/Skill_Init.sqf:39-49` | Patched in source Chernarus and propagated to Vanilla Takistan: client init now runs `Skill_Init.sqf` once before class default gear selection, then calls `WFBE_SK_FNC_Apply` without rerunning init. | Smoke Soldier/non-Soldier caps and respawn skill reapply. See [Client skill init idempotency](Client-Skill-Init-Idempotency). |
...
This was patched in source Chernarus and propagated to Vanilla Takistan by changing the 80-meter command-center scan to `["Base_WarfareBUAVterminal"]`. The code already wanted that class family, and the 8-meter nearby-player scan intentionally remains broad.

Validation:

- Source-only: done. Source Chernarus and Vanilla Takistan now have one narrowed `["Base_WarfareBUAVterminal"]` 80-meter scan and still have the broad 8-meter nearby-player scan.
...
This was patched in source Chernarus and Vanilla Takistan by removing the second client init call while preserving the later `WFBE_SK_FNC_Apply` call. It remains smoke pending for Soldier/non-Soldier caps and respawn reapply.

Validation:

- Source-only: done. Chernarus and Vanilla Takistan now each run `Skill_Init.sqf` once and still call `WFBE_SK_FNC_Apply`.
...
3. Completed: supply mission scan narrowing is source/Vanilla patched; smoke remains.
