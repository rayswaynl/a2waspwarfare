...
| P1 | B74.6/B74.7.2 branch-only client RPT/FPS, 30 FPS picker, JIP retry and AICOM cap route | `origin/claude/b745@d52f65466` is five commits past the older documented B74.5 head `b996bcb3`. The B74.6 delta `b996bcb3..712a7a668` is 9 files / +61 / -4 and clean; source Chernarus guards RHUD/common helper nil paths at `Client_UpdateRHUD.sqf:387-389`, `Common_GetSideStructures.sqf:8-11`, `Common_GetFactories.sqf:8-9` and `Common_GetClosestEntity.sqf:13`, while Chernarus plus maintained Vanilla/full modded forks guard death-camera state. The B74.7/B74.7.2 delta `712a7a668..d52f65466` is 5 source-Chernarus files / +49 / -14 and clean; B74.7 adds `CA_FPS30` at `Rsc/Dialogs.hpp:3517-3533` and persists action 5 as target FPS 30 at `FPSPicker_Open.sqf:8,46-52`, B74.7.2 tightens `Server_OnPlayerConnected.sqf:31,71-75` to avoid transient non-slot group adoption and nil-side silent strand, while B74.7.1 adds `WFBE_C_AICOM_TEAMS_HARD_CAP = 8` at `Init_CommonConstants.sqf:159`, clamps founding at `AI_Commander_Teams.sqf:113-114`, and caps the veteran-slot `+1` bump at `AI_Commander_Teams.sqf:184`. | B74.6 per-frame undefined-variable/RPT-spam claims and B74.7/B74.7.2 boot-smoke claims are branch-author evidence until paired with target mission package/hash and runtime logs. The RHUD/common helper guards, 30 FPS picker option, enrollment retry and AICOM hard cap are not propagated to maintained Vanilla in these deltas; Vanilla only receives the death-camera guard from B74.6. | Treat as branch smoke, not a general performance cleanup. Boot source Chernarus early-join/JIP before side structures replicate, exercise RHUD/action availability/factory scans, death/respawn camera paths, FPS picker 30/45/50/60 profile persistence, stamped-slot/nil-side enrollment retry and low-pop/banked-funds plus veteran-slot-armed AICOM founding, then compare client/server RPT volume, client FPS and AICOM team count against the older B74.5/B74.6/B74.7 heads. Record whether maintained Vanilla and full modded forks are intentionally in or out of scope before release wording. |
