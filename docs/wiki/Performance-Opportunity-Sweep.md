...
Docs/source `HEAD@46840f048bd4`, current Miksuu `b8389e748243` and `origin/perf/quick-wins@0076040f8a5e` still need a throttled display-54 wait while preserving input re-enable after display-open or timeout (`global_marking_monitor.sqf:57,62,64,68-69` with display-12 already throttled at `:80`). Current stable `origin/master@0139a3468609` and B69 `origin/claude/b69@8d465fcede7f` already add `sleep 0.1` at `global_marking_monitor.sqf:64` in both maintained roots; the stable launch line is `Init_Client.sqf:309` and B69 drifts to `:397`.
...
6. WASP marker dialog wait cleanup is target-specific: docs/source, current Miksuu and perf still need the throttle/refactor; stable/B69 need marker-dialog smoke before verified wording.
