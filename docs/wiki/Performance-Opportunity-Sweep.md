...
| P2 | WASP marker dialog wait propagation/smoke | Docs/source `HEAD@46840f048bd4` still has the unslept display-54 wait at `WASP/global_marking_monitor.sqf:57,62,64,68-69,80`; current stable `origin/master@0139a3468609`, B69 `origin/claude/b69@8d465fcede7f`, B74 `origin/claude/b74-aicom-spend@b23f557fc912` and `origin/feat/client-fps@709258e7e6f8` already add `sleep 0.1` at `:64` in both maintained roots; Miksuu `b8389e748243` and perf `0076040f8a5e` remain old-shape. | The hot-loop patch is no longer a current-stable/B69/B74/client-fps code task, but docs/source, current Miksuu and perf still need the one-line throttle or a deliberate `waitUntil` refactor if they remain targets. All targets still need marker-dialog smoke before verified-runtime wording. | For old-shape targets, add `sleep 0.1` before `findDisplay 54` or refactor to throttled `waitUntil`, then smoke map double-click marker naming, Enter prefixing, Escape cleanup and timeout/no-dialog input re-enable. See [WASP marker wait cleanup](WASP-Marker-Wait-Cleanup). |
...
Docs/source `HEAD@46840f048bd4`, current Miksuu `b8389e748243` and `origin/perf/quick-wins@0076040f8a5e` still need a throttled display-54 wait while preserving input re-enable after display-open or timeout (`global_marking_monitor.sqf:57,62,64,68-69` with display-12 already throttled at `:80`). Current stable `origin/master@0139a3468609`, B69 `origin/claude/b69@8d465fcede7f`, B74 `origin/claude/b74-aicom-spend@b23f557fc912` and `origin/feat/client-fps@709258e7e6f8` already add `sleep 0.1` at `global_marking_monitor.sqf:64` in both maintained roots; the launch lines are stable `Init_Client.sqf:309`, B69/B74 `:397`, client-fps Chernarus `:308` and client-fps Vanilla `:287`.
...
6. WASP marker dialog wait cleanup is target-specific: docs/source, current Miksuu and perf still need the throttle/refactor; stable/B69/B74/client-fps need marker-dialog smoke before verified wording.
