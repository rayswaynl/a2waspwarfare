...
| P1 | Hosted server FPS loop smoke/consolidation | Docs HEAD `d0161083` is source-unchanged from `a27086cd` and guards both maintained-root publishers at `serverFpsGUI.sqf:1` and `monitorServerFPS.sqf:1`; current stable `origin/master@0139a346` guards the single `serverFpsGUI.sqf` publisher at `:4`, starts it from `Init_Server.sqf:769` and has no maintained-root `monitorServerFPS.sqf`; Miksuu `b8389e74` and perf `0076040f` still keep the old branch-only sleep shape. No `origin/dev/july-update-hosted-server-fps-loop-fix` head was found on 2026-06-22. | The old busy-loop is no longer a current-stable `origin/master` defect, but Miksuu/perf old-shape refs still need deliberate propagation if they remain targets. Current source and stable differ on two-publisher versus single-publisher compatibility, the planned July branch must be created/restored before code work, and Arma smoke remains pending. | Smoke dedicated RHUD FPS updates and hosted/listen no-spin behavior on the target branch first; decide separately whether to keep `WFBE_VAR_SERVER_FPS` compatibility publishing or consolidate to `SERVER_FPS_GUI`. See [Hosted server FPS loop sleep](Hosted-Server-FPS-Loop-Sleep). |
...
Docs HEAD `d0161083` is source-unchanged from `a27086cd` for the checked FPS paths and already has the low-risk DR-19 shape in both maintained roots: both publishers exit immediately on `!isDedicated`, then keep the dedicated 8-second cadence. Current stable `origin/master@0139a346` and historical `a96fdda2` use a different fixed shape: one guarded `serverFpsGUI.sqf` publisher and no maintained-root `monitorServerFPS.sqf`; current stable starts that publisher from `Init_Server.sqf:769` and documents monitor removal at `:815-817`. The remaining current work is smoke plus the separate decision of whether to consolidate two closely related FPS variables; do not tell current stable to port the old two-publisher guard blindly. No `origin/dev/july-update-hosted-server-fps-loop-fix` head was found on 2026-06-22, so create or restore that branch before code work.
