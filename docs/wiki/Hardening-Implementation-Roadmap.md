...
| --- | --- | --- | --- |
| Hosted Server FPS Loop Fix | planned `dev/july-update-hosted-server-fps-loop-fix`; no current remote head found on 2026-06-24 | [Hosted server FPS loop sleep](Hosted-Server-FPS-Loop-Sleep), [Performance opportunity sweep](Performance-Opportunity-Sweep), [Testing workflow](Testing-Debugging-And-Release-Workflow#july-update-candidate-smoke-route) | Create or restore the target branch first, then adopt or verify the target shape deliberately. Current stable/B74.1/B74.2/B69/B74 and live FPS-named branches already use one guarded `SERVER_FPS_GUI` publisher; docs/source uses two guarded compatibility publishers; Miksuu/perf still need old-loop cleanup if targeted. Keep dedicated FPS publishing intact, prove hosted/listen no-spin behavior, and name Chernarus plus maintained Vanilla parity before release wording. |
| Takistan Airfield FPV Drone Bay | `origin/dev/july-2026-update@e3f530ed` planning route; no current `origin/dev/july-takistan-airfield-fpv-drone` head found on 2026-06-22 | [Takistan airfield FPV drone design](Takistan-Airfield-FPV-Drone-Design), [PR8 and Drone upstream lesson match](PR8-And-Drone-Upstream-Lesson-Match), [Testing workflow](Testing-Debugging-And-Release-Workflow#july-update-candidate-smoke-route) | Current evidence is closed draft PR #21 and `docs/july-2026-update.md` roadmap scaffold only. Create or restore an implementation branch first, then build Takistan-only and server-authoritative: server rechecks ownership, funds, UAV/EASA gates, one-drone-per-player, side cap, range/boundary, bunker state and base-protection before spawning or debiting. |
...
Branch/root status: the 2026-06-23 matrix in [Public variable channel index](Public-Variable-Channel-Index#send_message-direct-compile-branch-matrix) confirms docs/source `HEAD@3a32be92`, current stable `origin/master@0139a346`, current B69 `origin/claude/b69@8d465fce`, adjacent B74 `origin/claude/b74-aicom-spend@b23f557f`, Miksuu `b8389e748243`, `perf/quick-wins@0076040f` and historical `a96fdda2` release evidence all still keep the direct receiver/helper compile route in both maintained roots. Current stable/B69/B74 PVF dispatcher lookup changes do not close this direct channel.
...
| --- | --- | --- |
| [Commander vote AI/no-commander semantics](Commander-Vote-And-Reassignment-Playbook) | Current stable/B69/B74 `Server_VoteForCommander.sqf:18,26-27,43` count AI/no-commander votes and require `_highest >= _aiVotes`; docs/source/Miksuu/perf/historical refs still use the old `>= || <=` condition. `GUI_VoteMenu.sqf:87-89` can still preview AI/no commander for a distribution the current server assigns to a player. | Owner decides plurality/majority/tie/no-commander rules, then smoke player-majority, no-commander-majority, equal-vote, player-candidate tie and `RequestCommanderVote` restart cases on the exact target branch. |
