...
| Generated `version.sqf` target-root gap | DR-43a corrected 2026-06-04: `description.ext` and `initJIPCompatible.sqf` include `version.sqf`; current source Chernarus tracks it, but generated/terrain roots still need verification. | Keep explicit pre-pack/pre-test checks for generated Vanilla and any supported modded target; do not treat Chernarus presence as proof every mission root is packable. |
