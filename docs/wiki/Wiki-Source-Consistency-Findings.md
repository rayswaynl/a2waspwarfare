...
| Feature-Status; PV channel index | `HandleParatrooperMarkerCreation` is now a registered client PVF | **Superseded.** Current source Chernarus and maintained Vanilla Takistan include it in `_clientCommandPV`, compile the handler and register the PVEH. Keep this row only as historical evidence for DR-2; remaining gaps are Arma smoke and modded handler drift. | `Common/Init/Init_PublicVariables.sqf:25-46` | DR-2 |
