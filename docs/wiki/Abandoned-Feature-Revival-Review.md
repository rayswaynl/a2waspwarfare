...
- Source-only: init compiles the handler and registers the PVEH in source Chernarus and Vanilla Takistan.
...
- Best contradiction checks: confirm no hidden live sender for `WFBE_CL_MASH_MARKER_CREATED`; confirm no other source/Vanilla client-bound PVF sender is missing from `_clientCommandPV`; verify whether `RscMenu_Upgrade` is truly uncalled outside static text search.
