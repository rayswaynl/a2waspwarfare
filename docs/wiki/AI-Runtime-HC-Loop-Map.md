...
| Ambient patrol workers | `SRC/Server/FSM/server_patrols.sqf:26`, `:68`, `:71-72`; launched from `SRC/Server/FSM/server_town_ai.sqf:226-230` | Server | `sleep 30`; current source uses `!WFBE_GameOver || _team_alive` | Patrol can remain active-latched after team death because the reset happens only after the loop exits. Branch matrix lives in [Towns/camps/capture](Towns-Camps-And-Capture-Atlas#resistance-patrol-branch-matrix): `perf/quick-wins` and release Chernarus use `&&`, but current source/Vanilla and release Vanilla still need propagation. |
...
| Medium | Patrol exit condition is lifecycle-hostile. | `server_town_patrol.sqf:18`; `server_patrols.sqf:26`; `server_town_ai.sqf:226-230`; `server_patrols.sqf:71-72`; [branch matrix](Towns-Camps-And-Capture-Atlas#resistance-patrol-branch-matrix). | For ambient patrols, port or recreate the Chernarus `&&` fix from `perf/quick-wins` / release, propagate maintained Vanilla, and smoke `wfbe_patrol_active` clears after patrol death so resistance patrols can relaunch. Review `server_town_patrol.sqf` separately or in the same AI-loop pass. |
...
| Medium | AI town attack-path helper is compiled but dormant-looking in stable master, and its path-building branch clears existing waypoints before random/safety exits. | `Init_Server.sqf:45-47`; `Server_AI_SetTownAttackPath.sqf:18,41,80-109`; no static caller found for `WFBE_SE_FNC_AI_SetTownAttackPath` outside compile/docs in this scan. | If AI commander/order revival calls this helper, add a smoke where a distant team receives a town attack order and still has final depot/camp waypoints after random/safety branches. |
