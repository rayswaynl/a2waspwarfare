...
| RequestSpecial upgrade-sync tuple cleanup | Patch-ready consistency cleanup; current source/Vanilla, stable, Miksuu upstream, `perf/quick-wins` and release all keep the mixed `_args` / `_this` parser, while `_args = _this` means current behavior is equivalent. | [Support specials atlas](Support-Specials-And-Tactical-Modules-Atlas#upgrade-sync-branch-matrix) | Normalize `Server_HandleSpecial.sqf:67-73` to one tuple source, keep RequestUpgrade authority migration separate, propagate maintained Vanilla and smoke upgrade completion/non-server timer sync plus malformed payload rejection. |
