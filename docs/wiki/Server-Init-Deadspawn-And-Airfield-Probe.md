...
2. **Walk the circle.** For each segment `_i` from 0 to `_count-1` (`:84`): bearing `_ang = _i * _step` (`:85`), point on the ring at `_px = (_c select 0) + _radius * sin _ang`, `_py = (_c select 1) + _radius * cos _ang` (`:88-89`).
