...
| Supply mission command-center scan | `supplyMissionStarted.sqf:37-45` now filters the 80-meter command-center scan to `Base_WarfareBUAVterminal`; the 8-meter player/object scan remains broad by design. | Maintained Chernarus/Vanilla propagated; smoke pending. See [Supply mission scan narrowing](Supply-Mission-Scan-Narrowing). |
...
| Hosted server FPS loops | serverFpsGUI.sqf and monitorServerFPS.sqf now exit before their loops when !isDedicated; Init_Server.sqf:578,595 still starts them and dedicated publishing remains every 8 seconds. | Maintained Chernarus/Vanilla propagated; smoke pending. See [Hosted server FPS loop sleep](Hosted-Server-FPS-Loop-Sleep). |
