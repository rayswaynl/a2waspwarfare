...
| Gear/EASA purchase authority | `GUI_BuyGearMenu.sqf` applies gear and deducts funds client-side; `GUI_Menu_EASA.sqf` calls `EASA_Equip` and `ChangePlayerFunds` client-side. | Client-authoritative legacy path. Any public-server hardening needs a server PVF redesign or BattlEye script filtering. |
