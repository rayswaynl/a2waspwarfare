...
| Victory/endgame double-fire | Claude DR-36 traced `server_victory_threeway.sqf:23`: `!WFBE_GameOver` guards only the all-towns branch, and the side `forEach` does not break once a winner is set. | Correctness bug. Same-tick eliminations can double-fire endgame and overwrite/log the opposite winner. Parenthesize/guard both clauses and exit the loop after first winner. |
