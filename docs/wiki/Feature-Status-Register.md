...
| AI commander automation | On stable `origin/master`, AI commander constants and side-logic state/funds exist; `WFBE_SE_FNC_AI_Com_Upgrade` is compiled and `Server_AI_Com_Upgrade.sqf` performs real upgrade/funds logic, but no live scheduler/loop starts full autonomous commander behavior. Branch `origin/feat/ai-commander` head `4dba060e` adds a Chernarus-only supervisor, assign-types/assign-towns/produce workers and an explicit order executor. | **Branch-review risk.** Stable master remains partial/latent. `feat/ai-commander` is the current revival attempt, but it is source-Chernarus-only, changes AI commander default to on, still only guards old `UpdateSupplyTruck`, and needs dedicated/JIP/Vanilla smoke before being called revived. Use [AI commander autonomy audit](AI-Commander-Autonomy-Audit). |
