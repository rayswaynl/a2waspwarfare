...
2. Pick one subsystem and inspect source before editing docs.
3. Record what was checked, including files and line-level evidence where useful.
4. Update targeted pages only; avoid broad rewrites unless the existing model is wrong.
5. Update `agent-context.json` only when high-level facts, risks or page names change.
6. Flag disagreements clearly so Codex or a human can resolve them.
