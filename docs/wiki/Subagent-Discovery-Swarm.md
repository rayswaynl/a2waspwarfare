...
| Lorentz | `commander-hq-lifecycle-edge-cases` | Returned / selected harvest local | Reconfirmed commander reassignment call-shape/trust issues, HQ death killer assumptions, MHQ repair client authority/split counters and commander-disconnect no-auto-reassign behavior. Codex source-checked the existing canonical coverage in [Commander/HQ lifecycle](Commander-HQ-Lifecycle-Atlas), [Commander reassignment](Commander-Reassignment-Call-Shape), [Feature status](Feature-Status-Register) and DR-15/DR-20/DR-6 in [Deep-review findings](Deep-Review-Findings), then added the missing commander-disconnect fallback owner-decision row. |
| Hubble | `factories-purchase-queue-integrity` | Returned / selected harvest local | Confirmed normal player purchase path is client UI -> local `Client_BuildUnit.sqf`, not a server factory queue; reaffirmed counter/token cleanup and no cancel/refund semantics. Codex source-checked this as already canonical in [Factory/purchase atlas](Factory-And-Purchase-Systems-Atlas), [Factory queue cleanup](Factory-Queue-Counter-Token-Cleanup), [Feature status](Feature-Status-Register) and DR-14/DR-33 in [Deep-review findings](Deep-Review-Findings). |
| Banach | `town-ai-camp-capture-delegation` | Returned / selected harvest local | Reconfirmed town/camp capture runtime, town AI despawn safety, client-stamped supply amount/source, client-gated camp repair and stale AI supply-truck/supplyMissionActive paths. Codex source-checked this as already canonical: camp repair lives in [Towns/camps/capture atlas](Towns-Camps-And-Capture-Atlas), patrol loop caveats in [Server runtime](Server-Gameplay-Runtime-Atlas) / [Feature status](Feature-Status-Register), and supply/logistics drift in [Supply mission authority cleanup](Supply-Mission-Authority-Cleanup-Playbook) plus [Abandoned feature revival](Abandoned-Feature-Revival-Review). |
...
Harvest rule: Wave Q reports are leads until Codex source-checks and promotes them into owner pages. Curie's RHUD/endgame display-var collision, Linnaeus's side-supply negative-delta/direct-PV helper lesson, Lorentz's commander/HQ lifecycle edge cases, Banach's town/camp/supply routing check and Hubble's player-buy path correction are now promoted; no Wave Q lead should be treated as pending without a fresh source contradiction.
