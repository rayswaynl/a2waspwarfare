...
| ID | Classification | Evidence | Action |
| --- | --- | --- | --- |
| `sqf-ai-supply-truck-worker-comment-only` | Comment-only SQF worker; current stable/B74 safe-disables caller | Branch-split after the 2026-06-23 current-B74.1 refresh: docs/source `HEAD@6d4b514c12fc` still comments the `UpdateSupplyTruck` compile at `Server/Init/Init_Server.sqf:36` but raw-spawns the worker at `:382-383` in Chernarus and maintained Vanilla; current stable/B74.1 `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34` comments the compile at `:44`, initializes `wfbe_ai_supplytrucks`, and warning-disables legacy logistics at Chernarus `:624-625` and maintained Vanilla `:618-619`. B74.2 `origin/claude/b74.2-aicom@d472da6a`, B69 `origin/claude/b69@8d465fce` and adjacent B74 `origin/claude/b74-aicom-spend@b23f557f` keep safe-disable caller shapes. `AI_UpdateSupplyTruck.sqf:17` still points at missing `Server\FSM\supplytruck.fsm`; `git ls-tree` found no `supplytruck.fsm` in HEAD, current stable/B74.1, B74.2, B69, B74, Miksuu `b8389e748243`, perf, historical `a96fdda2` or historical `c20ce153`. Miksuu and `origin/perf/quick-wins@0076040f` still raw-spawn the worker, historical `a96fdda2` safe-disables it, and historical `c20ce153` guards only Chernarus while maintained Vanilla remains raw. Current origin exposes no `release/*` or `feat/ai-commander` heads on 2026-06-23. | Keep documented as dormant/broken logistics code. Do not simply uncomment; preserve/port the current-stable/B74 safe-disable or redesign AI logistics with a real server-owned loop/FSM. |
