...
| `icbm-launched-pveh-receiver-only` | Receiver-only legacy event handler | 2026-06-21 refresh: docs head `11f535d9`, stable `origin/master` `0139a346`, Miksuu `b8389e74` and `origin/perf/quick-wins` `0076040f` all keep `"ICBM_launched" addPublicVariableEventHandler` at `Client/FSM/updateclient.sqf:20` in source Chernarus and maintained Vanilla, plus the receiver comments in `Client/Module/Nuke/OnEventHandler_ICBM_Launch.sqf:5-9`. No active assignment or `publicVariable "ICBM_launched"` sender was found in those checked maintained roots. Current tactical nuke flow spawns `NukeIncoming`, sends `RequestSpecial` `"ICBM"` to the server, and broadcasts `HandleSpecial` `"icbm-display"` to clients. The old `origin/release/2026-06-feature-bundle` ref is not present after the 2026-06-21 fetch/prune, so older release mentions are historical until a release ref is restored. | Branch verification is refreshed for the live refs. Do not delete ICBM. Either retire the stale `ICBM_launched` handler/receiver docs or revive it deliberately with one sender and one documented notification path. |
...
| `ICBM_launched` PVEH | Could be revived as a clear client notification channel for tactical nukes, but it is receiver-only in the 2026-06-21 docs-head, stable, upstream and perf maintained-root checks; the old release ref was absent after fetch/prune and remains historical evidence only. | Must be reconciled with the current `NukeIncoming` / `RequestSpecial` / `HandleSpecial "icbm-display"` path and smoke-tested for friendly/enemy warnings, markers and damage. If retired, remove the handler and stale receiver comments together. |
