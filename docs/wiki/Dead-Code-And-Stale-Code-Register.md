...
| `reaktiv-init-unreachable` | Orphaned module script | `Common/Module/Reaktiv/Reaktiv_Init.sqf:5` compiles the HandleDamage helper, but current stable `origin/master` / local `master` / Miksuu `89ae9dad` `Init_Common.sqf:321-325` initializes ICBM, IRS and CIPHER without any Reaktiv init call. Branch check refreshed 2026-06-07: current source/Vanilla, current stable/Miksuu `89ae9dad` and perf `0076040f` keep the unreachable module in both maintained roots; historical `2cdf5fb8` carried the same maintained-root files before stable advanced; release `7ff18c49` removes it from maintained roots while current modded Napf/Eden/Lingor copies remain stale. | Already routed to module docs as dead/unreachable. Preserve only if an owner wants ERA armor revived and smoke-tested; otherwise release provides a maintained-root removal precedent, but modded cleanup still needs policy. |
...
| `Server/PVFunctions/LogGameEnd.sqf` | Old duplicate; never compiled. The live path is `Server/Functions/Server_LogGameEnd.sqf` (`WFBE_CO_FNC_LogGameEnd`, `Init_Server.sqf:68,93`, called from `server_victory_threeway.sqf:41`). The PVFunctions copy also has a broken `profileNamespace` ref. |
...
| Reaktiv ERA armor | The module is self-contained enough to inspect and could add vehicle survivability tuning, but it is unreachable in current/stable/Miksuu/perf maintained roots and absent from release `7ff18c49` maintained roots. | Must be wired in deliberately, checked against existing IRS/CM/inline rearmor handlers and smoke-tested for HandleDamage locality; otherwise archive/remove current maintained-root copies and handle stale modded copies under the modded mission policy. |
