...
| Active parameters with no runtime references | 5 scan-baseline leads from the docs/source tree: `WFBE_C_AI_MAX`, `WFBE_C_GAMEPLAY_BOMBS_ALTITUDE`, `WFBE_C_MODULE_BIS_HC`, `WFBE_C_MODULE_WFBE_IRS`, `WFBE_C_UNITS_CLEAN_TIMEOUT`. Current stable/B74.1 `origin/master@f8a76de34` rechecks close the two AI/cleanup rows for maintained roots; keep the scan output branch-scoped until regenerated. |
...
| `ai-max-visible-parameter-no-runtime-consumer` | Branch-split visible parameter | Docs/source `HEAD@0da6ba431`, current Miksuu `master@b8389e748243`, `origin/perf/quick-wins@0076040f` and historical release `a96fdda2` still expose/default `WFBE_C_AI_MAX` without an active maintained-root runtime reader found. Current stable/B74.1 `origin/master@f8a76de34`, B69 `8d465fce`, B74 `b23f557f` and B74.2 `d472da6a` read it at `Server/AI/Commander/AI_Commander_Produce.sqf:234` in both maintained roots. Player follower caps still use `WFBE_C_PLAYERS_AI_MAX`; local `d9506078` is not current Miksuu upstream. | Do not use this for player cap answers. For old-shape refs, port the AI commander production reader or hide/label the row. For current stable/B74.1/B74.2, smoke AI commander production sizing before release-complete wording. Canonical route: [Mission parameters](Mission-Parameters-Localization-And-Generated-Build-Inputs#parameter-cache-flow). |
| `units-clean-timeout-visible-parameter-comment-only-consumer` | Branch-split visible cleanup parameter | Docs/source `HEAD@0da6ba431`, current Miksuu `master@b8389e748243`, `origin/perf/quick-wins@0076040f` and historical release `a96fdda2` keep `WFBE_C_UNITS_CLEAN_TIMEOUT` comment-only in the old split at `Common_TrashObject.sqf:20`. Current stable/B74.1 `origin/master@f8a76de34`, B69 `8d465fce`, B74 `b23f557f` and B74.2 `d472da6a` use it at `Common_TrashObject.sqf:21` for non-man wreck cleanup while bodies use `WFBE_C_UNITS_BODIES_TIMEOUT`; empty vehicles separately use `WFBE_C_UNITS_EMPTY_TIMEOUT`. | For old-shape refs, port the B35/current split or hide/rename the row. For current stable/B74.1/B74.2, do not call it no-op; smoke body, wreck and empty-vehicle cleanup separately. Canonical route: [Mission parameters](Mission-Parameters-Localization-And-Generated-Build-Inputs#parameter-cache-flow). |
