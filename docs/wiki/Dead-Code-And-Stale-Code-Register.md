...
| `ai-max-visible-parameter-no-runtime-consumer` | Branch-split visible parameter | Docs branch `origin/docs/developer-wiki-index@ac932fbe`, `origin/perf/quick-wins@0076040f` and historical release `a96fdda2` still expose/default `WFBE_C_AI_MAX` without an active maintained-root runtime reader found. Current stable `origin/master@0139a346` and current Miksuu `master@d9506078` read it at `Server/AI/Commander/AI_Commander_Produce.sqf:89` in both maintained roots. Player follower caps still use `WFBE_C_PLAYERS_AI_MAX`. | Do not use this for player cap answers. For old-shape refs, port the AI commander production reader or hide/label the row. For current stable/Miksuu, smoke AI commander production sizing before release-complete wording. Canonical route: [Mission parameters](Mission-Parameters-Localization-And-Generated-Build-Inputs#parameter-cache-flow). |
| `units-clean-timeout-visible-parameter-comment-only-consumer` | Branch-split visible cleanup parameter | Docs branch `origin/docs/developer-wiki-index@ac932fbe`, `origin/perf/quick-wins@0076040f` and historical release `a96fdda2` keep `WFBE_C_UNITS_CLEAN_TIMEOUT` comment-only in the old split at `Common_TrashObject.sqf:20`. Current stable `origin/master@0139a346` and current Miksuu `master@d9506078` use it at `Common_TrashObject.sqf:21` for non-man wreck cleanup while bodies use `WFBE_C_UNITS_BODIES_TIMEOUT`; empty vehicles separately use `WFBE_C_UNITS_EMPTY_TIMEOUT`. | For old-shape refs, port the B35/current split or hide/rename the row. For current stable/Miksuu, do not call it no-op; smoke body, wreck and empty-vehicle cleanup separately. Canonical route: [Mission parameters](Mission-Parameters-Localization-And-Generated-Build-Inputs#parameter-cache-flow). |
...
| `rsc-clickabletext-soundpush-malformed` | Stale or malformed resource config | `Rsc/Ressources.hpp:556` has `soundPush[] = {, 0.2, 1};` in both maintained roots across docs/source `HEAD` `f7bc72a8`, current `origin/master` `cf2a6d6a`, Miksuu `upstream/master` `b8389e74`, `perf/quick-wins` `0076040f`, release `a96fdda2` and UI theme branches; `Ressources.hpp:92` shows the valid empty-sound precedent `{"", 0.2, 1}`. Current master/release now have 14 derived controls per root; docs/source/upstream/perf/theme branches have 17. | Use [Client UI systems](Client-UI-Systems-Atlas#clickable-text-soundpush-branch-matrix); replace with a valid value only after config/dialog smoke scope is named. |
...
| ID | Classification | Evidence | Action |
| --- | --- | --- | --- |
| `sqf-ai-supply-truck-worker-comment-only` | Comment-only SQF worker; current master safe-disables caller | `Server/Init/Init_Server.sqf:37` comments the `UpdateSupplyTruck` compile. Current `origin/master` `cf2a6d6a` initializes `wfbe_ai_supplytrucks` and logs the legacy logistics branch disabled at `:383-384` instead of spawning it; `AI_UpdateSupplyTruck.sqf:17` still points at missing `Server\FSM\supplytruck.fsm`. Miksuu `b8389e74` and `perf/quick-wins` still raw-spawn this worker in their maintained roots. | Keep documented as dormant/broken logistics code. Do not simply uncomment; preserve/port the safe-disable or redesign AI logistics with a real server-owned loop/FSM. |
...
| `client-task-system-comment-only` | Disabled legacy town-task UX path | Current source/Vanilla, stable `origin/master` / local `master` `89ae9dad`, perf `0076040f` and release `7ff18c49` all comment the old `TaskSystem` compile at `Client/Init/Init_Client.sqf:75` and old town-complete task spawn at `:759`; `Client_TaskSystem.sqf` remains in both maintained roots, with its own nested task-spawn comments at `:30,38`. Release `7ff18c49` separately revives commander `SetTask` sends as Objective Ping in `GUI_Menu_Command.sqf:336,344`, so do not classify that branch feature as old town TaskSystem revival. | Owner decision: hide/remove the old town-task residue or rebuild it as a new JIP-safe task feature. Treat release Objective Ping as a separate branch-only commander UI candidate. |
...
| Item | Evidence |
| --- | --- |
| `Client/Module/MASH/receiverMASHmarker.sqf` | The compile is commented at `Client/Init/Init_Client.sqf:132` and there is no other reference, so the client-side `WFBE_SE_MASH_MARKER_SENT` receiver PVEH is never installed. This is the **client** end of the MASH map-marker chain (distinct from the already-listed orphaned **server** `WFBE_CL_MASH_MARKER_CREATED` PVEH); both ends are dead, matching [Deep-review findings](Deep-Review-Findings) DR-34. Restore the compile to revive MASH map markers, or delete as intentionally-removed. |
...
| ID | Classification | Evidence | Action |
| --- | --- | --- | --- |
| `ui-duplicate-idd-collisions` | Duplicate misleading UI resource ID | In docs checkout `2fef1e3d`, `RscMenu_EASA` and `RscMenu_Economy` both use `idd = 23000` (`Rsc/Dialogs.hpp:3209`, `:3211`, `:3287`, `:3289`) and both have live `createDialog` callers (`GUI_Menu_Service.sqf:244`, `GUI_Menu.sqf:172`). Stable `cf2a6d6a` and release `a96fdda2` split those dialog IDs in both maintained roots; Miksuu `b8389e74` and perf `0076040f` still duplicate them. `RscOverlay` and `OptionsAvailable` both use `idd = 10200` everywhere checked (`Rsc/Titles.hpp:44`, `:46`, `:164`, `:165` in docs/Miksuu/perf; stable/release line-drift to `:168`, `:169`) and both are used through `cutRsc` paths (`Init_Client.sqf:149`, `Client_UpdateRHUD.sqf:7`). No checked maintained root has a `findDisplay 23000/10200` caller. | Do not delete any of these as dead. Treat as docs-ready UI parity cleanup or formal waiver through [UI resource parity cleanup](UI-Resource-Parity-Cleanup) and [UI IDD collision repair](UI-IDD-Collision-Repair): preserve or port stable/release dialog IDD split where target branches still duplicate `23000`, avoid introducing hard-coded IDD lookup assumptions, then smoke EASA, Economy, RHUD/FPS HUD, action icons and endgame stats. |
...
| P1 | Fix economy menu IDC `23004/23005/23006` parity. | Branch verification is done and centralized in [UI resource parity cleanup](UI-Resource-Parity-Cleanup): docs/Miksuu/perf keep stale disabled-state writes in both maintained roots, while stable `cf2a6d6a` and release `a96fdda2` rewrite the controller around dashboard `23020` in both maintained roots. The remaining work is target-branch UI parity cleanup plus Economy smoke, not another broad audit. |
| P1 | Fix or formally waive duplicate UI IDDs `23000` and `10200`. | Branch verification is done and centralized in [UI resource parity cleanup](UI-Resource-Parity-Cleanup): docs/Miksuu/perf still duplicate EASA/Economy `23000`, stable/release split those dialog IDs in both maintained roots, and title resources still share `10200` everywhere checked. No checked maintained root has a `findDisplay 23000/10200` caller, so remaining work is target-branch UI parity cleanup or explicit waiver plus smoke. |
| P1 | Keep visible AI/cleanup parameters branch-scoped. | Branch verification is centralized in [Mission parameters](Mission-Parameters-Localization-And-Generated-Build-Inputs): docs/perf/historical release refs still keep `WFBE_C_AI_MAX` readless and `WFBE_C_UNITS_CLEAN_TIMEOUT` comment-only, while current stable `origin/master@0139a346` and current Miksuu `master@d9506078` have maintained-root live readers for both. Remaining work is target-specific policy, port/hide decisions for old-shape refs, and host-parameter plus AI commander/body/wreck/empty-vehicle smoke, not another broad parameter audit. |
...
| TaskSystem / commander task assignment | Current source keeps old town TaskSystem disabled and commander `SetTask` sends commented; release `7ff18c49` reuses the commander Tasks tab as Objective Ping in both maintained roots. | Keep the concepts separate: old town tasks need a fresh JIP-safe design, while release Objective Ping needs OA menu smoke, target/audience validation and spam controls before promotion. |
