...
Status: docs only; current stable `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`, B74.2 `origin/claude/b74.2-aicom@21b62b04`, current B69 `origin/claude/b69@8d465fce` and adjacent B74 `origin/claude/b74-aicom-spend@b23f557f` have the server-side DR-47 tautology removed in both maintained roots, but vote UI preview/policy smoke remains open and older checked refs still carry the old comparison.
...
Checked again on 2026-06-23 after `git fetch --all --prune`. Current stable `origin/master@f8a76de34`, the same commit as `origin/claude/b74.1-aicom@f8a76de34`, B74.2 `origin/claude/b74.2-aicom@21b62b04`, current B69 `origin/claude/b69@8d465fce` and adjacent B74 `origin/claude/b74-aicom-spend@b23f557f` differ from the older `8c3942d2` / `cf2a6d6a` branch evidence: the vote worker no longer uses the old `>= || <=` tautology. The scoped `0139a3468609..origin/master`, `origin/master..origin/claude/b74.2-aicom`, `d472da6a..origin/claude/b74.2-aicom` and `origin/claude/b69..origin/claude/b74-aicom-spend` diffs are empty for the checked maintained-root `Server_VoteForCommander.sqf`, `GUI_VoteMenu.sqf`, `RequestNewCommander.sqf`, `Server_AssignNewCommander.sqf` and `GUI_Commander_VoteMenu.sqf` paths. Miksuu `b8389e748243`, `origin/perf/quick-wins@0076040f`, historical release commit `a96fdda2`, historical AI-commander commit `c20ce153` and docs/source `HEAD@e0d82714` still keep the old vote comparison. Current origin exposes no `release/*` or reassignment rescue head on 2026-06-23; `origin/claude/trello-vote-para-guards@92101abe` is branch-only vote-worker random tie-break plus paratrooper-guard context, not current stable/B74.2 closure.

| Surface | Current branch truth | Route |
| --- | --- | --- |
| Commander vote AI/no-commander outcome | Current stable `origin/master@f8a76de34` / B74.1, B74.2 `origin/claude/b74.2-aicom@21b62b04`, current B69 `origin/claude/b69@8d465fce` and adjacent B74 `origin/claude/b74-aicom-spend@b23f557f` Chernarus and maintained Vanilla still count `wfbe_vote == -1` into `_aiVotes` (`Server_VoteForCommander.sqf:18,26-27`) but now select a player commander only when `!_tie`, `_highestTeam != -1` and `_highest >= _aiVotes` (`:43`). Chernarus blame points to `cbc2294c4`; maintained Vanilla propagation points to `91dc6a75`; the checked `0139a3468609..f8a76de34`, current-stable..B74.2, `d472da6a..21b62b04` and B69..B74 vote-worker/preview diffs are empty. The client preview still treats row 0 or no strict majority as AI/no commander (`GUI_VoteMenu.sqf:88`), so UI/policy smoke remains open. Docs/source `HEAD@e0d82714`, Miksuu `b8389e748243`, perf `0076040f`, historical `a96fdda2` and historical `c20ce153` still use the old `>= || <=` comparison at `Server_VoteForCommander.sqf:43`. Branch-only `origin/claude/trello-vote-para-guards@92101abe` adds random tie-break handling at `Server_VoteForCommander.sqf:40-59` in both maintained roots, but it is not current stable/B74.2 behavior. | Current stable/B74.1/B74.2/B69/B74 server comparison is source-present/smoke-pending; this page owns the remaining UI/policy smoke matrix and old-branch route. |
| PR #80 random tied-player candidate | Open draft PR #80 / `origin/claude/trello-vote-para-guards@92101abe9f7b83bd7f658d03e4160d5f030aa3c7` is one clean commit on current stable base `f8a76de34`, with maintained-root vote/support payload. In both source Chernarus and maintained Vanilla, `Server_VoteForCommander.sqf:17` adds `_tiedTeams`; `:43-51` collects every tied team whose vote count equals `_highest`, selects one with `floor (random (count _tiedTeams))`, clears `_tie`, and then the existing grant line at `:59` can assign that randomly selected player team when `_highest >= _aiVotes`. Checked current stable, current B74.2, B69, adjacent B74, Miksuu and perf refs have no `_tiedTeams` hits in the maintained roots. | Branch-only commander vote candidate. Smoke player-candidate ties that beat/equal AI/no-commander votes, clear winners, zero-vote/AI-majority rounds, repeated runs for random variation, UI preview wording, JIP vote state and maintained Vanilla parity before promotion. Paratrooper cap from the same PR is owned by [Support specials](Support-Specials-And-Tactical-Modules-Atlas#pr-80-voteparatrooper-guard-branch-intel). |
| B74.5 primitive vote-roster display fallback | `origin/claude/b745@b996bcb3` adds source-Chernarus-only primitive rows for late joiners before live `wfbe_teams` group objects arrive. `GUI_VoteMenu.sqf:10-35` and `GUI_Commander_VoteMenu.sqf:10-32` render names from `WFBE_JIP_ROSTER_PRIMS` with row value `-1`; `GUI_VoteMenu.sqf:75-99,119-125` and `GUI_Commander_VoteMenu.sqf:69-91` suppress live prune/update loops while primitives are displayed and rebuild once player-led live groups arrive. The server/JIP producer is documented on [Player join/disconnect](Player-Join-Disconnect-And-AntiStack-Lifecycle#branch-intel---b745-primitive-roster-jip); UI ownership is on [Client UI systems](Client-UI-Systems-Atlas#current-branch-scope). No matching primitive-roster symbols were found in current stable, current B74.2, Miksuu master or maintained Vanilla Takistan checked paths. | Branch-only display/JIP fallback, not a vote outcome-policy change and not current stable/Vanilla behavior. Smoke must prove primitive rows stay visible without blank names/RPT spam, then hand over to live team-index rows before treating a selected player candidate as a normal castable/reassignable row. |
| Manual reassignment helper shape | Current stable `origin/master@f8a76de34` / B74.1, B74.2 `origin/claude/b74.2-aicom@21b62b04`, current B69 `origin/claude/b69@8d465fce` and adjacent B74 `origin/claude/b74-aicom-spend@b23f557f` Chernarus and maintained Vanilla unpack `_side = _this select 0` / `_commander = _this select 1` at `Server_AssignNewCommander.sqf:4-5`, while `RequestNewCommander.sqf:13-14` still spawns the helper and also sends `new-commander-assigned`. Duplicate senders remain at helper `:10` plus caller `RequestNewCommander.sqf:14`; the UI identity edge remains open. Docs/source `HEAD@60c35c05` still carries `_side = _this` in both maintained roots. | [Commander reassignment call shape](Commander-Reassignment-Call-Shape#current-branch-matrix) |
...
| --- | --- | --- |
| Commander vote resolution | DR-47 server tautology is fixed on current stable `origin/master@f8a76de34` / B74.1, B74.2 `origin/claude/b74.2-aicom@21b62b04`, current B69 `origin/claude/b69@8d465fce` and adjacent B74 `origin/claude/b74-aicom-spend@b23f557f`: player candidate assignment now requires `_highest >= _aiVotes`. Older checked refs still use `>= || <=`; `origin/claude/trello-vote-para-guards@92101abe` is branch-only tie-break context. | Source-present on current stable/B74.1/B74.2/B69/B74; Arma vote-smoke and UI/policy alignment still pending. |
| Vote UI preview | Client preview can still show AI/no commander for a distribution the current stable/B74.1/B69/B74 server resolves to a player, because `GUI_VoteMenu.sqf:88` uses row 0 / strict-majority preview logic rather than the server `_highest >= _aiVotes` rule. | Confirmed remaining mismatch risk. |
| Manual reassignment helper | DR-15 helper unpacking is source-present on current stable `origin/master@f8a76de34` / B74.1, current B69 `origin/claude/b69@8d465fce` and adjacent B74 `origin/claude/b74-aicom-spend@b23f557f`, but duplicate `new-commander-assigned` senders and UI visible-name targeting remain open. | Partial source-present; dedicated page owns exact patch/smoke shape. |
...
| Winner selection | Current stable/B74.1/B74.2/current B69/B74 `Server/Functions/Server_VoteForCommander.sqf:31-43` finds `_highest`, `_highestTeam` and `_tie`, then assigns a player commander only when `_highest >= _aiVotes` and the top player candidate is not tied. Older docs/source/Miksuu/perf/historical release/AI refs still use `_highest >= _aiVotes` OR `_highest <= _aiVotes` at `:43`. | Current stable/B74.1/B74.2/B69/B74 removed the tautology; old-branch code can still select any non-tied player candidate regardless of no-commander votes. |
| Final assignment | Current stable/current B69/B74 `Server/Functions/Server_VoteForCommander.sqf:49,54,57,61` applies the AI-commander lock override, writes `wfbe_commander`, broadcasts `commander-vote`, and stops AI commander state if a player commander wins. | Vote semantics affect commander, UI and AI handoff state. |
...
| B74.5 primitive rows | Branch-only `origin/claude/b745` source Chernarus adds primitive name rows from `WFBE_JIP_ROSTER_PRIMS` with value `-1` at `GUI_VoteMenu.sqf:28-35` and `GUI_Commander_VoteMenu.sqf:26-32`; both menus suppress live prune/update loops until live player-led groups arrive. | Primitive rows are display placeholders. Do not infer final vote semantics from them; normal row identity resumes only after live team-index rows are rebuilt. |
...
| Assign helper | Current stable/B74.1/current B69/B74 `Server/Functions/Server_AssignNewCommander.sqf:4-5,10` unpacks `_side` / `_commander` correctly, then sends another `new-commander-assigned`. `RequestNewCommander.sqf:13-14` still spawns the helper and sends the caller notification too. | DR-15 helper shape is source-present on current stable/B74.1/B69/B74, but duplicate notification ownership must still be decided. |
| Vanilla parity | Maintained Vanilla matches the named Chernarus current-stable/B69/B74 vote shape: the server comparison is source-present, while UI preview logic and old-branch vote comparison remain as documented above. Manual reassignment helper branch status stays on [Commander reassignment call shape](Commander-Reassignment-Call-Shape). | Use the reassignment owner page before claiming the broader commander lane fixed. |

## Current Vote Behavior

On current stable `origin/master@f8a76de34` / B74.1, B74.2 `origin/claude/b74.2-aicom@21b62b04`, current B69 `origin/claude/b69@8d465fce` and adjacent B74 `origin/claude/b74-aicom-spend@b23f557f`, the server tracks no-commander votes and now lets `_aiVotes` block a player candidate when the AI/no-commander count is greater than the highest player-candidate count. A player candidate can still win on equality because the current comparison is `_highest >= _aiVotes`. The branch-only `origin/claude/trello-vote-para-guards@92101abe` changes player-candidate ties that beat/equal AI votes into a random tied-team pick, but that is not current stable/B74.2 behavior.

The client vote menu can still suggest a different rule. It previews AI/no commander when the leading row is the No Commander row or when no row has more than half of the counted player slots. That creates a remaining player-facing mismatch: the UI can imply no commander while the current stable/B74.1/B69/B74 server assigns a human commander under the `_highest >= _aiVotes` rule.
...
4. Current stable/current B69/B74 maintained roots unpack the helper side correctly, so both caller and helper can send `new-commander-assigned`.

Patch or port the DR-15 helper call shape with [Commander reassignment call shape](Commander-Reassignment-Call-Shape) on old-shape targets. Current stable, current B69, adjacent B74, Miksuu, perf, historical release and historical AI-commander already fix helper unpacking in both maintained roots, but still need one notification owner or clients can receive duplicate commander messages.
...
| 4 | Preserve or port `Server_AssignNewCommander.sqf` payload unpacking per DR-15; current stable/B74.1/current B69/B74 maintained roots already use `_this select 0`, while docs/source and modded full forks still need old-shape handling. | Manual reassignment affects the intended side on every claimed target root. |
...
| B74.5 primitive roster late join | Primitive names appear when live `wfbe_teams` is missing/broken, no primitive row blanks or RPT spam occur, and the menu rebuilds to live team-index rows once live groups arrive. |
