...
| --- | --- | --- |
| Commander vote resolution | DR-47 server tautology is fixed on current stable `origin/master@f8a76de34` / B74.1, current B69 `origin/claude/b69@8d465fce` and adjacent B74 `origin/claude/b74-aicom-spend@b23f557f`: player candidate assignment now requires `_highest >= _aiVotes`. Older checked refs still use `>= || <=`. | Source-present on current stable/B74.1/B69/B74; Arma vote-smoke and UI/policy alignment still pending. |
