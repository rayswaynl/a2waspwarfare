...
Checked again on 2026-06-23 after `git fetch --all --prune`. Current stable `origin/master@f8a76de34`, the same commit as `origin/claude/b74.1-aicom@f8a76de34`, B74.2 `origin/claude/b74.2-aicom@d472da6a`, current B69 `origin/claude/b69@8d465fce` and adjacent B74 `origin/claude/b74-aicom-spend@b23f557f` differ from the older `8c3942d2` / `cf2a6d6a` branch evidence: the vote worker no longer uses the old `>= || <=` tautology. The scoped `0139a3468609..origin/master` diff touches no maintained-root `Server_VoteForCommander.sqf`, `GUI_VoteMenu.sqf`, `RequestNewCommander.sqf`, `Server_AssignNewCommander.sqf` or `GUI_Commander_VoteMenu.sqf` files. `origin/master..origin/claude/b74.2-aicom` and `origin/claude/b69..origin/claude/b74-aicom-spend` are empty for the checked vote/reassignment worker and preview paths. Miksuu `b8389e748243`, `origin/perf/quick-wins@0076040f`, historical release commit `a96fdda2`, historical AI-commander commit `c20ce153` and docs/source `HEAD@e0d82714` still keep the old vote comparison. Current origin exposes no `release/*`, vote or reassignment rescue head on 2026-06-23; broad `commander` branch matches are Codex AI-commander context/log branches, not vote-worker fixes.

| Surface | Current branch truth | Route |
| --- | --- | --- |
| Commander vote AI/no-commander outcome | Current stable `origin/master@f8a76de34` / B74.1, B74.2 `origin/claude/b74.2-aicom@d472da6a`, current B69 `origin/claude/b69@8d465fce` and adjacent B74 `origin/claude/b74-aicom-spend@b23f557f` Chernarus and maintained Vanilla still count `wfbe_vote == -1` into `_aiVotes` (`Server_VoteForCommander.sqf:18,26-27`) but now select a player commander only when `!_tie`, `_highestTeam != -1` and `_highest >= _aiVotes` (`:43`). Chernarus blame points to `cbc2294c4`; maintained Vanilla propagation points to `91dc6a75`; the checked `0139a3468609..f8a76de34`, B74.1..B74.2 and B69..B74 vote-worker/preview diffs are empty. The client preview still treats row 0 or no strict majority as AI/no commander (`GUI_VoteMenu.sqf:88`), so UI/policy smoke remains open. Docs/source `HEAD@e0d82714`, Miksuu `b8389e748243`, perf `0076040f`, historical `a96fdda2` and historical `c20ce153` still use the old `>= || <=` comparison at `Server_VoteForCommander.sqf:43`. | Current stable/B74.1/B74.2/B69/B74 server comparison is source-present/smoke-pending; this page owns the remaining UI/policy smoke matrix and old-branch route. |
...
| --- | --- | --- |
| Commander vote resolution | DR-47 server tautology is fixed on current stable `origin/master@f8a76de34` / B74.1, B74.2 `origin/claude/b74.2-aicom@d472da6a`, current B69 `origin/claude/b69@8d465fce` and adjacent B74 `origin/claude/b74-aicom-spend@b23f557f`: player candidate assignment now requires `_highest >= _aiVotes`. Older checked refs still use `>= || <=`. | Source-present on current stable/B74.1/B74.2/B69/B74; Arma vote-smoke and UI/policy alignment still pending. |
