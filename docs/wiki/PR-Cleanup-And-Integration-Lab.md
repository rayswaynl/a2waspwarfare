...
Last updated: 2026-06-22 for PR #50 Takistan AICOM port into B69.
...
Current board state was refreshed from GitHub on 2026-06-21 with `gh pr list --repo rayswaynl/a2waspwarfare --state all --limit 80 --json number,title,state,headRefName,baseRefName,isDraft,updatedAt,mergedAt,url`. The returned set had `3` open PRs, `12` merged PRs and `28` closed PRs at that moment. Targeted 2026-06-22 lookups show PR #47 merged `claude/b69-finalpieces` into `claude/b69` at `0094647d7b641bb79202e77e7f480d6d39aadcdb` on 2026-06-22T10:23:16Z, PR #48 merged `claude/b71-pr-harvest` into `claude/b69` at `4dcc10b143a0fc4d94e55f5506217d208994d4ff` on 2026-06-22T11:06:12Z, PR #49 merged `claude/b72-tints-on` into `claude/b69` at `39eed5c02d8ba9c5a27b7a4173607526edc0677e` on 2026-06-22T11:47:23Z, and PR #50 merged `claude/tk-aicom-port` into `claude/b69` at `b8530477ce4f8cc66c60a5d310a85d25c9cbc27c` on 2026-06-22T13:33:48Z. A later `git fetch --all --prune` removed local synthetic PR refs such as `origin/pr/*` and `miksuu/pr/*`; use the GitHub PR URLs, `headRefName`/`baseRefName`, `mergedAt` / `updatedAt` and remote branch heads as current evidence. Older ancestry notes below are preserved as historical results from the earlier PR-ref audit, not as refs future agents should expect to resolve locally.
...
| Merged into a non-master branch | [#42](https://github.com/rayswaynl/a2waspwarfare/pull/42), [#47](https://github.com/rayswaynl/a2waspwarfare/pull/47), [#48](https://github.com/rayswaynl/a2waspwarfare/pull/48), [#49](https://github.com/rayswaynl/a2waspwarfare/pull/49), [#50](https://github.com/rayswaynl/a2waspwarfare/pull/50) | #42 merged `claude/guer-merge` -> `claude/b39` on 2026-06-16T19:56:21Z. #47 merged `claude/b69-finalpieces` -> `claude/b69` on 2026-06-22T10:23:16Z at `0094647d7b64`; #48 merged `claude/b71-pr-harvest` -> `claude/b69` on 2026-06-22T11:06:12Z at `4dcc10b143a0`; #49 merged `claude/b72-tints-on` -> `claude/b69` on 2026-06-22T11:47:23Z at `39eed5c0`; #50 merged `claude/tk-aicom-port` -> `claude/b69` on 2026-06-22T13:33:48Z at `b8530477`. Treat these as branch-integration evidence, not as master merges by themselves. |
...
The old AICOM deploy route through PR #35 is closed. PR #29 (`ai-commander-main` -> `master`) and PR #31 (`perf/marker-consolidation` -> `master`) both merged on 2026-06-17T11:45:52Z; PR #30, PR #34, PR #35, PR #36, PR #37, PR #38, PR #39 and PR #41 are closed. Current source truth for merged AICOM/GUER work starts from `origin/master@0139a346`; current open review routing is PR #43 for B57/B68 soak/proposals at `b8a1505f`, PR #40 for the still-open client-FPS child stacked on a closed base, and PR #9 for Zargabad. PR #47, PR #48, PR #49 and PR #50 are now merged into branch `claude/b69`, not open and not master/stable routes.
...
### 2026-06-21 AICOM / Experital PR Board Refresh With 2026-06-22 PR #47-#50 Addenda
...
| [#50](https://github.com/rayswaynl/a2waspwarfare/pull/50) | `claude/tk-aicom-port` -> `claude/b69` | Merged 2026-06-22T13:33:48Z, merge commit `b8530477`; head `3b1106a1` | Branch-integration route only. The `39eed5c0..b8530477` merge delta is 22 maintained Vanilla/Takistan files / +2499 / -147 and ports the B69/B59-B72 AICOM route to Takistan. Current `origin/claude/b69@b8530477` is now both-map branch evidence, but still not a master/stable route or Arma-smoked proof. |
...
1. Current open PRs after the 2026-06-22 PR #47-#50 branch merges are PR #43, PR #40 and PR #9 unless a fresh GitHub board refresh proves otherwise.
2. Treat PR #43 as the live master-target soak/proposals route, and PR #47-#50 as merged branch evidence inside `claude/b69`, not open review routes.
...
- PR47 merged B69 finalpieces into `claude/b69` at merge commit `0094647d7b64`; PR48 and PR49 also merged into `claude/b69`; PR50 merged the Takistan AICOM port into `claude/b69` at `b8530477`. They are branch evidence, not master-target routes.
