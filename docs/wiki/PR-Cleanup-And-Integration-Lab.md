...
Last updated: 2026-06-22 for PR #49 B72 tints-on into B69.
...
Current board state was refreshed from GitHub on 2026-06-21 with `gh pr list --repo rayswaynl/a2waspwarfare --state all --limit 80 --json number,title,state,headRefName,baseRefName,isDraft,updatedAt,mergedAt,url`. The returned set had `3` open PRs, `12` merged PRs and `28` closed PRs at that moment. Targeted 2026-06-22 lookups show PR #47 merged `claude/b69-finalpieces` into `claude/b69` at `0094647d7b641bb79202e77e7f480d6d39aadcdb` on 2026-06-22T10:23:16Z, PR #48 merged `claude/b71-pr-harvest` into `claude/b69` at `4dcc10b143a0fc4d94e55f5506217d208994d4ff` on 2026-06-22T11:06:12Z, and PR #49 merged `claude/b72-tints-on` into `claude/b69` at `39eed5c02d8ba9c5a27b7a4173607526edc0677e` on 2026-06-22T11:47:23Z. A later `git fetch --all --prune` removed local synthetic PR refs such as `origin/pr/*` and `miksuu/pr/*`; use the GitHub PR URLs, `headRefName`/`baseRefName`, `mergedAt` / `updatedAt` and remote branch heads as current evidence. Older ancestry notes below are preserved as historical results from the earlier PR-ref audit, not as refs future agents should expect to resolve locally.
...
| Merged into a non-master branch | [#42](https://github.com/rayswaynl/a2waspwarfare/pull/42), [#47](https://github.com/rayswaynl/a2waspwarfare/pull/47), [#48](https://github.com/rayswaynl/a2waspwarfare/pull/48), [#49](https://github.com/rayswaynl/a2waspwarfare/pull/49) | #42 merged `claude/guer-merge` -> `claude/b39` on 2026-06-16T19:56:21Z. #47 merged `claude/b69-finalpieces` -> `claude/b69` on 2026-06-22T10:23:16Z at `0094647d7b64`; #48 merged `claude/b71-pr-harvest` -> `claude/b69` on 2026-06-22T11:06:12Z at `4dcc10b143a0`; #49 merged `claude/b72-tints-on` -> `claude/b69` on 2026-06-22T11:47:23Z at `39eed5c0`. Treat these as branch-integration evidence, not as master merges by themselves. |
...
1. Current open PRs after the 2026-06-22 PR #47 merge are PR #43, PR #40 and PR #9 unless a fresh GitHub board refresh proves otherwise.
2. Treat PR #43 as the live master-target soak/proposals route, and PR #47 as merged branch evidence inside `claude/b69`, not an open review route.
...
- PR47 merged B69 finalpieces into `claude/b69` at merge commit `0094647d7b64`; it is branch evidence, not a master-target route
