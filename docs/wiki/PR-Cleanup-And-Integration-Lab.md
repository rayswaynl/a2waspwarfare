...
| --- | --- | --- |
| [#84](https://github.com/rayswaynl/a2waspwarfare/pull/84) | Open non-draft stack on `feat/player-stats`, not a `master` target. GitHub reports `CLEAN` against base `feat/player-stats@e01e47e12a767c3406f8b89b7df5ea9d95260b87`; local `origin/master...origin/feat/player-stats-names` counts are not a useful merge gate because the PR base is the stats branch. | Keep routed through [Player stats branch audit](Player-Stats-Branch-Audit#pr-84-player-name-stack). |
| [#83](https://github.com/rayswaynl/a2waspwarfare/pull/83) | Open draft `claude/wiki-wins-batch` -> `master`, head `a61823265c82eb7e35c7c74d851647c2235cf169`. `gh api` and the GitHub connector both report `mergeable=false` / `dirty` with base SHA `f8a76de349da6f8b871d079c828436c10afb221c`; local counts against current `origin/master@f39665d79` are `80	8`. | Treat the earlier 46-file / +124 / -86 branch-intake as pre-B745/B751b evidence only. Rebase/split/recheck before merge or release wording. |
| [#80](https://github.com/rayswaynl/a2waspwarfare/pull/80), [#74](https://github.com/rayswaynl/a2waspwarfare/pull/74), [#72](https://github.com/rayswaynl/a2waspwarfare/pull/72), [#66](https://github.com/rayswaynl/a2waspwarfare/pull/66), [#63](https://github.com/rayswaynl/a2waspwarfare/pull/63) | Open draft `master` targets. GitHub reports `CLEAN`, but each still names base SHA `f8a76de349da6f8b871d079c828436c10afb221c`; local counts against current master are `80	1` for each branch. | PR #80, #74, #72 and #66 now have owner-page branch-intake rows; rerun source/smoke checks after rebase or direct merge planning because current master now includes B745-B751b. Route #63 through its owner lane before release wording. |
| [#59](https://github.com/rayswaynl/a2waspwarfare/pull/59), [#54](https://github.com/rayswaynl/a2waspwarfare/pull/54) | Open draft upstream-mining `master` targets. GitHub reports `CLEAN`; local merge-base with current master is `be2bbd084e1922386e819fed2a1566951b193db0`, and current master is `81` commits ahead with each branch `1` commit ahead. | Keep as upstream-mining candidates; do not reuse old branch stats as current-merge proof. |
| [#53](https://github.com/rayswaynl/a2waspwarfare/pull/53) | Open non-draft OA countermeasure candidate. GitHub reports `CLEAN`; local merge-base with current master is `cf2a6d6a4f963637f6fdd8563a3f4e58b5e79fdd`, with current master `509` commits ahead and branch `1` commit ahead. | Keep separate OA countermeasure smoke lane, not part of PR #83, B745/B751b or Zargabad. |
| [#9](https://github.com/rayswaynl/a2waspwarfare/pull/9) | Open non-draft map/content candidate. `gh api` and the GitHub connector report `mergeable=false` / `dirty`, base SHA `2cdf5fb8058b4ede4faa212ecc4fc52d9d83eef8`, 80 commits, 832 files, +77733 / -95; local counts against current master are `666	80`. | Keep as separate Zargabad map/content validation lane with its own runtime-tool gates before any current-master merge attempt. |
| [#85](https://github.com/rayswaynl/a2waspwarfare/pull/85), [#86](https://github.com/rayswaynl/a2waspwarfare/pull/86), [#87](https://github.com/rayswaynl/a2waspwarfare/pull/87), [#88](https://github.com/rayswaynl/a2waspwarfare/pull/88), [#89](https://github.com/rayswaynl/a2waspwarfare/pull/89) | Merged into `master` on 2026-06-24, advancing current source stable to `origin/master@f39665d7950ddf14cb0bbfacb7ee1e40121e93b4`. | Treat as current-master source history already routed through current-source/status/testing pages, not open PR cleanup items. |
...
| Open draft Trello / upstream candidates | [#74](https://github.com/rayswaynl/a2waspwarfare/pull/74), [#72](https://github.com/rayswaynl/a2waspwarfare/pull/72), [#66](https://github.com/rayswaynl/a2waspwarfare/pull/66), [#63](https://github.com/rayswaynl/a2waspwarfare/pull/63), [#59](https://github.com/rayswaynl/a2waspwarfare/pull/59), [#54](https://github.com/rayswaynl/a2waspwarfare/pull/54) | Current open draft master-target branch set from the 2026-06-24 post-B751b open-board refresh. GitHub reports `CLEAN` for these branches, but #74/#72/#66/#63 still use base `f8a76de34` with local counts `80	1`, while #59/#54 merge-base at `be2bbd084e` with local counts `81	1`. PR #74 now routes through support/tactical, Client UI and Testing; PR #72 routes through Construction/CoIn, Feature Status and Testing; PR #66 routes through Structure Damage/Friendly Fire, Feature Status and Testing. Route #63/#59/#54 through existing owner pages or fresh bounded PR-intake lanes before release wording. Do not infer runtime proof from draft/open status. |
| Open non-draft feature candidate | [#53](https://github.com/rayswaynl/a2waspwarfare/pull/53) | `feat/oa-auto-countermeasures` -> `master`, updated 2026-06-23T07:59:42Z. GitHub reports `CLEAN`, but local merge-base with post-B751b master is `cf2a6d6a` and counts are `509	1`. Keep as a separate OA countermeasure feature-smoke lane, not as part of PR #83 or the B74/B69/B745-B751b branch history. |
| Open map/content candidate | [#9](https://github.com/rayswaynl/a2waspwarfare/pull/9) | Zargabad low-pop mission remains open against `master`, updated 2026-06-20T05:17:48Z. `gh api` and the GitHub connector now report `mergeable=false` / `dirty`; base SHA is `2cdf5fb8058b4ede4faa212ecc4fc52d9d83eef8`, and local counts against `origin/master@f39665d79` are `666	80`. Keep as a separate map/content validation lane with its own runtime-tool gates before any current-master merge attempt. |
...
| Recent current-master merges | [#85](https://github.com/rayswaynl/a2waspwarfare/pull/85), [#86](https://github.com/rayswaynl/a2waspwarfare/pull/86), [#87](https://github.com/rayswaynl/a2waspwarfare/pull/87), [#88](https://github.com/rayswaynl/a2waspwarfare/pull/88), [#89](https://github.com/rayswaynl/a2waspwarfare/pull/89) | Merged into `master` on 2026-06-24 and advanced current source stable to `origin/master@f39665d7950ddf14cb0bbfacb7ee1e40121e93b4`. Their source evidence belongs on current-source/status/testing owner pages, not the open PR board. |
...
Treat the branch as maintained-root branch evidence, not current stable or release wording. In source Chernarus plus maintained Vanilla, the branch adds a `WF_Debug` early exit in `WFBE_CL_FNC_OnFiredSatchel` before the existing friendly-structure scan/deletion and `StructureTK` message (`Client_FNC_OnFired.sqf:12,16,35,41` on the branch), and adds a `WF_Debug` early return in `Server_BuildingHandleDamages.sqf` before the own-side/`sideEnemy` nulling gate and enemy-hit reduction path (`Server_BuildingHandleDamages.sqf:4,9,11-17` on the branch). Current master has the same functions without those debug exits (`Client_FNC_OnFired.sqf:12,32,38`; `Server_BuildingHandleDamages.sqf:4,9-15`), while `WF_Debug` defaults false in both roots at `initJIPCompatible.sqf:110` and only flips true under the existing debug condition at `:112`. Route owner detail through [Structure damage reduction and friendly-fire](Structure-Damage-Reduction-And-Friendly-Fire#2026-06-24-pr-66-debug-anti-tk-branch-addendum) and smoke through [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack).
...
3. Treat PR #84 as an open non-draft player-stats stack on `feat/player-stats`; route details through [Player stats branch audit](Player-Stats-Branch-Audit#pr-84-player-name-stack) and the branch-only stats smoke gate before any deployed/current-stable wording.
...
6. Treat PR #74 as an open draft artillery cooldown HUD candidate routed through support/tactical, Client UI and Testing; PR #72 as an open draft anti-base-trap static-defense candidate routed through Construction/CoIn, Feature Status and Testing; and PR #66 as an open draft debug anti-TK candidate routed through Structure Damage/Friendly Fire, Feature Status and Testing. All three remain clean-but-stale-base after B751b. PR #63/#59/#54 and PR #53 remain open Trello/upstream/OA candidates that need their own owner route if not already documented.
