...
| Side radio/message pipeline | `Server/Init/Init_Server.sqf:32`; `Server/Functions/Server_SideMessage.sqf`; callers in `server_town.sqf:199,230,233`, `Server_BuildingKilled.sqf:92` and `Server_ProcessUpgrade.sqf:85` | Server-owned town/base/upgrade radio messages. The payload switch is compact and shape-sensitive; smoke town under-attack/captured/lost, base destroyed and upgrade-complete messages before refactoring side-message text or dubbing. |
...
| Side-message payload shape | Under-documented runtime surface | `SideMessage` takes different parameter shapes for town, base and upgrade notifications. Keep it source-cited before adding new radio messages. |
