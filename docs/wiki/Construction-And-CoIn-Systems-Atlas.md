...
Checked 2026-06-14 against docs checkout `docs/developer-wiki-index` `4bd37b98`, stable `origin/master` `cf2a6d6a`, Miksuu `b8389e74`, `origin/perf/quick-wins` `0076040f` and release `origin/release/2026-06-feature-bundle` `a96fdda2` in Chernarus plus maintained Vanilla Takistan where present. The construction files cited below are unchanged from the earlier `1aa178f8` construction-atlas anchor snapshot, so existing docs/source line refs remain valid.

| Branch/root | Construction shape | Development meaning |
| --- | --- | --- |
| Docs checkout `4bd37b98` | Chernarus and maintained Vanilla carry the `Construction_StationaryDefense.sqf:12-15` `_availweapons` null guard and commander-team ARTY gunner handoff at `:91-93`; `Common_GetTeamArtillery.sqf:10-30` scans the player's group vehicles. | This is the current docs/source shape for patch carry-forward, but it still needs Arma smoke for grouped-base and Tactical fire-mission behavior. |
...
Focused construction scout 2026-06-04 found a small guard-order bug here. Docs checkout `4bd37b98` source Chernarus and maintained Vanilla Takistan now default `_availweapons` to `0` and read `_area getVariable "weapons"` only after `!isNull _area` (`Construction_StationaryDefense.sqf:12-15`). Stable/release still read `_area getVariable "weapons"` before a null guard at `:15`; Miksuu/perf still do so at `:13`. Arma smoke is still needed for defense placement with grouped-base mode on and off, and this does not close broader DR-6 construction authority hardening.
...
| --- | --- | --- | --- |
| Docs checkout `4bd37b98` Chernarus and maintained Vanilla | `Skill_Salvage.sqf:38` and `updatesalvage.sqf:50` call lowercase `ChangePlayerfunds`; `Init_Client.sqf:53,91` compiles `ChangePlayerFunds`. | `updatesalvage.sqf:10` uses `while {!gameOver || !(alive _vehicle)}`; both salvage paths delete wrecks locally at `Skill_Salvage.sqf:34` / `updatesalvage.sqf:46`. | Patch-ready, current-source-unpatched. |
| Stable `origin/master` `cf2a6d6a`, release `a96fdda2`, Miksuu `b8389e74` and `perf/quick-wins` `0076040f` | Same lowercase payout calls and compile casing in both maintained roots. | Same `||` loop and client-local deletion/reward shape in both maintained roots. | No branch rescue candidate found in these stable/release/upstream/fix refs. |
