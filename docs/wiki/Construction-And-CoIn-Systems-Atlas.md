...
| Auto-wall refreshed 2026-06-22 | All checked refs still keep one mission-global `isAutoWallConstructingEnabled`. Docs branch `89aadf8c` and perf `0076040f` initialize it `false` at `Init_Common.sqf:201` and consume it without structure exclusions at SmallSite `:110` / MediumSite `:125`. Current stable `0139a346` initializes it `true` at `:213` and current Miksuu `d9506078` at `:212`; both consume it at SmallSite `:123` / MediumSite `:160` with `AARadar` plus `CBRadar` or `Bank`/`Reserve`/`ArtilleryRadar` exclusions. Historical `a96fdda2` initializes true and only guards `AARadar`; B69 `35547c47` matches current-stable global-scope behavior with line drift. | No checked branch keys the state by player, side or requester. Keep the global-vs-preference decision open until source Chernarus plus maintained Vanilla are changed and smoke-tested with two players/sides. |
