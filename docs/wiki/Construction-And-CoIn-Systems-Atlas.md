...
| Auto-wall corrected 2026-06-22 | All checked refs still keep one mission-global `isAutoWallConstructingEnabled`. Docs branch `89aadf8c`, current Miksuu `b8389e748243` and perf `0076040f` initialize it `false` at `Init_Common.sqf:201` and consume it without structure exclusions at SmallSite `:110` / MediumSite `:125`. Current stable `0139a346` initializes it `true` at `:213` and consumes it at SmallSite `:123` / MediumSite `:160` with `AARadar` plus `CBRadar` or `Bank`/`Reserve`/`ArtilleryRadar` exclusions. Historical `a96fdda2` initializes true and only guards `AARadar`; B69 `35547c47` matches current-stable global-scope behavior with line drift. | No checked branch keys the state by player, side or requester. Keep the global-vs-preference decision open until source Chernarus plus maintained Vanilla are changed and smoke-tested with two players/sides. |
| Other checked construction debt | SmallSite still appends `_nearLogic` after construction while MediumSite removes it. 2026-06-22 current-B69 refresh: docs/source `HEAD@6b8eba5e` is unchanged from `3406ffa0` for checked construction paths, and current stable `origin/master@0139a346`, Miksuu `b8389e74`, perf `0076040f`, historical release `a96fdda2` and current B69 `8d465fce` all keep SmallSite add/add at `Construction_SmallSite.sqf:70,99` and MediumSite add/remove at `Construction_MediumSite.sqf:70,114` in both maintained roots; B69 checked construction paths are unchanged from prior proof `0a1ccb4d`. Salvage recheck 2026-06-22 keeps lowercase `ChangePlayerfunds`, the `updatesalvage.sqf:10` `||` loop and local delete/reward shape in docs `fb8d4ebc`, current stable `0139a346`, current Miksuu `b8389e748243`, perf `0076040f` and historical `a96fdda2`. | Keep these as patch-ready construction/salvage findings until source Chernarus plus maintained Vanilla are changed and smoked. |
...
Auto-wall state is global, not per-player or per-side. The source anchors are the `User14` client toggle at `coin_interface.sqf:180,207-217`, the thin server write at `Server/PVFunctions/RequestAutoWallConstructinChange.sqf:3-7`, and SmallSite/MediumSite consumers. [Current Branch Scope](#current-branch-scope) owns the branch comparison: docs branch `89aadf8c`, current Miksuu `b8389e748243` and perf `0076040f` consume it at SmallSite `:110` / MediumSite `:125` without exclusions, current stable `0139a346` consumes it at `:123` / `:160` with broader non-wall exclusions, historical release `a96fdda2` only guards `AARadar`, and B69 `35547c47` matches the current-stable global-scope behavior. No checked branch keys the state by player, side or requester. One user's toggle can therefore affect later small/medium construction globally.
...
- 2026-06-22 current-B69 refresh found no rescue in docs/source `HEAD@6b8eba5e` (unchanged from `3406ffa0` for checked construction paths), current stable `origin/master@0139a346`, Miksuu `b8389e74`, perf `0076040f`, historical release `a96fdda2` or current B69 `8d465fce` (checked construction paths unchanged from `0a1ccb4d`): SmallSite remains add/add at `:70,99`, while MediumSite remains add/remove at `:70,114` in both maintained roots. Current source continuity is summarized in [Current Branch Scope](#current-branch-scope), and the exact patch route lives in [Construction logic list cleanup](Construction-Logic-List-Cleanup). Treat this as a real patch candidate, not merely an old comment mismatch.
