...
Auto-wall state is global, not per-player or per-side. `Common/Init/Init_Common.sqf:201` initializes `isAutoWallConstructingEnabled = false`; `Client/Module/CoIn/coin_interface.sqf:207-217` toggles it through `RequestAutoWallConstructinChange`; the server handler writes one global variable at `Server/PVFunctions/RequestAutoWallConstructinChange.sqf:3-7`; and both `Construction_SmallSite.sqf:110-112` and `Construction_MediumSite.sqf:125-128` consume that same value when creating wall templates. One user's toggle can therefore affect later small/medium construction globally. Treat this as a workflow/authority risk before expanding auto-wall behavior.

Branch check 2026-06-05 found no branch rescue: current docs/source Chernarus and maintained Vanilla, stable `origin/master`, Miksuu upstream, `origin/perf/quick-wins` and `origin/release/2026-06-feature-bundle` all keep the same `User14` client toggle, thin `RequestAutoWallConstructinChange` server write and global SmallSite/MediumSite consumer shape. Release Chernarus has only a one-line drift in `Init_Common.sqf` (`:200` instead of `:201`); the scope is still global in both release roots. Future code work should decide whether this is an intentional match-wide switch or a player/side preference, then either label the UI behavior or key the state by side/requester and smoke two players/sides toggling before small and medium construction.
...
- Mini-scout follow-up 2026-06-04 rechecked the same lines directly against source Chernarus: SmallSite remains add/add, MediumSite remains add/remove. Branch check 2026-06-05 found the same shape in current source/Vanilla, stable `origin/master`, Miksuu upstream and release. Treat this as a real patch candidate, not merely an old comment mismatch.
...
- Commander-built artillery-class defenses inside a valid base area now use the current commander team as their gunner group when a commander team exists (`Construction_StationaryDefense.sqf:91-94`). This matches tactical artillery discovery, which scans `units group player` for occupied non-player artillery (`Common_GetTeamArtillery.sqf:10-30`). Non-artillery defenses still use the base area's `DefenseTeam`; unmanned artillery remains unmanned if the CoIn manning toggle is off.

Focused construction scout 2026-06-04 found a small guard-order bug here. The source-Chernarus and maintained Vanilla Takistan patch now defaults `_availweapons` to `0` and reads `_area getVariable "weapons"` only after `!isNull _area` (`Construction_StationaryDefense.sqf:12-15`). Arma smoke is still needed for defense placement with grouped-base mode on and off, and this does not close broader DR-6 construction authority hardening.
...
| Base-area `avail` / `weapons` reads can happen before null guards. | `coin_interface.sqf:256-263`, `:721-730`; server-side `Construction_StationaryDefense.sqf:12-15` is patched in source Chernarus and maintained Vanilla Takistan. | Client CoIn placement/sale still need guard-order cleanup; smoke empty, stale and race-pruned base-area lists before closing the whole base-area availability lane. |
...
| --- | --- | --- | --- |
| Current docs/source Chernarus (`HEAD` `8d6b7c2c`) | `Skill_Salvage.sqf:38` and `updatesalvage.sqf:50` call lowercase `ChangePlayerfunds`; `Init_Client.sqf:53,91` compiles `ChangePlayerFunds`. | `updatesalvage.sqf:10` uses `while {!gameOver || !(alive _vehicle)}`; both salvage paths delete wrecks locally at `Skill_Salvage.sqf:34` / `updatesalvage.sqf:46`. | Patch-ready, current-source-unpatched. |
| Maintained Vanilla Takistan (`HEAD` `8d6b7c2c`) | Same lowercase payout calls and uppercase compile target at the same maintained-root lines. | Same `||` loop and client-local deletion/reward shape. | Needs propagation with Chernarus. |
| Stable `origin/master` (`2cdf5fb8`) | Same shape in Chernarus and maintained Vanilla. | Same `||` loop and client-local deletion/reward shape. | Stable still carries the defect. |
| Current `miksuu/master` (`f532f706`) | Same shape in Chernarus and maintained Vanilla. | Same `||` loop and client-local deletion/reward shape. | Upstream master still carries the defect. |
| `origin/perf/quick-wins` (`0076040f`) and release `origin/release/2026-06-feature-bundle` (`3282ff3f`) | Same shape in Chernarus and maintained Vanilla. | Same `||` loop and client-local deletion/reward shape. | No branch rescue candidate found in these release/fix branches. |
...
| Salvage payout and deletion | Salvage payout casing, update-loop and client-local deletion/reward are still identical across current source/Vanilla, stable, Miksuu upstream, perf/quick-wins, release and historical Miksuu salvage branches. | Fix the `ChangePlayerFunds` casing first, propagate Vanilla, and smoke manual/truck salvage; handle server-owned salvage payout/deletion as a separate authority hardening lane. |
