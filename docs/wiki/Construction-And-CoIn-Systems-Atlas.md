...
| Auto-wall current B74 refresh 2026-06-23 | All checked refs still keep one mission-global `isAutoWallConstructingEnabled`, registered as `RequestAutoWallConstructinChange` at `Init_PublicVariables.sqf:21` and toggled from `User14` / `coin_interface.sqf:180,207-217`. Docs/source `HEAD@f3896116`, current Miksuu `b8389e748243` and perf `0076040f` initialize it `false` at `Init_Common.sqf:201` and consume it without structure exclusions at SmallSite `:110` / MediumSite `:125`. Current stable `0139a346` initializes it `true` at `:213`; B69 `8d465fce` and B74 `b23f557f` initialize it `true` at `:214`. Stable/B69/B74 consume it at SmallSite `:123` with `AARadar`/`CBRadar` exclusions and at MediumSite `:160` in maintained Vanilla, while B69/B74 Chernarus MediumSite drifts to `:163` because of unrelated bank-pending lines; the exclusion list remains `AARadar`, `Bank`, `Reserve` and `ArtilleryRadar`. Historical `a96fdda2` initializes true and only guards `AARadar` at SmallSite/MediumSite `:110/:125`. | B69..B74 and `35547c47..B74` checked auto-wall path deltas are empty; stable..B74 touches checked files only for unrelated AICOM/GuerVbied compile and Bank pending-reservation lines. No checked branch keys the state by player, side or requester, so the global-vs-preference decision remains open until source Chernarus plus maintained Vanilla are changed and smoke-tested with two players/sides. |
