...
Checked 2026-06-13 against docs checkout `docs/developer-wiki-index` `1aa178f8`, stable `origin/master` `cf2a6d6a`, Miksuu `b8389e74`, `origin/perf/quick-wins` `0076040f` and release `origin/release/2026-06-feature-bundle` `a96fdda2` in Chernarus plus maintained Vanilla Takistan where present.

| Branch/root | Construction shape | Development meaning |
| --- | --- | --- |
| Docs checkout `1aa178f8` | Chernarus and maintained Vanilla carry the `Construction_StationaryDefense.sqf:12-15` `_availweapons` null guard and commander-team ARTY gunner handoff at `:91-93`; `Common_GetTeamArtillery.sqf:10-30` scans the player's group vehicles. | This is the current docs/source shape for patch carry-forward, but it still needs Arma smoke for grouped-base and Tactical fire-mission behavior. |
...
Auto-wall state is global, not per-player or per-side. Docs checkout `1aa178f8`, Miksuu `b8389e74` and `perf/quick-wins` `0076040f` initialize `isAutoWallConstructingEnabled = false` at `Common/Init/Init_Common.sqf:201`; stable `origin/master` `cf2a6d6a` initializes it to `true` at `:202`; release `a96fdda2` initializes it to `true` at `:201`. All checked refs keep the `User14` client toggle at `Client/Module/CoIn/coin_interface.sqf:180,207-217`, the thin server write at `Server/PVFunctions/RequestAutoWallConstructinChange.sqf:3-7`, and SmallSite/MediumSite consumers at `Construction_SmallSite.sqf:110` / `Construction_MediumSite.sqf:125`. Stable/release add an `AARadar` exclusion at those consumer lines, but no checked branch keys the state by player, side or requester. One user's toggle can therefore affect later small/medium construction globally.

Branch check 2026-06-13 found no branch rescue for scope: Chernarus and maintained Vanilla on docs checkout `1aa178f8`, stable `cf2a6d6a`, Miksuu `b8389e74`, `perf/quick-wins` `0076040f` and release `a96fdda2` all keep the same global-state path. Future code work should decide whether this is an intentional match-wide switch or a player/side preference, then either label the UI behavior or key the state by side/requester and smoke two players/sides toggling before small and medium construction.
...
- Mini-scout follow-up 2026-06-04 rechecked the same lines directly against source Chernarus: SmallSite remains add/add, MediumSite remains add/remove. Branch check 2026-06-13 found the same shape in docs checkout `1aa178f8`, stable `origin/master` `cf2a6d6a`, Miksuu `b8389e74`, `perf/quick-wins` `0076040f` and release `a96fdda2` in both maintained roots. Treat this as a real patch candidate, not merely an old comment mismatch.
...
- Docs checkout `1aa178f8` commander-built artillery-class defenses inside a valid base area use the current commander team as their gunner group when a commander team exists (`Construction_StationaryDefense.sqf:91-93`). That matches the docs-checkout Tactical discovery shape, which scans `units group player` for occupied non-player artillery (`Common_GetTeamArtillery.sqf:10-30`). Non-artillery defenses still use the base area's `DefenseTeam`; unmanned artillery remains unmanned if the CoIn manning toggle is off.
- Stable `origin/master` `cf2a6d6a` and release `a96fdda2` use a different implementation: non-repair-truck artillery defenses set `WFBE_CommanderArtillery*` object variables at `Construction_StationaryDefense.sqf:133-135`, and Tactical discovery scans same-side marked guns inside the HQ/base-area radius at `Common_GetTeamArtillery.sqf:46-78`.
- Miksuu `b8389e74` and `perf/quick-wins` `0076040f` keep the older `DefenseTeam` defense-manning path without either commander-team gunner handoff or marker-based ARTY discovery in the checked maintained roots.

Focused construction scout 2026-06-04 found a small guard-order bug here. Docs checkout `1aa178f8` source Chernarus and maintained Vanilla Takistan now default `_availweapons` to `0` and read `_area getVariable "weapons"` only after `!isNull _area` (`Construction_StationaryDefense.sqf:12-15`). Stable/release still read `_area getVariable "weapons"` before a null guard at `:15`; Miksuu/perf still do so at `:13`. Arma smoke is still needed for defense placement with grouped-base mode on and off, and this does not close broader DR-6 construction authority hardening.
...
| Base-area `avail` / `weapons` reads can happen before null guards. | `coin_interface.sqf:256-263`, `:721-730`; docs checkout `1aa178f8` server-side `Construction_StationaryDefense.sqf:12-15` is patched in source Chernarus and maintained Vanilla Takistan, while stable/release still read before guard at `:15` and Miksuu/perf at `:13`. | Client CoIn placement/sale still need guard-order cleanup; smoke empty, stale and race-pruned base-area lists before closing the whole base-area availability lane. |
...
| --- | --- | --- | --- |
| Current docs/source Chernarus (`docs/developer-wiki-index` `1aa178f8`) | `Skill_Salvage.sqf:38` and `updatesalvage.sqf:50` call lowercase `ChangePlayerfunds`; `Init_Client.sqf:53,91` compiles `ChangePlayerFunds`. | `updatesalvage.sqf:10` uses `while {!gameOver || !(alive _vehicle)}`; both salvage paths delete wrecks locally at `Skill_Salvage.sqf:34` / `updatesalvage.sqf:46`. | Patch-ready, current-source-unpatched. |
| Maintained Vanilla Takistan (`docs/developer-wiki-index` `1aa178f8`) | Same lowercase payout calls and uppercase compile target at the same maintained-root lines. | Same `||` loop and client-local deletion/reward shape. | Needs propagation with Chernarus. |
| Stable `origin/master` (`cf2a6d6a`) | Same shape in Chernarus and maintained Vanilla; `Init_Client.sqf` line drift only (`ChangePlayerFunds` compile at `:53,91`). | Same `||` loop and client-local deletion/reward shape. | Stable still carries the defect. |
| Current `miksuu/master` (`b8389e74`) | Same shape in Chernarus and maintained Vanilla. | Same `||` loop and client-local deletion/reward shape. | Upstream master still carries the defect. |
| `origin/perf/quick-wins` (`0076040f`) and release `origin/release/2026-06-feature-bundle` (`a96fdda2`) | Same shape in Chernarus and maintained Vanilla. | Same `||` loop and client-local deletion/reward shape. | No branch rescue candidate found in these release/fix branches. |
...
| Stationary defense base-area null guard | Docs checkout `1aa178f8` `Construction_StationaryDefense.sqf:12-15` reads base-area `weapons` only after a non-null area check; stable/release still read before guard at `:15`, Miksuu/perf at `:13`. | Source Chernarus and maintained Vanilla Takistan are patched only in the docs checkout; smoke grouped-base defense placement and non-base defense placement before carrying forward. |
| Auto-wall toggle is global | Docs checkout `1aa178f8`, stable `cf2a6d6a`, Miksuu `b8389e74`, perf `0076040f` and release `a96fdda2` all keep `RequestAutoWallConstructinChange.sqf:3-7` writing global `isAutoWallConstructingEnabled`; SmallSite/MediumSite consume it at `:110` / `:125` with only stable/release adding an `AARadar` exclusion. | If auto-wall should be a player/commander preference, key it by side or requester and validate the sender. If global behavior is intentional, label it in UI/docs and smoke two players toggling it from opposite sides. |
...
| Salvage payout and deletion | Salvage payout casing, update-loop and client-local deletion/reward are still identical across docs checkout `1aa178f8`, stable `cf2a6d6a`, Miksuu `b8389e74`, perf `0076040f`, release `a96fdda2` and historical Miksuu salvage branches. | Fix the `ChangePlayerFunds` casing first, propagate Vanilla, and smoke manual/truck salvage; handle server-owned salvage payout/deletion as a separate authority hardening lane. |
