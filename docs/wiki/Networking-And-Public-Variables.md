...
| Current B69 `origin/claude/b69@8d465fce` | Matches current stable's registered PVF init/dispatcher shape in both maintained roots: value-only PVEHs at `Init_PublicVariables.sqf:56,61`, server namespace/CODE dispatch at `Server_HandlePVF.sqf:14-15`, and client namespace/CODE dispatch at `Client_HandlePVF.sqf:32-33`. The B69 `0a1ccb4d..8d465fce` checked PVF-path delta changes only `Client/PVFunctions/HandleSpecial.sqf`, adding default-off `aicom-team-merge` at `:57` with `WFBE_C_AICOM_HC_MERGE_ENABLE` gate at `:59` in both maintained roots. Direct `SEND_MESSAGE` remains unchanged from old B69/provenance refs: `updateclient.sqf:12`, `Client_onEventHandler_SEND_MESSAGE.sqf:27` and `Common_SendMessage.sqf:26,37-38` still carry the direct-channel compile route. For Objective Ping, B69 and adjacent B74 `origin/claude/b74-aicom-spend@b23f557f` match current stable's targeted `SetTask` sends at `GUI_Menu_Command.sqf:336,344` and `Init_PublicVariables.sqf:40` registration. | Treat B69 as branch-only evidence: dispatcher lookup and Objective Ping match current stable, but sender authentication, registered-handler allowlist/logging, Objective Ping smoke and DR-46 `SEND_MESSAGE` direct compile remain open. The new client special tag is a separate AICOM/HC branch feature, not a networking authority closure. |
