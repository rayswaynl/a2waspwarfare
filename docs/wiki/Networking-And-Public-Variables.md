...
| Server-bound payload element `0` names `SRVFNC<Command>`; client-bound element `1` names `CLTFNC<Command>`. | Branch-sensitive trust boundary: docs/perf still dispatch through `Call Compile`; current stable uses namespace+`CODE` lookup but still lacks an explicit registered allowlist. | [PVF dispatch implementation](PVF-Dispatch-Implementation-Playbook) |
