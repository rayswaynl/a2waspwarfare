...
| Current B69 `origin/claude/b69@8d465fce` | Matches current stable's registered PVF init/dispatcher shape in both maintained roots: value-only PVEHs at `Init_PublicVariables.sqf:56,61`, server namespace/CODE dispatch at `Server_HandlePVF.sqf:14-15`, and client namespace/CODE dispatch at `Client_HandlePVF.sqf:32-33`. The B69 `0a1ccb4d..8d465fce` checked PVF-path delta changes only `Client/PVFunctions/HandleSpecial.sqf`, adding default-off `aicom-team-merge` at `:57` with `WFBE_C_AICOM_HC_MERGE_ENABLE` gate at `:59` in both maintained roots. | Treat B69 as branch-only evidence: dispatcher lookup matches current stable, but sender authentication and registered-handler allowlist/logging remain open. The new client special tag is a separate AICOM/HC branch feature, not a networking authority closure. |
