...
2026-06-21 current-stable PVF dispatch refresh: `origin/master@0139a346` Chernarus and maintained Vanilla no longer use dispatch-time `Call Compile` in `Server_HandlePVF.sqf` or `Client_HandlePVF.sqf`. They resolve `_code = missionNamespace getVariable _script` and spawn only `CODE` at server `:14-15` and client `:32-33`. Chernarus blame is `7d60b02b4`; maintained Vanilla propagation blame is `9b49883cb`. `Init_PublicVariables.sqf:55-61` still precompiles and registers handlers without a registered-handler allowlist or warning log, so this is a partial dispatcher hardening, not full PVF/network authority closure.
