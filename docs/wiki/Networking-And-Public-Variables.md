...
2026-06-23 `SEND_MESSAGE` addendum: docs/source `HEAD@3a32be92` is unchanged from `40c477be` / `16247fc8f` for checked `SEND_MESSAGE` paths, and current B69 `origin/claude/b69@8d465fce` plus adjacent B74 `origin/claude/b74-aicom-spend@b23f557f` match current stable for this direct channel. Both maintained roots still register `SEND_MESSAGE` at `Client/FSM/updateclient.sqf:12`, compile receiver text at `Client_onEventHandler_SEND_MESSAGE.sqf:27`, and compile/broadcast helper text at `Common_SendMessage.sqf:26,37-38`; checked B69 diffs from `0a1ccb4d` and `b8530477` to current head, B69..B74, and master..B74 are empty for those paths.
