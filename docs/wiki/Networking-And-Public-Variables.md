...
Branch check 2026-06-05: current source/Vanilla, stable `origin/master`, Miksuu upstream, `origin/perf/quick-wins` and release `7195b331` all still use `Spawn (Call Compile _script)` in both generic dispatchers. Release shifts the client line to `Client_HandlePVF.sqf:32` because it adds headless-client filtering first, but it does not add allowlisted namespace lookup. The exact matrix and patch shape live in [PVF dispatch implementation](PVF-Dispatch-Implementation-Playbook#current-branch-matrix).
