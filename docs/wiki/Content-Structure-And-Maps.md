...
`version.sqf` is required terrain metadata and is generated by LoadoutManager for terrain outputs. The current Chernarus source mission tracks a copy, but generated Vanilla/modded/stub roots still need explicit verification before direct pack/test work; see [Tools and build workflow](Tools-And-Build-Workflow) and DR-43a.
