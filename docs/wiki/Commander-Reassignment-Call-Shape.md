...
| Current stable `origin/master@f8a76de34` / B74.1 Chernarus and maintained Vanilla | Helper unpacking is fixed: `_side = _this select 0`, `_commander = _this select 1` (`Server_AssignNewCommander.sqf:4-5`). This reassignment path is unchanged from `0139a3468609` to `f8a76de34`. | UI still selects by leader-name text, and both `RequestNewCommander.sqf:14` plus helper `:10` still send `new-commander-assigned`. | Current stable/B74.1 is source-present for DR-15 helper shape, but not release-complete for reassignment. |
