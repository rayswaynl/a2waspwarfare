...
Status: branch-split. Current stable `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`, B74.2 `origin/claude/b74.2-aicom@21b62b04`, current B69 `origin/claude/b69@8d465fce` and adjacent B74 `origin/claude/b74-aicom-spend@b23f557f` have the DR-15 helper unpacking fix in the maintained Chernarus and Vanilla roots, but docs/source `HEAD@60c35c05` and current stable/B74-shaped full modded forks still carry the old helper shape. Duplicate reassignment notifications, visible-name UI targeting, requester authority and smoke remain open.
...
Current stable `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`, B74.2 `origin/claude/b74.2-aicom@21b62b04`, current B69 `origin/claude/b69@8d465fce` and adjacent B74 `origin/claude/b74-aicom-spend@b23f557f` fix the maintained-root helper unpacking:
...
Branch route `commander-vote-b742-current-head-followup-2026-06-23` rechecked maintained Chernarus and Vanilla roots after `git fetch --all --prune` on 2026-06-23. Docs/source `HEAD@60c35c05` is unchanged from `337ed166` and `b44aaaf8` for the checked reassignment paths and still has the old helper unpacking. Current stable is `origin/master@f8a76de34`, the same commit as `origin/claude/b74.1-aicom@f8a76de34`; current B74.2 is `origin/claude/b74.2-aicom@21b62b04`. The scoped `0139a3468609..origin/master`, `origin/master..origin/claude/b74.2-aicom`, `d472da6a..origin/claude/b74.2-aicom` and `origin/claude/b69..origin/claude/b74-aicom-spend` diffs touch no maintained-root `RequestNewCommander.sqf`, `Server_AssignNewCommander.sqf` or `GUI_Commander_VoteMenu.sqf` files. Current B69 is `origin/claude/b69@8d465fce`; adjacent B74 is `origin/claude/b74-aicom-spend@b23f557f`. Older-B69-to-B74 reassignment path deltas are also empty. Current origin exposes no live `release/*` or reassignment rescue heads; `origin/claude/trello-vote-para-guards@92101abe` changes only `Server_VoteForCommander.sqf` among commander vote/reassignment files and is branch-only tie-break context, so `a96fdda2` and `c20ce153` remain historical evidence unless those refs are restored.
...
| Current stable `origin/master@f8a76de34` / B74.1 / B74.2 `origin/claude/b74.2-aicom@21b62b04` Chernarus and maintained Vanilla | Helper unpacking is fixed: `_side = _this select 0`, `_commander = _this select 1` (`Server_AssignNewCommander.sqf:4-5`). This reassignment path is unchanged from `0139a3468609` to `f8a76de34`, current-stable to B74.2 and previous B74.2 `d472da6a` to current B74.2. | UI still selects by leader-name text, and both `RequestNewCommander.sqf:14` plus helper `:10` still send `new-commander-assigned`. | Current stable/B74.1/B74.2 is source-present for DR-15 helper shape, but not release-complete for reassignment. |
