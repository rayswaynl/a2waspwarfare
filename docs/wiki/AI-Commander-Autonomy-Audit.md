...
Checked 2026-06-13 against docs checkout `6d05cb5a`, stable `origin/master` `cf2a6d6a`, Miksuu `b8389e74`, `origin/perf/quick-wins` `0076040f` and release `origin/release/2026-06-feature-bundle` `a96fdda2` in maintained Chernarus and Vanilla roots. The player upgrade menu treats upgrade cost tuples as `[supply, funds]`: docs checkout Chernarus reads supply/funds at `Client/GUI/GUI_UpgradeMenu.sqf:96-97` and debits client funds/side supply at `:158-159`; stable line drift is `:233-234,252-253`; release line drift is `:219,238-239`; Miksuu/perf line drift is `:206,226`. The AI worker validates with the same convention at `Server_AI_Com_Upgrade.sqf:34-36`.

| Branch / root | Evidence | Status |
| --- | --- | --- |
| Docs checkout `6d05cb5a` Chernarus and maintained Vanilla | Both roots read `_cost` with raw `(_to_upgrade select 1)` at `Server_AI_Com_Upgrade.sqf:27`, validate side supply against `_cost select 0` and AI funds against `_cost select 1` at `:34-36`, then debit AI funds with `_cost select 0` and side supply with `_cost select 1` at `:47,50`. | Patch-ready in the docs/source checkout. AI upgrade revival should align debit order and review whether the AI_ORDER level should be converted to a zero-based cost index before enabling a scheduler. |
