...
2026-06-22 current-head refresh: `origin/claude/b69@39eed5c0` is a branch-only B69 series on top of B68 head `b8a1505f`, now including merged PR [#47](https://github.com/rayswaynl/a2waspwarfare/pull/47), PR [#48](https://github.com/rayswaynl/a2waspwarfare/pull/48) and PR [#49](https://github.com/rayswaynl/a2waspwarfare/pull/49). The full isolated diff `b8a1505f..39eed5c0` is 22 files / +699 / -91: it adds `B69-IMPLEMENTATION-PLAN.md` and changes only current Chernarus mission files. The previous B69 merge head `4dcc10b143a0` already carried PR #48; PR #49 merged `claude/b72-tints-on@7e1026ea` as the second parent at `39eed5c02d8ba9c5a27b7a4173607526edc0677e` on 2026-06-22T11:47:23Z. `git diff --name-status b8a1505f..origin/claude/b69 -- Missions_Vanilla/[61-2hc]warfarev2_073v48co.takistan` is empty, and `gh pr list --head claude/b69 --state all` returned `[]` on 2026-06-22, so this remains branch evidence, not a master-target PR route.
...
| Branch piece | Evidence | Meaning |
| --- | --- | --- |
| Patch A/A-2 core carried forward at `39eed5c` | Constants for the fractional gate, vehicle-punch bonus and capture interrupt are in Chernarus `Common/Init/Init_CommonConstants.sqf:229-232`. Strategy computes `_strikeMinTowns` at `AI_Commander_Strategy.sqf:530-539`, logs launch gate/total at `:547`, scores vehicle punch at `:561-565` and sends HC strikers a `"defense"` order at `:576`. Capture interrupt snapshots the order sequence at `Common_RunCommanderTeam.sqf:709-712`, aborts camp/depot loops at `:810` and `:895`, clears sticky `doStop` with `doFollow` at `:867` and bails before `_captureDone` at `:916`. | HQ-strike and capture-interrupt branch code is source-present in Chernarus only. It still needs review, live HQ-strike/capture smoke, maintained Vanilla propagation or explicit Chernarus-only scope, and master-target PR routing before release wording. |
...
| B69 finalpieces town-punch and armed-transport addendum | `origin/claude/b69@39eed5c0` carries the former finalpieces Chernarus constants `WFBE_C_AICOM_TOWNPUNCH_HEAVY_MULT`, `WFBE_C_AICOM_TOWNPUNCH_LIGHT_MULT` and `WFBE_C_AICOM_ARMED_TRANSPORT_ONLY` at `Init_CommonConstants.sqf:389-390,404`. Type assignment and HC-resident team paths read the punch multipliers at `AI_Commander_AssignTypes.sqf:166-167` and `AI_Commander_Teams.sqf:330,335-336`; `Common_RunCommanderTeam.sqf:416` filters the cargo ride pool to armed hulls by default while the air path is unchanged. | Chernarus-only branch tuning/filter evidence now merged into B69 by PR #47. Smoke troop-transport behavior, team composition pressure and fallback cases before release wording; maintained Vanilla propagation is absent. |
| B71 harvest / PR #48 addendum | PR #48 merged `claude/b71-pr-harvest` into `claude/b69` at `4dcc10b143a0`. The merge delta `0094647d7b64..4dcc10b143a0` is 6 Chernarus files / +40 / -13. Source anchors: map-gated town markers and SpecOps exception at `updatetownmarkers.sqf:11,14-16`; terrain-grid profile fallback at `Init_Client.sqf:416-440`; faction tint side resolution at `Common_CreateVehicle.sqf:37-41` and `Common_AddVehicleTexture.sqf:1,7,262,271`; water-reject cap at `Common_GetRandomPosition.sqf:30,33-34`. | Branch-only B71 harvest now carried by B69. Smoke map-open/SpecOps supply cue behavior, profile terrain-grid persistence/default and water-heavy random-position callers before release wording. No maintained Vanilla propagation is present. |
| B72 tints-on / PR #49 addendum | PR #49 merged `claude/b72-tints-on` into `claude/b69` at `39eed5c0`. The merge delta `4dcc10b143a0..39eed5c0` is one Chernarus file / +1 / -1. Source anchor: `Common/Init/Init_CommonConstants.sqf:795` now nil-guards `WFBE_C_VEHICLE_TINTS = 1` while the long comment still warns about the first real in-engine repaint and possible revert to `0`. | Branch-only cosmetic default flip. Smoke vehicle spawn appearance across WEST/EAST/GUER/civilian hulls before release wording; maintained Vanilla propagation is absent. |
...
| No maintained Vanilla propagation for B69/finalpieces/B71/B72 yet | `git diff --name-status b8a1505f..origin/claude/b69 -- Missions_Vanilla/[61-2hc]warfarev2_073v48co.takistan`, `git diff --name-status 0094647d7b64..origin/claude/b69 -- Missions_Vanilla/[61-2hc]warfarev2_073v48co.takistan` and `git diff --name-status 4dcc10b143a0..origin/claude/b69 -- Missions_Vanilla/[61-2hc]warfarev2_073v48co.takistan` are empty. PR #47, PR #48 and PR #49 are merged into `claude/b69`, but the GitHub CLI still found no direct PR for `claude/b69` on 2026-06-22. | Before release wording, propagate or deliberately scope maintained Vanilla, inspect generated diffs and record Arma smoke. |

| Page | Use it for | Caveat |
| --- | --- | --- |
| [AI Commander B69 improvement roadmap](AI-Commander-B69-Improvement-Roadmap) | Review the 15-item recommended slate, the atomic HQ-strike package, HC-team merge/refill direction, supervisor heartbeat/watchdog route, garrison-vs-maneuver strength split and worker-stagger/FPS leads. | The page states it was produced on 2026-06-22 from a live B68 Chernarus audit, and now carries the `origin/claude/b69@39eed5c0` current-branch status after PR #47, PR #48 and PR #49 merged into B69. Before code work, re-check every touched path in the target branch and maintained Vanilla scope. |
| [AI Commander B69 implementation sketches](AI-Commander-B69-Implementation-Sketches) | Use as an engineer handoff for A2-OA-safe SQF patch shapes and soak-test expectations for the verified B69 items. | Several sketches now have Chernarus branch code at `39eed5c0`; remaining sketches and prep hooks still need line-anchor, branch-drift, generated-target and Arma smoke verification before being treated as implemented. |
...
1. `UpdateSupplyTruck = Compile preprocessFile "Server\AI\AI_UpdateSupplyTruck.sqf";` is still commented out. The docs branch/Miksuu/perf line is `Server/Init/Init_Server.sqf:36`; current stable `origin/master@0139a346` line drift is `:43`.
2. The truck-supply + AI-commander branch still exists. The docs branch checkout `ea0e0f1b` still initializes `wfbe_ai_supplytrucks` and raw-spawns `UpdateSupplyTruck` at `Init_Server.sqf:382-383`; current stable `origin/master@0139a346` does not spawn it, and instead initializes/log-disables legacy AI supply-truck logistics at `:462-463` in both maintained roots.
...
| Branch / root | Evidence | Status |
| --- | --- | --- |
| Docs branch checkout `docs/developer-wiki-index@ea0e0f1b` | Both maintained roots still comment out `UpdateSupplyTruck` at `Server/Init/Init_Server.sqf:36`, initialize `wfbe_ai_supplytrucks` at `:382`, and raw-spawn `UpdateSupplyTruck` at `:383` when truck supply and AI commanders are enabled. `Server/AI/AI_UpdateSupplyTruck.sqf:17` still references missing `Server\FSM\supplytruck.fsm`; `ls-tree` found no `supplytruck.fsm`. | The repo docs/source checkout remains old-shaped for this path. Do not use it as proof of current stable safety. |
| Current stable `origin/master@0139a346` | Both maintained roots still comment out `UpdateSupplyTruck` at `Server/Init/Init_Server.sqf:43`, then initialize `wfbe_ai_supplytrucks` and log-disable legacy AI supply-truck logistics at `:462-463` when truck supply and AI commanders are enabled. `Server/AI/AI_UpdateSupplyTruck.sqf:17` still references missing `Server\FSM\supplytruck.fsm`, but current stable does not call it from this branch; `ls-tree` found no `supplytruck.fsm`. | Safety shape is present in current stable; autonomous logistics remain disabled/not revived. |
