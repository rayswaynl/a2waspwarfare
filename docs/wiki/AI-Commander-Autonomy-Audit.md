...
2026-06-24 B74.5/B74.7.1 intake: current `origin/claude/b745@29ca9519bf1bd17ed1ef3f52a689fb2e8574db69` contains `origin/claude/b74.2-aicom@21b62b04` as an ancestor but is a broader branch-only bundle, not a B74.2 replacement or stable release claim. Its included B74.4 AICOM/JIP batch (`4d16fad70`) should be split by owner: primitive roster/vote/JIP now routes through [Client UI systems](Client-UI-Systems-Atlas#current-branch-scope), [Player join/disconnect](Player-Join-Disconnect-And-AntiStack-Lifecycle#branch-intel---b745b747-jip-enrollment) and [Commander vote/reassignment](Commander-Vote-And-Reassignment-Playbook#current-branch-scope). The B74.6 and B74.7 deltas are empty for the checked AICOM attack-heli, base-sell and MHQ relocation paths. The B74.7.1 top commit adds a separate AI-commander founding hard cap in source Chernarus only; the remaining AICOM helper evidence is below.

| B74.5 / B74.7.1 helper | Source evidence included in current `origin/claude/b745@29ca9519` | Scope and smoke before promotion |
| --- | --- | --- |
| B74.7.1 founding hard cap | Source Chernarus adds `WFBE_C_AICOM_TEAMS_HARD_CAP = 8` at `Common/Init/Init_CommonConstants.sqf:159`; `AI_Commander_Teams.sqf:113-114` reads the cap and clamps `_target` after the pop-curve plus low-pop banking-valve math. The `1cc995246..29ca9519` delta is two source-Chernarus files / +6 and has no maintained Vanilla, Modded or Tools payload. Checked current stable, current B74.2, B69, adjacent B74, Miksuu, perf, previous B74.6 and previous B74.7 refs have no `WFBE_C_AICOM_TEAMS_HARD_CAP` symbol. | Branch-only source-Chernarus performance/load-control retune. Smoke low-pop and banked-funds conditions that previously founded about 15 teams, verify no side exceeds eight AICOM teams, and confirm the cap does not starve high-pop or no-funds founding. Treat the branch author's in-place deploy/boot-smoke note as branch-author evidence until target package and server RPT are recorded. |
