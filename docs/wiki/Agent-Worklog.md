...
- Initial branch shape: merge-base with current stable `origin/master` is `f8a76de349da`; initial file scope is source Chernarus plus maintained Vanilla `Client_AmbulanceRedeployCircles.sqf`, `Init_BaseStructure.sqf` and `Init_Client.sqf`. No gameplay source edits planned.
- Planned source scope: compare branch against current stable, inspect HQ build-area and ambulance/redeploy map circle creation/removal in both maintained roots, then route exact branch-only evidence through existing UI, Commander/HQ, respawn/medic pages, Feature Status, Testing workflow and release-readiness gates.
- Result: routed the branch as **branch-only map-indicator work, not current stable behavior**. The branch payload is six maintained-root files / +192 and `git diff --check f8a76de34..77dd71ba303f` is clean.
- HQ evidence: branch `Init_BaseStructure.sqf` creates a local `HQBuildRange*` border ellipse using `WFBE_C_BASE_AREA_RANGE + WFBE_C_BASE_HQ_BUILD_RANGE` at Chernarus `:54-65` / Vanilla `:35-46`, then deletes it with the CC marker at Chernarus `:153-158` / Vanilla `:134-139`. Current stable only has `CCrange*` at Chernarus `:47-51` / Vanilla `:28-32`.
- Mobile-respawn evidence: branch `Init_Client.sqf:1227` launches `Client_AmbulanceRedeployCircles.sqf`; the watcher gates on `WFBE_C_RESPAWN_MOBILE`, combines own-side ambulance plus redeploy-truck class lists, sizes yellow local ellipses from `WFBE_C_RESPAWN_RANGES` and updates/deletes them every five seconds (`:12-18,30-35,41-48,57-68`). Current stable has no `Client_AmbulanceRedeployCircles`, `HQBuildRange` or `AmbRange_` hits.
...
- Updated owner pages, Feature Status, branch-only smoke pack, release-readiness gates, dashboard, pruning ledger and machine rows. Final mirror/validation results are recorded in the completion event.
