...
Checked 2026-06-24 for the upgrade-sync route against docs/source `HEAD@373e21c1274c` unchanged from `a7f383b6`, current stable/B74.1 `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`, current B74.2 `origin/claude/b74.2-aicom@21b62b04`, current B69 `origin/claude/b69@8d465fce`, adjacent B74 `origin/claude/b74-aicom-spend@b23f557f`, Miksuu upstream `b8389e748243`, `origin/perf/quick-wins@0076040f`, historical release commit `a96fdda2` and historical upgrade-queue commit `b061c905`. Targeted diffs show no checked `upgrade-sync` parser rescue: `a7f383b6..HEAD`, `d472da6a..origin/claude/b74.2-aicom`, `origin/master..origin/claude/b74.2-aicom` and `origin/claude/b69..origin/claude/b74-aicom-spend` are empty for the maintained-root handler/caller/process paths. Current stable/B74.1/B74.2 do add the server end-time lifecycle at `Server_ProcessUpgrade.sqf:27,55` plus client reads at `GUI_UpgradeMenu.sqf:82,86`; parser line drift and cleanup status route through [Support specials](Support-Specials-And-Tactical-Modules-Atlas#upgrade-sync-branch-matrix).
...
| Docs/source `HEAD@c70785e4be30` Chernarus and maintained Vanilla | Player UI still treats costs as `[supply, funds]` (`GUI_UpgradeMenu.sqf:96-97,139-140`), debits side supply locally at `:159`, then sends `RequestUpgrade` at `:161`. `RequestUpgrade.sqf:5` still spawns `WFBE_SE_FNC_ProcessUpgrade`, and `Server_ProcessUpgrade.sqf:12-18` trusts side/id/level/player flag. The AI upgrade worker remains old-shaped at `Server_AI_Com_Upgrade.sqf:27,34,36,47,50`; checked worker/menu diffs `031a253fe..HEAD` and `ade4d356..HEAD` are empty. | Current docs/source remains the player-upgrade authority target. AI debit/cost lookup, `upgrade-sync` and stale dialog branch detail route to the owner pages linked below. |
| Current stable/B74.1 `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`; current B74.2 `origin/claude/b74.2-aicom@21b62b04`; current B69/B74 | The checked current-stable/B74.1 maintained roots keep the player-upgrade server entry shape: the UI sends `RequestUpgrade` at `GUI_UpgradeMenu.sqf:279`, `RequestUpgrade.sqf:5` spawns `WFBE_SE_FNC_ProcessUpgrade`, and `Server_ProcessUpgrade.sqf:12-14` reads side/id/level from the payload. They also publish/clear `wfbe_upgrading_end_time` at `Server_ProcessUpgrade.sqf:27,55` and read it in `GUI_UpgradeMenu.sqf:82,86`. Current B74.2 has no checked handler/caller/process path delta versus current stable or previous B74.2 `d472da6a`. B69/B74 path deltas stay empty for checked `upgrade-sync` files. None of these close player upgrade authority or `upgrade-sync` tuple cleanup. | Use [AI Commander Autonomy Audit](AI-Commander-Autonomy-Audit#ai-upgrade-debit-branch-matrix), [Support specials](Support-Specials-And-Tactical-Modules-Atlas#upgrade-sync-branch-matrix) and [UI Resource Parity Cleanup](UI-Resource-Parity-Cleanup) for exact branch proof. |
...
Branch scope matters here, but [AI Commander Autonomy Audit](AI-Commander-Autonomy-Audit#ai-upgrade-debit-branch-matrix) owns the canonical matrix. The important upgrade-atlas takeaway is that docs/source `HEAD@c70785e4be30`, Miksuu `b8389e748243` and `perf/quick-wins` `0076040f` keep the old worker shape: raw AI-order cost lookup at `Server_AI_Com_Upgrade.sqf:27`, `[supply, funds]` validation at `:34,36`, and swapped debit at `:47,50`. Current stable/B74.1 `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`, current B74.2 `origin/claude/b74.2-aicom@21b62b04`, B69 and adjacent B74 fix both maintained roots with current-level cost lookup at `:75`, affordability gates at `:89,:92,:97`, AI funds debit at `:125`, fallback surcharge at `:131-132` and side-supply debit at `:136`. Checked `d472da6a..21b62b04` is empty for the maintained-root worker/menu paths. Live side branches are mixed, so recheck the target branch before porting. Before enabling or scheduling AI commander upgrades on old-shape targets, port current stable's worker or recreate the current-level cost lookup plus debit fix, then smoke affordability/debit and funds-fallback.
...
| Current-stable resolved / old-target patch-ready | Current stable/B74.1/B74.2/B69/B74 fix AI commander upgrade current-level cost lookup and funds/supply debit in both maintained roots, but docs/source, Miksuu and perf still keep raw AI-order lookup plus swapped debit. Historical `a96fdda2`, `b061c905` and `c20ce153` plus mixed live side branches are lessons unless selected as the target branch. | [AI upgrade debit matrix](AI-Commander-Autonomy-Audit#ai-upgrade-debit-branch-matrix); docs/source `HEAD@c70785e4be30` old shape at `Server_AI_Com_Upgrade.sqf:27,34,36,47,50`; current stable/B74.1/B74.2 resolved shape at `:75,89,92,97,125,131-132,136` | Do not reopen on current stable/B74-shaped refs. Port current stable's worker to old-shape targets that still matter, then smoke affordability/debit plus funds-fallback before scheduling AI upgrades. |
