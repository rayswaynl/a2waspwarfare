...
| Client timer sync | Docs/source `HEAD@bc21f520` has the same source shape as the older docs anchors at `Client/GUI/GUI_UpgradeMenu.sqf:167-172` and `Server/Functions/Server_HandleSpecial.sqf:67-74`; current stable/B69 line drift is `GUI_UpgradeMenu.sqf:292`, `Server_HandleSpecial.sqf:67-73` and `Server_ProcessUpgrade.sqf:32,35,41`; canonical matrix: [Support specials](Support-Specials-And-Tactical-Modules-Atlas#upgrade-sync-branch-matrix) |
