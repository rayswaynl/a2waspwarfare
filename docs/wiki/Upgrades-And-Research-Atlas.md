...
Checked 2026-06-23 for the upgrade-sync route against docs/source `HEAD@a7f383b6`, current stable/B74.1 `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`, visible B74.2 `origin/claude/b74.2-aicom@d472da6a`, current B69 `origin/claude/b69@8d465fce`, adjacent B74 `origin/claude/b74-aicom-spend@b23f557f`, Miksuu upstream `b8389e748243`, `origin/perf/quick-wins@0076040f` and historical release commit `a96fdda2`. Targeted diffs show no checked `upgrade-sync` parser rescue: `origin/master..origin/claude/b74.2-aicom` and `origin/claude/b69..origin/claude/b74-aicom-spend` are empty for the maintained-root handler/caller/process paths. Current stable/B74.1/B74.2 do add the server end-time lifecycle at `Server_ProcessUpgrade.sqf:27,55` plus client reads at `GUI_UpgradeMenu.sqf:82,86`; parser line drift and cleanup status route through [Support specials](Support-Specials-And-Tactical-Modules-Atlas#upgrade-sync-branch-matrix). Historical `b061c905` and `c20ce153` remain prior evidence until live heads are restored or rechecked.

The branch-only Barracks and Trello upgrade/HQ cost/icon rows below were checked later on 2026-06-23 against current stable `origin/master@f8a76de34`; they are separate from the `upgrade-sync` branch-scope matrix in this section.

| Branch / root | Evidence | Status |
| --- | --- | --- |
| Branch-only `origin/claude/trello-upgrade-hqcost-info@34fd202c8865` / draft PR #75 | One commit on current stable merge-base `origin/master@f8a76de34`; payload is six maintained-root files / +96 and `git diff --check` clean. In source Chernarus and maintained Vanilla, `Client_FNC_Special.sqf:204` keeps the existing `STR_WF_CHAT_Upgrade_Complete_Message`, then `:212-223` adds a branch-local `switch` that formats only artillery reload (`WFBE_UP_ARTYTIMEOUT`) from `WFBE_C_ARTILLERY_INTERVALS` and respawn range (`WFBE_UP_RESPAWNRANGE`) from `WFBE_C_RESPAWN_RANGES`. `stringtable.xml:459-473` adds the value-line keys in both roots. Current stable already defines the upgrade IDs at `Init_CommonConstants.sqf:44,47`, uses the same arrays in `Labels_Upgrades.sqf:34,46`, Tactical artillery code and respawn availability, but has no branch value-message keys or extra scalar upgrade-complete chat. | Branch-only info-text candidate, not current stable behavior. Smoke artillery-timeout and respawn-range upgrade completion for correct level/index values, confirm other upgrade types still emit only the generic completion message, and repeat source Chernarus plus maintained Vanilla before stable/release wording. |
| Branch-only `origin/claude/trello-upgrade-icons@baf8d9c304ad` / draft PR #77 | One commit on current stable merge-base `origin/master@f8a76de34`; payload is two maintained-root `Client/GUI/GUI_UpgradeMenu.sqf` files / +8 and `git diff --check` clean. In source Chernarus and maintained Vanilla, `GUI_UpgradeMenu.sqf:12` reads `WFBE_C_UPGRADES_IMAGES`, `:27-28` paints non-empty image paths into listbox `504001` column `0` during initial list fill, and `:173-174` repaints during refresh/reordering. The image array is already defined by `Labels_Upgrades.sqf:104-125`, including several intentionally empty strings for graceful no-icon rows. Current stable uses the selected upgrade image for detail control `504002`, but does not paint row icons in the upgrade list. | Branch-only visual/UI candidate, not current stable behavior and not an upgrade config change. Smoke initial list icon rendering, rows with empty image strings, queue/level refresh repaint, selection/detail-pane behavior, missing-texture RPT cleanliness and source Chernarus plus maintained Vanilla before stable/release wording. |
...
| Docs/source `HEAD@031a253fe` Chernarus and maintained Vanilla | Player UI still treats costs as `[supply, funds]` (`GUI_UpgradeMenu.sqf:96-97,139-140`), debits side supply locally at `:159`, then sends `RequestUpgrade` at `:161`. `RequestUpgrade.sqf:5` still spawns `WFBE_SE_FNC_ProcessUpgrade`, and `Server_ProcessUpgrade.sqf:12-18` trusts side/id/level/player flag. The AI upgrade worker remains old-shaped at `Server_AI_Com_Upgrade.sqf:27,34,36,47,50`; checked worker diff `ade4d356..HEAD` is empty. | Current docs/source remains the player-upgrade authority target. AI debit/cost lookup, `upgrade-sync` and stale dialog branch detail route to the owner pages linked below. |
| Current stable/B74.1 `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`; visible B74.2 `origin/claude/b74.2-aicom@d472da6a`; current B69/B74 | The checked current-stable/B74.1 maintained roots keep the player-upgrade server entry shape: the UI sends `RequestUpgrade` at `GUI_UpgradeMenu.sqf:279`, `RequestUpgrade.sqf:5` spawns `WFBE_SE_FNC_ProcessUpgrade`, and `Server_ProcessUpgrade.sqf:12-14` reads side/id/level from the payload. They also publish/clear `wfbe_upgrading_end_time` at `Server_ProcessUpgrade.sqf:27,55` and read it in `GUI_UpgradeMenu.sqf:82,86`. B74.2 has no checked handler/caller/process path delta versus current stable. B69/B74 path deltas stay empty for checked `upgrade-sync` files. None of these close player upgrade authority or `upgrade-sync` tuple cleanup. | Use [AI Commander Autonomy Audit](AI-Commander-Autonomy-Audit#ai-upgrade-debit-branch-matrix), [Support specials](Support-Specials-And-Tactical-Modules-Atlas#upgrade-sync-branch-matrix) and [UI Resource Parity Cleanup](UI-Resource-Parity-Cleanup) for exact branch proof. |
...
| Client timer sync | Docs/source `HEAD@a7f383b6` uses `Client/GUI/GUI_UpgradeMenu.sqf:171`, `Server/Functions/Server_HandleSpecial.sqf:67-73` and `Server_ProcessUpgrade.sqf:26,29,35`; current stable/B74.1/B74.2 line drift is `GUI_UpgradeMenu.sqf:292`, source Chernarus `Server_HandleSpecial.sqf:77-83`, maintained Vanilla `:67-73`, `Server_ProcessUpgrade.sqf:32,35,41`, plus server end-time `:27,:55`; canonical matrix: [Support specials](Support-Specials-And-Tactical-Modules-Atlas#upgrade-sync-branch-matrix) |
...
Branch scope matters here, but [AI Commander Autonomy Audit](AI-Commander-Autonomy-Audit#ai-upgrade-debit-branch-matrix) owns the canonical matrix. The important upgrade-atlas takeaway is that docs/source `HEAD@031a253fe`, Miksuu `b8389e748243` and `perf/quick-wins` `0076040f` keep the old worker shape: raw AI-order cost lookup at `Server_AI_Com_Upgrade.sqf:27`, `[supply, funds]` validation at `:34,36`, and swapped debit at `:47,50`. Current stable/B74.1 `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`, B74.2 `origin/claude/b74.2-aicom@d472da6a`, B69 and adjacent B74 fix both maintained roots with current-level cost lookup at `:75`, affordability gates at `:89,:92,:97`, AI funds debit at `:125`, fallback surcharge at `:131-132` and side-supply debit at `:136`. Live side branches are mixed, so recheck the target branch before porting. Before enabling or scheduling AI commander upgrades on old-shape targets, port current stable's worker or recreate the current-level cost lookup plus debit fix, then smoke affordability/debit and funds-fallback.
...
| Current-stable resolved / old-target patch-ready | Current stable/B74.1/B74.2/B69/B74 fix AI commander upgrade current-level cost lookup and funds/supply debit in both maintained roots, but docs/source, Miksuu and perf still keep raw AI-order lookup plus swapped debit. Historical `a96fdda2`, `b061c905` and `c20ce153` plus mixed live side branches are lessons unless selected as the target branch. | [AI upgrade debit matrix](AI-Commander-Autonomy-Audit#ai-upgrade-debit-branch-matrix); docs/source `HEAD@031a253fe` old shape at `Server_AI_Com_Upgrade.sqf:27,34,36,47,50`; current stable/B74.1 resolved shape at `:75,89,92,97,125,131-132,136` | Do not reopen on current stable/B74-shaped refs. Port current stable's worker to old-shape targets that still matter, then smoke affordability/debit plus funds-fallback before scheduling AI upgrades. |
