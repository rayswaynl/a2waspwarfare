...
Checked 2026-06-21 for the upgrade-sync route against docs-source base `docs/developer-wiki-index@ad05401a3`, current stable `origin/master@0139a346`, Miksuu upstream `b8389e74`, `origin/perf/quick-wins@0076040f`, historical release commit `a96fdda2`, historical `origin/feat/ai-commander` commit `c20ce153` and historical upgrade-queue commit `b061c905`. Targeted `git diff --name-only d4cfef80..ad05401a3` checks over the upgrade UI, server upgrade worker, `RequestUpgrade`, `Server_HandleSpecial`, upgrade constants/config and `Rsc/Dialogs.hpp` paths in Chernarus plus maintained Vanilla returned no source changes. The `d4cfef80` line anchors below therefore remain valid provenance for docs/source while current-stable line drift routes through [Support specials](Support-Specials-And-Tactical-Modules-Atlas#upgrade-sync-branch-matrix).
...
| Current stable `origin/master@0139a346` | Both maintained roots keep the player-upgrade server entry shape: the UI sends `RequestUpgrade` at `GUI_UpgradeMenu.sqf:279`, `RequestUpgrade.sqf:5` spawns `WFBE_SE_FNC_ProcessUpgrade`, and `Server_ProcessUpgrade.sqf:12-15` reads side/id/level/player flag from the payload. They fix the AI debit order and remove the stale old upgrade dialog, but still need smoke and do not close player upgrade authority, `upgrade-sync` tuple cleanup or the AI cost-index question. | Use [AI Commander Autonomy Audit](AI-Commander-Autonomy-Audit#ai-upgrade-debit-branch-matrix), [Support specials](Support-Specials-And-Tactical-Modules-Atlas#upgrade-sync-branch-matrix) and [UI Resource Parity Cleanup](UI-Resource-Parity-Cleanup) for exact branch proof. |
...
| Client timer sync | Docs checkout `1b07716d` has the same source shape as `d4cfef80` at `Client/GUI/GUI_UpgradeMenu.sqf:167-172` and `Server/Functions/Server_HandleSpecial.sqf:67-74`; current stable line drift is `GUI_UpgradeMenu.sqf:292`, `Server_HandleSpecial.sqf:67-73` and `Server_ProcessUpgrade.sqf:32,35,41`; canonical matrix: [Support specials](Support-Specials-And-Tactical-Modules-Atlas#upgrade-sync-branch-matrix) |
