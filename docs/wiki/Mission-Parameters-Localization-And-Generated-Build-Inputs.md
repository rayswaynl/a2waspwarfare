...
Visible-parameter runtime-consumer caveat, refreshed 2026-06-22: this is now branch-sensitive. Docs branch `origin/docs/developer-wiki-index@ac932fbe`, `origin/perf/quick-wins@0076040f` and historical release commit `a96fdda2` still expose/default `WFBE_C_AI_MAX` without an active maintained-root runtime reader found, and still mention `WFBE_C_UNITS_CLEAN_TIMEOUT` only in the commented old cleanup split at `Common_TrashObject.sqf:20`. Current stable `origin/master@0139a346` and current Miksuu `master@d9506078` are different: both maintained roots read `WFBE_C_AI_MAX` in `Server/AI/Commander/AI_Commander_Produce.sqf:89` as an AI commander production/template cap, and both use `WFBE_C_UNITS_CLEAN_TIMEOUT` in `Common_TrashObject.sqf:21` for non-man wreck cleanup while man bodies use fixed `WFBE_C_UNITS_BODIES_TIMEOUT`. Empty vehicles remain separate through `WFBE_C_UNITS_EMPTY_TIMEOUT` in `Server_HandleEmptyVehicle.sqf:20,26` on current stable and `:12,18` on old-shape docs/perf/release refs. Player follower limits are still `WFBE_C_PLAYERS_AI_MAX`, read by the buy menu/RHUD and modified by Soldier skill init; do not answer player follower-cap questions with `WFBE_C_AI_MAX`.
...
| Branch-sensitive AI/cleanup parameters | For old-shape docs/perf/historical-release targets, decide whether to port the current stable/Miksuu AI commander reader and body/wreck split, or hide/label the stale rows. For current stable/Miksuu, document `WFBE_C_AI_MAX` as AI commander production sizing and `WFBE_C_UNITS_CLEAN_TIMEOUT` as non-man wreck cleanup, not player follower cap or corpse timeout. | Host/admin parameter wording, AI commander production, player AI cap UI/RHUD, corpse cleanup, wreck cleanup and empty-vehicle cleanup all need separate smoke before release wording. |
