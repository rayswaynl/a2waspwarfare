...
Visible-parameter runtime-consumer caveat, refreshed 2026-06-23: this is branch-sensitive. Docs/source `HEAD@0da6ba431`, current Miksuu `master@b8389e748243`, `origin/perf/quick-wins@0076040f` and historical release commit `a96fdda2` still expose/default `WFBE_C_AI_MAX` without an active maintained-root runtime reader found, and still mention `WFBE_C_UNITS_CLEAN_TIMEOUT` only in the commented old cleanup split at `Common_TrashObject.sqf:20`. Current stable/B74.1 `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`, B69 `8d465fce`, B74 `b23f557f` and B74.2 `d472da6a` are different: both maintained roots read `WFBE_C_AI_MAX` in `Server/AI/Commander/AI_Commander_Produce.sqf:234` as an AI commander production/template cap, and use `WFBE_C_UNITS_CLEAN_TIMEOUT` in `Common_TrashObject.sqf:21` for non-man wreck cleanup while man bodies use `WFBE_C_UNITS_BODIES_TIMEOUT`. Current stable/B74.1/B74.2 also contain a draft-only `AI_Commander_HCTopUp.DRAFT.sqf:219` reader, but that is not the player follower cap contract. Empty vehicles remain separate through `WFBE_C_UNITS_EMPTY_TIMEOUT` in `Server_HandleEmptyVehicle.sqf:20,26` on current stable/B74.1-shaped refs and `:12,18` on old-shape docs/current-Miksuu/perf/release refs. Player follower limits are still `WFBE_C_PLAYERS_AI_MAX`, read by the buy menu/RHUD and modified by Soldier skill init; do not answer player follower-cap questions with `WFBE_C_AI_MAX`. Local `d9506078` is `origin/claude/*` branch evidence, not current Miksuu upstream.
