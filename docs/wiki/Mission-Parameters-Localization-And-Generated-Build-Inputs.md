...
Full lobby/start parameter index: [Mission start parameters index](Mission-Start-Parameters-Index). Current maintained Chernarus and Vanilla Takistan have identical `Rsc/Parameters.hpp` files in docs checkout `85679dba`, with 89 active lobby-visible parameters. `WFBE_C_GAMEPLAY_UPGRADES_CLEARANCE` is not declared in `Rsc/Parameters.hpp` at all (no lobby class) yet is still consumed at runtime; it is not host-selectable.
...
Current ordnance parameter caveat: `WFBE_C_GAMEPLAY_BOMBS_ALTITUDE` is declared in `Rsc/Parameters.hpp:275-281` and, as of B66 (current master `f8a76de3`), its altitude enforcement block is LIVE in the bomb handler (`Common/Functions/Common_HandleShootBombs.sqf:34-44`): the guard reads `getPos _unit_who_shot select 2`, and when `_limit > 0 && _objAltitude >= _limit` it `deleteVehicle _projectile` (`:36-43`). The companion `WFBE_C_GAMEPLAY_BOMBS_DISTANCE_RESTRICTION` is consumed by the live distance-delete path (`Common_HandleShootBombs.sqf:21-31`). Both guards are now independent exploit checks; treat the altitude parameter as active gameplay enforcement (`0` = "Disabled").
...
Hidden-or-forced parameter caveat: some runtime switches are not ordinary lobby controls. `WFBE_C_GAMEPLAY_UPGRADES_CLEARANCE` is not declared anywhere in `Rsc/Parameters.hpp` (the lobby class was removed; it is not host-selectable), yet constants/server init still use it (`Init_CommonConstants.sqf:619`, `Server/Init/Init_Server.sqf:571`) and boot code can force it to `7` (`initJIPCompatible.sqf:186,190`). `WFBE_C_ENVIRONMENT_WEATHER_VOLUMETRIC` exists in `Parameters.hpp:210-214`, but common constants and client init both force it to `0` (`Init_CommonConstants.sqf:212`, `Client/Init/Init_Client.sqf:218`). Treat these as internal/locked runtime state until the owner chooses to expose them deliberately.
...
Visible-parameter runtime-consumer caveat, refreshed 2026-06-24: this is branch-sensitive. Docs/source `HEAD@52b7f7443bbc` is source-unchanged from `0da6ba431` for the checked parameter/cache/cleanup/AI-consumer paths; docs/source, current Miksuu `master@b8389e748243`, `origin/perf/quick-wins@0076040f` and historical release commit `a96fdda2` still expose/default `WFBE_C_AI_MAX` without an active maintained-root `AI_Commander_Produce.sqf` reader found, and still mention `WFBE_C_UNITS_CLEAN_TIMEOUT` only in the commented old cleanup split at `Common_TrashObject.sqf:20`. Current stable/B74.1 `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`, current B74.2 `origin/claude/b74.2-aicom@21b62b04`, B69 `8d465fce` and B74 `b23f557f` are different: both maintained roots read `WFBE_C_AI_MAX` in `Server/AI/Commander/AI_Commander_Produce.sqf:234` as an AI commander production/template cap, and use `WFBE_C_UNITS_CLEAN_TIMEOUT` in `Common_TrashObject.sqf:21` for non-man wreck cleanup while man bodies use `WFBE_C_UNITS_BODIES_TIMEOUT`. Checked `d472da6a..21b62b04` is empty for these parameter-consumer paths; `origin/master..origin/claude/b74.2-aicom` only changes source-Chernarus tiered player-cap/AICOM constants and the side-AI ceiling around checked parameter-adjacent files, leaving the same `WFBE_C_AI_MAX` reader and cleanup split plus no maintained Vanilla payload. Current-origin AICOM side branches are mixed target-specific context: B74.1/B74.2/`tk-aicom-port` carry both live readers, while older AICOM side branches may wire only `WFBE_C_AI_MAX` or keep the cleanup split comment-only; no live `release/*` head was exposed in the 2026-06-24 branch-name sweep. Empty vehicles remain separate through `WFBE_C_UNITS_EMPTY_TIMEOUT` in `Server_HandleEmptyVehicle.sqf:20,26` on current stable/B74-shaped refs and `:12,18` on old-shape docs/current-Miksuu/perf/release refs. Player follower limits are still `WFBE_C_PLAYERS_AI_MAX`, read by the buy menu/RHUD and modified by Soldier skill init; B74.2 source Chernarus adds `WFBE_C_PLAYERS_AI_MAX_BY_TIER` as a player-cap refinement, not a reason to answer player follower-cap questions with `WFBE_C_AI_MAX`. Local `d9506078` is `origin/claude/*` branch evidence, not current Miksuu upstream.
...
| --- | --- | --- | --- |
| `WFBE_C_AI_COMMANDER_ENABLED` | `Rsc/Parameters.hpp:99-104` default `1` | `Init_CommonConstants.sqf:103` fallback `1` | Both layers agree: AI commander is ON by default. To test with it disabled, explicitly set the lobby parameter to 0. |
...
| Bomb altitude parameter is visible but dormant — **RESOLVED (B66, current master `f8a76de3`):** the altitude block is un-commented and live at `Common_HandleShootBombs.sqf:34-44`; no patch needed. | Originally: revive/smoke the commented altitude block or hide/rename the host parameter. Now satisfied in source. | Host/admin UX now matches runtime: altitude param enforces a real restriction (`0` = Disabled). |
...
| Branch-sensitive AI/cleanup parameters | For old-shape docs/current-Miksuu/perf/historical-release targets, decide whether to port the current stable/B74.1 AI commander reader and body/wreck split, or hide/label the stale rows. For current stable/B74.1/B74.2, document `WFBE_C_AI_MAX` as AI commander production sizing and `WFBE_C_UNITS_CLEAN_TIMEOUT` as non-man wreck cleanup, not player follower cap or corpse timeout. | Host/admin parameter wording, AI commander production, player AI cap UI/RHUD, corpse cleanup, wreck cleanup and empty-vehicle cleanup all need separate smoke before release wording. |
