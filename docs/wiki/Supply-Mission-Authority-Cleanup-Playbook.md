...
> **Branch-scope update (2026-06-05): do not collapse the supply branches.** Current docs/source Chernarus is still truck-only but has the narrowed 80 m command-center scan (`supplyMissionStarted.sqf:25-28`). `origin/master` remains broad. `origin/feat/supply-helicopter` head `262dc431` adds the heli path but still clears only `SupplyAmount`/`SupplyFromTown` on completion. `origin/release/2026-06-feature-bundle` head `3282ff3f` contains release commit `4cf443fe`: cooldown casing is standardized, `supplyMissionActive.sqf`/`checkCCProximity.sqf` are removed, the Chernarus command-center scan is heli-aware (`supplyMissionStarted.sqf:50-56`, truck 80 m / heli 400 m with 2D gate), and `SupplyByHeli` is cleared on completion. Release Vanilla still uses the broad 80 m command-center scan at `:28`. **Still open across public-baseline hardening:** server-owned mission state, duplicate-start guard (XR6), server-side cargo validation, friendly-side check on the delivery CC (XR15), and runtime smoke for repeated load/deliver/destroy cycles.
...
| Command-center scan narrowing | Branch-scoped done / smoke pending | Current docs/source has the truck-only narrowed scan at `supplyMissionStarted.sqf:25-28`; release head `3282ff3f` has heli-aware Chernarus radius and 2D gate at `:50-56` but release Vanilla remains broad at `:28`; `origin/master` remains broad. |
