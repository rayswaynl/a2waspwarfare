...
> **Branch-scope update (2026-06-06): do not collapse the supply branches.** The canonical branch/root table lives in [Supply mission architecture](Supply-Mission-Architecture#current-branch-matrix). Short version: current source/stable `origin/master` `89ae9dad`, Miksuu `89ae9dad` and `perf/quick-wins` `0076040f` still enumerate the 80 m command-center scan with `nearestObjects [..., [], 80]` and then post-filter by `Base_WarfareBUAVterminal`; release `7ff18c49` has the heli-aware typed scan, cooldown casing fix and dead-twin removal/comment in both maintained release roots. **Still open across public-baseline hardening:** server-owned mission state, duplicate-start guard, server-side cargo validation, friendly-side delivery-CC checks, player-object rescan/disconnect cleanup and runtime smoke for repeated load/deliver/destroy cycles.
...
| Dead twin script | Branch-scoped | Still present/compiled in current source/stable/Miksuu/perf; release `7ff18c49` comments the compile and removes the twin files in both maintained roots. Do not patch it as the live handler. |
...
| Command-center scan narrowing | Release-branch done / current-source open | Current source/stable/Miksuu/perf still use broad `nearestObjects [..., [], 80]` enumeration at `supplyMissionStarted.sqf:28` in both maintained roots, with a `Base_WarfareBUAVterminal` post-filter at `:25`. Release `7ff18c49` has the heli-aware typed scan in both maintained release roots at `:52,58`. Use [Supply mission architecture](Supply-Mission-Architecture#current-branch-matrix) for the matrix. |
...
| Command-center scan status is branch-scoped. | Current source/stable/Miksuu/perf use `nearestObjects [..., [], 80]` at `supplyMissionStarted.sqf:28` in both maintained roots, then post-filter `_x isKindOf "Base_WarfareBUAVterminal"` at `:25`; release `7ff18c49` uses `["Base_WarfareBUAVterminal"]` plus heli 400/truck 80 and 2D heli gate at `:52,58` in both maintained release roots. The 8 m nearby-player/object scan remains intentionally broad. | Do not claim current-source propagation for the typed scan until it is actually ported. During authority rewrites, preserve the terminal post-filter and port the release typed scan deliberately if the performance cleanup is in scope. |
| `supplyMissionActive.sqf` is a dead twin in current source/stable/Miksuu/perf, removed in release. | Current source compiles it as `WFBE_SE_FNC_SupplyMissionActive`, but the live start path is `supplyMissionStarted.sqf`; release `7ff18c49` comments the compile and removes the twin plus `checkCCProximity.sqf`. | Future owners should not patch the wrong script as the live path; either retire it in the source baseline or keep it clearly marked as dead until release is merged. |
...
6. If touching the command-center search, port or recreate the release `7ff18c49` typed scan deliberately; current source/stable/Miksuu/perf still enumerate `nearestObjects [..., [], 80]` before the terminal post-filter. Keep the 8 m nearby-player/object scan broad unless runtime evidence says otherwise.
7. Retire or explicitly mark `supplyMissionActive.sqf` as dead twin in the current source baseline; release `7ff18c49` already removed it from both maintained release roots.
