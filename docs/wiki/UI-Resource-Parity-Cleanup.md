...
Current docs/source note: 2026-06-24 refreshes rechecked docs checkout `docs/developer-wiki-index` for the Economy-control row. The repo docs branch `HEAD@427346b0d` is unchanged from `21f0d53b`, `d2a3f995` and `b5219d47` for checked Economy controller/resource paths. The 2026-06-23 refreshes still cover stale-upgrade and duplicate-IDD rows: checked stale-upgrade paths at `HEAD@0fdd5602` remain unchanged from `d4cfef80`, and checked Dialogs/Titles IDD paths at `HEAD@edbd341e` remain unchanged from `b5219d47`. Other matrix rows still preserve their named branch snapshot unless a row says otherwise.
