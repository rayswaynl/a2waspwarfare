...
Current docs/source note: 2026-06-23 refreshes rechecked docs checkout `docs/developer-wiki-index` for the stale upgrade, Economy-control and duplicate-IDD rows. The checked stale-upgrade paths at `HEAD@0fdd5602` remain unchanged from `d4cfef80`; the checked Economy controller/resource paths at `HEAD@21f0d53b` remain unchanged from `d2a3f995` and `b5219d47`; the checked Dialogs/Titles IDD paths at `HEAD@edbd341e` remain unchanged from `b5219d47`. Other matrix rows still preserve their named branch snapshot unless a row says otherwise.
...
| Economy stale control writes | Docs `HEAD@21f0d53b` remains source-unchanged from `d2a3f995` / `b5219d47` for checked Economy controller/resource paths. `GUI_Menu_Economy.sqf:7-8` writes `23004`, `23005`, `23006`, but `RscMenu_Economy` declares `23002`, `23003`, `23008`-`23016` and no `23020` (`Dialogs.hpp:3327,3339,3346`). | Same stale writes and declared-control shape. | Current stable `origin/master@0139a346`, current B69 `origin/claude/b69@8d465fce` and adjacent B74 `origin/claude/b74-aicom-spend@b23f557f` remove the stale writes in both maintained roots, use display controls `23002` and `23020` (`GUI_Menu_Economy.sqf:4,25`) and declare `23020` (`Rsc/Dialogs.hpp:3070`). B74 only changes Chernarus dashboard label text versus stable among checked paths. | Same stale writes in both maintained roots (`GUI_Menu_Economy.sqf:7-8`), with no declared `23020`. | Historical `a96fdda2` matches the current stable/B69/B74 controller/resource shape in both maintained roots (`GUI_Menu_Economy.sqf:4,25`; `Rsc/Dialogs.hpp:3006`). | Preserve/port the stable/B69/B74/release `23020` repair where the target still carries old-shape writes; smoke Economy disabled state, income controls, sell mode and supply-truck respawn. |
...
- Economy declared controls: docs/source `RscMenu_Economy` declares `23002`, `23003`, `23008`, `23009`, `23010`, `23011`, `23012`, `23013`, `23014`, `23015`, `23016`, not `23004`-`23006` or `23020`. Current stable/B69/B74 and historical `a96fdda2` add the read-only dashboard control as `23020` in both maintained roots, so old-shape targets should preserve or port that declared-control repair rather than reopening current stable.
...
2. Normalize Economy controller/control-map parity across old-shape source Chernarus and maintained Vanilla targets, using current stable/B69/B74 or historical release as comparison evidence.
