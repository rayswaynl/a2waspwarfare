...
Current docs/source note: 2026-06-23 refreshes rechecked docs checkout `docs/developer-wiki-index` for the stale upgrade and Economy-control rows. The checked stale-upgrade paths at `HEAD@0fdd5602` remain unchanged from `d4cfef80`; the checked Economy controller/resource paths at `HEAD@21f0d53b` remain unchanged from `d2a3f995` and `b5219d47`. Other matrix rows still preserve their named 2026-06-14 branch snapshot unless a row says otherwise.
