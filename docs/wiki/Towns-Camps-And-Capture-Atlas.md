...
Checked 2026-06-14 after `git fetch --all --prune`. Docs head `5243f91d` has no checked town/camp/patrol/respawn/buy source-path changes from the earlier `3eefcb00` atlas checkpoint, so the docs-checkout line anchors below still apply. Rechecked the Patrols v2 path on 2026-06-23: docs/source `HEAD@665df909` has no Patrols v2 startup hits, current stable/B74.1 is `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`, B74.2 is `origin/claude/b74.2-aicom@d472da6a`, and current origin exposes no live `release/*` or patrol rescue heads. Zero-camp helper/caller refs were refreshed again on 2026-06-23: docs/source `HEAD@6b8b12df` is unchanged from `91d1ccf2` / `ade4d356` for the checked helper/consumer paths, current B69 `origin/claude/b69@8d465fce` and adjacent B74 `origin/claude/b74-aicom-spend@b23f557f` have empty checked helper/caller deltas from `0a1ccb4d` and from B69 to B74, the then-current stable helper shape was `origin/master@0139a346`, and current origin exposes no live `release/*`, `feat/*camp*` or `feat/*town*` heads. Camp flag/repair refs were refreshed again on 2026-06-23: docs/source `HEAD@de911438` is unchanged from `28a7d9c5` for checked camp capture, `repair-camp` and `CampCaptured` client-handler paths; the current stable camp-capture shape now checked at `origin/master@f8a76de34`, current B69 is `origin/claude/b69@8d465fce`, adjacent B74 is `origin/claude/b74-aicom-spend@b23f557f`, current Miksuu is `b8389e748243`, perf is `origin/perf/quick-wins@0076040f`, historical release evidence is `a96fdda2`, and live naval-HVT evidence is `origin/feat/naval-hvt-objectives@2e1c5931`. Checked B69..B74 and `0a1ccb4d..B74` camp/repair/client-handler deltas are empty; current origin exposes no live `release/*`, `*camp*` or `*town*` rescue head. Treat older `89ae9dad` text below as historical unless this table preserves it for a still-relevant branch.

| Ref / root scope | Camp flag branch truth | Patrol branch truth | Zero-camp helper truth |
| --- | --- | --- | --- |
| Docs/source `HEAD@665df909` (camp paths unchanged from `28a7d9c5`; helper paths unchanged from `91d1ccf2` / `ade4d356`) | Chernarus and maintained Vanilla write captured camp `sideID` to `_newSID` at `server_town_camp.sqf:132`, then set the flag from old `_side` at `:135`; `repair-camp` changes `sideID` at `Server_HandleSpecial.sqf:165` and broadcasts at `:168` without a flag refresh. `CampCaptured.sqf` recolors markers only at `:22,:47`, so it does not repair the world flag object. | No `Server/FSM/server_side_patrols.sqf`, `WFBE_UP_PATROLS` or `WFBE_ACTIVE_PATROLS` startup hits were found in either maintained root; old town-patrol source remains the docs-checkout shape. | Both maintained roots keep `Common_GetTotalCamps.sqf:10` and `Common_GetTotalCampsOnSide.sqf:16` returning `1`; consumers include `server_town.sqf:179-195`, `Common_GetRespawnThreeway.sqf:7`, `Client_GetRespawnAvailable.sqf:69` and `GUI_Menu_BuyUnits.sqf:111-112`. |
| Current stable/B74.1 `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34` | Chernarus and maintained Vanilla fix independent capture: `server_town_camp.sqf:83-86` writes `_newSID`, sets texture from `str _newSide` at `:84`, then broadcasts `CampCaptured`; `repair-camp` still changes side/broadcasts without a flag refresh, with current line drift to Chernarus `Server_HandleSpecial.sqf:501,504` and Vanilla `:468,471`. | Patrols v2 is present in both maintained roots: `WFBE_UP_PATROLS = 23` and `WFBE_C_SIDE_PATROLS_MAX = 2` at `Init_CommonConstants.sqf:60,63`; Chernarus defaults/caps line-drift to `:710,:908-909`, Vanilla to `:512,:780`; runner compile is `Init_Common.sqf:107`; server driver starts at Chernarus `Init_Server.sqf:872` / Vanilla `:859`; friendly markers start at `Init_Client.sqf:495`; level-4 convoy/camp-sweep hooks are Chernarus `Common_RunSidePatrol.sqf:114,154,253` and Vanilla `:108,148,247`. | Helper fallback is unchanged from the earlier checked current-master shape; consumers line-drift to `server_town.sqf:167-183`, `Client_GetRespawnAvailable.sqf:92` and `GUI_Menu_BuyUnits.sqf:120-121`. |
...
B74.2 branch note, checked 2026-06-23: `origin/claude/b74.2-aicom@d472da6ae` branches from current stable/B74.1 `origin/master@f8a76de34`, has no GitHub PR route from `gh pr list --head claude/b74.2-aicom --state all`, changes 26 source Chernarus files / +324 / -24 and has no `Missions_Vanilla` payload. Town/camp relevant changes are branch-only: `WFBE_C_CAMPS_RANGE` becomes 11.5 at `Init_CommonConstants.sqf:487`; `WFBE_C_AICOM_CAMP_STALL_PASSES = 3` at `:357` is consumed by `Common_RunCommanderTeam.sqf:813,871-883`; tiered active-town/defender/patrol arrays are defined at `Init_CommonConstants.sqf:170,173-175` and consumed by `server_town_ai.sqf:61,64-65`, `Server_GetTownGroupsDefender.sqf:89,91` and `server_side_patrols.sqf:32,38`; town/camp stat credits are written at `server_town.sqf:240` and `server_town_camp.sqf:90`; and carrier HVT recapture respawns the deck hangar and updates `wfbe_hangar` / `wfbe_airfield_side` at `server_town.sqf:290,318-327`. Keep repair-camp flag debt and zero-camp helper debt separate; this branch does not prove maintained Vanilla/Takistan parity.
...
Camp flag texture branch detail is intentionally single-sourced in [Current Branch Scope](#current-branch-scope): current stable/B74.1 `origin/master@f8a76de34`, B69 `8d465fce`, adjacent B74 `b23f557f`, historical release `a96fdda2` and `origin/feat/naval-hvt-objectives@2e1c5931` fix independent capture in both maintained roots; docs/source `HEAD@de911438` and Miksuu `b8389e748243` still use the old owner; `perf/quick-wins` fixes Chernarus only; and repair-side world-flag refresh remains open everywhere checked. Use [Feature status](Feature-Status-Register) and [Source fix propagation queue](Source-Fix-Propagation-Queue) for the code-owner queue instead of restating the matrix here.
...
Checked 2026-06-14 after fetching `origin` and Miksuu upstream; current stable/B74.1 Patrols v2 line refs refreshed 2026-06-23 against `origin/master@f8a76de34`, with B74.2 checked at `origin/claude/b74.2-aicom@d472da6a`.
...
| B74.2 branch `origin/claude/b74.2-aicom@d472da6a` source Chernarus | Same side-upgrade driver, with branch-only pop-tier cap reads in `server_side_patrols.sqf:32,38` and town active cap reads in `server_town_ai.sqf:61,64-65`. | Same Patrols v2 runner path; checked B74.2 Patrols/town-AI diff has no `Missions_Vanilla` payload. | Branch-only population-tier overlay; do not claim maintained Vanilla parity until propagated or separately checked. |
...
B74.2 overlay: source Chernarus adds live population-tier reads for active town count, defender coefficient and side-patrol cap (`Init_CommonConstants.sqf:170,173-175`; `server_town_ai.sqf:61,64-65`; `Server_GetTownGroupsDefender.sqf:89,91`; `server_side_patrols.sqf:32,38`) plus client AI-cap reads in Buy Units/RHUD (`GUI_Menu_BuyUnits.sqf:37,41`; `Client_UpdateRHUD.sqf:354,358`). The checked B74.2 Patrols/town-AI diff has no maintained Vanilla payload.
...
| Branch-split / repair-side open | Independent camp-capture and repair-side world-flag status is branch-sensitive. Current stable, B69, adjacent B74, historical `a96fdda2` and naval HVT fix independent capture in both maintained roots; docs head and Miksuu still use the old owner; perf fixes Chernarus only; and repair-side flag refresh remains open everywhere checked. | [Current Branch Scope](#current-branch-scope); docs `server_town_camp.sqf:132,135` and `Server_HandleSpecial.sqf:165,168`; current stable repair line drift `Server_HandleSpecial.sqf:468,471`; B69/B74 repair drift Chernarus `:491,:494`, Vanilla `:468,:471` | This page, [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) |
