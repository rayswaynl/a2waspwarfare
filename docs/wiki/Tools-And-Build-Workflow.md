...
| `A2WASP_SKIP_ZIP=1` is verified before propagation-only runs. | Docs/source `HEAD@101930f47` implements it in `ZipManager.cs:11,96`, which skips `_MISSIONS.7z` packaging and avoids a packaging-only `7za` dependency. Current stable `origin/master@c4c9a6479`, current B74.2/B69/B74, historical `a96fdda2`, current Miksuu/perf and the local `feat/supply-helicopter` checkout do not have checked support, so missing `7za` will still throw after generation/copy. |
...
- Missing `7za` causes the final packaging step to throw unless the target branch implements and honors `A2WASP_SKIP_ZIP=1`; docs/source `HEAD@101930f47` does, while current stable `origin/master@c4c9a6479`, B74.2/B69/B74, historical `a96fdda2`, current Miksuu/perf and the local `feat/supply-helicopter` checkout do not in the checked refs. Inspect generated/copied files before assuming the whole run did nothing.
