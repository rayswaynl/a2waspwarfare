...
Local workspace note: `FileManager.FindA2WaspWarfareDirectory` now supports both an ancestor folder literally named `a2waspwarfare` and a normal repo root containing the directory `Missions\[55-2hc]warfarev2_073v48co.chernarus`, the directory `Tools\LoadoutManager`, and the file `AGENTS.md`. This lets Codex checkouts such as `work\a` run the tool without renaming the workspace.
