...
Source-refreshed 2026-06-14 on docs head `a640e722`; targeted diffs from `1f0b9018` through `HEAD` over checked Chernarus and maintained Vanilla respawn, MASH, skill, camp/threeway and AI-respawn paths returned no source changes at that checkpoint. MASH branch scope was refreshed 2026-06-22 on docs head `db3015f18ea3`: docs/perf/Miksuu-shaped roots still keep the old deploy/relay shape, while current stable `origin/master@0139a3468609`, historical `a96fdda28087` and B69-family refs remove the maintained-root deploy/module path. Paths below are relative to source root `Missions/[55-2hc]warfarev2_073v48co.chernarus` unless a maintained-root or branch note says otherwise; use the MASH matrix below for branch-sensitive deploy/marker claims.
...
Canonical statement for docs/Miksuu/perf-shaped roots: **local officer MASH respawn is source-supported; shared/JIP-safe MASH marker synchronization is orphaned in maintained roots.** Current stable `origin/master@0139a3468609`, historical `a96fdda28087` and B69-family refs are different: they remove the maintained-root MASH deploy/module path while leaving some config/classname residues. No Arma runtime smoke was run for this docs pass, so describe source paths as source-supported or source-removed rather than runtime-proven.
...
- Branch recheck on 2026-06-22 found the orphaned marker shape in docs head `db3015f18ea3`, Miksuu `b8389e748243` and `origin/perf/quick-wins@0076040f8a5e` maintained roots. Current stable `origin/master@0139a3468609`, historical `a96fdda28087`, `origin/claude/b69@0a1ccb4d05c5` and `origin/claude/b69-finalpieces@80d3267c1b2b` have no maintained-root `Client/Module/MASH` or `Server/Module/MASH` tree entries and no MASH init hooks in the checked maintained roots; all keep a `Skill_Apply.sqf:43` comment that the deploy ability was removed plus config/classname residues.

| Scope | Local MASH respawn | Shared marker relay | Development meaning |
| --- | --- | --- | --- |
| Docs head `db3015f18ea3` Chernarus and maintained Vanilla | Source-supported: deploy stores local `wfbe_mash`; respawn availability reads it. | Orphaned: active server receiver, commented client receiver compile, no maintained deploy sender. | Decide personal/squad/team semantics before changing code. |
...
| Current stable `origin/master@0139a3468609`, historical `a96fdda28087` and B69-family refs | MASH deploy/module path removed in maintained roots; class/config residues still mention MASH. | No maintained-root MASH module relay path found. | Treat MASH revival/removal decisions as branch-sensitive; do not port docs/Miksuu marker conclusions blindly. |
...
| Owner decision | MASH is local to the deployer on old-shape audited roots; team-wide MASH respawn is not proven. | [MASH split](Respawn-And-Death-Lifecycle-Atlas#mash-split-live-respawn-dead-marker-relay) owns the deploy/read anchors and branch matrix. | Decide whether MASH should be personal, squad/team-shared, marker-only, or remain removed on current-stable/B69-shaped targets. If shared, design server-owned registry and explicit JIP resend/pull. |
| Broken/dead edge | MASH marker synchronization is orphaned on docs/Miksuu/perf-shaped maintained roots; current stable/B69-shaped roots have no maintained-root MASH module relay path after removing the deploy ability. | [MASH split](Respawn-And-Death-Lifecycle-Atlas#mash-split-live-respawn-dead-marker-relay) owns the receiver/server-listener anchors and current-stable/B69 removal note. | Revive with server-held marker records and JIP resend on old-shape targets, or finish removal/cleanup deliberately on the target branch. |
