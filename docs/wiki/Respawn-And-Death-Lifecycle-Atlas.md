...
Source-refreshed 2026-06-14 on docs head `a640e722`; targeted diffs from `1f0b9018` through `HEAD` over checked Chernarus and maintained Vanilla respawn, MASH, skill, camp/threeway and AI-respawn paths returned no source changes at that checkpoint. MASH branch scope was refreshed again 2026-06-23 on docs/source `HEAD@443055cf`: checked maintained-root MASH deploy, marker relay, init and respawn-availability paths are unchanged from `2b5139219faa` / `db3015f18ea3`. Docs/perf/Miksuu-shaped roots still keep the old deploy/relay shape, while current stable `origin/master@0139a3468609`, current B69 `origin/claude/b69@8d465fcede7f`, adjacent B74 `origin/claude/b74-aicom-spend@b23f557fc912` and historical `a96fdda28087` remove the maintained-root deploy/module path. Older B69 proof refs `0a1ccb4d05c5` and finalpieces `80d3267c1b2b` remain provenance; diffs from both to current B69 touched `Init_Client.sqf` but had no MASH-related hunks, and the checked B69..B74 MASH path delta is empty. Paths below are relative to source root `Missions/[55-2hc]warfarev2_073v48co.chernarus` unless a maintained-root or branch note says otherwise; use the MASH matrix below for branch-sensitive deploy/marker claims.
...
Draft PR #78 / `origin/claude/trello-map-radius-circles@77dd71ba3` is a branch-only map-indicator candidate, not current stable behavior. It adds `Client/Functions/Client_AmbulanceRedeployCircles.sqf` in source Chernarus plus maintained Vanilla and launches it from `Init_Client.sqf:1227`; current stable has no `Client_AmbulanceRedeployCircles`, `AmbRange_` or launcher hits.

The watcher self-gates on `WFBE_C_RESPAWN_MOBILE` (`Client_AmbulanceRedeployCircles.sqf:12`), builds a side-local classname list from `WFBE_%1AMBULANCES` and `WFBE_%1REDEPLOYTRUCKS` (`:14-18`), reads the current side respawn-range upgrade and `WFBE_C_RESPAWN_RANGES` (`:30-35`), creates yellow local border ellipses for same-side vehicles of those types (`:41-48`) and then repositions, resizes or deletes rings every five seconds (`:57-68`). The radius matches the respawn availability radius in current stable (`Client_GetRespawnAvailable.sqf:33-37,55-58`; constants Chernarus `Init_CommonConstants.sqf:44,674,678`, Vanilla `:44,476,480`).
...
- Branch recheck on 2026-06-23 found the orphaned marker shape in docs/source `HEAD@443055cf` (unchanged from `2b5139219faa` and `db3015f18ea3` for checked MASH paths), Miksuu `b8389e748243` and `origin/perf/quick-wins@0076040f8a5e` maintained roots. Current stable `origin/master@0139a3468609`, current B69 `origin/claude/b69@8d465fcede7f`, adjacent B74 `origin/claude/b74-aicom-spend@b23f557fc912` and historical `a96fdda28087` have no maintained-root `Client/Module/MASH` or `Server/Module/MASH` tree entries and no MASH init hooks in the checked maintained roots; all keep a `Skill_Apply.sqf:43` comment that the deploy ability was removed plus config/classname residues. The B69 deltas `0a1ccb4d05c5..8d465fce` and `80d3267c1b2b..8d465fce` touched `Init_Client.sqf` but contained no MASH-related hunks, and `origin/claude/b69..origin/claude/b74-aicom-spend` is empty for the checked MASH deploy/relay/init/respawn paths.

| Scope | Local MASH respawn | Shared marker relay | Development meaning |
| --- | --- | --- | --- |
| Docs/source `HEAD@443055cf` Chernarus and maintained Vanilla, unchanged from `2b5139219faa` / `db3015f18ea3` for checked MASH paths | Source-supported: deploy stores local `wfbe_mash`; respawn availability reads it. | Orphaned: active server receiver, commented client receiver compile, no maintained deploy sender. | Decide personal/squad/team semantics before changing code. |
...
| Current stable `origin/master@0139a3468609`, current B69 `origin/claude/b69@8d465fce`, adjacent B74 `origin/claude/b74-aicom-spend@b23f557f` and historical `a96fdda2` | MASH deploy/module path removed in maintained roots; class/config residues still mention MASH. Older B69 refs `0a1ccb4d05c5` and `80d3267c1b2b` remain provenance for the same shape; checked B69..B74 MASH delta is empty. | No maintained-root MASH module relay path found. | Treat MASH revival/removal decisions as branch-sensitive; do not port docs/Miksuu marker conclusions blindly. |
...
| Owner decision | MASH is local to the deployer on old-shape audited roots; team-wide MASH respawn is not proven. | [MASH split](Respawn-And-Death-Lifecycle-Atlas#mash-split-live-respawn-dead-marker-relay) owns the deploy/read anchors and branch matrix. | Decide whether MASH should be personal, squad/team-shared, marker-only, or remain removed on current-stable/current-B69/B74-shaped targets. If shared, design server-owned registry and explicit JIP resend/pull. |
| Broken/dead edge | MASH marker synchronization is orphaned on docs/Miksuu/perf-shaped maintained roots; current stable/current-B69-shaped roots have no maintained-root MASH module relay path after removing the deploy ability. | [MASH split](Respawn-And-Death-Lifecycle-Atlas#mash-split-live-respawn-dead-marker-relay) owns the receiver/server-listener anchors and current-stable/current-B69 removal note. | Revive with server-held marker records and JIP resend on old-shape targets, or finish removal/cleanup deliberately on the target branch. |
