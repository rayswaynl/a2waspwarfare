...
| `origin/release/2026-06-feature-bundle` Chernarus | Heli-aware narrowed scan at `supplyMissionStarted.sqf:50-56`: terminal class filter, truck 80 m, heli 400 m plus 2D gate. | Fixed: `Init_Town.sqf:35` seeds `LastSupplyMissionRun`. | Removed/commented in `Init_Server.sqf:82`; `supplyMissionActive.sqf` / `checkCCProximity.sqf` are deleted by release cleanup. | Reads and clears `SupplyByHeli` (`supplyMissionCompleted.sqf:24,40-42`). | Release Chernarus has the most advanced cleanup shape, but it is not Vanilla parity and still needs server-owned mission state, duplicate-start guard, cargo validation, friendly-CC checks and smoke. |
| `origin/release/2026-06-feature-bundle` Vanilla | Still broad at `supplyMissionStarted.sqf:28`. | Still lowercase/uppercase split. | Compiled dead twin remains. | No `SupplyByHeli`. | Release branch is Chernarus-only for the supply cleanup lanes; do not call release fully propagated. |
...
| --- | --- | --- |
| `supplyMissionActive.sqf` | Dead twin in current docs/source and stable/upstream; release Chernarus removes it while release Vanilla still compiles it. It is compiled as `WFBE_SE_FNC_SupplyMissionActive`, but the live path is `supplyMissionStarted.sqf`, which self-registers the `WFBE_Client_PV_SupplyMissionStarted` handler. The dead twin still carries older broad-scan logic wherever it remains. | Remove the dead compile/function or keep it explicitly marked as retired; do not patch it as the live implementation. |
