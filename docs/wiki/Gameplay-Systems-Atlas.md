...
| `wfbe_structures_logic` consumer | Construction workers add/remove construction-site logic objects (`Server/Construction/Construction_SmallSite.sqf:70,99`; `Construction_MediumSite.sqf:70,114`). Building repair consumes and removes those logic entries while repairing/degrading/losing a site (`Server/Functions/Server_HandleBuildingRepair.sqf:81,99`). Use [Construction and CoIn systems atlas](Construction-And-CoIn-Systems-Atlas) for the owning map. |
