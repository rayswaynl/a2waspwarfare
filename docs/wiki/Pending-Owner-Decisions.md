...
| `origin/feat/buymenu-easa-qol` head `a66d4691` | Narrow Chernarus UI branch: `Client_UIFillListBuyUnits.sqf:104` tints unaffordable prices red, `GUI_Menu_BuyUnits.sqf:201-206` appends live queue counts to factory tabs and `GUI_Menu_EASA.sqf:29-37` marks the current aircraft loadout green and preselects it. Diff is 3 files, +42/-6; `git diff --check` is clean. | Decide whether to accept these as UI-only QoL changes and whether they need maintained Vanilla propagation. Before merge, smoke Buy Units with affordable/unaffordable units, crew/full-price expectations, queue count refresh without UI churn and EASA current-loadout selection. |
| `origin/feat/player-stats` head `e01e47e1` | Adds an off-by-default stats pipeline: `WFBE_C_STATS_ENABLED = false` (`Init_CommonConstants.sqf:443`), kill stat hooks (`RequestOnUnitKilled.sqf:51-65`), `Server/Stats/RecordStat.sqf`/`StatsFlush.sqf`, DiscordBot `StatsService` RPT tailing/accumulation and `DiscordBot.Tests/*` coverage. Diff is 23 files, +1919/-1; `git diff --check` is clean. | Decide the deployment model before enabling: RPT path/state files, stats JSON location, UID/privacy policy, log volume, crash-safe writes, DiscordBot test/build gate and whether this should remain in DiscordBot or move toward the mission extension. Runtime smoke should include disabled-by-default proof and enabled stats flush from real RPT lines. |
| `origin/perf/quick-wins` head `0076040f` | Chernarus-only fix branch: side-supply clamp shape (`Common_ChangeSideSupply.sqf:20-26`), crewless-buy queue decrement (`Client_BuildUnit.sqf:365-368`), paratrooper marker PV registration (`Init_PublicVariables.sqf:40`) and several server-loop / handler corrections across 18 files. `git diff --check` is clean. | Decide whether to cherry-pick, merge whole, or split into per-fix branches. Before release wording, reconcile with existing docs/source fixes, propagate maintained Vanilla, and smoke economy debit/no-credit inversion, factory queue counters, paratrooper markers, cleaner/camp nil-code behavior and special-handler regressions. |
| `origin/feat/commander-positions` head `560db61c` | Broad branch with merge base `f5985b77`, not current stable `2cdf5fb8`: 83 files, +524/-2025. It adds commander-buildable position anchors/compositions through `Structures_CO_US.sqf:168-174`, `RequestDefense.sqf:11-14` and new `Server_ConstructPosition.sqf`, but also carries Valhalla, UI, AFK/profile, HC/static-defense, performance-audit and Vanilla deltas. `git diff --check` reports trailing whitespace in source files. | Decide whether to split the construction-position feature from unrelated branch baggage. Promotion gates: clean whitespace, recheck DR-6 construction authority, smoke placement-at-clicked-position rather than map corner, CoIn/HQ undeploy cleanup, modular wall usability/pathing and Chernarus/Vanilla parity. |
| `origin/feature/zargabad-map` head `1fdcb37a` | Terrain branch: 832 files, +77702/-95. Adds `Missions_Vanilla/[31-2hc]warfarev2_073v48co.zargabad`, terrain integration (`ZARGABAD.cs:1-5`), source hooks (`initJIPCompatible.sqf:121-123`, `Server/Init/Init_Zargabad.sqf`) and Zargabad completion/runtime evidence guides. `git diff --check` reports generated mission trailing whitespace. | Decide whether Zargabad is a maintained Vanilla target, a separate low-pop experiment or a branch-only playtest. Before promotion: clean generated whitespace, validate LoadoutManager terrain generation, run hosted/dedicated/JIP/HC smoke, review screenshots/RPT packet, verify class loads and tune town/base/camp/economy balance. |
| `origin/feat/supply-helicopter` head `262dc431` | Current local supply-heli branch head has moved beyond the older PR #1 evidence head `ffeea4c2`. New current evidence includes Air-factory upgrade 3 load gate (`supplyMissionStart.sqf:21-29`), Air upgrade 4 cash-run branch (`supplyMissionCompleted.sqf:24-35`), load/unload timer constants (`Init_CommonConstants.sqf:172-173`) and lobby toggle (`Rsc/Parameters.hpp:4-7`). No maintained Vanilla `SupplyByHeli`/heli constant hits found. | Refresh PR #1 docs before merge review. Decide Air-upgrade level semantics, cash-run economy, pilot/commander payout behavior, load/unload timer UX and Vanilla propagation. Also review non-supply baggage and branch whitespace because the diff is not supply-only. |
