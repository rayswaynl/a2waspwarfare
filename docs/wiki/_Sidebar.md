# Sidebar

- Getting started
  - [Home](Home)
  - [Quickstart](Quickstart-For-Humans-And-Agents)
  - [Mission config/version graph](Mission-Config-Version-Include-Graph)
  - [AI assistant guide](AI-Assistant-Guide)
  - [AI assistant developer guide](AI-Assistant-Developer-Guide)
  - [LLM agent entry pack](LLM-Agent-Entry-Pack)
  - [Agent development pack alias](Agent-Development-Pack)
  - [Machine index](agent-machine-index.json)
  - [llms.txt](llms.txt)
- Status and coordination
  - [Agent context](Agent-Context)
  - [Agent collaboration protocol](Agent-Collaboration-Protocol)
  - [Agent worklog](Agent-Worklog)
  - [Agent worklog archive](Agent-Worklog-Archive)
  - [Progress dashboard](Progress-Dashboard)
  - [PR cleanup and integration lab](PR-Cleanup-And-Integration-Lab)
  - [Coordination board](Coordination-Board)
  - [Codebase coverage ledger](Codebase-Coverage-Ledger)
  - [Bottleneck removal queue](Bottleneck-Removal-Queue)
  - [Current source status](Current-Source-Status-Snapshot)
  - [Wiki mirror reconciliation](Wiki-Mirror-Reconciliation-Plan)
  - [Navigation inventory](Navigation-Inventory-And-Page-Status)
- Architecture
  - [Architecture overview](Architecture-Overview)
  - [Mission entrypoints and lifecycle](Mission-Entrypoints-And-Lifecycle)
  - [Lifecycle wait chain](Lifecycle-Wait-Chain)
  - [Player join/disconnect and AntiStack lifecycle](Player-Join-Disconnect-And-AntiStack-Lifecycle)
  - [Mission parameters/localization/build inputs](Mission-Parameters-Localization-And-Generated-Build-Inputs)
  - [Stringtable localization key-family catalog](Stringtable-Localization-Catalog)
  - [Source inventory](Source-Inventory)
  - [Content structure and maps](Content-Structure-And-Maps)
  - [Assets/config/localization/parameters](Assets-Config-Localization-And-Parameters-Atlas)
- Code and networking
  - [SQF code atlas](SQF-Code-Atlas)
  - [Function and module index](Function-And-Module-Index)
  - [Modules atlas](Modules-Atlas)
  - [NEURO AI-taxi module reference](NEURO-AI-Taxi-Module-Reference)
  - [Variable and naming conventions](Variable-And-Naming-Conventions)
  - [Networking and public variables](Networking-And-Public-Variables)
  - [Public variable channel index](Public-Variable-Channel-Index)
  - [PVF dispatch implementation](PVF-Dispatch-Implementation-Playbook)
  - [Server authority migration map](Server-Authority-Migration-Map)
- Gameplay systems
  - [Core systems index](Core-Systems-Index)
  - [Gameplay systems atlas](Gameplay-Systems-Atlas)
  - [Commander/HQ lifecycle atlas](Commander-HQ-Lifecycle-Atlas)
  - [Economy, towns and supply](Economy-Towns-And-Supply)
  - [Anti-stack skill-balance mechanic](Anti-Stack-Skill-Balance-Mechanic)
  - [Empty-side supply income stagnation](Empty-Side-Supply-Income-Stagnation)
  - [Towns, camps and capture atlas](Towns-Camps-And-Capture-Atlas)
  - [Victory and endgame atlas](Victory-And-Endgame-Atlas)
  - [Marker cleanup and restoration](Marker-Cleanup-Restoration-Systems-Atlas)
  - [Marker loop engine and registries](Marker-Loop-Engine-And-Registries)
  - [Map marker families content catalog](Map-Marker-Families-Content-Catalog)
  - [Marker subsystem function reference](Marker-Subsystem-Function-Reference)
  - [Client marker FSM updater map](Client-Marker-FSM-Updater-Map)
  - [Support specials and tactical modules](Support-Specials-And-Tactical-Modules-Atlas)
  - [Naval HVT objectives (carriers/SCUD)](Naval-HVT-Objectives-Atlas)
  - [SCUD saturation strike mechanic](Scud-Saturation-Strike-Mechanic)
  - [Takistan airfield FPV drone design](Takistan-Airfield-FPV-Drone-Design)
  - [Construction and CoIn systems](Construction-And-CoIn-Systems-Atlas)
  - [Structure damage reduction & friendly-fire](Structure-Damage-Reduction-And-Friendly-Fire)
  - [Construction logic list cleanup](Construction-Logic-List-Cleanup)
  - [Resistance supply scaffold](Resistance-Supply-Scaffold)
  - [GUER Insurgents branch audit](GUER-Insurgents-Branch-Audit)
  - [GUER insurgent player economy](GUER-Insurgent-Player-Economy)
  - [GUER air-defense loop (Ka-137/Mi-24)](GUER-Air-Defense-Loop)
  - [Upgrades and research atlas](Upgrades-And-Research-Atlas)
  - [Supply mission architecture](Supply-Mission-Architecture)
  - [Supply mission authority cleanup](Supply-Mission-Authority-Cleanup-Playbook)
  - [Current supply helicopters PR1](Current-Work-Supply-Helicopters-PR1)
  - [Respawn and death lifecycle](Respawn-And-Death-Lifecycle-Atlas)
- Content, reference and catalogs
  - [Faction unit/vehicle roster catalog](Faction-Unit-And-Vehicle-Roster-Catalog)
  - [Auxiliary/SF/civilian unit catalog](Auxiliary-And-Special-Forces-Unit-Catalog)
  - [Gear store loadout route catalog](Gear-Store-Loadout-Route-Catalog)
  - [Upgrade research (cross-faction)](Upgrade-Research-Cross-Faction-Reference)
  - [Gear store price and upgrade catalog](Gear-Store-Price-And-Upgrade-Catalog)
  - [Gear store catalog (complete, per faction)](Gear-Store-Catalog-Per-Faction)
  - [Defense structures catalog](Defense-Structures-Catalog)
  - [Artillery reference per faction](Artillery-Reference-Per-Faction)
  - [AI squad team templates catalog](AI-Squad-Team-Templates-Catalog)
  - [Town AI group composition catalog](Town-AI-Group-Composition-Catalog)
  - [Player skill abilities reference](Player-Skill-Abilities-Reference)
  - [Default gear template content catalog](Default-Gear-Template-Content-Catalog)
  - [Chernarus map content reference](Chernarus-Map-Content-Reference)
  - [Takistan map content reference](Takistan-Map-Content-Reference)
  - [IRS IR-smoke countermeasure](IRS-IR-Smoke-Missile-Countermeasure)
  - [Arty module special munitions](Arty-Module-Special-Munitions)
  - [Zeta cargo sling-load reference](Zeta-Cargo-Sling-Load-Reference)
  - [Spawn primitive function reference](Spawn-Primitive-Function-Reference)
  - [Kill and score pipeline](Kill-And-Score-Pipeline)
  - [Waypoint helper function reference](Waypoint-Helper-Function-Reference)
  - [Position and proximity function reference](Position-And-Proximity-Function-Reference)
  - [Side/team state function reference](Side-Team-State-Function-Reference)
  - [Player AI watchdog and recovery](Player-AI-Watchdog-And-Recovery)
  - [LoadoutManager data-model contributor guide](LoadoutManager-Data-Model-Contributor-Guide)
  - [Discord status bot setup and reference](Discord-Status-Bot-Setup-And-Reference)
  - [GLOBALGAMESTATS extension reference](GLOBALGAMESTATS-Extension-Reference)
  - [New player quickstart (player guide)](New-Player-Quickstart-Guide)
  - [Earning funds and score (player guide)](Earning-Funds-And-Score-Player-Guide)
  - [Vehicle service and logistics (player guide)](Vehicle-Service-And-Logistics-Player-Guide)
  - [Commander's handbook (player guide)](Commanders-Handbook)
  - [Tactical support menu](Tactical-Support-Menu-Player-Guide)
  - [Supply missions (player guide)](Supply-Mission-Player-Guide)
  - [In-game briefing & Diary field manual](In-Game-Briefing-And-Diary-Field-Manual)
  - [Playable maps catalog](Playable-Maps-Catalog)
  - [Faction root variables reference](Faction-Root-Variables-Reference)
  - [Faction base structures catalog](Faction-Base-Structures-Catalog)
  - [Counter-battery radar system](Counter-Battery-Radar-System)
  - [Bank, Reserve and Artillery Radar structures](Bank-Reserve-And-Artillery-Radar-Structures)
  - [Map ruleset model and object config](Map-Ruleset-Model-And-Object-Config)
  - [Vehicle countermeasure (flares/spoofing)](Vehicle-Countermeasure-Flares-And-Spoofing)
  - [UAV terminal and spotter system](UAV-Terminal-And-Spotter-System)
  - [Artillery firing function reference](Artillery-Firing-Function-Reference)
  - [Service Point pricing model](Service-Point-Pricing-Model)
  - [Medic redeployment truck (forward spawn)](Medic-Redeployment-Truck-Forward-Spawn)
  - [Side-patrol runtime and convoy mechanics](Side-Patrol-Runtime-And-Convoy-Mechanics)
  - [Day/night cycle and weather system](Day-Night-Cycle-And-Weather-System)
  - [Config lookup helper reference](Config-Lookup-Helper-Reference)
  - [CIPHER sort utilities reference](CIPHER-Sort-Utilities-Reference)
  - [Modded maps status and content](Modded-Maps-Status-And-Content)
  - [BattlEye filter setup and OA taxonomy](BattlEye-Filter-Setup-And-OA-Taxonomy)
  - [Player squad/group join protocol](Player-Squad-Group-Join-Protocol)
  - [AutoFlip vehicle recovery](AutoFlip-Vehicle-Recovery-Reference)
  - [Engine stealth fuel toggle](Engine-Stealth-Fuel-Toggle-Reference)
  - [Valhalla vehicle climbing-assist](Valhalla-Vehicle-Climbing-Assist)

  - [Missile and ordnance Fired-EH reference](Missile-And-Ordnance-Fired-EH-Reference)
  - [Vehicle equip and rearm reference](Vehicle-Equip-And-Rearm-Function-Reference)
  - [Array and collection utilities](Array-And-Collection-Utility-Reference)
  - [Server composition spawner reference](Server-Composition-Spawner-Function-Reference)
  - [Upgrade queue server loop](Upgrade-Queue-Server-Loop-Reference)
  - [Map boundaries and off-map enforcement](Map-Boundaries-And-Offmap-Enforcement)
  - [Namespace/profile/diagnostic utilities](Namespace-Profile-And-Diagnostic-Utility-Reference)
  - [Group bool getVariable A2-OA trap](Group-Bool-GetVariable-A2OA-Trap-Reference)
  - [Vehicle weapon balance init](Vehicle-Weapon-Balance-Init-Reference)
  - [View distance auto-throttle](View-Distance-And-Target-FPS-Auto-Throttle)
  - [Camp & respawn-camp getters](Camp-And-Respawn-Camp-Getter-Reference)
  - [Performance audit writer](Performance-Audit-Writer-Function-Reference)
  - [Site clearance (bulldozer)](Site-Clearance-Commander-Function-Reference)
  - [Factory queue cancel & refund](Factory-Queue-Cancel-And-Refund-Action)
  - [AI commander tunable constants](AI-Commander-Tunable-Constants-Reference)
  - [Experimental feature-flag constants](Experimental-Feature-Flag-Constants-Reference)
  - [Mission tunable constants catalog](Mission-Tunable-Constants-Catalog)
  - [Gear parsing & cargo capacity](Client-Gear-Parsing-And-Cargo-Capacity-Function-Reference)
  - [Structure dressing function](Server-Structure-Dressing-Function-Reference)
  - [Paradrop delivery functions](Server-Paradrop-Delivery-Function-Reference)
  - [ICBM nuke client VFX & radiation](ICBM-Nuke-Client-VFX-And-Radiation-Reference)
  - [Server HandleSpecial router](Server-HandleSpecial-Request-Router-Reference)
  - [LocalizeMessage chat router](LocalizeMessage-Chat-Notification-Router-Reference)
  - [Gear buy-menu render & price functions](Gear-Buy-Menu-Render-And-Price-Function-Reference)
  - [Server broadcast & telemetry loops](Server-Broadcast-And-Telemetry-Loop-Reference)
  - [Per-unit client init pipeline](Per-Unit-Client-Init-Pipeline-Reference)
  - [Vehicle marking & texture pipeline](Vehicle-Marking-And-Texture-Pipeline)
  - [Defense category & budget](Defense-Category-And-Budget-Reference)
  - [Legacy AI order primitives](Legacy-AI-Order-Primitive-Reference)
  - [Commander-team driver](Commander-Team-Driver-Reference)
  - [Static-defense manning](Static-Defense-Manning-Reference)
  - [End-of-game stats screen](End-Of-Game-Stats-Victory-Screen)
  - [AI commander execution loop](AI-Commander-Execution-Loop-Reference)
  - [Deployable bipod / weapon resting](Deployable-Bipod-Weapon-Resting)
  - [Town-economy getters](Town-Economy-Getter-Reference)
  - [Server-init deadspawn & airfield probe](Server-Init-Deadspawn-And-Airfield-Probe)
  - [GUER VBIED detonate action](GUER-VBIED-Detonate-Action)
  - [Resource income-tick engine](Resource-Income-Tick-Distribution-Engine)
  - [AI commander treasury accessors](AI-Commander-Treasury-Fund-Accessors)
  - [PVF send-helper contracts](PVF-Send-Helper-Contract-Reference)
  - [AICOM logging & AICOMSTAT telemetry](AI-Commander-Logging-And-AICOMSTAT-Telemetry)
  - [Group lifecycle & entity reaping](Group-Lifecycle-And-Entity-Reaping)
  - [Batch AI spawner orchestrator](Batch-AI-Spawner-Orchestrator)
  - [Client funds/income HUD readout](Client-Funds-Income-HUD-Readout)
  - [Server group GC & cap warning](Server-Group-GC-Cap-Warning-And-Zombie-Reaper)
  - [Town runtime tuning constants](Town-Runtime-Tuning-Constants)
  - [Client input/hotkey handler](Client-Input-Hotkey-Handler)
  - [WASP base-repair system](WASP-Base-Repair-System-Reference)
  - [WASP DropRPG launcher/ordnance](WASP-DropRPG-Launcher-And-Ordnance-Handler)
  - [CoIn construction-interface client engine](CoIn-Construction-Interface-Client-Engine-Reference)
  - [Town-capture garrison & airfield rebuild](Town-Capture-Garrison-And-Airfield-Rebuild)
  - [Map-control & minimap templates](Map-Control-Template-And-Minimap-Embed-Reference)
  - [Client FPS & state telemetry](Client-FPS-And-State-Telemetry-Reference)
  - [Client service-proximity getters](Client-Service-Structure-Proximity-Getters)
  - [Airfield-exclusive roster & unit hints](Airfield-Exclusive-Roster-And-Special-Unit-Hints)
  - [Unit-camera spectator system](Unit-Camera-Spectator-System-Reference)
  - [Town-garrison patrol/defense worker](Town-Garrison-Patrol-Defense-Worker)
  - [RequestTeamUpdate squad-discipline](Request-Team-Update-Squad-Discipline-Handler)
  - [Arma2Warfare GPT assistant](Arma2Warfare-GPT-Assistant-Reference)
  - [LoadoutManager build configs & defines](LoadoutManager-Build-Configurations-And-Generated-Defines)
  - [GLOBALGAMESTATS extension logging](GLOBALGAMESTATS-Extension-Logging-Subsystem)
  - [Discord bot instrumented logging](Discord-Bot-Instrumented-Logging-Framework)
  - [Eden/Everon & Taviana map content](Eden-Everon-And-Taviana-Map-Content-Reference)
- AI / HC
  - [AI headless and performance](AI-Headless-And-Performance)
  - [Headless client scaling and topology](Headless-Client-Scaling-And-Topology)
  - [AI runtime/HC loop map](AI-Runtime-HC-Loop-Map)
  - [Headless client init and stat loop](Headless-Client-Init-And-Stat-Loop)
  - [HC delegation target selection](Headless-Client-Delegation-Target-Selection)
  - [Player AI caps and role balance](Player-AI-Caps-And-Role-Balance)
  - [Old WarfareBE FPS comparison](Old-WarfareBE-Performance-Comparison)
  - [AI commander autonomy audit](AI-Commander-Autonomy-Audit)
  - [AI commander capture & fun plan](AI-Commander-Capture-And-Fun-Improvement-Plan)
  - [**AI commander B69 improvement roadmap**](AI-Commander-B69-Improvement-Roadmap)
  - [AI commander B69 implementation sketches](AI-Commander-B69-Implementation-Sketches)
  - [Headless delegation and failover](Headless-Delegation-And-Failover-Playbook)
  - [Commander reassignment call shape](Commander-Reassignment-Call-Shape)
- Quality and operations
  - [Feature status](Feature-Status-Register)
  - [Dead/stale code register](Dead-Code-And-Stale-Code-Register)
  - [Commander vote/reassignment](Commander-Vote-And-Reassignment-Playbook)
  - [Attack-wave authority](Attack-Wave-Authority-Playbook)
  - [Server runtime and operations](Server-Runtime-And-Operations)
  - [Server ops runbook](Server-Ops-Runbook)
  - [Server gameplay runtime atlas](Server-Gameplay-Runtime-Atlas)
  - [PerformanceAuditAnalyzer](PerformanceAuditAnalyzer)
  - [Performance opportunity sweep](Performance-Opportunity-Sweep)
  - [Documentation plan](Documentation-Implementation-Plan)
  - [Knowledge platform roadmap](Knowledge-Platform-Roadmap)
  - [Wiki quality audit](Wiki-Quality-Audit)
  - [Wiki pruning and relevance ledger](Wiki-Pruning-And-Relevance-Ledger)
- Hardening and authority
  - [Hardening roadmap](Hardening-Implementation-Roadmap)
  - [ICBM authority](ICBM-Authority-Playbook)
  - [Economy authority first cut](Economy-Authority-First-Cut)
  - [Abandoned feature revival](Abandoned-Feature-Revival-Review)
  - [Pending owner decisions](Pending-Owner-Decisions)
- UI / player workflows
  - [Client UI, HUD and menus](Client-UI-HUD-And-Menus)
  - [UI HUD and dialogs](UI-HUD-And-Dialogs)
  - [Player UI workflow map](Player-UI-Workflow-Map)
  - [Client UI systems atlas](Client-UI-Systems-Atlas)
  - [UI IDD collision repair](UI-IDD-Collision-Repair)
  - [UI control class library reference](UI-Control-Class-Library-Reference)
  - [UI theme palette and style macros](UI-Theme-Palette-And-Style-Macros-Catalog)
  - [Available-actions client gate FSM](Available-Actions-Client-Gate-FSM)
  - [Gear/loadout/EASA atlas](Gear-Loadout-And-EASA-Atlas)
  - [Gear template profile filter](Gear-Template-Profile-Filter)
  - [Vehicle cargo equip loop bounds](Vehicle-Cargo-Equip-Loop-Bounds)
  - [Factory and purchase systems atlas](Factory-And-Purchase-Systems-Atlas)
  - [Service menu affordability guards](Service-Menu-Affordability-Guards)
  - [WASP overlay](WASP-Overlay)
  - [Skin selector and class swap](Skin-Selector-And-Class-Swap-Reference)
  - [Earplugs audio toggle](Earplugs-Audio-Toggle-Reference)
  - [Mission audio catalog](Mission-Audio-Catalog)
  - [HQ radio knowledge-base catalog](HQ-Radio-Knowledge-Base-Conversation-Catalog)
  - [QoL trio player hints](QoL-Trio-Player-Hints-Reference)
  - [Player vehicle/travel actions](Player-Vehicle-And-Travel-Actions-Reference)
- Tooling / release / integrations
  - [Tools and build workflow](Tools-And-Build-Workflow)
  - [Operator monitor and CPU affinity tools](Operator-Monitor-And-CPU-Affinity-Tools-Reference)
  - [Tooling release readiness audit](Tooling-Release-Readiness-Audit)
  - [Source fix propagation queue](Source-Fix-Propagation-Queue)
  - [Agent release readiness ledger](Agent-Release-Readiness-Ledger)
  - [Testing/debugging/release workflow](Testing-Debugging-And-Release-Workflow)
  - [External integrations](External-Integrations)
  - [Integration trust boundary audit](Integration-Trust-Boundary-Audit)
  - [AntiStack database extension audit](AntiStack-Database-Extension-Audit)
- Community & Dev
  - [Community & Dev](Community-And-Dev)
  - [Miksuu upstream wiki import / archive index](Miksuu-Upstream-Wiki-Import)
  - [Upstream changelog feature leads](Upstream-Changelog-Feature-Leads)
  - [Developer history and upstream lessons](Developer-History-And-Upstream-Lessons)
  - [Upstream Miksuu commit intel](Upstream-Miksuu-Commit-Intel)
  - [Upstream mining ledger](Upstream-Mining-Ledger)
  - [PR8 and Drone upstream lesson match](PR8-And-Drone-Upstream-Lesson-Match)
- Compatibility and references
  - [HC upstream history and lessons](HC-Upstream-History-And-Lessons)
  - [Player stats branch audit](Player-Stats-Branch-Audit)
  - [BuyMenu EASA QoL branch audit](BuyMenu-EASA-QoL-Branch-Audit)
  - [Perf quick wins branch audit](Perf-Quick-Wins-Branch-Audit)
  - [Commander positions branch audit](Commander-Positions-Branch-Audit)
  - [Zargabad branch audit](Zargabad-Branch-Audit)
  - [Quad AI Commander concept](Quad-AI-Commander)
  - [Arma 2 OA external reference guide](Arma-2-OA-External-Reference-Guide)
  - [Arma 2 OA compatibility audit](Arma-2-OA-Compatibility-Audit)
  - [Arma 2 OA command versions](Arma-2-OA-Command-Version-Reference)
  - [Wiki source consistency](Wiki-Source-Consistency-Findings)
