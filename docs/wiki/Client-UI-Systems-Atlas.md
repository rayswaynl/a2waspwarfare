...
| EASA/service authority | `GUI_Menu_Service.sqf` and `GUI_Menu_EASA.sqf` are live aircraft/service UI paths. | Service/EASA spend and loadout changes are UI-originated; see [Deep-review findings](Deep-Review-Findings) DR-28, [Gear, loadout and EASA atlas](Gear-Loadout-And-EASA-Atlas) and the local [service menu affordability guards](Service-Menu-Affordability-Guards). |
