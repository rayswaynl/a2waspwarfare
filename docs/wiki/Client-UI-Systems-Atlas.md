...
| --- | --- | --- |
| Docs head `322ef95e` for vote/help/router, `427346b0d` for Economy-control routing, `edbd341e` for duplicate-IDD routing, `0fdd5602` for stale-upgrade dialog and `68bd4dc5` for broader UI anchors | Checked Chernarus and maintained Vanilla UI/Rsc paths are unchanged from `a03a1fb5`, `b5219d47` and the older `a71b42fe`, `2fef1e3d` and `f7bc72a8` atlas/resource snapshots; the stale-upgrade checked paths are also unchanged from `d4cfef80`, Economy controller/resource paths are unchanged from `21f0d53b` / `d2a3f995` / `b5219d47`, and vote/help/router paths are unchanged from `087152f5`. That archived docs/source shape still has visible `MenuAction = 19` as FPS HUD, handler-only `17/18` GPS zoom routes, stale `RscMenu_Upgrade` at `Rsc/Dialogs.hpp:2425,2428`, Economy stale writes to `23004/23005/23006` while `RscMenu_Economy` lacks those controls and `23020`, EASA/Economy dialog-resource drift, duplicate title `10200`, shared `currentCutDisplay` title ownership and malformed `RscClickableText.soundPush[]`. | Use this page for orientation and current docs-source line refs; use [UI resource parity cleanup](UI-Resource-Parity-Cleanup), [UI IDD collision repair](UI-IDD-Collision-Repair), [Commander vote/reassignment](Commander-Vote-And-Reassignment-Playbook) and the clickable-text matrix before making patch-ready claims. Current stable/B74.1 `origin/master@f8a76de34`, current B74.2 `21b62b04`, B69 `8d465fce` and B74 `b23f557f` remove the stale Economy writes, use declared dashboard `23020`, remove the stale old upgrade class and split EASA to `idd = 24000`; current stable also fixes the clickable-text base config. Title IDD/handle, vote/help/router and other branch-scoped UI work remain open. |
| Current stable/B74.1 `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`; current B74.2 `origin/claude/b74.2-aicom@21b62b04` | Current stable/B74.1 maintained roots remove stale `RscMenu_Upgrade`, split EASA to `idd = 24000`, rewrite Economy around declared dashboard control `23020`, expose visible action `19` as GPS enablement and carry valid `RscClickableText.soundPush[] = {"", 0.2, 1};` at `Rsc/Ressources.hpp:556`. Current B74.2 has no checked Economy dialog/resource delta over current stable and no checked `d472da6a..21b62b04` Economy delta. They still keep vote/help cleanup debt, handler-only `17/18` GPS zoom routes, duplicate title IDD and shared `currentCutDisplay` title ownership. Scoped diff `0139a3468609..origin/master` changes only source-Chernarus Economy dashboard label text among checked Economy paths and only `Init_Client.sqf` among checked vote/help/router paths. Current B74.2 has no vote/help/router controller rescue, though source-Chernarus `Init_Client.sqf` now line-drifts through tint-resource and join/connect ACK failover work. | Treat current stable/B74.1/B74.2 as source-present for Economy/resource parity and clickable-text base config only; do not import that status into archived docs/Miksuu/perf/release refs. Title and vote/help/router work remain open, and B74.2 is branch-only source Chernarus work outside these controller/resource clusters. |
