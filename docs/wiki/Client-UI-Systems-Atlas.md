...
Unless a row names another ref, source anchors below are valid for docs head `docs/developer-wiki-index` `68bd4dc5`. Rechecked 2026-06-14: targeted source diffs from `a03a1fb5` and `b5219d47` to `HEAD` over checked Chernarus and maintained Vanilla UI/Rsc paths return no changes, preserving the earlier `a71b42fe`, `2fef1e3d` and `f7bc72a8` source-anchor snapshots. The vote/help/main-menu matrix was separately refreshed on 2026-06-23 at docs head `322ef95e`; checked vote/help/router paths are unchanged from `087152f5` and `68bd4dc5`, while current stable/B74.1 is `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`. Current B74.2 `origin/claude/b74.2-aicom@21b62b04` changes source-Chernarus `Init_Client.sqf` for tint-resource guarding and join/connect ACK failovers, but not the checked vote/help/router controller debt. The Economy dialog/resource route was refreshed on 2026-06-24 at repo docs branch `HEAD@427346b0d`; checked Economy paths are unchanged from `21f0d53b`, `d2a3f995` and `b5219d47`, while current stable/B74.1 `f8a76de34` and current B74.2 `21b62b04` both keep the stable-shaped `23020` dashboard repair in both maintained roots. The stale old upgrade dialog route was refreshed on 2026-06-24 at repo docs branch `HEAD@208062b4e`; checked upgrade dialog/controller paths are unchanged from `427346b0d`, `0fdd5602` and `d4cfef80`, while current stable/B74.1 `f8a76de34`, current B74.2 `21b62b04`, B69 `8d465fce`, B74 `b23f557f` and historical `a96fdda2` have no maintained-root stale `RscMenu_Upgrade` hits. The duplicate-IDD route was refreshed on 2026-06-24 at repo docs branch `HEAD@5990557155b0`; checked Dialogs/Titles IDD paths are unchanged from `edbd341e` / `b5219d47`, while current stable/B74.1 `f8a76de34`, current B74.2 `21b62b04`, B69 `8d465fce`, B74 `b23f557f` and historical `a96fdda2` split EASA to `24000` in both maintained roots. Miksuu `b8389e748243` and `origin/perf/quick-wins` `0076040f` still keep the old duplicate dialog IDD.
...
| --- | --- | --- |
| Docs head `322ef95e` for vote/help/router, `427346b0d` for Economy-control routing, `208062b4e` for stale-upgrade dialog routing, `5990557155b0` for duplicate-IDD routing and `68bd4dc5` for broader UI anchors | Checked Chernarus and maintained Vanilla UI/Rsc paths are unchanged from `a03a1fb5`, `b5219d47` and the older `a71b42fe`, `2fef1e3d` and `f7bc72a8` atlas/resource snapshots; the stale-upgrade checked paths are also unchanged from `427346b0d`, `0fdd5602` and `d4cfef80`, Economy controller/resource paths are unchanged from `21f0d53b` / `d2a3f995` / `b5219d47`, Dialogs/Titles IDD paths are unchanged from `edbd341e` / `b5219d47`, and vote/help/router paths are unchanged from `087152f5`. That archived docs/source shape still has visible `MenuAction = 19` as FPS HUD, handler-only `17/18` GPS zoom routes, stale `RscMenu_Upgrade` at `Rsc/Dialogs.hpp:2425,2428` with missing `GUI_Menu_Upgrade.sqf` and missing `wf_*.paa` icon block, Economy stale writes to `23004/23005/23006` while `RscMenu_Economy` lacks those controls and `23020`, EASA/Economy duplicate `23000`, duplicate title `10200`, shared `currentCutDisplay` title ownership and malformed `RscClickableText.soundPush[]`. | Use this page for orientation and current docs-source line refs; use [UI resource parity cleanup](UI-Resource-Parity-Cleanup), [UI IDD collision repair](UI-IDD-Collision-Repair), [Commander vote/reassignment](Commander-Vote-And-Reassignment-Playbook) and the clickable-text matrix before making patch-ready claims. Current stable/B74.1 `origin/master@f8a76de34`, current B74.2 `21b62b04`, B69 `8d465fce`, B74 `b23f557f` and historical `a96fdda2` remove the stale Economy writes, use declared dashboard `23020`, remove the stale old upgrade class and split EASA to `idd = 24000`; current stable also fixes the clickable-text base config. Title IDD/handle, vote/help/router and other branch-scoped UI work remain open. |
| Current stable/B74.1 `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`; current B74.2 `origin/claude/b74.2-aicom@21b62b04` | Current stable/B74.1 maintained roots remove stale `RscMenu_Upgrade`, split EASA to `idd = 24000`, rewrite Economy around declared dashboard control `23020`, expose visible action `19` as GPS enablement and carry valid `RscClickableText.soundPush[] = {"", 0.2, 1};` at `Rsc/Ressources.hpp:556`. Current B74.2 has no checked stale-upgrade or Economy dialog/resource delta over current stable and no checked `d472da6a..21b62b04` stale-upgrade/Economy delta; both maintained roots keep only live `WFBE_UpgradeMenu` at `Rsc/Dialogs.hpp:4-7`, opened from `GUI_Menu.sqf:190`. They still keep vote/help cleanup debt, handler-only `17/18` GPS zoom routes, duplicate title IDD and shared `currentCutDisplay` title ownership. Scoped diff `0139a3468609..origin/master` is empty for checked stale-upgrade paths, changes only source-Chernarus Economy dashboard label text among checked Economy paths and only `Init_Client.sqf` among checked vote/help/router paths. Current B74.2 has no vote/help/router controller rescue, though source-Chernarus `Init_Client.sqf` now line-drifts through tint-resource and join/connect ACK failover work. | Treat current stable/B74.1/B74.2 as source-present for stale-upgrade deletion, Economy/resource parity and clickable-text base config only; do not import that status into archived docs/Miksuu/perf/release refs. Title and vote/help/router work remain open, and B74.2 is branch-only source Chernarus work outside these controller/resource clusters. |
| B74.2 AICOM branch `origin/claude/b74.2-aicom@21b62b04` | Branch merge-base is current stable/B74.1 `origin/master@f8a76de34`; `gh pr list --head claude/b74.2-aicom --state all` returned `[]`; payload is 29 source Chernarus files / +424 / -38, `git diff --check` clean, and has no `Missions_Vanilla` diff. UI-touching changes include live pop-tier player-AI caps in RHUD (`Client_UpdateRHUD.sqf:354-360`) and Buy Units (`GUI_Menu_BuyUnits.sqf:37-43`), bounded AICOM marker-feed requests (`updateaicommarkers.sqf:47-55`) with targeted server replay (`Init_Server.sqf:927-943`), carrier airfield fallback (`Client_GetClosestAirport.sqf:27-35`), carrier respawn availability/execution (`Client_GetRespawnAvailable.sqf:117-124`; `Client_OnRespawnHandler.sqf:50-61`), default vehicle tints off (`Init_CommonConstants.sqf:840`), tint legend resource guarding (`Init_Client.sqf:351`) and join/connect ACK failovers (`Init_Client.sqf:654-681,693-714`). | Branch-only UI/runtime candidate; route AI/pop-tier through [AI commander autonomy](AI-Commander-Autonomy-Audit#b69-roadmap-and-sketch-route), carrier capture through [Towns/camps](Towns-Camps-And-Capture-Atlas#current-branch-scope), stats through [Player stats branch audit](Player-Stats-Branch-Audit#current-status) and smoke through [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack). Before promotion, smoke JIP marker feed replay, RHUD/buy cap updates across pop tiers, carrier air-shop fallback, carrier deck respawn, tint resource guard, join/connect failover and Chernarus-only versus maintained Vanilla scope. |
| B74.5 primitive roster payload in current `origin/claude/b745@712a7a668` | Branch includes B74.2 as an ancestor and still carries the `4d16fad70` primitive vote-roster fallback only in source Chernarus among the checked vote/JIP paths. `GUI_VoteMenu.sqf:10-35` builds from live `WFBE_Client_Teams` when any team has a player leader, otherwise adds names from `WFBE_JIP_ROSTER_PRIMS` with row value `-1`; `:75-99` hands over once live groups arrive; `:119-125` gates the per-tick name/vote/color update while primitives are visible. `GUI_Commander_VoteMenu.sqf:10-32,69-91` mirrors the primitive build/prune gate. No `WFBE_JIP_ROSTER_PRIMS`, `ROSTER-PUSH` or `CLIENTROSTER` hits were found in `origin/master@f8a76de34`, `origin/claude/b74.2-aicom@21b62b04`, `miksuu/master@b8389e74` or maintained Vanilla Takistan checked paths. The later `b996bcb3..712a7a668` B74.6 delta does not touch the checked vote/JIP fallback paths. | Branch-only source-Chernarus vote/JIP display fallback, not current stable or maintained Vanilla behavior. Route server/JIP plumbing through [Player join/disconnect](Player-Join-Disconnect-And-AntiStack-Lifecycle#branch-intel---b745-primitive-roster-jip), vote outcome caveats through [Commander vote/reassignment](Commander-Vote-And-Reassignment-Playbook#current-branch-scope) and branch smoke through [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack). Smoke late join with delayed/broken `wfbe_teams`, both vote menus, primitive rows staying visible without blank names/RPT spam, and handover from primitive `-1` rows to live team-index rows before treating votes as castable player-team choices. |
| B74.6 client RPT/FPS regression fix `origin/claude/b745@712a7a668` | Current-head-only delta `b996bcb3..712a7a668` is one B74.6 commit, 9 files / +61 / -4 and clean under `git diff --check`. The UI-touching source-Chernarus change guards `Client_UpdateRHUD.sqf:387-389` so `_baseStructures` becomes `[]` when `WFBE_CO_FNC_GetSideStructures` returns nil or a non-array before `forEach _baseStructures` at `:399`. The same delta hardens common helpers used by action/UI scans at `Common_GetSideStructures.sqf:8-11`, `Common_GetFactories.sqf:8-9` and `Common_GetClosestEntity.sqf:13`. Maintained Vanilla receives only the death-camera guard in `Client_OnKilled.sqf`, not these RHUD/common helper guards. | Branch-only client RPT/FPS regression route. Keep RHUD/common-helper smoke with UI/performance owners: join early before side structures replicate, open normal action/RHUD flows, verify no nil-array `forEach`/undefined `_object` RPT spam, and confirm maintained Vanilla/Takistan scope before parity wording. Death-camera guard routes through [Respawn/death lifecycle](Respawn-And-Death-Lifecycle-Atlas#death-handler-responsibilities). |
...
| Trello map-radius circles candidate `origin/claude/trello-map-radius-circles@77dd71ba` / draft PR #78 | Branch merge-base is current stable `origin/master@f8a76de34`; payload is six maintained-root files / +192 / -4 and `git diff --check` clean. In source Chernarus plus maintained Vanilla Takistan, the branch adds `Client/Functions/Client_AmbulanceRedeployCircles.sqf:1-69`, spawns it at `Init_Client.sqf:1227`, and creates client-local yellow `AmbRange_` Ellipse/Border markers around own-side ambulance and redeploy-truck classnames. The range comes from `WFBE_C_RESPAWN_RANGES` at the local `WFBE_UP_RESPAWNRANGE` level and resizes/repositions every 5 seconds. The same branch adds side-colored local `HQBuildRange` Ellipse/Border markers in `Init_BaseStructure.sqf:54-68` (Vanilla `:35-49`) using `WFBE_C_BASE_AREA_RANGE + WFBE_C_BASE_HQ_BUILD_RANGE`, and removes them on structure death at Chernarus `:151-158` / Vanilla `:132-139`. Current stable has the underlying constants/lists and respawn/base validation paths but no maintained-root `Client_AmbulanceRedeployCircles`, `AmbRange_` or `HQBuildRange` symbols. | Branch-only local map-marker/map-indicator candidate, not current stable behavior. Route map ownership through [Client UI systems](Client-UI-Systems-Atlas#current-branch-scope), HQ/build-radius ownership through [Commander/HQ lifecycle](Commander-HQ-Lifecycle-Atlas#current-branch-scope) and [Construction/CoIn systems](Construction-And-CoIn-Systems-Atlas#current-branch-scope), mobile-spawn behavior through [Respawn and death lifecycle](Respawn-And-Death-Lifecycle-Atlas#branch-intel---mobile-respawn-map-circles), redeploy-truck eligibility caveats through [Medic redeployment truck](Medic-Redeployment-Truck-Forward-Spawn#branch-intel---map-radius-circle-candidate), promotion through [Feature status](Feature-Status-Register#partial--deferred--needs-review) and smoke through [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack). |
...
- UI resource parity cleanup owns the branch/root matrix for the stale old upgrade dialog, Economy missing-control writes and duplicate IDDs. Short version after the 2026-06-24 stale-upgrade, Economy and duplicate-IDD refreshes: docs/source, current Miksuu and perf keep the old dialog class, stale Economy-control writes and EASA/Economy duplicate `23000`; current stable/B74.1 `origin/master@f8a76de34`, current B74.2 `21b62b04`, B69 `8d465fce`, B74 `b23f557f` and historical release `a96fdda2` remove the stale old upgrade class, remove the stale Economy writes and use declared dashboard `23020` in both maintained roots. Current stable/B74.1/B74.2/B69/B74/release split EASA to `24000` and keep Economy on `23000`, but still leave `RscOverlay`/`OptionsAvailable` duplicated on `idd = 10200` and sharing the `currentCutDisplay` title-handle path. Use [UI resource parity cleanup](UI-Resource-Parity-Cleanup) before changing these rows, then [UI IDD collision repair](UI-IDD-Collision-Repair) for duplicate-IDD and title-handle patch details.
...
- UI dialog lifecycle scout 2026-06-04 (**RESOLVED, current master f8a76de3**): `GUI_Menu_Economy.sqf:6-8` now resets `MenuAction`, `mouseButtonDown` and `mouseButtonUp` all to `-1` on open, matching comparable map-click dialogs (`GUI_Menu_Command.sqf:3-4`, `GUI_Menu_Tactical.sqf:137-138`, `GUI_Menu_UnitCamera.sqf:8-9`, `GUI_RespawnMenu.sqf:28`). Its map-sell path consumes `mouseButtonUp` at `GUI_Menu_Economy.sqf:107`. The earlier missing-`mouseButtonUp`-reset latch debt no longer exists; no smoke recommendation remains.
...
| Status/debug/admin channels | Audience split seeded / branch scope refreshed | Repo docs branch `HEAD@4a1bc13edde6` remains source-unchanged from `a27086cd` / `d0161083` for checked FPS paths: `Server/GUI/serverFpsGUI.sqf:1,6-7`; `Server/Module/serverFPS/monitorServerFPS.sqf:1,5-6`; `Client/Client_UpdateRHUD.sqf:113,199-201,367-369`; `Common/Functions/Common_PerformanceAudit.sqf:84,160,196,234`; `Client/Module/supplyMission/townSupplyStatus.sqf:1-8`; `Client/PVFunctions/Available.sqf:1`; `Client/FSM/updateclient.sqf:12-24,161-163,230-232`. | Player HUD FPS uses `SERVER_FPS_GUI`; `WFBE_VAR_SERVER_FPS` is a compatibility/public-variable status channel in docs-source; PerformanceAudit is RPT/tooling output; supply/status scripts are transient player/admin signals. Current stable/B74.1/B74.2/B69/B74 and live FPS-named branches use one guarded `SERVER_FPS_GUI` publisher. Route PV semantics through [Networking and public variables](Networking-And-Public-Variables) and FPS/runtime cleanup through [Hosted server FPS loop sleep](Hosted-Server-FPS-Loop-Sleep) plus [Performance opportunity sweep](Performance-Opportunity-Sweep). |
...
| Menu/list icons, affordability colors and resource images | `Rsc/Dialogs.hpp:1171-1232,1525-1614,2110-2123,2372-2378,2638-2831,3463-3469`; `Rsc/Ressources.hpp:92,143,218,300-312,540-556`; `Client/GUI/GUI_Menu.sqf:29-35,47-57,101-170,191-207`; `Client/GUI/GUI_Menu_BuyUnits.sqf:84-90,165-176,237,495-496`; `Client/Functions/Client_UIFillListBuyUnits.sqf:34-62,68-90,105,107`. | Dialog IDC state, factory/unit configs, player funds, queue/build state, upgrade labels and `Client\Images\*.paa/.jpg` resources. | Local player using menus. | Dialog open/refresh loops and list-fill functions; buy-menu tabs refresh while the menu is open. | Dialog close controls clear UI state; resource cleanup belongs to [UI resource parity cleanup](UI-Resource-Parity-Cleanup). | Missing/stale image references can break many menus at once; older branches with malformed `RscClickableText.soundPush[]` need branch-scoped propagation/smoke before reuse. | Current stable/B74.1 `origin/master@f8a76de34`, current B74.2 `origin/claude/b74.2-aicom@21b62b04`, B69/B74 and GUER branch hits keep the clickable-text base-control value fixed in both maintained roots at `Rsc/Ressources.hpp:556`; archived docs/source `HEAD@14d74f9d2`, Miksuu `b8389e748243`, perf `0076040f`, historical `a96fdda2` and UI theme evidence remains branch-scoped old-shape until rechecked or merged. | Open main, buy units, gear, tactical, respawn and upgrade menus; verify icons load, unaffordable states are clear and no missing texture/config errors appear. |
| Status, debug, admin and performance counters | Repo docs branch `HEAD@4a1bc13edde6`: `Server/GUI/serverFpsGUI.sqf:1,6-7`; `Server/Module/serverFPS/monitorServerFPS.sqf:1,5-6`; `Client/Client_UpdateRHUD.sqf:113,199-201,367-369`; `Common/Functions/Common_PerformanceAudit.sqf:84,160,196,234`; `Client/Module/supplyMission/townSupplyStatus.sqf:1-8`; `Client/PVFunctions/Available.sqf:1`; `Client/FSM/updateclient.sqf:12-24,161-163,230-232`. | `diag_fps`, `SERVER_FPS_GUI`, `WFBE_VAR_SERVER_FPS`, PerformanceAudit buffers/flushes, supply-status values and client update loop state. | Split four ways: local player HUD, all-client PV status channels, admin/operator status text, and RPT/tooling consumers. | FPS publisher loops every 8 seconds on dedicated servers, RHUD/client update loops, event-style supply-status scripts and PerformanceAudit flush interval. | PerformanceAudit flushes buffered records; status channels stop with script/runtime termination; `SERVER_FPS_GUI` HUD use is independent from `WFBE_VAR_SERVER_FPS` compatibility publishing. | Do not prune player HUD, compatibility PVs, admin signals and developer RPT counters as one feature. Current docs-source and stable/B74-shaped refs use different FPS publisher shapes, so smoke/consolidation decisions must be branch-scoped. | Docs head `4a1bc13edde6` keeps two guarded publishers in both maintained roots; current stable/B74.1 `origin/master@f8a76de34`, current B74.2 `21b62b04`, B69, B74, client-fps, ASR FPS and historical `a96fdda2` use one guarded `serverFpsGUI.sqf` publisher; Miksuu `b8389e748243` and perf `0076040f` still keep the old two-publisher loop shape. Owner route: [Hosted server FPS loop sleep](Hosted-Server-FPS-Loop-Sleep). | Dedicated FPS-HUD updates, hosted/listen no-spin, `WFBE_VAR_SERVER_FPS` consumer grep before consolidation, supply mission status smoke, and client/server RPT audit collection with the audit parameter enabled. |
...
The status/debug row is not a single UI feature. In current docs-source `HEAD@4a1bc13edde6`, unchanged from `a27086cd` / `d0161083` for the checked FPS paths, RHUD/FPS HUD reads `SERVER_FPS_GUI` at `Client/Client_UpdateRHUD.sqf:113`, and both maintained roots publish it from `Server/GUI/serverFpsGUI.sqf:6-7` after the early `!isDedicated` exit at `:1`. The second docs-source publisher, `Server/Module/serverFPS/monitorServerFPS.sqf:5-6`, still publishes `WFBE_VAR_SERVER_FPS`; no maintained-root player-HUD reader was found in this pass, so treat it as compatibility/status-channel evidence until generated or modded consumers are deliberately retired. Current stable/B74.1/B74.2/B69/B74 and the live FPS-named branches omit that monitor publisher in maintained roots, so target-branch smoke decides the expected contract.
...
The more direct title lifecycle bug is independent of duplicate IDDs. `EndOfGameStats` has `idd=90000`, but it sets and clears `uiNamespace["currentCutDisplay"]` through the same `onLoad`/`onUnload` helpers as `OptionsAvailable` (`Titles.hpp:532-540` in docs/Miksuu/perf, `:580-588` in current stable/B74.1/B74.2/B69/B74 and historical release). `GUI_EndOfGameStats.sqf:13,34-44,86-93` cuts and writes stat bars through that key on docs/Miksuu/perf/Vanilla stable-shaped roots; current stable/B74.1/B74.2 source Chernarus line-drifts to `:15,36-46,88-95` because of the GUER-label addition. The RHUD loop keeps running and can re-cut `OptionsAvailable` when the shared key is null (`Client_UpdateRHUD.sqf:89-92,190` in docs/Miksuu/perf; `:89-92,266` in current stable/B74.1/B74.2/B69/B74; `:89-92,258` in historical release). The 2026-06-23 B74.2 current-head follow-up found `d472da6a..21b62b04` empty for checked title-handle paths; the stable-to-B74.2 diff changes source-Chernarus RHUD AI-cap text only, not the title handle. All checked maintained roots keep this shared-handle shape; patch title work by splitting display ownership or by gating RHUD/action-icon recreation once endgame begins. Use [UI IDD collision repair](UI-IDD-Collision-Repair) for the patch checklist instead of duplicating it here.
...
`GUI_Menu_Command.sqf` owns team selection, AI team templates, behavior/combat/formation/speed combos, move/task controls and respawn factory choice. Current stable/B74.1 `origin/master@f8a76de34`, current B74.2 `origin/claude/b74.2-aicom@21b62b04`, current B69 `origin/claude/b69@8d465fce`, adjacent B74 `origin/claude/b74-aicom-spend@b23f557f` and live `origin/feat/naval-hvt-objectives@2e1c59317186` carry the targeted Objective Ping path in both maintained roots: the menu gathers task data and HQ speech context at `GUI_Menu_Command.sqf:315-344`, then sends `SetTask` to the selected leader or side leaders at `:336,344` and registers the client handler at `Init_PublicVariables.sqf:40`. Repo docs branch `HEAD@c5937045f`, Miksuu `b8389e748243`, `origin/perf/quick-wins@0076040f` and historical `c20ce153` still keep those sends commented at `GUI_Menu_Command.sqf:335,337,343` while registering `SetTask` at `:33`; historical `a96fdda2` has live sends with `:36` registration but no live `release/*` head. The old town `TaskSystem` remains disabled/residue and separate from Objective Ping; current stable/B74.1/B74.2/B69/B74/naval-HVT remove `Client_TaskSystem.sqf` while docs/Miksuu/perf/c20 keep it.

Vote menu cleanup edge: `WFBE_Client_Teams_Count` is initialized as `count WFBE_Client_Teams` (`Init_Client.sqf:403` on current stable/B74.1 `origin/master@f8a76de34`, then refreshed from reconciled side teams at `:534`; current B74.2 `origin/claude/b74.2-aicom@21b62b04` line-drifts source Chernarus to `:409,:540` while maintained Vanilla stays `:403,:534`), but `GUI_Commander_VoteMenu.sqf:58-66` and `GUI_VoteMenu.sqf:29,61-66` use it as an inclusive `for ... to` maximum. Existing `isNil` guards make this mostly a refresh-loop polish bug rather than a proven crash, but vote UI changes should switch to `(WFBE_Client_Teams_Count - 1)` or `forEach`.
...
Help dialog lifecycle edge: `RscMenu_Help` stores the display as `uiNamespace["dialog_HelpPanel"]` on load, but unload clears `uiNamespace["cti_dialog_ui_onlinehelpmenu"]` and calls `GUI_Menu_Help.sqf` with `onUnload` (`Rsc/Dialogs.hpp:3446-3447` on docs head `322ef95e`; current stable/B74.1/B74.2/B69/B74 use `:3172-3173`; historical `a96fdda2` uses `:3108-3109`). The controller only implements `onLoad` and `onHelpLBSelChanged` (`GUI_Menu_Help.sqf:5-10`). This looks like stale namespace wiring, so avoid building new help-panel state on the old unload variable without cleaning it.

Main-menu orphan route: docs/source `HEAD@72007de93f00` keeps the `322ef95e` / `087152f5` / `68bd4dc5` line shape and still handles `MenuAction == 17/18` for GPS zoom at `GUI_Menu.sqf:202,206`, while the audited `WF_Menu` resource block exposes actions `1-13`, `16` and `19` only (`Rsc/Dialogs.hpp:1226` emits `MenuAction = 19`). Current stable/B74.1 `origin/master@f8a76de34`, current B74.2 `origin/claude/b74.2-aicom@21b62b04`, current B69 `8d465fce` and adjacent B74 `b23f557f` keep handler-only zoom routes at `GUI_Menu.sqf:270,274` but reuse the visible `MenuAction = 19` control as GPS enablement (`Rsc/Dialogs.hpp:1244,1252`; `GUI_Menu.sqf:241`). Current stable/B74.1/B69/B74 refresh `WFBE_Client_Teams_Count` from reconciled side teams at `Init_Client.sqf:534`; current B74.2 line-drifts source Chernarus to `:540`, but it remains `count`, not a maximum index. Miksuu `b8389e74` and perf `0076040f` keep the docs-checkout FPS-HUD meaning for visible action `19`; historical `a96fdda2` matches the stable GPS action shape with Chernarus/Vanilla line drift. Treat the old zoom router cases as dead UX baggage unless a hidden/branch control is deliberately reintroduced and smoke-tested.
...
| --- | --- | --- | --- | --- |
| Docs/source `HEAD@72007de93f00` | Both maintained roots are unchanged from `322ef95e`, `087152f5` and `68bd4dc5` for checked vote/help/router paths: `WFBE_Client_Teams_Count = count WFBE_Client_Teams` at `Init_Client.sqf:273`; `GUI_VoteMenu.sqf:29,49,61` and `GUI_Commander_VoteMenu.sqf:58` loop through that inclusive value; row coloring still writes `[_i+1,0]` at `GUI_VoteMenu.sqf:80,82`. | Both maintained roots set `dialog_HelpPanel` and clear `cti_dialog_ui_onlinehelpmenu` at `Rsc/Dialogs.hpp:3446-3447`; `GUI_Menu_Help.sqf:6,10` handles load and selection, not unload. | Both roots keep handler-only `MenuAction == 17/18` zoom at `GUI_Menu.sqf:202,206`; visible `MenuAction = 19` is `CA_FPSHUD_Button` at `Rsc/Dialogs.hpp:1219-1227` and toggles `RUBFPSHUD` at `GUI_Menu.sqf:197`. | Patch-ready UI cleanup; checked docs-source paths are unchanged from the prior matrix. |
| Current stable/B74.1 `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`, current B74.2 `origin/claude/b74.2-aicom@21b62b04`, current B69 `8d465fce` and adjacent B74 `b23f557f` | Both maintained roots keep the same inclusive vote-loop and row-color shape. Current stable/B74.1/B69/B74 use `Init_Client.sqf:403` plus a reconciled team refresh at `:534`; current B74.2 line-drifts source Chernarus only to `:409,:540` while maintained Vanilla stays `:403,:534`; `GUI_VoteMenu.sqf:29,49,61,80,82` and `GUI_Commander_VoteMenu.sqf:58` are unchanged. | Both maintained roots set `dialog_HelpPanel` and clear `cti_dialog_ui_onlinehelpmenu` at `Rsc/Dialogs.hpp:3172-3173`; `GUI_Menu_Help.sqf:6,10` still has no `onUnload` case. | Both maintained roots keep `MenuAction == 17/18` handlers at `GUI_Menu.sqf:270,274`; visible `MenuAction = 19` is `CA_GPS_Button` at `Rsc/Dialogs.hpp:1244-1252` and GPS enablement at `GUI_Menu.sqf:241`. `d472da6a..21b62b04` and `origin/master..origin/claude/b74.2-aicom` touch source-Chernarus `Init_Client.sqf` only among checked paths; the delta is vehicle-tint guard, join/connect ACK failover and intro-music hook, not vote/help/router logic. B69..B74 checked paths are empty. | Current stable/B74.1/B74.2/B69/B74 change the visible action-19 meaning versus docs/Miksuu/perf, but do not rescue vote loops, row color, help unload or old zoom-router cleanup. Live `origin/claude/trello-vote-para-guards@92101abe` is server vote/paratrooper/Tactical context and has no checked vote/help/main-menu-router path delta. |
| B74.5 payload in current `origin/claude/b745@712a7a668` | Source Chernarus `GUI_VoteMenu.sqf:10-35,75-99,119-125` adds primitive name rows from `WFBE_JIP_ROSTER_PRIMS` when live `wfbe_teams` group objects are not usable; primitive rows use value `-1`, skip live prune/update loops, then rebuild from live team indexes once player-led groups arrive. `GUI_Commander_VoteMenu.sqf:10-32,69-91` mirrors the same display fallback for commander-side reassignment. The `b996bcb3..712a7a668` B74.6 delta does not touch these checked vote/help/router paths. | No checked help-menu lifecycle delta over B74.2. | No checked main-menu `17/18/19` router delta over B74.2. | Branch-only Chernarus late-JIP display fallback. It does not close the older stable vote-loop/row-color cleanup, help unload mismatch or GPS router debt, and it must not be described as current stable or Vanilla behavior. |
...
| Historical release commit `a96fdda2` | Same vote shapes in both maintained roots at `Init_Client.sqf:288`, `GUI_VoteMenu.sqf:29,49,61` and `GUI_Commander_VoteMenu.sqf:58`. | Same mismatch in both maintained roots at `Rsc/Dialogs.hpp:3108-3109`. | The historical release commit exposes `CA_GPS_Button` / `MenuAction = 19` at `Rsc/Dialogs.hpp:1231-1240`, but the old `17/18` handlers remain at `GUI_Menu.sqf:243,247` in Chernarus and `:235,239` in Vanilla. | Historical release matches stable GPS visibility but does not rescue vote/help or old zoom-router cleanup; current origin exposes no live `release/*` head in the checked branch scan. |
...
| Objective Ping versus old town tasks | Current stable/B74.1 `origin/master@f8a76de34`, current B74.2 `origin/claude/b74.2-aicom@21b62b04`, B69/B74 and `origin/feat/naval-hvt-objectives` send `SetTask` at `GUI_Menu_Command.sqf:336,344`; docs/source `HEAD@c5937045f`, Miksuu, perf and historical `c20ce153` still comment those sends at `:335,:337,:343`; historical `a96fdda2` has live sends but no live `release/*` head. | Treat Objective Ping as source-present / smoke-pending only on the current stable-shaped branches. Keep old town-task revival separate and JIP/task-spam gated. |
...
Fast travel is a local Tactical menu flow, not a `RequestSpecial` server asset path. The menu reads `WFBE_C_GAMEPLAY_FAST_TRAVEL` and fast-travel range constants at `GUI_Menu_Tactical.sqf:22-23`; current stable/B74.1 `origin/master@f8a76de34` equals `origin/claude/b74.1-aicom@f8a76de34`, defaults to `1` free travel and uses `2` for fee mode at source Chernarus `Common/Init/Init_CommonConstants.sqf:612,621-624` and maintained Vanilla `:414,423-426`. In fee mode, the destination refresh calculates `_fee` from distance and `WFBE_C_GAMEPLAY_FAST_TRAVEL_PRICE_KM`, hides towns the player cannot afford, draws marker-text price labels, and later debits locally when the player clicks the travel destination (`GUI_Menu_Tactical.sqf:185,195-196,215,403-404` in both maintained roots). Current B74.2 `origin/claude/b74.2-aicom@21b62b04` has no checked Tactical GUI delta and only line-drifts source Chernarus constants to `Init_CommonConstants.sqf:625,634-637`; checked `d472da6a..21b62b04` is empty for the four maintained-root fast-travel paths. The old `//--- TODO: Travel fee...` comment is absent on current stable/B74.1/B74.2/B69/B74 but still present in docs/source `HEAD@003d0c4c044e`, current Miksuu, perf and historical release refs, so treat it as stale policy/UX cleanup evidence in those targets rather than proof that fast travel is absent.

| Root / branch | Fee-mode destination list | Debit / prompt shape | Status |
| --- | --- | --- | --- |
| Current stable/B74.1 `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34` | Both maintained roots hide unaffordable town destinations and draw marker text with `$fee` (`GUI_Menu_Tactical.sqf:185,195-196,215`). | Both maintained roots recalculate and locally call `ChangePlayerFunds` at `:403-404`; no confirmation prompt or second affordability recheck is present. The old completed TODO is absent. | Docs-ready owner-decision cleanup; do not reopen stale TODO removal on current stable/B74.1. |
| Current B74.2 `origin/claude/b74.2-aicom@21b62b04` | No checked Tactical GUI delta versus current stable/B74.1; source Chernarus constants drift to `Init_CommonConstants.sqf:625,634-637` while maintained Vanilla stays current-stable shaped. Checked `d472da6a..21b62b04` is empty for the four maintained-root fast-travel paths. | Same local debit shape at `GUI_Menu_Tactical.sqf:403-404`; `origin/master..origin/claude/b74.2-aicom` only touches source Chernarus `Init_CommonConstants.sqf` among checked fast-travel paths. | Branch-visible line drift only for this lane; no fast-travel UX rescue. |
| B69 `origin/claude/b69@8d465fce` and B74 `origin/claude/b74-aicom-spend@b23f557f` | Same paid-mode Tactical behavior as current stable, with source Chernarus constants at B69 `Init_CommonConstants.sqf:580,589-592` and B74 `:593,602-605`. | Same local debit shape at `GUI_Menu_Tactical.sqf:403-404`; checked B69..B74 fast-travel path delta only touches source Chernarus constants. | Adjacent B74-era refs are source-present for the same UX debt, not fixes. |
| Docs/source `HEAD@003d0c4c044e`, current Miksuu `b8389e748243`, perf `0076040f` and historical release `a96fdda2` | Same maintained-root behavior with branch line drift; docs/source is unchanged from `1154f5d43` for the checked fast-travel path set and keeps constants at `Init_CommonConstants.sqf:218,227-230`, while historical release constants sit at `:231,240-243`. Docs/source, Miksuu, perf and release keep the old TODO at `GUI_Menu_Tactical.sqf:147`. | Same local debit shape at `GUI_Menu_Tactical.sqf:404-405`; adjacent support debits drift between refs, but they are not the fast-travel debit. | No checked old-shape ref rescues the fee-mode UX/policy debt; current origin exposes no live `release/*` or fast-travel feature heads on 2026-06-24. |
...
`RscClickableText` is a base UI resource class. B74.2 current-head follow-up 2026-06-24: docs/source `HEAD@14d74f9d2` is unchanged from `1d4282eca8fd` / `0bb0f89f` / `b5219d47` for checked resource/dialog/title paths and still carries the malformed base value. Current stable/B74.1 `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`, current B74.2 `origin/claude/b74.2-aicom@21b62b04`, current B69 `origin/claude/b69@8d465fce`, adjacent B74 `origin/claude/b74-aicom-spend@b23f557f` and the live GUER branch hits define `class RscClickableText` at `Rsc/Ressources.hpp:541` and the valid empty-sound `soundPush[] = {"", 0.2, 1};` at `:556` in both maintained roots, matching the older valid precedent at `:92`. Chernarus received the stable fix in `1a5e0b40`; maintained Vanilla received the propagated generated-file fix in `9b49883c`. Checked `d472da6a..21b62b04`, `origin/master..origin/claude/b74.2-aicom` and B69..B74 resource/dialog/title deltas are empty; GUER branch hits touch `Dialogs.hpp` / `Titles.hpp` only and retain the valid base value. The blast radius remains branch-sensitive because docs/source, current Miksuu, perf, historical release and UI theme checkpoints carry the malformed base value, and `Rsc/Dialogs.hpp` derives 14 controls from the base class in current stable/B74.1/B74.2/B69/B74/GUER branch hits while old-shape checked branches derive 17 controls.

| Root / branch | `RscClickableText.soundPush[]` | Derived clickable controls | Status |
| --- | --- | --- | --- |
| Docs/source `HEAD@14d74f9d2` Chernarus + maintained Vanilla | `Rsc/Ressources.hpp:556` keeps `{, 0.2, 1}` in both maintained roots; `:92` shows valid `{"", 0.2, 1}` precedent. Checked `Rsc/Ressources.hpp` / `Rsc/Dialogs.hpp` / `Rsc/Titles.hpp` paths are unchanged from `1d4282eca8fd`, `0bb0f89f`, `b5219d47` and the earlier `a71b42fe` / `f7bc72a8` matrix snapshots. | 17 `: RscClickableText` controls in each `Rsc/Dialogs.hpp` root, including gear slots, WF menu toggles, construction/CoIn, artillery and economy controls. | Patch-ready, docs/source-unpatched; propagate Vanilla with any source edit. |
| Current stable/B74.1 `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`, current B74.2 `origin/claude/b74.2-aicom@21b62b04`, current B69 `8d465fce`, adjacent B74 `b23f557f` and GUER branch hits `c8ef67d8` / `0f53f34d` Chernarus + maintained Vanilla | Fixed/source-present in both maintained roots: `Rsc/Ressources.hpp:556` keeps `{"", 0.2, 1}`. The old malformed value is absent from the checked maintained roots; checked `0139a3468609..origin/master`, `origin/master..origin/claude/b74.2-aicom`, `d472da6a..origin/claude/b74.2-aicom` and `origin/claude/b69..origin/claude/b74-aicom-spend` resource/dialog/title deltas are empty. GUER branch hits only change adjacent `Dialogs.hpp` / `Titles.hpp` surfaces among checked files and retain the valid base value. | 14 derived controls in each maintained root at `Rsc/Dialogs.hpp:670,710,742,817,868,1194,1204,1214,1587,1642,2186,2448,2828,3133`. | Current stable/B74.1/B74.2/B69/B74/GUER branch hits no longer need the base config patch; keep propagation/recheck work branch-scoped and run representative Arma 2 OA dialog smoke before release-validated wording. |
| Current Miksuu `master` (`b8389e748243`) | Same malformed base-class value in both maintained roots. | 17 derived controls in both maintained roots, with line drift from docs/source. | Direct `git ls-remote` rechecked this as the current Miksuu head; no upstream rescue. |
| `origin/perf/quick-wins` (`0076040f`) | Same malformed base-class value in both maintained roots. | 17 derived controls in both maintained roots. | Old-shape branch and not a rescue for the base config. |
| Historical release `a96fdda2`; no live `release/*` head exposed on 2026-06-24 | Same malformed base-class value in both maintained roots. | 14 derived controls in each maintained release root at `Rsc/Dialogs.hpp:657,697,729,804,855,1181,1191,1201,1537,1592,2122,2384,2764,3069`. | Historical release evidence does not rescue the base config. |
...
Practical patch rule: for current stable/B74.1/B74.2/B69/B74/GUER branch hits, do not reopen the base-class edit; it is already source-present in Chernarus and maintained Vanilla. For older branches that still carry `{, 0.2, 1}`, change only the base-class array to the valid local empty-sound precedent `{"", 0.2, 1}` unless a code owner wants a real click sound, then propagate maintained Vanilla deliberately. In all cases, run an Arma 2 OA dialog smoke that opens representative inheritors from gear, WF menu/HUD toggles, construction/CoIn, artillery and economy; treat modded mission copies as unsupported until their broader conflict/generation blockers are resolved.
