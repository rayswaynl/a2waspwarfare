...
| Current stable/B74.1 `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`; B74.2 `origin/claude/b74.2-aicom@d472da6a` | Both maintained roots remove stale `RscMenu_Upgrade`, split EASA to `idd = 24000`, rewrite Economy around the current dashboard controls, expose visible action `19` as GPS enablement and carry valid `RscClickableText.soundPush[] = {"", 0.2, 1};` at `Rsc/Ressources.hpp:556`. They still keep vote/help cleanup debt, handler-only `17/18` GPS zoom routes, duplicate title IDD and shared `currentCutDisplay` title ownership. Scoped diff `0139a3468609..origin/master` changes only `Init_Client.sqf` among checked vote/help/router paths, and `origin/master..origin/claude/b74.2-aicom` is empty for those paths. | Treat current stable/B74.1 as source-present for resource parity and clickable-text base config only; do not import that status into archived docs/Miksuu/perf/release refs. Title and vote/help/router work remain open, and B74.2 is branch-only source Chernarus work outside this vote/help/router cluster. |
| B74.2 AICOM branch `origin/claude/b74.2-aicom@d472da6ae` | Branch merge-base is current stable/B74.1 `origin/master@f8a76de34`; `gh pr list --head claude/b74.2-aicom --state all` returned `[]`; payload is source Chernarus only and has no `Missions_Vanilla` diff. UI-touching changes include live pop-tier player-AI caps in RHUD (`Client_UpdateRHUD.sqf:354-360`) and Buy Units (`GUI_Menu_BuyUnits.sqf:37-43`), bounded AICOM marker-feed requests (`updateaicommarkers.sqf:47-55`) with targeted server replay (`Init_Server.sqf:921-930`), carrier airfield fallback (`Client_GetClosestAirport.sqf:27-35`), carrier respawn availability/execution (`Client_GetRespawnAvailable.sqf:117-124`; `Client_OnRespawnHandler.sqf:50-61`) and default vehicle tints off (`Init_CommonConstants.sqf:840`). | Branch-only UI/runtime candidate; route AI/pop-tier through [AI commander autonomy](AI-Commander-Autonomy-Audit#b69-roadmap-and-sketch-route), carrier capture through [Towns/camps](Towns-Camps-And-Capture-Atlas#current-branch-scope), stats through [Player stats branch audit](Player-Stats-Branch-Audit#current-status) and smoke through [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack). Before promotion, smoke JIP marker feed replay, RHUD/buy cap updates across pop tiers, carrier air-shop fallback, carrier deck respawn and Chernarus-only versus maintained Vanilla scope. |
