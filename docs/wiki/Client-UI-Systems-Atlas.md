...
| --- | --- | --- | --- |
| Docs checkout `b5219d47` Chernarus + maintained Vanilla | `Rsc/Ressources.hpp:556` keeps `{, 0.2, 1}` in both maintained roots; `:92` shows valid `{"", 0.2, 1}` precedent. Source unchanged from the earlier `a71b42fe` / `f7bc72a8` matrix snapshots for checked UI paths. | 17 `: RscClickableText` controls in each `Rsc/Dialogs.hpp` root, including gear slots, WF menu toggles, construction/CoIn, artillery and economy controls. | Patch-ready, docs/source-unpatched; propagate Vanilla with any source edit. |
