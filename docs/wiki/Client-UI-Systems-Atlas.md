...
| Stale old upgrade dialog | DR-24: `RscMenu_Upgrade` references missing `Client/GUI/GUI_Menu_Upgrade.sqf`; its old icon controls also point at missing `Client\Images\wf_*.paa` assets. Live flow uses `WFBE_UpgradeMenu`. | Treat `RscMenu_Upgrade` as dead/stale unless a later pass proves a dynamic opener and restores or removes the missing asset references. |
