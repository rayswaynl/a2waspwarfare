...
Main-menu orphan route: docs head `322ef95e` keeps the `087152f5` / `68bd4dc5` line shape and still handles `MenuAction == 17/18` for GPS zoom at `GUI_Menu.sqf:202,206`, while the audited `WF_Menu` resource block exposes actions `1-13`, `16` and `19` only (`Rsc/Dialogs.hpp:1226` emits `MenuAction = 19`). Current stable/B74.1 `origin/master@f8a76de34`, B74.2 `d472da6a`, current B69 `8d465fce` and adjacent B74 `b23f557f` keep handler-only zoom routes at `GUI_Menu.sqf:270,274` but reuse the visible `MenuAction = 19` control as GPS enablement (`Rsc/Dialogs.hpp:1244,1252`; `GUI_Menu.sqf:241`). Current stable/B74.1/B69/B74 also refresh `WFBE_Client_Teams_Count` from reconciled side teams at `Init_Client.sqf:534`, but that remains `count`, not a maximum index. Miksuu `b8389e74` and perf `0076040f` keep the docs-checkout FPS-HUD meaning for visible action `19`; historical `a96fdda2` matches the stable GPS action shape with Chernarus/Vanilla line drift. Treat the old zoom router cases as dead UX baggage unless a hidden/branch control is deliberately reintroduced and smoke-tested.
