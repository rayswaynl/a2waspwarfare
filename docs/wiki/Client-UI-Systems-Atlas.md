...
| Visible B74.2 `origin/claude/b74.2-aicom@d472da6a` | No checked Tactical GUI delta versus current stable/B74.1; source Chernarus constants drift to `Init_CommonConstants.sqf:625,634-637` while maintained Vanilla stays current-stable shaped. | Same local debit shape at `GUI_Menu_Tactical.sqf:403-404`; the B74.2 scoped diff only touches source Chernarus `Init_CommonConstants.sqf` among checked fast-travel paths. | Branch-visible line drift only for this lane; no fast-travel UX rescue. |
...
| Docs/source Tactical GUI `e82c67547`, constants `1154f5d43`, current Miksuu `b8389e748243`, perf `0076040f` and historical release `a96fdda2` | Same maintained-root behavior with branch line drift; docs/source constants sit at `Init_CommonConstants.sqf:218,227-230`, historical release constants at `:231,241-243`. Docs/source, Miksuu, perf and release keep the old TODO at `GUI_Menu_Tactical.sqf:147`. | Same local debit shape at `GUI_Menu_Tactical.sqf:404-405`; adjacent support debits drift between refs, but they are not the fast-travel debit. | No checked old-shape ref rescues the fee-mode UX/policy debt; current origin exposes no live `release/*` or fast-travel feature heads on 2026-06-23. |
