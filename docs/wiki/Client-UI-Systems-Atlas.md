...
| Indicator family | Initial source anchors | Exploration to do |
| --- | --- | --- |
| HUD/title indicators | `Rsc/Titles.hpp`; `Client/Client_UpdateRHUD.sqf`; `Client/FSM/updateavailableactions.fsm`; `Client/FSM/client_title_capture.sqf`; `Client/GUI/GUI_EndOfGameStats.sqf`; `Client/Module/CoIn/coin_interface.sqf` | Inventory RHUD, FPS-only HUD, action icons, capture bar, end stats and CoIn placement feedback. Verify ownership of `currentCutDisplay`, visibility toggles, overlap risk and which indicators are user-facing versus debug/admin. |
| Map and tactical markers | `Client/Init/Init_Markers.sqf`; `Client/FSM/updatetownmarkers.sqf`; `Client/FSM/updateteamsmarkers.sqf`; `Common/Common_MarkerUpdate.sqf`; `Common/Common_AARadarMarkerUpdate.sqf`; `Client/GUI/GUI_Menu_Tactical.sqf`; `Client/GUI/GUI_RespawnMenu.sqf` | Build a marker inventory for towns, camps, teams, HQs, AARadar aircraft, artillery/ICBM/fast-travel rings, respawn selection and commander orders. Record audience, JIP behavior, cleanup owner, update rate and performance counters. |
| Support and special markers | `Client/PVFunctions/HandleParatrooperMarkerCreation.sqf`; `Server/Support/Support_Paratroopers.sqf`; `Server/Module/MASH/MASHMarker.sqf`; `Client/Module/MASH/receiverMASHmarker.sqf`; `Client/Functions/Client_FNC_Special.sqf`; `Client/Functions/Client_RequestFireMission.sqf` | Reconcile working, partial and dead support indicators: paratrooper drop markers, MASH marker relay, UAV spotted areas, fire-mission/ICBM markers and marker-deletion timers. Keep this tied to [Abandoned feature revival](Abandoned-Feature-Revival-Review) and [Support specials](Support-Specials-And-Tactical-Modules-Atlas). |
| Menu/list icons and affordance indicators | `Rsc/Dialogs.hpp`; `Client/GUI/GUI_Menu.sqf`; `Client/GUI/GUI_Menu_BuyUnits.sqf`; `Client/Functions/Client_UIFillListBuyUnits.sqf`; `Client/GUI/GUI_Menu_EASA.sqf`; `Client/GUI/GUI_UpgradeMenu.sqf`; `Client/Functions/Client_UI_Gear_*` | Audit factory tab icons, crew/lock icons, gear slots/templates, upgrade images/progress text, EASA current-loadout highlighting, affordability colors and queue-count labels. Include branch-only `feat/buymenu-easa-qol` evidence but keep branch status explicit. |
| Status channels and debug/admin indicators | `Server/GUI/serverFpsGUI.sqf`; `Server/Module/serverFPS/monitorServerFPS.sqf`; `Client/Client_UpdateRHUD.sqf`; `Common/Functions/Common_PerformanceAudit.sqf`; `Client/Module/supplyMission/townSupplyStatus.sqf`; `Client/PVFunctions/Available.sqf` | Decide which status surfaces are useful for public players, admins or only developers: server FPS, client FPS, performance-audit marker counters, supply/town status and available-action hints. Check for duplicate publishers and orphan consumers. |
| Asset and path integrity | `Client/Images/*`; `Rsc/Dialogs.hpp:2628-2821`; `Rsc/Ressources.hpp:540-563`; `Common/Config/Core_Upgrades/Labels_Upgrades.sqf` | Verify every local `Client\Images\*.paa/.jpg` reference exists in Chernarus and maintained Vanilla. Known starting risks: missing `Client\Images\wf_*.paa` tactical/upgrade icons and malformed `RscClickableText.soundPush[]` are already documented; fold them into the indicator audit instead of rediscovering them. |

Suggested output for this lane: one compact indicator matrix with `surface`, `owner script`, `state source`, `audience`, `update cadence`, `cleanup path`, `known risk`, `branch scope` and `smoke target`. Do not turn this into a visual redesign until the runtime ownership map exists.
...
| Shared title lifecycle handle | `OptionsAvailable` and `EndOfGameStats` both set/clear `uiNamespace["currentCutDisplay"]` from their `onLoad`/`onUnload` scripts; current source/Vanilla, stable/upstream and release all keep the shared key. RHUD/action-icon recovery can recreate `OptionsAvailable` while endgame stats writes through the same key. | Split title display variables or gate RHUD/action-icon recreation during endgame; smoke RHUD/FPS, action icons and endgame stat bars together. |
| Malformed clickable-text base control config | [Clickable text soundPush branch matrix](Client-UI-Systems-Atlas#clickable-text-soundpush-branch-matrix) owns the current branch/root check. Every checked maintained root/branch keeps `RscClickableText.soundPush[] = {, 0.2, 1};` at `Rsc/Ressources.hpp:556`, while the same file shows the valid empty-sound shape `{"", 0.2, 1}` at `:92`. | Patch-ready config cleanup. Fix the base class before deriving new clickable controls from it; then smoke representative gear, WF menu/HUD toggle, CoIn/construction, artillery and economy clickable controls in Arma 2 OA. |
...
| Tactical fast-travel fee UX | [Tactical fast-travel fee branch matrix](Client-UI-Systems-Atlas#tactical-fast-travel-fee-branch-matrix) owns the current branch/root check. Every checked maintained root/branch keeps the fee-mode TODO, hidden unaffordable destinations, marker-only fee text and local action-time debit. | Functional but partial UX. Decide whether paid destinations should be shown disabled/explained and whether a confirmation/action-time recheck is needed before debiting. |
...
| --- | --- | --- | --- |
| Current docs/source Chernarus (`HEAD` `ad21f967`) | `GUI_Menu_Tactical.sqf:185-199` hides unaffordable town destinations and `:208-217` draws marker text with `$fee`. | `:403-406` recalculates and locally calls `ChangePlayerFunds`; no confirmation prompt or second affordability recheck is present. | Docs-ready owner-decision cleanup. |
| Maintained Vanilla Takistan (`HEAD` `ad21f967`) | Same maintained-root shape. | Same local debit shape. | Keep in parity with any source Chernarus UX/policy change. |
| Stable `origin/master` (`2cdf5fb8`) and Miksuu upstream (`f532f706`) | Same maintained-root shape in Chernarus and Vanilla. | Same local debit shape. | No stable/upstream rescue. |
| `origin/perf/quick-wins` (`0076040f`) | Same maintained-root shape in Chernarus and Vanilla. | Same local debit shape. | Perf branch does not touch this Tactical UX. |
| Release `origin/release/2026-06-feature-bundle` (`3282ff3f`) | Same maintained-root shape in Chernarus and Vanilla. | Same local debit shape. | Release branch does not rescue this lane. |
...
| --- | --- | --- | --- |
| Current docs/source Chernarus (`HEAD` `47eebde2`) | `Rsc/Ressources.hpp:556` keeps `{, 0.2, 1}`; `:92` shows valid `{"", 0.2, 1}` precedent. | 17 `: RscClickableText` controls in `Rsc/Dialogs.hpp`. | Patch-ready, current-source-unpatched. |
| Maintained Vanilla Takistan (`HEAD` `47eebde2`) | Same malformed base-class value at `Rsc/Ressources.hpp:556`. | Same 17 derived clickable controls. | Needs propagation with Chernarus. |
| Stable `origin/master` (`2cdf5fb8`) and Miksuu upstream (`f532f706`) | Same malformed base-class value in both maintained roots. | Same 17 derived controls in both maintained roots. | No stable/upstream rescue. |
| `origin/perf/quick-wins` (`0076040f`) | Same malformed base-class value in both maintained roots. | Same 17 derived controls in both maintained roots. | Perf branch does not touch this UI config. |
| Release `origin/release/2026-06-feature-bundle` (`3282ff3f`) | Same malformed base-class value in both maintained roots. | Release Chernarus has 16 derived controls after UI drift; release Vanilla keeps 17. | Release branch does not rescue the base config. |
| UI theme branches `origin/feat/wf-menu-ops-console` (`0767c0b5`) and `origin/feat/wf-menu-ux-phase1` (`87d86257`) | Same malformed base-class value in both maintained roots. | Same 17 derived controls in both maintained roots. | Theme branches still need config smoke before promotion. |
