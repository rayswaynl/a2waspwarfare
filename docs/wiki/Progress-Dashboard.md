...
| Lane | Status | Output |
| --- | --- | --- |
| `trello-audio-cues-branch-intel-2026-06-23` | Published / validated | Routed newly fetched `origin/claude/trello-audio-cues@a0e4fb458` through [Support specials](Support-Specials-And-Tactical-Modules-Atlas#trello-audio-cue-branch-intel), [Modules / IRS](Modules-Atlas#irs--ir-smoke-missile-countermeasure-commonmoduleirs), [Assets/config/localization](Assets-Config-Localization-And-Parameters-Atlas#current-branch-scope), [Feature status](Feature-Status-Register), [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack) and release-readiness gates. The branch is one commit on current stable merge-base `origin/master@f8a76de34`; payload is four maintained-root files / +74 / -2 and `git diff --check` is clean. #90 adds client-local friendly-side paratrooper `commanderNotification` plus temporary `paraDZ_` marker at `HandleParatrooperMarkerCreation.sqf:42-61`; #91 adds a delayed IR-smoke ready cue with `ARTY_cooldown_over` at `IRS_OnIncomingMissile.sqf:51-61`. Existing sound classes are reused; hardcoded `Paradrop` marker text and smoke coverage remain gates. No gameplay source changed. |
| `trello-upgrade-hqcost-info-branch-intel-2026-06-23` | Published / validated | Routed draft PR #75 / `origin/claude/trello-upgrade-hqcost-info@34fd202c8865` through [Client UI systems](Client-UI-Systems-Atlas#current-branch-scope), [Upgrades and research](Upgrades-And-Research-Atlas#current-branch-scope), [Commander/HQ lifecycle](Commander-HQ-Lifecycle-Atlas#current-branch-scope), [Feature status](Feature-Status-Register), [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack) and release-readiness gates. The branch merge-bases directly on current stable `origin/master@f8a76de34`; payload is six maintained-root files / +96 and `git diff --check` clean. It adds value lines for artillery reload and respawn range upgrade completion at `Client_FNC_Special.sqf:204,212-223`, appends New HQ cost in Transfer Menu at `GUI_TransferMenu.sqf:124-129`, and adds string keys at `stringtable.xml:459-473,1055-1061` in source Chernarus plus maintained Vanilla. Current stable has the constants/arrays but no branch UI/string lines. No gameplay source changed. |
| `trello-artillery-ui-branch-intel-2026-06-23` | Published / validated | Routed newly fetched Trello artillery UI branches through [Support specials](Support-Specials-And-Tactical-Modules-Atlas#trello-artillery-ui-branch-intel), [Client UI systems](Client-UI-Systems-Atlas#current-branch-scope), [Feature status](Feature-Status-Register), [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack) and release-readiness gates. `origin/claude/trello-arty-cooldown-hud@471fc0580fd6` and `origin/claude/trello-arty-to-gunner@2dc3e7403399` both merge-base directly on current stable `origin/master@f8a76de34`, so no older-base drift was found. Cooldown HUD payload is four files / +134 / -8 and adds `RUBHUD_Arty` controls at `Client_UpdateRHUD.sqf:27,259-288,452` plus `Rsc/Titles.hpp:177,478-490`. Crew-all payload is six files / +217 / -8 and adds Tactical `MenuAction == 50`, button idc `17040` and crew strings at `GUI_Menu_Tactical.sqf:571-626`, `Rsc/Dialogs.hpp:2364-2371` and `stringtable.xml:3192-3213`. Both payloads pass `git diff --check`; current stable lacks the new controls/actions/strings. No gameplay source changed. |
| `trello-base-integrity-branch-intel-2026-06-23` | Published / validated | Routed newly fetched `origin/claude/trello-base-integrity-fixes@66d80160` through [Zeta Cargo sling-load](Zeta-Cargo-Sling-Load-Reference#branch-intel---trello-base-integrity-candidate), [Support specials](Support-Specials-And-Tactical-Modules-Atlas), [ICBM authority](ICBM-Authority-Playbook#branch-intel---trello-97-in-progress-construction), [Construction and CoIn systems](Construction-And-CoIn-Systems-Atlas#current-branch-scope), [Feature status](Feature-Status-Register), [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack) and release-readiness gates. The branch is two commits on current stable merge-base `origin/master@f8a76de34`; payload is six maintained-root files / +70 / -10 and `git diff --check` is clean. #87 adds friendly-HQ lift filtering plus ground-drop behavior; #97 deletes in-progress construction logics inside the nuke blast. It remains branch-only, does not close ICBM authority, and maintained Vanilla still needs the Zeta detach-argument parity before manual-detach fix claims. No gameplay source changed. |
| `upstream-barracks-4th-level-branch-intel-2026-06-23` | Published / validated | Routed newly fetched `origin/claude/upstream-barracks-4th-level@e83de9c4` through [Upgrades and research](Upgrades-And-Research-Atlas#current-branch-scope), [Factory and purchase systems](Factory-And-Purchase-Systems-Atlas#current-branch-scope), [Feature status](Feature-Status-Register), [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack) and release-readiness gates. The branch is one commit on merge-base `be2bbd084`, an ancestor of current stable `origin/master@f8a76de34`; payload versus merge-base is 64 files / +700 / -688 across source Chernarus plus maintained Vanilla and `git diff --check` is clean. It expands Barracks metadata to four levels at `Upgrades_CO_US.sqf:33,61,89,125`, adds `[WFBE_UP_GEAR,4]` at `:185`, reshuffles representative infantry required levels in `Core_US.sqf` / `Core_RU.sqf`, and adds a `US_Soldier_AT_EP1` MAAWS HEAT replacement at `Common_CreateUnit.sqf:101`. Current stable does not include the candidate; porting must preserve later B74.1 player-stats constants. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | `documentation-finisher-loop` | Latest published lane: `trello-audio-cues-branch-intel-2026-06-23`. Continue bounded source-backed docs passes from this dashboard, Feature Status, Source Fix queue, Upstream mining ledger and pruning ledger. |
