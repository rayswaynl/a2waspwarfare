...
| Lane | Status | Output |
| --- | --- | --- |
| `upstream-hq-repair-price-branch-intel-2026-06-23` | Published / validated | Routed newly fetched `origin/claude/upstream-hq-repair-price@440be3da0bda` through [Commander/HQ lifecycle](Commander-HQ-Lifecycle-Atlas#branch-intel---upstream-hq-repair-price-candidate), [Feature status](Feature-Status-Register), [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack) and release-readiness gates. The branch is one clean payload commit on merge-base `be2bbd084`, an ancestor of current stable `origin/master@f8a76de34`; payload is source Chernarus plus maintained Vanilla `Action_RepairMHQ.sqf`, `Init_Unit.sqf` and `stringtable.xml` (+76/-26). It formats repair-truck action labels and post-repair next-price hints, but has no `Server/`, `RequestMHQRepair.sqf` or public-variable payload, so MHQ repair remains authority-light. Direct current-stable diffs include unrelated B74.1 `Init_CommonConstants.sqf:914-920` player-stats drift that must be preserved during any port. No gameplay source changed. |
| `source-fix-queue-current-b741-scope-2026-06-23` | Published / validated | Refreshed [Source fix propagation queue](Source-Fix-Propagation-Queue#current-branch-scope-2026-06-23), [Current source status snapshot](Current-Source-Status-Snapshot#2026-06-22-current-head-caveat), [Testing workflow](Testing-Debugging-And-Release-Workflow#propagated-fix-smoke-pack) and release-readiness source-only machine rows with current stable/B74.1 propagated-fix evidence. Current stable `origin/master@f8a76de34` equals `origin/claude/b74.1-aicom@f8a76de34`; scoped queue-path diff from `0139a3468609` to `f8a76de34` touches only maintained-root `Client/Init/Init_Client.sqf`, so current stable/B74.1 skill init/apply anchors are now `:805/:828`. Paratrooper registration remains `Init_PublicVariables.sqf:38`, FPS remains one guarded `serverFpsGUI.sqf:4` publisher with `SERVER_FPS_GUI` at `:9-10`, supply scan remains `supplyMissionStarted.sqf:7,55,61,83`, player-list reset is still inside the loop at `playerObjectsList.sqf:18`, and commander ARTY remains marker-based at `Construction_StationaryDefense.sqf:166-168` plus `Common_GetTeamArtillery.sqf:41-78`. No gameplay source changed. |
| `upstream-an2-mapicons-branch-intel-2026-06-23` | Published / validated | Routed two newly fetched branch-only candidates through [Valhalla vehicle climbing assist](Valhalla-Vehicle-Climbing-Assist#branch-intel---an-2-fast-lift-candidate), [Client UI systems](Client-UI-Systems-Atlas#map-and-marker-ui), [Feature status](Feature-Status-Register), [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack) and the release-readiness gates. `origin/claude/upstream-an2-climb-boost@63e6b4142` branches from merge-base `be2bbd084` and adds `An2_TK_EP1` low-gear actions at `Init_Unit.sqf:87-90` plus an `Air` assist target `_min = 100` at `Func_Client_LowGear.sqf:68-75` in source Chernarus and maintained Vanilla. `origin/claude/upstream-blinking-mapicons@6e31f7fd5` adds mounted gunner/commander soldier-marker blinking at `Client_BookkeepBlinkingIcons.sqf:96-110` in both roots. Current stable `origin/master@f8a76de34` does not include either candidate; porting must preserve later B74.1 `Init_CommonConstants.sqf:914-920` player-stats drift. No gameplay source changed. |
| `hq-score-miksuu-branch-intel-2026-06-23` | Published / validated | Refreshed [Commander/HQ lifecycle](Commander-HQ-Lifecycle-Atlas#hq-kill-score-and-bounty-branch-matrix), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue), [Upstream Miksuu commit intel](Upstream-Miksuu-Commit-Intel) and [Developer history](Developer-History-And-Upstream-Lessons) with Miksuu score/HQ/bounty branch-name classification. Current Miksuu `master@b8389e748243` still keeps the double-award shape. `ScoreForKillingFactories@f17445c15190`, `Fix0ScoreBountyBug@415615c9662a` and `FixNukeMoneyGain@2ad3a7d38837` are old-layout one-award ancestry, `AntiHQTeamkillDisabledInDebugMode@5bafe566951a` is branch-only one-award evidence, and `BountyModifierTo4@cc127ef40b4d` predates the HQ score award. No gameplay source changed. |
| `pvf-sender-auth-current-b741-refresh-2026-06-23` | Published / validated | Refreshed DR-55/PVF sender-auth evidence across [Feature status](Feature-Status-Register), [Server authority migration](Server-Authority-Migration-Map#registered-server-pvf-handler-authority-matrix), [PVF dispatch implementation](PVF-Dispatch-Implementation-Playbook#sender-authentication-boundary), [Networking](Networking-And-Public-Variables) and [Public variable channel index](Public-Variable-Channel-Index). Docs/source `HEAD@925d56e01a1e` remains old-shape for checked PVF paths. Current stable `origin/master@f8a76de34` equals `origin/claude/b74.1-aicom@f8a76de34`; current stable/B74.1, B69 and B74 still pass only `(_this select 1)` at `Init_PublicVariables.sqf:56,61` while dispatching via namespace/`CODE` guards at server `Server_HandlePVF.sqf:14-15` and client `Client_HandlePVF.sqf:32-33`. Checked B69..B74 and B74..B74.1 generic PVF deltas are empty, so DR-55 remains open. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | `documentation-finisher-loop` | Latest published lane: `upstream-hq-repair-price-branch-intel-2026-06-23`. Continue bounded source-backed docs passes from this dashboard, the refreshed [PR cleanup lab](PR-Cleanup-And-Integration-Lab), Feature Status, Source Fix queue and pruning ledger. |
...
| Shared docs | Live | GitHub wiki + docs mirror when present | Wiki and mirror should stay in parity after scoped validation on mirror-bearing branches. If the active gameplay branch lacks `docs/wiki`, work in a wiki checkout and do not claim repo-mirror parity until a mirror checkout is inspected. |
