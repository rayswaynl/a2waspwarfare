...
| `patrols-v2-b742-current-head-followup-2026-06-23` | Published / validated | Refreshed Patrols v2 B74.2 current-head rows through [Towns/camps/capture](Towns-Camps-And-Capture-Atlas#patrols-v2-side-upgrade-path), [Feature status](Feature-Status-Register) and [Source Fix queue](Source-Fix-Propagation-Queue). Current B74.2 is `origin/claude/b74.2-aicom@21b62b04`; checked `d472da6a..21b62b04` diffs are empty for Patrols/town-AI/marker/UI cap files. B74.2 still adds only source-Chernarus pop-tier cap reads at `Init_CommonConstants.sqf:170,173-176`, `server_side_patrols.sqf:32,38`, `server_town_ai.sqf:61,64-65`, `Server_GetTownGroupsDefender.sqf:89,91`, Buy Units `:37,:41` and RHUD `:354,:358`; no `Missions_Vanilla` Patrols payload exists. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready / documentation-finisher loop | `documentation-finisher-loop` | RHUD/endgame title-handle B74.2 current-head follow-up is published; pick the next bounded source-backed docs lane from the queue/status pages before editing. |
