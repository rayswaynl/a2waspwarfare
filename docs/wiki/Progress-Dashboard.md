...
| Lane | Status | Output |
| --- | --- | --- |
| `ai-commander-upgrade-debit-b742-current-head-followup-2026-06-24` | Published / validated | Refreshed AI commander upgrade debit/cost lookup rows through [AI commander autonomy](AI-Commander-Autonomy-Audit#ai-upgrade-debit-branch-matrix), [Upgrades and research](Upgrades-And-Research-Atlas#ai-commander-flow), [Feature status](Feature-Status-Register) and [Source Fix queue](Source-Fix-Propagation-Queue). Current B74.2 is `origin/claude/b74.2-aicom@21b62b04`; checked `d472da6a..21b62b04`, `origin/master..origin/claude/b74.2-aicom`, docs-source continuity and B69..B74 diffs are empty for maintained-root worker/menu paths. Docs/source `HEAD@c70785e4be30` is unchanged from `031a253fe` / `ade4d356` and remains old-shape at `Server_AI_Com_Upgrade.sqf:27,34,36,47,50`, while current stable/B74.1/B74.2/B69/B74 use the resolved maintained-root worker at `:75,:89,:92,:97,:125,:131-132,:136`. No gameplay source changed. |
| `town-ai-vehicle-despawn-b742-current-head-followup-2026-06-24` | Published / validated | Refreshed town AI vehicle despawn rows through [Town AI vehicle despawn safety](Town-AI-Vehicle-Despawn-Safety#current-branch-matrix), [Feature status](Feature-Status-Register) and [Source Fix queue](Source-Fix-Propagation-Queue). Current B74.2 is `origin/claude/b74.2-aicom@21b62b04`; checked `d472da6a..21b62b04` is empty for `server_town_ai.sqf` in both maintained roots. Docs/source `HEAD@ce3cf871e1e7` is unchanged from `2a4020586` / `3a9ba92e` and remains old-shape unsafe at `:214`; current B74.2 keeps the source-Chernarus crew-count guard at `:337` while maintained Vanilla remains leader-only at `:319`. No checked current ref contains `Server_CleanupExpiredTownDefenseAssets.sqf`. No gameplay source changed. |
| `fast-travel-fee-b742-current-head-followup-2026-06-24` | Published / validated | Refreshed fast-travel fee UX rows through [Client UI systems](Client-UI-Systems-Atlas#tactical-fast-travel-fee-branch-matrix), [Feature status](Feature-Status-Register) and [Source Fix queue](Source-Fix-Propagation-Queue). Current B74.2 is `origin/claude/b74.2-aicom@21b62b04`; checked `d472da6a..21b62b04` is empty for Tactical GUI/constants paths. `origin/master..origin/claude/b74.2-aicom` and B69..B74 only line-drift source-Chernarus constants. Docs/source `HEAD@003d0c4c044e` is unchanged from `1154f5d43` for checked fast-travel paths and keeps the old TODO/fee/debit shape, while current stable/B74.1/B74.2/B69/B74 keep the paid-mode local hide/marker/debit behavior with the old TODO absent. No gameplay source changed. |
| `requestspecial-upgrade-sync-b742-current-head-followup-2026-06-24` | Published / validated | Refreshed RequestSpecial `upgrade-sync` rows through [Support specials](Support-Specials-And-Tactical-Modules-Atlas#upgrade-sync-branch-matrix), [Upgrades and research](Upgrades-And-Research-Atlas#current-branch-scope), [Feature status](Feature-Status-Register) and [Source Fix queue](Source-Fix-Propagation-Queue). Current B74.2 is `origin/claude/b74.2-aicom@21b62b04`; checked `d472da6a..21b62b04`, `origin/master..origin/claude/b74.2-aicom`, docs-source continuity and B69..B74 diffs are empty for the six maintained-root handler/caller/process paths. Docs/source `HEAD@373e21c1274c` is unchanged from `a7f383b6`; checked refs still mix `_args` and `_this`, so this remains patch-ready parser cleanup debt separate from B74.1/B74.2 end-time state. No gameplay source changed. |
| `buy-menu-price-key-b742-current-head-followup-2026-06-24` | Published / validated | Refreshed buy-menu price/key rows through [Factory and purchase systems](Factory-And-Purchase-Systems-Atlas#buy-menu-price-and-driver-key-branch-matrix), [Feature status](Feature-Status-Register) and [Source Fix queue](Source-Fix-Propagation-Queue). Current B74.2 is `origin/claude/b74.2-aicom@21b62b04`; checked `d472da6a..21b62b04` is empty for buy-menu price/key paths and `Client_BuildUnit.sqf` in both maintained roots. Docs/source `HEAD@b8180bb319ed` is unchanged from `4120137f` / `8d611092` / `ebd86fad0` for checked price/key paths; current stable/B74.1/B74.2/B69/B74 and historical `a96fdda2` fix selected-detail/key mirroring but still lack a level-0 `UNIT_COST_MODIFIER` reset. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready / docs finisher loop | `documentation-finisher-loop` | AI commander upgrade debit B74.2 current-head follow-up is published and validated; pick the next bounded source-backed docs lane before editing. |
