...
| Lane | Status | Output |
| --- | --- | --- |
| `coordination-board-open-task-route-refresh` | Published / validated | Refreshed [Coordination board](Coordination-Board) and this dashboard so the active docs loop no longer points future agents at [Instructions for Codex](Instructions-For-Codex) as if it were a live queue. Current open docs work now routes through the dashboard, pruning ledger, Feature Status, pending owner decisions, machine status and fresh source evidence. No gameplay source changed. |
