...
| Lane | Status | Output |
| --- | --- | --- |
| `server-runtime-atlas-source-scope-route` | Published / validated | Refreshed [Server gameplay runtime atlas](Server-Gameplay-Runtime-Atlas) and [Server runtime and operations](Server-Runtime-And-Operations) so server startup anchors name current docs checkout `92c5cf05`, preserve the earlier `6afcc58e` source-anchor snapshot as unchanged, and distinguish stable `origin/master` `cf2a6d6a`, release `a96fdda2`, Miksuu `b8389e74` and `perf/quick-wins` `0076040f` for AI supply-truck startup, Patrols v2 and server FPS publishers. No gameplay source changed. |
