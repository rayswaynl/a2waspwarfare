...
| Lane | Status | Output |
| --- | --- | --- |
| `service-easa-affordability-authority-route` | Published / validated | Refreshed [Service menu affordability guards](Service-Menu-Affordability-Guards) with a branch/root matrix: current docs/source and maintained Vanilla still need action-time price/funds/context guards; stable/Miksuu upstream improved service button-enable ordering but still debit client-side; release Chernarus only partially guards rearm/refuel; release Vanilla and exact-funds EASA remain open. Condensed [Feature status](Feature-Status-Register) and [Source fix propagation queue](Source-Fix-Propagation-Queue). No gameplay source changed. |
