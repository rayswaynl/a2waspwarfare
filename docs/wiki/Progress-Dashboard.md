...
| Lane | Status | Output |
| --- | --- | --- |
| `trello-map-radius-circles-branch-intel-2026-06-23` | Published / validated | Routed draft PR #78 / `origin/claude/trello-map-radius-circles@77dd71ba303f` through [Client UI systems](Client-UI-Systems-Atlas#current-branch-scope), [Commander/HQ lifecycle](Commander-HQ-Lifecycle-Atlas#current-branch-scope), [Respawn/death lifecycle](Respawn-And-Death-Lifecycle-Atlas#branch-intel---mobile-respawn-map-circles), [Medic redeployment truck](Medic-Redeployment-Truck-Forward-Spawn#branch-intel---map-radius-circle-candidate), [Feature status](Feature-Status-Register), [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack) and release-readiness gates. The branch merge-bases directly on current stable `origin/master@f8a76de34`; payload is six maintained-root files / +192 and `git diff --check` clean. It adds a side-colored HQ build-area map ellipse at branch `Init_BaseStructure.sqf` Chernarus `:54-65` / Vanilla `:35-46`, plus a yellow own-side ambulance/redeploy class-radius watcher launched from `Init_Client.sqf:1227`. Current stable has build/respawn constants and `CCrange*`, but no `HQBuildRange`, `AmbRange_` or watcher launcher hits. The watcher does not duplicate full redeploy eligibility gates, so promotion needs explicit mismatch smoke. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | `documentation-finisher-loop` | Latest published lane: `trello-map-radius-circles-branch-intel-2026-06-23`. Continue bounded source-backed docs passes from this dashboard, Feature Status, Source Fix queue, Upstream mining ledger and pruning ledger. |
