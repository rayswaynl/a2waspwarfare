...
| `commander-vote-reassign-branch-scope-route` | Published / validated | Refreshed [Commander vote/reassignment](Commander-Vote-And-Reassignment-Playbook) as the commander correctness gateway: added a top route table and current branch scope for docs checkout `e2c9f6ed`, stable `origin/master` `cf2a6d6a`, Miksuu `b8389e74`, `perf/quick-wins` `0076040f`, release `a96fdda2` and `feat/ai-commander` `c20ce153`; confirmed vote AI/no-commander semantics remain unpatched everywhere checked; routed DR-15 helper branch truth to [Commander reassignment call shape](Commander-Reassignment-Call-Shape#current-branch-matrix); refreshed commander rows in [Feature status](Feature-Status-Register) plus [Source fix propagation queue](Source-Fix-Propagation-Queue); and aligned Objective Ping wording to current stable/release targeted-send evidence. No gameplay source changed. |
| `ai-runtime-hc-loop-branch-scope-route` | Published / validated | Refreshed [AI runtime and HC loop map](AI-Runtime-HC-Loop-Map) as the AI/HC runtime gateway: added a top route table and current branch scope for docs checkout `b9e80da0`, stable `origin/master` `cf2a6d6a`, Miksuu `b8389e74`, `perf/quick-wins` `0076040f`, release `a96fdda2` and `feat/ai-commander` `c20ce153`; routed AI supply-truck detail to [AI commander autonomy](AI-Commander-Autonomy-Audit#ai-supply-truck-branch-matrix); refreshed [Town AI vehicle despawn safety](Town-AI-Vehicle-Despawn-Safety#current-branch-matrix); and aligned [Feature status](Feature-Status-Register) plus [Source fix propagation queue](Source-Fix-Propagation-Queue) so older `89ae9dad` persistent-defense helper evidence is historical, not current branch truth. No gameplay source changed. |
| `towns-camps-capture-atlas-branch-scope-route` | Published / validated | Refreshed [Towns, camps and capture](Towns-Camps-And-Capture-Atlas) as the town/camp gateway: added a top route table and current branch scope for docs checkout `3eefcb00`, stable `origin/master` `cf2a6d6a`, Miksuu `b8389e74`, `perf/quick-wins` `0076040f` and release `a96fdda2`; separated stable/release independent camp-capture flag fixes from still-open repair-camp flag refresh; refreshed zero-camp helper consumer line drift; and aligned [Feature status](Feature-Status-Register) plus [Source fix propagation queue](Source-Fix-Propagation-Queue). No gameplay source changed. |
| `upgrades-research-atlas-branch-scope-route` | Published / validated | Refreshed [Upgrades and research](Upgrades-And-Research-Atlas) as the upgrades gateway: added a top route table and current branch scope for docs checkout `e785f1e9`, stable `origin/master` `cf2a6d6a`, Miksuu `b8389e74`, `perf/quick-wins` `0076040f`, release `a96fdda2`, `origin/feat/ai-commander` `c20ce153` and `origin/feat/upgrade-queue-stacking` `b061c905`; routed AI upgrade debit and `upgrade-sync` detail to canonical matrices; refreshed the stale old upgrade dialog branch split; and aligned [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue), [AI commander autonomy](AI-Commander-Autonomy-Audit#ai-upgrade-debit-branch-matrix) and [Support specials](Support-Specials-And-Tactical-Modules-Atlas#upgrade-sync-branch-matrix). No gameplay source changed. |
