...
| Lane | Status | Output |
| --- | --- | --- |
| `modules-atlas-current-scope-route-prune` | Published / validated | Refreshed [Modules atlas](Modules-Atlas) so current docs head `86244c24` is the visible module source-continuity checkpoint. Targeted diffs from `20a19676` to `HEAD` over checked Chernarus common/client/server init, unit-creation, module folders and paratrooper support paths return no source changes; branch refs still resolve to stable `cf2a6d6a`, release `a96fdda2`, Miksuu `b8389e74` and perf `0076040f`. Updated the local Reaktiv dormant-module table to current docs head and kept exact branch matrices routed to owner pages except for that compact local module table. No gameplay source changed. |
