...
| Lane | Status | Output |
| --- | --- | --- |
| `empty-supply-truck-current-b69-head-refresh-2026-06-22` | Published / validated | Refreshed [Marker cleanup/restoration](Marker-Cleanup-Restoration-Systems-Atlas#empty-supply-truck-branch-matrix), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine rows after current B69 advanced to `origin/claude/b69@8d465fce` while the empty-vehicle collector/handler paths stayed unchanged from older B69/finalpieces proof. Docs/source `HEAD@a3bbcd48` is unchanged from `b4e10b5f` for checked paths; current stable `origin/master@0139a346`, current B69, direct current Miksuu `b8389e748243`, perf `0076040f` and historical `a96fdda2` all still route `emptyVehicles` through `emptyvehiclescollector.sqf:9,12,15-17,19` and set supply-truck `_delay = 86400` in both maintained roots. B69 checked path deltas `0094647d..8d465fce` and `80d3267c..8d465fce` are empty; no live `release/*` or cleanup/supply rescue head was exposed. No gameplay source changed. |
| `upgrade-sync-current-b69-head-refresh-2026-06-22` | Published / validated | Refreshed [Support specials](Support-Specials-And-Tactical-Modules-Atlas#upgrade-sync-branch-matrix), [Upgrades and research](Upgrades-And-Research-Atlas#current-branch-scope), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine rows after current B69 stayed at `origin/claude/b69@8d465fce` for the `upgrade-sync` tuple shape. Docs/source `HEAD@bc21f520` is unchanged from `ff8dd884` for checked handler/caller/process-upgrade paths; current stable `origin/master@0139a346` and current B69 both keep `Server_HandleSpecial.sqf:3,67-73` reading side from `_args` but id/level from `_this`, caller `GUI_UpgradeMenu.sqf:292` and sync lifecycle `Server_ProcessUpgrade.sqf:32,35,41`. The checked B69 delta `0a1ccb4d..8d465fce` is empty; Miksuu/perf still call from `:241`, historical `a96fdda2` from `:254`, and historical `b061c905` from `:268`. No gameplay source changed. |
| `hq-score-current-b69-head-refresh-2026-06-22` | Published / validated | Refreshed [Commander/HQ lifecycle](Commander-HQ-Lifecycle-Atlas), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine rows after current B69 stayed at `origin/claude/b69@8d465fce` for the DR-50 scoring shape. Docs/source `HEAD@97e4cdd0`, current stable `origin/master@0139a346`, current B69, current Miksuu `b8389e748243`, perf `0076040f` and historical `a96fdda2` all still keep generic HQ building score plus a separate `_score = 900` HQ bounty in both maintained roots. Current B69 line drift is generic award `Server_OnHQKilled.sqf:64,66`, non-teamkill guard `:106`, `_score = 900` at `:109` and second award `:112`; B69 base-fall smoke/sting at `:57,103` is not a scoring fix. No gameplay source changed. |
| `pvf-sender-auth-current-b69-head-refresh-2026-06-22` | Published / validated | Refreshed [Feature status](Feature-Status-Register), [Server authority migration map](Server-Authority-Migration-Map), [PVF dispatch implementation](PVF-Dispatch-Implementation-Playbook), [Networking and public variables](Networking-And-Public-Variables), [Public variable channel index](Public-Variable-Channel-Index), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine rows after current B69 advanced to `origin/claude/b69@8d465fce`. Docs/source `HEAD@4d4610f1` is unchanged from `ade4d356` for checked PVF init/dispatcher paths. Current stable `origin/master@0139a346` and current B69 both use `missionNamespace`/`CODE` dispatch but still forward only `(_this select 1)` at `Init_PublicVariables.sqf:56,61`, so DR-55 remains open. The `0a1ccb4d..8d465fce` checked PVF-path delta changes only `Client/PVFunctions/HandleSpecial.sqf` in both maintained roots, adding default-off `aicom-team-merge` at `:57,59`; no gameplay source changed. |
| `camp-count-helper-current-b69-head-refresh-2026-06-22` | Published / validated | Refreshed [Towns, camps and capture atlas](Towns-Camps-And-Capture-Atlas), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine rows after current B69 advanced to `origin/claude/b69@8d465fce` while `git diff --name-status 0a1ccb4d..origin/claude/b69` was empty for checked camp helper/caller paths. Docs/source `HEAD@91d1ccf2` is unchanged from `ade4d356` for checked helper/caller files. Docs/source, current stable `origin/master@0139a346`, current Miksuu `b8389e748243`, perf `0076040f`, historical release `a96fdda2` and current B69 all keep `Common_GetTotalCamps.sqf:10` and `Common_GetTotalCampsOnSide.sqf:16` returning `1` for zero-camp towns in both maintained roots. Affected caller families remain capture mode 2/capture-rate math, threeway respawn and depot infantry buys; no checked branch splits real-count from safe-denominator behavior. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | `documentation-finisher-loop` | Latest published lane: `empty-supply-truck-current-b69-head-refresh-2026-06-22`. Continue bounded source-backed docs passes from this dashboard, the refreshed [PR cleanup lab](PR-Cleanup-And-Integration-Lab), Feature Status, Source Fix queue and pruning ledger. |
