...
| Lane | Status | Output |
| --- | --- | --- |
| `factory-destroyed-purchase-refund-branch-route` | Published / validated | Added [Factory and purchase systems](Factory-And-Purchase-Systems-Atlas#destroyed-factory-refund-branch-matrix) branch scope for destroyed-factory purchase refund behavior: current source Chernarus, maintained Vanilla, stable `origin/master` `2cdf5fb8`, Miksuu upstream `f532f706` and `perf/quick-wins` `0076040f` all keep local debit after `BuildUnit` spawn and no refund in the dead/null factory exit. Release `7195b331` carries `_currentCost` plumbing plus an empty/crewless-branch refund, but the dead/null factory exit remains queue cleanup only. Condensed [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine records. No gameplay source changed. |
