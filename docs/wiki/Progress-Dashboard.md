...
| Lane | Status | Output |
| --- | --- | --- |
| `requestspecial-upgrade-sync-branch-route` | Published / validated | Refreshed [Support specials](Support-Specials-And-Tactical-Modules-Atlas#upgrade-sync-branch-matrix) with a branch/root matrix: current source Chernarus, maintained Vanilla, stable `origin/master`, Miksuu upstream, `perf/quick-wins` and release `d482c742` all keep the same mixed `_args` / `_this` parser for `upgrade-sync`; because `_args = _this`, this is consistency cleanup debt rather than a confirmed runtime break. Condensed [Upgrades](Upgrades-And-Research-Atlas), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine records. No gameplay source changed. |
...
| `wasp-marker-wait-branch-route` | Published / validated | Refreshed [WASP marker wait cleanup](WASP-Marker-Wait-Cleanup) with a branch/root check: current source Chernarus, maintained Vanilla, stable `origin/master`, Miksuu upstream, `perf/quick-wins` and the feature-bundle release all keep the display-54 input-lock busy wait, while the display-12 sibling already uses throttled `waitUntil`. Condensed [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine records. No gameplay source changed. |
