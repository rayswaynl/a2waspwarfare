...
| Lane | Status | Output |
| --- | --- | --- |
| `duplicate-ui-idd-current-b742-head-refresh-2026-06-24` | Published / validated | Refreshed [UI resource parity cleanup](UI-Resource-Parity-Cleanup), [UI IDD collision repair](UI-IDD-Collision-Repair), [Client UI systems](Client-UI-Systems-Atlas), [Feature status](Feature-Status-Register), [Dead/stale code](Dead-Code-And-Stale-Code-Register) and duplicate IDD machine rows so EASA/Economy/title-ID evidence routes current stable/B74.1 through `origin/master@f8a76de34` and current B74.2 through `origin/claude/b74.2-aicom@21b62b04` instead of older `0139a346`-only wording. Repo docs branch `HEAD@5990557155b0` is unchanged from `edbd341e`/`b5219d47` for checked `Rsc/Dialogs.hpp`/`Rsc/Titles.hpp` IDD paths. Docs/Miksuu/perf keep duplicate EASA/Economy `23000`; current stable/B74.1/B74.2/B69/B74 and historical `a96fdda2` split EASA to `24000` while Economy stays `23000`; title `10200` remains shared by `RscOverlay`/`OptionsAvailable`; no checked hard-coded `findDisplay 23000/24000/10200` callers. No gameplay source changed. |
| `paratrooper-marker-b742-context-refresh-2026-06-24` | Published / validated | Refreshed [Paratrooper marker revival](Paratrooper-Marker-Revival), [Feature status](Feature-Status-Register), [Source Fix queue](Source-Fix-Propagation-Queue#current-branch-scope-2026-06-23), compact context, release-readiness and machine rows so paratrooper marker registration routes current stable/B74.1 through `origin/master@f8a76de34` and current B74.2 through `origin/claude/b74.2-aicom@21b62b04` instead of older `0139a346`-only wording. Repo docs branch `HEAD@d2da2a9c9388` is source-unchanged from `7e88d609` for the checked sender/handler/registration paths. Current stable/B74.1/B74.2/B69/B74 register `HandleParatrooperMarkerCreation` at `Common/Init/Init_PublicVariables.sqf:38` in both maintained roots; docs/source keeps `:39`; historical `a96fdda2` keeps `:34`; Miksuu omits the checked registration and perf is Chernarus-only. No gameplay source changed. |
| `ai-runtime-town-ai-b742-route-refresh-2026-06-24` | Published / validated | Refreshed [AI runtime and HC loop map](AI-Runtime-HC-Loop-Map#current-branch-scope) so the Town AI tracked-vehicle cleanup route names current B74.2 `origin/claude/b74.2-aicom@21b62b04` instead of the older `d472da6a` route. Repo docs branch `HEAD@6ee7c754abb6` is source-unchanged from `ce3cf871e1e7` for both maintained-root `server_town_ai.sqf` files and remains old-shape at `:214`. Current stable/B74.1, B69 and adjacent B74 protect source Chernarus at `:312`/`:325`; current B74.2 line-drifts source Chernarus to `:324`/`:337`; maintained Vanilla remains leader-only at `:319`. Checked `d472da6a..21b62b04` is empty for both maintained-root Town AI files. No gameplay source changed. |
| `stale-upgrade-dialog-current-b742-head-refresh-2026-06-24` | Published / validated | Refreshed [UI resource parity cleanup](UI-Resource-Parity-Cleanup), [Client UI systems](Client-UI-Systems-Atlas), [Feature status](Feature-Status-Register), [Dead/stale code](Dead-Code-And-Stale-Code-Register) and stale upgrade dialog machine rows from older current-stable `0139a346` wording to current stable/B74.1 `origin/master@f8a76de34` plus current B74.2 `origin/claude/b74.2-aicom@21b62b04`. Repo docs branch `HEAD@208062b4e` is unchanged from `427346b0d`, `0fdd5602` and `d4cfef80` for checked upgrade dialog/controller paths and still carries stale `RscMenu_Upgrade` at `Rsc/Dialogs.hpp:2425`, missing `GUI_Menu_Upgrade.sqf` onLoad at `:2428` and missing `wf_*.paa` icon refs at `:2634-2821` in both maintained roots. Current stable/B74.1, previous B74.2 `d472da6a`, current B74.2, B69 `8d465fce`, B74 `b23f557f` and historical `a96fdda2` have no checked maintained-root stale-class hits and keep only live `WFBE_UpgradeMenu` at `Rsc/Dialogs.hpp:4-7`; checked B74.2/current-stable/B69-B74 upgrade-dialog diffs are empty. No gameplay source changed. |
| `economy-dialog-current-b742-head-refresh-2026-06-24` | Published / validated | Refreshed [UI resource parity cleanup](UI-Resource-Parity-Cleanup), [Client UI systems](Client-UI-Systems-Atlas), [Feature status](Feature-Status-Register), [Dead/stale code](Dead-Code-And-Stale-Code-Register) and Economy dialog machine rows from older current-stable `0139a346` wording to current stable/B74.1 `origin/master@f8a76de34` plus current B74.2 `origin/claude/b74.2-aicom@21b62b04`. Repo docs branch `HEAD@427346b0d` is unchanged from `21f0d53b` / `d2a3f995` / `b5219d47` for checked Economy controller/resource paths and still writes stale `23004`/`23005`/`23006` at `GUI_Menu_Economy.sqf:7-8`, while `RscMenu_Economy` lacks those controls and `23020`. Current stable/B74.1/B74.2/B69/B74 and historical `a96fdda2` remove the stale writes, use `DisplayCtrl 23020` at `GUI_Menu_Economy.sqf:25` and declare `23020` at `Rsc/Dialogs.hpp:3070` / historical `:3006`; checked B74.2 head diffs are empty and `0139a3468609..origin/master` only changes source-Chernarus dashboard label text. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready / documentation finisher loop | `documentation-finisher-loop` | Duplicate UI IDD current stable/B74.2 route refresh is published; pick the next bounded source-backed docs lane from Feature Status, Source Fix queue, pruning ledger or hardening backlog. |
