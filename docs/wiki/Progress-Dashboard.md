...
| Lane | Status | Output |
| --- | --- | --- |
| `client-ui-atlas-source-scope-route` | Published / validated | Refreshed [Client UI systems atlas](Client-UI-Systems-Atlas) source-scope wording for current docs checkout `a71b42fe`: rechecked vote/help/main-menu and clickable-text UI paths as unchanged from earlier `f7bc72a8`, fixed the `MenuAction 19` branch split so docs/Miksuu/perf are FPS HUD while stable `cf2a6d6a` and release `a96fdda2` are GPS enablement, and updated the clickable-text matrix without changing gameplay/UI source. |
