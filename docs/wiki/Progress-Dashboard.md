...
| Lane | Status | Output |
| --- | --- | --- |
| `sqf-atlas-compile-registry-refresh` | Published / validated | Refreshed [SQF code atlas](SQF-Code-Atlas) as the compile/navigation map: replaced broad related-routing bullets with a "How To Use This Atlas" table, refreshed the compile registry snapshot from docs branch `04a60e43` to 738 `preprocessFile` references (460 line-numbered, 278 plain, 22 commented), corrected the Client area / `Init_Client.sqf` counts, and added the branch-local caveat because `origin/master` `cf2a6d6a` has mission-source drift. No gameplay source changed. |
