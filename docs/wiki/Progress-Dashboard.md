...
| Lane | Status | Output |
| --- | --- | --- |
| `hq-kill-score-double-award-branch-route` | Published / validated | Split DR-50 out of the weak catch-all row. [Commander/HQ lifecycle](Commander-HQ-Lifecycle-Atlas#hq-kill-score-and-bounty-branch-matrix) now owns the branch matrix: current source Chernarus and maintained Vanilla, stable `origin/master` `cf2a6d6a`, Miksuu `b8389e74`, `perf/quick-wins` `0076040f` and release `a96fdda2` all keep generic HQ building score plus a second HQ bounty score in `Server_OnHQKilled.sqf`. [Feature status](Feature-Status-Register) and [Source fix propagation queue](Source-Fix-Propagation-Queue) now route the patch shape: one non-teamkill award, zero teamkill score, Vanilla propagation and enemy/friendly HQ-kill smoke. No gameplay source changed. |
| `ai-supply-truck-current-master-safe-disable-route` | Published / validated | Refreshed AI supply-truck logistics status after `origin/master` advanced to `cf2a6d6a`. [AI commander autonomy](AI-Commander-Autonomy-Audit#ai-supply-truck-branch-matrix), [Support specials](Support-Specials-And-Tactical-Modules-Atlas), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue), [Abandoned feature revival](Abandoned-Feature-Revival-Review) and [Dead/stale code](Dead-Code-And-Stale-Code-Register) now say current Chernarus, maintained Vanilla and release `a96fdda2` log-disable legacy AI supply-truck logistics at `Init_Server.sqf:383-384` instead of raw-spawning `UpdateSupplyTruck`. Miksuu `b8389e74` and perf `0076040f` still raw-spawn; `feat/ai-commander` guards only Chernarus. No gameplay source changed. |
| `coordination-dashboard-hygiene-audit` | Published / validated | Audited the coordination surfaces after the P0 network batch. [`agent-status.json`](agent-status.json), [`agent-collaboration.json`](agent-collaboration.json) and [`agent-events.jsonl`](agent-events.jsonl) parse cleanly; the checked live-claim set has no conflicting active owner beyond the ongoing `documentation-finisher-loop`, Claude autonomous-ready, watchlists and future code-owner lanes; Latest Batch was already compact at five rows before this refresh and remains five rows after trimming the oldest row. Verified wiki/docs mirror parity for the touched coordination files before editing. No gameplay source changed. |
| `clickabletext-soundpush-current-master-refresh` | Published / validated | Refreshed the patch-ready [clickable text soundPush matrix](Client-UI-Systems-Atlas#clickable-text-soundpush-branch-matrix) after branch heads moved to docs/source `f7bc72a8`, `origin/master` `cf2a6d6a`, Miksuu `b8389e74` and release `a96fdda2`. All checked maintained roots still keep `RscClickableText.soundPush[] = {, 0.2, 1};` at `Rsc/Ressources.hpp:556`; `:92` remains the valid empty-sound precedent. [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and [Dead/stale code](Dead-Code-And-Stale-Code-Register) now name the current refs and the corrected 14-control current-master/release count. No gameplay source changed. |
| `p0-network-current-master-route` | Published / validated | Refreshed the P0 networking branch heads after `origin/master` advanced to `cf2a6d6a`, Miksuu to `b8389e74` and release to `a96fdda2`. [PVF dispatch implementation](PVF-Dispatch-Implementation-Playbook#current-branch-matrix) now shows current/release clients still compile at `Client_HandlePVF.sqf:32` after the HC filter, Miksuu/perf clients at `:22`, and all checked refs/roots at `Server_HandlePVF.sqf:14` with no allowlist. [Public variable channel index](Public-Variable-Channel-Index#send_message-direct-compile-branch-matrix), [Networking](Networking-And-Public-Variables), [Feature status](Feature-Status-Register) and [Source fix propagation queue](Source-Fix-Propagation-Queue) now keep DR-46 `SEND_MESSAGE` separate from PVF dispatcher lookup and DR-55 sender authentication. No gameplay source changed. |
