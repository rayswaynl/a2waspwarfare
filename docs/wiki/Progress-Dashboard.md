...
| Lane | Status | Output |
| --- | --- | --- |
| `mash-marker-current-b742-route-refresh-2026-06-24` | Published / validated | Refreshed MASH marker/deploy branch wording across [Respawn/death lifecycle](Respawn-And-Death-Lifecycle-Atlas#mash-split-live-respawn-dead-marker-relay), [Abandoned feature revival](Abandoned-Feature-Revival-Review#mash-markers), [Feature status](Feature-Status-Register), [Public variable channel index](Public-Variable-Channel-Index), [Dead/stale code](Dead-Code-And-Stale-Code-Register), support/modules overviews and machine rows. Repo docs branch `HEAD@f938b0c0b7ea` is unchanged from `443055cf` / `2b5139219faa` / `db3015f18ea3` for checked MASH paths and remains old-shape with local deploy plus orphaned marker sync. Current stable/B74.1 `origin/master@f8a76de34`, current B74.2 `origin/claude/b74.2-aicom@21b62b04`, B69, B74 and historical `a96fdda2` remove the maintained-root deploy/module path; checked old-stable/current/B74.2/B69-B74 MASH deltas are empty. No gameplay source changed. |
| `factory-buy-refund-current-b742-feature-status-refresh-2026-06-24` | Published / validated | Refreshed factory buy/refund and `AIBuyUnit` branch-scope wording across [Factory and purchase systems](Factory-And-Purchase-Systems-Atlas), [Feature status](Feature-Status-Register), [Source Fix queue](Source-Fix-Propagation-Queue) and machine rows. Repo docs branch `HEAD@f9601c92e0a4` is unchanged from `b8180bb319ed` / `cacd46f09163` / `4120137f` / `8d611092` for checked buy/build paths and remains old-shape; current stable/B74.1 `origin/master@f8a76de34`, current B74.2 `origin/claude/b74.2-aicom@21b62b04`, B69, B74 and historical `a96fdda2` refund the real dead/null factory abort. Docs/source, Miksuu, perf and historical QoL `a66d4691` remain old-shape. Current stable/B74.1/B74.2/B69/B74 call `AIBuyUnit` from `AI_Commander_Produce.sqf:320` for AI-team production, but player buys remain client-local with no `RequestBuyUnit` / `RequestBuildUnit` PVF. No gameplay source changed. |
| `client-skill-init-current-b742-context-refresh-2026-06-24` | Published / validated | Refreshed client skill-init idempotency branch wording across [Client skill init idempotency](Client-Skill-Init-Idempotency), [Source Fix queue](Source-Fix-Propagation-Queue), [Feature status](Feature-Status-Register), [Testing workflow](Testing-Debugging-And-Release-Workflow) and machine rows. Repo docs branch `HEAD@1df5200fa485` is unchanged from `b2738971` for checked `Init_Client.sqf` / `Skill_Init.sqf` paths and keeps one `Skill_Init` plus later `WFBE_SK_FNC_Apply` at `:547,:571`. Current stable/B74.1 `origin/master@f8a76de34` uses `:805,:828` in both maintained roots; current B74.2 `origin/claude/b74.2-aicom@21b62b04` keeps source Chernarus `:828,:851` and maintained Vanilla `:805,:828`; Miksuu/perf remain duplicate-shaped. No gameplay source changed. |
| `commander-built-arty-current-b742-context-refresh-2026-06-24` | Published / validated | Refreshed commander-built ARTY branch wording across [Construction and CoIn systems](Construction-And-CoIn-Systems-Atlas), [Source Fix queue](Source-Fix-Propagation-Queue), [Feature status](Feature-Status-Register), [Testing workflow](Testing-Debugging-And-Release-Workflow) and machine rows. Repo docs branch `HEAD@3dc43a3dca7d` is unchanged from `f5bcaf91` for checked ARTY paths and keeps commander-team gunner handoff. Current stable/B74.1 `origin/master@f8a76de34`, current B74.2 `origin/claude/b74.2-aicom@21b62b04`, B69 and B74 share marker-based `WFBE_CommanderArtillery*` discovery at `Construction_StationaryDefense.sqf:166-168` / `Common_GetTeamArtillery.sqf:41-78`; Miksuu/perf remain old `DefenseTeam` shape, and checked Trello artillery heads are UI-only for this ownership lane. No gameplay source changed. |
| `hosted-fps-status-current-b742-refresh-2026-06-24` | Published / validated | Refreshed hosted server FPS publisher/status-counter branch wording across [Hosted server FPS](Hosted-Server-FPS-Loop-Sleep), [Client UI systems](Client-UI-Systems-Atlas), [Feature status](Feature-Status-Register), [Source Fix queue](Source-Fix-Propagation-Queue), [Performance sweep](Performance-Opportunity-Sweep), runtime/testing gates and machine rows. Repo docs branch `HEAD@4a1bc13edde6` is unchanged from `d0161083` / `a27086cd` for checked FPS paths and keeps two guarded publishers. Current stable/B74.1 `origin/master@f8a76de34`, current B74.2 `origin/claude/b74.2-aicom@21b62b04`, B69, B74, live client-fps, live ASR FPS and historical `a96fdda2` use one guarded `serverFpsGUI.sqf:4` publisher and no maintained-root monitor file; B74.2 only line-drifts source-Chernarus init/removal anchors for this lane. Miksuu/perf quick-wins remain old-loop-shaped. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | `documentation-finisher-loop` | Latest MASH marker/deploy B74.2 route refresh is published and validated. Pick the next bounded source-backed docs refresh or pruning lane after a fresh coordination read. |
...
| --- | --- | --- | --- |
| `documentation-finisher-loop` | Codex | Ready / ongoing | Latest bounded pass `mash-marker-current-b742-route-refresh-2026-06-24` is published and validated; next pass should start with a fresh coordination read and a new claim. |
