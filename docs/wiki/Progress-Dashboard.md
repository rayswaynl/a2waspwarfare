...
| `camp-count-helper-current-stable-refresh-2026-06-22` | Published / validated | Refreshed the [Towns/camps/capture zero-camp helper matrix](Towns-Camps-And-Capture-Atlas#current-branch-scope), [Feature status](Feature-Status-Register) and [Source fix queue](Source-Fix-Propagation-Queue) after lower rows still cited stale stable `origin/master cf2a6d6a` evidence. Docs branch `ade4d356`, current stable `origin/master@0139a346`, Miksuu `b8389e74`, perf `0076040f`, historical `a96fdda2` and active B69 `origin/claude/b69@0a1ccb4d` all still return `1` for zero-camp towns at `Common_GetTotalCamps.sqf:10` and `Common_GetTotalCampsOnSide.sqf:16`; current-stable consumers are `server_town.sqf:167-183`, `Client_GetRespawnAvailable.sqf:92` and `GUI_Menu_BuyUnits.sqf:120-121`. No gameplay source changed. |
| `ai-upgrade-debit-current-stable-resolution-refresh-2026-06-22` | Published / validated | Refreshed the [AI commander upgrade debit matrix](AI-Commander-Autonomy-Audit#ai-upgrade-debit-branch-matrix), [Upgrades and research](Upgrades-And-Research-Atlas#current-branch-scope), [Feature status](Feature-Status-Register) and [Source fix queue](Source-Fix-Propagation-Queue) after rows still treated current stable as debit-fixed but cost-index-open. Docs branch `ade4d356`, Miksuu `b8389e74` and `origin/perf/quick-wins@0076040f` still keep raw AI-order cost lookup plus swapped funds/supply debit at `Server_AI_Com_Upgrade.sqf:27,47,50`; current stable `origin/master@0139a346` resolves both maintained roots with current-level cost lookup at `:75` and corrected debits at `:125,136`, with funds-fallback surcharge at `:131-132`. Historical `a96fdda2`, `b061c905` and `c20ce153` are commit-only lessons because current origin exposes no matching heads. No gameplay source changed. |
| `pvf-sender-auth-current-stable-refresh-2026-06-22` | Published / validated | Refreshed DR-55 PVF/PVEH sender-authentication docs after a 2026-06-22 source pass split current-stable dispatcher lookup from requester context. Docs `HEAD@ade4d356`, Miksuu `b8389e74` and `origin/perf/quick-wins@0076040f` keep value-only PVEHs plus `Call Compile` dispatchers; current stable `origin/master@0139a346` and B69 `origin/claude/b69@0a1ccb4d` use `missionNamespace`/`CODE` dispatch but still forward only the value tuple at `Init_PublicVariables.sqf:56,61`. Updated Feature Status, Server Authority, PVF dispatch, Networking and machine rows. No gameplay source changed. |
| `ai-supply-truck-current-stable-anchor-refresh-2026-06-22` | Published / validated | Refreshed the [AI commander supply-truck matrix](AI-Commander-Autonomy-Audit#ai-supply-truck-branch-matrix), [Abandoned feature revival](Abandoned-Feature-Revival-Review), [Feature status](Feature-Status-Register) and [Source fix queue](Source-Fix-Propagation-Queue) after lower rows still flattened docs-branch raw-spawn evidence with current stable safety. Docs branch `ea0e0f1b`, Miksuu `b8389e74` and `origin/perf/quick-wins@0076040f` still raw-spawn the commented/missing worker; current stable `origin/master@0139a346` safe-disables legacy AI supply-truck logistics at `Init_Server.sqf:43,462-463`; historical `a96fdda2` and `c20ce153` are commit-only evidence because current origin exposes no `release/*` or `feat/ai-commander` heads. No gameplay source changed. |
