...
| Lane | Status | Output |
| --- | --- | --- |
| `dashboard-footer-live-route-refresh` | Published / validated | Refreshed this dashboard's `Continue Reading` footer so the next click goes to [Coordination board](Coordination-Board) instead of the demoted [Bottleneck removal queue](Bottleneck-Removal-Queue). No gameplay source changed. |
