...
| Lane | Status | Output |
| --- | --- | --- |
| `wiki-mirror-and-guer-current-source-sync` | Published / validated | Rebased over live wiki `master@e4fc668`, then restored the useful docs-only GUER audit by rewriting it to current `origin/master@0139a346` source truth: `9af83596` is merged, and Chernarus plus maintained Vanilla Takistan both carry the GUER gate, overlay, economy loop, VBIED action/handler and buy/service hooks. Routed through [Feature status](Feature-Status-Register), [Gameplay systems atlas](Gameplay-Systems-Atlas), [`_Sidebar.md`](_Sidebar) and `agent-context.json`. Validation passed: JSON/JSONL parse in both worktrees, `docs/validate-wiki.ps1`, `git diff --check` with LF/CRLF warnings only, and final wiki/docs SHA parity (`261` files each, `differingCommon=0`). No gameplay source changed. |
