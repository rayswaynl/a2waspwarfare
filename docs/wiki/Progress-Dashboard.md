...
| Lane | Status | Output |
| --- | --- | --- |
| `service-easa-affordability-current-stable-refresh-2026-06-22` | Published / validated | Refreshed [Service menu affordability guards](Service-Menu-Affordability-Guards), [Gear/loadout/EASA](Gear-Loadout-And-EASA-Atlas), [Feature status](Feature-Status-Register) and [Source fix queue](Source-Fix-Propagation-Queue) after service/EASA rows still mixed older `89ae9dad` / release wording with current stable. Docs `HEAD@db3015f18ea3` and current Miksuu upstream `master@b8389e74` keep old direct service rearm/refuel debits and strict EASA `_funds > price`; current stable `origin/master@0139a346`, local checkpoint `d9506078` and historical release `a96fdda2` partially guard service rearm/refuel but leave repair/heal positive-price-only plus strict EASA; B69 finalpieces only changes Chernarus EASA to exact-funds `>=`. No gameplay source changed. |
