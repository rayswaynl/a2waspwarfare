...
| `lifecycle-entrypoints-wait-chain-source-scope-route-prune` | Published / validated | Refreshed [Mission entrypoints and lifecycle](Mission-Entrypoints-And-Lifecycle) and [Lifecycle wait-chain](Lifecycle-Wait-Chain) so current docs head `05664f17` is the visible lifecycle source-continuity checkpoint. Targeted diffs from `9da5f1d0` to `HEAD` over checked Chernarus bootstrap, town-init, common/server/client/headless init and HC role-helper paths return no source changes; branch refs still resolve to stable `cf2a6d6a`, release `a96fdda2`, Miksuu `b8389e74` and perf `0076040f`. Routed duplicated branch-scope wording from the wait-chain page back to the entrypoints Source Scope. No gameplay source changed. |
