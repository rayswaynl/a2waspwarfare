...
| Lane | Status | Output |
| --- | --- | --- |
| `mash-marker-relay-decision-route` | Published / validated | Folded Noether's read-only MASH scout into [Respawn and death lifecycle](Respawn-And-Death-Lifecycle-Atlas#mash-split-live-respawn-dead-marker-relay): local officer MASH respawn is source-supported, while shared/JIP-safe marker synchronization is orphaned in current source/Vanilla, stable/upstream and release. Condensed [Feature status](Feature-Status-Register), [PV channel index](Public-Variable-Channel-Index) and [Dead/stale code register](Dead-Code-And-Stale-Code-Register) to route through that canonical MASH split. No gameplay source changed. |
| `ui-resource-parity-cleanup` | Published / validated | Added [UI resource parity cleanup](UI-Resource-Parity-Cleanup) as the canonical matrix for stale `RscMenu_Upgrade`, Economy `23004/23005/23006` stale writes, EASA/Economy duplicate `23000` and title duplicate `10200`. Source check: current docs/source and stable/upstream keep the old UI resource shape; release Chernarus removed the old upgrade class, moves EASA to `24000` and rewrites Economy around `23020`; release Vanilla still carries the old shape; title `10200` remains duplicated everywhere checked. Condensed repeated wording in [Client UI systems](Client-UI-Systems-Atlas), [Feature status](Feature-Status-Register), [Dead/stale code register](Dead-Code-And-Stale-Code-Register) and [UI IDD collision repair](UI-IDD-Collision-Repair). No gameplay source changed. |
| `supply-branch-scope-status-collapse` | Published / validated | Collapsed split supply-status wording into one canonical branch/root matrix on [Supply mission architecture](Supply-Mission-Architecture#current-branch-matrix). Source check: docs/source Chernarus plus maintained Vanilla have the truck-only narrowed command-center scan; `origin/master` and `miksuu/master` remain broad; release Chernarus has the heli-aware cleanup from `4cf443fe`; release Vanilla remains broad and still compiles the dead twin. Updated [Supply scan narrowing](Supply-Mission-Scan-Narrowing), [Supply authority cleanup](Supply-Mission-Authority-Cleanup-Playbook), [Feature status](Feature-Status-Register) and machine records. No gameplay source changed. |
| `main-menu-gps-orphan-actions-branch-scope` | Published / validated | Source-checked main-menu GPS zoom actions `17/18`: current source Chernarus plus maintained Vanilla, stable master, Miksuu upstream and release all keep the router cases, while fixed-string action searches found no `MenuAction = 17/18` emitter in maintained roots. Updated [Client UI systems](Client-UI-Systems-Atlas), [Feature status](Feature-Status-Register), [Dead/stale code register](Dead-Code-And-Stale-Code-Register) and machine backlog records so this is a docs-ready low-risk UI cleanup or deliberate revival lane, not another broad UI audit. No gameplay source changed. |
| `extension-discordbot-contract-stale-scope` | Published / validated | Refined Extension/DiscordBot contract findings: `GLOBALGAMESTATS` still has stale future-field comments/default-array drift, but the live DiscordBot reader guards uptime/player-count slots and mainly degrades display/fallback if `database.json` shape is short or incompatible. Also clarified that DiscordBot's `SharedWithLoadoutManager` terrain copy carries a stale `WriteToFile` generator API with no bot caller; live mission write/propagation belongs to `Tools/LoadoutManager`. Updated [External integrations](External-Integrations), [Dead/stale code register](Dead-Code-And-Stale-Code-Register) and machine records. No gameplay source changed. |
