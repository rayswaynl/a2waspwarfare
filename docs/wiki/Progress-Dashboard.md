...
| `fast-travel-fee-current-stable-refresh-2026-06-22` | Published / validated | Refreshed the [Tactical fast-travel fee branch matrix](Client-UI-Systems-Atlas#tactical-fast-travel-fee-branch-matrix), [Feature status](Feature-Status-Register) and [Source fix queue](Source-Fix-Propagation-Queue) so paid fast-travel UX uses current stable `origin/master@0139a346` line refs. Current stable still hides unaffordable destinations, draws `$fee` marker text and debits locally, but the stale completed TODO is absent there; docs branch `a489e6ff`, Miksuu `b8389e74`, perf `0076040f` and historical release `a96fdda2` keep the TODO and old line shape. No gameplay source changed. |
