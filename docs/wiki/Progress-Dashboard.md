...
| Lane | Status | Output |
| --- | --- | --- |
| `function-module-index-route-prune` | Published / validated | Refreshed [Function and module index](Function-And-Module-Index) so current docs head `6ecac3ae` is the visible source-family checkpoint. Targeted diffs from `1f0b9018` to `HEAD` over checked Chernarus common/client/server init, commander helper, AI upgrade, MASH, Reaktiv and paratrooper support paths return no source changes; branch refs still resolve to stable `cf2a6d6a`, release `a96fdda2`, Miksuu `b8389e74` and perf `0076040f`. Replaced repeated commander DR-15, Reaktiv, MASH and paratrooper branch prose with routes to canonical owner pages. No gameplay source changed. |
