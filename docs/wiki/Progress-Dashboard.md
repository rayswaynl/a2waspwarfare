...
| `town-ai-vehicle-despawn-current-b69-head-refresh-2026-06-22` | Published / validated | Refreshed [Town AI vehicle despawn safety](Town-AI-Vehicle-Despawn-Safety), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine rows after current B69 stayed at `origin/claude/b69@8d465fce` while `git diff --name-status 0a1ccb4d..origin/claude/b69` was empty for checked `server_town_ai.sqf` paths. Docs/source `HEAD@948db4ab`, current stable `origin/master@0139a346`, current Miksuu `b8389e748243`, perf `0076040f`, historical release `a96fdda2` and `origin/feat/aicom-fleet-improvements@cc5090bed4ad` still delete tracked town-AI vehicles with only `!(isPlayer leader group _x)` in both maintained roots. Current B69 Chernarus has the candidate crew scan at `server_town_ai.sqf:325`, but B69 maintained Vanilla remains leader-only at `:319`; no checked current ref contains `Server_CleanupExpiredTownDefenseAssets.sqf`. No gameplay source changed. |
| `side-supply-current-b69-head-refresh-2026-06-22` | Published / validated | Refreshed [Economy authority first cut](Economy-Authority-First-Cut), [Resistance supply scaffold](Resistance-Supply-Scaffold), [Economy, towns and supply](Economy-Towns-And-Supply), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine rows after current B69 advanced to `origin/claude/b69@8d465fce` while `git diff --name-status 39eed5c0..origin/claude/b69` was empty for checked side-supply paths. Current B69 remains Chernarus-only candidate evidence: Chernarus floors west/resistance/east negatives to `0` at `Server_ChangeSideSupply.sqf:12,36,60` and registers `wfbe_supply_temp_resistance` at `:25`, while maintained Vanilla still has the old floor at `Common_ChangeSideSupply.sqf:25` and `Server_ChangeSideSupply.sqf:12,36`. Current stable and Miksuu remain old-shape in both maintained roots; perf fixes Chernarus arithmetic only. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | `documentation-finisher-loop` | Latest published lane: `pvf-sender-auth-current-b69-head-refresh-2026-06-22`. Continue bounded source-backed docs passes from this dashboard, the refreshed [PR cleanup lab](PR-Cleanup-And-Integration-Lab), Feature Status, Source Fix queue and pruning ledger. |
