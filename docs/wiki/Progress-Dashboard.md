...
| `vehicle-cargo-b742-current-head-followup-2026-06-24` | Published / validated | Refreshed vehicle/backpack cargo loop-bound rows through [Vehicle cargo equip loop bounds](Vehicle-Cargo-Equip-Loop-Bounds#current-branch-matrix), [Gear/loadout/EASA atlas](Gear-Loadout-And-EASA-Atlas#current-branch-scope), [Feature status](Feature-Status-Register) and [Source Fix queue](Source-Fix-Propagation-Queue). Current B74.2 is `origin/claude/b74.2-aicom@21b62b04`; checked `d472da6a..21b62b04` is empty for checked cargo helper paths. Docs/source `HEAD@c96ee1670ddb`, current Miksuu and historical EASA QoL remain old-shape; current stable/B74.1/B74.2/B69/B74 and historical `a96fdda2` fix all five loops in both maintained roots, while `origin/perf/quick-wins@0076040f` fixes Chernarus only. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready / docs finisher loop | `documentation-finisher-loop` | Buy-menu price/key B74.2 current-head follow-up is published and validated; pick the next bounded source-backed docs lane before editing. |
