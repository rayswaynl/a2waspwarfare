...
| `b745-b7472-enrollment-stamped-slot-refresh-2026-06-24` | Published / validated | Refreshed `origin/claude/b745` after the branch advanced again to `d52f65466`. [Feature status](Feature-Status-Register), [Current source status](Current-Source-Status-Snapshot#2026-06-24-b745-branch-intake), [Player join/disconnect](Player-Join-Disconnect-And-AntiStack-Lifecycle#branch-intel---b745b747-jip-enrollment), [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack), [AI commander autonomy](AI-Commander-Autonomy-Audit#b745--b7471-aicom-follow-up), [Client UI systems](Client-UI-Systems-Atlas#current-branch-scope), [Performance opportunity sweep](Performance-Opportunity-Sweep) and [Pruning ledger](Wiki-Pruning-And-Relevance-Ledger) now route the B74.7.2 stamped-slot enrollment follow-up: `a9145e4da..d52f65466` is one source-Chernarus `Server_OnPlayerConnected.sqf` file / +7 / -2, no `Missions_Vanilla`, `Modded_Missions` or `Tools` payload, and it requires playable-unit UID matches to come from a group stamped with `wfbe_side` while the nil-side path re-arms bounded retry diagnostics. Full branch payload is now 133 files / +4138 / -4401 over `origin/master@f8a76de34`. No gameplay source changed. |
| `b745-b7471-veteran-cap-refresh-2026-06-24` | Published / validated | Refreshed `origin/claude/b745` after the branch advanced again to `a9145e4da`. [Feature status](Feature-Status-Register), [Current source status](Current-Source-Status-Snapshot#2026-06-24-b745-branch-intake), [AI commander autonomy](AI-Commander-Autonomy-Audit#b745--b7471-aicom-follow-up), [Performance opportunity sweep](Performance-Opportunity-Sweep), [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack) and [Pruning ledger](Wiki-Pruning-And-Relevance-Ledger) now route the B74.7.1 veteran-slot hard-cap follow-up: `29ca9519..a9145e4da` is one source-Chernarus file / +1 / -1, no `Missions_Vanilla`, `Modded_Missions` or `Tools` payload, and it caps `AI_Commander_Teams.sqf:184` with `min _teamsHardCap` so the veteran `+1` path cannot exceed the eight-team cap. Full branch payload is now 133 files / +4131 / -4399 over `origin/master@f8a76de34`. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | `documentation-finisher-loop` | Latest bounded pass `pr84-player-stats-name-stack-intake-2026-06-24` is published and validated; ready for the next source-backed docs lane. |
...
| --- | --- | --- | --- |
| `documentation-finisher-loop` | Codex | Ongoing / ready | Latest bounded pass `pr84-player-stats-name-stack-intake-2026-06-24` is published/validated; pick the next narrow source-backed docs lane after fresh coordination reads. |
