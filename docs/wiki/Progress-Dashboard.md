...
| Lane | Status | Output |
| --- | --- | --- |
| `server-init-bind-cleanup-route` | Published / validated | Added [Server init bind cleanup](Server-Init-Bind-Cleanup) as the DR-43b owner page. Source check: current docs/source Chernarus, maintained Vanilla, stable and Miksuu upstream keep live duplicate binds for `LogGameEnd`, `PlayerObjectsList` and `AwardScorePlayer` plus commented AFK/FPS/MASH remnants; release Chernarus removed the three live duplicates, but release Vanilla still keeps the old shape. Condensed [Feature status](Feature-Status-Register), [Pending owner decisions](Pending-Owner-Decisions), [SQF atlas](SQF-Code-Atlas) and [Server runtime](Server-Gameplay-Runtime-Atlas). No gameplay source changed. |
