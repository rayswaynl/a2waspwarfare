...
| `economy-atlas-branch-scope-route` | Published / validated | Refreshed [Economy, towns and supply](Economy-Towns-And-Supply) as a branch-scoped economy gateway: added a top route table, current branch scope for docs checkout `6d05cb5a`, stable `origin/master` `cf2a6d6a`, Miksuu `b8389e74`, `perf/quick-wins` `0076040f` and release `a96fdda2`, split truck-only versus heli/cash-run supply reward flow, and aligned the [AI commander autonomy](AI-Commander-Autonomy-Audit#ai-upgrade-debit-branch-matrix) debit matrix so owner-page routing does not contradict current stable/release source. No gameplay source changed. |
