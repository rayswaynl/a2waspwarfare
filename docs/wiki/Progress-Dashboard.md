...
| Lane | Status | Output |
| --- | --- | --- |
| `resource-income-current-b741-b742-refresh-2026-06-23` | Published / validated | Refreshed resource-income cap/payout/display evidence through [Economy, towns and supply](Economy-Towns-And-Supply#resource-income-branch-matrix), [Resource income tick distribution](Resource-Income-Tick-Distribution-Engine), [Feature status](Feature-Status-Register) and [Source Fix queue](Source-Fix-Propagation-Queue). Docs/source `HEAD@52d6b3aa3001` is source-unchanged from `c8ec223a` for checked income files and keeps the old cap guard at `updateresources.sqf:31,49,63,67`. Current stable/B74.1 `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34` and B74.2 `origin/claude/b74.2-aicom@d472da6a` match for checked income files; Chernarus gates side supply/team paychecks at `:69,:87,:101`, uses hybrid-refill/supply-multiplier behavior, keeps over-cap fallback at `:126-132` and adds B74.1 AICOM taper at `:57-67,:106,:130`, while maintained Vanilla keeps non-hybrid/raw-supply/no-taper fallback. `Client_GetIncome.sqf:24-28` still omits the system-4 display multiplier. No gameplay source changed. |
| `send-message-current-b741-b742-refresh-2026-06-23` | Published / validated | Refreshed DR-46 `SEND_MESSAGE` direct compile evidence through [Public variable channel index](Public-Variable-Channel-Index#send_message-direct-compile-branch-matrix), [Networking](Networking-And-Public-Variables), [Feature status](Feature-Status-Register) and [Source Fix queue](Source-Fix-Propagation-Queue). Docs/source `HEAD@cd9ba541d583` is source-unchanged from `3a32be92` / `40c477be` / `16247fc8f` for checked receiver/helper paths. Current stable/B74.1 `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`, B74.2 `origin/claude/b74.2-aicom@d472da6a`, B69 `origin/claude/b69@8d465fce`, adjacent B74 `origin/claude/b74-aicom-spend@b23f557f`, Miksuu, perf and historical `a96fdda2` all keep direct receiver registration at `updateclient.sqf:12`, receiver `call compile` at `Client_onEventHandler_SEND_MESSAGE.sqf:27` and helper compile/broadcast at `Common_SendMessage.sqf:26,37-38` in both maintained roots. Current origin exposes no live release/message/network/security rescue head. No gameplay source changed. |
| `buy-menu-price-key-current-b741-b742-refresh-2026-06-23` | Published / validated | Refreshed buy-menu price/key evidence through [Factory and purchase systems](Factory-And-Purchase-Systems-Atlas#buy-menu-price-and-driver-key-branch-matrix), [Feature status](Feature-Status-Register), [Source Fix queue](Source-Fix-Propagation-Queue), pruning ledger and machine rows. Docs/source checked buy-menu price/key paths remain old-shape and unchanged from `4120137f` / `8d611092` / `ebd86fad0`: selected-detail price drift, no level-0 `UNIT_COST_MODIFIER` reset and driver-key split. Current stable/B74.1 `origin/master@f8a76de34`, B69 `8d465fce`, B74 `b23f557f`, B74.2 `d472da6a` and historical `a96fdda2` fix selected-detail display and driver-key mirroring in both maintained roots, but still lack the level-0 reset. B74.2 only line-drifts source Chernarus AI-cap/constants anchors; `origin/claude/trello-buymenu-gear-display@db1291beba` is branch-only special AT gear/ammo preview evidence, not price/key closure. No gameplay source changed. |
| `factory-refund-current-b741-b742-refresh-2026-06-23` | Published / validated | Refreshed destroyed-factory purchase refund evidence through [Factory and purchase systems](Factory-And-Purchase-Systems-Atlas#destroyed-factory-refund-branch-matrix), [Feature status](Feature-Status-Register) and [Source Fix queue](Source-Fix-Propagation-Queue). Docs/source `HEAD@cacd46f09163` is source-unchanged from `4120137f` / `8d611092` for checked buy/build paths and still debits after `BuildUnit` spawn without passing `_currentCost`; the dead/null factory abort only cleans queue state. Current stable/B74.1 `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`, B69 `origin/claude/b69@8d465fce`, adjacent B74 `origin/claude/b74-aicom-spend@b23f557f`, B74.2 `origin/claude/b74.2-aicom@d472da6a` and historical `a96fdda2` pass `_currentCost` into `BuildUnit` and refund the real dead/null factory abort in both maintained roots. B74.2 only line-drifts source Chernarus buy-menu anchors; old-shape Miksuu/perf/QoL refs remain patch-ready. No gameplay source changed. |
| `side-supply-current-b741-b742-refresh-2026-06-23` | Published / validated | Refreshed side-supply clamp, reason-string and resistance-supply evidence through [Economy authority](Economy-Authority-First-Cut#side-supply-branch-matrix), [Resistance supply scaffold](Resistance-Supply-Scaffold#current-branch-matrix), [Feature status](Feature-Status-Register), [Source Fix queue](Source-Fix-Propagation-Queue), [Public variable index](Public-Variable-Channel-Index) and [Server authority map](Server-Authority-Migration-Map). Docs/source `HEAD@973d817817d9` is source-unchanged from `a908284c` / `4db90f1c` for checked side-supply/resistance paths. Current stable/B74.1 `origin/master@f8a76de34` equals `origin/claude/b74.1-aicom@f8a76de34`; Chernarus now floors server negatives to `0` and wires `wfbe_supply_temp_resistance`, but maintained Vanilla remains old-shape. B74.2 `origin/claude/b74.2-aicom@d472da6a` has no checked side-supply/resistance delta. DR-44 side/channel/requester validation and reason parsing remain open. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | `documentation-finisher-loop` | Latest published lane: `resource-income-current-b741-b742-refresh-2026-06-23`. Continue bounded source-backed docs passes from this dashboard, Feature Status, Source Fix queue, Upstream mining ledger and pruning ledger. |
