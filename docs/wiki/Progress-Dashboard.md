...
| `stable-low-priority-todo-cluster-prune` | Published / validated | Collapsed three vague Feature Status TODO rows into one branch-checked low-priority cluster: common side-presence/defender TODO, CoIn border relocation TODO and AI attack-path polish TODO all remain present across current source/Vanilla, stable, Miksuu upstream, `perf/quick-wins` and release. Owner pages now carry the useful details. No gameplay source changed. |
