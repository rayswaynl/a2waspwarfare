...
| `town-ai-despawn-current-b74-refresh-2026-06-23` | Published / validated | Refreshed [Town AI vehicle despawn safety](Town-AI-Vehicle-Despawn-Safety#current-branch-matrix), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine rows with adjacent B74 evidence. Docs/source `HEAD@3a9ba92e` is unchanged for checked Chernarus/Vanilla `server_town_ai.sqf` paths and still deletes tracked `wfbe_active_vehicles` at `:214` without a player `crew` check. Current stable `origin/master@0139a346`, Miksuu `b8389e748243`, perf `0076040f`, historical `a96fdda2` and live `origin/feat/aicom-fleet-improvements@cc5090bed4ad` remain unsafe in both maintained roots. B69 `origin/claude/b69@8d465fce` and B74 `origin/claude/b74-aicom-spend@b23f557f` are identical for checked paths: Chernarus has the candidate crew guard at `:325`, maintained Vanilla remains unsafe at `:319`, and no checked current ref contains `Server_CleanupExpiredTownDefenseAssets.sqf`. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | `documentation-finisher-loop` | Latest published lane: `attack-wave-authority-current-b74-refresh-2026-06-23`. Continue bounded source-backed docs passes from this dashboard, the refreshed [PR cleanup lab](PR-Cleanup-And-Integration-Lab), Feature Status, Source Fix queue and pruning ledger. |
