...
| Lane | Status | Output |
| --- | --- | --- |
| `supply-scan-current-stable-refresh-2026-06-22` | Published / validated | Refreshed [Supply mission scan narrowing](Supply-Mission-Scan-Narrowing), [Supply mission architecture](Supply-Mission-Architecture), [Supply mission authority cleanup](Supply-Mission-Authority-Cleanup-Playbook), [Performance sweep](Performance-Opportunity-Sweep), [Current source snapshot](Current-Source-Status-Snapshot), [Feature status](Feature-Status-Register) and [Source fix queue](Source-Fix-Propagation-Queue) after older rows mixed `89ae9dad` / `cf2a6d6a` / live-release wording with current stable. Docs `HEAD@15563691` keeps the truck-only typed scan at `supplyMissionStarted.sqf:25,28,44`; current stable `origin/master@0139a346` carries the heli-aware typed scan at `:7,55,61,83`; direct current Miksuu `master@b8389e748243` and perf `0076040f` still broad-enumerate/post-filter at `:25,28,44`; historical `a96fdda2` / `7ff18c49` remain release-line evidence because current origin exposes no `release/*` head. No gameplay source changed. |
| `gear-template-profile-current-head-refresh-2026-06-22` | Published / validated | Refreshed [Gear template profile filter](Gear-Template-Profile-Filter), [Gear/loadout/EASA](Gear-Loadout-And-EASA-Atlas), [Feature status](Feature-Status-Register) and [Source fix queue](Source-Fix-Propagation-Queue) after the profile/template rows still used the 2026-06-21 docs-head anchor. Current docs head `72b5f0de98f9` is source-unchanged from `43c3ba05` for checked template paths and still keeps undefined `_u_upgrade` plus the six-field `Init_ProfileGear` import guard in both maintained roots. Current stable `origin/master@0139a3468609` fixes only the save-filter comparisons; current Miksuu `master@b8389e748243`, perf `0076040f8a5e`, historical release `a96fdda28087` and historical EASA QoL `a66d46912e2a` remain old-shape. No live release/gear/template rescue head was found, and no gameplay source changed. |
| `empty-supply-truck-timeout-current-stable-refresh-2026-06-22` | Published / validated | Refreshed [Marker cleanup/restoration](Marker-Cleanup-Restoration-Systems-Atlas#empty-supply-truck-branch-matrix), [Feature status](Feature-Status-Register) and [Source fix queue](Source-Fix-Propagation-Queue) after the empty supply-truck cleanup row still cited older `cf2a6d6a` stable and live-release wording. Docs head `b4e10b5f`, current stable `origin/master@0139a346`, direct current Miksuu `master@b8389e748243`, perf `0076040f`, B69 `0094647d` and B69 finalpieces `80d3267c` all keep `emptyvehiclescollector.sqf:9-19,30` and supply-truck `_delay = 86400` in both maintained roots. Current origin exposes no live `release/*` or cleanup/supply feature heads; historical `a96fdda2` release wording is not current release proof. No gameplay source changed. |
| `b69-pr47-merged-head-refresh-2026-06-22` | Published / validated | Refreshed the B69 route in [AI commander autonomy audit](AI-Commander-Autonomy-Audit#b69-roadmap-and-sketch-route), [B69 roadmap](AI-Commander-B69-Improvement-Roadmap#current-branch-status), [B69 sketches](AI-Commander-B69-Implementation-Sketches), [Feature status](Feature-Status-Register) and [PR cleanup lab](PR-Cleanup-And-Integration-Lab). `origin/claude/b69@0094647d7b64` now includes PR #47 merged from `claude/b69-finalpieces` at `0094647d7b641bb79202e77e7f480d6d39aadcdb` on 2026-06-22T10:23:16Z. The merge delta from `0a1ccb4d05c5` is 8 Chernarus AI commander/client files / +242 / -13 with no maintained Vanilla diff; full B69 from B68 `b8a1505f` is 17 files / +659 / -78. No gameplay source changed. |
| `service-easa-affordability-current-stable-refresh-2026-06-22` | Published / validated | Refreshed [Service menu affordability guards](Service-Menu-Affordability-Guards), [Gear/loadout/EASA](Gear-Loadout-And-EASA-Atlas), [Feature status](Feature-Status-Register) and [Source fix queue](Source-Fix-Propagation-Queue) after service/EASA rows still mixed older `89ae9dad` / release wording with current stable. Docs `HEAD@9b3fc38e` remains source-unchanged from `8b71e2a1` for checked service/EASA paths, and current Miksuu upstream `master@b8389e748243` keeps old direct service rearm/refuel debits plus strict EASA `_funds > price`; current stable `origin/master@0139a346`, local checkpoint `d9506078` and historical release `a96fdda2` partially guard service rearm/refuel but leave repair/heal positive-price-only plus strict EASA; B69 finalpieces only changes Chernarus EASA to exact-funds `>=`. No gameplay source changed. |
