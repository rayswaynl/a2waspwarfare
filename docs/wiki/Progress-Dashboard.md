...
| `hq-score-current-b741-refresh-2026-06-23` | Published / validated | Refreshed [Commander/HQ lifecycle](Commander-HQ-Lifecycle-Atlas#hq-kill-score-and-bounty-branch-matrix), [Feature status](Feature-Status-Register) and [Source fix propagation queue](Source-Fix-Propagation-Queue) with current stable/B74.1 HQ-score evidence. Docs/source `HEAD@c2d513ecb` is source-unchanged from `9b7eb4bc` for checked HQ-score files. Current stable `origin/master@f8a76de34` equals `origin/claude/b74.1-aicom@f8a76de34` and still awards generic `_points` at `Server_OnHQKilled.sqf:64/:66` plus second `_score = 900` at `:109/:112` in both maintained roots; Chernarus coefficient line drifts to `Init_CommonConstants.sqf:753`, maintained Vanilla stays `:555`, and the value remains `3`. B69/B74/B74.1 base-fall smoke/sting code is not a DR-50 scoring fix. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | `documentation-finisher-loop` | Latest published lane: `source-fix-queue-current-b741-scope-2026-06-23`. Continue bounded source-backed docs passes from this dashboard, the refreshed [PR cleanup lab](PR-Cleanup-And-Integration-Lab), Feature Status, Source Fix queue and pruning ledger. |
