...
| `pr8-release-current-branch-route` | Published / validated | Refreshed [PR cleanup lab](PR-Cleanup-And-Integration-Lab), [Testing workflow](Testing-Debugging-And-Release-Workflow), [Feature status](Feature-Status-Register) and release readiness for current release `a96fdda2`: the release branch is no longer the old `7195b331` / `7ff18c49` head, stable/release carry marker-based commander-built ARTY discovery in both maintained roots, delegated town-AI update-back still needs HC smoke, and static-defense HC update-back remains design work. No gameplay source changed. |
