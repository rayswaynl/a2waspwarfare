...
| `headless-client-init-mirror-sync-2026-06-21` | Published / validated | Synced the repo `docs/wiki` mirror with live wiki `master@e03c3bf` after a top-level SHA256 comparison found four mirror gaps: `_Sidebar.md`, `Home.md`, `agent-context.json` and missing `Headless-Client-Init-And-Stat-Loop.md`. The new page was already source-stamped against `origin/master@0139a346` and documents HC role detection, `Init_HC.sqf`, reseat/deadspawn protection, `HC_StatLoop.sqf`, `HCStat` and `HCSIDE` telemetry routes. No gameplay source changed. |
