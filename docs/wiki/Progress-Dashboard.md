...
| `task-objective-ping-current-b742-refresh-2026-06-24` | Published / validated | Refreshed old town `TaskSystem` / commander Objective Ping branch wording across [Abandoned feature revival](Abandoned-Feature-Revival-Review), [Feature status](Feature-Status-Register), [Commander/HQ lifecycle](Commander-HQ-Lifecycle-Atlas), [Client UI systems](Client-UI-Systems-Atlas), [Networking/public variables](Networking-And-Public-Variables) and machine rows. Repo docs branch `HEAD@c5937045f` still keeps `GUI_Menu_Command.sqf:335,337,343` `SetTask` sends commented plus `Client_TaskSystem.sqf`; current stable/B74.1 `origin/master@f8a76de34`, current B74.2 `origin/claude/b74.2-aicom@21b62b04`, B69, B74 and naval-HVT send targeted Objective Ping at `GUI_Menu_Command.sqf:336,344` with `SetTask` registration at `Init_PublicVariables.sqf:40` and remove the old town helper. Miksuu/perf/c20 remain old-shape; historical `a96fdda2` has live ping sends but no live `release/*` head. No gameplay source changed. |
| `pvf-dispatch-agent-context-current-b742-refresh-2026-06-24` | Published / validated | Refreshed compact [`agent-context.json`](agent-context.json) PVF dispatch wording so future agents route current stable/B74.1 through `origin/master@f8a76de34` and current B74.2 through `origin/claude/b74.2-aicom@21b62b04` instead of older `0139a346` wording. Repo docs branch `HEAD@af208505c229` is unchanged from `925d56e01a1e`/`a0721301d4f5` for checked PVF init/dispatcher/send-helper paths and still dispatches with `Spawn (Call Compile _script)` at `Server_HandlePVF.sqf:14` and `Client_HandlePVF.sqf:22` in both maintained roots. Current stable/B74.1/B74.2/B69/B74 use `missionNamespace getVariable` plus `CODE` guards at server `:14-15` and client `:32-33`; Miksuu/perf/historical `a96fdda2` remain old-shape. Canonical hardening detail remains [PVF Dispatch implementation playbook](PVF-Dispatch-Implementation-Playbook); no gameplay source changed. |
| `wasp-marker-wait-agent-context-current-b742-refresh-2026-06-24` | Published / validated | Refreshed compact [`agent-context.json`](agent-context.json) WASP display-54 wait wording so future agents route current stable/B74.1 through `origin/master@f8a76de34` and current B74.2 through `origin/claude/b74.2-aicom@21b62b04` instead of older `0139a346`/B69-only wording. Repo docs branch `HEAD@8703f4c3b7d5` is unchanged from `46840f048bd4` for checked `Client/Init/Init_Client.sqf` plus `WASP/global_marking_monitor.sqf` paths and still keeps the unslept display-54 loop in both maintained roots. Current stable/B74.1/B74.2/B69/B74/client-fps carry `sleep 0.1` before `findDisplay 54`; Miksuu, perf and historical `a96fdda2` remain old-shape. Canonical detail remains [WASP marker wait cleanup](WASP-Marker-Wait-Cleanup); no gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | `documentation-finisher-loop` | Hosted FPS/status-counter current B74.2 refresh is published; pick the next bounded source-backed documentation lane and claim before editing. |
