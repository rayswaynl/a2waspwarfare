...
| `open-pr-board-post-b751b-refresh-2026-06-24` | Published / validated | Refreshed [PR cleanup and integration lab](PR-Cleanup-And-Integration-Lab#2026-06-24-post-b751b-open-board-refresh) after PR #89/B751b advanced current source stable to `origin/master@f39665d7950ddf14cb0bbfacb7ee1e40121e93b4`. The live open roster remains PR #84, #83, #80, #74, #72, #66, #63, #59, #54, #53 and #9. PR #83 and PR #9 now report `dirty` / not mergeable through `gh api` plus the GitHub connector; PR #80/#74/#72/#66/#63 are `CLEAN` but still based at `f8a76de34` with current master 80 commits ahead; PR #59/#54 are clean but merge-base at `be2bbd084e`; PR #53 is clean but merge-base at `cf2a6d6a`; PR #84 remains a clean stack on `feat/player-stats`, not a `master` PR. PR #85-#89 are merged current-master history, not open cleanup-board items. No gameplay source changed. |
| `b751b-takistan-chernarus-slots-intake-2026-06-24` | Published / validated | Routed PR #89/B751b / `origin/feat/tk-chernarus-slots@05ded7660` as current-master source evidence at `origin/master@f39665d79`. [Current source status](Current-Source-Status-Snapshot#2026-06-24-b745b748-master-merge-intake), [Feature status](Feature-Status-Register), [Source fix queue](Source-Fix-Propagation-Queue), [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack) and [Pruning ledger](Wiki-Pruning-And-Relevance-Ledger) now separate the Takistan-only roster/texture delta from source Chernarus gameplay fixes: scoped `7bfcf09a9..origin/master` is 19 maintained Vanilla Takistan files / +511 / -1224 and clean under `git diff --check`; all changed paths are Takistan `mission.sqm` plus 18 added `Textures/*.paa`, with no source Chernarus, Modded, Tools or Extension payload. Merged Takistan now has 34 `PLAY CDG` slots (14 WEST, 14 EAST, 4 GUER, 2 CIV), ids `7000-7033`, owner syncs at `mission.sqm:599`, `:618` and `:2831`. PR #89 reports B751b boot smoke, but independent package/hash, generated `version.sqf`, RPT and texture-load proof remain release gates. No gameplay source changed. |
| `b751-current-master-aicom-hardcap-intake-2026-06-24` | Published / validated | Routed PR #88/B751 / `origin/feat/b751-tk-aicom13-clientfixes@5aea1ce4016a862de413a198ba8bab0a1ce868c2` as current-master source evidence at `origin/master@7bfcf09a9`. [Current source status](Current-Source-Status-Snapshot#2026-06-24-b745b748-master-merge-intake), [Feature status](Feature-Status-Register), [Source fix queue](Source-Fix-Propagation-Queue), [AI commander autonomy](AI-Commander-Autonomy-Audit#b745--b748-aicom-follow-up), [Client UI systems](Client-UI-Systems-Atlas#current-branch-scope), [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack) and [Pruning ledger](Wiki-Pruning-And-Relevance-Ledger) now route the B751 delta over B750: 14 maintained-root files / +16 / -10; full `f8a76de34..origin/master` is 242 files / +8330 / -9146. Both maintained roots raise `WFBE_C_AICOM_TEAMS_HARD_CAP` to 13 (`Init_CommonConstants.sqf:159`; clamps at `AI_Commander_Teams.sqf:113-114,184`) and add GUER/JIP client-error guards at `updateavailableactions.fsm:58-59`, `Client_UIFillListBuyUnits.sqf:40`, `GUI_Menu.sqf:130`, `GUI_Menu_Tactical.sqf:172`, `Init_Client.sqf:812` and `Action_RepairMHQDepot.sqf:7`. B751 does not touch `Client_UI_Gear_FillCargoList.sqf` or `server_town.sqf`; source `git diff --check` reports trailing whitespace at `updateavailableactions.fsm:59` in both roots. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | None | Published the Source Fix post-B751b current-stable wording cleanup; pick the next bounded source-backed wiki lane from current master, open PRs or patch-ready registers. |
