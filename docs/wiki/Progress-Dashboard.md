...
| `stale-upgrade-dialog-branch-scope` | Published / validated | Source-checked stale `RscMenu_Upgrade`: docs/source, stable and Miksuu master keep the missing-handler class in Chernarus plus maintained Vanilla; release branch removed Chernarus via `460c0312` but left Vanilla stale. Updated [Dead/stale code register](Dead-Code-And-Stale-Code-Register), [Client UI systems](Client-UI-Systems-Atlas), [Upgrades and research](Upgrades-And-Research-Atlas), [Feature status](Feature-Status-Register) and machine backlog records. No gameplay source changed. |
