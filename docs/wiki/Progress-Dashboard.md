...
| `client-modal-loop-cadence-index` | Published / validated | Indexed the under-owned client modal cadence surface from the returned UI loop scout. Current Chernarus and maintained Vanilla both poll Buy Gear, Transfer, Respawn and Upgrade menus at `sleep .01` (`GUI_BuyGearMenu.sqf:503`, `GUI_TransferMenu.sqf:94`, `GUI_RespawnMenu.sqf:113`, `GUI_UpgradeMenu.sqf:282`) with no local `PerformanceAudit_Record` writer in those controllers. Vote/team menus poll at `sleep 0.05`, standard menu controllers mostly at `sleep 0.1`, and representative long-running HUD/marker loops already have audit writers. [Client UI systems](Client-UI-Systems-Atlas#client-modal-loop-cadence) now owns the cadence map; [Performance opportunity sweep](Performance-Opportunity-Sweep) routes the `.01` loops as measurement-first P3 work. No gameplay source changed. |
