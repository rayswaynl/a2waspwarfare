...
| Lane | Status | Output |
| --- | --- | --- |
| `upgrades-research-current-source-route` | Published / validated | Refreshed [Upgrades and research](Upgrades-And-Research-Atlas) and [AI commander autonomy](AI-Commander-Autonomy-Audit#ai-upgrade-debit-branch-matrix) for docs checkout `d4cfef80`: confirmed cited upgrade files are unchanged from the earlier `e785f1e9` line-anchor snapshot, kept stable `cf2a6d6a`, release `a96fdda2`, Miksuu `b8389e74`, perf `0076040f`, AI-commander `c20ce153` and upgrade-queue `b061c905` branch split intact, and aligned AI debit/stale upgrade-dialog status rows in [Feature status](Feature-Status-Register) plus [Source fix propagation queue](Source-Fix-Propagation-Queue). No gameplay source changed. |
| `construction-coin-atlas-current-source-route` | Published / validated | Refreshed [Construction and CoIn systems](Construction-And-CoIn-Systems-Atlas) and the [Construction logic list cleanup](Construction-Logic-List-Cleanup) route for docs checkout `4bd37b98`: updated construction branch scope across stable `cf2a6d6a`, release `a96fdda2`, Miksuu `b8389e74` and perf `0076040f`; compressed repeated auto-wall, StationaryDefense, SmallSite/MediumSite and salvage branch prose into canonical links; aligned [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and the [Function/module index](Function-And-Module-Index) with the 2026-06-14 source-fix anchor. No gameplay source changed. |
| `current-source-status-snapshot-route` | Published / validated | Refreshed [Current source status snapshot](Current-Source-Status-Snapshot) for docs checkout `76674ae4`, stable `cf2a6d6a`, release `a96fdda2`, Miksuu `b8389e74` and perf `0076040f`: added a current routing matrix for Patrols v2, paratrooper PVs, `Skill_Init`, hosted FPS, supply scans/player-list indexing and commander-built ARTY; marked older 2026-06-03/05 matrices historical; and corrected [Source fix propagation queue](Source-Fix-Propagation-Queue) so stable/release are no longer overstated as carrying the supply player-list index fix. No gameplay source changed. |
| `supply-mission-architecture-branch-scope-route` | Published / validated | Refreshed [Supply mission architecture](Supply-Mission-Architecture) as the supply owner page for docs checkout `8a6695b8`: replaced stale PR-vs-master wording with a current branch matrix, split docs/source truck-only typed scan from stable/release heli-aware supply/cash-run shape, corrected cooldown and dead-twin branch status across stable `cf2a6d6a`, Miksuu `b8389e74`, perf `0076040f` and release `a96fdda2`, and aligned [Economy/Towns/Supply](Economy-Towns-And-Supply) plus [Feature status](Feature-Status-Register). No gameplay source changed. |
| `assets-config-parameters-source-scope-route` | Published / validated | Refreshed [Assets/config/localization/parameters atlas](Assets-Config-Localization-And-Parameters-Atlas) as the config/media gateway for docs checkout `85679dba`: added use-route and current branch-scope tables, routed duplicated parameter detail to [Mission parameters](Mission-Parameters-Localization-And-Generated-Build-Inputs) plus [Mission start parameters index](Mission-Start-Parameters-Index), corrected generated `version.sqf` wording across the canonical version route, [Feature status](Feature-Status-Register) and current planning/status rows, and verified Chernarus/Vanilla parameter parity plus compared refs `cf2a6d6a`, `b8389e74`, `0076040f` and `a96fdda2`. No gameplay source changed. |
