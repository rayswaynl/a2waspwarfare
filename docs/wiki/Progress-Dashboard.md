...
| Lane | Status | Output |
| --- | --- | --- |
| `clickabletext-soundpush-current-stable-closeout` | Published / validated | Refreshed the [Client UI systems clickable-text matrix](Client-UI-Systems-Atlas#clickable-text-soundpush-branch-matrix) after current stable moved to `origin/master@0139a346`: both maintained roots now carry valid `RscClickableText.soundPush[] = {"", 0.2, 1};` at `Rsc/Ressources.hpp:556`, with Chernarus fixed by `1a5e0b40` and maintained Vanilla propagated by `9b49883c`. [Feature status](Feature-Status-Register) and [Source fix queue](Source-Fix-Propagation-Queue) now mark the lane source-present on current stable and keep older docs/Miksuu/perf/release/theme evidence branch-scoped. No gameplay source changed. |
