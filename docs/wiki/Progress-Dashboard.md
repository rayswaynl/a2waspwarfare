...
| `factory-purchase-atlas-route-prune` | Published / validated | Refreshed [Factory and purchase systems atlas](Factory-And-Purchase-Systems-Atlas) so current docs head `3b911594` is the visible factory/purchase source-continuity checkpoint. Targeted diffs from `8d611092` to `HEAD` over checked Chernarus and maintained Vanilla factory/purchase paths return no source changes; branch refs still resolve to stable `cf2a6d6a`, release `a96fdda2`, Miksuu `b8389e74`, perf `0076040f` and BuyMenu/EASA QoL `a66d4691`. Replaced repeated buy-menu price/key, destroyed-factory refund and queue-risk prose with routes to the atlas matrices and [Factory queue cleanup](Factory-Queue-Counter-Token-Cleanup). No gameplay source changed. |
