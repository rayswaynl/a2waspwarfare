...
| `wasp-marker-wait-current-stable-propagation-refresh-2026-06-22` | Published / validated | Corrected the WASP marker display-54 wait route across [WASP marker wait cleanup](WASP-Marker-Wait-Cleanup), [Performance opportunity sweep](Performance-Opportunity-Sweep), [WASP overlay](WASP-Overlay), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue), [Current source snapshot](Current-Source-Status-Snapshot) and machine rows. Docs/source `HEAD@46840f048bd4` still has the unslept display-54 loop in Chernarus and maintained Vanilla (`Init_Client.sqf:267`; `global_marking_monitor.sqf:57,62,64,68-69,80`). Current stable `origin/master@0139a3468609` and current B69 `origin/claude/b69@8d465fce` already carry `sleep 0.1` at `global_marking_monitor.sqf:64` in both maintained roots; Chernarus fix commit `4805c778876d`, Vanilla propagation commit `9b49883cb936`. Current Miksuu `b8389e748243` and perf `0076040f8a5e` remain old-shape, and current origin exposes no `release/*` heads. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | `documentation-finisher-loop` | Latest published lane: `camp-count-helper-current-b69-head-refresh-2026-06-22`. Continue bounded source-backed docs passes from this dashboard, the refreshed [PR cleanup lab](PR-Cleanup-And-Integration-Lab), Feature Status, Source Fix queue and pruning ledger. |
