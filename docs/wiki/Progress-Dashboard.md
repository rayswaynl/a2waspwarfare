...
| Lane | Status | Output |
| --- | --- | --- |
| `july-update-route-sync` | Published / validated | Routed the two July dev-branch candidates from this dashboard into [Hardening roadmap](Hardening-Implementation-Roadmap#july-update-candidate-route), [Testing workflow](Testing-Debugging-And-Release-Workflow#july-update-candidate-smoke-route), [`agent-machine-index.json`](agent-machine-index.json) and [`agent-release-readiness.json`](agent-release-readiness.json) so future code owners can start from normal agent entrypoints. No gameplay source changed. |
| `dead-code-oa-compatibility-scan-refresh` | Published / validated | Reran the Arma 2 OA / Arma 3-style API scan with PowerShell 7 and refreshed [Dead/stale code register](Dead-Code-And-Stale-Code-Register): `docs/analysis/dead-code-oa-compatibility-scan.json` now scans `3205` text files, still reports `0` code-risk implementation hits, and records `416` docs/reference hits plus `1132` OA-safe inverse-trap hits. The register now tells agents to run this scanner with `pwsh` because Windows PowerShell 5.1 lacks the required `System.IO.Path.GetRelativePath` API. |
| `old-be-current-wasp-fps-opportunity-scout-2` | Returned / harvested | The final read-only FPS scout report is folded into the canonical old-BE/performance/testing pages without mirroring the full report. [Old WarfareBE performance comparison](Old-WarfareBE-Performance-Comparison#2026-06-05-final-fps-scout-appendix) records the report path and new deltas, [Performance sweep](Performance-Opportunity-Sweep) carries the refined order, [Testing workflow](Testing-Debugging-And-Release-Workflow#full-server-fps-opportunity-pack) adds AI-source/static-HC/view-distance gates, and [Feature status](Feature-Status-Register) keeps the dynamic cap as owner-decision work. No gameplay source changed. |
| `server-runtime-operations-gateway` | Published / validated | Added [Server runtime and operations](Server-Runtime-And-Operations) as a thin gateway for prompts that ask for server runtime or operations. It routes to [Server gameplay runtime atlas](Server-Gameplay-Runtime-Atlas), [Server ops runbook](Server-Ops-Runbook), integrations, authority, HC, performance and testing owner pages without duplicating their evidence. Navigation, MkDocs, `llms.txt`, machine context and inventory counts were updated. No gameplay source changed. |
| `fps-helper-lane-compaction` | Published / validated | Removed harvested Old BE/FPS helper windows from live lane surfaces in this dashboard, [`agent-status.json`](agent-status.json) and [`agent-collaboration.json`](agent-collaboration.json). Their reports remain preserved in [Agent worklog](Agent-Worklog), [`agent-events.jsonl`](agent-events.jsonl) and the canonical old-BE/player-cap/performance/testing/Feature Status pages. No gameplay source changed. |
| `old-fps-provenance-cleanup` | Published / validated | Removed stale docs-branch commit wording from [Old WarfareBE performance comparison](Old-WarfareBE-Performance-Comparison): the page now routes current Wasp truth through source paths and [Current source status snapshot](Current-Source-Status-Snapshot) instead of freezing the archaeology baseline to an old docs-only commit hash. Refreshed [`agent-machine-index.json`](agent-machine-index.json) metadata after the FPS opportunity harvest. No gameplay source changed. |
