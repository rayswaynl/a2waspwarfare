...
| `commander-hq-source-scope-route` | Published / validated | Refreshed [Commander and HQ lifecycle](Commander-HQ-Lifecycle-Atlas) as the commander/HQ gateway for docs checkout `f82a9127`: added use-route and current branch-scope tables, corrected DR-15 helper wording to branch-split status, refreshed HQ score double-award line drift, and aligned [Feature status](Feature-Status-Register) plus [Source fix propagation queue](Source-Fix-Propagation-Queue) for HQ score, reassignment and Objective Ping branch evidence across stable `cf2a6d6a`, Miksuu `b8389e74`, perf `0076040f`, release `a96fdda2` and feat-ai `c20ce153`. No gameplay source changed. |
