...
| `side-supply-compact-context-current-b742-refresh-2026-06-24` | Published / validated | Refreshed side-supply compact context and machine-index routing so DR-22 no longer points at older `origin/master@0139a346` evidence. Repo docs branch `HEAD@cce86ab80a5d` is unchanged from `3bedb86dca88` for checked side-supply/resistance/reason paths and remains old-shape. Current stable/B74.1 `origin/master@f8a76de34` and current B74.2 `origin/claude/b74.2-aicom@21b62b04` match for checked side-supply/resistance paths: source Chernarus floors west/resistance/east server negatives to `0` and adds `wfbe_supply_temp_resistance`, while maintained Vanilla remains old-shape. Reason parsing and side/channel/amount/requester validation remain open. No gameplay source changed. |
| `supply-cooldown-current-b742-context-refresh-2026-06-24` | Published / validated | Refreshed supply cooldown/authority compact context and short owner-page wording so DR-18 is branch-scoped instead of routed through older `origin/master@0139a346` evidence. Repo docs branch `HEAD@915f4441c1c9` is unchanged from `15563691` for checked supply paths and, like Miksuu `b8389e748243` and perf `0076040f`, keeps the lowercase seed vs capital read cooldown split. Current stable/B74.1 `origin/master@f8a76de34`, current B74.2 `origin/claude/b74.2-aicom@21b62b04`, B69 and adjacent B74 seed/read `LastSupplyMissionRun` with default `0` and carry the supply-heli typed-scan/dead-twin cleanup shape in both maintained roots. No gameplay source changed. |
| `ai-commander-supervisor-current-b742-route-refresh-2026-06-24` | Published / validated | Refreshed AI commander supervisor/run-flag branch wording across [AI headless/performance](AI-Headless-And-Performance#current-branch-scope), [AI commander autonomy audit](AI-Commander-Autonomy-Audit), [Feature status](Feature-Status-Register) and compact machine rows. Repo docs branch `HEAD@c0e865626ff2` lacks checked maintained-root `AI_Commander.sqf` / `AI_Commander_Produce.sqf` and keeps old init scaffolding/reset evidence. Current stable/B74.1 `origin/master@f8a76de34` compiles/spawns the supervisor, sets/clears `wfbe_aicom_running`, calls the upgrade worker and calls `AIBuyUnit` for AI-team production in both maintained roots. Current B74.2 `origin/claude/b74.2-aicom@21b62b04` keeps maintained Vanilla stable-shaped and adds source-Chernarus-only pop-tier/GUER drift. Miksuu/perf/historical old refs lack the maintained-root supervisor files. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | None | Latest pass `mission-entrypoints-lifecycle-current-b742-route-refresh-2026-06-24` is published/validated; next agent should pick a fresh bounded source-backed lane after coordination reads. |
...
| --- | --- | --- | --- |
| `documentation-finisher-loop` | Codex | Ongoing / ready | Latest bounded pass `mission-entrypoints-lifecycle-current-b742-route-refresh-2026-06-24` is published/validated; next work should claim a new narrow source-backed docs lane before editing. |
