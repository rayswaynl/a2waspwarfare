...
| Lane | Status | Output |
| --- | --- | --- |
| `trello-artillery-ux-branch-intel-2026-06-23` | Published / validated | Routed draft PR #79 / `origin/claude/trello-artillery-ux@c4459d4312ef` through [Support specials](Support-Specials-And-Tactical-Modules-Atlas#trello-artillery-ui-branch-intel), [Client UI systems](Client-UI-Systems-Atlas#current-branch-scope), [Feature status](Feature-Status-Register), [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack) and release-readiness gates. The branch merge-bases directly on current stable `origin/master@f8a76de34`; payload is eight maintained-root files / +100 / -20 and `git diff --check` clean. It adds effective min/max range text to Tactical artillery rows at `GUI_Menu_Tactical.sqf:42-57`, passes firing-gun count into the artillery team warning at `Client_RequestFireMission.sqf:58-60`, and moves barrage marker removal to server-side delayed `deleteMarker` through new `ArtyMarkerCleanup`. Current stable lacks those branch rows/placeholders/tags and still deletes these markers locally from the caller. No gameplay source changed. |
| `trello-map-radius-circles-branch-intel-2026-06-23` | Published / validated | Routed draft PR #78 / Trello #80/#76 / `origin/claude/trello-map-radius-circles@77dd71ba` through [Client UI systems](Client-UI-Systems-Atlas#current-branch-scope), [Commander/HQ lifecycle](Commander-HQ-Lifecycle-Atlas#current-branch-scope), [Construction and CoIn systems](Construction-And-CoIn-Systems-Atlas#current-branch-scope), [Respawn lifecycle](Respawn-And-Death-Lifecycle-Atlas#branch-intel---mobile-respawn-map-circles), [Medic redeployment truck](Medic-Redeployment-Truck-Forward-Spawn#branch-intel---map-radius-circle-candidate), [Feature status](Feature-Status-Register), [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack) and release-readiness gates. The branch is one commit on current stable merge-base `origin/master@f8a76de34`; payload is six maintained-root files / +192 / -4 and `git diff --check` is clean. It adds local HQ build-area circles with radius `WFBE_C_BASE_AREA_RANGE + WFBE_C_BASE_HQ_BUILD_RANGE` and local yellow ambulance/redeploy range circles from `WFBE_C_RESPAWN_RANGES`, in source Chernarus plus maintained Vanilla Takistan. Current stable has the constants/lists but no `Client_AmbulanceRedeployCircles`, `AmbRange_` or `HQBuildRange` symbols. The watcher does not duplicate full redeploy eligibility gates, so promotion needs explicit mismatch smoke. No gameplay source changed. |
| `trello-upgrade-icons-branch-intel-2026-06-23` | Published / validated | Routed draft PR #77 / Trello #164 / `origin/claude/trello-upgrade-icons@baf8d9c304ad` through [Client UI systems](Client-UI-Systems-Atlas#current-branch-scope), [Upgrades and research](Upgrades-And-Research-Atlas#current-branch-scope), [Feature status](Feature-Status-Register), [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack) and release-readiness gates. The branch is one commit on current stable merge-base `origin/master@f8a76de34`; payload is two maintained-root `GUI_UpgradeMenu.sqf` files / +8 and `git diff --check` is clean. It paints existing `WFBE_C_UPGRADES_IMAGES` entries into upgrade-list column `0` at `GUI_UpgradeMenu.sqf:27-28` and `:173-174`, using image paths already defined in `Labels_Upgrades.sqf:104-125`. Current stable has no upgrade-menu `lnbSetPicture` calls. No gameplay source changed. |
| `trello-auto-bipod-branch-intel-2026-06-23` | Published / validated | Routed draft PR #76 / `origin/claude/trello-auto-bipod@5af02b8438e1` through [Deployable bipod / weapon resting](Deployable-Bipod-Weapon-Resting#branch-intel---trello-auto-bipod-candidate), [Feature status](Feature-Status-Register), [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack) and release-readiness gates. The branch is one commit on current stable merge-base `origin/master@f8a76de34`; payload is four maintained-root files / +46 / -2 and `git diff --check` is clean. It adds `Bipod_AddAutoDeploy` at `Common_Bipod.sqf:74-82`, attaches local `AnimChanged`, and calls the existing `15 call Bipod_ON` path on prone animation `amovppnemstpsraswrfldnon`; `Client_OnKilled.sqf:90` reattaches after death/respawn if the helper exists. Current stable lacks those auto-deploy symbols. This is client-local branch behavior, not a whitelist, localization, recoil-model, sound-media, server or PV change. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | `documentation-finisher-loop` | Latest published lane: `trello-artillery-ux-branch-intel-2026-06-23`. Continue bounded source-backed docs passes from this dashboard, Feature Status, Source Fix queue, Upstream mining ledger and pruning ledger. |
