...
| `client-ui-atlas-continuity-route` | Published / validated | Refreshed [Client UI systems atlas](Client-UI-Systems-Atlas), [UI resource parity cleanup](UI-Resource-Parity-Cleanup), [UI IDD collision repair](UI-IDD-Collision-Repair), [Feature status](Feature-Status-Register) and [Source fix propagation queue](Source-Fix-Propagation-Queue) so current docs checkout `b5219d47` is the visible source-continuity checkpoint for checked UI/Rsc paths. Verified no checked UI source changes from `a71b42fe`, `2fef1e3d` or `f7bc72a8` to `HEAD`; stable `cf2a6d6a`, release `a96fdda2`, Miksuu `b8389e74` and perf `0076040f` remain routed through owner matrices. No gameplay source changed. |
