...
| `dead-code-ai-supply-truck-current-stable-refresh-2026-06-22` | Published / validated | Refreshed the [Dead/stale code AI supply-truck worker row](Dead-Code-And-Stale-Code-Register#sqf-reachability-findings) after it still cited current `origin/master cf2a6d6a` for the safe-disable. Current stable `origin/master@0139a346` comments the worker compile at `Init_Server.sqf:43`, initializes `wfbe_ai_supplytrucks`, and warning-disables legacy logistics at `:462-463` in both maintained roots; the docs checkout and `origin/perf/quick-wins@0076040f` still raw-spawn the uncompiled worker, and `AI_UpdateSupplyTruck.sqf:17` still points at missing `Server/FSM/supplytruck.fsm`. Historical `a96fdda2` safe-disables, while historical `c20ce153` guards Chernarus only. No gameplay source changed. |
| `camp-flag-texture-current-stable-refresh-2026-06-22` | Published / validated | Refreshed the [Towns/camps/capture camp flag matrix](Towns-Camps-And-Capture-Atlas#current-branch-scope), [Feature status](Feature-Status-Register) and [Source fix queue](Source-Fix-Propagation-Queue) after lower rows still cited stale stable `origin/master cf2a6d6a` evidence. Docs branch `28a7d9c5` and Miksuu `b8389e74` still use old-owner independent-capture flags; `perf/quick-wins@0076040f` fixes Chernarus only; current stable `origin/master@0139a346`, historical `a96fdda2`, B69 `origin/claude/b69@0a1ccb4d` and new `origin/feat/naval-hvt-objectives@2e1c5931` fix independent capture in both maintained roots. `repair-camp` still changes side/broadcasts without flag refresh everywhere checked. No gameplay source changed. |
