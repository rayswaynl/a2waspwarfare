...
| `buy-menu-price-key-current-master-b750-refresh-2026-06-24` | Published / validated | Refreshed the [Factory and purchase systems](Factory-And-Purchase-Systems-Atlas#buy-menu-price-and-driver-key-branch-matrix) buy-menu price/key matrix after fetch-before-commit advanced current stable to `origin/master@7bfcf09a9` via PR #88/B751 on top of PR #87/B750. Source check found current master keeps selected-detail display, list price and purchase debit on the modifier-bearing formula in both maintained roots (`Client_UIFillListBuyUnits.sqf:96`; `GUI_Menu_BuyUnits.sqf:105,:420,:621`) and mirrors driver-default lowercase plus uppercase compatibility writes (`GUI_Menu_BuyUnits.sqf:48-49,:194-195,:443-444,:488,:494,:500,:533-534,:546-547`). The remaining current-master price/key defect is still the level-0 `UNIT_COST_MODIFIER` reset: `Init_CommonConstants.sqf:661` initializes once and `Client_UIFillListBuyUnits.sqf:11,14` only write the discounted levels. B751 is now current-master adjacent hardening (`Client_UIFillListBuyUnits.sqf:40` faction-filter nil `_c` guard; `Init_CommonConstants.sqf:159` AICOM hard cap), not a price/key closure. No gameplay source changed. |
| `b750-factory-purchase-owner-route-2026-06-24` | Published / validated | Routed the purchase-side B750 evidence from PR #87 / `origin/feat/b750-factory-aicom@605ad7326` into [Factory and purchase systems](Factory-And-Purchase-Systems-Atlas#b750-buy-list-and-squad-cap-current-master-route) instead of leaving it only on broad status/UI pages. Source check against `origin/master@c4c9a6479` found the B750 purchase subset is four maintained-root files / +24 / -16 and clean under `git diff --check`: both source Chernarus and maintained Vanilla guard nil `_c` before `QUERYUNITUPGRADE` at `Client_UIFillListBuyUnits.sqf:88-92`, restore infantry cap growth at `GUI_Menu_BuyUnits.sqf:137-138`, and keep commander bonus at `:139-145`. [Feature status](Feature-Status-Register), [Pruning ledger](Wiki-Pruning-And-Relevance-Ledger), worklog and machine rows now route buy-list/squad-cap smoke through the factory owner page while leaving AICOM effectiveness on [AI commander autonomy](AI-Commander-Autonomy-Audit#b745--b748-aicom-follow-up). No gameplay source changed. |
| `b745-loadoutmanager-vanilla-merge-intake-2026-06-24` | Published / validated | Fetch-before-commit found PR #87 / `origin/feat/b750-factory-aicom@605ad7326` merged on top of PR #85, PR #86 and direct B75, advancing current stable to `origin/master@c4c9a6479`. [Feature status](Feature-Status-Register), [Current source status](Current-Source-Status-Snapshot#2026-06-24-b745b748-master-merge-intake), [Source fix queue](Source-Fix-Propagation-Queue), [Tools/build workflow](Tools-And-Build-Workflow#loadoutmanager), [Player join/disconnect](Player-Join-Disconnect-And-AntiStack-Lifecycle#branch-intel---b745b747-jip-enrollment), [AI commander autonomy](AI-Commander-Autonomy-Audit#b745--b748-aicom-follow-up), [Client UI systems](Client-UI-Systems-Atlas#current-branch-scope), [Performance sweep](Performance-Opportunity-Sweep), [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack) and [Pruning ledger](Wiki-Pruning-And-Relevance-Ledger) now route the B745/B748/B749/B75/B750 source evidence: broad B745 226 files / +7611 / -9082, B748 9 files / +265 / -7, B749 14 files / +308 / -21, B75 2 maintained-root files / +88 / -2, B750 8 maintained-root files / +72 / -54, full `f8a76de34..origin/master` 238 files / +8316 / -9138. Maintained Vanilla now carries the checked B74.7.2 nil-side retry, B748 settings, B75 VBIED reward and B750 buy-list/AICOM changes; target-package smoke, generated `version.sqf`, LoadoutManager output and release/package proof remain open. No gameplay source changed. |
| `pr84-player-stats-name-stack-intake-2026-06-24` | Published / validated | Routed open non-draft PR #84 / `origin/feat/player-stats-names@177539ed` as a stack on `origin/feat/player-stats@e01e47e1`, not current stable or deployed behavior. [PR cleanup](PR-Cleanup-And-Integration-Lab#2026-06-24-pr-84-player-stats-name-stack-intake), [Player stats branch audit](Player-Stats-Branch-Audit#pr-84-player-name-stack), [Feature status](Feature-Status-Register), [Current source status](Current-Source-Status-Snapshot#2026-06-24-pr84-player-stats-name-stack-intake), [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack) and [Pruning ledger](Wiki-Pruning-And-Relevance-Ledger) now record the PR #84 branch topology: clean GitHub merge state, one scoped commit / 6 files / +57 / -7, clean `git diff --check`, no `Missions_Vanilla`, `Modded_Missions`, `Tools` or `Extension` payload. Source Chernarus `StatsFlush.sqf` appends optional `~<name>`; DiscordBot parses/accumulates/serializes nullable `name`; test rerun is blocked locally by missing .NET SDK, so Arma RPT name/separator smoke plus SDK-backed tests remain open. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | None | Pick the next bounded source-backed wiki lane from current master, open PRs or patch-ready registers; avoid gameplay source edits unless explicitly requested. |
