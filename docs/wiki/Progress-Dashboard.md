...
| Lane | Status | Output |
| --- | --- | --- |
| `service-easa-current-b741-b742-branch-refresh-2026-06-23` | Published / validated | Refreshed service/EASA affordability and branch-only UI/wiring evidence through [Service menu affordability guards](Service-Menu-Affordability-Guards#current-branch-matrix), [Gear/loadout/EASA atlas](Gear-Loadout-And-EASA-Atlas#current-branch-scope), [Feature status](Feature-Status-Register), [Source Fix queue](Source-Fix-Propagation-Queue), pruning ledger and machine rows. Docs/source `HEAD@b798fd66` remains old-shape and unchanged from `e9dd7f37` / `8906ee89` / `9b3fc38e` / `8b71e2a1` for checked service/EASA paths. Current stable/B74.1 `origin/master@f8a76de34`, B74.2 `d472da6a`, B69 `8d465fce`, B74 `b23f557f` and historical `a96fdda2` partially guard service rearm/refuel in both maintained roots but leave repair/heal action-time affordability/context open; Chernarus exact-funds EASA is fixed, maintained Vanilla remains strict `>`. `origin/claude/trello-service-ammo-count@9159f935` is branch-only service ammo-percent UI; `origin/claude/trello-easa-weapon-categories@08819118` is branch-only LoadoutCat wiring. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | `none` | Last batch published `service-easa-current-b741-b742-branch-refresh-2026-06-23`; continue with a fresh bounded docs-only lane after coordination reads. |
