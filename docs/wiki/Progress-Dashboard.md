...
| `version-sqf-loadoutmanager-current-b741-refresh-2026-06-23` | Published / validated | Refreshed generated `version.sqf` and LoadoutManager release-gate evidence through [Mission config/version graph](Mission-Config-Version-Include-Graph), [Tools/build workflow](Tools-And-Build-Workflow), [Source Fix queue](Source-Fix-Propagation-Queue), Feature Status, release-readiness and agent machine rows. Docs/source `HEAD@7b1187d32`, current stable/B74.1 `origin/master@f8a76de34`, B74.2 `origin/claude/b74.2-aicom@d472da6a`, B69, B74, Miksuu `b8389e748243`, perf `0076040f` and historical `a96fdda2` do not track live Chernarus/Vanilla `version.sqf`; stable-shaped B74 refs track only the Chernarus template. `A2WASP_SKIP_ZIP` support is docs/source-only in checked refs; no gameplay source changed. |
| `fast-travel-fee-current-b741-b742-refresh-2026-06-23` | Published / validated | Refreshed Tactical fast-travel fee evidence through [Client UI systems](Client-UI-Systems-Atlas#tactical-fast-travel-fee-branch-matrix), [Feature status](Feature-Status-Register), [Source Fix queue](Source-Fix-Propagation-Queue) and machine rows. Current stable/B74.1 `origin/master@f8a76de34` equals `origin/claude/b74.1-aicom@f8a76de34` and keeps the paid-mode local Tactical behavior in both maintained roots: hidden unaffordable destinations and `$fee` marker text at `GUI_Menu_Tactical.sqf:185,195-196,215`, local debit at `:403-404`, source Chernarus constants `Init_CommonConstants.sqf:612,621-624` and maintained Vanilla `:414,423-426`. B74.2 `origin/claude/b74.2-aicom@d472da6a` only drifts source Chernarus constants to `:625,634-637` among checked fast-travel paths. Docs/source, current Miksuu, perf and historical release keep the old TODO/line shape. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | `documentation-finisher-loop` | Latest published lane: `side-supply-current-b741-b742-refresh-2026-06-23`. Continue bounded source-backed docs passes from this dashboard, Feature Status, Source Fix queue, Upstream mining ledger and pruning ledger. |
