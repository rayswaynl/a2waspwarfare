...
| `old-vs-current-fps-opportunity-harvest` | Published / validated | Promoted the completed FPS opportunity scout report into canonical docs without duplicating the full report: [Performance sweep](Performance-Opportunity-Sweep) now ranks the opportunity pack, [Player AI caps](Player-AI-Caps-And-Role-Balance#dynamic-cap-proposal) carries the role-aware dynamic cap table, [Old WarfareBE comparison](Old-WarfareBE-Performance-Comparison#2026-06-05-opportunity-audit-appendix) links the appendix, [Testing workflow](Testing-Debugging-And-Release-Workflow#full-server-fps-opportunity-pack) owns the 10/15/20/25/30 matrix, and [Feature status](Feature-Status-Register) tracks the cap/scoreboard as owner-decision gameplay/performance work. No gameplay source changed. |
