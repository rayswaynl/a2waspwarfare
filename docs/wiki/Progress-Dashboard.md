...
| Lane | Status | Output |
| --- | --- | --- |
| `gear-template-current-stable-scope-refresh-2026-06-21` | Published / validated | Refreshed the [Gear template profile filter](Gear-Template-Profile-Filter), [Gear/loadout/EASA atlas](Gear-Loadout-And-EASA-Atlas), [Feature status](Feature-Status-Register), [Source fix queue](Source-Fix-Propagation-Queue) and machine rows after source checks split the old flattened `_u_upgrade` finding: current docs `docs/developer-wiki-index@43c3ba05` remains source-unchanged from `8b71e2a1` and still has `_u_upgrade` at `Client_UI_Gear_SaveTemplateProfile.sqf:33,52,75`, while current stable `origin/master@0139a346` fixes the save-filter comparisons at `:34,57,82` in both maintained roots. `Init_ProfileGear.sqf:17,25` import bounds and AddTemplate/FillTemplates policy remain open; Miksuu `b8389e74`, perf `0076040f`, historical `a96fdda2` and historical `a66d4691` keep old save-filter shape. No gameplay source changed. |
| `factory-destroyed-purchase-refund-current-stable-refresh-2026-06-21` | Published / validated | Refreshed the [Factory and purchase systems refund matrix](Factory-And-Purchase-Systems-Atlas#destroyed-factory-refund-branch-matrix), [Feature status](Feature-Status-Register), [Source fix queue](Source-Fix-Propagation-Queue) and hardening backlog after current stable moved from old `cf2a6d6a` line anchors to `origin/master@0139a346`. Current stable and historical `a96fdda2` refund the dead/null factory abort in both maintained roots; docs-source `ebd86fad0`, Miksuu, perf and historical QoL `a66d4691` remain old-shape with no dead/null refund. Intermediate `7ff18c49` only refunded the empty/crewless branch and is superseded. Current origin exposes no `release/*` or `feat/buymenu-easa-qol` heads. No gameplay source changed. |
| `commander-reassignment-current-stable-closeout-2026-06-21` | Published / validated | Refreshed [Commander reassignment call shape](Commander-Reassignment-Call-Shape#current-branch-matrix), [Feature status](Feature-Status-Register) and [Source fix queue](Source-Fix-Propagation-Queue) after a 2026-06-21 recheck showed current stable `origin/master@0139a346` Chernarus and maintained Vanilla already unpack `_side = _this select 0` / `_commander = _this select 1` at `Server_AssignNewCommander.sqf:4-5`. Docs branch `b44aaaf8` and current stable full modded Napf/Eden/Lingor forks still carry `_side = _this`; duplicate `new-commander-assigned` senders at `RequestNewCommander.sqf:14` and helper `:10`, visible-name UI targeting and requester authority remain open. No gameplay source changed. |
| `patrols-v2-current-stable-refresh-2026-06-21` | Published / validated | Refreshed the [Towns/camps/capture atlas](Towns-Camps-And-Capture-Atlas#patrols-v2-side-upgrade-path), [AI runtime/HC loop map](AI-Runtime-HC-Loop-Map#current-branch-scope), [AI/headless gateway](AI-Headless-And-Performance#current-branch-scope), [Server runtime atlas](Server-Gameplay-Runtime-Atlas#branch-scope-for-source-anchors), [Feature status](Feature-Status-Register), [Source fix queue](Source-Fix-Propagation-Queue) and live machine rows against current stable `origin/master@0139a346`. Current stable and maintained Vanilla still carry Patrols v2 through `WFBE_UP_PATROLS = 23`, side/defender caps, `server_side_patrols.sqf`, `Common_RunSidePatrol.sqf`, `Server_HandleSpecial.sqf` level-4 convoy/camp-sweep hooks, friendly `updatepatrolmarkers.sqf` markers and `server_patrols.sqf:31` `&&`. Current origin exposes no `release/*` heads; `a96fdda2` remains historical evidence. No gameplay source changed. |
