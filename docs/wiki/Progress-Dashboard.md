...
| Lane | Status | Output |
| --- | --- | --- |
| `b68-aicom-live-branch-route-2026-06-21` | Published / validated | Routed PR #43 / `origin/claude/b57-soak-proposals@b8a1505f` B68 evidence into the [AI commander audit](AI-Commander-Autonomy-Audit#pr-43--b68-live-soak-branch), [marker catalog](Map-Marker-Families-Content-Catalog#aicom-wildcard-events-server-global), [Feature status](Feature-Status-Register) and [PR cleanup lab](PR-Cleanup-And-Integration-Lab). B68 is live Chernarus branch evidence, not merged-master/release wording: commit `b8a1505f` removes the W17 global Supply Convoy marker route at `AI_Commander_Wildcard.sqf:994`, adds/tunes AICOM attack/retreat controls at `Init_CommonConstants.sqf:277-284,319`, `AI_Commander_Strategy.sqf:41-68,99,441,571-590` and `AI_Commander_Produce.sqf:90-151`, while current `origin/master@0139a346` still creates the global W17 marker at `AI_Commander_Wildcard.sqf:981-984`. No gameplay source changed. |
| `upgrade-sync-current-stable-refresh-2026-06-21` | Published / validated | Refreshed the [Support specials upgrade-sync matrix](Support-Specials-And-Tactical-Modules-Atlas#upgrade-sync-branch-matrix), [Upgrades and research](Upgrades-And-Research-Atlas#current-branch-scope), [Feature status](Feature-Status-Register) and [Source fix queue](Source-Fix-Propagation-Queue) after current stable moved from old `cf2a6d6a` evidence to `origin/master@0139a346`. Current stable still keeps the mixed `_args` / `_this` parser at `Server_HandleSpecial.sqf:67-73` in Chernarus and maintained Vanilla; the caller moved to `GUI_UpgradeMenu.sqf:292`, the sync lifecycle is now `Server_ProcessUpgrade.sqf:32,35,41`, docs-source base `ad05401a3` is unchanged from `ff8dd884` for checked paths, and `a96fdda2` / `b061c905` are historical because current origin exposes no `release/*` or `feat/upgrade-queue-stacking` heads. No gameplay source changed. |
| `commander-vote-current-stable-closeout-2026-06-21` | Published / validated | Refreshed [Commander vote/reassignment](Commander-Vote-And-Reassignment-Playbook#current-branch-scope), [Feature status](Feature-Status-Register) and [Source fix queue](Source-Fix-Propagation-Queue) after current stable `origin/master@0139a346` proved the old DR-47 server vote tautology is source-present-fixed in both maintained roots: `_aiVotes` is still counted at `Server_VoteForCommander.sqf:18,26-27`, but player assignment now requires `_highest >= _aiVotes` at `:43`. UI preview at `GUI_VoteMenu.sqf:88` still needs policy/smoke alignment; Miksuu `b8389e74`, perf `0076040f`, historical `a96fdda2` and historical `c20ce153` keep the old `>= || <=` condition. No gameplay source changed. |
| `victory-endgame-current-stable-refresh-2026-06-21` | Published / validated | Refreshed the [Victory/endgame atlas](Victory-And-Endgame-Atlas#current-branch-scope), [Feature status](Feature-Status-Register) and [Source fix queue](Source-Fix-Propagation-Queue) after current stable `origin/master@0139a346` proved the old `cf2a6d6a` / release-head wording was stale. Docs/Miksuu/perf/historical `a96fdda2` keep the older opposite-side logger block (`server_victory_threeway.sqf:23-41`), where all-towns wins log the loser. Current stable adds the HQ nil/null skip (`:18-22`) and uses `[_x]` for WASPSTAT/logger (`:41-49`), so HQ/factory eliminations still log the loser and the full-condition guard/side-loop exit remain open. `origin/claude/b57-soak-proposals@b8a1505f` has Chernarus-only guard/client-label edits, not stable or Vanilla propagation. Current origin exposes no `release/*` heads. No gameplay source changed. |
| `source-fix-queue-current-stable-scope-refresh-2026-06-21` | Published / validated | Refreshed [Source fix propagation queue](Source-Fix-Propagation-Queue#current-branch-scope-2026-06-21) from stale stable/release branch-scope wording to checked docs-source base `docs/developer-wiki-index@d30d23466` and current stable `origin/master@0139a346`. The queue now names current stable queue PVFs/paratrooper registration, `Skill_Init`/apply, single FPS publisher, heli-aware supply scan, marker-based commander ARTY and Patrols v2 `Server/FSM` paths, keeps supply player-list indexing docs-only, and treats `a96fdda2` as historical because current origin exposes no `release/*` heads. No gameplay source changed. |
...
| `indicator-exploration-backlog` | Codex / future UI scout | Matrix seeded / per-family detail remains | Use the seeded [indicator surface matrix](Client-UI-Systems-Atlas#indicator-surface-matrix) to pick one family at a time for deeper branch/root validation, owner-page integration or pruning; do not restart a broad "all indicators" checklist. |
...
- Salvage payout/loop branch scope: [Construction and CoIn systems](Construction-And-CoIn-Systems-Atlas#salvage-branch-matrix), [Feature status](Feature-Status-Register) and [Source fix propagation queue](Source-Fix-Propagation-Queue) now treat salvage as inherited current-source/current-stable debt: docs checkout `10097961`, current stable `origin/master@0139a346`, Miksuu `b8389e74`, perf `0076040f` and historical release commit `a96fdda2` all keep the lowercase payout call, `||` loop and client-local deletion/reward shape; current origin has no live `release/*` head.
