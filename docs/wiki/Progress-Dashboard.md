...
| `commander-arty-smoke-lane-dashboard-sync` | Published / validated | Synced this dashboard and [`agent-status.json`](agent-status.json) with [Coordination board](Coordination-Board): [commander-built ARTY](Construction-And-CoIn-Systems-Atlas) is part of the source-propagated smoke-pending lane, alongside client skill init, hosted FPS, supply scan and paratrooper markers. No gameplay source changed. |
