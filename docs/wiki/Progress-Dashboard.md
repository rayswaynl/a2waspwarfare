...
| `client-ui-title-resource-current-branch-route` | Published / validated | Refreshed [Client UI systems atlas](Client-UI-Systems-Atlas), [UI resource parity cleanup](UI-Resource-Parity-Cleanup) and [UI IDD collision repair](UI-IDD-Collision-Repair) for current branch truth: docs checkout `2fef1e3d`, Miksuu `b8389e74` and perf `0076040f` still keep stale `RscMenu_Upgrade`, Economy stale writes and EASA/Economy duplicate `23000` in both maintained roots; stable `cf2a6d6a` and release `a96fdda2` remove/repair those resource paths in both maintained roots; title `10200` duplication and `currentCutDisplay` sharing remain open everywhere checked. No gameplay source changed. |
