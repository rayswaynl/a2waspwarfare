...
| Lane | Status | Output |
| --- | --- | --- |
| `hosted-fps-current-branch-route` | Published / validated | Refreshed [Hosted server FPS loop sleep](Hosted-Server-FPS-Loop-Sleep) for docs checkout `db7667c9`, stable `cf2a6d6a`, release `a96fdda2`, Miksuu `b8389e74` and perf `0076040f`: docs/source keeps two guarded FPS publishers, stable/release now use one guarded `serverFpsGUI` publisher with the redundant monitor removed, and Miksuu/perf remain old-loop-shaped. Aligned [Feature status](Feature-Status-Register), [Pending owner decisions](Pending-Owner-Decisions) and machine readiness rows. No gameplay source changed. |
