...
| `support-specials-source-scope-route` | Published / validated | Refreshed [Support specials and tactical modules](Support-Specials-And-Tactical-Modules-Atlas) as the support/tactical gateway for docs checkout `ff8dd884`: added a current source snapshot for Tactical UI, PVF transport, server special dispatch, artillery, UAV/nuke/Zeta/RU para-ammo anchors; replaced repeated dispatch/artillery prose with owner-page routes; and refreshed the `upgrade-sync` matrix/status rows against stable `cf2a6d6a`, Miksuu `b8389e74`, perf `0076040f`, release `a96fdda2` and upgrade-queue `b061c905`. No gameplay source changed. |
