...
| `networking-pv-gateway-source-scope-route-prune` | Published / validated | Refreshed [Networking and public variables](Networking-And-Public-Variables) so current docs head `1e16527b` is the visible network/PVF source-continuity checkpoint. Targeted diffs from `4277a2ad` to `HEAD` over checked Chernarus and maintained Vanilla network, PVF and direct-channel paths return no source changes; branch refs still resolve to stable `cf2a6d6a`, release `a96fdda2`, Miksuu `b8389e74` and perf `0076040f`. Routed repeated dispatch-time `Call Compile` proof and old recheck wording back to Current Branch Scope plus [Public variable channel index](Public-Variable-Channel-Index), [PVF dispatch implementation](PVF-Dispatch-Implementation-Playbook) and [Server authority migration map](Server-Authority-Migration-Map). No gameplay source changed. |
| `marker-cleanup-atlas-source-scope-route-prune` | Published / validated | Refreshed [Marker cleanup/restoration](Marker-Cleanup-Restoration-Systems-Atlas) so current docs head `7eea6b6c` is the visible marker/cleanup source-continuity checkpoint. Targeted diffs from `634a907b` to `HEAD` over checked Chernarus and maintained Vanilla marker, cleaner, restorer and empty-vehicle paths return no source changes; branch refs still resolve to stable `cf2a6d6a`, release `a96fdda2`, Miksuu `b8389e74` and perf `0076040f`. Routed repeated empty supply-truck 24-hour timeout proof back to the atlas matrix and aligned [Feature status](Feature-Status-Register) plus [Source fix propagation queue](Source-Fix-Propagation-Queue). No gameplay source changed. |
| `gear-loadout-easa-atlas-source-scope-route-prune` | Published / validated | Refreshed [Gear, loadout and EASA atlas](Gear-Loadout-And-EASA-Atlas) so current docs head `3fdf1898` is the visible gear/EASA source-continuity checkpoint. Targeted diffs from `8b71e2a1` to `HEAD` over checked Chernarus, maintained Vanilla and `Tools/LoadoutManager` gear/EASA paths return no source changes; branch refs still resolve to stable `cf2a6d6a`, release `a96fdda2`, Miksuu `b8389e74`, perf `0076040f` and BuyMenu/EASA QoL `a66d4691`. Routed repeated profile-template, cargo-loop, service/EASA and UI IDD branch proof back to the atlas Current Branch Scope and owner pages. No gameplay source changed. |
| `construction-coin-atlas-source-scope-route-prune` | Published / validated | Refreshed [Construction and CoIn systems](Construction-And-CoIn-Systems-Atlas) so current docs head `7d248610` is the visible construction/CoIn source-continuity checkpoint. Targeted diffs from `4bd37b98` and `1aa178f8` to `HEAD` over checked Chernarus and maintained Vanilla construction/CoIn, base-area, structure-config, commander ARTY and salvage paths return no source changes; branch refs still resolve to stable `cf2a6d6a`, release `a96fdda2`, Miksuu `b8389e74` and perf `0076040f`. Routed repeated auto-wall, SmallSite, StationaryDefense and salvage branch proof back to the atlas Current Branch Scope, [Construction logic list cleanup](Construction-Logic-List-Cleanup), [Source fix propagation queue](Source-Fix-Propagation-Queue) and the local salvage matrix. No gameplay source changed. |
