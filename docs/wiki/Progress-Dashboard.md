...
| Lane | Status | Output |
| --- | --- | --- |
| `wiki-mirror-current-wiki-sync-2026-06-21` | Published / validated | Synced the repo `docs/wiki` mirror on `origin/docs/developer-wiki-index@11f535d9` to the live GitHub wiki checkout `master@743daf4` after a normalized top-level comparison found `116` differences. The batch copied `260` wiki files into `docs/wiki`, removed the mirror-only `GUER-Insurgents-Branch-Audit.md`, preserved wiki coordination updates, and left gameplay source untouched. |
| `qol-trio-player-hints-reference` | Published / validated | Added [QoL trio player hints](QoL-Trio-Player-Hints-Reference) for stable `master@0139a346`: the page documents the hardcoded `WFBE_C_QOL_TRIO` / `WFBE_C_QOL_ADVISOR_INTERVAL` gates, salvage payout toast, factory-upgrade unlock banner, player-initiated upgrade sound split, last-purchase timestamp coupling and the unspent-funds / commander Patrols advisor nudges. Routed through [Home](Home), [`_Sidebar.md`](_Sidebar), [Player UI workflow](Player-UI-Workflow-Map), [Experimental feature-flag constants](Experimental-Feature-Flag-Constants-Reference) and `agent-context.json`. No gameplay source changed. |
| `engine-stealth-fuel-toggle-reference` | Published / validated | Added [Engine stealth fuel toggle](Engine-Stealth-Fuel-Toggle-Reference) for stable `master@0139a346`: the page documents the live `Client/Module/Engines` stop/fuel-drain/start flow, purchased `Tank` / `Wheeled_APC` action attach, WASP extra start-vehicle attach, vehicle state keys `ID` / `Fuel` / `stopped`, and the refuel-service guard that exits while stealth is marked stopped. Rebased follow-up kept this canonical page, dropped a duplicate local slug, and added the `"stopped"` variable-name caveat plus Smoke Targets table. Routed through [Home](Home), [`_Sidebar.md`](_Sidebar), [Modules atlas](Modules-Atlas), [Function and module index](Function-And-Module-Index) and `agent-context.json`. No gameplay source changed. |
| `player-vehicle-travel-actions-reference` | Published / validated | Added [Player vehicle and travel actions](Player-Vehicle-And-Travel-Actions-Reference) for stable `master@0139a346`: the page catalogs live player-facing vehicle/travel scroll actions from `Common/Init/Init_Unit.sqf`, including repair-route links, Low Gear, manual flip, boat Push, transport HALO, cargo eject, plane Taxi Reverse and lock/unlock paths. It preserves repair authority detail on existing owner pages, routes the current lock-path distinction, and is wired through [Home](Home), [`_Sidebar.md`](_Sidebar), [AutoFlip vehicle recovery](AutoFlip-Vehicle-Recovery-Reference), [Valhalla vehicle climbing-assist](Valhalla-Vehicle-Climbing-Assist), [Player UI workflow](Player-UI-Workflow-Map) and `agent-context.json`. No gameplay source changed. |
| `skin-selector-class-swap-reference` | Published / validated | Added [Skin selector and class swap](Skin-Selector-And-Class-Swap-Reference) for stable `master@0139a346`: the page documents the default-off gate, hidden WF-menu shortcut, join/WF-menu/User11 openers, WEST/EAST class pools, Spotter-only ghillie filter, replacement-player apply lifecycle, action/handler restoration and respawn persistence. Wired through [Home](Home), [`_Sidebar.md`](_Sidebar), [Player UI workflow](Player-UI-Workflow-Map), [Player skill abilities](Player-Skill-Abilities-Reference), [WASP overlay](WASP-Overlay), [Feature status](Feature-Status-Register) and `agent-context.json`. No gameplay source changed. |
