...
| `source-fix-queue-branch-scope-refresh` | Published / validated | Refreshed [Source fix propagation queue](Source-Fix-Propagation-Queue) as a branch-scope gateway: updated LoadoutManager root discovery to docs checkout `56d2f856`, stable `origin/master` `cf2a6d6a`, Miksuu `b8389e74`, `perf/quick-wins` `0076040f` and release `a96fdda2`; added a current branch-scope table for propagated fixes; and separated docs-checkout commander-team ARTY handoff from stable/release marker-based ARTY discovery. No gameplay source changed. |
