...
| Lane | Status | Output |
| --- | --- | --- |
| `visible-parameter-current-b741-b742-refresh-2026-06-23` | Published / validated | Refreshed `WFBE_C_AI_MAX` and `WFBE_C_UNITS_CLEAN_TIMEOUT` branch-scope evidence through [Mission parameters](Mission-Parameters-Localization-And-Generated-Build-Inputs#parameter-cache-flow), [Feature status](Feature-Status-Register), [Source Fix queue](Source-Fix-Propagation-Queue), [Dead/stale code register](Dead-Code-And-Stale-Code-Register) and machine rows. Docs/source `HEAD@0da6ba431`, current Miksuu `b8389e748243`, perf `0076040f` and historical `a96fdda2` keep the old readless/comment-only shape. Current stable/B74.1 `origin/master@f8a76de34` equals `origin/claude/b74.1-aicom@f8a76de34`; B69 `8d465fce`, B74 `b23f557f` and B74.2 `d472da6a` read `WFBE_C_AI_MAX` at `AI_Commander_Produce.sqf:234` and `WFBE_C_UNITS_CLEAN_TIMEOUT` at `Common_TrashObject.sqf:21` in both maintained roots. Player follower caps still use `WFBE_C_PLAYERS_AI_MAX`; no gameplay source changed. |
| `upgrade-sync-current-b741-b742-refresh-2026-06-23` | Published / validated | Refreshed `RequestSpecial` `upgrade-sync` tuple-parser evidence through [Support specials](Support-Specials-And-Tactical-Modules-Atlas#upgrade-sync-branch-matrix), [Upgrades and research](Upgrades-And-Research-Atlas#current-branch-scope), [Feature status](Feature-Status-Register) and [Source Fix queue](Source-Fix-Propagation-Queue), plus machine rows. Docs/source `HEAD@a7f383b6` keeps the mixed parser at `Server_HandleSpecial.sqf:3,67-73`, caller `GUI_UpgradeMenu.sqf:171` and sync lifecycle `Server_ProcessUpgrade.sqf:26,29,35`. Current stable/B74.1 `origin/master@f8a76de34` equals `origin/claude/b74.1-aicom@f8a76de34`; B74.2 `origin/claude/b74.2-aicom@d472da6a` has no checked path delta. Both keep the mixed parser while adding server end-time state at `Server_ProcessUpgrade.sqf:27,55`; no gameplay source changed. |
| `b742-aicom-branch-intel-2026-06-23` | Published / validated | Routed `origin/claude/b74.2-aicom@d472da6ae` as source Chernarus-only branch evidence over current stable/B74.1 `origin/master@f8a76de34`; no PR route was found and no `Missions_Vanilla` payload exists. Evidence is now on [AI commander autonomy](AI-Commander-Autonomy-Audit#b69-roadmap-and-sketch-route), [AI capture plan](AI-Commander-Capture-And-Fun-Improvement-Plan), [Client UI systems](Client-UI-Systems-Atlas#current-branch-scope), [Towns/camps](Towns-Camps-And-Capture-Atlas#current-branch-scope), [Player stats](Player-Stats-Branch-Audit#current-status), [Feature status](Feature-Status-Register) and [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack), plus release-readiness and machine rows. The branch changes 26 source Chernarus files / +324 / -24 and `git diff --check` is clean; it adds pop-tier AICOM caps, marker-feed replay, camp anti-stall/range tuning, tiered town pressure, carrier air-shop/respawn hooks, source Chernarus stats fast-follow writers, AA squad trims and tint default changes. Promotion remains smoke- and scope-gated; no gameplay source changed. |
| `ai-supply-truck-current-b741-refresh-2026-06-23` | Published / validated | Refreshed autonomous AI supply-truck dead/stale-code evidence through [AI commander autonomy audit](AI-Commander-Autonomy-Audit#ai-supply-truck-branch-matrix), [Abandoned feature revival review](Abandoned-Feature-Revival-Review#ai-commander-supply-trucks), [Feature status](Feature-Status-Register), [Source Fix queue](Source-Fix-Propagation-Queue) and [Dead/stale code register](Dead-Code-And-Stale-Code-Register), plus machine rows. Current stable/B74.1 `origin/master@f8a76de34` equals `origin/claude/b74.1-aicom@f8a76de34` and safe-disables the truck-supply + AI-commander branch at Chernarus `Init_Server.sqf:44,624-625` and maintained Vanilla `:44,618-619`; B74.2 `origin/claude/b74.2-aicom@d472da6a` keeps the same caller shape. Docs/source `HEAD@6d4b514c12fc`, Miksuu `b8389e748243` and perf `0076040f` still raw-spawn the commented worker, and no checked ref restores `Server\FSM\supplytruck.fsm`. No gameplay source changed. |
| `attack-wave-authority-current-b741-refresh-2026-06-23` | Published / validated | Refreshed DR-41 attack-wave direct-PV authority evidence through [Attack-wave authority](Attack-Wave-Authority-Playbook#branch--root-matrix), [Feature status](Feature-Status-Register), [Source Fix queue](Source-Fix-Propagation-Queue), [Public variable channel index](Public-Variable-Channel-Index) and [Server authority migration](Server-Authority-Migration-Map), plus machine rows. Current stable/B74.1 `origin/master@f8a76de34` equals `origin/claude/b74.1-aicom@f8a76de34`; scoped current-stable/B74/B74.1 diffs only touch constants, while `ATTACK_WAVE_INIT` / `ATTACK_WAVE_DETAILS` request/detail trust remains unchanged in source Chernarus and maintained Vanilla. Repo-root `BattlEyeFilter/publicvariable.txt` still only filters `kickAFK`. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | `documentation-finisher-loop` | Latest published lane: `upgrade-sync-current-b741-b742-refresh-2026-06-23`. Continue bounded source-backed docs passes from this dashboard, Feature Status, Source Fix queue, Upstream mining ledger and pruning ledger. |
