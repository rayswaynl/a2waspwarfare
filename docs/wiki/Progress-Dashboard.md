...
| `commander-reassignment-current-b74-refresh-2026-06-23` | Published / validated | Refreshed [Commander reassignment call shape](Commander-Reassignment-Call-Shape#current-branch-matrix), [Commander vote/reassignment](Commander-Vote-And-Reassignment-Playbook), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine rows with adjacent B74 evidence. Docs/source `HEAD@60c35c05` is unchanged from `337ed166` / `b44aaaf8` for checked reassignment paths and still carries `_side = _this` in both maintained roots. Current stable `origin/master@0139a346`, B69 `origin/claude/b69@8d465fce`, B74 `origin/claude/b74-aicom-spend@b23f557f`, Miksuu `b8389e748243`, perf `0076040f`, historical `a96fdda2` and historical `c20ce153` all fix maintained-root helper unpacking at `Server_AssignNewCommander.sqf:4-5`, but all checked maintained roots still keep duplicate `new-commander-assigned` senders and visible-name UI selection. B69..B74 and older-B69-to-B74 reassignment-path deltas are empty; modded full forks remain old-shape. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | `documentation-finisher-loop` | Latest published lane: `upgrade-sync-current-b74-refresh-2026-06-23`. Continue bounded source-backed docs passes from this dashboard, the refreshed [PR cleanup lab](PR-Cleanup-And-Integration-Lab), Feature Status, Source Fix queue and pruning ledger. |
