...
| `vote-help-ui-current-b741-b742-refresh-2026-06-23` | Published / validated | Refreshed vote/help/main-menu UI cleanup evidence through [Client UI systems](Client-UI-Systems-Atlas#vote-help-and-main-menu-branch-matrix), [Feature status](Feature-Status-Register), [Source Fix queue](Source-Fix-Propagation-Queue), [Dead/stale code register](Dead-Code-And-Stale-Code-Register) and machine rows. Docs/source `HEAD@322ef95e` is unchanged from `087152f5` / `68bd4dc5` for checked vote/help/router paths. Current stable/B74.1 `origin/master@f8a76de34` equals `origin/claude/b74.1-aicom@f8a76de34`; B74.2 `origin/claude/b74.2-aicom@d472da6a` has no checked vote/help/router delta. Current stable/B74.1/B74.2/B69/B74 still keep inclusive vote loops, `GUI_VoteMenu.sqf` row-color `[_i+1,0]`, help namespace mismatch and handler-only `MenuAction` `17/18` GPS zoom routes. Visible `MenuAction = 19` is GPS enablement on B74-shaped refs, not a rescue for old zoom handlers. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | `documentation-finisher-loop` | Latest published lane: `factory-refund-current-b741-b742-refresh-2026-06-23`. Continue bounded source-backed docs passes from this dashboard, Feature Status, Source Fix queue, Upstream mining ledger and pruning ledger. |
