...
| `mash-marker-relay-branch-scope` | Published / validated | Source-checked MASH marker relay: maintained source/Vanilla, stable, Miksuu upstream and release all keep the orphaned server relay plus commented receiver while maintained deploy paths do not send `WFBE_CL_MASH_MARKER_CREATED`. Updated [Respawn lifecycle](Respawn-And-Death-Lifecycle-Atlas), [Support specials](Support-Specials-And-Tactical-Modules-Atlas), [PV channel index](Public-Variable-Channel-Index), [Feature status](Feature-Status-Register), [Dead/stale code register](Dead-Code-And-Stale-Code-Register) and machine backlog records. No gameplay source changed. |
