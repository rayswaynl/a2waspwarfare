...
| `b745-b7471-veteran-cap-refresh-2026-06-24` | Published / validated | Refreshed `origin/claude/b745` after the branch advanced again to `a9145e4da`. [Feature status](Feature-Status-Register), [Current source status](Current-Source-Status-Snapshot#2026-06-24-b745b748-master-merge-intake), [AI commander autonomy](AI-Commander-Autonomy-Audit#b745--b748-aicom-follow-up), [Performance opportunity sweep](Performance-Opportunity-Sweep), [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack) and [Pruning ledger](Wiki-Pruning-And-Relevance-Ledger) now route the B74.7.1 veteran-slot hard-cap follow-up: `29ca9519..a9145e4da` is one source-Chernarus file / +1 / -1, no `Missions_Vanilla`, `Modded_Missions` or `Tools` payload, and it caps `AI_Commander_Teams.sqf:184` with `min _teamsHardCap` so the veteran `+1` path cannot exceed the eight-team cap. Full branch payload is now 133 files / +4131 / -4399 over `origin/master@f8a76de34`. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | None | B750 factory/purchase owner-page route is published/validated; pick the next bounded source-backed docs lane after fresh coordination reads. |
