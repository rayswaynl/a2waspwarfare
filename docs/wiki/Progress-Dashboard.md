...
| `sqf-network-branch-caveat-route` | Published / validated | Refreshed [SQF code atlas](SQF-Code-Atlas) and [Networking/public variables](Networking-And-Public-Variables) gateway caveats for current branch truth: docs checkout `8701aacc` has no maintained mission-root or LoadoutManager source changes since the prior atlas anchors, `UpdateSupplyTruck` now routes to the AI commander branch matrix instead of an unresolved note, and supply-heli is documented as stable/release-present but docs/Miksuu/perf truck-only. No gameplay source changed. |
| `client-skill-init-current-branch-route` | Published / validated | Refreshed [Client skill init idempotency](Client-Skill-Init-Idempotency) and the [Feature status](Feature-Status-Register) row for current branch truth: docs checkout `719455a2`, stable `cf2a6d6a` and release `a96fdda2` carry the single `Skill_Init.sqf` plus later `WFBE_SK_FNC_Apply` shape in both maintained roots, while Miksuu `b8389e74`, perf `0076040f` and `feat/ai-commander` `c20ce153` still duplicate `Skill_Init`. Updated machine context rows and kept Arma Soldier/respawn smoke pending. No gameplay source changed. |
| `sqf-pvf-registry-route` | Published / validated | Pruned stale registered-PVF command copies from [SQF code atlas](SQF-Code-Atlas), made [Public variable channel index](Public-Variable-Channel-Index#current-branch-registry-matrix) the branch-aware registry owner, and aligned [Networking/public variables](Networking-And-Public-Variables): docs checkout `40c97e74` has 13 server / 15 client PVFs in both maintained roots, stable `cf2a6d6a` and release `a96fdda2` add `RequestEnqueue` / `RequestDequeue`, Miksuu `b8389e74` and `feat/ai-commander` `c20ce153` omit paratrooper registration, and perf `0076040f` is Chernarus-only for paratrooper registration. No gameplay source changed. |
