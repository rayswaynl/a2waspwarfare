...
| `parameters-display-22005-comment-scope` | Published / validated | Source-checked the Parameters display IDC `22005` residue: current source, maintained Vanilla, modded copies, stable/upstream and release all keep the old uptime write inside a block comment, while the live dialog writes rows through `22003` and has no `22005` resource control. Updated [Client UI systems](Client-UI-Systems-Atlas), [Dead/stale code register](Dead-Code-And-Stale-Code-Register) and machine records so this is a safe comment-cleanup lane, not a live UI defect. No gameplay source changed. |
