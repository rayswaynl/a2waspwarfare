...
| Lane | Status | Output |
| --- | --- | --- |
| `gear-template-profile-branch-route` | Published / validated | Refreshed [Gear template profile filter](Gear-Template-Profile-Filter) with a branch/root matrix: current source Chernarus, maintained Vanilla, stable `origin/master`, Miksuu upstream and the feature-bundle release all still carry the undefined `_u_upgrade` save filter plus the six-field profile import guard. Condensed [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and [Gear/loadout/EASA atlas](Gear-Loadout-And-EASA-Atlas). No gameplay source changed. |
