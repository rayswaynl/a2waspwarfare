...
| Lane | Status | Output |
| --- | --- | --- |
| `pr63-repair-antispam-post-b751b-route-2026-06-24` | Published / validated | Routed open draft PR #63 / `origin/claude/trello-repair-antispam@9ba13a66` as branch-only stale-base repair anti-spam evidence after current master advanced to `origin/master@4a163bb65`. [WASP base repair](WASP-Base-Repair-System-Reference#2026-06-24-pr-63-repair-anti-spam-branch-addendum), [Service menu guards](Service-Menu-Affordability-Guards#current-branch-matrix), [PR cleanup](PR-Cleanup-And-Integration-Lab#2026-06-24-pr-63-repair-anti-spam-intake), [Feature status](Feature-Status-Register), [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack) and [Pruning ledger](Wiki-Pruning-And-Relevance-Ledger) now say PR #63 still bases at `f8a76de34`, GitHub mergeability is currently `null` / `unknown`, and local divergence is `95 1`. Base-relative payload is eight maintained-root files / +82 / -12, clean `git diff --check`, no Modded/Tools/Extension payload. Branch anchors add `WASP_BaseRepair_Running`, `wfbe_veh_repairing`, repair price `0` while already repairing and `STR_WF_INFO_Repair_Veh_InProgress`; current master lacks those symbols. Rebase/recheck plus source Chernarus and maintained Vanilla base/service repair smoke are required before stable wording. No gameplay source changed. |
| `pr66-debug-antitk-post-b751b-route-2026-06-24` | Published / validated | Routed open draft PR #66 / `origin/claude/trello-debug-antitk@98440121` as branch-only clean-but-stale-base debug anti-TK evidence after B751b. [Structure damage/Friendly Fire](Structure-Damage-Reduction-And-Friendly-Fire#2026-06-24-pr-66-debug-anti-tk-branch-addendum), [PR cleanup](PR-Cleanup-And-Integration-Lab#2026-06-24-pr-66-debug-anti-tk-intake), [Feature status](Feature-Status-Register), [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack) and [Pruning ledger](Wiki-Pruning-And-Relevance-Ledger) now say PR #66 is GitHub-clean but still based at `f8a76de34` while current master is `origin/master@f39665d79`; local divergence is `80 1`. Base-relative payload is four maintained-root files / +10 / -0, clean `git diff --check`, no Modded/Tools/Extension payload. Branch anchors add `WF_Debug` early exits to `WFBE_CL_FNC_OnFiredSatchel` and `Server_BuildingHandleDamages`; current master lacks those exits and `WF_Debug` defaults false. Rebase/recheck plus source Chernarus and maintained Vanilla debug/non-debug smoke are required before stable wording. No gameplay source changed. |
| `pr72-antibasetrap-post-b751b-route-2026-06-24` | Published / validated | Routed open draft PR #72 / `origin/claude/trello-antibasetrap@b79c5cee` as branch-only clean-but-stale-base anti-base-trap static-defense gate evidence after B751b. [PR cleanup](PR-Cleanup-And-Integration-Lab#2026-06-24-pr-72-anti-base-trap-gate-intake), [Feature status](Feature-Status-Register), [Construction and CoIn](Construction-And-CoIn-Systems-Atlas#server-request-handlers), [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack) and [Pruning ledger](Wiki-Pruning-And-Relevance-Ledger) now say PR #72 is GitHub-clean but still based at `f8a76de34` while current master is `origin/master@f39665d79`; local divergence is `80 1`. Base-relative payload is eight maintained-root files / +296 / -2, clean `git diff --check`, no Modded/Tools/Extension payload. Branch anchors add `WFBE_C_BASE_DEFENSE_BUILD_BLOCK_*`, `BaseDefenseTrapGate` and an `_atReject` `RequestDefense` gate; current master keeps only the older budget/threat gate. Rebase/recheck plus source Chernarus and maintained Vanilla smoke are required before stable wording. No gameplay source changed. |
| `pr74-artillery-cooldown-hud-post-b751b-route-2026-06-24` | Published / validated | Routed open draft PR #74 / `origin/claude/trello-arty-cooldown-hud@471fc0580fd6` as branch-only clean-but-stale-base artillery RHUD cooldown HUD evidence after B751b. [Support specials](Support-Specials-And-Tactical-Modules-Atlas#trello-artillery-ui-branch-intel), [Client UI systems](Client-UI-Systems-Atlas#current-branch-scope), [Feature status](Feature-Status-Register), [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack), [PR cleanup](PR-Cleanup-And-Integration-Lab#2026-06-24-post-b751b-open-board-refresh) and [Pruning ledger](Wiki-Pruning-And-Relevance-Ledger) now say PR #74 is GitHub-clean but still based at `f8a76de34` while current master is `origin/master@f39665d79`; local divergence is `80 1`. Base-relative payload is four maintained-root UI files / +134 / -8, clean `git diff --check`, no Modded/Tools/Extension payload. Current master keeps Tactical cooldown state but no `RUBHUD_Arty` / `_RHUDUpdateArty`; rebase/recheck plus source Chernarus and maintained Vanilla RHUD smoke are required before stable wording. No gameplay source changed. |
| `source-fix-current-branch-scope-post-b751b-wording-2026-06-24` | Published / validated | Cleaned stale current-stable wording in [Source fix propagation queue](Source-Fix-Propagation-Queue#current-branch-scope-2026-06-24) and the July hosted-FPS row. Current master is `origin/master@f39665d7950ddf14cb0bbfacb7ee1e40121e93b4`; current-master queue anchors still show one guarded `SERVER_FPS_GUI` publisher, no maintained-root `monitorServerFPS.sqf`, paratrooper registration, heli-aware typed supply scan, marker-based commander ARTY and the player-list index caveat. Exact `origin/master@f8a76de34` current-stable phrases in Source Fix now read as pre-B745/B751b B74.1/B74.2 baseline evidence. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | None | PR #63 repair anti-spam route is published; pick the next bounded source-backed docs lane from open PRs #59/#54/#53/#9, Feature Status reconciliation or another current-head refresh. |
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | Hosted Server FPS Loop Fix | planned `dev/july-update-hosted-server-fps-loop-fix` | Adopt/verify the DR-19 hosted/listen busy-spin fix for the July update target. Choose the target branch shape deliberately: current master `origin/master@f39665d79` and the pre-B745/B751b B74.1 snapshot `origin/master@f8a76de34` both use one guarded `SERVER_FPS_GUI` publisher, while docs/source keeps two guarded compatibility publishers. | ~5-20 LOC for a two-publisher loop guard on old-shape refs; ~20-50 LOC if cleanup/consolidation is included. | Queued for July update, but no current remote `origin/dev/july-update-hosted-server-fps-loop-fix` head was found on 2026-06-22. Create or restore the branch before code work. | Target-branch-specific: current-master/stable-shaped branches keep dedicated `SERVER_FPS_GUI` publishing and no monitor loop; docs/source-shaped branches keep `SERVER_FPS_GUI` plus `WFBE_VAR_SERVER_FPS`. In all cases, hosted/listen runs do not spin FPS publisher loops and Chernarus/maintained Vanilla parity is named before release wording. |
...
- AICOM/PR board route: [PR cleanup and integration lab](PR-Cleanup-And-Integration-Lab#2026-06-24-post-b751b-open-board-refresh) was refreshed after PR #89/B751b advanced current master to `origin/master@f39665d79`. Current open PRs remain #84, #83, #80, #74, #72, #66, #63, #59, #54, #53 and #9. PR #83 and PR #9 are now `dirty` / not mergeable; PR #84 is a player-stats stack on `feat/player-stats`, not a `master`-target PR; PR #85-#89 are merged current-master history. PR #43 and PR #40 are closed as of 2026-06-22; older rows calling them open are historical. PR #35 and deploy-child PR #34/#36-#39/#41 remain closed history; PR #29 and PR #31 merged on 2026-06-17.
