...
| Lane | Status | Output |
| --- | --- | --- |
| `pruning-ledger-lead-state-closeout` | Published / validated | Converted [Wiki pruning ledger](Wiki-Pruning-And-Relevance-Ledger) "Fresh Backlog Leads" from stale open-looking suggestions into an open/closed state table. It now records mirror parity, machine-file navigation, Home/onboarding pruning, navigation inventory, archive demotion and onboarding overlap as closed for this pruning pass. [Navigation inventory](Navigation-Inventory-And-Page-Status) now shows `Continue Reading` gap families as `None` instead of listing pages that were already fixed. No gameplay source changed. |
| `old-fps-scout-addendum-harvest` | Published / validated | Harvested the returned `old-warfarebe-fps-scout` addendum into owner pages without adding another report page: [Performance sweep](Performance-Opportunity-Sweep) now exposes the easy-win bundle, [AI/headless](AI-Headless-And-Performance) names the server-FPS consumer/consolidation caveat and delegated-town-AI optimization, [Old WarfareBE comparison](Old-WarfareBE-Performance-Comparison) records the current-vs-old nuance, and [Feature status](Feature-Status-Register) tracks the harvested findings as test/config work rather than gameplay patches. No gameplay source changed. |
...
| `mirror-and-source-consistency-closeout` | Published / validated | Closed old mirror/SQF audit residue: [Wiki mirror reconciliation plan](Wiki-Mirror-Reconciliation-Plan) now records full repo-mirror/wiki checkout parity (`full-diffCount=0`), and [Wiki/source consistency findings](Wiki-Source-Consistency-Findings) marks stale SQF compile-count warnings closed by the current dated [SQF code atlas](SQF-Code-Atlas) recount. Validation passed; no gameplay source changed. |
| `wiki-quality-round2-closeout` | Published / validated | Closed stale Round 2 Codex-lane items in [Wiki quality audit](Wiki-Quality-Audit): SQF atlas compile counts/DR-34 MASH wording, DR-44/DR-20/DR-40/DR-19/DR-45 cross-links, Coordination Board active-lane state, Progress Dashboard Claude row and sidebar HC duplicate are now all marked resolved from current docs evidence. Validation passed; no gameplay source changed. |
| `tooling-release-audit-status-closeout` | Published / validated | Closed the stale [Tooling release readiness audit](Tooling-Release-Readiness-Audit) warning that `agent-status.json` still said propagated fixes had Vanilla propagation pending. Current status routes those lanes as source-propagated/smoke-pending, so the audit now points future agents at smoke and release-branch scope instead of a dead status wording task. Validation passed; no gameplay source changed. |
| `pr-board-state-refresh` | Published / validated | Refreshed [PR cleanup lab](PR-Cleanup-And-Integration-Lab) from live GitHub PR metadata: #2, #3, #11, #12, #15, #16 and #19 are now closed; #4, #8, #9, #13, #14, #17 and #18 remain open. The page now warns that synthetic `origin/pr/*` refs were pruned and future agents should use PR URLs/head branches plus the dev lab branches. Validation passed; no gameplay source changed. |
| `agent-machine-index-bootstrap` | Published / validated | Added [`agent-machine-index.json`](agent-machine-index.json) as a compact page-to-source/risk lookup for agents, linked it from the LLM entry pack, `llms.txt`, Agent Context, sidebar and machine entrypoint, and documented JSONL vNext envelope conventions while tolerating legacy rows. Navigation inventory marks it as the only intentional sidebar-only machine-file link. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Active | `documentation-finisher-loop` | Closed stale pruning-ledger lead wording so completed navigation/onboarding work no longer reads as open backlog; continue source-backed wiki pruning/status upkeep. |
