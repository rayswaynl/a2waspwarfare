...
| `miksuu-current-head-d950-correction-2026-06-22` | Published / validated | Corrected current-facing Miksuu provenance after direct `git ls-remote https://github.com/miksuu/a2waspwarfare.git refs/heads/master` showed upstream `master@b8389e748243`, while local `d9506078` belongs to `origin/claude/*` branches. Updated cargo-loop, auto-wall, visible-parameter and LoadoutManager root-discovery rows so current Miksuu is treated as old-shape: inclusive cargo loops, auto-wall false default/no exclusions, no live `WFBE_C_AI_MAX` reader, comment-only `WFBE_C_UNITS_CLEAN_TIMEOUT`, and ancestor-name-only LoadoutManager root discovery. Current stable `origin/master@0139a346` remains separately source-present where documented. No gameplay source changed. |
| `mash-marker-current-stable-branch-scope-refresh-2026-06-22` | Published / validated | Refreshed the MASH branch split in [Respawn and death lifecycle](Respawn-And-Death-Lifecycle-Atlas#mash-split-live-respawn-dead-marker-relay), [Feature status](Feature-Status-Register), [Abandoned feature revival](Abandoned-Feature-Revival-Review), [Dead/stale code](Dead-Code-And-Stale-Code-Register), [Public variable channel index](Public-Variable-Channel-Index), [Support specials](Support-Specials-And-Tactical-Modules-Atlas) and adjacent module/UI/hardening routes. Docs head `db3015f18ea3`, Miksuu `b8389e748243` and perf `0076040f8a5e` keep local MASH deploy plus orphaned marker relay in both maintained roots. Current stable `origin/master@0139a3468609`, historical `a96fdda28087`, `origin/claude/b69@0a1ccb4d05c5` and `origin/claude/b69-finalpieces@80d3267c1b2b` remove the maintained-root deploy/module path and keep only `Skill_Apply.sqf:43` removal wording plus residues. No gameplay source changed. |
| `salvage-payout-current-head-refresh-2026-06-22` | Published / validated | Refreshed [Construction and CoIn salvage](Construction-And-CoIn-Systems-Atlas#salvage-branch-matrix), [Feature status](Feature-Status-Register) and [Source fix queue](Source-Fix-Propagation-Queue) after salvage rows still carried 2026-06-21 current-head wording. Docs branch `fb8d4ebc`, current stable `origin/master@0139a346`, current Miksuu `master@b8389e748243`, perf `0076040f` and historical `a96fdda2` all keep lowercase `ChangePlayerfunds` payout calls, `updatesalvage.sqf:10` OR-loop and client-local delete/reward shape in both maintained roots. Current origin exposes no `release/*` or `feat/*salvage*` heads; Miksuu historical salvage heads remain inherited debt. No gameplay source changed. |
