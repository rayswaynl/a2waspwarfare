...
| Lane | Status | Output |
| --- | --- | --- |
| `side-supply-reason-string-branch-route` | Published / validated | Added [Economy authority](Economy-Authority-First-Cut#side-supply-reason-string-branch-matrix) branch scope for the `Common_ChangeSideSupply.sqf` reason parsing off-by-one: current source Chernarus, maintained Vanilla, stable `origin/master`, Miksuu upstream, `perf/quick-wins` and release `fb3084c2` all keep the `count _this > 3` guard that drops 3-argument caller reasons such as `AttackWave.sqf:40`, while 4-argument supply mission completion preserves its reason. Condensed [Economy, towns and supply](Economy-Towns-And-Supply), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine records. No gameplay source changed. |
| `resource-income-payout-branch-route` | Published / validated | Refreshed [Economy, towns and supply](Economy-Towns-And-Supply#resource-income-branch-matrix) with a branch/root matrix: current source Chernarus, maintained Vanilla, stable `origin/master`, Miksuu upstream, `perf/quick-wins` and release `fb3084c2` all keep the resource updater's shared supply/paycheck/AI-commander guard and the income-system `4` server/client display mismatch. Condensed [Economy authority](Economy-Authority-First-Cut), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine records. No gameplay source changed. |
| `release-7195b331-status-refresh` | Published / validated | Refreshed release-head evidence after `origin/release/2026-06-feature-bundle` moved through `fb3084c2` to `7195b331`: current release Chernarus and maintained Vanilla now carry paratrooper marker registration, single `Skill_Init`, hosted-FPS guard/removal, narrowed supply command-center scan, camp-capture flag texture fix and resistance-patrol `&&`; `7195b331` additionally replaces the old FPS-only menu slot with a GPS toggle. Commander-built ARTY ownership still remains absent from release and stable master. Updated [Current source snapshot](Current-Source-Status-Snapshot), [PR cleanup lab](PR-Cleanup-And-Integration-Lab), [Feature status](Feature-Status-Register), [Source fix queue](Source-Fix-Propagation-Queue) and machine records. No gameplay source changed. |
| `requestspecial-upgrade-sync-branch-route` | Published / validated | Refreshed [Support specials](Support-Specials-And-Tactical-Modules-Atlas#upgrade-sync-branch-matrix) with a branch/root matrix: current source Chernarus, maintained Vanilla, stable `origin/master`, Miksuu upstream, `perf/quick-wins` and release `fb3084c2` all keep the same mixed `_args` / `_this` parser for `upgrade-sync`; because `_args = _this`, this is consistency cleanup debt rather than a confirmed runtime break. Condensed [Upgrades](Upgrades-And-Research-Atlas), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine records. No gameplay source changed. |
| `vote-help-ui-edge-branch-route` | Published / validated | Refreshed [Client UI systems](Client-UI-Systems-Atlas#vote-help-and-main-menu-branch-matrix) with a branch/root matrix: current source Chernarus, maintained Vanilla, stable `origin/master`, Miksuu upstream, `perf/quick-wins` and the feature-bundle release all keep the vote inclusive-loop/row-color, help namespace and GPS router-only shapes. Condensed [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine records. No gameplay source changed. |
| `side-supply-clamp-branch-route` | Published / validated | Refreshed [Economy authority first cut](Economy-Authority-First-Cut#side-supply-branch-matrix) with a branch/root check: current source Chernarus, maintained Vanilla, stable `origin/master`, Miksuu upstream and release still keep the overspend-as-credit floor plus payload-side trust, while `perf/quick-wins` fixes only Chernarus arithmetic. Condensed [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine records. No gameplay source changed. |
