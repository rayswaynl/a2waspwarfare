...
| `camp-count-helper-fallback-branch-route` | Published / validated | Refreshed [Towns, camps and capture](Towns-Camps-And-Capture-Atlas) with a branch/root check: current source Chernarus, maintained Vanilla, stable `origin/master`, Miksuu upstream, `perf/quick-wins` and the feature-bundle release all keep zero-camp helper fallbacks returning `1` and feeding capture mode 2, threeway respawn and depot-buy gates. Condensed [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine records. No gameplay source changed. |
| `clickabletext-soundpush-branch-route` | Published / validated | Refreshed [Client UI systems](Client-UI-Systems-Atlas#clickable-text-soundpush-branch-matrix) with a branch/root matrix: current source Chernarus, maintained Vanilla, stable `origin/master`, Miksuu upstream, `perf/quick-wins`, release and UI theme branches all keep malformed `RscClickableText.soundPush[] = {, 0.2, 1};` in the base resource class. Condensed [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine ledgers. No gameplay source changed. |
