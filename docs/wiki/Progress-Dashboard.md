...
| `vote-help-ui-b742-current-head-followup-2026-06-24` | Published / validated | Refreshed [Client UI systems](Client-UI-Systems-Atlas#vote-help-and-main-menu-branch-matrix), [Feature status](Feature-Status-Register) and [Source Fix queue](Source-Fix-Propagation-Queue) from stale B74.2 `origin/claude/b74.2-aicom@d472da6a` wording to current `origin/claude/b74.2-aicom@21b62b04`. Docs/source `HEAD@72007de93f00` is unchanged from `322ef95e` / `087152f5` / `68bd4dc5` for checked vote/help/router paths. Current B74.2 still keeps inclusive vote loops, `GUI_VoteMenu.sqf:80,82` row-color offset, help load/unload namespace mismatch and handler-only `MenuAction` `17/18`; its `d472da6a..21b62b04` delta only touches source-Chernarus `Init_Client.sqf` vehicle-tint/join-ACK/intro-music code among checked paths. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready / documentation finisher loop | `documentation-finisher-loop` | Economy dialog current B74.2 head refresh is published; pick the next bounded source-backed lane after reading the coordination surfaces and avoiding active owners. |
