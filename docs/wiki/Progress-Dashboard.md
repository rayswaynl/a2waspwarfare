...
| `empty-supply-truck-b742-current-head-followup-2026-06-24` | Published / validated | Refreshed empty supply-truck cleanup timeout rows through [Marker cleanup/restoration](Marker-Cleanup-Restoration-Systems-Atlas#empty-supply-truck-branch-matrix), [Feature status](Feature-Status-Register) and [Source Fix queue](Source-Fix-Propagation-Queue). Current B74.2 is `origin/claude/b74.2-aicom@21b62b04`; checked `d472da6a..21b62b04` is empty for `emptyvehiclescollector.sqf` and `Server_HandleEmptyVehicle.sqf` in both maintained roots. Docs/source `HEAD@248baa289ca5` is unchanged from `499c2543` / `b4e10b5f`; current stable/B74.1/B74.2/B69/B74, Miksuu, perf and historical `a96fdda2` still route supply-truck classes through `_delay = 86400` in both maintained roots. No live release/cleanup/supply rescue head was exposed. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready / docs finisher loop | `documentation-finisher-loop` | Town AI vehicle despawn B74.2 current-head follow-up is published and validated; pick the next bounded source-backed docs lane before editing. |
