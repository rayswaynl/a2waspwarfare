...
| `ai-headless-performance-gateway-route` | Published / validated | Refreshed [AI, headless and performance](AI-Headless-And-Performance) as the broad AI/HC/performance gateway for docs checkout `ee383941`: added route and branch-scope tables, corrected `origin/feat/ai-commander` to current head `c20ce153`, routed AI commander/HC/town-AI/performance detail to owner pages, and corrected [AI runtime/HC loop map](AI-Runtime-HC-Loop-Map) so release `a96fdda2` is no longer described as carrying Patrols v2. No gameplay source changed. |
