...
| `feature-status-auto-wall-duplicate-prune` | Published / validated | Removed the stale duplicate `Auto-wall construction toggle is global` row from [Feature status](Feature-Status-Register) after rechecking that the branch-checked row remains source-backed by current Chernarus and maintained Vanilla `isAutoWallConstructingEnabled` paths. [Construction and CoIn](Construction-And-CoIn-Systems-Atlas) remains canonical. No gameplay source changed. |
| `salvage-payout-cleanup-branch-route` | Published / validated | Refreshed [Construction and CoIn](Construction-And-CoIn-Systems-Atlas) with a branch/root check: current source Chernarus, maintained Vanilla, stable `origin/master`, Miksuu upstream, `perf/quick-wins`, the feature-bundle release and the Miksuu salvage branches all keep lowercase salvage payout calls, local wreck deletion and the `updatesalvage.sqf` `||` loop. Condensed [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and the machine ledgers. No gameplay source changed. |
...
| `rhud-endgame-title-handle-branch-route` | Published / validated | Refreshed [UI IDD collision repair](UI-IDD-Collision-Repair) with a title display-handle branch matrix: current source Chernarus, maintained Vanilla, stable `origin/master`, latest Miksuu upstream and the feature-bundle release all still share `currentCutDisplay` between `OptionsAvailable`/RHUD/action icons and `EndOfGameStats`. Added the compact [Feature status](Feature-Status-Register) row and condensed [Client UI systems atlas](Client-UI-Systems-Atlas) plus [Source fix propagation queue](Source-Fix-Propagation-Queue). No gameplay source changed. |
