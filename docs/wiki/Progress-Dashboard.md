...
| `vehicle-cargo-loop-bounds-current-miksuu-refresh-2026-06-22` | Superseded / corrected | The earlier cargo refresh had treated local `d9506078` as current Miksuu. The corrected branch split is: docs branch `b2544207`, current Miksuu `b8389e748243` and historical EASA QoL `a66d4691` still carry inclusive loops at `Common_EquipVehicle.sqf:27,33,39` and `Common_EquipBackpack.sqf:35,41` in both maintained roots. Current stable `origin/master@0139a346` and historical `a96fdda2` fix all five loops in both maintained roots; `perf/quick-wins@0076040f` fixes Chernarus only. No gameplay source changed. |
