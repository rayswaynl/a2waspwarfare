...
| Lane | Status | Output |
| --- | --- | --- |
| `commander-hq-atlas-route-prune` | Published / validated | Refreshed [Commander/HQ lifecycle](Commander-HQ-Lifecycle-Atlas) so current docs head `8c3942d2` is the visible commander/HQ source-continuity checkpoint. Targeted diffs from `f82a9127` and `e2c9f6ed` to `HEAD` over checked Chernarus and maintained Vanilla commander vote, reassignment, HQ score, disconnect, task and base-area/HQ authority paths return no source changes; branch refs still resolve to stable `cf2a6d6a`, release `a96fdda2`, Miksuu `b8389e74`, perf `0076040f` and AI commander `c20ce153`. Replaced repeated manual-reassignment branch proof with routes to [Commander reassignment call shape](Commander-Reassignment-Call-Shape) and [Commander vote/reassignment](Commander-Vote-And-Reassignment-Playbook), and added missing disconnect/base-area source anchors. No gameplay source changed. |
| `towns-camps-atlas-route-prune` | Published / validated | Refreshed [Towns, camps and capture](Towns-Camps-And-Capture-Atlas) so current docs head `5243f91d` is the visible town/camp source-continuity checkpoint. Targeted diffs from `3eefcb00` to `HEAD` over checked Chernarus and maintained Vanilla town init, town/camp AI loops, camp helpers, respawn, buy-menu and patrol source paths return no source changes; branch refs still resolve to stable `cf2a6d6a`, release `a96fdda2`, Miksuu `b8389e74`, perf `0076040f` and AI commander `c20ce153`. Replaced repeated camp-flag and zero-camp helper branch prose with routes to the atlas [Current Branch Scope](Towns-Camps-And-Capture-Atlas#current-branch-scope), [Feature status](Feature-Status-Register) and [Source fix propagation queue](Source-Fix-Propagation-Queue), while preserving Patrols v2 routing. No gameplay source changed. |
| `support-specials-authority-route-prune` | Published / validated | Refreshed [Support specials and tactical modules](Support-Specials-And-Tactical-Modules-Atlas) so current docs head `6c919abf` is the visible support/tactical source-continuity checkpoint. Targeted diffs from `ff8dd884` to `HEAD` over checked Chernarus and maintained Vanilla Tactical UI, `RequestSpecial`, artillery, UAV/Nuke/Zeta/MASH, AAR/ordnance, para-ammo and Rsc parameter paths return no source changes; branch refs still resolve to stable `cf2a6d6a`, release `a96fdda2`, Miksuu `b8389e74`, perf `0076040f`, upgrade queue `b061c905`, drone strike `8ca4be90` and recon UAV `563418ea`. Replaced repeated `RequestSpecial` authority and support transport scout prose with routes to [Server authority migration map](Server-Authority-Migration-Map), [ICBM authority](ICBM-Authority-Playbook), [Service menu affordability guards](Service-Menu-Affordability-Guards), [AI commander autonomy](AI-Commander-Autonomy-Audit), [Client UI systems](Client-UI-Systems-Atlas) and branch-only support smoke routes. No gameplay source changed. |
| `economy-supply-atlas-route-prune` | Published / validated | Refreshed [Economy, towns and supply](Economy-Towns-And-Supply) so current docs head `8cb43e4e` is the visible economy/supply source-continuity checkpoint. Targeted diffs from `8a6695b8` and `6d05cb5a` to `HEAD` over checked Chernarus and maintained Vanilla economy/supply constants, resource loop, income display, supply mission, side-supply helper, AI upgrade, kill-bounty and stringtable paths return no source changes. Replaced repeated AI debit, side-supply, supply-mission truck/heli and AI logistics branch proof with routes to [Economy authority first cut](Economy-Authority-First-Cut), [Supply mission architecture](Supply-Mission-Architecture), [AI commander autonomy](AI-Commander-Autonomy-Audit) and [Upgrades and research](Upgrades-And-Research-Atlas). No gameplay source changed. |
| `upgrades-atlas-route-prune` | Published / validated | Refreshed [Upgrades and research atlas](Upgrades-And-Research-Atlas) so current docs head `1b07716d` is the visible upgrade/research source-continuity checkpoint. Targeted diffs from `d4cfef80` to `HEAD` over checked Chernarus and maintained Vanilla upgrade UI/server/config/Rsc paths return no source changes; branch refs still resolve to stable `cf2a6d6a`, release `a96fdda2`, Miksuu `b8389e74`, perf `0076040f`, AI commander `c20ce153` and upgrade queue `b061c905`. Replaced repeated AI debit, `upgrade-sync` and stale upgrade-dialog branch proof with routes to [AI commander autonomy](AI-Commander-Autonomy-Audit#ai-upgrade-debit-branch-matrix), [Support specials](Support-Specials-And-Tactical-Modules-Atlas#upgrade-sync-branch-matrix) and [UI resource parity cleanup](UI-Resource-Parity-Cleanup). No gameplay source changed. |
