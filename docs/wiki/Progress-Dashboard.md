...
| Lane | Status | Output |
| --- | --- | --- |
| `gameplay-systems-atlas-route-prune` | Published / validated | Refreshed [Gameplay systems atlas](Gameplay-Systems-Atlas) so current docs head `2d88dd5a` is the visible gameplay gateway source-continuity checkpoint. Targeted diffs from `ca40f202` to `HEAD` over checked Chernarus town, town-AI/patrol, economy, commander, construction, factory and victory source paths return no source changes; branch refs still resolve to stable `cf2a6d6a`, release `a96fdda2`, Miksuu `b8389e74` and perf `0076040f`. Replaced repeated DR-15, Patrols v2, latent `AIBuyUnit` and victory/endgame branch prose with routes to owner pages. No gameplay source changed. |
