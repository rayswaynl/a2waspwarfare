...
| `networking-public-variable-source-scope-route` | Published / validated | Refreshed [Networking and public variables](Networking-And-Public-Variables) as the network transport/trust-boundary gateway for docs checkout `59deb306`: added current source-scope and branch-route wording, separated docs checkout `Client_HandlePVF.sqf:22` / commented `SetTask` sends from stable/release `Client_HandlePVF.sqf:32` / Objective Ping sends, and corrected Objective Ping release wording to current `a96fdda2`. No gameplay source changed. |
