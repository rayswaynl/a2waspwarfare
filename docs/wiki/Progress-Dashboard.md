...
| --- | --- | --- | --- |
| Codex | Active | `documentation-finisher-loop` | Keep closing stale docs/machine-status gaps; latest work deep-audited `origin/feature/zargabad-map` and surfaced the key distinction: static Zargabad validation passes, but runtime hosted/dedicated/JIP/HC evidence remains open. Next best candidates are supply-heli runtime smoke evidence intake, P0/P1 accuracy drift with exact source refs, or perf quick-wins branch audit. |
