...
| `client-skill-release-status-refresh` | Published / validated | Corrected stale release-branch wording for [Client skill init idempotency](Client-Skill-Init-Idempotency): current release head `7195b331` now has one `Skill_Init.sqf` call at `Client/Init/Init_Client.sqf:564` and `WFBE_SK_FNC_Apply` at `:587` in both maintained release roots. [Current source snapshot](Current-Source-Status-Snapshot), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine records now agree; stable `origin/master` still duplicates and Arma smoke remains pending. |
| `gear-template-creation-gate-branch-route` | Published / validated | Added [Gear template profile filter](Gear-Template-Profile-Filter#creation-gate-branch-matrix) branch scope for template creation/display/save gate alignment: current source Chernarus, maintained Vanilla, stable `origin/master` `2cdf5fb8`, Miksuu upstream `f532f706`, `perf/quick-wins` `0076040f` and release `7195b331` all keep the AddTemplate Barracks-or-Gear creation gate, Gear-only FillTemplates display gate and undefined `_u_upgrade` save filter. Condensed [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine records. No gameplay source changed. |
| `ai-supply-truck-branch-route` | Published / validated | Added [AI commander autonomy](AI-Commander-Autonomy-Audit#ai-supply-truck-branch-matrix) branch scope for the config-gated AI supply-truck trap: current source/Vanilla, stable `origin/master`, Miksuu upstream and `perf/quick-wins` keep the commented `UpdateSupplyTruck` compile plus raw gated spawn and missing `supplytruck.fsm`; release `7195b331` logs/disables the branch in both maintained roots; `feat/ai-commander` guards only Chernarus. Condensed [Abandoned feature revival](Abandoned-Feature-Revival-Review), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine records. No gameplay source changed. |
| `dead-code-parameter-duplicate-prune` | Published / validated | Removed a duplicate parameter-findings table from [Dead/stale code register](Dead-Code-And-Stale-Code-Register#parameter-and-config-findings). The actionable `WFBE_C_AI_MAX` and `WFBE_C_UNITS_CLEAN_TIMEOUT` findings now stay single-sourced in [Current Findings](Dead-Code-And-Stale-Code-Register#current-findings), the priority backlog and [Mission parameters](Mission-Parameters-Localization-And-Generated-Build-Inputs#parameter-cache-flow). No gameplay source changed. |
