...
| `version-sqf-current-b74-refresh-2026-06-23` | Published / validated | Refreshed [Mission config/version graph](Mission-Config-Version-Include-Graph), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue#rule) and release-readiness generated-input gate with current `version.sqf` evidence. Fresh docs mirror checkout `HEAD@ded20d3d` has no present source Chernarus or maintained Vanilla Takistan `version.sqf`; checked docs/source, current stable `origin/master@0139a346`, B69 `origin/claude/b69@8d465fce`, B74 `origin/claude/b74-aicom-spend@b23f557f`, current Miksuu `b8389e748243` and perf `0076040f` do not track live Chernarus/Vanilla `version.sqf` files. B69/B74 track only a Chernarus `version.sqf.template`, which does not satisfy the live generated input gate. No gameplay source changed. |
| `loadoutmanager-root-current-b74-refresh-2026-06-23` | Published / validated | Refreshed [Tools and build workflow](Tools-And-Build-Workflow), [Source fix propagation queue](Source-Fix-Propagation-Queue#rule), [Testing workflow](Testing-Debugging-And-Release-Workflow) and release-readiness tooling gate with branch-sensitive LoadoutManager root discovery. Docs/source `HEAD@ffdfaa24` accepts an ancestor named `a2waspwarfare` or markers `Missions` + `Missions_Vanilla` + `Tools/LoadoutManager/LoadoutManager.csproj`; current stable `origin/master@0139a346`, B69 `origin/claude/b69@8d465fce`, B74 `origin/claude/b74-aicom-spend@b23f557f` and historical `a96fdda2` accept named-root or Chernarus mission + `Tools/LoadoutManager` + `AGENTS.md`; current Miksuu `b8389e748243` and perf `0076040f` still require the named ancestor. No gameplay source changed. |
| `victory-endgame-current-b74-refresh-2026-06-23` | Published / validated | Refreshed [Victory/endgame atlas](Victory-And-Endgame-Atlas#current-branch-scope), [Feature status](Feature-Status-Register) and [Source fix propagation queue](Source-Fix-Propagation-Queue) with current stable/B69/B74 match-outcome evidence. Docs/source `HEAD@4c87efd2` remains old-shape for checked victory/endgame paths. Current stable `origin/master@0139a346` keeps the gated default path with HQ nil/null skip, mixed guard and direct `_x` WASPSTAT/logger semantics in both maintained roots. Current B69 `origin/claude/b69@8d465fce` and adjacent B74 `origin/claude/b74-aicom-spend@b23f557f` partially patch Chernarus only (`server_victory_threeway.sqf:16,26,45`; `Client_EndGame.sqf:6-11`; `GUI_EndOfGameStats.sqf:8-10`), while maintained Vanilla remains stable-shaped for the server loop and lacks the GUER stats label. No gameplay source changed. |
| `rhud-endgame-title-handle-current-b74-refresh-2026-06-23` | Published / validated | Refreshed [UI IDD collision repair](UI-IDD-Collision-Repair#title-display-handle-branch-matrix), [Feature status](Feature-Status-Register) and [Source fix propagation queue](Source-Fix-Propagation-Queue) with current stable/B69/B74 RHUD/endgame `currentCutDisplay` evidence. Docs/source `HEAD@92b52e73` is unchanged from `edbd341e` / `b5219d47` for checked title-handle paths and still shares `currentCutDisplay` between `OptionsAvailable`/RHUD/action icons and `EndOfGameStats` in both maintained roots. Current stable `origin/master@0139a346`, B69 `origin/claude/b69@8d465fce`, B74 `origin/claude/b74-aicom-spend@b23f557f`, current Miksuu `b8389e748243`, perf `0076040f` and historical `a96fdda2` all keep the shared-handle shape in both maintained roots. Checked `B69..B74` title-handle deltas are empty; checked `origin/master..B74` only adds unrelated Chernarus GUER-label / vehicle-tint title content. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | `documentation-finisher-loop` | Latest published lane: `hq-score-current-b741-refresh-2026-06-23`. Continue bounded source-backed docs passes from this dashboard, the refreshed [PR cleanup lab](PR-Cleanup-And-Integration-Lab), Feature Status, Source Fix queue and pruning ledger. |
