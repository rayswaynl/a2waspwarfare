...
| Lane | Status | Output |
| --- | --- | --- |
| `client-ui-atlas-owner-route-prune` | Published / validated | Refreshed [Client UI systems](Client-UI-Systems-Atlas) as an orientation page: synced live wiki-only atlas deltas into `docs/wiki`, added a "How To Use This Page" owner-route table, condensed duplicated UI risk rows into canonical owner links, and updated current-head UI branch refs for `origin/master` `cf2a6d6a`, Miksuu `b8389e74`, `perf/quick-wins` `0076040f` and release `a96fdda2`. Current/release send Objective Ping `SetTask` at `GUI_Menu_Command.sqf:336,344`; Miksuu still comments those sends. Fast-travel fee UX and malformed `RscClickableText.soundPush[]` remain source-present / patch-ready through the atlas matrices. No gameplay source changed. |
