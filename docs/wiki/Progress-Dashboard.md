...
| Lane | Status | Output |
| --- | --- | --- |
| `economy-authority-current-branch-route` | Published / validated | Refreshed [Economy authority first cut](Economy-Authority-First-Cut) for docs checkout `f52ccee8`, stable `cf2a6d6a`, release `a96fdda2`, Miksuu `b8389e74` and perf `0076040f`: docs/stable/release/Miksuu still keep the side-supply overspend-as-credit floor, payload-side temp-channel trust and reason-string off-by-one in both maintained roots; perf fixes only the Chernarus arithmetic floor; and resource-income payout/display drift remains open everywhere checked. Aligned [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine rows. No gameplay source changed. |
