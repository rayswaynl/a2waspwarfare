...
| `mission-lifecycle-source-scope-route` | Published / validated | Refreshed [Mission entrypoints and lifecycle](Mission-Entrypoints-And-Lifecycle) and [Lifecycle wait-chain](Lifecycle-Wait-Chain) for current docs checkout `9da5f1d0`: added source-scope/branch-scope routing, corrected generated `version.sqf` presence wording, replaced the duplicated boot-flag mini-table with the canonical wait-chain link, and fixed current-checkout line drift for client init and HC/wait-chain anchors without changing gameplay source. |
