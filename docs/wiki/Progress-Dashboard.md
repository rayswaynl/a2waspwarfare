...
| `icbm-launched-receiver-branch-scope` | Published / validated | Source-checked `ICBM_launched`: current source/Vanilla, stable/upstream and release all keep the client PVEH receiver and stale handler comments, while fixed-string searches found no active assignment or `publicVariable "ICBM_launched"` sender. Current live nuke display/damage path remains `NukeIncoming` -> `RequestSpecial ["ICBM", ...]` -> `HandleSpecial "icbm-display"`. Updated [PV channel index](Public-Variable-Channel-Index), [Dead/stale code register](Dead-Code-And-Stale-Code-Register) and machine records. No gameplay source changed. |
