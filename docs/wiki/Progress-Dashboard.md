...
| `factory-purchase-atlas-branch-scope-route` | Published / validated | Refreshed [Factory and purchase systems](Factory-And-Purchase-Systems-Atlas) as the purchase gateway: added a top route table and current branch scope for docs checkout `8d611092`, stable `origin/master` `cf2a6d6a`, Miksuu `b8389e74`, `perf/quick-wins` `0076040f`, release `a96fdda2` and `origin/feat/buymenu-easa-qol` `a66d4691`; separated stable/release selected-detail/key/refund fixes from docs/Miksuu/perf old-shape debt; and aligned the factory rows in [Feature status](Feature-Status-Register) plus [Source fix propagation queue](Source-Fix-Propagation-Queue). No gameplay source changed. |
