...
| `construction-atlas-branch-scope-route` | Published / validated | Refreshed [Construction and CoIn systems](Construction-And-CoIn-Systems-Atlas) as a construction gateway: added a top route table and current branch-scope matrix for docs checkout `1aa178f8`, stable `origin/master` `cf2a6d6a`, Miksuu `b8389e74`, `perf/quick-wins` `0076040f` and release `a96fdda2`; split docs-checkout commander-team ARTY from stable/release marker-based ARTY discovery; and updated construction rows in [Feature status](Feature-Status-Register) plus [Source fix propagation queue](Source-Fix-Propagation-Queue). No gameplay source changed. |
