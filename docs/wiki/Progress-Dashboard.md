...
| `economy-menu-idc-branch-scope` | Published / validated | Source-checked Economy menu IDC drift: current source/stable/upstream Chernarus plus maintained Vanilla write stale disabled-state controls `23004/23005/23006`, while `RscMenu_Economy` declares `23002`, `23003` and `23008+`. Release Chernarus rewrote the path around dashboard IDC `23020`, but release Vanilla still carries the stale writes. Updated [Client UI systems](Client-UI-Systems-Atlas), [Feature status](Feature-Status-Register), [Dead/stale code register](Dead-Code-And-Stale-Code-Register) and machine backlog records. No gameplay source changed. |
