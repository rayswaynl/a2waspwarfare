...
| `commander-reassignment-agent-context-current-head-cleanup-2026-06-24` | Published / validated | Refreshed stale DR-15 commander reassignment compact context in [`agent-context.json`](agent-context.json) and recorded the pruning route through [Commander vote/reassignment](Commander-Vote-And-Reassignment-Playbook#current-branch-scope) plus [Commander reassignment call shape](Commander-Reassignment-Call-Shape#current-branch-matrix). Docs/source tree `HEAD@1e08d0d91` still uses `_side = _this` at `Server_AssignNewCommander.sqf:3` in both maintained roots, while current stable/B74.1 `origin/master@f8a76de34`, current B74.2 `21b62b04`, B69 `8d465fce`, adjacent B74 `b23f557f`, Miksuu `b8389e748243`, perf `0076040f`, historical `a96fdda2` and `c20ce153` use `_side = _this select 0` at `:4`. Duplicate notifications, visible-name UI targeting, requester authority, docs/source propagation, modded forks and Arma smoke remain open. No gameplay source changed. |
| `loadoutmanager-agent-context-b742-current-head-cleanup-2026-06-24` | Published / validated | Refreshed compact LoadoutManager/generated-input context in [`agent-context.json`](agent-context.json), [Source Fix queue](Source-Fix-Propagation-Queue#rule) machine facts and [`agent-release-readiness.json`](agent-release-readiness.json) from stale B74.2 `d472da6a` wording to current `origin/claude/b74.2-aicom@21b62b04`. Docs/source `HEAD@101930f47` is unchanged through repo `HEAD@025d5cf` for checked tooling/generated-input paths; stable/B74.2/B69/B74 use marker-root discovery without checked `A2WASP_SKIP_ZIP`, while Miksuu/perf remain named-root-only. `origin/codex/version-sqf-current-b74@499c2543` is a docs/wiki tip with docs-source tooling shape, not a current stable/B74.2 generated-input rescue. No gameplay source changed. |
| `victory-endgame-b742-current-head-followup-2026-06-24` | Published / validated | Refreshed [Victory/endgame atlas](Victory-And-Endgame-Atlas#current-branch-scope), [Feature status](Feature-Status-Register) and [Source Fix queue](Source-Fix-Propagation-Queue) from stale B74.2 `origin/claude/b74.2-aicom@d472da6a` wording to current `origin/claude/b74.2-aicom@21b62b04`. Docs/source `HEAD@c2b614e8cc87` is unchanged from `4c87efd2` / `d30d23466` / `a0a86da2` / `2f2132f8` for checked victory/endgame paths. Current B74.2 matches current stable/B74.1 for checked server/client/logger behavior; checked `d472da6a..21b62b04` is empty for maintained-root victory paths, and `origin/master..origin/claude/b74.2-aicom` only line-drifts source-Chernarus victory constants to `Init_CommonConstants.sqf:869-870`. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready / documentation finisher loop | `documentation-finisher-loop` | Stale upgrade dialog current B74.2 head refresh is published; pick the next bounded source-backed lane after reading the coordination surfaces and avoiding active owners. |
