...
| Lane | Status | Output |
| --- | --- | --- |
| `sqf-atlas-source-snapshot-route` | Published / validated | Refreshed [SQF code atlas](SQF-Code-Atlas) as the compile/source-map gateway: rechecked docs checkout `b13308ff` against the earlier `04a60e43` Chernarus source tree, confirmed the 738-reference preprocess snapshot remains valid, fixed the regenerate command's path trim, and replaced stale server-init caveats with branch-specific routes for duplicate binds, AI supply-truck startup and server FPS monitor scope. No gameplay source changed. |
