...
| Lane | Status | Output |
| --- | --- | --- |
| `b745-aicom-attack-heli-base-route-2026-06-24` | Published / validated | Split the remaining B74.5/B74.4 AICOM helper evidence into owner routes. [AI commander autonomy](AI-Commander-Autonomy-Audit#b745--b744-aicom-follow-up) now distinguishes branch-only source-Chernarus attack-heli cap/base-reaper, AH1Z enrollment and AI base-sell evidence from the already-existing MHQ relocation worker; [Commander/HQ lifecycle](Commander-HQ-Lifecycle-Atlas#current-branch-scope) owns the MHQ boundary note; [Construction/CoIn](Construction-And-CoIn-Systems-Atlas#authority-and-risk-register) keeps the caveat that AI structure recycling is not general sell-authority closure. Checked current stable `origin/master@f8a76de34`, current B74.2 `origin/claude/b74.2-aicom@21b62b04`, Miksuu `b8389e74` and B74.5 maintained Vanilla Takistan have no matching new attack-heli/base-sell symbols. No gameplay source changed. |
| `b745-primitive-roster-vote-ui-route-2026-06-24` | Published / validated | Split B74.5/B74.4 primitive roster evidence into owner pages without promoting it to current stable or maintained Vanilla behavior. [Client UI systems](Client-UI-Systems-Atlas#current-branch-scope) now owns the primitive vote-row display fallback; [Player join/disconnect](Player-Join-Disconnect-And-AntiStack-Lifecycle#branch-intel---b745-primitive-roster-jip) owns the server `WFBE_JIP_ROSTER_%1` push and early receiver; [Commander vote/reassignment](Commander-Vote-And-Reassignment-Playbook#current-branch-scope) keeps the caveat that primitive `-1` rows are display placeholders until live team-index rows arrive, not a vote-outcome policy change. Checked `origin/master@f8a76de34`, `origin/claude/b74.2-aicom@21b62b04`, `miksuu/master@b8389e74` and maintained Vanilla Takistan paths have no matching primitive-roster symbols. No gameplay source changed. |
| `b745-branch-intake-2026-06-24` | Published / validated | Classified newly fetched `origin/claude/b745@b996bcb3` as a docs-only branch-intake bundle, not current stable, PR-routed, maintained Vanilla or release-ready behavior. The branch is based on `origin/master@f8a76de34`, has no PR route, is clean under `git diff --check`, includes B74.2 `21b62b04` as an ancestor, and changes 126 files over stable / 105 files over B74.2. Updated [Feature status](Feature-Status-Register), [Current source status](Current-Source-Status-Snapshot#2026-06-24-b745-branch-intake), [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack), [AI commander autonomy](AI-Commander-Autonomy-Audit#b745--b744-aicom-follow-up), [Tools/build workflow](Tools-And-Build-Workflow#tooling-project-inventory) and coordination/machine surfaces. No gameplay source changed. |
| `mission-entrypoints-lifecycle-current-b742-route-refresh-2026-06-24` | Published / validated | Refreshed [Mission entrypoints/lifecycle](Mission-Entrypoints-And-Lifecycle#source-scope), [Lifecycle wait-chain](Lifecycle-Wait-Chain), [Architecture overview](Architecture-Overview#current-branch-scope), [AI/headless](AI-Headless-And-Performance#current-branch-scope), [Headless delegation](Headless-Delegation-And-Failover-Playbook#current-behavior) and the mission-entrypoints machine row so boot/HC routing no longer points through older June/`origin/master@0139a346` wording. Repo docs branch `HEAD@55cb55e2170f` is unchanged from `05664f17` / `2de2de92` for checked boot/runtime paths. Current stable/B74.1 `origin/master@f8a76de34`, current B74.2 `origin/claude/b74.2-aicom@21b62b04`, B69, B74, Miksuu, perf and historical release-line refs keep the same front-door topology; B74.2 changes only source-Chernarus `Init_Client.sqf` / `Init_Server.sqf` among checked boot paths. Docs/source/Miksuu/perf/historical refs keep simple HC sleep+notify; current stable/B74-shaped refs use reseat/deadspawn/watchdog HC bootstrap. No gameplay source changed. |
| `commander-hq-reassignment-route-current-b742-refresh-2026-06-24` | Published / validated | Refreshed [Commander/HQ lifecycle](Commander-HQ-Lifecycle-Atlas#current-branch-scope) so its manual-reassignment gateway routes through current stable/B74.1 and current B74.2 evidence instead of older `8c3942d2` / `cf2a6d6a` wording. Repo docs branch `HEAD@1f07b39d` is unchanged from `60c35c05` for checked reassignment paths and still has old helper unpacking in both maintained roots. Current stable/B74.1 `origin/master@f8a76de34`, current B74.2 `origin/claude/b74.2-aicom@21b62b04`, B69, adjacent B74, Miksuu, perf and historical release/AI refs fix helper unpacking in maintained roots, but duplicate notifications, visible-name UI targeting, requester authority and full modded fork drift remain open. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | `documentation-finisher-loop` | B74.5 AICOM attack-heli/base helper route split is published and mirrored. Next pass should claim a new bounded source-backed docs lane after rereading coordination surfaces. |
...
| --- | --- | --- | --- |
| `documentation-finisher-loop` | Codex | Ongoing / ready | Latest bounded pass `b745-aicom-attack-heli-base-route-2026-06-24` is published/validated; next work should claim a new narrow source-backed docs lane before editing. |
