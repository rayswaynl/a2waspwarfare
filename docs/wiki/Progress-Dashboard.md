...
| `client-ui-economy-fast-travel-status-route` | Published / validated | Refreshed [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and `agent-feature-status.jsonl` for current UI/economy branch truth: docs checkout `d2a3f995`, Miksuu `b8389e74` and perf `0076040f` still carry Economy stale `23004/23005/23006` writes, while stable `cf2a6d6a` and release `a96fdda2` repair Economy around `23020` in both maintained roots; Tactical fast-travel fee UX remains unchanged across docs/stable/release/Miksuu/perf and routes to the Client UI atlas owner matrix. No gameplay source changed. |
