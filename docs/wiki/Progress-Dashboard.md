...
| Lane | Status | Output |
| --- | --- | --- |
| `send-message-current-stable-refresh` | Published / validated | Refreshed the [Public variable channel index DR-46 matrix](Public-Variable-Channel-Index#send_message-direct-compile-branch-matrix) against current stable `origin/master@0139a346`: Chernarus and maintained Vanilla still register direct `SEND_MESSAGE` at `Client/FSM/updateclient.sqf:12`, compile payload text at `Client_onEventHandler_SEND_MESSAGE.sqf:27`, and helper-compile/broadcast at `Common_SendMessage.sqf:26,38`. Miksuu `b8389e74`, perf `0076040f` and historical release commit `a96fdda2` keep the same shape; current origin exposes no `release/*` heads on 2026-06-21. No gameplay source changed. |
| `wasp-marker-wait-current-stable-refresh-2026-06-21` | Published / validated | Refreshed [WASP marker wait cleanup](WASP-Marker-Wait-Cleanup), [WASP overlay](WASP-Overlay), [Performance opportunity sweep](Performance-Opportunity-Sweep), [Feature status](Feature-Status-Register) and [Source fix queue](Source-Fix-Propagation-Queue) after current stable moved to `origin/master@0139a346`: both maintained roots still keep the display-54 input-lock busy wait at `WASP/global_marking_monitor.sqf:57,62,65,69-70`, the display-12 sibling is throttled at `:81`, and the helper now launches from `Client/Init/Init_Client.sqf:309`. Miksuu `b8389e74` and perf `0076040f` remain unrescued; current origin exposes no `release/*` heads on 2026-06-21. No gameplay source changed. |
| `map-boundaries-current-head-refresh` | Published / validated | Refreshed [Map boundaries and off-map enforcement](Map-Boundaries-And-Offmap-Enforcement) from historical `cf2a6d6a4` caveat wording to current `origin/master@0139a346` evidence. Updated shifted anchors for common init, boundary parameter/defaults, client compile bindings, off-map timeout, stringtable warning, paradrop support and server town distribution, and clarified the `Server/Init/Init_Towns.sqf` route. No gameplay source changed. |
| `pvf-dispatcher-current-stable-closeout-2026-06-21` | Published / validated | Refreshed the registered PVF dispatcher status after current stable moved to `origin/master@0139a346`: both maintained roots now use `missionNamespace getVariable` plus a `CODE` check at `Server_HandlePVF.sqf:14-15` and `Client_HandlePVF.sqf:32-33`, while docs checkout and `origin/perf/quick-wins` still use dispatch-time `Call Compile`. Updated [PVF dispatch implementation](PVF-Dispatch-Implementation-Playbook), [Networking and public variables](Networking-And-Public-Variables), [Public variable channel index](Public-Variable-Channel-Index), [Feature status](Feature-Status-Register) and [Source fix queue](Source-Fix-Propagation-Queue) to keep explicit allowlisting/logging, DR-55 sender authentication and direct `SEND_MESSAGE` compile open. No gameplay source changed. |
| `hq-score-current-stable-refresh` | Published / validated | Rechecked the [Commander/HQ lifecycle HQ-kill score matrix](Commander-HQ-Lifecycle-Atlas#hq-kill-score-and-bounty-branch-matrix) against current stable `origin/master@0139a346`: Chernarus and maintained Vanilla still double-award HQ destruction score through `Server/Functions/Server_OnHQKilled.sqf:23,52/54,80,83,86`, with `WFBE_C_BUILDINGS_SCORE_COEF = 3` at `Common/Init/Init_CommonConstants.sqf:529`. [Feature status](Feature-Status-Register) and [Source fix queue](Source-Fix-Propagation-Queue) now name current stable explicitly while older docs/Miksuu/perf/release/feat-ai rows stay branch-scoped. No gameplay source changed. |
| `icbm-launched-pveh-branch-scope-refresh` | Published / validated | Refreshed [Dead/stale code register](Dead-Code-And-Stale-Code-Register) for the receiver-only `ICBM_launched` PVEH. The row now names docs head `4c01dfb`, stable `origin/master@0139a346`, Miksuu `upstream/master@b8389e74` and `origin/perf/quick-wins@0076040f`, records that current origin exposes no `release/*` heads on 2026-06-21, and routes the owner decision to retire the stale handler/comments or revive the channel with one sender plus ICBM smoke. No gameplay source changed. |
...
- Salvage payout/loop branch scope: [Construction and CoIn systems](Construction-And-CoIn-Systems-Atlas#salvage-branch-matrix), [Feature status](Feature-Status-Register) and [Source fix propagation queue](Source-Fix-Propagation-Queue) now treat salvage as inherited current-source debt: all checked roots keep the lowercase payout call, `||` loop and client-local deletion/reward shape.
