...
| `side-supply-current-stable-refresh-2026-06-21` | Published / validated | Refreshed [Economy authority first cut](Economy-Authority-First-Cut#side-supply-branch-matrix), [Feature status](Feature-Status-Register) and [Source fix queue](Source-Fix-Propagation-Queue) after current stable `origin/master@0139a346` proved the older `cf2a6d6a` / release-head wording was stale. Docs branch `7047da5d9` is source-unchanged from `f52ccee8` for the checked side-supply/reason paths; docs/current stable/Miksuu keep the overspend-as-credit floor and payload-side trust in both maintained roots, with current-stable anchors at `Common_ChangeSideSupply.sqf:25` and `Server_ChangeSideSupply.sqf:12,36`. `origin/perf/quick-wins@0076040f` fixes only the Chernarus arithmetic floor; Vanilla propagation, side/channel/requester validation and reason parsing remain open. Current origin exposes no `release/*` heads. No gameplay source changed. |
