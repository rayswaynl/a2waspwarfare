...
| Lane | Status | Output |
| --- | --- | --- |
| `mission-start-parameter-index` | Published / validated | Added [Mission start parameters index](Mission-Start-Parameters-Index): 89 active lobby-visible parameters from maintained Chernarus/Vanilla `Rsc/Parameters.hpp`, with source-order, defaults, choices, category ranges and caveats for commented/forced/no-op/mislabeled parameters. |
| `air-event-parameter-doc-gap` | Published / validated | Answered and documented the old Air Event setting: `WFBE_AIR_EVENT_ENABLED` still exists, feeds `IS_air_war_event`, and affects event economy, town start mode, upgrade clearance, ICBM availability and Avenger/Tunguska-style heavy-AA factory content. Canonical route: [Mission parameters](Mission-Parameters-Localization-And-Generated-Build-Inputs#air-event-override-caveat). |
| `supply-scan-release-status-refresh` | Published / validated | Corrected stale release wording in [Supply mission architecture](Supply-Mission-Architecture), [Supply mission scan narrowing](Supply-Mission-Scan-Narrowing), [Current source snapshot](Current-Source-Status-Snapshot), [Feature status](Feature-Status-Register) and machine rows: current release head `7195b331` has the heli-aware narrowed command-center scan and dead-twin cleanup in both maintained release roots. Stable/upstream/perf still keep the broad 80 m command-center scan; Arma truck/heli smoke remains pending. |
| `hosted-fps-release-status-refresh` | Published / validated | Corrected stale hosted-FPS release wording in [Hosted server FPS loop sleep](Hosted-Server-FPS-Loop-Sleep), [Pending owner decisions](Pending-Owner-Decisions) and machine rows: current release head `7195b331` has guarded `serverFpsGUI.sqf` in both maintained roots and no redundant `monitorServerFPS.sqf` file; stable `origin/master` still keeps the old loop shape and Arma smoke remains pending. |
| `paratrooper-release-status-refresh` | Published / validated | Corrected stale release-branch wording for [Paratrooper marker revival](Paratrooper-Marker-Revival): docs/source and current release head `7195b331` register `HandleParatrooperMarkerCreation` in both maintained roots, while stable `origin/master` still omits it and Arma marker smoke remains pending. Updated [Current source snapshot](Current-Source-Status-Snapshot), [Feature status](Feature-Status-Register) and machine records. |
| `send-message-direct-compile-branch-route` | Published / validated | Added [Public variable channel index](Public-Variable-Channel-Index#send_message-direct-compile-branch-matrix) branch scope for DR-46: current source Chernarus, maintained Vanilla, stable `origin/master` `2cdf5fb8`, Miksuu upstream `f532f706`, `origin/perf/quick-wins` `0076040f` and release `7195b331` all keep direct `SEND_MESSAGE` receiver registration plus receiver/helper `call compile` of message text. Condensed [Networking](Networking-And-Public-Variables), [Hardening roadmap](Hardening-Implementation-Roadmap), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine records. No gameplay source changed. |
...
| `indicator-exploration-backlog` | Codex / future UI scout | Queued exploration | Build the indicator matrix from [Client UI systems](Client-UI-Systems-Atlas#indicator-exploration-backlog): HUD/title surfaces, map/tactical markers, support markers, menu/list icons, status channels and asset/path integrity before pruning or redesigning indicators. |
