...
| Lane | Status | Output |
| --- | --- | --- |
| `gear-template-profile-current-b74-refresh-2026-06-23` | Published / validated | Refreshed [Gear template profile filter](Gear-Template-Profile-Filter), [Gear/loadout/EASA atlas](Gear-Loadout-And-EASA-Atlas), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine rows with adjacent B74 evidence. Docs/source `HEAD@c7f361f4` is unchanged from `72b5f0de98f9` / `43c3ba05` for checked gear-template paths and still carries undefined `_u_upgrade` at `Client_UI_Gear_SaveTemplateProfile.sqf:33,52,75` plus `Init_ProfileGear.sqf:17,25` import-bound debt. Current stable `origin/master@0139a346`, B69 `origin/claude/b69@8d465fce` and B74 `origin/claude/b74-aicom-spend@b23f557f` fix the save-filter comparison at `:34,:57,:82` in both maintained roots, but keep the import guard and AddTemplate/FillTemplates policy split. B69..B74 and older-B69-to-B74 checked deltas are empty; no live release/gear/template rescue head is exposed. No gameplay source changed. |
| `camp-flag-current-b74-refresh-2026-06-23` | Published / validated | Refreshed [Towns, camps and capture atlas](Towns-Camps-And-Capture-Atlas#current-branch-scope), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine rows with adjacent B74 camp-flag evidence. Docs/source `HEAD@de911438` is unchanged from `28a7d9c5` for checked camp flag, `repair-camp` and `CampCaptured` client-handler paths and still flags old `_side` at `server_town_camp.sqf:135`. Current stable `origin/master@0139a346`, current B69 `origin/claude/b69@8d465fce`, adjacent B74 `origin/claude/b74-aicom-spend@b23f557f`, historical `a96fdda2` and live `origin/feat/naval-hvt-objectives@2e1c5931` fix independent capture in both maintained roots at `server_town_camp.sqf:83-86`; Miksuu `b8389e748243` remains old-owner and perf `0076040f` fixes Chernarus only. Repair-side world-flag refresh remains open everywhere checked; B69..B74 and older-B69-to-B74 checked deltas are empty. No gameplay source changed. |
| `auto-wall-current-b74-refresh-2026-06-23` | Published / validated | Refreshed [Construction and CoIn systems](Construction-And-CoIn-Systems-Atlas#current-branch-scope), [Server composition spawner reference](Server-Composition-Spawner-Function-Reference#wall-templates), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine rows with adjacent B74 evidence. Docs/source `HEAD@f3896116` is unchanged from `89aadf8c` for checked auto-wall paths. Docs/source, current Miksuu `b8389e748243` and perf `0076040f` keep the old false-default/no-exclusion global shape. Current stable `origin/master@0139a346`, current B69 `origin/claude/b69@8d465fce` and adjacent B74 `origin/claude/b74-aicom-spend@b23f557f` still keep one mission-global `isAutoWallConstructingEnabled`; stable/B69/B74 default it true and consume it with SmallSite `AARadar`/`CBRadar` plus MediumSite `AARadar`/`Bank`/`Reserve`/`ArtilleryRadar` exclusions. B69..B74 and older-B69-to-B74 checked auto-wall deltas are empty; current origin exposes no live release/auto-wall/construction/CoIn rescue head. No gameplay source changed. |
| `attack-wave-authority-current-b74-refresh-2026-06-23` | Published / validated | Refreshed [Attack-wave authority](Attack-Wave-Authority-Playbook#branch--root-matrix), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine rows with adjacent B74 evidence. Docs/source `HEAD@6d263dc` is unchanged from `1c1ea559` / `f3e157f2` for checked request/detail/client-price paths. Current stable `origin/master@0139a346`, current B69 `origin/claude/b69@8d465fce` and B74 `origin/claude/b74-aicom-spend@b23f557f` all keep `ATTACK_WAVE_INIT` client `_supply` / `_side` trust and `ATTACK_WAVE_DETAILS` detail-payload trust in both maintained roots; stable/B69/B74 buy-price consumers are `Client_UIFillListBuyUnits.sqf:90` and `GUI_Menu_BuyUnits.sqf:99,416`. B69..B74 and older-B69-to-B74 checked path deltas only touch constants; B74 Chernarus constants drift to `Init_CommonConstants.sqf:528,572-574`, maintained Vanilla remains `:349,393-395`. No live release/attack-wave/support rescue head is exposed. No gameplay source changed. |
| `upgrade-sync-current-b74-refresh-2026-06-23` | Published / validated | Refreshed [Support specials](Support-Specials-And-Tactical-Modules-Atlas#upgrade-sync-branch-matrix), [Upgrades and research](Upgrades-And-Research-Atlas#current-branch-scope), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine rows with adjacent B74 evidence. Docs/source `HEAD@b27717f5` is unchanged from `bc21f520` / `ff8dd884` for checked handler/caller/process-upgrade paths. Current stable `origin/master@0139a346`, current B69 `origin/claude/b69@8d465fce` and B74 `origin/claude/b74-aicom-spend@b23f557f` all keep mixed `_args` / `_this` parsing at `Server_HandleSpecial.sqf:3,67-73`, caller `GUI_UpgradeMenu.sqf:292` and sync lifecycle `Server_ProcessUpgrade.sqf:32,35,41` in both maintained roots. Checked B69..B74 and older-B69-to-B74 deltas are empty; B74's other `Server_HandleSpecial` hunks are unrelated to `upgrade-sync`. No live release/upgrade/special/queue rescue head is exposed. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | `documentation-finisher-loop` | Latest published lane: `gear-template-profile-current-b74-refresh-2026-06-23`. Continue bounded source-backed docs passes from this dashboard, the refreshed [PR cleanup lab](PR-Cleanup-And-Integration-Lab), Feature Status, Source Fix queue and pruning ledger. |
