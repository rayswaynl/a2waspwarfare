...
| Lane | Status | Output |
| --- | --- | --- |
| `networking-public-variable-route-prune` | Published / validated | Refreshed [Networking and public variables](Networking-And-Public-Variables) as the network architecture gateway: added a top "How To Use This Page" routing table, current branch-scope source anchors for generic PVF dispatch and direct `SEND_MESSAGE`, and made the PVF registration list an orientation copy that routes complete inventory to [Public variable channel index](Public-Variable-Channel-Index), dispatcher matrices to [PVF dispatch implementation](PVF-Dispatch-Implementation-Playbook) and per-handler authority to [Server authority migration map](Server-Authority-Migration-Map). No gameplay source changed. |
