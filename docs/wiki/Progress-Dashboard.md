...
| Lane | Status | Output |
| --- | --- | --- |
| `attack-wave-authority-dr41` | Integrated locally | DR-41 is now cross-linked from [Networking/PV](Networking-And-Public-Variables), [Hardening roadmap](Hardening-Implementation-Roadmap), [Economy](Economy-Towns-And-Supply), [Feature status](Feature-Status-Register), [Server authority map](Server-Authority-Migration-Map) and [`agent-hardening-backlog.jsonl`](agent-hardening-backlog.jsonl). |
...
| `attack-wave-authority-dr41` | Future code owner | Confirmed playbook | `ATTACK_WAVE_INIT` is confirmed high-risk; use [`agent-hardening-backlog.jsonl`](agent-hardening-backlog.jsonl) and [Server authority map](Server-Authority-Migration-Map) before patching. |
