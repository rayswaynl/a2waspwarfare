...
| `architecture-sqf-anchor-continuity-route` | Published / validated | Refreshed [Architecture overview](Architecture-Overview), [SQF code atlas](SQF-Code-Atlas), [Server gameplay runtime atlas](Server-Gameplay-Runtime-Atlas), [Server runtime and operations](Server-Runtime-And-Operations) and [Networking/public variables](Networking-And-Public-Variables) so current docs checkout `4277a2ad` is the visible source-continuity checkpoint. Verified the checked architecture, SQF compile-count, server-runtime and network source paths are unchanged from the older anchor snapshots; kept stable `cf2a6d6a`, release `a96fdda2`, Miksuu `b8389e74` and perf `0076040f` routed through owner pages. No gameplay source changed. |
