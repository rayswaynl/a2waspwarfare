...
| `ai-headless-gateway-fps-route-prune` | Published / validated | Refreshed [AI, headless and performance](AI-Headless-And-Performance) and [AI runtime/HC loop map](AI-Runtime-HC-Loop-Map) so current docs head `ca028bff` is the visible AI/HC source-continuity checkpoint. Targeted diffs from `ee383941` and broader maintained mission-root diffs from `b9e80da0` to `HEAD` return no checked Chernarus or maintained Vanilla source changes. Replaced repeated server-FPS publisher detail in the AI gateway with routes to [Server gameplay runtime atlas](Server-Gameplay-Runtime-Atlas#branch-scope-for-source-anchors), [Hosted server FPS loop sleep](Hosted-Server-FPS-Loop-Sleep), [Performance opportunity sweep](Performance-Opportunity-Sweep), [Testing workflow](Testing-Debugging-And-Release-Workflow) and [Source fix propagation queue](Source-Fix-Propagation-Queue#validation-matrix). No gameplay source changed. |
