...
| Lane | Status | Output |
| --- | --- | --- |
| `commander-vote-current-b74-refresh-2026-06-22` | Published / validated | Refreshed [Commander vote/reassignment](Commander-Vote-And-Reassignment-Playbook), [Commander/HQ lifecycle](Commander-HQ-Lifecycle-Atlas), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue), [Pending owner decisions](Pending-Owner-Decisions), [Hardening roadmap](Hardening-Implementation-Roadmap) and machine rows with current B74 vote evidence. Current stable `origin/master@0139a346`, current B69 `origin/claude/b69@8d465fce` and adjacent B74 `origin/claude/b74-aicom-spend@b23f557f` use `_highest >= _aiVotes` at `Server_VoteForCommander.sqf:43` in both maintained roots, while `GUI_VoteMenu.sqf:88` still previews AI/no commander by row-0/strict-majority logic. Docs/source `HEAD@e0d82714`, current Miksuu `b8389e748243`, perf `0076040f`, historical `a96fdda2` and historical `c20ce153` keep the old `>= || <=` comparison. The checked B69..B74 vote worker/preview delta is empty; no live release/vote/reassignment rescue head is exposed. No gameplay source changed. |
| `pvf-dispatch-current-b74-refresh-2026-06-22` | Published / validated | Refreshed [PVF dispatch implementation](PVF-Dispatch-Implementation-Playbook), [Networking and public variables](Networking-And-Public-Variables), [Public variable channel index](Public-Variable-Channel-Index), [Server authority migration map](Server-Authority-Migration-Map), [Hardening roadmap](Hardening-Implementation-Roadmap), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine rows with current B74 dispatcher evidence. Docs/source `HEAD@a0721301d4f5`, current Miksuu `b8389e748243`, perf `0076040f8a5e` and historical `a96fdda2` keep dispatch-time `Call Compile`. Current stable `origin/master@0139a346`, B69 `origin/claude/b69@8d465fce` and B74 `origin/claude/b74-aicom-spend@b23f557f` use `missionNamespace getVariable` plus `CODE` guards at `Server_HandlePVF.sqf:14-15` and `Client_HandlePVF.sqf:32-33` in both maintained roots, but still have value-only PVEHs at `Init_PublicVariables.sqf:56,61` and no registered-handler allowlist/rejection log. No live release/PVF/network/auth/public heads are exposed. No gameplay source changed. |
| `wasp-marker-wait-current-b74-clientfps-refresh-2026-06-22` | Published / validated | Refreshed [WASP marker wait cleanup](WASP-Marker-Wait-Cleanup), [Performance opportunity sweep](Performance-Opportunity-Sweep), [WASP overlay](WASP-Overlay), [Current source snapshot](Current-Source-Status-Snapshot), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine rows with current B74/client-fps branch evidence. Docs/source `HEAD@46840f048bd4`, current Miksuu `b8389e748243`, perf `0076040f8a5e` and historical `a96fdda2` keep the old unslept display-54 wait. Current stable `origin/master@0139a346`, B69 `origin/claude/b69@8d465fce`, B74 `origin/claude/b74-aicom-spend@b23f557f` and `origin/feat/client-fps@709258e7` carry `sleep 0.1` at `global_marking_monitor.sqf:64` in both maintained roots; B69/B74 launch at `Init_Client.sqf:397`, client-fps launches at Chernarus `:308` and Vanilla `:287`. No live `release/*` head is exposed, and Arma marker-dialog smoke remains pending. No gameplay source changed. |
| `resource-income-current-b69-b74-refresh-2026-06-22` | Published / validated | Refreshed [Economy, towns and supply](Economy-Towns-And-Supply#resource-income-branch-matrix), [Economy authority first cut](Economy-Authority-First-Cut), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue), [resource income tick engine](Resource-Income-Tick-Distribution-Engine) and machine rows with current stable/B69/B74 evidence. Docs/source `HEAD@c8ec223a`, Miksuu `b8389e748243`, perf `0076040f` and historical `a96fdda2` keep the old cap guard at `updateresources.sqf:31` over side supply (`:49`), team paychecks (`:63`) and AI commander funds (`:67`). Current stable `origin/master@0139a346` keeps side supply/team paychecks gated at `:45,:63,:77` but has AI commander fallback at `:102-109`. Current B69 `origin/claude/b69@8d465fce` and B74 `origin/claude/b74-aicom-spend@b23f557f` are identical for checked income files, with fallback at `:115-121`, Chernarus hybrid-refill cash checks and Chernarus supply multiplier behavior while maintained Vanilla stays raw-supply/non-hybrid. `Client_GetIncome.sqf:24-28` still omits the system-4 `1.5` display multiplier. No gameplay source changed. |
| `objective-ping-current-b69-b74-refresh-2026-06-22` | Published / validated | Refreshed [Feature status](Feature-Status-Register), [Client UI systems](Client-UI-Systems-Atlas#commander-and-tactical-menus), [Commander/HQ lifecycle](Commander-HQ-Lifecycle-Atlas), [Networking and public variables](Networking-And-Public-Variables), [Player UI workflow](Player-UI-Workflow-Map), [PVF send-helper contract](PVF-Send-Helper-Contract-Reference), [Abandoned feature revival](Abandoned-Feature-Revival-Review) and machine rows with current Objective Ping branch evidence. Docs/source `HEAD@86ab85b9d0b1`, Miksuu `b8389e748243`, perf `0076040f` and historical `c20ce1534be0` keep `SetTask` sends commented at `GUI_Menu_Command.sqf:335,337,343` with `Init_PublicVariables.sqf:33` registration. Current stable `origin/master@0139a346`, B69 `origin/claude/b69@8d465fce`, B74 `origin/claude/b74-aicom-spend@b23f557f` and `origin/feat/naval-hvt-objectives@2e1c59317186` send Objective Ping at `GUI_Menu_Command.sqf:336,344` with `:40` registration. Historical `a96fdda2` has live sends/`:36` registration but no live `release/*` head. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | `documentation-finisher-loop` | Latest published lane: `commander-vote-current-b74-refresh-2026-06-22`. Continue bounded source-backed docs passes from this dashboard, the refreshed [PR cleanup lab](PR-Cleanup-And-Integration-Lab), Feature Status, Source Fix queue and pruning ledger. |
