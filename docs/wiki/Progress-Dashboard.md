...
| Lane | Status | Output |
| --- | --- | --- |
| `resource-income-current-stable-refresh-2026-06-22` | Published / validated | Refreshed the [Economy towns/supply resource-income branch matrix](Economy-Towns-And-Supply#resource-income-branch-matrix), [Economy authority](Economy-Authority-First-Cut), [Feature status](Feature-Status-Register) and [Source fix queue](Source-Fix-Propagation-Queue) after source checks split old-shape refs from current stable. Docs branch `d7a30e15`, Miksuu `b8389e74`, `origin/perf/quick-wins@0076040f` and historical `a96fdda2` still keep AI commander funds inside the cap guard; current stable `origin/master@0139a346` still guards side supply/team paychecks but has AI-commander over-cap income/stipend fallback at `updateresources.sqf:102-109`. Income-system `4` display drift remains. No gameplay source changed. |
| `auto-wall-current-stable-scope-refresh-2026-06-22` | Published / validated | Refreshed the auto-wall global-toggle branch matrix after Feature Status and Source Fix queue still cited stale `cf2a6d6a` / `b8389e74` stable-Miksuu scope. Source checks split the refs: docs branch `origin/docs/developer-wiki-index@89aadf8c` and `origin/perf/quick-wins@0076040f` keep the old false-default/no-exclusion global at SmallSite `:110` / MediumSite `:125`; current stable `origin/master@0139a346` and current Miksuu `master@d9506078` keep the same global but default it `true` and consume it at `:123` / `:160` with AARadar plus related structure exclusions; historical `a96fdda2` only adds the AARadar guard; B69 `origin/claude/b69@35547c47` matches current-stable global scope. Updated [Construction and CoIn systems](Construction-And-CoIn-Systems-Atlas), [Feature status](Feature-Status-Register), [Source fix queue](Source-Fix-Propagation-Queue) and machine rows. No gameplay source changed. |
| `indicator-status-counter-branch-split-2026-06-22` | Published / validated | Refreshed the [Client UI status-counter audience split](Client-UI-Systems-Atlas#status-counter-audience-split), [Hosted server FPS loop sleep](Hosted-Server-FPS-Loop-Sleep) and [Performance opportunity sweep](Performance-Opportunity-Sweep) so player HUD `SERVER_FPS_GUI`, compatibility `WFBE_VAR_SERVER_FPS`, PerformanceAudit RPT/tooling output and transient status channels are no longer treated as one pruneable feature. Source scope: docs `a27086cd`, current stable `origin/master@0139a346`, Miksuu `b8389e74`, perf `0076040f` and historical `a96fdda2`; no gameplay source changed. |
| `b69-patch-a-branch-status-2026-06-22` | Published / validated | Refreshed the B69 AI commander docs for `origin/claude/b69@35547c47`: isolated `b8a1505f..35547c47` is Patch A branch code in Chernarus only, with HQ-strike constants at `Init_CommonConstants.sqf:229-232`, scaled strike gate / vehicle-punch picker / HC `"defense"` order in `AI_Commander_Strategy.sqf:511-559`, no maintained Vanilla diff and no PR for `claude/b69` on 2026-06-22. The docs explicitly keep capture interrupt as not implemented because `Common_RunCommanderTeam.sqf` has no branch diff. No gameplay source changed. |
| `visible-parameter-runtime-consumer-current-stable-refresh-2026-06-22` | Published / validated | Refreshed visible mission-parameter runtime-consumer routing after status pages still treated `WFBE_C_AI_MAX` and `WFBE_C_UNITS_CLEAN_TIMEOUT` as current no-op/comment-only cleanup. Source checks split the refs: docs branch `origin/docs/developer-wiki-index@ac932fbe`, `origin/perf/quick-wins@0076040f` and historical `a96fdda2` keep the old shape, while current stable `origin/master@0139a346` and current Miksuu `master@d9506078` read `WFBE_C_AI_MAX` in `AI_Commander_Produce.sqf:89` and `WFBE_C_UNITS_CLEAN_TIMEOUT` in `Common_TrashObject.sqf:21` in both maintained roots. Updated [Mission parameters](Mission-Parameters-Localization-And-Generated-Build-Inputs), [Mission start parameters](Mission-Start-Parameters-Index), [Dead/stale code](Dead-Code-And-Stale-Code-Register), [Player AI caps](Player-AI-Caps-And-Role-Balance), [Feature status](Feature-Status-Register), [Source fix queue](Source-Fix-Propagation-Queue) and machine rows. No gameplay source changed. |
