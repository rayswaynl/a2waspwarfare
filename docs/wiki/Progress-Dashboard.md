...
| Lane | Status | Output |
| --- | --- | --- |
| `factory-queue-token-branch-route` | Published / validated | Refreshed [Factory queue cleanup](Factory-Queue-Counter-Token-Cleanup) with a branch/root matrix: current docs/source, maintained Vanilla, stable and Miksuu upstream still keep both DR-33a crewless queue-counter leak and DR-33b low-entropy FIFO token; `perf/quick-wins` and release Chernarus patch only the crewless counter leak; maintained Vanilla propagation and token cleanup remain open. Condensed [Feature status](Feature-Status-Register) and [Source fix propagation queue](Source-Fix-Propagation-Queue). No gameplay source changed. |
