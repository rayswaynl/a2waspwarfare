...
| `source-fix-salvage-duplicate-prune` | Published / validated | Removed the older duplicate salvage row from [Source fix propagation queue](Source-Fix-Propagation-Queue#patch-ready-but-not-in-current-code). The queue now keeps one canonical salvage entry routed to [Construction and CoIn](Construction-And-CoIn-Systems-Atlas#salvage-branch-matrix), preserving the branch matrix, first casing fix, maintained Vanilla propagation and manual/truck salvage smoke gates. No gameplay source changed. |
