...
| Lane | Status | Output |
| --- | --- | --- |
| `client-ui-atlas-branch-route-prune` | Published / validated | Refreshed [Client UI systems atlas](Client-UI-Systems-Atlas) so current docs head `a03a1fb5` is the visible UI source-continuity checkpoint for checked UI/Rsc anchors. Targeted source diffs from `b5219d47` to `HEAD` over Chernarus and maintained Vanilla `Rsc`, `Client/GUI`, `Init_Client.sqf`, `Client_UpdateRHUD.sqf` and `updateavailableactions.fsm` return no source changes. Replaced repeated `wf-menu-ops-console` and `buymenu-easa-qol` branch-only detail with routes to [Feature status](Feature-Status-Register#partial--deferred--needs-review), [Pending owner decisions](Pending-Owner-Decisions#branch-only-feature-promotion-decisions), [Testing workflow](Testing-Debugging-And-Release-Workflow#branch-only-feature-smoke-pack) and [BuyMenu/EASA QoL branch audit](BuyMenu-EASA-QoL-Branch-Audit). No gameplay source changed. |
| `architecture-overview-tooling-route-prune` | Published / validated | Refreshed [Architecture overview](Architecture-Overview) so current docs head `adb9dbc8` is the visible source-continuity checkpoint for checked runtime/tooling anchors. Targeted source diffs from `4277a2ad` to `HEAD` over `description.ext`, `initJIPCompatible.sqf`, Common/Server/Client/Headless init and LoadoutManager `FileManager`/`SqfFileGenerator`/`ZipManager` paths return no source changes. Replaced repeated tooling branch detail with links to [Tools/build workflow](Tools-And-Build-Workflow), [Source fix propagation queue](Source-Fix-Propagation-Queue) and [Mission config/version include graph](Mission-Config-Version-Include-Graph). No gameplay source changed. |
| `server-runtime-gateway-route-prune` | Published / validated | Refreshed [Server gameplay runtime atlas](Server-Gameplay-Runtime-Atlas) and [Server runtime and operations](Server-Runtime-And-Operations) so current docs head `a6785f51` is the visible server-runtime source-continuity checkpoint. Checked `Init_Server.sqf`, server FSM/module paths, `Server_SideMessage.sqf`, `Server_OnHQKilled.sqf` and `serverFpsGUI.sqf` are unchanged from `4277a2ad`; repeated FPS, AI supply-truck, Patrols v2 and dormant-hook branch detail now routes through owner pages instead of being restated in the runtime-risk table. No gameplay source changed. |
| `sqf-atlas-compile-route-prune` | Published / validated | Pruned [SQF code atlas](SQF-Code-Atlas) back into a route-first compile map: PVF registry branch counts now route to [Public variable channel index](Public-Variable-Channel-Index#current-branch-registry-matrix), direct attack-wave proof to [Attack-wave authority](Attack-Wave-Authority-Playbook), `LogGameEnd` duplicate-bind status to [Server init bind cleanup](Server-Init-Bind-Cleanup), and disabled/deferred compile signals to their owner pages. Rechecked current docs head `34212d6a`; checked PVF/server/client/common-init and AI supply-truck source files are unchanged from `4277a2ad`. No gameplay source changed. |
| `networking-pv-gateway-prune-route` | Published / validated | Pruned [Networking and public variables](Networking-And-Public-Variables) back into a route-first architecture gateway while preserving its client-bound PVF runtime matrix. Removed copied PVF command inventories, duplicated dispatch-wrapper explanation, residual handler proof and attack-wave source proof now owned by [Public variable channel index](Public-Variable-Channel-Index), [PVF dispatch implementation](PVF-Dispatch-Implementation-Playbook), [Server authority migration map](Server-Authority-Migration-Map) and [Attack-wave authority](Attack-Wave-Authority-Playbook). Rechecked current docs head `aa1a4b76`; checked network source files are unchanged from `4277a2ad`. No gameplay source changed. |
