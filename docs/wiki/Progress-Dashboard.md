...
| Lane | Status | Output |
| --- | --- | --- |
| `server-runtime-branch-scope-route` | Published / validated | Refreshed [Server gameplay runtime atlas](Server-Gameplay-Runtime-Atlas) and [Server runtime and operations](Server-Runtime-And-Operations) as branch-aware runtime gateways: added a top server-loop route table, separated docs-checkout `6afcc58e` anchors from `origin/master` `cf2a6d6a` startup truth, and routed AI supply-truck, Patrols v2 and FPS publisher branch deltas to their owner pages. No gameplay source changed. |
