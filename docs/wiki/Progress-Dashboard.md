...
| `empty-supply-truck-current-b741-b742-refresh-2026-06-23` | Published / validated | Refreshed empty supply-truck 24-hour cleanup evidence through [Marker cleanup/restoration](Marker-Cleanup-Restoration-Systems-Atlas#empty-supply-truck-branch-matrix), [Feature status](Feature-Status-Register), [Source Fix queue](Source-Fix-Propagation-Queue) and machine rows. Docs/source `HEAD@499c2543` is unchanged from `b4e10b5f` for checked collector/handler paths. Current stable/B74.1 `origin/master@f8a76de34` equals `origin/claude/b74.1-aicom@f8a76de34`; B74.2 `origin/claude/b74.2-aicom@d472da6a` has no checked empty-vehicle collector/handler delta. Current stable/B74.1/B74.2/B69/B74 still drain `WF_Logic` `emptyVehicles` through `WFBE_SE_FNC_HandleEmptyVehicle` and set supply-truck `_delay = 86400` in both maintained roots. No gameplay source changed. |
