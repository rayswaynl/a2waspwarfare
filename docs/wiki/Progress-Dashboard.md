...
| Lane | Status | Output |
| --- | --- | --- |
| `wiki-mirror-current-wiki-sync` | Published / validated | Reconciled the repo mirror against the current human wiki baseline `origin/master@3745638`: initial SHA256 parity scan found `35` top-level `docs/wiki` drift candidates, and the mechanical mirror copy updates the repo mirror without gameplay source edits. Validation passed for JSON/JSONL parse, wiki validation, `git diff --check` and full wiki/docs normalized-content parity (`diffCount=0`). |
| `client-ui-atlas-current-source-scope-route-prune` | Published / validated | Refreshed [Client UI systems atlas](Client-UI-Systems-Atlas) so current docs head `68bd4dc5` is the visible UI/Rsc source-continuity checkpoint. Targeted diffs from `a03a1fb5` and `b5219d47` to `HEAD` over checked Chernarus and maintained Vanilla `description.ext`, `Rsc`, `Client/GUI`, `Init_Client.sqf`, `Client_UpdateRHUD.sqf`, client action/marker FSMs and core UI helper paths return no source changes; branch refs still resolve to stable `cf2a6d6a`, release `a96fdda2`, Miksuu `b8389e74`, perf `0076040f`, `wf-menu-ops-console` `0767c0b5` and BuyMenu/EASA QoL `a66d4691`. Kept the atlas route-first and preserved detailed UI branch behavior on owner pages. No gameplay source changed. |
| `server-runtime-operations-current-source-scope-route-prune` | Published / validated | Refreshed [Server runtime and operations](Server-Runtime-And-Operations) so current docs head `f74b3822` is the visible server-runtime source-continuity checkpoint. Targeted diffs from `a6785f51` and `4277a2ad` to `HEAD` over checked Chernarus and maintained Vanilla `Init_Server.sqf`, server FSM/module paths, `Server_SideMessage.sqf`, `Server_OnHQKilled.sqf` and server-FPS publisher paths return no source changes; branch refs still resolve to stable `cf2a6d6a`, release `a96fdda2`, Miksuu `b8389e74` and perf `0076040f`. Kept the gateway compact and routed branch-sensitive AI supply-truck, Patrols v2 and FPS publisher behavior to [Server gameplay runtime atlas](Server-Gameplay-Runtime-Atlas#branch-scope-for-source-anchors). No gameplay source changed. |
| `architecture-overview-current-source-scope-route-prune` | Published / validated | Refreshed [Architecture overview](Architecture-Overview) so current docs head `2de2de92` is the visible top-level architecture source-continuity checkpoint. Targeted diffs from `adb9dbc8` and `4277a2ad` to `HEAD` over the cited runtime, bootstrap and LoadoutManager tooling paths return no source changes; branch refs still resolve to stable `cf2a6d6a`, release `a96fdda2`, Miksuu `b8389e74` and perf `0076040f`. Updated Current Branch Scope to keep the overview as an orientation gateway and route branch behavior, line drift and smoke gates to subsystem owner pages. No gameplay source changed. |
| `sqf-atlas-current-source-scope-route-prune` | Published / validated | Refreshed [SQF code atlas](SQF-Code-Atlas) so current docs head `94b09c73` is the visible SQF compile/source-continuity checkpoint while the original `04a60e43` compile-count snapshot stays provenance. The count command still returns `738` total `preprocessFile` references (`460` `preprocessFileLineNumbers`, `278` plain, `22` commented), and targeted diffs from `34212d6a` plus `4277a2ad` to `HEAD` over checked Chernarus and maintained Vanilla SQF/PVF/init paths return no source changes. Routed repeated PVF no-drift proof back to the compile summary and canonical [Public variable channel index](Public-Variable-Channel-Index), [Networking and public variables](Networking-And-Public-Variables) and [PVF dispatch implementation](PVF-Dispatch-Implementation-Playbook) pages, with explicit dispatcher line anchors added locally. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Active | `deploy-aicom-experital-merge-risk-audit` | Auditing `deploy/2026-06-12-aicom-experital` against `origin/master`, release/Experital refs and PR #34; first output is a compact GitHub status plus source-backed findings/proposed-change notices. |
...
| `deploy-aicom-experital-merge-risk-audit` | Codex | Active / reporting | Audit `deploy/2026-06-12-aicom-experital` against `origin/master`, `origin/release/2026-06-feature-bundle`, `origin/experital` and PR #34 for Arma 2 OA SQF correctness, merge risk, code quality and efficiency. Use PR #30 for broad GitHub status unless the branch owner chooses a narrower target; Claude leads implementation decisions. |
