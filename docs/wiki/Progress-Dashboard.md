...
| `hardening-backlog-wikirefs-validator-cleanup-2026-06-23` | Published / validated | Normalized legacy `agent-hardening-backlog.jsonl` `wikiRefs` so every referenced wiki target resolves as a page: stripped section anchors from page refs, removed the nonexistent `Agent-Hardening-Backlog` page ref, and added missing `wikiRefs` from existing canonical docs/wiki/evidence fields. Tightened two Arma 2 OA compatibility/reference lines so intentional modern/A3-only terms carry same-line guardrail wording. `ValidateWiki.ps1 -WikiPath <live checkout> -SkipGitDiffCheck` now passes through machine references, exact machine references and the Arma 2 OA compatibility guardrail scan. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready | `documentation-finisher-loop` | Latest published lane: `trello-upgrade-hqcost-info-branch-intel-2026-06-23`. Continue bounded source-backed docs passes from this dashboard, Feature Status, Source Fix queue, Upstream mining ledger and pruning ledger. |
