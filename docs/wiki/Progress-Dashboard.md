...
| Lane | Status | Output |
| --- | --- | --- |
| `gear-easa-branch-scope-route` | Published / validated | Refreshed [Gear/loadout/EASA atlas](Gear-Loadout-And-EASA-Atlas) as the gear/service gateway: added current branch scope for docs checkout `8b71e2a1`, stable `origin/master` `cf2a6d6a`, Miksuu `b8389e74`, `perf/quick-wins` `0076040f`, release `a96fdda2` and `feat/buymenu-easa-qol` `a66d4691`; confirmed profile/template save/import and creation/display debt remains open across checked roots; corrected [Vehicle cargo equip loop bounds](Vehicle-Cargo-Equip-Loop-Bounds) to show stable/release fixed both maintained roots while perf is Chernarus-only; and refreshed [Service menu affordability guards](Service-Menu-Affordability-Guards), [Feature status](Feature-Status-Register) and [Source fix propagation queue](Source-Fix-Propagation-Queue) so partial stable/release service guards are not overstated as full authority hardening. No gameplay source changed. |
| `victory-endgame-branch-scope-route` | Published / validated | Refreshed [Victory/endgame atlas](Victory-And-Endgame-Atlas) as the match-end gateway: added a top route table and current branch scope for docs checkout `2f2132f8`, stable `origin/master` `cf2a6d6a`, Miksuu `b8389e74`, `perf/quick-wins` `0076040f` and release `a96fdda2`; confirmed the default victory winner/guard/no-break defect remains open across checked Chernarus and maintained Vanilla roots; clarified that `Server/PVFunctions/LogGameEnd.sqf` is present only on docs/Miksuu/perf and absent on stable/release; and aligned [Server init bind cleanup](Server-Init-Bind-Cleanup), [Feature status](Feature-Status-Register), [Pending owner decisions](Pending-Owner-Decisions), [SQF code atlas](SQF-Code-Atlas) and [Dead/stale code](Dead-Code-And-Stale-Code-Register) to the branch split. No gameplay source changed. |
| `gameplay-systems-atlas-route-prune` | Published / validated | Refreshed [Gameplay systems atlas](Gameplay-Systems-Atlas) as the broad gameplay gateway: added a top route table and current branch scope for docs checkout `ca40f202`, stable `origin/master` `cf2a6d6a`, Miksuu `b8389e74`, `perf/quick-wins` `0076040f` and release `a96fdda2`; routed town, economy, commander, upgrades, construction, factory, AI/HC, supports and victory detail to owner pages; corrected the stale commander DR-15 universal-defect wording to branch-split evidence; and replaced older HQ score/idempotency line refs with owner-page routing. No gameplay source changed. |
| `modules-atlas-reaktiv-branch-scope-route` | Published / validated | Refreshed [Modules atlas](Modules-Atlas) as the module gateway: replaced repeated covered-elsewhere prose with a top route table and owner-page routes for ICBM/EASA/AntiStack/supply/MASH/UAV/serverFPS/AFK; refreshed the Reaktiv branch matrix for docs checkout `20a19676`, stable `origin/master` `cf2a6d6a`, Miksuu `b8389e74`, `perf/quick-wins` `0076040f` and release `a96fdda2`; confirmed docs/Miksuu/perf still carry unreachable maintained-root Reaktiv files while stable/release have no maintained-root hits, and all checked refs still carry stale modded Napf/Eden/Lingor copies; aligned [Dead/stale code](Dead-Code-And-Stale-Code-Register). No gameplay source changed. |
| `marker-cleanup-restoration-branch-scope-route` | Published / validated | Refreshed [Marker cleanup/restoration](Marker-Cleanup-Restoration-Systems-Atlas) as the marker/lifecycle cleanup gateway: added a top route table and current branch scope for docs checkout `634a907b`, stable `origin/master` `cf2a6d6a`, Miksuu `b8389e74`, `perf/quick-wins` `0076040f` and release `a96fdda2`; updated cleanup startup anchors to `Init_Server.sqf:528-560`; refreshed the empty supply-truck matrix so all checked Chernarus and maintained Vanilla roots still drain `emptyVehicles` through `emptyvehiclescollector.sqf:9,15-19,30` and hard-code supply-truck `_delay = 86400` at `Server_HandleEmptyVehicle.sqf:23`; and aligned [Feature status](Feature-Status-Register) plus [Source fix propagation queue](Source-Fix-Propagation-Queue). No gameplay source changed. |
