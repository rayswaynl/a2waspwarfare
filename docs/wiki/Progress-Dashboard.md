...
| `function-module-index-source-scope-route` | Published / validated | Refreshed [Function and module index](Function-And-Module-Index) as a SQF source-family gateway for docs checkout `1f0b9018`: added source-scope and route tables, corrected commander reassignment wording to branch-split helper evidence, routed Reaktiv/MASH/paratrooper status to owner pages, and aligned [Respawn/death lifecycle](Respawn-And-Death-Lifecycle-Atlas) plus [Feature status](Feature-Status-Register) for current MASH and paratrooper branch scope without changing gameplay source. |
