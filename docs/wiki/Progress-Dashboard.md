...
| `patrols-v2-current-master-route` | Published / validated | Corrected current-master patrol status after `origin/master` advanced to `cf2a6d6a`. Current Chernarus and maintained Vanilla now retire the old `server_town_ai` random-town patrol launch, add `server_side_patrols.sqf` plus `Common_RunSidePatrol.sqf`, use `&&` at `server_patrols.sqf:26`, and maintain friendly patrol markers through `WFBE_ACTIVE_PATROLS`. [Towns/camps/capture](Towns-Camps-And-Capture-Atlas#patrols-v2-side-upgrade-path), [AI runtime](AI-Runtime-HC-Loop-Map), [Feature status](Feature-Status-Register) and [Source fix propagation queue](Source-Fix-Propagation-Queue) now route old DR-57/AI1 wording as historical `89ae9dad` branch evidence and current Patrols v2 as source-present / smoke-pending. No gameplay source changed. |
