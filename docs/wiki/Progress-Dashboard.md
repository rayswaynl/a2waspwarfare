...
| Lane | Status | Output |
| --- | --- | --- |
| `clickabletext-soundpush-b742-current-head-followup-2026-06-24` | Published / validated | Refreshed clickable-text soundPush B74.2 current-head rows through [Client UI systems](Client-UI-Systems-Atlas#clickable-text-soundpush-branch-matrix), [Feature status](Feature-Status-Register), [Source Fix queue](Source-Fix-Propagation-Queue) and [Dead/stale code](Dead-Code-And-Stale-Code-Register). Current B74.2 is `origin/claude/b74.2-aicom@21b62b04`; checked `d472da6a..21b62b04`, `origin/master..origin/claude/b74.2-aicom` and B69..B74 resource/dialog/title deltas are empty. Current stable/B74.1/B74.2/B69/B74 and GUER branch hits carry valid `RscClickableText.soundPush[] = {"", 0.2, 1};` at `Rsc/Ressources.hpp:556` in both maintained roots; docs/source `HEAD@14d74f9d2`, Miksuu, perf, historical release and UI theme refs remain old-shape. No gameplay source changed. |
| `rhud-endgame-title-handle-b742-current-head-followup-2026-06-23` | Published / validated | Refreshed RHUD/endgame shared title display handle rows through [UI IDD collision repair](UI-IDD-Collision-Repair#title-display-handle-branch-matrix), [Client UI systems](Client-UI-Systems-Atlas#hud-display-ownership), [Feature status](Feature-Status-Register) and [Source Fix queue](Source-Fix-Propagation-Queue). Current B74.2 is `origin/claude/b74.2-aicom@21b62b04`; checked `d472da6a..21b62b04` title-handle deltas are empty, while `origin/master..origin/claude/b74.2-aicom` only changes source-Chernarus RHUD AI-cap display text among checked files. All checked refs still share `currentCutDisplay` between `OptionsAvailable`/RHUD/action icons and `EndOfGameStats`; the UI lifecycle bug remains patch-ready. No gameplay source changed. |
| `service-easa-b742-current-head-followup-2026-06-23` | Published / validated | Refreshed service/EASA B74.2 current-head rows through [Service menu affordability guards](Service-Menu-Affordability-Guards#current-branch-matrix), [Gear/loadout/EASA atlas](Gear-Loadout-And-EASA-Atlas#current-branch-scope), [Feature status](Feature-Status-Register) and [Source Fix queue](Source-Fix-Propagation-Queue). Current B74.2 is `origin/claude/b74.2-aicom@21b62b04`; checked `d472da6a..21b62b04` and `origin/master..origin/claude/b74.2-aicom` diffs are empty for Chernarus/Vanilla service menu, EASA menu/module and stringtable paths. Stable-shaped refs partially guard service rearm/refuel and fix Chernarus exact-funds EASA only; repair/heal, action-time context, maintained Vanilla exact-funds and client/server authority remain open. Trello service ammo and EASA category branches are UI/wiring evidence only. No gameplay source changed. |
| `loadoutmanager-b742-current-head-followup-2026-06-23` | Published / validated | Refreshed LoadoutManager root-discovery, skip-zip and generated `version.sqf` rows through [Source Fix queue](Source-Fix-Propagation-Queue#rule), [Tools/build workflow](Tools-And-Build-Workflow#loadoutmanager), [Mission config/version graph](Mission-Config-Version-Include-Graph), [release readiness](agent-release-readiness.json) and pruning/machine rows. Current B74.2 is `origin/claude/b74.2-aicom@21b62b04`; checked `d472da6a..21b62b04`, `origin/master..origin/claude/b74.2-aicom` and B69..B74 deltas are empty for LoadoutManager root discovery, packaging, version generation, live/template `version.sqf` presence and Chernarus/Vanilla include consumers. Docs/source `HEAD@101930f47` implements marker-root discovery and `A2WASP_SKIP_ZIP`; current stable/B74-shaped refs keep marker-root discovery but no skip-zip support. Live generated `version.sqf` remains absent in a clean checkout; tracked Chernarus `version.sqf.template` is reference-only and does not satisfy maintained Vanilla. No gameplay source changed. |
| `commander-vote-b742-current-head-followup-2026-06-23` | Published / validated | Refreshed commander vote/reassignment rows through [Commander vote/reassignment](Commander-Vote-And-Reassignment-Playbook#current-branch-scope), [Commander reassignment call shape](Commander-Reassignment-Call-Shape#current-branch-matrix), [Commander/HQ lifecycle](Commander-HQ-Lifecycle-Atlas), [Public variable channel index](Public-Variable-Channel-Index), [Feature status](Feature-Status-Register) and [Source Fix queue](Source-Fix-Propagation-Queue). Current B74.2 is `origin/claude/b74.2-aicom@21b62b04`; checked `d472da6a..21b62b04` and `origin/master..origin/claude/b74.2-aicom` diffs are empty for source Chernarus and maintained Vanilla vote/reassignment paths. Stable/B74-shaped roots keep DR-47 vote comparison at `Server_VoteForCommander.sqf:43` and DR-15 helper unpacking at `Server_AssignNewCommander.sqf:4-5`; docs/source/full modded forks, GUI preview/visible-name targeting, duplicate notification and requester authority remain open. `origin/claude/trello-vote-para-guards@92101abe` is branch-only vote-worker tie-break context. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready / documentation-finisher loop | `documentation-finisher-loop` | ClickableText soundPush B74.2 current-head follow-up is published; pick the next bounded source-backed docs lane from the queue/status pages before editing. |
