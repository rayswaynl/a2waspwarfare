...
| `attack-wave-authority-branch-route` | Published / validated | Hardened [Attack-wave authority playbook](Attack-Wave-Authority-Playbook#branch--root-matrix) with a branch/root matrix: current source Chernarus, maintained Vanilla, stable `origin/master`, Miksuu upstream, `perf/quick-wins`, release, AI commander branch and historical Miksuu `AttackWave` branch all keep direct `ATTACK_WAVE_INIT` supply/side trust plus trusted `ATTACK_WAVE_DETAILS`. Condensed [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine rows. No gameplay source changed. |
| `legacy-compiled-aliases-no-static-callers` | Published / validated | Folded Aquinas' read-only compiled-helper scout into [Function and module index](Function-And-Module-Index#legacy-compiled-aliases-with-no-static-callers). Current Chernarus and maintained Vanilla still compile old globals including `AITownResitance`, `EquipLoadout`, `GetGroupFromConfig`, `GetSafePlace`, `GetUnitsBelowHeight`, `UseStationaryDefense`, `ReplaceArray`, `FireArtillery`, `GetSideUpgrades` and client `HandlePVF`; a whole-symbol scan found zero active static callers after excluding compile lines, comments and the helper file itself. The docs explicitly keep shared helpers live where newer `WFBE_*` names call the same files, such as `WFBE_CO_FNC_FireArtillery`, `WFBE_CO_FNC_GetSideUpgrades` and `WFBE_CL_FNC_HandlePVF`. No gameplay source changed. |
