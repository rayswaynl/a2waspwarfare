...
| Lane | Status | Output |
| --- | --- | --- |
| `support-specials-route-upgrade-sync-refresh` | Published / validated | Refreshed [Support specials and tactical modules](Support-Specials-And-Tactical-Modules-Atlas) as the support/tactical gateway: added a top routing table, branch-scope note and current-head `upgrade-sync` matrix. The matrix now distinguishes docs checkout `295cc9d5`, stable `origin/master` `cf2a6d6a`, Miksuu `b8389e74`, `perf/quick-wins` `0076040f` and release `a96fdda2`; all checked roots still keep the mixed `_args` / `_this` parser in `Server_HandleSpecial.sqf:67-73`, with caller line drift in `GUI_UpgradeMenu.sqf`. No gameplay source changed. |
