...
| `requestspecial-upgrade-sync-branch-route` | Published / validated | Refreshed [Support specials](Support-Specials-And-Tactical-Modules-Atlas#upgrade-sync-branch-matrix) with a branch/root matrix: current source Chernarus, maintained Vanilla, stable `origin/master`, Miksuu upstream, `perf/quick-wins` and release snapshot `d482c742` all keep the same mixed `_args` / `_this` parser for `upgrade-sync`; because `_args = _this`, this is consistency cleanup debt rather than a confirmed runtime break. Condensed [Upgrades](Upgrades-And-Research-Atlas), [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine records. No gameplay source changed. |
...
| `fast-travel-fee-branch-route` | Published / validated | Refreshed [Client UI systems](Client-UI-Systems-Atlas#tactical-fast-travel-fee-branch-matrix) with a branch/root matrix: current source Chernarus, maintained Vanilla, stable `origin/master`, Miksuu upstream, `perf/quick-wins` and release all keep the Tactical fee-mode TODO, hidden unaffordable destinations, marker-only price text and local debit. Condensed [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and support routing. No gameplay source changed. |
...
| `empty-supply-truck-cleanup-branch-route` | Published / validated | Refreshed [Marker cleanup/restoration](Marker-Cleanup-Restoration-Systems-Atlas#empty-supply-truck-branch-matrix) with a branch/root matrix: current source Chernarus, maintained Vanilla, stable `origin/master`, Miksuu upstream, `perf/quick-wins` and release all keep `Server_HandleEmptyVehicle.sqf` hard-coding a 24-hour empty timeout for supply trucks while `emptyvehiclescollector.sqf` only drains the queue into the handler. Condensed [Feature status](Feature-Status-Register), [Source fix propagation queue](Source-Fix-Propagation-Queue) and machine records. No gameplay source changed. |
