...
| Lane | Status | Output |
| --- | --- | --- |
| `wiki-mirror-current-wiki-sync` | Published / validated | Reconciled the repo mirror against the current human wiki baseline `origin/master@3745638`: initial SHA256 parity scan found `35` top-level `docs/wiki` drift candidates, and the mechanical mirror copy updates the repo mirror without gameplay source edits. Validation passed for JSON/JSONL parse, wiki validation, `git diff --check` and full wiki/docs parity (`diffCount=0`). |
