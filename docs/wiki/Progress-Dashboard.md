...
| `salvage-payout-b742-current-head-followup-2026-06-24` | Published / validated | Refreshed [Construction/CoIn systems](Construction-And-CoIn-Systems-Atlas#salvage-branch-matrix), [Feature status](Feature-Status-Register) and [Source Fix queue](Source-Fix-Propagation-Queue) from stale B74.2 `origin/claude/b74.2-aicom@d472da6a` wording to current `origin/claude/b74.2-aicom@21b62b04`. Docs/source `HEAD@408479a914c7` is unchanged from `795cdbbf0` for checked salvage paths. Current B74.2 still keeps lowercase `ChangePlayerfunds`, the `updatesalvage.sqf:10` `||` loop and client-local delete/reward shape. Checked `d472da6a..21b62b04` only touches source-Chernarus `Init_Client.sqf` client-init/tint/ACK/intro-music code among checked salvage-adjacent paths. No gameplay source changed. |
...
| --- | --- | --- | --- |
| Codex | Ready / documentation finisher loop | `documentation-finisher-loop` | Current commander reassignment context cleanup is published; pick the next bounded source-backed lane after reading the coordination surfaces and avoiding active owners. |
