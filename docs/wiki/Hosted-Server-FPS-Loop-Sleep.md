...
Checked 2026-06-22 after fetching origin and Miksuu `master`. Docs HEAD `d0161083` is source-unchanged from `a27086cd` for the checked hosted-FPS paths and keeps the branch-local two-publisher DR-19 fix in Chernarus and maintained Vanilla: both `serverFpsGUI.sqf:1` and `monitorServerFPS.sqf:1` exit before their loops on `!isDedicated`. Current stable `origin/master@0139a346` and historical release commit `a96fdda2` use a different fixed shape: one guarded `serverFpsGUI.sqf` publisher and no maintained-root `Server/Module/serverFPS/monitorServerFPS.sqf` file. Miksuu `master@b8389e74` and `origin/perf/quick-wins@0076040f` still keep the old two unguarded publisher loops in checked maintained roots. Current origin exposes no `release/*` heads, and no `origin/dev/july-update-hosted-server-fps-loop-fix` head was found on 2026-06-22. Arma 2 OA smoke is still pending.

## Current Branch Scope

| Ref / root scope | FPS publisher shape | Development meaning |
| --- | --- | --- |
| Docs HEAD `d0161083` Chernarus and maintained Vanilla | Source unchanged from `a27086cd`: `Server/GUI/serverFpsGUI.sqf:1` and `Server/Module/serverFPS/monitorServerFPS.sqf:1` both exit on `!isDedicated`; `Init_Server.sqf:578,595` still starts both scripts. `serverFpsGUI.sqf:6-7` publishes `SERVER_FPS_GUI`; `monitorServerFPS.sqf:5-6` publishes `WFBE_VAR_SERVER_FPS`; `Client_UpdateRHUD.sqf:113` reads `SERVER_FPS_GUI`. | Branch-local source carries the low-risk two-publisher guard. Smoke dedicated publish plus hosted/listen no-spin before release-ready wording. |
| Current stable `origin/master@0139a346` | Both maintained roots guard `serverFpsGUI.sqf` at `:4`, publish `SERVER_FPS_GUI` at `:9-10`, and `Client_UpdateRHUD.sqf:113,130` reads that variable; `Init_Server.sqf:769` starts the publisher and `:815-817` documents removal of `monitorServerFPS.sqf`, which is absent. | Stable is no longer old-loop-shaped. Smoke the single-publisher target branch rather than porting the docs-branch two-publisher shape blindly. The planned July dev branch must be created or restored before code work because no current remote head was found. |
...
- Current stable `origin/master@0139a346` has the guarded `serverFpsGUI.sqf` shape at `:4-12`, starts it from `Init_Server.sqf:769`, documents redundant monitor removal at `:815-817` and lacks the maintained-root `Server/Module/serverFPS/monitorServerFPS.sqf` file in both maintained roots. Historical `a96fdda2` has the same single-publisher shape with older init anchors `:579` and `:595-597`.
