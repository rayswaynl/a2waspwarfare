...
`hosted-server-fps-loop-sleep` is source/Vanilla patched and smoke pending as of 2026-06-02. This is the implementation pass for DR-19 / backlog id `server-fps-hosted-loop-sleep`.
...
- Chernarus and Vanilla Takistan `serverFpsGUI.sqf` each have one early `!isDedicated` exit, one loop, one `sleep 8` and no inner `if (isDedicated)` branch.
- Chernarus and Vanilla Takistan `monitorServerFPS.sqf` each have one early `!isDedicated` exit, one loop, one `sleep 8` and no inner `if (isDedicated)` branch.
