...
Checked 2026-06-24 for the `upgrade-sync` tuple lane against docs/source `HEAD@373e21c1274c` unchanged from `a7f383b6`, current stable/B74.1 `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`, current B74.2 `origin/claude/b74.2-aicom@21b62b04`, current B69 `origin/claude/b69@8d465fce`, adjacent B74 `origin/claude/b74-aicom-spend@b23f557f`, Miksuu `b8389e748243`, `origin/perf/quick-wins@0076040f`, historical release commit `a96fdda2` and historical upgrade-queue commit `b061c905`. Targeted diffs `a7f383b6..HEAD`, `d472da6a..origin/claude/b74.2-aicom`, `origin/master..origin/claude/b74.2-aicom` and `origin/claude/b69..origin/claude/b74-aicom-spend` are empty for the six maintained-root handler/caller/process paths. Current origin exposes no live `release/*` head for the checked pattern; `trello-upgrade-hqcost-info` has no scoped diff, and `trello-upgrade-icons` only line-drifts the GUI icon work.
...
### PR #80 Vote/Paratrooper Guard Branch Intel

Checked 2026-06-24 after fetch. Open draft PR #80 / `origin/claude/trello-vote-para-guards@92101abe9f7b83bd7f658d03e4160d5f030aa3c7` is one clean commit on merge-base/current stable `origin/master@f8a76de34`. Payload is 10 maintained-root files / +148 / -14 across source Chernarus plus maintained Vanilla Takistan, and `git diff --check origin/master..origin/claude/trello-vote-para-guards` is clean. The scoped diff has no `Modded_Missions` or `Tools` payload.

For paratroopers, both maintained roots add a Tactical client gate at `GUI_Menu_Tactical.sqf:373-393`: when `WFBE_C_PARA_RESPECT_CAP` is enabled, the branch computes the same player AI cap shape used by Buy Units, checks live group size with `GetLiveUnits`, and blocks with `STR_WF_INFO_Para_MaxGroup` before fee debit, cooldown stamp or `RequestSpecial ["Paratroops", ...]` send. The server backstop is `Support_Paratroopers.sqf:40,53-54`: only player-led teams are checked, and requests that would make `count units _playerTeam + count _units` exceed the cap exit with an RPT log. The opt-out/default is `WFBE_C_PARA_RESPECT_CAP = 1` at Chernarus `Init_CommonConstants.sqf:664` and Vanilla `:466`; the localized hint key is `stringtable.xml:1118`.

Treat this as branch-only support/tactical behavior. Current stable, current B74.2, B69, adjacent B74, Miksuu and perf refs have no `WFBE_C_PARA_RESPECT_CAP` or `STR_WF_INFO_Para_MaxGroup` hits in the maintained roots. Before promotion, smoke cap-at and below-cap Tactical calls, no fee/cooldown/request on client block, server backstop rejection from a forged/desynced request, `WFBE_C_PARA_RESPECT_CAP = 0` legacy behavior, AI commander wildcard paradrops staying uncapped, and maintained Vanilla parity. Commander tie-break behavior from the same PR is owned by [Commander vote/reassignment](Commander-Vote-And-Reassignment-Playbook#current-branch-scope).

### Trello Artillery UI Branch Intel

Checked 2026-06-24 after PR #89/B751b. Open draft PR #74 / `origin/claude/trello-arty-cooldown-hud@471fc0580fd6f641b2455c7eb12e5af21baafe2a` is GitHub-clean (`mergeable=true` / `clean`), but its recorded base SHA and local merge-base are still pre-B745/B751b `f8a76de349da6f8b871d079c828436c10afb221c`. Current master is `origin/master@f39665d7950ddf14cb0bbfacb7ee1e40121e93b4`, and local `origin/master...origin/claude/trello-arty-cooldown-hud` counts are `80 1`. Treat the base-relative payload as branch evidence, not current stable or release-ready behavior.

PR #74's base-relative diff remains four maintained-root files / +134 / -8, clean under `git diff --check f8a76de34..origin/claude/trello-arty-cooldown-hud`, with no `Modded_Missions`, `Tools` or `Extension` payload. Current master still has Tactical-menu cooldown state at `GUI_Menu_Tactical.sqf:562-563,574` and initializes `fireMissionTime = -1000` at `Init_Client.sqf:431`, but it has no `RUBHUD_Arty` / `_RHUDUpdateArty` hits in the maintained roots. Rows below without explicit 2026-06-24 wording preserve 2026-06-23 `f8a76de34` branch-intake proof until a separate post-B751b owner pass rechecks them.

| Branch | Evidence | Status |
| --- | --- | --- |
| `origin/claude/trello-arty-cooldown-hud@471fc0580fd6` / draft PR #74 | Open draft PR #74 is clean but stale-base after B751b: base SHA / merge-base `f8a76de34`, current master `f39665d79`, local counts `80 1`. Base-relative payload is four maintained-root files / +134 / -8 and `git diff --check f8a76de34..origin/claude/trello-arty-cooldown-hud` is clean. In source Chernarus plus maintained Vanilla, `Client_UpdateRHUD.sqf:20,27` extends the RHUD helper/cache surface with `_RHUDUpdateArty` and `1372/1373`, `:256-288` reads global `fireMissionTime` and computes available/countdown text, and `:452` calls it during the RHUD refresh. `Rsc/Titles.hpp:177` adds `RUBHUD_Arty` / `RUBHUD_Arty_Value` to `OptionsAvailable`, while `:478-489` defines those controls. | Branch-only clean-but-stale-base UI candidate. It surfaces the existing local artillery timeout state on RHUD; it is not a server-authority change. Rebase/recheck against post-B751b master, then run visual/client smoke in source Chernarus plus maintained Vanilla before stable wording. |
| `origin/claude/trello-arty-to-gunner@2dc3e7403399` | Payload is six files / +217 / -8 and `git diff --check` clean against the 2026-06-23 `f8a76de34` branch base. In source Chernarus plus maintained Vanilla, `GUI_Menu_Tactical.sqf:571-626` adds `MenuAction == 50`, flattens `WFBE_%1_ARTILLERY_CLASSNAMES`, selects player-group artillery with empty driver/gunner seats, and uses existing group AI with `moveInDriver` / `moveInGunner`. `Rsc/Dialogs.hpp:2364-2371` adds button idc `17040`; `stringtable.xml:3192-3213` adds the crew-artillery label, tooltip and result hints. | Branch-only Tactical UI candidate. It seats existing dismounted group AI and spawns no new units. Smoke must cover player-owned artillery, empty/no-AI/no-artillery cases, locality, and maintained Vanilla parity before promotion. |
| `origin/claude/trello-artillery-ux@c4459d4312ef` / draft PR #79 | Payload is eight maintained-root files / +100 / -20 and `git diff --check` clean against the 2026-06-23 `f8a76de34` branch base. In source Chernarus plus maintained Vanilla, `GUI_Menu_Tactical.sqf:42-57` reads display names, min/max arrays and `WFBE_C_ARTILLERY` to append effective `min-max m` range text to each artillery row. `Client_RequestFireMission.sqf:31-38` keeps global `WF_createMarker` creation but sends `RequestSpecial ["ArtyMarkerCleanup", ...]` instead of caller-local deletion; `:58-60` passes `_gunCount` as `%3` to the localized warning. `Server_HandleSpecial.sqf` adds server-side delayed `deleteMarker` cleanup at Chernarus `:144-160` / Vanilla `:134-150`; `stringtable.xml` updates `STR_WF_INFO_Arty_called_message` for the third placeholder at Chernarus `:9304-9311` / Vanilla `:9304-9310`. | Branch-only artillery UX/cleanup candidate. It is not a fire-mission authority fix and the new `RequestSpecial` cleanup tag should get marker-name / forged-payload review before promotion. Smoke per-type range text, gun-count messaging, caller disconnect/leave cleanup, JIP marker visibility and maintained Vanilla parity. |
...
| Current stable/B74.1 `origin/master@f8a76de34` / `origin/claude/b74.1-aicom@f8a76de34`; current B74.2 `origin/claude/b74.2-aicom@21b62b04` | Both roots call from `GUI_UpgradeMenu.sqf:292`. Source Chernarus keeps `_args = _this` at `Server_HandleSpecial.sqf:3`, the case at `:77`, `_side = _args select 1` at `:79`, `_upgrade_id` / `_upgrade_level` from `_this select 2/3` at `:80-81`, and sync set at `:83`; maintained Vanilla keeps the same parser at `:3,67,69-73`. `Server_ProcessUpgrade.sqf:27,55` now publishes/clears `wfbe_upgrading_end_time`, while sync store/wait/release remain `:32,:35,:41`. `d472da6a..21b62b04` and `origin/master..origin/claude/b74.2-aicom` are empty for the checked handler/caller/process paths. | No current stable/B74.1/B74.2 parser rescue. Treat the tuple cleanup as still patch-ready, but do not roll it into the end-time UI work. |
...
| Paratroopers | Working/partial | `RequestSpecial -> HandleSpecial -> KAT_Paratroopers`; server creates transport/units and sends marker callback. Missing server-side fee/cooldown/upgrade validation. PR #80 adds a branch-only maintained-root AI-cap client gate plus player-team server backstop; it is not current stable behavior until smoke/promotion. |
...
| MASH | Branch-split / removed on current stable/B74-shaped roots | Current stable/B74.1 `origin/master@f8a76de34` and current B74.2 `origin/claude/b74.2-aicom@21b62b04` remove the officer MASH deploy skill action in maintained roots (`Skill_Apply.sqf:43`); no portable deploy path remains there. Old-shape docs/Miksuu/perf roots still carry local deploy plus orphaned marker relay/receiver stubs. The `WFBE_%1MASHES` config vars are commented out in all Core_Root/*.sqf files, and MASH as a buildable base defense structure in Core_Structures configs is a separate system. |
...
`Client/Module/Skill/*` owns officer MASH/supply actions on old-shape docs/Miksuu/perf roots. There, MASH deploy creates local tent state used by respawn lookup, while marker sync is stale: the server relay exists, the client receiver compile is commented, and no maintained Chernarus/Vanilla sender was found. Current stable/B74.1 `origin/master@f8a76de34`, current B74.2 `origin/claude/b74.2-aicom@21b62b04`, historical `a96fdda28087` and B69/B74-family refs remove the maintained-root deploy/module path and keep only `Skill_Apply.sqf:43` removal wording plus residues. Checked current-stable..B74.2 and previous-B74.2..current-B74.2 MASH path deltas are empty. Modded `eden`/`lingor` sender lines are drift, not maintained-marker proof. Use [Respawn/death lifecycle](Respawn-And-Death-Lifecycle-Atlas) for the branch-sensitive respawn split.
...
- Bomb altitude restriction is live when `WFBE_C_GAMEPLAY_BOMBS_ALTITUDE` is non-zero. `Rsc/Parameters.hpp:284-288` exposes the parameter, default 2000, and the B66 altitude check/delete block in `Common_HandleShootBombs.sqf:36-44` is now un-commented: it reads the firing unit's `getPos` altitude and, when `_limit > 0` and altitude is at/above the limit, vehicle-chats `STR_WF_MESSAGE_BombAltitudeRestriction` and deletes the projectile.
...
The main risks are locality and player-facing false positives: bomb distance and missile masking lean on local `cursorTarget`, the unit init path exits for non-local player scope, and bomb altitude reads the firing unit's `getPos` altitude on the local client. Treat future aircraft-balance work as runtime-test-heavy, not just config editing.
...
| Bomb altitude restriction is live | The B66 altitude guard in `Common_HandleShootBombs.sqf:36-44` is active when `WFBE_C_GAMEPLAY_BOMBS_ALTITUDE > 0`. Smoke at/above and below the limit, with the parameter disabled (0), and confirm no false positives from local `getPos` altitude before tightening defaults. |
