...
| 9 | **MV-22 Osprey heavy-lift event** | `MV22` | `Core_USMC` only -> dormant in player buy flow (USMC not live WEST). | **Implemented hook (Lane 44):** default-off `WFBE_C_MV22_REINFORCEMENT_FLAVOUR` lets the first WEST W19 Heliborne QRF swap its lift hull to `MV22`, reusing the W6/W19 air-insertion path. | **M** |
...
| `MV22` | Tiltrotor transport | USMC buy list + US aircraft pool; Lane 44 adds a default-off first-WEST-W19-QRF lift-hull hook, still not player-buyable. |
