...
| 4 | **Civilian boat traffic on the coast** | `Smallboat_1`, `Smallboat_2`, `Fishing_Boat`, `PBX`, `Zodiac` | All watercraft dormant (non-naval maps, CIV/USMC factions). | **Hook shipped default-off** via `WFBE_C_COASTAL_TRAFFIC_ENABLE`: ambient fishing boats plus one PBX near Chernarus coastal towns as neutral no-crew dressing, with simulation gated to player proximity. `Zodiac` remains reserved for future naval-map work. | **M** |
...
| `Smallboat_1`, `Smallboat_2`, `Fishing_Boat` | `Core_CIV.sqf` | CIV faction, never a player shop. Lane 42 now has a default-off Chernarus ambient traffic hook for a strict no-crew subset. |
