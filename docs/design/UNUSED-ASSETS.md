...
| 3 | **AN-2 "Smuggler Run" a-life encounter** | `An2_TK_EP1`, `An2_1_TK_CIV_EP1`, `An2_2_TK_CIV_EP1` | An2 is TKA-buyable + in RU/GUE pools + airfield list; the two **civilian An2 variants** are GUE-pool only and never spawn on CH/TK. | Lane 11 ships a default-off server-only `Server_An2SmugglerRun.sqf`: a rare low-flying cargo AN-2 crosses the map, then drops a marked crate and credits the killer side/team if shot down. Flip `WFBE_C_AN2_SMUGGLER_RUN` to 1 to test; live default remains inert. | **M** hook shipped / smoke pending |
...
| 8 | **Avenger HMMWV "SAM ambush"** | `HMMWV_Avenger_DES_EP1` | US-buyable but rarely bought; never spawned by AI air-defence (GUER air-def uses Igla soldiers, not Avengers). | Wildcard/event: enemy air incursion triggers a **crewed Avenger** to spawn and defend a town for a window (a WEST mirror of the GUER Igla air-def loop). Makes owning the air actually contested. | **M** |
...
| `HMMWV_Avenger_DES_EP1` | US buyable | never AI-spawned; SAM-ambush event (#8) |
