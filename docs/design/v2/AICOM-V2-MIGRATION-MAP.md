...
| Box `update-PublicStats.ps1` | Livehost `C:\WASP\` | `EMPTYGRP|v1|` (Gap G2 prefix mismatch FIXED on box 2026-08-03 — script now parses `EMPTYGRP\|v1\|` at line 756; verified against live RPT 2026-08-06), AICOMSTAT families for :8080 dashboard | Gap G2 parse fixed; note: `performance.groupHealth` is deliberately REDACTED from the public `stats.json` (script line ~2222) — per-side empty data is parsed but not published; re-adding the public gauge is an owner call, not a defect | Add `AICOM2|` parse pass for :8080 dashboard when the unified grammar ships. |
