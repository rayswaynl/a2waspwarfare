...
- Air-contact altitude is a DICE ROLL, not a fixed ceiling (owner rule 2026-07-06): each detection sweep in which enemy air is present inside the bubble, the town rolls an engagement ceiling uniformly between `AICOMV2_GDIR_AIR_CEILING_MIN_M` (default 100) and `AICOMV2_GDIR_AIR_CEILING_MAX_M` (default 600). Air at or below the rolled value is air contact that sweep; air above it is ignored that sweep. Consequences (intended): below 100 m is ALWAYS contact; above 600 m is NEVER contact; in between, pilots can never learn a guaranteed-safe altitude — a fast transit may slip through on a lucky roll, while a loitering aircraft re-rolls every sweep and accumulates risk toward certain activation. Once a town's AA tier is active, it stays active for the episode regardless of later rolls (rolls gate ACTIVATION, not engagement). The existing `WFBE_C_TOWNS_DETECTION_RANGE_AIR` constant is left untouched (its default is not changed; the builder may retire it in favour of the new dials with a mapping note).
...
T1 pure-core: an air-contact event yields exactly one `kind=aaPicket` order of at most 3 groups; a ground-contact event supersedes an active picket with a full materialization order (no double-spawn of the AA fraction); picket write-back conserves strength; air below `AICOMV2_GDIR_AIR_CEILING_MIN_M` always yields contact and air above `AICOMV2_GDIR_AIR_CEILING_MAX_M` always yields `noop`; between the bounds, outcomes follow an injected deterministic roll (the pure core takes the roll as an input — no engine RNG in T1); repeated sweeps with a loitering aircraft between the bounds activate within a bounded number of rolls.
