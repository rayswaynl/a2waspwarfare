...
| 10 | **SUPERSEDED — ICBM/SCUD TEL range is map-specific** (`CH 10350 m` / `TK 8240 m`) | The shipped map branch scales the cap by map width: CH preserves 10350 (`~67%` of CH width), while TK uses 8240 (`~64.4%`), preserving the map-fraction rationale and avoiding map-spanning TEL geometry on Takistan. | S (done) | `Init_CommonConstants.sqf` (`WFBE_C_ICBM_TEL_RANGE`) |
...
| ICBM/SCUD TEL range cap | `Init_CommonConstants.sqf` (`WFBE_C_ICBM_TEL_RANGE`) | 10350 | 8240 | **OK — superseded** by the shipped cmdcon42 Action #7 map branch; map-fraction rationale preserved (CH `~67%` vs TK `~64.4%` map width). |
