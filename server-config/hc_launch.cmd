...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference = 'Stop'; try { $hc1Token = [regex]::Escape('-name=HC-AI-Control-1'); @(Get-CimInstance -ClassName Win32_Process | Where-Object { $_.Name -eq 'ArmA2OA.exe' -and [string]$_.CommandLine -match ('(^|\s)' + $hc1Token + '(\s|$)') }) | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction Stop }; exit 0 } catch { exit 1 }"
