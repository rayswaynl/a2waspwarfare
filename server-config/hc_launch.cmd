...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference = 'Stop'; try { @(Get-CimInstance -ClassName Win32_Process | Where-Object { $_.Name -eq 'ArmA2OA.exe' -and [string]$_.CommandLine -like '*HC-AI-Control-1*' }) | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction Stop }; exit 0 } catch { exit 1 }"
