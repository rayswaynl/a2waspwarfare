...
2) 7-Zip not found. The tool now auto-detects 7-Zip (the `7za` env var, the standard `C:\Program Files\7-Zip` install, or PATH) and packs `_MISSIONS.7z` automatically. If none is found it logs a line and skips only the optional packing step (the CH->TK mirror still completes) — install 7-Zip or set the `7za` env var to enable easy-deploy packing.
