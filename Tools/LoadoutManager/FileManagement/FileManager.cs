...
public class FileManager
{
        ...
        List<string> _copyErrors)
    {
        ...
        foreach (var directory in Directory.GetDirectories(_source))
        {
            ...
            // Only when copying to the vanilla desert maps, which keep their own
            // map-specific Textures / Server\Config / Core_Artillery
            if (shouldSkipDirectory &&
                (_destination.Contains("co.takistan") || _destination.Contains("co.zargabad")))
            {
    ...
    private static bool ShouldSkipFile(string _fileName, bool _isModdedTerrain)
    {
        return (_fileName.EndsWith("mission.sqm", StringComparison.OrdinalIgnoreCase) ||
               ...
               _fileName.EndsWith("version.sqf.template", StringComparison.OrdinalIgnoreCase) ||
...
";
    ...
    // Returns the SQF block that injects the opt-in "[SEAD] Anti-radar guidance" EASA row into
    // EASA_Init.sqf for F35B and Su34 (owner ruling 2026-08-02, flag WFBE_C_SEAD_EASA_ROW default 0).
    // Called by BaseTerrain.cs via InsertGeneratedCodeInToAFile after WriteToFile writes EASA_Init,
    // same regen-safe pattern as GenerateGuerEasaKa137Block(): the marker pair is regenerated by
    // SqfFileGenerator.GenerateEndOfTheEasaFile() on every run, then this call re-fills it.
    //
    // The row carries an empty [[],[]] weapon/ammo pair by design: GUI_Menu_EASA.sqf special-cases
    // selection of this row (matched via WFBE_EASA_SEAD_ROW_F35B / WFBE_EASA_SEAD_ROW_SU34, the index
    // this block records) and routes it to WFBE_EASA_FNC_AttachSEADRow instead of EASA_Equip.sqf, so
    // picking it never removes/adds ordnance. Appending in-place with `find` + `set` (rather than a new
    // vehicle entry like Ka-137's) is required because the row must land inside F35B's/Su34's EXISTING
    // per-vehicle loadout array, not as a new vehicle.
    public static string GenerateSeadEasaRowBlock()
    {
        return @"
