...
Use the dynamic `git rev-parse` form only when validating an archive built from that same checkout. For the current PR #125 package tuple, pass `-ExpectedGit cf4e93af1a` with the matching archive SHA proof instead of a companion-branch HEAD.
