...
The builder accepts only the registered 240-unit/two-anchor or 360-unit/three-anchor campaigns; it rejects one-anchor, unequal-anchor and larger Utes requests. The partition sample interval may not exceed its configured measurement duration. These arms are a one-dimension group-partition test, so they also require `busRate=0` and `schedulerMode=off`; command-line overrides that enable either extra workload are rejected.
...
The partition aggregator requires at least three complete `PASS` repetitions per arm; `--min-repetitions` may raise but never lower that floor. It checks the exact Utes three-town topology, source/lab/git and a normalized partition-contract digest, typed and independently reconciled `RESULT` evidence, the exact globally ordered phase/event barrier and clocks, the registered `5/4/3/2/2` arm batch mapping, zero vehicles/crew/oversize/create failures, exact realized-group and anchor-index evidence, equal 120-member anchors, 98%-minimum target member/group exposure without more than one sampling interval of overcount, zero objects/groups remaining after cleanup, and integer path/arrival/route totals reconciled to every group+transition `PATHLEG` row. Path rows must remain inside the configured measurement window; each ARRIVED elapsed value must match its paired STARTED clock, and every route median is recomputed from those rows. Every MEASURE sample must carry FPS, AI/groups, work counters and finite non-negative `probeMs`; raw arrays are re-aggregated instead of trusting JSON summary flags. Arm reports include probe-cost median/p95/max distributions so observer cost remains visible. Any rejected input invalidates the supplied campaign; the tool never suppresses generic comparer warnings.
...
WASPLAB|v1|PATHLEG|run=...|t=...|measureT=...|group=...|transition=...|routeId=...|from=...|to=...|units=...|elapsed=...|status=STARTED|ARRIVED
WASPLAB|v1|SCHED|run=...|budgetMs=...|elapsedMs=...|frameDelta=...|due=...|deferred=...
WASPLAB|v1|ALERT|run=...|state=DEGRADED|COLLAPSED|OK|hcPct=...
WASPLAB|v1|RESULT|run=...|status=PASS|FAIL|reason=...|measureDuration=...|measureT=...|fpsSamples=...|fpsExpected=...|fpsCoveragePct=...|aiPeak=...|groupsPeak=...|histogram=...|anchorRequested=...|anchorMembers=...|memberSeconds=...|groupSeconds=...|pathLegsStarted=...|...
```
...
The registered density/group-partition arms do not issue waypoints while groups are still being created. They emit `SPAWN -> SETTLE -> GO -> MEASURE -> CLEANUP`, reset benchmark counters at `GO`, then keep a fixed post-GO measurement duration. At the end of MEASURE the controller fences new partition sampler/path work, stops the workers, drains any in-flight iteration while the phase is still MEASURE, performs the final exposure accumulation, and only then emits CLEANUP. A path iteration that reaches its target after the closing fence or configured end does not credit an ARRIVED row or issue the replacement leg; its previously emitted STARTED row remains the active frontier. Member-seconds and group-seconds are capped at the configured end and therefore exclude both the partition-dependent ramp and shutdown drain. The monitor and aggregator reconcile every `REALIZED`, SAMPLE and `PATHLEG` row with cumulative/direct evidence, including opaque run identity, strictly ordered unique event indices, dual clocks, per-group transition frontiers, elapsed/route totals, and observer-cost distributions. Other proving-ground recipes retain PR #997's original total-duration, wall-clock warm-up and immediate path/combat-order semantics; partition-only drain bookkeeping is not executed inside their sample loop.
