...
$primaryTownRows = @($parsedTowns | Where-Object { $_.MaxSV -ge 75 })
$districtTownRows = @($parsedTowns | Where-Object { $_.MaxSV -ge 50 -and $_.MaxSV -lt 75 })
$flankTownRows = @($parsedTowns | Where-Object { $_.MaxSV -lt 50 })
...
$report.Add("- Population value tiers: primary [$($primaryTownRows.Count)] maxSV [$((@($primaryTownRows | Measure-Object -Property MaxSV -Sum).Sum))], district/market [$($districtTownRows.Count)] maxSV [$((@($districtTownRows | Measure-Object -Property MaxSV -Sum).Sum))], flank [$($flankTownRows.Count)] maxSV [$((@($flankTownRows | Measure-Object -Property MaxSV -Sum).Sum))].")
$report.Add("- SP/SV intent: city, airfield, districts, market and Northwest/Rahim objectives carry most supply value; farm/outskirt objectives stay low-density route anchors.")
$report.Add("")
...
$report.Add("- WDDM exports are flattened by ``CreateDefenseTemplate`` on spawn, so treat Z offsets as documentation only and prove any slope-side fix with runtime screenshots/coordinates instead of vertical template values.")
...
$report.Add("- Population/SV sanity: city belt objectives should feel like the main fight; farms/outskirts should feel like lower-value SP/SV route anchors, not match-deciding economy hubs.")
