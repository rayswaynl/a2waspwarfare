...
Add-Gate $gates "runtime-approval" "pending" "Steff must explicitly approve local Arma launch/RPT collection before runtime commands are executed."
...
$packet = [ordered]@{
	...
	commands = [ordered]@{
		...
		runtimeSummary = "& .\Tools\PrTestHarness\Rpt\New-WaspReleaseRptSummary.ps1 -RptDirectory `"<release-candidate-rpts>`" -Recurse -ExpectedMarker `$expectedReleaseMarkers -RuntimePacketManifestPath `"<release-candidate-rpts>\runtime-rpt-packet-manifest.json`" -ExpectedCandidate $ExpectedCandidate -ExpectedGit $ReleaseGit -ExpectedArchiveSha256 $expectedArchiveSha256 -RequireRuntimePacketManifest -OutDirectory `"<release-candidate-rpts>\summary`" -Force"
	...
	runtimeChecklist = @(
		...
		"Release RPT summary is generated with -RuntimePacketManifestPath, -ExpectedCandidate, -ExpectedGit, -ExpectedArchiveSha256 and -RequireRuntimePacketManifest against exactly one scored RPT directory, and records matching release identity, matching rptRootHash, matching copiedRptSha256 values, validation.requested=true, validation.overall=pass, all required packet-validator gates passing and the exact ten copied RPT files from runtime-rpt-packet-manifest.json before summary overall can pass.",
		...
		"AICOM side discovery, heartbeat, WEST/EAST ticks, AICOMSTAT event, WEST and EAST TEAM_FOUNDED, CMDRSTAT and at least one autonomous action/progress token per side such as ASSAULT_DISPATCH, COMBATSTAT, FRONT, POSTURE or SNAP.",
