Private ["_target","_caller","_id","_args","_command","_callerName","_isHeadless","_clientFps","_hasDialog","_vehicleType","_cursorType","_visibleMap","_groupUnits","_vehicleSpeed","_cameraView","_gpsBefore","_shownGps","_gpsChanged","_hasItemGps","_rubHud","_rubGps","_zoomGps","_wfDisplay","_wfMenuOpen","_wfTopText","_wfGpsButtonFound","_serviceDisplay","_serviceMenuOpen","_serviceStatusText","_serviceTextLen","_serviceInfoPos","_serviceClipRisk","_tacticalMenuOpen","_buyMenuOpen","_restoreGpsAfterAudit"];
...
if ((!hasInterface) || {_isHeadless}) exitWith {
	diag_log Format ["[WASP-PR8-STRESS-CLIENT] command=%1 skipped headless/non-interface hasInterface=%2 isHeadless=%3", _command, hasInterface, _isHeadless];
};
...
if (_serviceMenuOpen) then {
	...
	_serviceClipRisk = _serviceTextLen > 115;
};
_tacticalMenuOpen = !(isNull (findDisplay 17000));
_buyMenuOpen = !(isNull (findDisplay 12000));
_gpsChanged = false;
if (_gpsBefore) then {_gpsChanged = !_shownGps} else {_gpsChanged = _shownGps};
...
diag_log Format ["[WASP-PR8-STRESS-CLIENT] CLIENT_GPS_STATE command=%1 hasItemGPS=%2 shownGPS=%3 gpsBefore=%4 changed=%5 RUBHUD=%6 RUBGPS=%7 zoomgps=%8 visibleMap=%9 dialog=%10", _command, _hasItemGps, _shownGps, _gpsBefore, _gpsChanged, _rubHud, _rubGps, _zoomGps, _visibleMap, _hasDialog];
if (_restoreGpsAfterAudit) then {
	showGPS _gpsBefore;
	diag_log Format ["[WASP-PR8-STRESS-CLIENT] CLIENT_GPS_RESTORE command=%1 gpsBefore=%2 shownGPS=%3", _command, _gpsBefore, shownGPS];
};
diag_log Format ["[WASP-PR8-STRESS-CLIENT] CLIENT_UI_TEXT_STATE command=%1 wfMenu=%2 topStrip='%3' gpsButtonFound=%4 serviceMenu=%5 serviceLen=%6 tacticalMenu=%7 buyMenu=%8", _command, _wfMenuOpen, _wfTopText, _wfGpsButtonFound, _serviceMenuOpen, _serviceTextLen, _tacticalMenuOpen, _buyMenuOpen];
if (_serviceMenuOpen) then {
	diag_log Format ["[WASP-PR8-STRESS-CLIENT] CLIENT_SERVICE_CLIP_AUDIT display=20000 infoIdc=20021 infoPos=%1 textLen=%2 clipRisk=%3 text='%4'", _serviceInfoPos, _serviceTextLen, _serviceClipRisk, _serviceStatusText];
} else {
	diag_log Format ["[WASP-PR8-STRESS-CLIENT] CLIENT_SERVICE_CLIP_AUDIT display=missing infoIdc=20021 infoPos=[] textLen=0 clipRisk=false reason=noDisplay command=%1", _command];
};
