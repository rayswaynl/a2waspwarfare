...
<#
.SYNOPSIS
    Guards the failure-atomic GUER FOB construction-start contract.
...
.DESCRIPTION
    ...
    LocationLogicStart. This static contract reserves the FOB token until the
    worker publishes a one-shot construction-start result, and makes each
    construction worker retain the exact created object instead of a
    nearby-entity lookup. It also keeps the client preview and server gate
    aligned with the configured factory footprint.
#>
[CmdletBinding()]
param()
...
Assert-Match $request '\[_classname,\s*resistance,\s*_pos,\s*_dir,\s*_index,\s*_startResultKey\]\s+ExecVM' "handler passes its result key to construction"
...
Assert-Ordered $request 'if ((_startResult select 0) == 1)' "deleteVehicle _truck" "truck consumption waits for acknowledged construction start"
