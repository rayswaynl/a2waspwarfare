...
# label a log that does not carry build= in WASPSCALE
python analyze_soak.py arma2oaserver.RPT --build-label Build86

# compare this run against one or more prior JSON dumps
python analyze_soak.py arma2oaserver.RPT --compare build85.json --compare build86.json
...
| `--build-label LABEL` | override the build label when the RPT has no `build=` field |
| `--compare <json>` | add a prior analyzer JSON file to the per-build KPI comparison (repeatable) |
...
Build-86 family counters); adding the HC RPT unlocks the **capture-driver /
per-town dogpile** detail in section 7.
...
6. **BUILD 86 TELEMETRY** — tolerant counters and last-3 samples for the
   expected Build-86 log families: `MHQRELOC RELAXED`, `BUILD_ROAD_*`,
   `PATROL` naval-skip, `SCUD/TEL`, `EASA/gear`, and the `[WFBE (SKIN)]`
   B0..B6 chain.
7. **HOLD / SEE-SAW** — max simultaneous towns held per side (from POSTURE
   `myTowns=`), captures per town (**dogpile** flag at ≥4), and — if the HC RPT
   is supplied — the HC `CAPTURED [` driver counts per town.
8. **PERF** — WASPSCALE server-fps / HC-fps min/median/max, AI_TOT curve, GUER
   peak, and fps at peak AI load (the FPS-cliff check).
9. **PER-BUILD KPI SUMMARY / COMPARISON** — one compact row for the current
   run, plus prior JSON rows and deltas when `--compare` is supplied.
10. **VERDICT** — PASS/WATCH/FAIL per KPI + overall (worst-of).

### Per-build comparison

Save JSON from any run:

```bash
python analyze_soak.py arma2oaserver.RPT --build-label Build86 --json > build86.json
```

Then compare a later run against it:

```bash
python analyze_soak.py arma2oaserver.RPT --build-label Build87 --compare build86.json
```

The comparison row tracks build/map, duration, arrival %, zombies, W↔E kill
share, WEST/EAST churn per hour, MHQ aborts, Build-86 family hits, and median
server FPS. JSON output includes the same `kpis`, `build86_telemetry`, and
`comparison.rows` fields for dashboards or release notes.
