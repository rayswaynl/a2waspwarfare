...
try {
    ...
    $resultJson = Get-Content -LiteralPath (Join-Path $results "$($r.runId).json") -Raw | ConvertFrom-Json
    Assert ($resultJson.config.flags -is [pscustomobject]) "result config.flags remains an object"
