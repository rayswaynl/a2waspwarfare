...
try {
    ...
    Write-Host "`n[2] -FromRpt grades the sample; boot-smoke must not be silently skipped or error as PASS"
    $r = & $script -Name idle-soak -FromRpt $sample -LedgerPath $ledger -ResultsDir $results
    Assert ($r.verdict -eq 'FAIL')                   "idle-soak verdict FAIL when sample lacks boot markers ($($r.verdict))"
    ...
    Assert ($r.bootSmoke.ran -eq $true)              "boot-smoke ran when the tool is present"
    Assert ($r.bootSmoke.verdict -ne 'ERROR')         "boot-smoke invocation returned a verdict ($($r.bootSmoke.verdict))"
    Assert ($r.bootSmoke.verdict -eq 'FAIL')          "boot-smoke failure is visible for the incomplete sample"
    ...
    Assert ($row.lenses.errors -eq 'FAIL')           "row errors lens FAIL"
    $vout = & python $validate $ledger 2>&1 | Out-String
    Assert ($vout -match 'CONFORMANCE OK')           "ledger conforms to run_result.schema.json"

    Write-Host "`n[4] -FromRpt retains a missed WATCH assert when boot-smoke is a hard FAIL"
    $r2 = & $script -Name big-assault -FromRpt $sample -LedgerPath $ledger -ResultsDir $results
    Assert ($r2.verdict -eq 'FAIL')                  "big-assault verdict FAIL from boot-smoke ($($r2.verdict))"
