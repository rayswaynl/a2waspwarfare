...
$brief.Add("powershell -NoProfile -ExecutionPolicy Bypass -File Tools\New-ZargabadEvidenceFolder.ps1 -OutputPath `".\zargabad-evidence`" -Force")
...
$brief.Add("- Use the runtime report's `Evidence Bundle Checklist` and the generated `README-zargabad-evidence.md` screenshot row mapping before taking screenshots; prefer the exact filenames `population-city-airfield.png`, `population-farms-outskirts.png`, `base-west-sightline.png`, `base-east-sightline.png`, `base-static-arcs.png`, `wall-origin-axis.png`, `wall-gaps-pathing.png`, `town-defense-blocking.png`, `priority-defense-arcs.png`, `rim-illegal-removal.png`, and `rim-legal-allow.png`.")
