...
Assert-True "runtime report tool emits objective coverage" ($runtimeReportSource -match '## Objective Coverage' -and $runtimeReportSource -match 'Guides/Zargabad-Completion-Gates\.md' -and $runtimeReportSource -match 'Fully build out the Zargabad mission' -and $runtimeReportSource -match 'Fortification review uses WDDM')
...
Assert-True "runtime report validator requires all Claude Notes rows" ($runtimeReportValidatorSource -match 'Claude Notes required rows are present' -and $runtimeReportValidatorSource -match 'Hosted boot context' -and $runtimeReportValidatorSource -match 'Dedicated boot context' -and $runtimeReportValidatorSource -match 'Population and SP/SV placement feel' -and $runtimeReportValidatorSource -match 'Base safety and spawn sightlines' -and $runtimeReportValidatorSource -match 'Economy and factory pricing feel')
...
Assert-True "runtime report validator requires Codex action recommendations" ($runtimeReportValidatorSource -match 'Claude Notes PASS rows include Codex action recommendation' -and $runtimeReportValidatorSource -match 'keep\|tune\|revert\|investigate\|patch\|retest')
Assert-True "runtime report validator requires objective coverage rows" ($runtimeReportValidatorSource -match 'Objective Coverage required rows are present' -and $runtimeReportValidatorSource -match 'Objective Coverage rows are all PASS' -and $runtimeReportValidatorSource -match 'Objective Coverage PASS rows include Codex action recommendation' -and $runtimeReportValidatorSource -match 'SP/SV, town centers, and camps match likely population' -and $runtimeReportValidatorSource -match 'Mystery feature uses existing mission code cheaply and stays under 100 LOC')
Assert-True "runtime report validator requires row-specific Claude evidence" ($runtimeReportValidatorSource -match 'Claude Notes .* evidence is specific' -and $runtimeReportValidatorSource -match 'hosted' -and $runtimeReportValidatorSource -match 'dedicated' -and $runtimeReportValidatorSource -match 'zargabad-map-audit\\.md' -and $runtimeReportValidatorSource -match 'Population and SP/SV placement feel' -and $runtimeReportValidatorSource -match 'Recommended Codex action')
...
Assert-True "runtime report validator checks screenshot evidence roots" ($runtimeReportValidatorSource -match 'EvidenceRoot' -and $runtimeReportValidatorSource -match 'Get-ScreenshotReferences' -and $runtimeReportValidatorSource -match 'Test-ScreenshotImageFile' -and $runtimeReportValidatorSource -match 'PNG/JPEG files under evidence root')
Assert-True "runtime report validator requires screenshots for visual rows" ($runtimeReportValidatorSource -match 'key visual Claude Notes rows include screenshot references' -and $runtimeReportValidatorSource -match 'Population and SP/SV placement feel' -and $runtimeReportValidatorSource -match 'Priority defense mix arcs')
Assert-True "runtime report validator checks real map audit packet attachment" ($runtimeReportValidatorSource -match 'map audit packet attachment exists under evidence root' -and $runtimeReportValidatorSource -match 'Test-MapAuditPacketFile' -and $runtimeReportValidatorSource -match 'Population Flow' -and $runtimeReportValidatorSource -match 'WDDM Fortification Review')
...
Assert-True "Claude brief tool points to completion gates guide" ($claudeBriefSource -match 'Guides/Zargabad-Completion-Gates\.md' -and $claudeBriefSource -match 'objective coverage checklist' -and $claudeBriefSource -match 'matching objective row has current static and runtime proof')
...
Assert-True "Claude brief tool points to evidence bundle checklist filenames" ($claudeBriefSource -match 'Evidence Bundle Checklist' -and $claudeBriefSource -match 'README-zargabad-evidence\.md' -and $claudeBriefSource -match 'population-city-airfield\.png' -and $claudeBriefSource -match 'base-static-arcs\.png' -and $claudeBriefSource -match 'wall-gaps-pathing\.png' -and $claudeBriefSource -match 'priority-defense-arcs\.png')
...
Assert-True "Claude runtime handoff asks for real map audit packet evidence" ($claudeRuntimeHandoffSource -match 'zargabad-map-audit\.md' -and $claudeRuntimeHandoffSource -match 'Population Flow' -and $claudeRuntimeHandoffSource -match 'Rim Test Points' -and $claudeRuntimeHandoffSource -match 'Claude Screenshot Targets')
