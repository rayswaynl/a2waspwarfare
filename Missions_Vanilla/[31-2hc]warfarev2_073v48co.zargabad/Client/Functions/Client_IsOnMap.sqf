...
//--- Centre of the boundary box. Defaults to the map-corner-derived centre [size/2,size/2]
//--- (unchanged for every existing island); an island may override it (e.g. Zargabad pins the
//--- box on the central valley so a tight size clips the exploitable edge-hills symmetrically).
_sqrradHR = missionNamespace getVariable ['WFBE_BOUNDARIES_CENTER', [_sqrradH,_sqrradH]];
_position = [getPos player select 0, getPos player select 1];

_difx = (_position select 0) - (_sqrradHR select 0);
_dify = (_position select 1) - (_sqrradHR select 1);
