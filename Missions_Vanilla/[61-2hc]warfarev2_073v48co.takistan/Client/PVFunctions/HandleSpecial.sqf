...
switch (_request) do {
	...
	case "uav-auth-token": {Private ["_challenge","_expires","_token"]; if (count _args != 3) exitWith {}; _token = _args select 0; _expires = _args select 1; _challenge = _args select 2; if (typeName _token != "STRING" || {_token == ""} || {typeName _expires != "SCALAR"} || {_expires <= time}) exitWith {}; if (_challenge != (missionNamespace getVariable [Format ["wfbe_uav_auth_challenge_%1", getPlayerUID player], ""])) exitWith {}; missionNamespace setVariable [Format ["wfbe_uav_cap_client_%1", getPlayerUID player], [_token, _expires]];};
	case "uav-purchase-result": {Private ["_expected","_msg","_ok","_token"]; if (count _args != 4) exitWith {}; _ok = _args select 0; _msg = _args select 1; _token = _args select 3; _expected = missionNamespace getVariable [Format ["wfbe_uav_purchase_token_%1", getPlayerUID player], ""]; if (typeName _ok != "BOOL" || {_expected == ""} || {_token != _expected}) exitWith {}; missionNamespace setVariable [Format ["wfbe_uav_purchase_token_%1", getPlayerUID player], ""]; missionNamespace setVariable [Format ["wfbe_uav_purchase_status_%1", getPlayerUID player], if (_ok) then {1} else {-1}]; if (_ok) then {[sideJoinedText,'UnitsCreated',count crew playerUAV] Call UpdateStatistics; [sideJoinedText,'VehiclesCreated',1] Call UpdateStatistics} else {if (!isNull playerUAV) then {{if (!isPlayer _x) then {deleteVehicle _x}} forEach crew playerUAV; deleteVehicle playerUAV; playerUAV = objNull}; if (typeName _msg == "STRING" && {_msg != ""}) then {hint _msg}};};
