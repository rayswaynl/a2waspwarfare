	...
	if(_changeCommander && !gameOver) then {
		...
		if (!isNull(commanderTeam)) then {
			if (commanderTeam == Group player) then {
				if (!IsNull _MHQ) then {
					_MHQ setVariable ["wfbe_mhq_unlock_aid", _MHQ addAction [localize "STR_WF_Unlock_MHQ","Client\Action\Action_ToggleLock.sqf", [false], 95, false, true, '', 'alive _target && {(locked _target) > 0}']]; //--- fix(hunt): store the LOCAL action id for exact removal on commander change
					_MHQ setVariable ["wfbe_mhq_lock_aid", _MHQ addAction [localize "STR_WF_Lock_MHQ","Client\Action\Action_ToggleLock.sqf", [true], 94, false, true, '', 'alive _target && {(locked _target) == 0}']];
				};
