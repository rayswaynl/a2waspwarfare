...
if (missionNamespace getVariable ["WFBE_C_VICTORY_OUTRO_FX", true]) then {
	[_side, getPos (_blist select 0)] Spawn {
		...
		if (isNull _flare) then {
			["WARNING", Format ["Client_EndGame.sqf: victory particle fountain create failed at %1.", _fxPos]] Call WFBE_CO_FNC_LogContent;
		} else {
		...
		if (isNull _light) then {
			["WARNING", Format ["Client_EndGame.sqf: victory lightpoint create failed at %1.", _fxPos]] Call WFBE_CO_FNC_LogContent;
		} else {
