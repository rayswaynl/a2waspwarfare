...
if (!isNull _site) then {
	...
	{ if ([_x] Call WFBE_CO_FNC_IsRealPlayer && {alive _x} && {side _x == _bAttrSide} && {(_x distance _bAttrPos) < _bDist}) then {_bNear = _x; _bDist = _x distance _bAttrPos} } forEach playableUnits;
