...
if (!_reject) then {
	...
	//--- Empty-vehicle reaper exemption (extends the merged #1439/#1440 wfbe_is_guer_fob pattern to WEST/EAST
	//--- FOBs). Tent + mast are structures (never reaper-eligible) but tagged for completeness; the building
	//--- repair truck IS reaper-eligible, so tag it too so an abandoned FOB truck is not eaten by
	//--- Server_HandleEmptyVehicle.sqf / HandleSpecial.sqf (bounded: <= WFBE_C_FOB_CAP_PER_SIDE per side).
	_tent setVariable ["wfbe_is_fob", true, true];
	if (!isNull _antenna) then {_antenna setVariable ["wfbe_is_fob", true, true]};
	if (!isNull _truck) then {_truck setVariable ["wfbe_is_fob", true, true]};
