...
if ((_structure getVariable ["wfbe_structure_type", ""]) == "Bank" && (missionNamespace getVariable ["WFBE_C_ECONOMY_BANK", 0]) > 0) then {
	...
	if ((!isNull _killer) && (isPlayer _killer) && !_teamkill) then {
		...
		[getPlayerUID _killer, "BankPayout", [_bankBounty, true]] Call WFBE_CO_FNC_SendToClients; //--- E5: route the $25k payout by the REAL UID, not the display-masked _killer_uid ("xxxxxxx" when WFBE_C_GAMEPLAY_UID_SHOW==0 -> no client matched -> $0 paid)
