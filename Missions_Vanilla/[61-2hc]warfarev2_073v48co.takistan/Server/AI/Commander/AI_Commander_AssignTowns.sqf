...
//--- cap reflects real massed force and rolls freed teams to the NEXT town once one is captured.
{
	...
	if (!isNull _team) then {
		if (({alive _x} count (units _team)) > 0) then {
			...
			_curOrd = [_team, "wfbe_aicom_townorder", []] Call WFBE_CO_FNC_GroupGetValue;
...

{
	...
	if ([_team, "wfbe_aicom_dispatch_open", false] Call WFBE_CO_FNC_GroupGetBool) then {
		...
		_dord = [_team, "wfbe_aicom_townorder", []] Call WFBE_CO_FNC_GroupGetValue;
	...
	_modeNow = toLower ([_team, "wfbe_teammode", "towns"] Call WFBE_CO_FNC_GroupGetValue);
	_canDrive = false;
	_explicitMode = false;
	...
	if (_canDrive) then {
			...
			if (_garDead) then {
				...
				if (!isNull _hqG) then {
					...
					if ([_team, "wfbe_aicom_hc", false] Call WFBE_CO_FNC_GroupGetBool) then {
						_team setVariable ["wfbe_aicom_order", [[_team] Call WFBE_CO_FNC_AICOMNextOrderSeq, "defense", getPos _hqG], true];
					};
		...
		if (!_explicitMode) then {
			_mode = [_team, "wfbe_teammode", ""] Call WFBE_CO_FNC_GroupGetValue;
			_goto = [_team, "wfbe_teamgoto", objNull] Call WFBE_CO_FNC_GroupGetValue;
				...
				if (typeName _goto != "OBJECT") then {
					...
					if (isNull _goto) then {
						...
						if ((_goto getVariable "sideID") == _sideID) then {
							...
							_ord = [_team, "wfbe_aicom_townorder", []] Call WFBE_CO_FNC_GroupGetValue;
							if (count _ord < 3 || {(_ord select 0) != _goto}) then {
								...
								if (typeName _gotoSince != "SCALAR") then {_gotoSince = -1};
								...
								if (_stallSecs > 0 && {(time - _gotoSince) > _stallSecs}) then {
									...
									_saKeep = [];
									{ if ((typeName _x) == "ARRAY" && {count _x >= 2} && {(typeName (_x select 0)) == "OBJECT"} && {!isNull (_x select 0)} && {(_x select 1) > time} && {(_x select 0) != _goto}) then {_saKeep set [count _saKeep, _x]} } forEach _saBl;
								...
								if (!_needs && {time - (_ord select 1) > (missionNamespace getVariable ["WFBE_C_AICOM_STUCK_SECS", 210])}) then {
									...
									if ((behaviour _ldr != "COMBAT") && {_ldr distance (_ord select 2) < _movedThr} && {_ldr distance _goto > _farThr}) then {
										...
										if (_strk > (missionNamespace getVariable ["WFBE_C_AICOM_STUCK_ABANDON", 4])) then {
											...
											_abBl = [_team, "wfbe_aicom_blacklist", []] Call WFBE_CO_FNC_GroupGetValue;
			...
			if (_needs) then {
				...
				if (_bootstrap) then {
					...
					_blList = [_team, "wfbe_aicom_blacklist", []] Call WFBE_CO_FNC_GroupGetValue;
					_blKeep = [];
					_blTowns = [];
				...
				if (!isNil "_target") then {
					if (!isNull _target) then {
						...
						if ([_team, "wfbe_aicom_hc", false] Call WFBE_CO_FNC_GroupGetBool) then {
							...
							_hcStrk   = [_team, "wfbe_aicom_stuckstrikes", 0] Call WFBE_CO_FNC_GroupGetValue;
							...
							_team setVariable ["wfbe_aicom_order", [[_team] Call WFBE_CO_FNC_AICOMNextOrderSeq, "towns-target", _hcDest, _hcStrk], true]; //--- UNSTUCK FIX (Ray 2026-06-16): carry the strike tier as order element 3 so it stays in sync with the seq it belongs to (reader: Common_RunCommanderTeam). The wfbe_aicom_unstuck flag (line ~367) is kept for the gear-slow governor + logging.
						} else {
							if (_useArc) then {
						...
						_priorOrd  = [_team, "wfbe_aicom_townorder", []] Call WFBE_CO_FNC_GroupGetValue;
