...
//--- Must not touch any existing line above or below; insert between harass block and expansion lane.
...
if ((missionNamespace getVariable ["WFBE_C_AICOM_FEINT_ENABLE", 0]) > 0 && {!_expandFirst} && {!_concentrate}) then {
	private ["_feintTgt","_feintTeam","_feintT0","_feintDur","_feintInterval","_feintGrp","_feintLdr","_feintAlive","_feintMode","_feintRelief","_feintStrike","_feintHasVeh","_feintExpiry","_feintRecalled","_feintFar","_feintD","_feintI"];
	...
	//--- DISPATCH PASS: only when per-side cooldown elapsed (prevents feint while one is live).
	if ((time - _feintT0) >= _feintInterval) then {
		...
		if (!isNull _feintTgt) then {
			...
			while {isNull _feintTeam && {_feintI < (count _teams)}} do {
				...
				if (!isNull _feintGrp && {_feintAlive > 0} && {!isNull _feintLdr} && {!isPlayer _feintLdr}
					&& {_feintGrp != _garGrp} && {isNull _feintRelief} && {!_feintStrike}
