...
if (!_fromFocus) then {
	...
	_scored = [];
	//--- r26 topology: AUTO fist is the side's ground-main-effort. Skip offshore naval-HVT from the
	//--- scorer when NAVAL_AIR_ONLY (distance-only adjacency treats a sea deck as "front-adjacent" to a
	//--- coastal own town). Empty-scored guardrail after the loop re-admits naval if no land candidate remains.
	{
		private ["_tt","_dNear","_sc","_garTier"];
		_tt = _x;
		if (!((missionNamespace getVariable ["WFBE_C_AICOM_NAVAL_AIR_ONLY", 1]) > 0 && {_tt getVariable ["wfbe_is_naval_hvt", false]})) then {
			_dNear = _tt Call _frontDist;
	...
	//--- Never-idle: if every capturable town is a naval HVT, score them so the fist still publishes.
...
if (_harassN > 0) then {
	if ((missionNamespace getVariable ["WFBE_C_AICOM_HARASS_FALLBACK", 0]) > 0) then {
		...
		_hfCands = [];
		{
			private ["_tt","_depth"];
			_tt = _x;
			//--- r26 topology: harass is a MOUNTED ground detachment - skip offshore naval-HVT (no sea path).
			if (!((missionNamespace getVariable ["WFBE_C_AICOM_NAVAL_AIR_ONLY", 1]) > 0 && {_tt getVariable ["wfbe_is_naval_hvt", false]})) then {
		...
		//--- legacy path (WFBE_C_AICOM_HARASS_FALLBACK=0): deepest pick, r26 still skips naval-HVT for ground harass.
		{
			private ["_tt","_depth"];
			_tt = _x;
			if (!((missionNamespace getVariable ["WFBE_C_AICOM_NAVAL_AIR_ONLY", 1]) > 0 && {_tt getVariable ["wfbe_is_naval_hvt", false]})) then {
...
_neutTowns = [];
{
	...
	//--- r26 topology: expansion lane is ground teams peeling for soft neutrals - never an offshore carrier.
	if (_sid != _sideID && {_sid != _enemyID} && {!(_tt in _fist)} && {!((missionNamespace getVariable ["WFBE_C_AICOM_NAVAL_AIR_ONLY", 1]) > 0 && {_tt getVariable ["wfbe_is_naval_hvt", false]})}) then {
...
_dedupOn = (missionNamespace getVariable ["WFBE_C_AICOM_EXPAND_DEDUP", 0]) > 0;
{
	...
	if (!isNull _grp) then {
		...
		if (_alive > 0 && {!isNull _ldr} && {!isPlayer _ldr} && {_grp != _garGrp}
			...
			//--- r26 topology / B756: detect transport-heli teams so naval-HVT can still be assigned to air.
			...
			//--- Ground-only skip for offshore naval HVT (mirrors AssignTowns B756).
			private "_navalOk";
			_navalOk = {
				private "_tn";
				_tn = _this;
				((missionNamespace getVariable ["WFBE_C_AICOM_NAVAL_AIR_ONLY", 1]) <= 0) || {!(_tn getVariable ["wfbe_is_naval_hvt", false])} || _teamAir
			};
			...
			if (!isNull _harassTgt && {_harassAssigned < _harassN} && {_hasVeh} && {(_ldrPos distance _harassTgt) <= _reach} && {_harassTgt Call _navalOk}) then {
				...
				if (_expandCount < _expandN) then {
					...
					if (_dedupOn) then {
						{ _ev = _ldrPos distance _x; if (_ev < _expandWarnDist) then {_expandWarnDist = _ev; _expandWarnTown = _x}; if (_ev <= _reach && {_ev < _eD} && {!(_x in _expandClaimed)} && {_x Call _navalOk}) then {_eD = _ev; _eTgt = _x} } forEach _neutTowns;
					} else {
						{ _ev = _ldrPos distance _x; if (_ev < _expandWarnDist) then {_expandWarnDist = _ev; _expandWarnTown = _x}; if (_ev <= _reach && {_ev < _eD} && {_x Call _navalOk}) then {_eD = _ev; _eTgt = _x} } forEach _neutTowns;
					};
				...
				if (isNull _tgt) then {
					...
					if ((missionNamespace getVariable ["WFBE_C_AICOM_SPREAD_MODE", 1]) > 0) then {
						...
						_bestIdx = -1;
						{
							...
							if (_v <= _reach && {_v < _tgtD} && {(_fistCounts select _forEachIndex) < (_fistCaps select _forEachIndex)} && {_x Call _navalOk}) then {_tgtD = _v; _tgt = _x; _bestIdx = _forEachIndex};
						} forEach _fist;
						//--- fall-through: everything in reach is capped (or nothing in reach) -> pick the LEAST-LOADED
						//--- land-reachable fist town so no team idles (preserves the never-idle SAD guarantee).
						if (isNull _tgt) then {
							private ["_leastLoad"];
							_leastLoad = 1e9;
							{
								if ((_fistCounts select _forEachIndex) < _leastLoad && {_x Call _navalOk}) then {_leastLoad = _fistCounts select _forEachIndex; _tgt = _x; _bestIdx = _forEachIndex};
							} forEach _fist;
						};
						...
						{ _v = _ldrPos distance _x; if (_v <= _reach && {_v < _tgtD} && {_x Call _navalOk}) then {_tgtD = _v; _tgt = _x} } forEach _fist;
						if (isNull _tgt) then { { _v = _ldrPos distance _x; if (_v < _tgtD && {_x Call _navalOk}) then {_tgtD = _v; _tgt = _x} } forEach _fist };
					};
				};
			};
			...
			if (_tnTeamOn) then {
				...
				_tnBestT = objNull; _tnBestW = 0;
				{
					...
					if (_tnW2 > _tnBestW && {(_ldrPos distance _x) <= _reach} && {_x Call _navalOk}) then {_tnBestW = _tnW2; _tnBestT = _x};
				} forEach _tgtTowns;
