...
while {!gameOver && {(missionNamespace getVariable [_clOwnerKey, _clOwnerSeq]) == _clOwnerSeq}} do {
	...
	//--- + capped. Human count uses the shared predicate so HC and caster bodies are excluded consistently.
	private ["_pcN2","_pcMult","_baseMult"];
	_pcN2 = count ([] Call WFBE_CO_FNC_RealPlayers);
