...
if ((missionNamespace getVariable ["WFBE_C_KILL_TALLY_DECAL", 0]) > 0) then {
	_tallyMk = "this spawn {private ['_veh','_last','_cnt','_tier','_light','_bright','_color']; _light = objNull; _veh = _this; _last = -1; while {alive _veh} do {_cnt = _veh getVariable ['wfbe_kill_tally',0]; if ((abs (_cnt - _last)) > 0) then {_last = _cnt; _light = _veh getVariable ['mks_tally',objNull]; if (_cnt <= 0) then {if !(isNull _light) then {deleteVehicle _light; _veh setVariable ['mks_tally',objNull]}} else {if (isNull _light) then {_light = '#lightpoint' createVehicleLocal (position _veh); _veh setVariable ['mks_tally',_light]; _light attachTo [_veh,[0,0.5,1.1]]}; _tier = 1; if (_cnt >= 3) then {_tier = 2}; if (_cnt >= 6) then {_tier = 3}; if (_cnt >= 10) then {_tier = 4}; _bright = 0.025; _color = [1.0,0.55,0.05]; switch (_tier) do {case 2: {_bright = 0.032; _color = [1.0,0.33,0.02]}; case 3: {_bright = 0.040; _color = [1.0,0.08,0.0]}; case 4: {_bright = 0.050; _color = [1.0,0.85,0.6]};}; _light setLightBrightness _bright; _light setLightColor _color; _light setLightAmbient _color};}; sleep 2;}; _light = _veh getVariable ['mks_tally',objNull]; if !(isNull _light) then {deleteVehicle _light};}";
