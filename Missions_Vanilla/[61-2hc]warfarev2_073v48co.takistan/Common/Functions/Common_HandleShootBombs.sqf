...
Private ["_unit_targeted","_unit_who_shot", "_weapon", "_ammo", "_magazine","_projectile","_limit_distance","_distance_target_player","_player_name","_unit_who_shot_name","_vehicle","_objPosition","_objAltitude","_limit"];
...
//--- r53: defaulted restriction params (0 = disabled / unrestricted) so nil keys never throw on compare.
_limit_distance = missionNamespace getVariable ["WFBE_C_GAMEPLAY_BOMBS_DISTANCE_RESTRICTION", 0];
...
if (_limit_distance > 0 && {_distance_target_player >= _limit_distance}) then { // distance IS restrictive.
_vehicle = vehicle _unit_who_shot;
hint localize "STR_WF_MESSAGE_BombDistanceRestriction";
//--- r53: projectile may already be gone (engine despawn) - isNull before delete.
...
_limit = missionNamespace getVariable ["WFBE_C_GAMEPLAY_BOMBS_ALTITUDE", 0];
