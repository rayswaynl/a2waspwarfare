...
//--- AI.
	...
	//--- cmdcon42-l (Ray 2026-07-02) AICOM VEHICLE AIRLIFT: when an air-mobile leg launches (Common_AICOMAirLeg) and the team owns a LIGHT ground vehicle (HMMWV/UAZ/Vodnik-class - config armor <= *_MAXARMOR, so wheeled/tracked APCs (LAV/BTR/Bradley) and tanks are EXCLUDED), the transport SLINGS it below the heli (attachTo) and flies it to a DEEP drop point *_DEPTH m BEYOND the ordered town along the town->enemy-HQ (enemy-rear) axis, so the vehicle + crew land 1-2km BEHIND the lines and attack the objective from the rear (Ray's flanking intent). ONE lift per leg; NEVER lifts the team's only transport; the drop point must clear water + a flatness check (shortens depth toward the dest if not, floor at the dest). The pax insert still uses the normal hot-LZ point on the same flight path.
	if (isNil "WFBE_C_AICOM_VEHLIFT") then {WFBE_C_AICOM_VEHLIFT = 1};                            //--- 1 = an air-mobile leg SLINGS one owned LIGHT ground vehicle + deep-drops it behind the lines (default). 0 = pax-only air legs (legacy).
	...
	if (isNil "WFBE_C_AICOM_VEHLIFT_MAXARMOR") then {WFBE_C_AICOM_VEHLIFT_MAXARMOR = 120};        //--- config-armor ceiling that separates LIGHT (Offroad/UAZ 20-36, HMMWV 80, Vodnik 85/100, BRDM-2 120 - LIFTABLE) from wheeled APCs (BTR90 150 / LAV25 ~200) and tanks (T72 690 / M1A1 850). CHOSEN 120 (not 80): HMMWV armor is EXACTLY 80 so <=80 sits on the edge AND rejects the Vodnik (85/100) which we WANT to lift; the clean gap in vanilla A2/OA is 100/120 (Vodnik/BRDM) -> 150 (BTR90), so 120 admits every light 4x4 and excludes the first real APC. getNumber (configFile>>CfgVehicles>>type>>"armor") <= this = liftable (AND "Car" AND NOT "Wheeled_APC"). A per-class allowlist (WFBE_C_AICOM_VEHLIFT_ALLOW) is a fallback for hulls whose armor reads 0/unreliable.
