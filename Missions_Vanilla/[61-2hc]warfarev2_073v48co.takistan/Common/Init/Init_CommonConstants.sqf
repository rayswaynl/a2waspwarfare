...
//--- Artillery.
	...
	if (isNil "WFBE_C_DROPPEDITEMS_CLEANER_DEFER_FIRST") then {WFBE_C_DROPPEDITEMS_CLEANER_DEFER_FIRST = 1}; //--- Default ON: move the first empty whole-island sweep from ~90s to the steady cadence. This avoids boot overlap but does not bound the query itself: current deferred Chernarus sweeps still cost ~2.5-2.8s. 0 = restore the early phase.
	if (isNil "WFBE_C_DROPPEDITEMS_CLEANER_SLICED_SCAN") then {WFBE_C_DROPPEDITEMS_CLEANER_SLICED_SCAN = 0}; //--- Default OFF pending test-server A/B. 1 = replace the one-frame full-map weaponholder query with nine complete-coverage map cells and cooperative yields; cleanup rules/cap stay unchanged.
	if (isNil "WFBE_C_DROPPEDITEMS_CLEANER_SLICE_SLEEP") then {WFBE_C_DROPPEDITEMS_CLEANER_SLICE_SLEEP = 0.05}; //--- s between opt-in spatial query slices; excluded from active-ms audit, included in cycleMs wall time.
