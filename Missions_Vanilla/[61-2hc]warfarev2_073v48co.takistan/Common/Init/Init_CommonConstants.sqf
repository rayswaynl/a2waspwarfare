...
//--- AI.
	...
	if (isNil "WFBE_C_FPS_DOWNSCALE") then {WFBE_C_FPS_DOWNSCALE = 0}; //--- diagnosis wasp-waspscale-poptier-fps-gap-20260808: WFBE_PopTier (AI_Commander_Teams.sqf ~L298) is driven ONLY by human headcount, so a low-pop/HC-heavy soak box can sit at tier 0 - the HIGHEST WFBE_C_TOTAL_AI_MAX_BY_TIER cap - while measured server+HC fps are in the teens with nothing to shed load. 0 (default) = OFF, byte-identical to HEAD: no sampler, no latch, WFBE_PopTier computation unchanged. 1 = ARMED: AI_Commander.sqf's per-tick sampler latches WFBE_FpsThrottleActive once BOTH floors below are breached continuously for WFBE_C_FPS_DOWNSCALE_SUSTAIN_S seconds, and AI_Commander_Teams.sqf adds WFBE_C_FPS_DOWNSCALE_STEP to the published tier while latched - the SAME single WFBE_PopTier source every AI-cap/team-target/patrol-cap/town-cap consumer already reads, so no other file needs new reading logic.
	if (isNil "WFBE_C_FPS_DOWNSCALE_FLOOR") then {WFBE_C_FPS_DOWNSCALE_FLOOR = 20}; //--- server diag_fps at/below which the sustained-low streak accumulates. Only read while WFBE_C_FPS_DOWNSCALE > 0.
	if (isNil "WFBE_C_FPS_DOWNSCALE_HC_FLOOR") then {WFBE_C_FPS_DOWNSCALE_HC_FLOOR = 20}; //--- min diag_fps across fresh (<=120s) registered HCs at/below which the streak accumulates; no fresh HC report does NOT veto the streak (server-fps-only boxes still latch). Only read while WFBE_C_FPS_DOWNSCALE > 0.
	if (isNil "WFBE_C_FPS_DOWNSCALE_SUSTAIN_S") then {WFBE_C_FPS_DOWNSCALE_SUSTAIN_S = 120}; //--- continuous seconds both floors above must stay breached (sampled at the existing ~15s AI_Commander supervisor tick) before the latch sets; a single tick back above either floor clears it immediately. Only read while WFBE_C_FPS_DOWNSCALE > 0.
	if (isNil "WFBE_C_FPS_DOWNSCALE_STEP") then {WFBE_C_FPS_DOWNSCALE_STEP = 1}; //--- tier-index steps added to the population-derived WFBE_PopTier while the latch is active; every existing consumer already clamps its select to the array's last index. Only read while WFBE_C_FPS_DOWNSCALE > 0.
	...
	if (isNil "WFBE_C_AICOM_AIR_FACTORY_ENABLES_HELI") then {WFBE_C_AICOM_AIR_FACTORY_ENABLES_HELI = 0}; //--- fix0808h (owner ruling 2026-08-08): 1 -> 0. At BASE the AI commander must respect the researched air tier (the RU AI program holds AIR 1/2 at entries 22/24 - the Build83 "program omits it" note was stale). A held AIRFIELD still waives all air tiers (_freeAirWaive) - the designed early-air shortcut. Rollback: 1.
