...
if (_updateDefenses) then {
	...
	//--- Init_Coin normally follows commonInitComplete, but CoIn callbacks can re-enter after a side change.
	//--- CIV intentionally has no WFBE_CIVDEFENSENAMES registry, so a missing side list must remain inert.
	//--- The isNil guard in the forEach below further protects against any individual entry that might be
	//--- missing its config var (e.g. if a defence class is unrecognised by the engine).
...
for [{_i=_i}, {_i<count _structures}, {_i = _i+1}] do {
  ...
  _itemCostEntry = [0, _structureCosts select _i];
  _itemCatEntry = 0;
