...
//--- cmdcon43 (Build 88): use the GLOBAL ctrlShow [idc,bool] form. The display-scoped
//--- `(_display displayCtrl idc) ctrlShow bool` SILENTLY NO-OPS on idd createDialog menus
//--- in A2-OA 1.64 (same trap documented in GUI_Menu_Command.sqf) - the legend would never
//--- have shown. Global form is the mission's proven idiom (GUI_BuyGearMenu/GUI_Menu_*).
if (_uiV2 > 0) then {
	ctrlShow [511005, true];
} else {
...
while {WFBE_RespawnTime > 0 && dialog && alive player} do {
	...
	if (WFBE_MenuAction == 1) then {
		...
		if (!WFBE_RespawnDefaultGear && {isNil {player getVariable "wfbe_custom_gear"}}) then {
			private ["_capBpObj","_capBp","_capBpContent","_capPri","_capSec","_capPis","_capParsed"];
			_capBpObj = unitBackpack player;
			_capBp = if (!isNull _capBpObj) then {typeOf _capBpObj} else {""};
			_capBpContent = if (!isNull _capBpObj) then {[getWeaponCargo _capBpObj, getMagazineCargo _capBpObj]} else {[]};
