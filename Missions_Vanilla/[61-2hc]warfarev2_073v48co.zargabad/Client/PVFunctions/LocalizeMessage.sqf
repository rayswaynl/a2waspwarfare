...
switch (_localize) do {
	...
	case "CBRadarNeedsAAR": {if (count _this > 1) then {(_this select 1) Call WFBE_STRUCT_REFUND}; if (count _this > 2) then {(_this select 2) Call WFBE_STRUCT_LIVE_ROLLBACK}; _txt = Localize "CBRadarNeedsAAR"};
	case "AARadarAlreadyBuilt": {if (count _this > 1) then {(_this select 1) Call WFBE_STRUCT_REFUND}; if (count _this > 2) then {(_this select 2) Call WFBE_STRUCT_LIVE_ROLLBACK}; _txt = Localize "AARadarAlreadyBuilt"}; //--- fable/ew-economy
