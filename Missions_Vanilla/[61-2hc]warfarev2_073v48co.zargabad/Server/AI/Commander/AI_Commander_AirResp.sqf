...
if (_posCacheOn) then {
	_seenVeh = [];
	{
		if ([_x, false] Call WFBE_CO_FNC_IsRealPlayer && {alive _x} && {(side _x) == _enemySide}) then {
			_ePlayer = _x;
...
_bestTown = objNull; _bestCount = 0;
{
	...
	_nearCount = if (_posCacheOn) then {
		...
		{if (_x isKindOf "Man") then {[_x, false] Call WFBE_CO_FNC_IsRealPlayer && {alive _x} && {(side _x) == _enemySide}} else {({[_x, false] Call WFBE_CO_FNC_IsRealPlayer && {alive _x} && {(side _x) == _enemySide}} count (crew _x)) > 0}} count (_lanePos nearEntities [["Man","LandVehicle","Air"], _senseRadius])
	};
