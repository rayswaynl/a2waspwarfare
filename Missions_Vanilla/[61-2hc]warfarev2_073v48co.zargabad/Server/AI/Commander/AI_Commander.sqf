...
while {!gameOver && {(missionNamespace getVariable [_ownerKey, _ownerSeq]) == _ownerSeq}} do {
	...
	if (_active) then {
		...
		//--- AICOM CARGO AIRDROP Stage A: AI-only cargo plane + infantry + up to two staggered para-vehicles.
		//--- The master flag is checked here as well as inside the worker, so flag-off performs no town scan,
	...
	if (time - (missionNamespace getVariable ["wfbe_srvperf_t", -999]) >= 300) then {
		...
		private ["_aiW","_aiE","_aiG","_humN","_tier","_bt","_mn","_ci","_hcFps","_hcReg2"]; _aiW=0;_aiE=0;_aiG=0;_humN=0; { if (!isPlayer _x) then { switch (side _x) do { case west:{_aiW=_aiW+1}; case east:{_aiE=_aiE+1}; case resistance:{_aiG=_aiG+1} } } } forEach allUnits; _humN = count ([] Call WFBE_CO_FNC_RealPlayers); _tier = missionNamespace getVariable ["WFBE_PopTier",0];
