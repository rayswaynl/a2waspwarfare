...

{
	...
	if (_canDrive) then {
		...
		//--- ours and either the initial window remains or hostile [active attack] contact persists - treat the team as explicitly
		//--- ordered so this tick does NOT retarget it off the centre mid-fight (stops the see-saw). Once the town
		//--- flips back, or the quiet timer window lapses, the latch is CLEARED and the team retargets normally. A2-OA-safe (plain getVariable
		...
		if ((missionNamespace getVariable ["WFBE_C_AICOM_HOLD_MODE", 1]) > 0) then {
			...
			if (!isNil "_ht") then {
				if (typeName _ht == "OBJECT" && {!isNull _ht}) then {
					...
					if (_htSide == _sideID && {(time < _htUntil) || {_htUnderAttack}}) then {_htLive = true};
				};
			};
			if (_htLive) then {
				_explicitMode = true;
