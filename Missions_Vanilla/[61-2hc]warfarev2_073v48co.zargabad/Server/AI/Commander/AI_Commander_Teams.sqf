...
if (_sideAINow >= _aiCapTier) exitWith {
	...
	if ((time - _foundCapLast) >= 300) then {
		...
		diag_log ("AICOMSTAT|v2|EVENT|" + _sideText + "|" + str (round (time / 60)) + "|FOUND_SKIP|reason=side-cap|count=" + str _foundCapCount + "|sideAI=" + str _sideAINow + "|tierCap=" + str _aiCapTier + "|pc=" + str _sidePcN);
