	...
	//=== (2) MAINTAIN: spawn one sortie per eligible town, under the global active cap =========
	{
		...
		if ((count _sorties) < _maxActive
			...
			if (!_hasSortie) then {
				...
				{ if ([_x, false] Call WFBE_CO_FNC_IsRealPlayer && {alive _x} && {(side _x) != civilian} && {!((name _x) in WFBE_C_HC_NAMES)} && {(_x distance _town) < _playerRange}) exitWith { _hasPlayerNear = true; }; } forEach playableUnits;
