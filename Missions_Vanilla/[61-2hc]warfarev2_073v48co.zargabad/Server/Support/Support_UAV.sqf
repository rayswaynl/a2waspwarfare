...
_operator = if (count _args > 3) then {_args select 3} else {objNull};
if (isNull _operator || {!isPlayer _operator} || {!(_operator in units _playerTeam)}) then {_operator = leader _playerTeam};
