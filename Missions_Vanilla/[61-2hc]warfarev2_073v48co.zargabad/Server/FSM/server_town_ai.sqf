					...
					if (_currentEnemies > 0) then {
						...
						if (_sortieValid) then {
							...
							if ((missionNamespace getVariable ["WFBE_C_TOWNS_SORTIES_PROXIMITY", 0]) > 0) then {
								...
								{
									if ([_x, false] Call WFBE_CO_FNC_IsRealPlayer && {alive _x} && {(side _x) != civilian} && {!((name _x) in WFBE_C_HC_NAMES)} && {(_x distance _town) < _sortieProximityRange}) exitWith { _sortieProximityOk = true; };
