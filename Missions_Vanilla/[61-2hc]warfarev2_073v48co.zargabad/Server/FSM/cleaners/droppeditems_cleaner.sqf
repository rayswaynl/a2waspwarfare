...
//--- drop created 1s before the cycle has zero guaranteed loot window (mines_cleaner age-gates;
...
//--- gear pile under a looting player's feet. 0 = off (legacy wipe-regardless). Hold is capped by
...
while {!WFBE_GameOver} do {
	...
	_perfScanned = _perfScanned + _perfWeaponholders;
	{
		...
		if (isNull _x) then {} else {
			...
			if (!_whSkip) then {
				//--- Locality gate (re-port of fable/cleanup-locality-2 that never landed on master):
				//--- deleteVehicle on a NON-LOCAL object silently no-ops in A2 OA. Holders from
				//--- PLAYER gear drops / DropRPG are client-local; HC-delegated AI death piles are
				//--- HC-local. Bare server delete counted them "deleted" while they persisted forever.
				//--- cleanup-trash-object cannot carry them (holders are "alive"). Dedicated
				//--- cleanup-weaponholder case re-validates local + reap stamp + isKindOf WeaponHolder.
				if (local _x) then {
					deleteVehicle _x;
