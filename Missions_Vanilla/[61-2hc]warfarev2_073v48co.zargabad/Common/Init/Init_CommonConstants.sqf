...
//--- AI.
	...
	if (isNil "WFBE_C_TEST_TOWN_CAP") then {WFBE_C_TEST_TOWN_CAP = -1}; //--- AI-BEHAVIOR-LOOP-DESIGN.md sec6.1: TEST-ONLY scoping filter (NOT a world edit) - caps how many towns are eligible AI targets to the first N (stable array-index order). -1 = off (full map in play, byte-identical). Consumed by AI_Commander_AssignTowns.sqf (_uncaptured truncation) + AI_Commander_Snapshot.sqf (census truncation, feeds AI_Commander_Allocate.sqf via the snapshot).
	if (isNil "WFBE_C_TEST_TEAM_CAP") then {WFBE_C_TEST_TEAM_CAP = -1}; //--- AI-BEHAVIOR-LOOP-DESIGN.md sec6.2: TEST-ONLY late-override of the founding _target team count (mirrors WFBE_C_TEST_POPTIER_PIN's single-purpose override style). -1 = off (PC-tier curve + WFBE_C_AICOM_TEAMS_HARD_CAP govern as today, byte-identical). Consumed by AI_Commander_Teams.sqf right after the existing hard-cap clamp.
