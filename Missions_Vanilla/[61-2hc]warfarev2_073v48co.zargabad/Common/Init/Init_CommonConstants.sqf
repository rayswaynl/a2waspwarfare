...
//--- AI.
	...
	if (isNil "WFBE_C_AICOM_EFF_BIAS_EXP") then {WFBE_C_AICOM_EFF_BIAS_EXP = 0.3};  //--- AICOM v2 (Ray "much more varied"): 0.5->0.3 - flatten the cost-effectiveness bias so the founding draw spreads across far more templates (less repetition).
		...
		if (isNil "WFBE_C_AICOM_MECH_BIAS")  then {WFBE_C_AICOM_MECH_BIAS  = 1.8}; //--- OWNER DESIGN REFINEMENT 2026-07-20 07:52 (wasp-takistan-aicom-capture-stall-20260720): 1.5 -> 1.8. B756 previously trimmed 2.0->1.5 because heavy overtook infantry 38%/32% - the owner now explicitly wants FEWER pure-infantry squads (troops mounted in at least lightly-armed vehicles), so this pushes back toward mechanized without returning to the earlier over-correction; re-soak and retune alongside DISMOUNT_BIAS/MOTOR_BIAS below. Pairs with WFBE_C_AICOM_DISMOUNT_BIAS to favour IFV/APC dismount-carriers over bare MBT within the heavy pick.
		if (isNil "WFBE_C_AICOM_MOTOR_BIAS") then {WFBE_C_AICOM_MOTOR_BIAS = 1.7}; //--- OWNER DESIGN REFINEMENT 2026-07-20 07:52: 1.4 -> 1.7, proportional to the MECH_BIAS bump above (fewer pure-foot squads; motorized also carries its own dismounts).
		...
		if (isNil "WFBE_C_AICOM_DISMOUNT_BIAS") then {WFBE_C_AICOM_DISMOUNT_BIAS = 1.9}; //--- OWNER DESIGN REFINEMENT 2026-07-20 07:52: 1.6 -> 1.9, proportional to the MECH_BIAS bump above - keep favouring dismount-CARRYING templates specifically (IFV/APC+squad) over bare MBT within the heavy pick, since dismount-carriers are what still let a mechanized team work camps on foot.
