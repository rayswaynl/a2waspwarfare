...
//--- AI.
	...
	if (isNil "WFBE_C_AICOM_CAP_PENDING_TTL") then {WFBE_C_AICOM_CAP_PENDING_TTL = 300}; //--- fable/aicom-econ-triad F2: seconds a pending-spawn ledger entry counts before aging out (must cover the 270s HC pending-slot reaper window; longer = more conservative production).
