...
//--- Artillery.
	...
	if (isNil "WFBE_C_BASE_RANDOM_PURE") then {WFBE_C_BASE_RANDOM_PURE = 0}; //--- cmdcon41 (Ray): random-PURE default (original unfiltered Miksuu random). //--- Build84 (backlog#2): 1 = Miksuu-original UNFILTERED pure-random when MODE=2 (skips the B62 airfield / B66 egress-edge / rotation filters in Init_Server); 0 = hardened filtered random (default). //--- fable/zg-west-collapse (2026-07-10, owner-approved ZG-only override of the cmdcon41=1 value): live soak showed WEST teams stuck AT SPAWN (STUCKSTAT distStart=0-16m, distTgt=880m, 13/13, never egress) with RANDOM_PURE=1 skipping the B66 egress-road gate on ZG's dense-urban terrain - the exact "boxed into a corner with a single egress road" failure the gate below was built to reject. 0 re-enables B62/B66/rotation filtering so WEST's base lands somewhere its teams can actually leave. B66 anti-collapse (WFBE_B66_EGRESS_RELAX) protects ZG's thin road net. Rollback: 1.
