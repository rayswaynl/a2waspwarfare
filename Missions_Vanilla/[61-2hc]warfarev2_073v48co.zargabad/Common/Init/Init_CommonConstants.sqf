...
if (isNil "WFBE_C_HC_LOBBY_TIMEOUT") then {WFBE_C_HC_LOBBY_TIMEOUT = 180}; //--- s of mission time AFTER THE MISSION GOES LIVE: hard fail-open. A permanently missing HC opens the server anyway (logged loudly) instead of locking it out forever. Also the window in which the client-side gate is armed at all, so an ordinary mid-match JIP joiner never sees it. RE-ANCHORED 2026-07-30: the clock now starts at the mission-live stamp Init_HcLobbyLock.sqf takes on `commonInitComplete && townInit` (published as WFBE_HC_LOBBY_DEADLINE, read by both sides), not at mission-init - so raising the ops-side mission-live gate (MissionLiveTimeoutSeconds in C:\WASP\provision\Start-Wasp-*HC.ps1) can no longer silently eat this budget. DERIVED from measurement on the live 2-HC box, builds m0730e-h, 2026-07-30. The HCs are launched only after that script sees MATCH|v1|START|, so the budget must cover launch offset + HC boot. Launch offset of the LAST HC = ~5s marker detection (3s RPT poll + flush) + 15s PostLiveBufferSeconds + 20s HcStaggerSeconds per HC after the first = 40s at 2 HCs, 80s at 4 HCs. HC boot->seated, measured: HC1 on real Steam 19.2/19.3/19.4s to "Init_HC.sqf: reseated onto CIVILIAN" in its own client RPT plus the launcher's 2s taskkill wait = ~21s; HC2 under Sandboxie 47s (m0730f) and 58s (m0730e) from launch to "Headless client is now connected" - sandboxed Steam is the slow path, so 58s is the one that sizes this. Worst seated-by = 80 + 58 = 138s on the 4-HC box (98s at 2 HCs). x1.3 on the boot term because the HCs now boot against a LIVE simulating server instead of a loading one: 80 + 76 = 156s -> 180s. The old 90 was never actually exercised (the gate never held, so both HCs were always seated by mission time ~4); 150 covers 2 HCs but leaves only 12s over the measured 4-HC worst case. The old "keep it under the ~120s deadspawn-transit invulnerability budget" note no longer applies: Init_Client.sqf re-asserts allowDamage false on every pass of the hold and re-arms a fresh 120s watchdog on release, so a hold longer than 120s is covered end to end.
