...
//--- LS-GUARD: In Last Stand event mode there are no capturable towns (towns=[]).
//--- Skip the entire town-starting-mode block to avoid any edge cases with empty arrays
//--- and to make intent explicit. townInitServer is set to true below regardless.
if (IS_laststand_event) exitWith {
	diag_log "[WFBE-LS] Server\Init\Init_Towns.sqf: IS_laststand_event=true - skipping town assignment (no towns on Namalsk Last Stand).";
	townInitServer = true;
};
