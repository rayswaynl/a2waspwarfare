...
if (isNil "_teams") exitWith {};
{
	...
	if (!isNull _team && {!isPlayer (leader _team)}) then {
		if (_aliveCount > 0) then {
			_mode  = [_team, "wfbe_teammode", "towns"] Call WFBE_CO_FNC_GroupGetBool; //--- fix(hunt): G1 - unset group var returns nil (fresh HC-founded team before its first dispatch), toLower nil threw and killed the worker every tick
			_modeL = toLower _mode;
			if (_modeL == "move" || _modeL == "patrol" || _modeL == "defense") then {
				_goto = [_team, "wfbe_teamgoto", [0,0,0]] Call WFBE_CO_FNC_GroupGetBool; //--- fix(hunt): G1 guard (same trap)
				...
				if (_realGoto) then {
					...
					_prevMode    = [_team, "wfbe_exec_lastmode", ""] Call WFBE_CO_FNC_GroupGetBool; //--- fix(hunt): G1 - a never-seeded latch read nil, the comparisons threw, the order was never committed AND the latch never written: a just-founded team froze on its first human order forever
					...
					if (_changed) then {
						...
						diag_log ("AICOM2|v1|ORDER|war-room-task|" + str _side + "|" + str (round (time / 60)) + "|team=" + (str _team) + "|mode=" + _modeL + "|goto=" + str [round (_goto select 0), round (_goto select 1)]);
