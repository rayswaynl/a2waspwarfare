...
if (count _live > 0) then {
	...
	//--- cmdcon33 cleanup (Ray 2026-07-01): the DUPLICATE cmdcon31 starved-infantry fallback that used to sit here
	//--- (after the air-gate strip) was REMOVED. The canonical fallback at ~L316 runs BEFORE the first empty-set
	//--- exitWith and the infantry template it admits carries no StaticWeapon (survives the static-strip) and air-mask
	//--- 0 (survives the air-gate), so by this point _eligible already holds an infantry template and this second
	//--- admit was provably unreachable dead code. Verified 2026-07-01 (aicom-teams + adversarial verify).
	if (count _eligible == 0) exitWith {};
