...
private ["_side","_sideID","_sideText","_logik","_teams","_target","_aiTeams","_pending","_g","_hcs","_live","_templates","_tmplUpgrades","_upgrades","_eligible","_i","_u","_ok","_k","_doc","_track","_pref","_pick","_template","_price","_cn","_ud","_funds","_structures","_facClass","_facNames","_facIdx","_fac","_facObj","_real","_foundedTeams","_editorTeams","_totalGroups","_hcUnit","_base","_extra","_maxExtra","_fundsPerExtraTeam","_lastDynTarget",
              "_allUnits","_allGroups","_allVehicles","_refundPending","_refundPendingLive","_refundToken",
...
if (count _live > 0) then {
	...
	//--- Authority closure: issue a server-only capability before the HC can create a group. The later
	//--- aicom-team-created registration must consume this exact side/owner/token entry, so a broad client
	//--- bus replay cannot mint a new transport receipt for an unrelated or already-existing team.
	_refundToken = "aicom-dispatch|" + str (round (diag_tickTime * 1000)) + "|" + str (round (random 1000000000));
	...
	_refundPending = _refundPendingLive + [[_sideID, owner _hcUnit, time, _refundToken]];
	missionNamespace setVariable ["WFBE_AICOM_TEAM_REFUND_PENDING", _refundPending];
	[_hcUnit, "HandleSpecial", ['delegate-aicom-team', _sideID, _template, _spawnPos, _w7SkillSend, _pick, _padClass, _foundType, _isJetTeam, _runwayDir, _refundToken]] Call WFBE_CO_FNC_SendToClient;
