...
if (_rlIdx < 0) exitWith {
	...
	diag_log Format ["CONSTRUCTION|v1|reject|reason=unknown-structure-type|script=SmallSite|type=%1|pos=%2", _type, _position];
};
...
//--- MULTI-INSTANCE structure types this script builds (Barracks/CommandCenter/Aircraft/ServicePoint -
//--- Light/Heavy have the SmallSite-sibling of this block in Construction_MediumSite.sqf). Kept as a
//--- SEPARATE codeblock (not folded into _releasePending) so the existing CBRadar/AARadar/Bank release
//--- paths - including their diag_log call count - are completely unchanged. Removes exactly ONE
//--- reservation (the oldest) from the RequestStructure.sqf pending array; see that file for the
//--- accept-time reserve. Called from every abort exitWith below AND once on the success tail.
...
_releaseCapPending = {
	...
	if ((missionNamespace getVariable ["WFBE_C_STRUCTURES_CAP_SERVER", 1]) > 0 && {_rlType in ["Barracks","CommandCenter","Aircraft","ServicePoint"]}) then {
...
if((missionNamespace getVariable [Format["WFBE_AUTOWALL_%1", _side], true]) && !(_rlType in ["AARadar","CBRadar"]))then{ //--- wiki-wins: per-side toggle (was the global isAutoWallConstructingEnabled, shared across sides)
	...
	//--- A structure type with no WFBE_NEURODEF_<type>_WALLS template leaves _wallTpl
...
if (!isNull _site) then {
	...
	{ if ([_x, false] Call WFBE_CO_FNC_IsRealPlayer && {alive _x} && {side _x == _bAttrSide} && {(_x distance _bAttrPos) < _bDist}) then {_bNear = _x; _bDist = _x distance _bAttrPos} } forEach playableUnits;
