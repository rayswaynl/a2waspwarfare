...
while {!WFBE_GameOver && {(missionNamespace getVariable [_clOwnerKey, _clOwnerSeq]) == _clOwnerSeq}} do {
	...
	if(_west > 0 || _east > 0 || _resistance > 0) then {
		...
		if(_captured) then {
			...
			_capSide = _newSide;
			{ if ([_x, false] Call WFBE_CO_FNC_IsRealPlayer && {alive _x} && {side _x == _capSide}) then {_capUid = getPlayerUID _x; if (_capUid != "") then {[_capUid, WFBE_STAT_CAPTURES_TOWN, 1] call WFBE_SE_FNC_RecordStat}} } forEach _objects;
			...
			if ((missionNamespace getVariable ["WFBE_C_STATLOG", 0]) == 1) then {
				...
				{if (!isNull _x && {alive _x} && {side _x == _wsSideCap} && {_x isKindOf "Man"}) then {if ([_x, false] Call WFBE_CO_FNC_IsRealPlayer) then {_wsPn = _wsPn + 1; if (_wsWho == "") then {_wsWho = name _x} else {_wsWho = _wsWho + "," + name _x}} else {_wsAn = _wsAn + 1}}} forEach _objects;
