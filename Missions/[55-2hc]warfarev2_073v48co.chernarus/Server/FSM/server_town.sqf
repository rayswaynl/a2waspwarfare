...
while {!WFBE_GameOver && {(missionNamespace getVariable [_clOwnerKey, _clOwnerSeq]) == _clOwnerSeq}} do {
	...
	for "_i" from 0 to ((count towns) - 1) step 1 do
	{
		...
		_maxSupplyValue = _location getVariable ["maxSupplyValue", 30];
				...
				//--- Nil-safe sideID (parity with server_town_camp N9 / cmdcon44-d SV reads). A plain
				//--- 1-arg getVariable on an unset sideID yields Undefined -> GetSideFromID default
				//--- sideLogic and poisons present-side / dominion / flip bookkeeping for that scan.
	...
	if(_west > 0 || _east > 0 || _resistance > 0) then {
		...
		if(_captured) then {
			...
			//--- BUGHUNT 2026-07-30: (1) reset each bulk-flipped camp supplyValue to town starting SV —
			//--- SetCampsToSide always did this, but F8 LEGACY_SKIP (default 1) suppresses that path and the
			//--- per-camp block never ported the SV heal, so new-owner camps could re-flip almost immediately.
			//--- (2) CampCaptured 5th arg _is_bulk=true skips client camp bounty/score (design: no bulk credit).
