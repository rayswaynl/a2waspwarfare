		...
		if (_drop) then {
			...
			if (!_boatHasPlayer && !_staticHasPlayer) then {
				...
				if (!isNull _eStatic) then {
					if (alive _eStatic) then { {["usv-static-unit", _x, ""] Call WFBE_CO_FNC_LogVehDelete; deleteVehicle _x} forEach (crew _eStatic); };
					["usv-static-hull", _eStatic, ""] Call WFBE_CO_FNC_LogVehDelete; deleteVehicle _eStatic;
				};
				if (!isNull _eBoat) then {
					if (alive _eBoat) then { {["usv-boat-unit", _x, ""] Call WFBE_CO_FNC_LogVehDelete; deleteVehicle _x} forEach (crew _eBoat); };
...
if (({isPlayer _x} count (units _eGrp)) == 0) then { deleteGroup _eGrp; };
			...
			if (_gateActive && {count _route > 0} && {!isNull _eBoat} && {alive _eBoat}) then {
				...
				//--- r77: null/deleted route GameLogic would throw on getPos and kill the whole maintain loop.
	...
	if (_gateActive && {count _flotilla < _count} && {count _route > 0}) then {
		...
		if (_nextClass != "") then {
			...
			//--- r77: never spawn a boat on a dry/null route logic (mission.sqm WP on shoreline or deleted).
			//--- Scan the circuit for a surfaceIsWater anchor; skip this tick if none found.
