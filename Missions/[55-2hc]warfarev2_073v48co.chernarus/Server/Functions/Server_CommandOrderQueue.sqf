...
*/
private ["_logic","_queue","_keep","_entry","_team","_type","_target","_issuer","_seq","_queuedAt","_last","_cool","_drop","_sides","_side","_cmd"];
_sides = [west, east];
while {true} do {
	{
		_side = _x;
		...
		if (!isNull _logic) then {
			...
			_cool = missionNamespace getVariable ["WFBE_C_AICOM_DIRECT_COOLDOWN", 1.5]; if (typeName _cool != "SCALAR" || {_cool < 0}) then {_cool = 1.5};
			{
				...
				if (typeName _entry != "ARRAY" || {count _entry < 6}) then {_drop = "malformed"} else {
					_team = _entry select 0; _type = _entry select 1; _target = _entry select 2; _issuer = _entry select 3; _seq = _entry select 4; _queuedAt = _entry select 5;
