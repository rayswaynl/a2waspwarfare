...
_vehicle allowCrewInImmobile true;
	//--- Tank War: AI helicopter combat behaviour. WASP issues NO flight orders to AI air; give each AI-crewed heli a
	//--- contour flight altitude + a SAD hunt on the nearest enemy town so gunships actually engage armor. Event-gated, server-side.
	if (IS_tank_war_event && {_unitType isKindOf "Air"} && {!isPlayer (leader _team)}) then {
		[_vehicle, _team, _sideID, _unitType] spawn {
			...
			_alt = if (_typ isKindOf "Plane") then {400 + random 150} else {110 + random 70};
			...
			if (!isNull _tgt) then {
				[_grp, true, [[(getPos _tgt), "SAD", 500, 30, "", []], [(getPos _tgt), "CYCLE", 60, 30, "", []]]] Call AIWPAdd;
			};
		};
	};
	...
	if (!isNil "_aiRally" && {count _aiRally >= 2} && {!isPlayer (leader _team)} && {!isNull (driver _vehicle)} && {!(_unitType isKindOf "Air")}) then {
		(driver _vehicle) commandMove _aiRally; //--- ground-only: never road-snap a helicopter (would fight the CAP / fly it into terrain)
	};
