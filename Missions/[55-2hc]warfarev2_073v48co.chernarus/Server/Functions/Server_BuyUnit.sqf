...
if (!(alive _building)||{isNull _team}||{[leader _team] Call WFBE_CO_FNC_IsRealPlayer}) exitWith {
	...
	if ([leader _team] Call WFBE_CO_FNC_IsRealPlayer) then {["INFORMATION", Format ["Server_BuyUnit.sqf: Unit [%1] has been canceled, player [%2] has replace the ai.", _unitType, name (leader _team)]] Call WFBE_CO_FNC_LogContent};
};
...
while {(count _queu == 0) || {!((_id select 0) in [_queu select 0])}} do {  //--- queue-fix (Generic-error crash): the factory FIFO is SHARED - player buys push STRING tokens (Client_BuildUnit.sqf:198) while AI buys push NUMBER tokens (AI_Commander_Produce.sqf:502 -> _id select 0). A raw != on a mixed-type head threw "Generic error" on A2-OA (empty-guard via lazy || only stopped [] select 0). Mirror the proven type-safe player idiom (Client_BuildUnit.sqf:278): (x in [head]) is false on type-mismatch, never throws.
	...
	if (!(alive _building)||(isNull _building)||{isNull _team}||{[leader _team] Call WFBE_CO_FNC_IsRealPlayer}) exitWith {
		if (!isNull _team) then {
			_team setVariable ["wfbe_queue", (_team getVariable "wfbe_queue") - [_id]];
		};
		_queu = _building getVariable "queu";
...
if (_refunded || {!(alive _building)} || {isNull _team} || {[leader _team] Call WFBE_CO_FNC_IsRealPlayer}) exitWith {
	if (!isNull _team) then {
		_team setVariable ["wfbe_queue", (_team getVariable "wfbe_queue") - [_id]];
	};
	...
	if (_refunded && {alive _building} && {!([leader _team] Call WFBE_CO_FNC_IsRealPlayer)}) then {["INFORMATION", Format ["Server_BuyUnit.sqf: Unit [%1] build skipped - treasury already refunded earlier this cycle (abort condition reverted during wait; no double-value).", _unitType]] Call WFBE_CO_FNC_LogContent};
};
...
if (_unitType isKindOf "Man") then {
	...
	if (!isNil "_aiRally" && {count _aiRally >= 2} && {![leader _team] Call WFBE_CO_FNC_IsRealPlayer} && {!isNull _soldier}) then {
...
_vehicle allowCrewInImmobile true;
	...
	private "_aiRally";
	_aiRally = _building getVariable "wfbe_aicom_factory_rally";
	if (!isNil "_aiRally" && {count _aiRally >= 2} && {![leader _team] Call WFBE_CO_FNC_IsRealPlayer} && {!isNull (driver _vehicle)}) then {
