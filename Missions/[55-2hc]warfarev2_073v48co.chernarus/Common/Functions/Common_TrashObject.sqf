...
if !(isNull _object) then {
	...
	//--- null-guard after sleep: concurrent GC / killed-EH may delete the body while we wait (native getPos/distance crash 014EFCF4 class).
	...
	if (_isMan && {(missionNamespace getVariable ["WFBE_C_UNITS_BODIES_PROX", 0]) > 0}) then {
		...
		_held = 0;
		while {_held < _delay && {!isNull _object} && {({isPlayer _x && {alive _x} && {_x distance _object < _prox}} count playableUnits) > 0}} do {
