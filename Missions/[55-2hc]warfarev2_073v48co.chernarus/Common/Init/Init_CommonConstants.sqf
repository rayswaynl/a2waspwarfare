...
//--- Artillery.
	...
	if (isNil "WFBE_C_BASE_EGRESS_MAP_BOUNDS") then {WFBE_C_BASE_EGRESS_MAP_BOUNDS = if (worldName == "Takistan") then {1} else {0}}; //--- Guard default ON on Takistan, OFF elsewhere (Zargabad pre-set above already uses map bounds): 1 = use the Init_Boundaries worldName size for random-start egress checks.
