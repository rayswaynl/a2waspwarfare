...
//--- AI.
	...
	if (isNil "WFBE_C_AI_COMMANDER_WILDCARD_COST") then {WFBE_C_AI_COMMANDER_WILDCARD_COST = 0};       //--- funds the AI commander pays per wildcard draw. 0 = free/legacy (feature inert); >0 = purchase-gated (per-side afford check + cooldown). Intended live value 8000 (Ray 2026-07-07). claude-gaming.
	...
	if (isNil "WFBE_C_AICOM_GRUDGE")                then {WFBE_C_AICOM_GRUDGE                = 0};     //--- master switch. 0 = inert (no stamping, no scorer bonus, no sub-options). 1 = armed.
	...
	if (isNil "WFBE_C_AICOM_PHASE_ENABLE") then {WFBE_C_AICOM_PHASE_ENABLE = 0}; //--- AI-BEHAVIOR-LOOP-DESIGN.md sec1: per-team phase-state var wfbe_aicom_phase (MARCH/CAMP_SWEEP/CENTER_PUSH/CONSOLIDATE/NEXT_TARGET), stamped HC-side in Common_RunCommanderTeam.sqf at 5 existing choke points. 0 = off: var never written/read anywhere, zero behavior change.
	if (isNil "WFBE_C_AICOM_DWELL_ENABLE") then {WFBE_C_AICOM_DWELL_ENABLE = 0}; //--- AI-BEHAVIOR-LOOP-DESIGN.md sec2: cumulative per-team-per-town dwell clock (wfbe_aicom_dwell_town0), surviving a RELEASE->repick-same-town cycle, consumed as a 4th sibling abandon trigger in AI_Commander_AssignTowns.sqf. 0 = off: var never stamped, existing 3-trigger abandon ladder untouched.
...
//--- Default 0 = byte-identical to HEAD (the PV loop below never spawns).
	if (isNil "WFBE_C_HC_CIV_RESLOT") then {WFBE_C_HC_CIV_RESLOT = 0};
...
if (isNil "WFBE_C_RADIUSHOLD_ENABLE") then {WFBE_C_RADIUSHOLD_ENABLE = 0}; //--- fable/radius-hold-primitive (GR-2026-07-08a): master gate for the shared radius-presence-hold primitive (Common_RadiusHold.sqf). DARK by default - 0 refuses every registration and the dispatcher never spawns.
...
if (isNil "WFBE_C_NAVALHVT_BUBBLE_ENABLE") then {WFBE_C_NAVALHVT_BUBBLE_ENABLE = 0}; //--- fable/radius-hold-primitive: naval-HVT carrier bubble consumer master gate (Init_NavalHVT.sqf). DARK by default - camps-on-deck stays the live path until armed.
...
if (isNil "WFBE_C_ZG_KOTH_ENABLE") then {WFBE_C_ZG_KOTH_ENABLE = 0}; //--- fable/radius-hold-primitive consumer (GR-2026-07-08a, stacked on PR #916): Zargabad KotH city-core hold master flag. DARK by default; map-gated to Zargabad regardless (Init_ZgKoth.sqf).
