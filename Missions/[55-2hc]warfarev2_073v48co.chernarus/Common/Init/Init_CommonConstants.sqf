...
if (isNil "WFBE_C_SERVER_READY_DELAY") then {WFBE_C_SERVER_READY_DELAY = 90}; //--- B50: seconds after serverInitFull before the server broadcasts WFBE_MissionReady (lets HC reslot + AICOM bootstrap settle). Joining clients hold on the loading screen until then - avoids the mid-bootstrap JIP stall/black-screen. //--- B49: relaxed START gate (m). A disengaged team detours to service if NO enemy within this (was the full SAFE_DIST=600, which blocked every grinding team so the feature never fired). The hard en-route abort still uses SAFE_DIST; COMBAT teams are still never pulled out.
