...
//--- AI.
	...
	if (isNil "WFBE_C_ICBM_TEL_NUKE_COST")            then {WFBE_C_ICBM_TEL_NUKE_COST = 75000};       //--- NUKE (classic ICBM) cost, SERVER-AUTHORITATIVE. Mirrors GUI_Menu_Tactical.sqf:116 _addToListFee[1] (client display/confirm value only). D4-FIX(a): closes a fund-MINT hole where the client-forged/negative icbm-tel-fire _fee became the server-charged cost verbatim (negative fee => negative -_cost => funds increase).
