...
//--- AI.
	...
	if (isNil "WFBE_C_AICOM_RELIEF_HOLD") then {WFBE_C_AICOM_RELIEF_HOLD = 240};  //--- s.
...
//--- Units.
	...
	if (isNil "WFBE_C_VEHICLE_TINTS") then {WFBE_C_VEHICLE_TINTS = 0}; // Vehicle faction body TINTS (cheap one-shot setObjectTexture colour strings in Common_AddVehicleTexture.sqf). Decoupled from WFBE_C_VEHICLE_MARKINGS so the tints can be LIVE while the expensive #lightpoint markings stay OFF. 1 enabled, 0 disabled. DEFAULT 0 (opt-in): the B66 side-resolve bug meant the tints were silently INERT in prod (resolved from a crewless hull = civilian -> no match); B67 fixed the resolution (now reads the authoritative _createSide), so enabling this would for the FIRST TIME repaint selections 0+1 (often the whole hull) with a flat procedural colour on EVERY vehicle (WEST near-black / EAST olive / GUER tan) - unverified in-engine and possibly ugly. Flip to 1 only after an in-engine cosmetic check. Matches WFBE_C_VEHICLE_DECALS (also opt-in).
	if (isNil "WFBE_C_VEHICLE_DECALS") then {WFBE_C_VEHICLE_DECALS = 0}; // Vehicle faction DECALS: stock A2/OA flag .paa applied to a single secondary texture selection via setObjectTexture (no new art). Appended to wfbe_pending_texture on the same JIP-safe path as the tints/salvage blocks in Common_AddVehicleTexture.sqf. Decoupled from WFBE_C_VEHICLE_MARKINGS (#lightpoints) and WFBE_C_VEHICLE_TINTS (procedural body colours): all three can be toggled independently for A-B. 1 enabled, 0 disabled. DEFAULT 0 (opt-in, unverified in-engine - flag .paa on a body selection NEEDS-IN-ENGINE-VERIFY per class).
