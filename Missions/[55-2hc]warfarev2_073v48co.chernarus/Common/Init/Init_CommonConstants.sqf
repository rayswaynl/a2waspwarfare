...
//--- AI.
	...
	if (isNil "WFBE_C_AICOM_FUNDS_SINK_THRESHOLD")  then {WFBE_C_AICOM_FUNDS_SINK_THRESHOLD  = 150000};  //--- funds: only drain a commander's hoard ABOVE this. Ray 2026-07-27: 1000000 -> 150000 (owner). The 1M figure was picked against the 1.5M WEALTH_CAP, but a real match STARTS at 30000 funds and an AI side never approached 1M, so the sink could never fire - the hoard symptom it exists to cure went untreated. 150000 = 5x a normal starting treasury: comfortably above what a fully-fielded 16-team roster costs, so normal play is untouched, but a genuine hoard now bleeds into push waves. Rollback: 1000000.
