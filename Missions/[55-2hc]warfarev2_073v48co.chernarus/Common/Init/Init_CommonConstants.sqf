...
//--- AI.
	...
	if (isNil "WFBE_C_RESPAWNST_COOLDOWN")            then {WFBE_C_RESPAWNST_COOLDOWN = 30};           //--- s SERVER-SIDE per-side cooldown on "RespawnST" (force-kills every AI supply truck for a side). Defence in depth under WFBE_C_SEC_HARDENING alongside the requester-identity check; the client's own 5s ctrlEnable throttle is not authoritative. 0 = disable.
