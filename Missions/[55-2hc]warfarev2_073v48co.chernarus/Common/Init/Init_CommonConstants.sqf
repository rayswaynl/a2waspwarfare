...
//--- See docs/design/v2/aicom-v2-commander-town-ledger.md for the full spec.
	...
	if (isNil "AICOMV2_CTL_COMP_LINK") then {AICOMV2_CTL_COMP_LINK = 0}; //--- EXPERIMENT (fable/ctl-comp-link, LAB-ONLY): 0=off/byte-identical. Composition-by-str: skews WHAT the DEFENDER garrison spawns (Server_GetTownGroupsDefender.sqf) by the CTL ledger strength wfbe_ctl_str - thin (str<=0.5) drops heavy-role entries to infantry-only, invested (str>=1.25) doubles heavy-role weight. Independent of AICOMV2_CTL_GARRISON_LINK (composable). Requires AICOMV2_LANE_CMD_TOWN_LEDGER>0. Emits CTLSTAT|v1|DEF|COMP.
