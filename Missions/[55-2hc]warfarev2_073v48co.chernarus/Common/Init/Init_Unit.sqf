...
//--- headless client `player` IS the HC's own playable unit and IS local, so the guard below does NOT
...
//--- waitUntil re-evaluates its condition every frame on the HC's single saturated core. Exit before
//--- any initialization wait; the HC needs none of the client-side init below (actions/markers/UI).
//--- Nil-guarded: real clients + server are byte-identical.
