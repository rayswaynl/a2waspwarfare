...
_createBorder = {
	...
	for "_i" from 1 to _total do {
		...
		//--- FAIL-CLEAN r44: null create must not setposasl/setdir (script error + sparse border)
		if (isNull _a) then {
		...
		if (count _params > 0) then {
			...
			if (typeof _preview != _itemclass_preview) then {
				...
				if (isNull _preview) then {
					["WARNING", Format ["coin_interface.sqf: preview create failed class %1.", _itemclass_preview]] Call WFBE_CO_FNC_LogContent;
					_logic setVariable ["BIS_COIN_preview",nil];
					_logic setVariable ["BIS_COIN_params",[]];
				} else {
