...
_salvagerRange = missionNamespace getVariable ["WFBE_C_UNITS_SALVAGER_SCAVENGE_RANGE", 50];
_percentage = missionNamespace getVariable ["WFBE_C_UNITS_SALVAGER_SCAVENGE_RATIO", 30];
...
_overAllCost = 0;
{
//--- r53: skip concurrent-delete holes; bare deleteVehicle on non-local wrecks silently no-ops
//--- (HC-owned hulls) while still paying the player. Mirror Client/FSM/updatesalvage.sqf locality.
