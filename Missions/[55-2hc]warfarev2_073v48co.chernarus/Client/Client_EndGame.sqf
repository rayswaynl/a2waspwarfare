...
if (missionNamespace getVariable ["WFBE_C_VICTORY_OUTRO_FX", true]) then {
	[_side, getPos (_blist select 0)] Spawn {
		...
		//--- FAIL-CLEAN r45: flare create null must not setParticle*/setDropInterval
		if (isNull _flare) then {
			["WARNING", Format ["Client_EndGame.sqf: victory flare create failed at %1.", _fxPos]] Call WFBE_CO_FNC_LogContent;
		} else {
		...
		//--- FAIL-CLEAN r45: light create null must not setLight*/pulse loop
		if (isNull _light) then {
			["WARNING", Format ["Client_EndGame.sqf: victory light create failed at %1.", _fxPos]] Call WFBE_CO_FNC_LogContent;
		} else {
		...
		if !(isNull _flare) then {_flare setDropInterval 0};
